#!/usr/bin/env python3
"""Source-bound v37 build, with an optional small, archive-free core build."""
...
O=H.parent/'GTF-I-v36-finite-alphabet-memory'
E=H/'evidence'
BASE='d6b89112389b97a16a287fa32f3f75e4c2d72e0a'
PREVIOUS='1a08578710d55a2379cea56c43feb5eca56340a2'
OLD_NAMES=['GTF-I-v36-finite-alphabet-memory','GTF-I-v35-sharp-memory','GTF-I-v34-compatible-dynamics','GTF-I-v33-compatible-lifts','GTF-I-v32-causal-width','GTF-I-v31-compatible-memory','GTF-I-v30-streaming-geometry',
...
def main()->None:
    ...
    names=['wrong-centroid','missing-smoothing-scale','reversed-loss','fake-state-gap',
           'wrong-hessian-sign','unpaid-noise','early-ready','wrong-binary-tv']
    controls=[]
    for mode in [[],['-O']]:
        for name in names:
    ...
    with tempfile.TemporaryDirectory(prefix='gtf37-latex-') as td:
        work=Path(td)
    ...
    must=['thm:main','def:machine','lem:terminal','prop:separate','lem:centroid-entropy',
          'lem:entropy-gap','lem:sparse-root','thm:occupation','lem:net-upper',
          'cor:actual-error','thm:rational','prop:qubit','thm:compiler','cor:coin-rate']
    ...
    receipt={'schema':'gtf37.build/1','core_only':args.core_only,
    ...
    (root/'GENERAL_THETA_FOUNDATIONS_I_V37_REVIEW_READY.md').write_text(
      '# General Theta Foundations I — Revision 37\n\n**Entropy Dissipation and Sharp Finite-Alphabet Memory**\n\n'
      f'Native source commit: `{receipt["source_commit"]}`. Controlling r21: `{BASE}`.\n\n'
      f'[English article ({pages} pages)](papers/{H.name}/paper.pdf) · [Native LaTeX](papers/{H.name}/main.tex) · [Response to r21](papers/{H.name}/RESPONSE_TO_REFEREE.md)\n\n'
      ...
      f'[Unchanged v36 article](papers/{H.name}/supporting-results.pdf) · [Mathematical archive](papers/{H.name}/complete-manuscript.pdf) · [Development archive](papers/{H.name}/complete-development.pdf)\n\n'
      'Matched Theta(N^((d-1)/2)) for the finite spherical spectral-gap class, eliminating the logarithmic cardinality gap; quadratic entropy cost of conditional-centroid compression; gap-weighted occupation; Theta(N) classical labels and log2 N+O(1) label bits for the five rational qubit commands; fixed-duration padded finite-coin implementation.\n\n'
      'The spherical gap is a full L2 assumption and is not numerically evaluated for the explicit gates. Constants and finite integer optima are not sharp. Atomic and padded nonuniform label counts are not uniform computational space. The quantum operations and exact upper construction are inherited. All predecessor results remain unchanged; independent analytic pipeline closure and priority certification are not inferred.\n')
