#!/usr/bin/env python3
"""Run inherited author suites, v22 exact diagnostics, and the complete build."""
...
def main():
    ...
    report={'version':22,'python':platform.python_version(),
            'submission_basis':'7c44bdccc91667c583b5d5cbcff3f8d9160a57d6',
            'controlling_review':'36910a7c6fd08e5ad2e8e10db7c2d8c71c463de8',
    ...
    for version in (*range(10,16),17,18,19,20,21,22):
    ...
    report['inherited_assertions_total']=sum(x['assertions'] for x in report['suites'] if x['version']<22)
    optimized=out/'V22_OPTIMIZED_RERUN.json'
    run([sys.executable,'-O','tests/verify_v22.py',str(optimized)],out/'v22-optimized-stdout.txt')
    if optimized.read_bytes()!=(out/'V22_AUTHOR_RERUN.json').read_bytes():
        raise ValueError('Optimized v22 diagnostic receipt differs')
    report['v22_optimized_identical_receipt']=True
