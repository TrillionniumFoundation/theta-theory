#!/usr/bin/env python3
"""Source-bound v36 build, with an optional small, archive-free core build."""
...
O=H.parent/'GTF-I-v35-sharp-memory'
E=H/'evidence'
BASE='6da551c5b804b683240d5f5631e1c4f5689e21bd'
PREVIOUS='aabc0731a0c913f7b9e75e6efc08f7af665bdf23'
OLD_NAMES=['GTF-I-v35-sharp-memory','GTF-I-v34-compatible-dynamics','GTF-I-v33-compatible-lifts','GTF-I-v32-causal-width','GTF-I-v31-compatible-memory','GTF-I-v30-streaming-geometry',
...
def main()->None:
    ...
    names=['dependent-word','product-sufficiency','finite-spectrum-is-gap','haar-is-one-letter',
           'free-sampler-clock','probabilities-are-languages','missing-conjugation','unpriced-antipode',
           'wrong-terminal-tv','overlapping-packets']
    controls=[]
    for mode in [[],['-O']]:
        for name in names:
    ...
    with tempfile.TemporaryDirectory(prefix='gtf36-latex-') as td:
        work=Path(td)
    ...
    must=['def:machine','prop:separate','lem:terminal','lem:packet','thm:packet-budget',
          'lem:mixing','lem:finite-distortion','thm:spectral','lem:net-upper','thm:rational',
          'prop:qubit','thm:monomial','prop:no-character','thm:compiler','cor:coin-rate']
    ...
    receipt={'schema':'gtf36.build/1','core_only':args.core_only,
    ...
    (root/'GENERAL_THETA_FOUNDATIONS_I_V36_REVIEW_READY.md').write_text(
      '# General Theta Foundations I — Revision 36\n\n**Finite Alphabets, Spectral Gaps, and Hidden Memory**\n\n'
      f'Native source commit: `{receipt["source_commit"]}`. Controlling r20: `{BASE}`.\n\n'
      f'[English article ({pages} pages)](papers/{H.name}/paper.pdf) · [Native LaTeX](papers/{H.name}/main.tex) · [Response to r20](papers/{H.name}/RESPONSE_TO_REFEREE.md)\n\n'
      ...
      f'[Unchanged v35 article](papers/{H.name}/supporting-results.pdf) · [Mathematical archive](papers/{H.name}/complete-manuscript.pdf) · [Development archive](papers/{H.name}/complete-development.pdf)\n\n'
      'Full-word packet correction; finite spectral-gap alphabets with a logarithmic mixing cost; five explicit rational noncommuting gates with width between c N/log(N+1) and C N; exact qubit numerical behavior; matched cube-root resonant monomial class; charged sparse-row finite-coin synthesis.\n\n'
      'The spectral-gap logarithmic order gap is retained. The qubit comparison is numerical accuracy, not strict-cutpoint language equivalence. Atomic and self-paced finite-coin models remain distinct. All predecessor proofs are preserved; independent analytic pipeline closure and priority certification are not inferred.\n')
