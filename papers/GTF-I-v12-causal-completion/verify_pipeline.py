...
for c in g['components']:
    ...
    need(c['full_historical_target_closed_by_v12'] is False,'Unsupported historical closure declaration')
    for k in c['inherited_theorem_labels']:need(k in labels,'Absent inherited label '+k)
    for edge in c['new_edges']+c['v12_edges']:
...
print(json.dumps({'status':'passed','components':11,'retained_v11_microscopic_edges':2,'new_v12_operator_edges':3,'review_blob_checked_from_git':git.returncode==0,'primary_a2_dependency':False,'scope':'Source identity, schema and label-existence checks; not theorem verification.'},indent=2))
