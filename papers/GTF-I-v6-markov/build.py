#!/usr/bin/env python3
"""Hash-bound build of the full and same-source principal sixth editions."""
...
def main()->None:
    ...
    for path,digest in manifest['input_sha256'].items():
        require(not Path(path).is_absolute() and '..' not in Path(path).parts,'Unsafe manifest path')
    ...
    for name,record in history['active_a2']['files'].items():
        pp=ROOT/record['copied_path'];data=pp.read_bytes()
        require(sha(pp)==record['sha256'],'Current A2 snapshot changed: '+name)
        blob=hashlib.sha1(b'blob '+str(len(data)).encode()+b'\0'+data).hexdigest()
        require(blob==record['git_blob'],'Current A2 blob changed: '+name)
    inherited=json.loads((REPO/'papers/GTF-I-v5-intrinsic/SOURCE_MANIFEST.json').read_text())
    for path,digest in inherited['input_sha256'].items():
        require(sha(REPO/path)==digest,'Inherited v5 input changed: '+path)
    fullrefs=(REPO/'papers/GTF-I-v5-intrinsic/references.tex').read_text().replace(
    ...
    core_paths=[PREFIX+'/'+p for p in ['introduction.tex','markov-model.tex','renewal-suffix.tex',
        'markov-pressure.tex','critical-renewal.tex','nonregenerative-filter.tex','operational.tex']]
    core_paths+=['papers/GTF-I-v4/posterior-orbits.tex']
    ...
    old_paths += ['papers/GTF-I-v5-intrinsic/'+p for p in ['introduction.tex','intrinsic-model.tex',
    ...
    a2body=(ROOT/'a2-finite-budget.tex').read_text()
    a2labels=set(re.findall(r'\\label\{([^}]+)\}',a2body))
    ...
    if source and (REPO/'.git').exists():
        ...
        allowed=[PREFIX+'/', '.github/revision-inputs/gtf-i-v6-markov/',
                 '.github/workflows/general-theta-foundations-i-v6-markov.yml',
                 'GTF_I_V6_MARKOV_INDEX.md']
    ...
    legacy={}
    for name,path in [('V5','papers/GTF-I-v5-intrinsic/verify.py'),('V4','papers/GTF-I-v4/verify.py'),('V3','papers/GTF-I-v3/verify.py'),
    ...
    outputs={};maps={}
    with tempfile.TemporaryDirectory(prefix='gtf-v6-markov-') as tmp:
        base=Path(tmp)
        ...
        for job,filename in [('main','paper.pdf'),('principal','principal-paper.pdf')]:
            ...
            must=oldlabels|corelabels|a2labels if job=='main' else corelabels
            ...
            outputs[job]={'file':filename,'pages':pages,'sha256':sha(ROOT/filename),
                          ...
                          'new_statement_locations':{k:v for k,v in lm.items() if k.startswith(('thm:v6','lem:v6','cor:v6','prop:v6','ex:v6'))}}
    ...
    receipt={'status':'passed','edition':'GTF I sixth Markov revision','source_commit':source,
