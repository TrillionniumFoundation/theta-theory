#!/usr/bin/env python3
"""Source-pinned A2 v149 assembly, exact regressions and native PDF publication.
...
BASE=HERE.parent/'v148';ROOT=HERE.parents[3]
PREFIX='papers/A2-v17-boundary-information-coarsening/article/v149'
BRANCH='revision/a2-v149-intrinsic-pencil-strata-deformations-2026-09-24'
REVIEW='dc3a951e26de17e0e69403b1d8cfdfb5e28bc306'
REPORT='reviews/a2-v148-independent-harsh-top4-2026-09-24/REFEREE_REPORT.md'
REPORT_BLOB='b508fc349213e461d6745a2c1330245fedf4b48a'
GENERATED={'SOURCE_LOCK_V149.json','NONDELETION_V149.json','PROVENANCE_MANIFEST_V149.json'}
...
def entry(commit=None):
 lead=(f'Native PDFs and 26 successful regression scripts are bound to source commit `{commit}`.' if commit else 'The revision source is published; native verification is recorded only in the v149 build receipt.')
 put(ROOT/'CURRENT_REVIEW_ENTRY.md',f'''# A2 referee entry — revision 149

{lead} Regression checks do not certify universal proofs, historical priority, or journal acceptance.

Revision branch: `{BRANCH}`. Controlling review tip: `{REVIEW}`.
Report addressed: [{REPORT}]({REPORT}).

**Principal article:** [geometry.pdf]({PREFIX}/geometry.pdf) · [LaTeX]({PREFIX}/geometry.tex).

[Response to v148]({PREFIX}/RESPONSE_TO_V148_REPORT.md) · [Reading guide]({PREFIX}/README.md) · [Issue matrix]({PREFIX}/ISSUE_MATRIX_V149.json).
...
[Build receipt]({PREFIX}/evidence/BUILD_RECEIPT_V149.json) · [Source lock]({PREFIX}/SOURCE_LOCK_V149.json) · [Preservation]({PREFIX}/NONDELETION_V149.json) · [Literature audit]({PREFIX}/LITERATURE_AUDIT_V149.md) · [Proof-scope audit]({PREFIX}/PROOF_SCOPE_AUDIT_V149.md).

[Applications]({PREFIX}/applications.pdf) are separate; the [historical archive]({PREFIX}/archive-v144.pdf) remains non-submitted. Only geometry.pdf is the principal article.
...
def assemble():
 ...
 hist=HERE/'history/v148-root';hist.mkdir(parents=True,exist_ok=True);records={}
 for p in BASE.rglob('*'):
 ...
 dump(HERE/'INHERITED_SOURCE_V149.json',{'predecessor_review_commit':REVIEW,'files':records})
 report=ROOT/REPORT
 if report.exists():
  ...
  dest=HERE/'review_inputs/v148_REFEREE_REPORT.md';dest.parent.mkdir(exist_ok=True);shutil.copy2(report,dest)
...
def build():
 ...
 scripts=['checks/'+n+'.py' for n in names]+['check_v145.py','check_v146.py','check_local_v146.py','check_v147.py','check_v148.py','check_v149.py'];records=[]
 for script in scripts:
  ...
  dump(evidence/'EXECUTED_CHECKS_V149.json',records)
...
def verify():
 ...
 records=json.loads((evidence/'EXECUTED_CHECKS_V149.json').read_text())
 checks['all_26_scripts_pass']=len(records)==26 and all(r['returncode']==0 and sha(HERE/r['script'])==r['sha256'] for r in records)
 newexact=json.loads((evidence/'REVISION149_EXACT.json').read_text());checks['new_exact_witnesses_pass']=newexact['ok'] is True
 inherited=json.loads((HERE/'INHERITED_SOURCE_V149.json').read_text())['files']
 checks['every_v148_source_preserved']=all((HERE/r['preserved_at']).is_file() and sha(HERE/r['preserved_at'])==r['sha256'] for r in inherited.values())
 checks['all_v148_parts_unchanged']=all(sha(HERE/p.relative_to(BASE))==sha(p) for p in (BASE/'parts').rglob('*.tex'))
 ...
 checks['all_v148_principal_math_blocks_retained']=not(Counter(oldblocks)-Counter(newblocks))
 checks['all_v148_principal_labels_retained']=set(labels(old))<=set(labels(main))
 checks['applications_only_metadata_changed']=(HERE/'applications.tex').read_text().replace('A2 revision 149','A2 revision 146')==(BASE/'applications.tex').read_text()
 checks['archive_driver_unchanged']=sha(HERE/'archive-v144.tex')==sha(BASE/'archive-v144.tex')
 required={'lem:universal-gcd','thm:determinant-stratum','prop:relative-first-relation','lem:primitive-pencil','thm:pencil-stack-equivalence','cor:pencil-specialization-stack','thm:transverse-pencil-directions','thm:fixed-support-grassmannians'}
 checks['all_eight_new_results_in_principal_article']=required<=set(labels(main))
 commit=os.environ.get('A2_SOURCE_COMMIT','LOCAL-PREFLIGHT-NOT-REMOTE')
 receipt={'revision':149,'source_commit':commit,'controlling_review_commit':REVIEW,'controlling_review_blob':REPORT_BLOB,'all_checks_pass':all(checks.values()),'checks':checks,'executed_script_count':len(records),'new_exact_check_count':newexact['check_count'],'executed_scripts':records,'pdfs':pdfs,'labels':{n:len(labels(t)) for n,t in texts.items()},'preserved_predecessor_source_files':len(inherited),'preserved_principal_math_blocks':len(oldblocks),'proof_certified_by_computation':False,'Ballico_1993_full_text_comparison_completed':False,'historical_priority_certified':False,'journal_acceptance_asserted':False}
 locator={}
 for lab,num,page in re.findall(r'\\newlabel\{([^}]+)\}\{\{([^}]+)\}\{([^}]+)\}',(HERE/'geometry.aux').read_text()):
  if lab in required:locator[lab]={'number':num,'page':page}
 dump(evidence/'THEOREM_LOCATOR_V149.json',locator)
 dump(evidence/'BUILD_RECEIPT_V149.json',receipt)
 dump(HERE/'NONDELETION_V149.json',{'source_commit':commit,'predecessor_review_commit':REVIEW,'all_source_bytes_preserved':checks['every_v148_source_preserved'],'all_principal_math_blocks_retained':checks['all_v148_principal_math_blocks_retained'],'all_v148_parts_unchanged':checks['all_v148_parts_unchanged'],'source_files':inherited,'preservation_is_not_proof_certification':True})
 dump(HERE/'PROVENANCE_MANIFEST_V149.json',{'source_commit':commit,'sha256':{str(p.relative_to(HERE)):sha(p) for p in HERE.rglob('*') if source(p,p.relative_to(HERE)) and p.name not in GENERATED}})
 dump(HERE/'SOURCE_LOCK_V149.json',{'revision':149,'source_commit':commit,'controlling_review_commit':REVIEW,'principal_manuscript':'geometry.pdf','separate_application':'applications.pdf','non_submitted_archive':'archive-v144.pdf','receipt':'evidence/BUILD_RECEIPT_V149.json','all_checks_pass':receipt['all_checks_pass'],'historical_priority_certified':False})
 print(json.dumps(receipt,indent=2),flush=True)
 if not receipt['all_checks_pass']:raise RuntimeError('v149 audit failed')
 entry(commit)
if __name__=='__main__':
 if len(sys.argv)!=2 or sys.argv[1] not in {'assemble','build','verify'}:raise SystemExit('Usage: revision_v149.py assemble|build|verify')
 globals()[sys.argv[1]]()
