...
The bridge probability decays exponentially with the number of flights.
Its exponent records linearized collision dynamics, whereas its conditional
endpoint law retains a nonlinear profile involving repeated returns to both
contacts.  We prove a relative boundary law on a fixed offset collar and
invert that profile without reflection symmetry, equal contacts, or supplied
curvatures.  A single positive-offset endpoint law of each contact type,
together with the gap, suffices.  The inverse first removes the unknown
flux amplitude by a four-density identity and then recovers the labelled
contact jets through explicit determinant-one blocks at every order.
For connected analytic boundaries, intrinsic channel matching recovers
complete obstacles and the Euclidean period lattice under stated marked
...
complete boundary images in the separate channel frames.  If the measured
channel graph contains two signature-rigid cycles with independent marked
deck displacements and a rooted signature-rigid spanning tree reaching
every obstacle orbit, the data determine the entire marked periodic table,
...
is Theorem~\ref{thm:v24-lattice-gram-recovery}.  Their composition is
...
pilot at the already selected final even flight number.  The new
...
sampling; Proposition~\ref{prop:v26-position-deficiency} proves a strict
finite-sample distinction on an explicit registered local family.
...
The reference-dominated representative has a uniform LAN expansion; the
original family is asymptotically equivalent to that representative on
compact local sets, and its finite local subexperiments have the stated
Gaussian limit.  Waiting counts identify the normal quotient to a level
...
De Simoi, Kaloshin and Leguil~\cite{DSKL} study marked-length determination
for analytic open dispersing billiards under stated symmetry and genericity
assumptions.  Finamore and Leguil~\cite{FinamoreLeguil} use an enriched
marked length spectrum and a CAT(0) construction for finite-horizon Sinai
billiards.  Our local relative law requires neither their spectral data nor
their global symmetry hypotheses.  Conversely, the intrinsic inverse here
assumes signed channel laws, onsets, marked deck labels, and signature-rigid
incidence structure.  These data are not asserted to follow from an ordinary
or enriched marked length spectrum.  Without a proved relation between
these observation maps, neither subsumption nor a direct generalization
is claimed.
...
and adds the same-experiment direct-position benchmark.  Part II proves the
boundary information results, distinguishes common domination from reference
absolute continuity, and compares the residual-time and count coarsenings.
