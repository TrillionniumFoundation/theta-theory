...
\(V\)-ruling cannot be exchanged with the other ruling.  The induced
isomorphism of the trivial \(\Pj(V)\)-bundles, after pullback by
\(\sigma\), defines a morphism
\(\Sigma_0(R)\to\PGL(V)\).  The Grassmannian \(\Sigma_0(R)\)
is connected and projective and has only constant global regular
functions, whereas \(\PGL(V)\) is affine.  This morphism is therefore
constant.  Denote its value by \(g\).
...
The scalar determinant equation of the reduced cone is transported
by \(\alpha\).  Division of the transported degree-sixteen ideal
by that equation consequently transports \((J_{R'})_{12}\) to
\((J_R)_{12}\).  The universal diagonal-block identity gives
\[
 \C\beta_{R,\lambda}
