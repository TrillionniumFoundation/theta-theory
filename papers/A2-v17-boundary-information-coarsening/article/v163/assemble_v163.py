#!/usr/bin/env python3
"""Assemble complete v163 manuscripts; preserve every v162 mathematical block.

The v158 helper functions supply compilation and embedded reference maps.
The mathematical input is the complete, byte-locked v162 master, not a
...
code=code.replace('from frontmatter_v158 import','from frontmatter_v163 import')
code=code.replace('revision 158},pdfauthor','revision 163},pdfauthor')
exec(compile(code,str(helper),'exec'))
PREV=HERE.parent/'v162'/'geometry.tex'
PREV_HASH='7ccaca8309f9de7da7c134e2c96cf2b82266e3bba1f7a1402f8ee5533b97058d'
REVIEW='7e9c057907cc4502858a54f32ab0c0cfa264680f'
BASE='299a77c6f77c4d5aec6e5737e1b1ba471ce5b7c8'
INPUTS=('embedded-comparison-v163.tex','complete-contact-fibre-v163.tex','products-and-strata-v163.tex','tangent-and-effective-v163.tex')
...
I_APPS=['The intrinsic polarization and a projective moduli completion','The intrinsic source and its apolar envelope','Universal property of the intrinsic envelope','The incidence centre from failure multiplication','Effective boundary groupoids and relative lifting obstructions','Algebraic envelope forms and invariant relative deformations','Tangent maps, linearizations, and effective transfer']
II_CORE=['Power coefficients and their twists','Quadratic sections of reciprocal normalization fibres','Universal power ideals','The reciprocal power graph and complete quadrics','Versal contact coordinates for regular symmetric pencils','Fixed-target comparison of contact graphs','Euclidean charts of the ambient Hilbert boundary','The complete first nonreduced contact fibre','Products and adjacency for the first nonreduced boundary']
II_APPS=['A non-equidimensional ambient Hilbert fibre','Multiple reduced incidence and its complete Hilbert boundary','Higher contact, thick tails, and singular multiplication graphs','Power graphs of every rank','Contacts and minimal indices of arbitrary pencils','Boundary contacts and spectral schemes','A projective incidence compactification for pencils','The simple corank-two incidence modification','A computed boundary of the pencil incidence space','Ordinary collision strata and their Hilbert fibres','Singularities of the universal power-zero schemes']
...
def main():
    ...
    if sha(PREV)!=PREV_HASH:raise RuntimeError('Complete v162 source hash mismatch')
    old=PREV.read_text();start=old.index('\\begin{abstract}');end=old.index('% BEGIN PRESERVED PART')
    (HERE/'PREVIOUS_MASTER_FRONTMATTER_V162.tex').write_text(old[start:end])
    for name in ('reconstruction','divisor-geometry'):
        ...
        (HERE/('PREVIOUS_'+name.upper().replace('-','_')+'_FRONTMATTER_V162.tex')).write_text(t[a:b])
    ...
    master=old[:start].replace('revision 162','revision 163')+'\\begin{abstract}\n'+MASTER_ABSTRACT+'\n\\end{abstract}\n\\maketitle\n'+MASTER_INTRO+'\n'+body
    ...
    preamble+='\\usepackage{alphalph}\n'
    i_body=i_body.replace('\\appendix\n','\\appendix\n\\renewcommand{\\thesection}{\\AlphAlph{\\value{section}}}\n',1)
    ii_body=ii_body.replace('\\appendix\n','\\appendix\n\\renewcommand{\\thesection}{\\AlphAlph{\\value{section}}}\n',1)
    ...
    write_json('PAPER_MAP_V163.json',dict(I_main=I_CORE,I_appendices=I_APPS+old_i_appendices,II_main=II_CORE,II_appendices=ii_names[len(II_CORE):],all_body_blocks_partitioned_exactly_once=True,section_labels={t:labels(parts[t]) for t in ordered}))
    preservation=dict(revision=163,controlling_review_commit=REVIEW,complete_predecessor_commit=BASE,predecessor_sha256=PREV_HASH,predecessor_labels=len(labels(old)),current_labels=len(labels(master)),predecessor_math_blocks=sum(blocks(old).values()),current_math_blocks=sum(blocks(master).values()),missing_labels=sorted(missing),duplicate_labels=dups,all_predecessor_math_blocks_retained_byte_for_byte=not lost,body_partitioned_exactly_once=True,predecessor_source_modified=False,all_three_previous_frontmatters_archived=True)
    write_json('NONDELETION_V163.json',preservation)
    ...
    if args.build:
        ...
        for name in ('geometry','reconstruction','divisor-geometry'):
            ...
            if name!='geometry':
                for label,(num,page) in aux_labels(name).items():
                    if label.endswith(('v161','v162','v163')) and not num.startswith(('I.','II.')):
        ...
        write_json('THEOREM_INDEX_V163.json',index)
    receipt['input_sha256']={p.name:sha(p) for p in sorted(HERE.iterdir()) if p.name in INPUTS+('frontmatter_v163.py','assemble_v163.py','check_v163.py')}
    receipt['helper_sha256']=sha(helper)
    for n in ('EXACT_CHECKS_V163.json','INHERITED_V162_CHECKS_RERUN.json'):
        if (HERE/n).exists():receipt[n+'_sha256']=sha(HERE/n)
    write_json('BUILD_RECEIPT_V163.json',receipt)
