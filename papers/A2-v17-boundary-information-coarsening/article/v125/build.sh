...
dump_failure_logs() {
  status=$?
  echo "v125 build failed with status $status" >&2
...
test -f ../v123/parts/02-relations.tex || {
  echo 'Missing preserved v123 mathematical source' >&2
  exit 1
}
test -f parts/09-stratified-nilpotent.tex || {
  echo 'Missing v125 stratified nilpotent source' >&2
  exit 1
}
...
for pass in 1 2 3; do
  pdflatex -interaction=nonstopmode -halt-on-error geometry.tex \
    > "evidence/geometry-pass-${pass}.log" 2>&1
done
