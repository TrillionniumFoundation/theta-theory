...
\end{theorem}
\begin{proof}
We specify the classical input. On a complex nondegenerate symmetric
space of dimension \(m\), full orthogonal orbits of self-adjoint
nilpotents are indexed by all partitions \(\lambda\) of \(m\).
Their dimensions are \((m^2-\sum_a(\lambda'_a)^2)/2\), and
the closure order is dominance. These are the statements in
\cite[\S1, p.~3, equation~(2) and the following dominance criterion]{TrevisiolSymmetric}.
They concern self-adjoint matrices, not the skew-adjoint orthogonal
Lie algebra. The Grassmannian Segre-stratum comparison is
\cite[Theorem~5.1]{FevolaMandelshtamSturmfels}.
