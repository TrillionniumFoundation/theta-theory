...
nonreduced scheme also recovers the original relation web on a nonempty invariant open.
...
generically unique.  This gives the inverse problem a geometric
...
including information at the deepest projection-corank stratum.  The following conclusion is proved, with
an explicit reconstruction open, in
Section~\ref{sec:intrinsic-web-reconstruction}.

\begin{theorem}[Intrinsic web reconstruction]
\label{thm:main-web-reconstruction}
There is a nonempty invariant open \(G_4^{\mathrm{rec}}\subset
G_4^\circ\) such that, for \(R\in G_4^{\mathrm{rec}}\) and
\(R'\in G_4^\circ\),
\[
 \widehat D_R\simeq\widehat D_{R'}\quad\Longrightarrow\quad
 R'=gR\ \text{for some }g\in\PGL(V).
\]
Consequently the abstract failure scheme determines \(B_R\) on
this open and separates the generic finite web-to-K3 packet.
\end{theorem}
...
line in the Pluecker space.  Two explicit quadratic relations show
that, on a nonempty open, its intersection with the Grassmannian is
one reduced point.  The global tensor rulings of the normal cone
...
case from a reduced secant pair.  These are statements about the
component invariant: equality of that invariant is not asserted to
imply isomorphism of exceptional failure schemes.
...
nonreduced normal cone to recover the web generically, and
...
and polarized reconstruction lead immediately to
Section~\ref{sec:intrinsic-web-reconstruction}, which proves the web
inverse theorem, its naturality, and the exceptional-fibre theorem.
The boundary atlas and global residual calculations then supply the
supporting geometric laws.  The relative-primary construction is
presented as a tool for the family and specialization results.
Deformation along the reduction packages the computed graded algebra.
The moduli and classical inverse-data comparisons complete the main
text; weighted models and exact certificate material remain in the
appendices.  All boundary tables, collision calculations, determinant
proofs, and relative descent arguments are included.
...
the reconstruction of the polarized K3 and, generically, of the web
from the intrinsic nonreduced scheme.  Ballico's 1993 paper remains cited as a documentary boundary;
