...
def main():
    ...
    if repo:
        ...
        for version in ("v110", "v109", "v108"):
            out = repo / "build-v111" / version
            out.mkdir(parents=True, exist_ok=True)
            command = ["latexmk", "-g", "-pdf", "-interaction=nonstopmode", "-halt-on-error",
                       "-file-line-error", "-outdir=" + str(out), "article/" + version + "/paper.tex"]
            (out / "latexmk.stdout").write_text(run(command, paper_root))
    ...
    receipt = {"status": "passed", "source_bound": not args.local,
               "scope": "principal-only local" if args.local else "exact-source four-volume review",
