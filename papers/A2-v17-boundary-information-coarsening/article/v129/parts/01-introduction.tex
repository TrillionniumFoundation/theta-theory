...
argument is invoked.  Relative assassins then turn the finite tower
into geometric support packets with constant embedded multiplicities.
...
relative assassins.  Constructibility of fibrewise associated points
...
the polarized K3 from an abstract nonreduced failure scheme.  Revision 130 does not need an unproved generic-injectivity assertion.  Its broader structural consequence is theorem-level: the universal symmetric-power determinant-completion identity induces a finite, base-change-compatible primary stratification for the entire universal family of quotient maps, and the \((1,4,6)\) boundary atlas is one geometric specialization of that general statement.
