...
and how much geometry can be recovered from finitely many such
observations. The logarithmic action curve modulo the detector space
connects the two questions: its secants encode exact action information,
its tangents control inversion, and its nontrivial fibres describe
which geometric changes can be hidden by the apparatus.

There are two main conclusions. First, for finite-dimensional detector
spaces whose derivatives are rational, regular identification is
classified by a finite list of residue and double-pole obstructions
(Theorem~\ref{thm:v85-polar}). This is a coefficient-space test, not a
search over unknown actions. Under principal-part closure it gives all
maximal admissible enlargements in a fixed ambient divisor class
(Corollary~\ref{cor:v85-maximal}). The same pole data, including order
at infinity, give one uniform clock budget
(Theorem~\ref{thm:v85-budget}). Positivity of an annihilator is neither
assumed nor necessary.

Second, finite selective readout records yield geometric confidence
sets. In a two-arm travel-time experiment, the propagation action is
a boundary distance and the longer reference action is unknown. The
reference may change with the endpoint pair; both readout channels and
all relative detector coefficients may change as well. A compact
forward model gives data-derived action intervals without supplying a
channel signal floor to the estimator
(Theorem~\ref{thm:v85-confidence}). On a boundary mesh of diameter $h$,
let $w_h$ be the largest reported interval width. On the precise local
boundary-rigidity stability class described in
Section~\ref{sec:v85-finite-geometry}, every feasible metric satisfies
\[
                 \|\widehat g-\psi^*g\|_{C^2}
                         \le C(w_h+2Lh)^\mu,
                     \qquad 0<\mu<1,
\]
for a boundary-fixing diffeomorphism $\psi$. Theorem~\ref{thm:v85-geometric}
proves this implication directly from finitely many categorical
records, with no interpolation of the unknown nuisance fields.
Its geometric input is the stability theorem of
Stefanov--Uhlmann~\cite{V85SU}, with its hypotheses retained explicitly.

On regular channel classes the certificate has width of order
$\sqrt{\log(m_h/\zeta)/n}$ for $n$ readouts in each cell. A balanced
surface design with total readout budget $N$ gives the sufficient
$C^2$ pullback rate $(\log(N/\zeta)/N)^{\mu/4}$.
For a finite-dimensional geometric prior, Theorem~\ref{thm:v85-parametric-risk}
instead proves matching $N^{-1}$ bounds for mean squared parameter
loss on a finitely selected boundary design. The alternatives used in
its lower bound are actual metrics. Theorem~\ref{thm:v85-geometric-clock}
also realizes the missing-clock fibre by non-boundary-isometric simple
metrics: $q+3$ clocks are worst-case sharp for the endpoint-dependent
reference class. Finally Theorem~\ref{thm:v85-raw} charges all raw
attempts, including failures. Its certificate uses observed acceptance
counts, rather than treating retained readouts as free preparations.

The underlying exact action theory remains useful beyond the two-arm
application. Theorem~\ref{thm:v83-main} gives the sharp polynomial
inverse with unknown visible count and a possibly nontrivial component
covering. Theorems~\ref{thm:v84-design} and \ref{thm:v84-generic} give
finite and generic analytic clock designs; prescribed singularities,
a positive-kernel-free example, and action divisors are treated in
Section~\ref{sec:v84-spaces}. The complete polynomial fibres appear in
Theorem~\ref{thm:v84-fibre}. The finite-record covering theorem recovers
monodromy, while the geometric constructions retain their exact
observation domains and full primitives. The boundary-metric experiment
needs only the pointwise action inverse, not a supplied covering.
...
The general secant criterion is not a characterization of arbitrary
moment measures. Its finite-design argument belongs beside the
analytic experiment theory of Sontag~\cite{V85Sontag} and the
finite-measurement inverse theory of Alberti--Santacesaria~\cite{V85AS}.
The finite residue and double-pole tests of
Theorem~\ref{thm:v85-polar} give an additional structural classification
within the rational-derivative category.
Finite-rate-of-innovation and Prony methods also recover finite
singular supports \cite{V84VMB,V84BY}; their linear moment or
convolution samples are not our projective matrices or sampled
logarithms with an additive unknown detector space.
...
The noisy pairwise-permutation experiment of \cite{V85NZ} is different
from this vertex-readout experiment. Blind bilinear calibration in
\cite{V85LS} likewise provides an important comparison, but not an
inverse for the present nonlinear action and two-channel nuisance law.
These distinctions are detailed at the relevant theorems rather than
used to claim novelty for spectral decomposition itself.
...
the common-lift premise by literal identities of reconstructed lifts on
an observed invariant domain; its elementary group criterion does not
certify an unobserved invariant domain or an exact root from a noisy
compatibility residual. For the exact surface application,
Pestov--Uhlmann's boundary-distance theorem supplies the metric step
\cite{V84PU}. For finite noisy observations we use the stated local
stability estimate of \cite{V85SU} and prove the complete acquisition,
confidence and geometric error chain. The partial-data metric results
in \cite{V84SUV} solve different geometric inverse problems and are
not inferred from the finite-dimensional clock theorem.
...
$u,u+d_0\in F((-\infty,T_1))$ gives the bracketing interval. Reverse
orientation for negative $d_0$. No action sorting is used. Once $y$ is
known, the single contrast $\int_y^xw_0$ is strictly increasing in $x$,
including $x=y$, so $q+2$ nodes identify every remaining component.
The first contrast vanishes exactly at equal actions.
