...
every complex pencil, including singular pencils.  More precisely,
there is a smallest uniform infinitesimal order at which the pencil
appears.  The global geometry needed for this inverse is small: one
projective line in the socle already suffices.
...
cone then recovers the two tensor rulings.  Precisely one is
projectively trivial over the reduction; the other is distinguished
already on a Schubert line.  Properness forces the induced map to
\(\PGL(V)\) to be constant.  Finally, cancellation of the intrinsic
...
the original parameter Grassmannian.  A single unmarked Artinian normal
slice does not supply the global ruling distinction used in the proof;
we make no corresponding local-algebra classification assertion.
...
relative socle ideal need not be the absolute nilradical.  These
statements do not identify unmarked moduli stacks or deformation
groupoids.
...
failure scheme.  A theorem-level comparison with the distinct earlier
paper \cite{Ballico93} still requires its complete text; no claim of
nonanticipation relative to that unread text is made.

The present article is the reconstruction argument and its spectral
readout.  The reciprocal-likelihood theorem and projective critical
