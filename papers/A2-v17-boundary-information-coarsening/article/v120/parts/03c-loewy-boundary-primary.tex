...
\begin{theorem}[Complete primary decomposition in length five]
...
localization. Each associated stratum actually meets the frame open:
one may set $H$ equal to the identity matrix. Descent is not a
specialization assertion. The ideals $P,Q$ express the intrinsic
conditions \eqref{eq:loewy-factor-surface}; their sum expresses
$K=S_2$. The determinant ideal is intrinsic, and the residual ideal
$J=I_{\mathrm{tf}}:(\delta^2)$ is invariant under a change of frame.
Thus $Q_0$ and $Q_*$ can equally be formed from these invariant ideal
sheaves. The factor surfaces may be interchanged by an automorphism
of $B$, but their unordered collection and the decomposition descend.
The frame morphism is a smooth faithfully flat torsor, with no new
associated primes introduced by its regular fibres. Alternatively,
all assertions follow on the standard Grassmannian charts by solving
for a frame minor, leaving exactly the displayed extended ideals.
This establishes the claims on the whole of $X$.
