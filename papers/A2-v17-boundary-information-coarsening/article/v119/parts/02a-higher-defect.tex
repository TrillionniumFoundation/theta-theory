...
$m\ge d-1$. On the exact-rank $s$ locus $X_s$ of $\mu_{d-1}$ there
...
flags $B\twoheadrightarrow Q\supset C$ of these ranks, where $C$ is
unital, $Q/C$ is locally free of rank $r$, and the action of $C$ on
$Q/C$ is a subbundle injection. The inverse sends the flag to the
preimage of $C$.
\end{theorem}
\begin{proof}
...
subalgebra bundle. Locally write $C=\langle1,T\rangle$. At every
geometric point $T$ is nonscalar. A nonscalar two by two matrix has
a cyclic vector: its minimal polynomial has degree two. After a
local lift of such a vector, the map $C\to M$, $c\mapsto cv$, has
invertible determinant. Thus $M$ is locally free of rank one as a
$C$-module. The exact sequence of $C$-modules
