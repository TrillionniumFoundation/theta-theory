...
the principal ideal domain $\C[\delta]$. It is flat by the
valuation-ring flatness criterion \cite[Tag~0539]{Stacks164}.
...
$Z^r\times(\Pj^1)^s$; all these parameter-space components are
reduced normal crossings. The full central pullback is instead
$X_2^r\times(\Pj^1)^s$, whose conic factors are $\Pj^4$, not $S$.
Thus the horizontal and full fibres have different component
dimensions even though both keep all choices at separated supports.
\end{corollary}
\begin{proof}
...
smooth ordered model. The last assertion is the complete-fibre
