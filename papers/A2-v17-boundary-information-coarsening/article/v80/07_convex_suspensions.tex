...
a fixed $C^{k+4}$ bound on $L$ on the enlarged compact configuration
square, fixed $C^{k+2}$ bounds on source and efficiency on the enlarged
phase and endpoint domains, and positive source and efficiency floors,
...
Proposition~\ref{prop:v80-stationary-bounds}, with $m=k+2$, gives
the required action and density derivative bounds on the fixed compact
region using exactly the orders in the theorem.  The twist estimate and the Schur bound
...
derivatives make a common collar possible, with the explicit radius
in Theorem~\ref{thm:v80-four-convex}.  The action-anchor bound
