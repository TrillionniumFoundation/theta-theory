        ...
        for i,page in enumerate(doc):
            for b in page.get_text('blocks'):
                if b[0]<0 or b[1]<0 or b[2]>page.rect.width+1 or b[3]>page.rect.height+1:
                    outside.append({'page':i+1,'bounds':list(b[:4])})
        pdf_info[name]['outside_page_blocks']=outside
    logchecks[name+'_page_bounds']=not outside
    logchecks[name+'_no_overfull_boxes']='Overfull' not in log
    logchecks[name+'_title_revision139']='revision 139' in fitz.open(p).metadata.get('title','').lower()
...
r139=json.loads((ROOT/'evidence/REVISION139_EXACT.json').read_text())
r138=json.loads((ROOT/'evidence/REVISION138_EXACT.json').read_text())
r137=json.loads((ROOT/'evidence/REVISION137_EXACT.json').read_text())
...
scripts=['exact_k3','exact_corank_two','stratified_rank_two','boundary_atlas','generic_boundary_atlas',
 'revision131_exact','revision132_exact','revision133_exact','revision134_exact','revision135_exact','revision136_exact','revision137_exact','revision138_exact','revision139_exact']
checks={**logchecks,
 'focused_article_under_50_pages':pdf_info['geometry']['pages']<50,
 'complete_mathematics_retained':pdf_info['complete']['pages']>=105,
 'all_inherited_mathematical_labels_retained':set(m['inherited_mathematical_labels'])<=set(labels),
 ...
 'all_fourteen_script_execution_logs_present':all((ROOT/'evidence'/(n+'.log')).exists() for n in scripts),
 'new139_exact_checks':r139['ok'],
 'sharp_finite_neighbourhood_and_family_proofs_in_main':all(n in main_labels for n in ['thm:sharp-finite-pencil','lem:first-relation-cone','eq:intrinsic-first-relation','thm:finite-parameter-immersion','lem:line-subspace-immersion','cor:finite-flat-family','cor:finite-hyperelliptic']),
 'v138_nondeletion_receipt_passes':json.loads((ROOT/'NONDELETION_V139.json').read_text())['ok'],
 'new138_exact_checks':r138['ok'],
 'inherited137_exact_checks':r137['ok'],
 'automatic_natural_family_and_moduli_results_in_main':all(n in main_labels for n in ['thm:automatic-quadratic-coefficients','thm:natural-pencil-reconstruction','cor:intrinsic-segre-data','ex:same-discriminant-distinct-failure','cor:hyperelliptic-failure-invariant','eq:general-coefficient-map','sec:operator-signatures']),
 'inherited136_exact_checks':r136['ok'],
 'intrinsic_criterion_second_family_and_full_proof_lemmas_in_main':all(n in main_labels for n in ['thm:structural-coefficient-reconstruction','thm:polar-system-torelli','lem:rank-one-fano-scheme','lem:full-orthogonal-harmonics','eq:full-intrinsic-chain','ex:nondiagonal-uniform-web','ex:polar-nilpotence-three']),
...
receipt={'revision':139,'source_commit':m['assembly_commit'],'reviewed_commit':m['reviewed_commit'],
 ...
 'new_exact_result':r139,
 'overfull_box_warnings':{n:[l for l in log.splitlines() if 'Overfull' in l] for n,log in logs.items()},
 'evidence_scope':{'structural_proofs_not_machine_certified':r139['structural_proofs_not_machine_certified']+r138['structural_proofs_not_machine_certified']+r137['structural_proofs_not_machine_certified']+r136['structural_proofs_not_machine_certified']+r135['structural_proofs_not_machine_certified']+r134['structural_proofs_not_machine_certified'],
 'documentary_open':['B137.1: complete theorem-level comparison with Ballico 1993'],
 ...
 'classification of the entire ambient binary-Jacobian boundary','original cube-zero web failure-scheme theorem in dimensions greater than four',
 'equivalence of moduli stacks or classification of all polar-scheme automorphisms']}}
...
if not receipt['ok']:raise SystemExit('v139 verification failed')
