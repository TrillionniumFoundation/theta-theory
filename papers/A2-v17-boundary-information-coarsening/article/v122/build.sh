...
# Only generated files of this revision are removed. No authoring script runs.
for doc in paper geometry applications; do
  rm -f -- "$doc.aux" "$doc.out" "$doc.toc" "$doc.fls" "$doc.pdf" "$doc.log"
done
