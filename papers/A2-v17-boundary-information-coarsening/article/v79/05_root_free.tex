...
Let $U(s,t)$ and $V(s,v)$ be single-valued actions of actual regular
physical trajectories, for one fixed reflection word and its one-flight
extension, on their entire product domains.  Every trajectory on these
domains has the specified itinerary, clear positive flights and
nongrazing reflections.  The two actions use the same initial boundary
embedding and scalar coordinate, the same unit speed and the same
outgoing half-plane.  They are not merely continuations of stationary
values outside the physical branch.  Suppose their products have a
common connected initial interval $[a,b]$, including its endpoints.  For fixed $(t,v)$ put
\[
