...
\subsection{Local multipliers}
\label{sec:v15-notation}
For the local hyperbolic and contact-jet calculations, the channel
index is suppressed and the following convention is fixed:
\begin{equation}\label{eq:v15-multiplier-convention}
 ...
 \end{aligned}
\end{equation}
The number $\mathfrak r_b$ is a curvature ratio, whereas $r_b$ is
the derivative at zero of the physical first-hit map $\varphi_b$.
Thus $\mathfrak r_0\mathfrak r_1=1$, $r_0r_1=\lambda$, and
$R_b=\varphi_{1-b}\circ\varphi_b$ has multiplier $\lambda$.
The integer $j$ counts flights; $n$ iterations of $R_b$ mean $j=2n$.
The letter $\rho$ in exponential estimates is a freely chosen decay
bound with a strict margin; it is not the exact factor $\varrho$.
The inverse Morse coordinate $\chi_b(t)$ used below is unrelated to
$\mathfrak r_b$. These conventions apply in particular to both
Sections~\ref{sec:v14-hyperbolic} and~\ref{sec:v12-two-contact}.
...
\subsection{Hyperbolic coordinates and the boundary measure}
\label{sec:v14-forward-position}
A local analytic hyperbolic normal form already explains a relative
endpoint product law. This is a closer forward comparison than a global
inverse-spectral theorem. Proposition~\ref{prop:v14-normal-form} derives
the mixed-boundary formula and transfers it through the physical
projection charts. Its canonical flux is
\[
 T_n=\frac{\Delta(I)^n}{1-nI\Delta'(I)/\Delta(I)},
 \qquad I=st\Delta(I)^n.
\]
The actual flux is $|T_n/\det D\Phi_n|$, not $T_n$ alone. For a
billiard return, $j=2n$ and $\lambda=e^{-2\gamma}$; the projection
recovers the exact reference value $a_b/\sinh(2n\gamma)$. Thus
relative analytic factorization is not claimed as a phenomenon outside
classical local hyperbolic structure. The uniform parameter clause of
the proposition assumes a supplied uniformly smooth family of charts;
it does not obtain those charts from arbitrary smooth obstacle graphs.

There is also a classical smooth scalar comparison, independent of a
two-dimensional normalizing chart. A one-dimensional smooth contraction
has a unique normalized smooth linearizer; Lemma~\ref{lem:v15-scalar-product}
recalls the elementary product proof of this case of Sternberg's
theorem~\cite{Sternberg,EN}. For the physical return, its density is
\[
 \mathfrak B_b(u)=\exp\!\left\{
   \sum_{i=0}^{\infty}\log\frac{R_b'(R_b^i(u))}{\lambda}\right\}.
\]
The problem specific to the relative law is to identify this classical
density with the physical half-line Fredholm amplitude $B_b$, including
its normalization. Theorem~\ref{thm:v14-intrinsic-density} proves the
exact first-flight identity
\[
 B_{1-b}(\varphi_b(u))\varphi_b'(u)=r_b B_b(u)
\]
by normalizing finite Schur concatenation before taking the limit.
Corollary~\ref{cor:v15-determinant-product} then gives $B_b=\mathfrak B_b$
and an exact finite-product remainder in every fixed mixed norm.
Consequently $\zeta_b(u)=\int_0^uB_b(x)\,\dd x$ is the normalized
scalar linearizer, and $B_b=\lim_{n\to\infty}\lambda^{-n}(R_b^n)'$.
The coordinate, its uniqueness and its iterate limits are classical;
the physical determinant identification connects them to the relative
probability theorem. The scalar construction does not itself establish
that theorem's finite-bridge estimates or its residual-time integral.

In this coordinate the action has ordinary Lebesgue sublevel widths.
The complete normalized width satisfies
\[
 \mathcal W_b(E)=\int_0^E x^{-1/2}\mathcal V_b(x)\,\dd x,
 \qquad \mathcal V_b(E)=\sqrt E\,\mathcal W_b'(E)\quad(E>0).
\]
The weighted derivative extends smoothly to zero with value one.
Thus the even laws determine a single complete invariant, represented
equivalently by $\mathcal V_b$ or by $\mathcal W_b$
(Corollary~\ref{cor:v14-width}). The width is an interpretation of the
energy pushforward, not a second independent rigidity result. It gives
the difference of the two inverse branches of the action; it does not
supply their midpoint for a general asymmetric contact. The even-contact
geometric inverse uses its additional geometric hypotheses and its own
block calculation. The scalar comparison and the width equivalence
were emphasized in the memorandum~\cite{A2ReviewV14}.

This comparison locates the contribution in the full passage from
smooth physical graphs, through uniform relative flux and residual-time
integration, to a complete intrinsic invariant and its charged inverse
experiment. The analytic product mechanism and classical scalar
linearization are ingredients of that explanation. The independent-contact inverse and physical finite-jet
image give separate geometric content; they are not consequences of
writing a canonical normal form. Section~\ref{sec:v14-hyperbolic}
states precisely the overlap, including the odd-flight extension of
the analytic comparison, and the additional hypotheses needed for a
uniform chart-based proof.
...
the analytic and classical scalar comparisons, the physical branch
measure, and nonlinear
energy compatibility, followed by the two-contact limiting
