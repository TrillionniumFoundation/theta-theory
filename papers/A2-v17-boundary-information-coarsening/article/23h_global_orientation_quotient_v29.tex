...
scale.  Any fixed-order stability estimate for the equivariant signed
inverse on its exact law image, valid for the representative pairs used in
reflection-invariant orbit metrics, descends with the same constant to those
metrics.  On arbitrary perturbed densities the corresponding action,
amplitude and finite-jet assertion uses the transported-anchor extension of
Proposition~\ref{prop:v29-transported-anchor} and
Corollary~\ref{cor:v29-density-orbit-stability}.  It does not use a fixed
signed anchor in both orientation sectors.
\end{corollary}
...
For the stability assertion on exact laws, let \(F\) be the equivariant
signed inverse between the relevant fixed-order data and reconstruction
spaces, with reflection-invariant
...
order-independent stability for analytic continuation.  For perturbed
densities the same argument applies to the explicitly equivariant extension
in \eqref{eq:v29-exact-off-model-equivariance}, on its stated domain.  The
fixed-anchor formula alone has no such off-model equivariance; see
Remark~\ref{rem:v29-fixed-anchor-witness}.  No arbitrary perturbed density
is thereby declared globally realizable.
\end{proof}
