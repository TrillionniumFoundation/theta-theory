...
OLD=Path(os.getenv('A2_PREDECESSOR_DIR',str(ROOT.parent/'v143')))
...
checks={
 ...
 'complete_predecessor_source_archive_exact':all(sha(p)==sha(ROOT/'history/v143'/p.relative_to(OLD)) for p in archive_files),
 ...
 'principal_theorem_and_new_critical_results_in_main':all(x in main for x in ['thm:principal-finite-v143','thm:sharp-finite-pencil','thm:universal-finite-neighbourhood','thm:relative-spectral-readout','thm:critical-correspondence-v143','thm:real-critical-cover-v143','thm:projective-critical-v144','thm:critical-ramification-v144','prop:triple-critical-v144']),
 ...
 'current_issue_matrix_revision':issue['revision']==144,
...
try:
 ...
 subprocess.run(['git','diff','--exit-code','HEAD','--',p+'/parts',p+'/checks',p+'/*.tex',p+'/build.sh',p+'/verify_v144.py'],cwd=repo,check=True,stdout=subprocess.PIPE)
...
for name in ['geometry','supplement','complete']:
 ...
 with fitz.open(p) as doc:
  ...
  checks[name+'_metadata']='revision 144' in doc.metadata.get('title','').lower()
...
scripts=['exact_k3','exact_corank_two','stratified_rank_two','boundary_atlas','generic_boundary_atlas']+['revision'+str(i)+'_exact' for i in range(131,141)]+['revision141_spectral_exact','revision141_likelihood_exact','revision142_exact','revision143_critical_exact','revision144_projective_exact']
checks['all_twenty_scripts_executed']=all((ROOT/'evidence'/(x+'.log')).exists() for x in scripts)
...
excluded={'PROVENANCE_MANIFEST_V144.json','NONDELETION_V144.json','SOURCE_LOCK_V144.json'}
...
manifest={'revision':144,'source_commit':source_commit,'review_commit':REVIEW,'reviewed_manuscript_commit':REVIEWED,'mathematical_predecessor':PIN,'source_sha256':{str(p.relative_to(ROOT)):sha(p) for p in sorted(files)},'Ballico_1993_full_text_obtained':False,'exhaustive_priority_verified':False}
(ROOT/'PROVENANCE_MANIFEST_V144.json').write_text(json.dumps(manifest,indent=2)+'\n')
nondeletion={'revision':144,'predecessor':PIN,'inherited_labels':old_labels,'inherited_label_count':len(old_labels),'current_label_count':len(labels),'inherited_mathematical_blocks':sum(oldproof.values()),'unchanged_blocks_retained':not(oldproof-newproof),'inherited_numbered_blocks':sum(oldnumbered.values()),'unchanged_numbered_blocks_retained':not(oldnumbered-newnumbered),'all_old_part_hashes':{p.name:sha(p) for p in parts},'all_old_check_hashes':{p.name:sha(p) for p in scripts_old},'active_inputs':sorted(active),'missing_mathematical_blocks':list((oldproof-newproof).keys()),'missing_numbered_blocks':list((oldnumbered-newnumbered).keys())}
(ROOT/'NONDELETION_V144.json').write_text(json.dumps(nondeletion,indent=2)+'\n')
receipt={'revision':144,'ok':all(checks.values()),'checks':checks,'pdfs':pdfs,'inherited_label_count':len(old_labels),'current_label_count':len(labels),'inherited_mathematical_blocks':sum(oldproof.values()),'source_commit':source_commit,'review_commit':REVIEW,'reviewed_manuscript_commit':REVIEWED,'mathematical_predecessor':PIN,'github_run_id':os.getenv('GITHUB_RUN_ID'),'workflow_trigger_commit':os.getenv('GITHUB_SHA'),'executed_scripts':scripts,'source_manifest_sha256':sha(ROOT/'PROVENANCE_MANIFEST_V144.json'),'runtime':{'python':platform.python_version(),'sympy':sympy.__version__,'numpy':numpy.__version__,'pymupdf':fitz.VersionBind},'documentary_open':['B140.2: full Ballico 1993 theorem-level six-axis comparison','B140.3: no exhaustive historical-priority certification; exact inspected-source comparison and operator identity supplied'],'not_certified':['formal verification of all proofs','exhaustive historical priority','top-four editorial acceptance','unmarked moduli-stack equivalence']}
(ROOT/'evidence/BUILD_RECEIPT_V144.json').write_text(json.dumps(receipt,indent=2)+'\n')
lock={'revision':144,'source_commit':source_commit,'controlling_referee_report_commit':REVIEW,'reviewed_manuscript_commit':REVIEWED,'mathematical_predecessor_commit':PIN,'pdfs':pdfs,'receipt_sha256':sha(ROOT/'evidence/BUILD_RECEIPT_V144.json'),'source_manifest_sha256':sha(ROOT/'PROVENANCE_MANIFEST_V144.json'),'run_id':os.getenv('GITHUB_RUN_ID'),'freeze_policy':'The exact source SHA, not the movable branch name, defines this review object. A subsequent mathematical revision requires a new branch.'}
(ROOT/'SOURCE_LOCK_V144.json').write_text(json.dumps(lock,indent=2)+'\n')
print(json.dumps(receipt,indent=2))
if not receipt['ok']:raise SystemExit('v144 checks failed')
