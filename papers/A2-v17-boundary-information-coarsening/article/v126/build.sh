...
dump_failure_logs() {
  status=$?
  echo "v126 build failed with status $status" >&2
...
test -f ../v125/parts/09-stratified-nilpotent.tex || { echo 'Missing preserved v125 mathematical source' >&2; exit 1; }
test -f parts/09a-rees-specialization.tex || { echo 'Missing v126 Rees specialization source' >&2; exit 1; }
