...
\begin{theorem}[Normalization and all same-annihilator signed intersections over the split rank-two base]
...
that subspace is a domain. The connected cover and its deck action are also established
algebraically in Proposition~\ref{prop:algebraic-phase-descent} with $m=2$.
This identifies the reduced incidence as the
...
presentation.
\end{proof}
\begin{proposition}[Primary decomposition and Hilbert series of the fibre]
\label{prop:fibre-primary-decomposition}
In the notation of \eqref{eq:rank-two-fibre-ring}, let
$\mathfrak a=(a_1,\ldots,a_c)$, $\mathfrak b=(b_1,\ldots,b_c)$,
and $\mathfrak m=\mathfrak a+\mathfrak b$. For $c\ge2$ an
irredundant primary decomposition is
\begin{equation}\label{eq:fibre-primary-decomposition}
 J_c=\mathfrak a\cap\mathfrak b\cap(J_c+\mathfrak m^3).
\end{equation}
The last ideal is $\mathfrak m$-primary, and the Hilbert series is
\begin{equation}\label{eq:fibre-hilbert-series}
 \operatorname{Hilb}_{R/J_c}(t)
   =\frac{2}{(1-t)^c}-1+\binom c2t^2.
\end{equation}
\end{proposition}
\begin{proof}
The intersection $\mathfrak a\cap\mathfrak b$ is generated by all
mixed monomials $a_i b_j$. Every mixed monomial of degree at least
three belongs to $J_c$, by the cubic calculation in the preceding
proof. In degree two, intersection with $J_c+\mathfrak m^3$ leaves
exactly the degree-two part of $J_c$. Homogeneity proves
\eqref{eq:fibre-primary-decomposition} degree by degree.
The quotient by $J_c+\mathfrak m^3$ is an Artinian local ring with
maximal ideal the image of $\mathfrak m$; every zero divisor is
nilpotent. Thus this ideal is $\mathfrak m$-primary. Neither minimal
prime can be omitted: $a_1^3$ witnesses the need for $\mathfrak b$
and $b_1^3$ for $\mathfrak a$. The embedded component cannot be
omitted because $a_1b_2$ survives modulo $J_c$ when $c\ge2$.
The reduced quotient has in each positive degree the direct sum of
the pure $a$- and pure $b$-monomials, and in degree zero one common
constant. The nilradical adds exactly $\binom c2$ degree-two
classes. This proves \eqref{eq:fibre-hilbert-series} and verifies
algebraically that the three primes listed above are all the
associated primes. For $c=1$ the third component is redundant and
$J_1=(a_1b_1)$ is already reduced.
\end{proof}
...
\input{parts/02h-relative-wall-proofs.tex}
\input{parts/02i-simple-normal-determinant.tex}
...
\end{theorem}
\begin{proof}
Proposition~\ref{prop:relative-residual-divisor} gives the relative
integral divisor, Lemma~\ref{lem:wall-open-nonempty} supplies a
simultaneous dense open, and Proposition~\ref{prop:normal-block-factorization}
gives the intrinsic determinant identity. We spell out how they lead
to the local equation. At residual dimensions $k'=k-2$, $c'=c-1$ the expected codimension
