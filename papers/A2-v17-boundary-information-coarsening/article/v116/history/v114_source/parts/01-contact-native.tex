\section{Calibrated information metrics and the geometric reduction}\label{sec:information-reduction}
...
\]
Here codimension refers to complex codimension of the reduced closed set.
Theorem~\ref{thm:global-intro} gives the precise formula.
...
The numerical formula has a structural refinement. Write $a=Lq-p+1$ and $b=Lc-3$. In the stable range $c\ge8$, $k\ge2c+1$, Theorem~\ref{thm:structure} proves that the maximal-minor scheme is integral and Cohen--Macaulay when $a<b$. Its generic annihilator is a nondegenerate Hankel form. At $a=b$ the scheme is reduced and Cohen--Macaulay, with precisely one nondegenerate component and $2^{L-1}$ signed secant components. If $b<a$, the latter are exactly the maximal-dimensional components. They are rational and generically smooth in the determinantal scheme. The signs record independent choices of the two isotropic hyperplanes of a rank-two form, modulo their simultaneous interchange; the common-secant family above is only the all-equal choice.

The change of regime has an intrinsic residual description. At two distinct secant points, write $U_\nu\cap\operatorname{rad}H_\ell=gK_\nu$, where $g$ is their vanishing quadratic. The wall is
\[
 L\binom{c}{2}=2k-5,
\]
exactly the saturation threshold for multiplication of the $(c-1)$-planes $K_\nu$ in the $(k-2)$-dimensional residual space. This identity determines the conormal dimension of a general signed component. The expected-codimension cycle and local conditioning exponents follow from the structural theorem. The earlier all-dimension codimension formula is not restricted by the additional hypotheses of this refinement.
...
Failure of quadratic normality has a substantial antecedent literature. Green--Lazarsfeld \cite[Theorems 1 and 3]{GL86} study complete very ample linear series on curves, giving normal-generation criteria and finite secant configurations explaining failure under degree and cohomology hypotheses. Ballico \cite[Theorem 0.2]{Ballico96} studies failure cycles for a fixed embedded surface and detects failure on finite linear sections, using a Grassmannian incidence and cohomology. The principle that secants can explain quadratic-normality failure is therefore classical. Here the complete series on $\Pj^1$ is fixed and normally generated; the varying objects are its proper $c$-dimensional subseries, and for $L>1$ the observation uses a direct sum of within-subseries products, without cross-products. Our structural assertion concerns the components and generic scheme structure over this product of Grassmannians. Neither the complete-series degree criterion nor the surface-section failure-cycle theorem supplies this signed component classification or the residual wall. We distinguish these particular conclusions from the classical inputs on Hankel ranks, determinantal resolutions, and cycles \cite{CMSV,EN,Kustin,KL,Fulton}.
