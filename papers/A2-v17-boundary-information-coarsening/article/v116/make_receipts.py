...
def source_paths():
    ...
    out.append(HERE/'evidence/V115_SOURCE_MANIFEST.json')
    return sorted(out)
def main():
    ...
    source={'kind':'A2 revision 116 source-bound receipt','controlling_review_commit':REVIEW,'reviewed_revision_commit':BASE,'mathematical_source_commit':commit,'source_commit_namespace':'repository_branch_build' if commit else 'standalone_rebuild','remote_publication_verified_by_this_script':False,'sha256':hashes}
    pdfs={}
    ...
    receipt={'kind':'A2 v116 source-bound PDF build; NOT proof certification','built_utc':datetime.now(timezone.utc).isoformat(),'mathematical_source_commit':commit,'source_receipt_sha256':digest(evidence/'SOURCE_RECEIPT.json'),'diagnostics_sha256':digest(evidence/'DIAGNOSTICS.json'),'inherited_diagnostics_sha256':digest(evidence/'V115_RERUN_DIAGNOSTICS.json'),'pdfs':pdfs,'toolchain':{'python':sys.version,'pdflatex':run('pdflatex','--version').splitlines()[0]},'reproducibility_epoch':os.environ.get('SOURCE_DATE_EPOCH'),'remote_publication_verified_by_this_script':False,'proof_certification':False}
