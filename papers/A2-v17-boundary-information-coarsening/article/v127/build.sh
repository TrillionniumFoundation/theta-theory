...
dump_failure_logs() {
  status=$?
  echo "v127 build failed with status $status" >&2
...
test -f parts/01b-boundary-atlas.tex || { echo 'Missing v127 atlas statement' >&2; exit 1; }
test -f parts/09b-boundary-atlas-proofs.tex || { echo 'Missing v127 atlas proof' >&2; exit 1; }
test -f parts/09a-rees-specialization.tex || { echo 'Missing v127 DNC section' >&2; exit 1; }
