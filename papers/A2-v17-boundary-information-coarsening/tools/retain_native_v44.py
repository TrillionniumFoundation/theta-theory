#!/usr/bin/env python3
"""Prepare native deliveries, then verify index or fetched committed Git objects.
...
def validate(evidence: Path, source: str) -> dict:
    ...
    require(imported['producer_source_commit']==source and imported['sha256']==sha256(evidence/'two_collision.aux') and recorder['generated_inputs']['two_collision.aux']['sha256']==imported['sha256'],'Companion producer/consumer mismatch')
...
def main() -> None:
    ...
    require(re.fullmatch(r'deliveries/a2-v(?:44|43-repaired)/[0-9a-f]{40}',args.destination.as_posix()) is not None,'Unexpected delivery path')
