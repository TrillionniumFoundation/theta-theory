#!/usr/bin/env python3
"""Build all three native A2 v56 entries from an immutable Git snapshot.
...
def main() -> None:
    ...
    report = {'status': 'started', 'entrypoints': [s + '.tex' for s in ENTRIES],
              'scope': 'Principal journal article plus complete technical manuscript and two-collision companion; all source-matched',
    ...
    try:
        with tempfile.TemporaryDirectory(prefix='a2-source-pinned-') as temporary:
            ...
            for stem in ENTRIES:
                ...
                imported = {}
                producers = ('two_collision',) if stem == 'main' else (('two_collision', 'main') if stem == 'rigidity' else ())
                for producer in producers:
                    completed = report['entries'].get(producer, {})
                    require(completed.get('status') == 'passed', 'Required producer not source-verified: ' + producer)
                    aux = out / (producer + '.aux')
                    require(aux.is_file(), 'Required auxiliary file missing: ' + producer)
                    shutil.copy2(aux, work / aux.name)
                    imported[aux.name] = {
                        'bytes': aux.stat().st_size, 'sha256': sha256(aux),
                        'producer_entry': producer + '.tex',
                        'producer_source_commit': manifest['source_commit'],
                        'producer_pdf_sha256': completed['product']['sha256'],
                        'producer_recorder_sha256': sha256(out / (producer + '-recorder-inputs.json'))}
                if imported:
                    write_json(out / (stem + '-imported-generated-inputs.json'), imported)
