#!/usr/bin/env python3
"""v54-only CLI for the preserved native-artifact and Git-object verifier."""
...
def main() -> None:
    ...
    require(re.fullmatch(r'deliveries/a2-v54/[0-9a-f]{40}',args.destination.as_posix()) is not None,
    ...
    if args.operation=='prepare':
        ...
        readme.write_text(readme.read_text().replace('v44 verification ledger','v54 verification ledger'))
    else:
        result=verify(args.destination,args.ref)
        if args.attestation:
            require(args.ref!='INDEX','An index is not a published commit')
