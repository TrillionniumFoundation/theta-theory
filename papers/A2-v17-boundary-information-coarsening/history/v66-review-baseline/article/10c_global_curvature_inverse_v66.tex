...
of an iterate exceeds its displayed error bound, then $z(s)>0$.
...
\eqref{eq:v66-fixed-order-error}.  Iteration of the quadratic algorithm
can be stopped with its error in \eqref{eq:v66-iteration-error} added
to this bound.  Error in an observed density is the input here, not a
number of full-phase preparations; the preparation theorems retain their
own observation and calibration hypotheses.
\end{proof}
