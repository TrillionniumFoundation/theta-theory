# Principal article: theorem inputs and comparison references — v60

This is the current reading-based dependency ledger. It carries forward the corrected v57 classification, which the latest v59 referee report keeps closed. The v56 all-comparisons classification remains superseded; historical maps and responses are not overwritten. Literal reference occurrences and declared roles are in `DEPENDENCY_MAP_V60.json`. Neither ledger is a formal proof certificate.
...
## Conditional real-observation estimate in revision 60

`article/23a3_conditional_observation_inverse_v60.tex` supplies three complete internal proofs. The contracted-evaluation criterion isolates the finite-low/high-tail mechanism. The scalar restriction lemma is proved by interpolation and an interior Cauchy contour. The contact theorem uses the complete v59 local inverse on a smaller disc and the real density-ratio identity, with the added analytic neighborhood, radius loss and positive margins stated in its hypotheses. Its weak-density/histogram clause additionally assumes a real Lipschitz bound and proves its smoothing/cell-average estimates internally.

The proof uses the bounded finite leading-variable splitting from the end of the v59 theorem; Hessians are recovered as bounded action coefficient functionals rather than extra observations. The two radii are reconciled by uniqueness, not an assumed radius-improvement property of the inverse. No full-manuscript theorem is needed for this local bound. The historical global histogram theorem is a comparison, not an unstated premise.

The new directed path is `thm:v26-density-inverse` (its elementary identity is displayed directly), `lem:v60-real-to-disc`, and `thm:v59-analytic-contact-inverse` -> `thm:v60-real-observation-stability`. The conditional weak-observation conclusion is not the old unconditional same-disc claim: the former has a fixed local analytic prior and a smaller output disc, while the latter is expressly not asserted. The acquisition theorem input to Corollary 19.9 remains substantive and unchanged.
