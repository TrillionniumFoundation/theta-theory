...
one-dimensional scalar density with the amplitude of the already proved
half-line law. The scalar coordinate itself is classical, as recalled
in Lemma~\ref{lem:v15-scalar-product}. No smooth two-dimensional
normal-form theorem is used in this identification.
...
\input{article/16a_scalar_linearization}
...
\[
 ...
 \mathfrak r_b=\sqrt{\frac{c_b}{c_{1-b}}},\qquad
 r_b=\mathfrak r_b\varrho,\qquad
 R_b=\varphi_{1-b}\circ\varphi_b,\qquad
 \varrho=e^{-\gamma},\qquad \lambda=\varrho^2.
\]
...
\input{article/16b_determinant_transport}
...
Conversely, on $E>0$,
\begin{equation}\label{eq:v15-width-equivalence}
 \mathcal V_b(E)=\sqrt E\,\mathcal W_b'(E),\qquad
 \mathcal W_b(0)=0,\qquad
 \mathcal W_b(E)=2\sqrt E+O(E^{3/2}).
\end{equation}
The first expression extends smoothly to $E=0$ with value one.
Thus the normalized width and the energy profile are equivalent
coordinates on the same invariant, not two independent invariants.
The pushforward theorem determines a sublevel measure; the width
formula expresses that measure in the classical stable coordinate.
...
connect the analytic and classical scalar interpretations to the
constructed smooth amplitude. Equation~\eqref{eq:v15-physical-product}
gives their exact relation; \eqref{eq:v15-width-equivalence} records
that the width is a coordinate on the previously determined invariant.
Neither interpretation is counted as an additional rigidity theorem.
