...
\end{equation}
The input to the exact germ inverse is the normalized germ $G_b$.
If instead the full unnormalized probability germ $p_b(d)$ is supplied,
its leading coefficient and its normalization are
\[
 \alpha_2=\lim_{d\downarrow0}\frac{p_b(d)}{d^2}
          =\frac{1}{2A\sinh(2\gamma)},\qquad
 G_b(d)=\frac{p_b(d)}{\alpha_2d^2}.
\]
This observation concerns exact germs, not a finite-sample estimate of a
limit at zero. The finite-family experiment below supplies the fixed
free area in its family specification; the general unknown-normalization
experiment uses the separately charged smooth-class pilot. No derivative
or leading coefficient is supplied as a free statistical observation.
