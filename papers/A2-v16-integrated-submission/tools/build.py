#!/usr/bin/env python3
"""Clean-build the A2 v12 article and companion; record actual diagnostics.
...
def main() -> None:
    ...
    result={'schema':'a2-v12-clean-build-v1','clean_auxiliary_state':True,
            'order':['two_collision','main'],'documents':{},
            'remote_ci':False,'formal_proof_verification':False}
    with tempfile.TemporaryDirectory(prefix='a2-v12-clean-') as tmp:
        work=Path(tmp)
        for p in root.rglob('*'):
        ...
        env['SOURCE_DATE_EPOCH']='1788958800';env['FORCE_SOURCE_DATE']='1'
        for stem in result['order']:
            cmd=['latexmk','-pdf','-pdflatex=pdflatex -no-shell-escape %O %S','-interaction=nonstopmode','-halt-on-error',stem+'.tex']
            ...
            expected_notices=[line for line in fatal if line=='Package epstopdf Warning: Shell escape feature is not enabled.']
            fatal=[line for line in fatal if line not in expected_notices]
            ...
            result['documents'][stem]={'command':cmd,'return_code':run.returncode,
                ...
                'expected_security_notices':expected_notices,
