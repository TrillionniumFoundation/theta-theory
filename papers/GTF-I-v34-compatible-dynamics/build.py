#!/usr/bin/env python3
"""Source-bound v34 build, with an optional small, archive-free core build."""
...
O=H.parent/'GTF-I-v33-compatible-lifts'
E=H/'evidence'
BASE='ee524c3e210ede7d6ff927c2bd2a2a1a104cb28c'
PREVIOUS='aab96108125317342e51e6a946efc4d778429338'
OLD_NAMES=['GTF-I-v33-compatible-lifts','GTF-I-v32-causal-width','GTF-I-v31-compatible-memory','GTF-I-v30-streaming-geometry',
...
def main()->None:
    ...
    names=['angular-linear','omit-moment-loss','signed-fejer','deficiency-factor',
    ...
    with tempfile.TemporaryDirectory(prefix='gtf34-latex-') as td:
        work=Path(td)
    ...
    must=['thm:main34','def:machine34','prop:separate34','lem:phase34',
          'lem:angular34','lem:moments34','lem:fejer34','thm:fourier34',
          'prop:terminal34','prop:upper34','prop:blackwell34','thm:diagram34',
          'prop:cyclic34','cor:autonomy34']
    ...
    receipt={'schema':'gtf34.build/1','core_only':args.core_only,
       ...
       'scope':'Executed finite algebra, labelled numerical diagnostics, source-bound build and preservation; not independent proof certification, sharp hidden-width determination, priority clearance or journal acceptance.'}
    ...
    (root/'GENERAL_THETA_FOUNDATIONS_I_V34_REVIEW_READY.md').write_text(
      '# General Theta Foundations I — Revision 34\n\n**Fourier Budgets for Hidden Causal Memory**\n\n'
      f'Native source commit: `{receipt["source_commit"]}`. Controlling r18: `{BASE}`.\n\n'
      f'[Focused English article ({pages} pages)](papers/{H.name}/paper.pdf) · [Native LaTeX](papers/{H.name}/main.tex) · [Response to r18](papers/{H.name}/RESPONSE_TO_REFEREE.md)\n\n'
      ...
      f'[Unchanged v33 supporting article](papers/{H.name}/supporting-results.pdf) · [Mathematical archive](papers/{H.name}/complete-manuscript.pdf) · [Development archive](papers/{H.name}/complete-development.pdf)\n\n'
      'All-hidden-state Fourier occupation budget; robust N^(1/5) lower bound at bounded-type angles; exact inherited N^(1/3) upper construction; logarithmic rational-angle lower bound; main-family stochastic-diagram deficiency obstruction.\n\n'
      'The hidden lower bound does not restrict to vector states or count projected vertices. The exponent gap 1/5 versus 1/3 remains. Blackwell/deficiency and symmetric lifts are credited. Fixed-length autonomy and anytime autonomy are separate tasks. Independent historical analytic gates are not claimed closed. Large cumulative PDFs are optional archives, not the compact referee package.\n')
