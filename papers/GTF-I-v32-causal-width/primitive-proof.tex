\section{The retained two-cut primitive}
\label{sec:primitive}
The following proof is retained from revision 31 \cite{GTF31}; its role
here is the input to Corollary~\ref{cor:pareto-sum}, not a new claim.
...
its attainability in dimension two is used. The required two-dimensional trace argument has been proved here;
its general-dimensional geometric background is recorded in \cite{GTF31}.

\begin{theorem}[Exact incompatible minima and their Pareto frontier]
\label{thm:obstruction}
For every $9/10\le h\le1$, both cuts of \eqref{eq:primitive}
...
second coordinates of \eqref{eq:primitive}. They form a
...
\end{equation}
and the fixed coordinates from \eqref{eq:primitive} enclose the
...
columns of \eqref{eq:primitive} unchanged and has last columns
