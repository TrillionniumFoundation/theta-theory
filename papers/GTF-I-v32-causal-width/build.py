#!/usr/bin/env python3
"""Source-bound v32 build, with an optional small, archive-free core build."""
...
O=H.parent/'GTF-I-v31-compatible-memory'
E=H/'evidence'
BASE='0e69fdcfc11067a7de9a6122fc603f6a5e2253d8'
PREVIOUS='90dc2c8f0c75e2cfbb95cc61b247ce4d5e216eb9'
OLD_NAMES=['GTF-I-v31-compatible-memory','GTF-I-v30-streaming-geometry',
...
def main()->None:
    ...
    names=['nonstochastic-row','unread-command','polygon-radius','unit-state-projection',
           'free-tag','separate-minima-splice','golden-net','volume-constant']
    controls=[]
    for mode in [[],['-O']]:
        for name in names:
    ...
    with tempfile.TemporaryDirectory(prefix='gtf32-latex-') as td:
        work=Path(td)
    ...
    must=['thm:headline','def:machine','lem:lift','thm:volume','cor:effective','prop:static-three',
          'thm:rotation-unbounded','thm:polygon-upper','thm:quantitative','thm:tagged-sum',
          'cor:pareto-sum','thm:obstruction']
    ...
    receipt={'schema':'gtf32.build/1','core_only':args.core_only,
       ...
       'scope':'Executed finite identities, source-bound build and preservation; not numerical solution of delta_M, independent proof certification, priority clearance or journal acceptance.'}
    ...
    (root/'GENERAL_THETA_FOUNDATIONS_I_V32_REVIEW_READY.md').write_text(
      '# General Theta Foundations I — Revision 32\n\n**Causal Width Beyond Cutwise Positive Rank**\n\n'
      f"Native source commit: `{receipt['source_commit']}`. Controlling r16 report: `{BASE}`.\n\n"
      f'[Focused English article ({pages} pages)](papers/{H.name}/paper.pdf) · [Native LaTeX](papers/{H.name}/main.tex) · [Response to r16](papers/{H.name}/RESPONSE_TO_REFEREE.md)\n\n'
      f'[Small referee package](papers/{H.name}/evidence/REFEREE_PACKAGE.zip) · [Core sources](papers/{H.name}/evidence/CORE_SOURCES.zip) · [Executed build receipt](papers/{H.name}/evidence/BUILD_RECEIPT.json)\n\n'
      f'[Unchanged v31 supporting article](papers/{H.name}/supporting-results.pdf) · [Complete mathematical archive](papers/{H.name}/complete-manuscript.pdf) · [Complete development archive](papers/{H.name}/complete-development.pdf)\n\n'
      'A fixed-alphabet uniformly full-support family has separate positive minimum three at every internal cut, but unbounded simultaneous clocked width. The all-hidden-state reachable-section volume budget gives an explicit logarithmic lower bound for a golden-angle rotation, with an exact square-root upper construction; a rational rotation gives effective algebraic excluded-horizon certificates. Tagged feasible profiles add, giving the exact growing frontier K1,K2>=3m and K1+K2>=7m.\n\n'
      'Stationary irrational-rotation obstructions, invariant cones, accessible quotients, and weighted reservoir sampling are credited. The width bounds are not sharp. The rational-angle numerical expansion constants, fully adaptive collision optimum and independent A2/B4/C2 closure are not claimed. The large cumulative PDFs are optional archives, not the compact referee submission.\n')
