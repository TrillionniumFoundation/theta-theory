...
menu. Choose the common normalization $H=16$ for all three total
horizons $n+2$, $n=1,2,3$, and fix $L\ge8$ once. At each fixed
calibration, the nine normalized future nodes and the matrix $F_L$
are therefore identical in the three problems. The nine singular
values satisfy, componentwise,
...
\end{corollary}
\begin{proof}
The common normalization is admissible for every horizon in question,
since $5\max_{|u|,|v|\le1/16}(3+v)=245/16<16$.
...
\]
They follow by balancing adjacent terms. These are crossover orders of
the comparison envelope, not exact optimizer transition thresholds. The relevant gaps are not
