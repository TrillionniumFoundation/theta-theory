#!/usr/bin/env python3
"""Run inherited author suites, v21 exact diagnostics, and the complete build."""
...
def main():
    ...
    report={'version':21,'python':platform.python_version(),
            'submission_basis':'6f103ad252d7c65f140720f4095585026f7bb1b9',
    ...
    for version in (*range(10,16),17,18,19,20,21):
    ...
    report['inherited_assertions_total']=sum(x['assertions'] for x in report['suites'] if x['version']<21)
    optimized=out/'V21_OPTIMIZED_RERUN.json'
    run([sys.executable,'-O','tests/verify_v21.py',str(optimized)],out/'v21-optimized-stdout.txt')
    if optimized.read_bytes()!=(out/'V21_AUTHOR_RERUN.json').read_bytes():
        raise ValueError('Optimized v21 diagnostic receipt differs')
    report['v21_optimized_identical_receipt']=True
