...
Theorem~\ref{thm:sharp-common-moments} gives a matching conclusion
throughout the collision chamber. For a dominated prior family with a
fixed interior margin, and a common moment name with a fixed accuracy
slack, the minimax regret is of order $\Xi_N(M,a)+\delta^2$ at every
calibration. Its converse constructs genuine compatible priors at that
calibration and uses positive likelihoods to control the overlap of
acquired-history laws. The construction serves all compatible
calibrations and priors with one program. Thus no equality pattern is
revealed either by the data or by a hidden choice of program.
Corollary~\ref{cor:advice-saturation} identifies the corresponding
threshold as
\[
 \mathcal M_\delta(a)=\max_{n,\ell}
       \mathcal V_{N-n,\ell}(a)\delta^{-\ell}.
\]
Every nonzero exterior-volume term contributes to this threshold; it is
not determined by the rank or by the separated-chain floor alone.
The interior and accuracy margins are stated explicitly, since a
boundary consistency class can be degenerate. The result is a minimax
law for imperfect model descriptions, not a precision lower bound for
programs provided with different exact priors.

For the intersecting two-parameter arrangement, this threshold has the
three terms $\delta^{-6}$, $\rho^2\delta^{-8}$, and
$\rho^2\tau\delta^{-9}$, where $\rho$ measures distance to the
intersection and $\tau$ distance to the nearest collision line.
Corollary~\ref{cor:uncertain-intersections} gives the complete statement,
including arbitrarily high contact with a collision line.
