...
\[
 ...
 \frac{\partial y}{\partial u}=\frac{32(1-x)}{(8+3u)^2}.
\]
Thus the failure-weighted joint density in $(x,u)$ is $1024(1-x)^2/(8+3u)^3$. At fixed $u$ the admissible $x$ intervals are
\[
 \begin{cases}
 [(2-29u/4)/(8-5u),\,3/4],&-8/7\le u\le0,\\
 [1/4,\,(24-23u)/(4(8-5u))],&0\le u\le8/9.
 \end{cases}
\]
Integrating in $x$ gives the unconditional subprobability density
\begin{equation}\label{eq:v29-density}
 f(z)=
 \begin{cases}
 512\left(\dfrac{27}{(8-5u)^3}-\dfrac1{(8+3u)^3}\right),&-8/7\le u\le0,\\[6pt]
 512\left(\dfrac{27}{(8+3u)^3}-\dfrac1{(8-5u)^3}\right),&0\le u\le8/9,\\[6pt]
 0,&\text{otherwise},
 \end{cases}
 \qquad u=48(z-1/2).
\end{equation}
