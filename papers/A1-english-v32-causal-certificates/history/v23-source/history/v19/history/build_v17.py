...
def prepare():
    ...
    v15_manifest=json.loads((ROOT/'V15_PRESERVATION_MANIFEST.json').read_text())
    ...
    v16_manifest=json.loads((ROOT/'V16_PRESERVATION_MANIFEST.json').read_text())
    v16=Counter(v16_manifest['proof_sha256'])
    v16_statements=Counter(v16_manifest['statement_sha256'])
    if sum(v16.values()) != 84 or sum(v16_statements.values()) != 87:
        raise ValueError('Unexpected pinned v16 block counts')
    if v16-current:raise ValueError('A complete v16 proof was changed or removed')
    if v16_statements-statements:raise ValueError('A complete v16 statement was changed or removed')
    if set(v16_manifest['named_results'])-set(labels):
        raise ValueError('A v16 named result is absent')
    if sum(current.values()) != 88 or sum(statements.values()) != 91:
        raise ValueError('The four new complete results and proofs are not all compiled')
    ...
    report.update(retained_v16_proofs_byte_identical=sum(v16.values()),
                  retained_v16_statements_byte_identical=sum(v16_statements.values()),
                  retained_v16_named_results=len(v16_manifest['named_results']),
                  compiled_statement_blocks=sum(statements.values()),
                  new_complete_proof_blocks=sum((current-v16).values()),
                  new_complete_statement_blocks=sum((statements-v16_statements).values()))
