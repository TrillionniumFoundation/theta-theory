...
edge_counts={k:0 for k in ('new_edges','v12_edges','v13_edges','v14_edges')}
for c in g['components']:
    need(c['inherited_status']==expected[c['component']]['status'],'Inherited status altered')
    need(c['full_historical_target_closed_by_v14'] is False,'Unsupported full historical closure')
...
need(g['base_commit']=='e8a5354659a65c3c909cd927c2f4f628ea1b1a6c','Unexpected v13 parent')
ancestor=subprocess.run(['git','merge-base','--is-ancestor',review['commit'],g['base_commit']],cwd=ROOT,capture_output=True)
if os.environ.get('GITHUB_ACTIONS')=='true':need(ancestor.returncode==0,'Controlling review not an ancestor of the v13 parent')
