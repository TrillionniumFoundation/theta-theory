#!/usr/bin/env python3
"""Source-bound v41 build, with an optional small, archive-free core build."""
...
O=H.parent/'GTF-I-v39-orbit-response'
E=H/'evidence'
BASE='9707844addcd9eb478eb863514f288923f9d40fd'
PREVIOUS='28890c62dd69f217bf2f1c205ff9542eb537c2ef'
OLD_NAMES=['GTF-I-v39-orbit-response','GTF-I-v38-orbit-entropy','GTF-I-v37-spectral-memory','GTF-I-v36-finite-alphabet-memory','GTF-I-v35-sharp-memory','GTF-I-v34-compatible-dynamics','GTF-I-v33-compatible-lifts','GTF-I-v32-causal-width','GTF-I-v31-compatible-memory','GTF-I-v30-streaming-geometry',
...
def main()->None:
    ...
    names=['nilpotent-adjoint-dimension','missing-special-unitary-constraint','highest-weight-always-minimal',
           'isotypic-trace-instead-of-top-eigenvalue','coordinate-copy-is-canonical','free-query-norm',
           'unpaid-branch-amplification','minimum-over-effective-group','conflate-primary-versions']
    controls=[]
    for mode in [[],['-O']]:
        for name in names:
    ...
    with tempfile.TemporaryDirectory(prefix='gtf41-latex-') as td:
        work=Path(td)
    ...
    must += ['prop:action41','prop:activation41','thm:intrinsic-width41','cor:error41','prop:mixed41',
             'thm:adjoint41','prop:standard41','thm:exterior41']
    ...
    receipt={'schema':'gtf41.build/1','core_only':args.core_only,
    ...
    (root/'GENERAL_THETA_FOUNDATIONS_I_V41_REVIEW_READY.md').write_text(
      '# General Theta Foundations I — Revision 41\n\n**Intrinsic Action Gaps and Numerical Memory**\n\n'
      f'Native source commit: `{receipt["source_commit"]}`. Controlling r25: `{BASE}`.\n\n'
      f'[English article ({pages} pages)](papers/{H.name}/paper.pdf) · [Native LaTeX](papers/{H.name}/main.tex) · [Response to r25](papers/{H.name}/RESPONSE_TO_REFEREE.md)\n\n'
      ...
      f'[Unchanged v39 article](papers/{H.name}/supporting-results.pdf) · [Mathematical archive](papers/{H.name}/complete-manuscript.pdf) · [Development archive](papers/{H.name}/complete-development.pdf)\n\n'
      'Intrinsic sphere/Lebesgue action gap; canonical isotypic activation with commutant spectral formula; arbitrary finite seeds; dominant-only expansion and matching width; a faithful ten-command rational SO(3) x SO(2) experiment without a full group gap and with Theta(N) width.\n\n'
      'Minimum orbits are computed for every compact simple adjoint type and the stated classical defining/spin and second exterior-power families. Chen–Wu v1 and v2 are separately primary-source verified and pinned. Existing v40 work branches and all older paths are untouched.\n\n'
      'The model remains nonuniform clocked atomic-row label width. General highest-weight orbit classification, finite integer optima and the original-page LPS audit are not claimed. No independent historical analytic gate or journal outcome is inferred. Large archives are optional.\n')
