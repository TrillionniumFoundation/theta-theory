...
The finite-horizon question has two logically separate parts: whether a
proposed machine realizes every word, and how close the best machine
of a prescribed profile can be. Section~\ref{sec:cert46} treats both
with exact finite certificates. The all-word Gram identity checks
numerical equality rather than average agreement; a backward recursion
extracts a failed word when equality does not hold. It yields a
polynomial-size existential real formulation of exact feasibility.
The even-moment hierarchy and a rational grid theorem then bound the
actual optimized terminal error from both sides. Their convergence
is a theorem about all legal hidden machines, whereas the accompanying
program checks given rational realizations and small complete grids.
These tasks must not be conflated.

The ingredients are deliberately distinguished from their use here.
Polynomial-time equivalence of probabilistic automata is classical
\cite{Tzeng92}; finite Gram and tensor manipulations are standard in
weighted-automata theory \cite{BPP15}. Real quantifier elimination gives
effectiveness, not a practical generic minimizer \cite{BPR}. The
categorical-output comparison imports the known nonnegative-rank
complexity and field distinction \cite{Shitov18}; it is not a hardness
theorem for the fixed planar binary-query subclass. The finite
certificate formulation adds neither an unpriced state nor an extra
query to the numerical experiment.

Three previously established components are retained. First,
...
arithmetic laws are inherited \cite{GTF35,GTF42,GTF43}. The distortion-rate comparison and the uniform finite-bit
realization with its charged resource ledger were established in the
preceding revision. The present extension is the finite all-word and
actual-error certificate framework of Section~\ref{sec:cert46}. Dirichlet approximation,
