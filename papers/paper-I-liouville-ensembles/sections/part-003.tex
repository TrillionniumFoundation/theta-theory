...
Zero asymptotic variance would make $J_R-\int J_Rd\mu_{R,q}$ a measurable
coboundary, hence every periodic orbit would have the same average by the
Liv\v{s}ic periodic-orbit criterion.  The translating orbit of
\cref{lem:traveling-orbit}, its reverse, and a normal period-two orbit have
distinct averages.  Therefore the variance is positive for every real $q$ in
the spectral window.  Strict convexity and Legendre duality give the rest.
\end{proof}
...
The span is one because the translating periodic orbit has current sum one.
