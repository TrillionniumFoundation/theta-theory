#!/usr/bin/env python3
"""Source-bound v42 build, with an optional small, archive-free core build."""
...
O=H.parent/'GTF-I-v41-intrinsic-action'
E=H/'evidence'
BASE='fdc5acf7e7913dcfc379f5f19d5c97ab21306496'
PREVIOUS='08773b2705c5ac92716df5daf42a54f3b61bca34'
OLD_NAMES=['GTF-I-v41-intrinsic-action','GTF-I-v39-orbit-response','GTF-I-v38-orbit-entropy','GTF-I-v37-spectral-memory','GTF-I-v36-finite-alphabet-memory','GTF-I-v35-sharp-memory','GTF-I-v34-compatible-dynamics','GTF-I-v33-compatible-lifts','GTF-I-v32-causal-width','GTF-I-v31-compatible-memory','GTF-I-v30-streaming-geometry',
...
def main()->None:
    ...
    for name,value in manifest['byte_identical_analytic_modules'].items():
        require(sha(H/name)==value,'Changed inherited analytic module '+name)
    ...
    names=['one-step-is-expanding','drop-packet-commands','merge-packet-products','free-query-norm',
           'drop-copy-rank','unpaid-component','omit-polygon-dilation','duplicate-query-gain']
    controls=[]
    for mode in [[],['-O']]:
        for name in names:
    ...
    with tempfile.TemporaryDirectory(prefix='gtf42-latex-') as td:
        work=Path(td)
    ...
    must=['thm:main','def:machine','prop:law','prop:gap-dual','thm:block-gap',
          'thm:calibration','prop:calibration-sdp','prop:calibration-examples','thm:frame',
          'thm:dominant-block','thm:binary-example','lem:packet42','lem:packet-separation',
          'prop:toral-lower','lem:denominator','prop:toral-upper','prop:zero-blocks',
          'prop:algebraic-angles','prop:components','prop:quotients',
          'thm:uniform-orbits','thm:occupation','thm:orbit-upper']
    ...
    receipt={'schema':'gtf42.build/1','core_only':args.core_only,
    ...
    with zipfile.ZipFile(E/'REFEREE_PACKAGE.zip','w',zipfile.ZIP_DEFLATED) as z:
        for p in [H/'paper.pdf',H/'RESPONSE_TO_REFEREE.md',H/'LITERATURE_AUDIT.md',
                  E/'CORE_SOURCES.zip',E/'THEOREM_LOCATIONS.json',E/'BUILD_RECEIPT.json',H/'GAP_CERTIFICATE.json',H/'PRIMARY_SOURCE_AUDIT.json']:
            z.write(p,arcname=p.name)
    root=H.parent.parent if H.parent.name=='papers' else H.parent
    (root/'GENERAL_THETA_FOUNDATIONS_I_V42_REVIEW_READY.md').write_text(
      '# General Theta Foundations I — Revision 42\n\n**Alphabet Dynamics and Calibrated Numerical Memory**\n\n'
      f'Native source commit: `{receipt["source_commit"]}`. Controlling independent r27: `{BASE}`.\n\n'
      f'[English article ({pages} pages)](papers/{H.name}/paper.pdf) · [Native LaTeX](papers/{H.name}/main.tex) · [Response to r27](papers/{H.name}/RESPONSE_TO_REFEREE.md)\n\n'
      ...
      f'[Unchanged v41 article](papers/{H.name}/supporting-results.pdf) · [Mathematical archive](papers/{H.name}/complete-manuscript.pdf) · [Development archive](papers/{H.name}/complete-development.pdf)\n\n'
      'Nongapped r-angle alphabets have matched Theta(N^(r/(2r+1))) hidden width under the stated dual badly approximable condition, including fixed small wordwise error and an O(2) reflection extension. Every finite-block gap is zero.\n\n'
      'Optimized alphabet/block gaps, a rational binary SO(3) one-step/two-step separation, exact algebraic query calibration, SDP certificates, frame stability and paid finite components are proved. Three inherited analytic modules are byte-identical.\n\n'
      'The rational SO(3) example uses a qualitative external gap theorem; finite checks do not compute it. The model is nonuniform clocked atomic-row label width. General nongapped classification, finite exact optima, original-page LPS verification and unrelated analytic closure are not inferred. Older paths and the separate pre-existing v42 work branch are untouched. Large archives are optional.\n')
