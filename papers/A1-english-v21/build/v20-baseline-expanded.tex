...
The affine hypothesis can be removed altogether. The appropriate invariant
is a family of maximal Vandermonde products on the future exponent set.
It is defined at a collision itself, does not require a chosen path into
that collision, and determines both attainable checkpoint resolution and
causal memory. We first establish a uniformly conditioned Newton flag,
then prove its attainable transversality. The covering and causal arguments
are given separately, so that a local flag is never substituted for a
global description of the reachable set.
...
\end{equation}
The first condition is equivalent to $0\in\operatorname{ri}K_U$ in
Theorem~\ref{thm:v19-rank-alternative}; it permits the definition of the
pencil in \eqref{eq:kernel-pencil}. The second condition says that at
...
\end{theorem}
\begin{proof}
If $\mu\in\mathcal P_U$, a nonnegative nonzero continuous function
in $W_U$ would have strictly positive integral, a contradiction.
Conversely, if $0\notin\operatorname{ri}K_U$, finite-dimensional
separation, as in Theorem~\ref{thm:v19-rectangular}, gives a linear
combination of the coordinates of $G_U$ which is nonnegative on $X$
and nonzero somewhere. This is a nonzero member of $W_U\cap C(X)_+$.
Thus the first condition is equivalent to $0\in\operatorname{ri}K_U$.
Theorem~\ref{thm:v19-rank-alternative} then gives $\mathcal P_U\ne
\varnothing$. In particular it also gives $1\notin W_U$.
