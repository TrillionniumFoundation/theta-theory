...
The long-bridge construction has a different purpose from those finite
inverses: the identical-even germ is already identifiable at one flight,
and the count experiment uses only three or four flight numbers. Its
additional content is a relative approximation of physical experiments
whose bridge lengths and sample sizes both grow. Section~\ref{sec:v7-critical}
identifies a nontrivial testing profile in a positive-offset joint limit;
Section~\ref{sec:v7-bracket} separately acquires the coarse count bracket
from a fixed local interval. These are distinct consequences, not an
assertion that longer bridges add otherwise absent contact coordinates.
