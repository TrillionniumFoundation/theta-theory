#!/usr/bin/env python3
"""Clean-build the A2 v7 article and companion; record actual diagnostics.
...
def main() -> None:
    ...
    out=root/'verification-v7';out.mkdir(exist_ok=True)
    result={'schema':'a2-v7-clean-build-v1','clean_auxiliary_state':True,
            'order':['two_collision','main'],'documents':{},
            'remote_ci':False,'formal_proof_verification':False}
    with tempfile.TemporaryDirectory(prefix='a2-v7-clean-') as tmp:
        work=Path(tmp)
        for p in root.rglob('*'):
        ...
        for stem in result['order']:
            ...
            if run.returncode:
                raise RuntimeError(f'{stem}: build failed; inspect verification-v7/{stem}.build.txt')
