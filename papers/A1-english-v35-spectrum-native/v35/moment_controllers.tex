...
The proposition identifies the postprocessing region. The next lemma
supplies its compactness and explicit support function; the corollary
then excludes arbitrary stochastic rows from the controller domain.
...
The equivalence in \eqref{eq:v33-realize-row} is the integral
identity proved in Proposition~\ref{prop:v34-blackwell}. For the
support function, the accepted terms are maximized
...
\end{corollary}
\begin{proof}
For distinct rows, Proposition~\ref{prop:v34-blackwell} gives the
first inequality by preservation of the common submeasure
$\lambda_{kl}$. For equal rows the left side is one. Since
$1-g_k\ge\eta$, the lower bound is at least $\eta$ in both cases.
Two disjoint point-mass rows violate this bound when $J,M\ge2$,
which proves proper inclusion.
\end{proof}
