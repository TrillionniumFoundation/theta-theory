#!/usr/bin/env python3
"""Execute the v14 author checks and full build, with optional pinned review probe.

No neighboring source folder is required for the standalone v14 validation.
The optional review probe deliberately uses the unchanged v13 source, never
...
EXPECTED = {10: 7904, 11: 8207, 12: 26158, 13: 12944, 14: 7400}
...
def main():
    ...
    report = {'version': 14, 'python': platform.python_version(),
              'source_files_verified': verify(),
              'controlling_review': '00b0844a7c2fb16b3231abf163d5004fd9e253cd',
              'submission_basis': 'fc6465b86fbc7ee6a4e8f3ccfb54ea32a64dc8b6',
    ...
    old = json.loads((ROOT/'history/V13_SOURCE_MANIFEST.json').read_text())
    old_files = old['sha256']
    unchanged = ['finite_compiler.py','certified_compiler.py','construction_contracts.py']
    unchanged += [f'tests/test_v{n}.py' for n in range(10,14)]
    ...
    for version,count in EXPECTED.items():
        name = f'V{version}_AUTHOR_RERUN.json' if version < 14 else 'V14_DIRECTIONAL_DIAGNOSTICS.json'
        target = out/name
    ...
    report['unchanged_author_suite_total'] = sum(EXPECTED[n] for n in range(10,14))
    report['new_author_diagnostic_total'] = EXPECTED[14]
    report['author_assertion_total'] = sum(EXPECTED.values())
    if args.prior_review:
        repo = ROOT.parents[1]
        script = repo/'reviews/a1-english-v13-common-moment-2026-09-06/reproduce_review.py'
        baseline = ROOT.parent/'A1-english-v13'
        ...
        elapsed = run([sys.executable,str(script),'--source-root',str(baseline),'--output',str(target)],out/'prior-review-stdout.txt')
        data = json.loads(target.read_text())
        if data['assertions'] != 4909 or data['status'] != 'passed':
            raise ValueError('Unexpected original v13 review-probe outcome')
        report['prior_review_probe'] = {'source_version':13,'assertions':data['assertions'],
            'passed':True,'seconds':elapsed,'result_sha256':digest(target),
            'scope':'Original independent-review probe rerun on its unchanged submission, not a new independent review of v14.'}
