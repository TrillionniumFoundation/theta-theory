...
def main()->None:
    ...
    if not args.skip_diagnostics:
        ...
        for edition,path in [('v10','GTF-I-v10-marked-duality/verify.py'),('v9','GTF-I-v9-causal-minimax/verify.py'),('v8','GTF-I-v8-decision-spectrum/verify.py'),('v7','GTF-I-v7-structural/verify.py'),('v6','GTF-I-v6-markov/verify.py'),('v5','GTF-I-v5-intrinsic/verify.py'),('v4','GTF-I-v4/verify.py'),('v3','GTF-I-v3/verify.py'),('v2','GTF-I-v2/verify.py'),('v1','GTF-I-v2/legacy/tools/verify.py')]:
    ...
    with tempfile.TemporaryDirectory(prefix='gtf-v11-build-') as work:
        work=Path(work)
    ...
    require(set(predecessor)<=set(maps['complete-development']),'Missing v10 companion labels: '+str(set(predecessor)-set(maps['complete-development'])))
    for rel,digest in {**inherited,**new}.items():
    ...
    receipt={'edition':'General Theta Foundations I, eleventh resource-comparison revision','review_commit':REVIEW,'source_commit':source_commit,'checkout_kind':checkout_kind,
             ...
             'source_manifest_sha256':sha(HERE/'SOURCE_MANIFEST.json'),'preserved_v10_companion_labels':len(predecessor),'retained_labels_resolved':len(retained),'shared_canonical_labels_same_numbers':len(core),
