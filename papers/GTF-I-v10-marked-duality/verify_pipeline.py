...
def validate(contract:dict)->dict:
    review=contract['controlling_review'];data=(REPO/review['path']).read_bytes()
    require(hashlib.sha1(('blob %d\0'%len(data)).encode()+data).hexdigest()==review['git_blob'],'Controlling report blob changed')
    ...
    labels=set()
    source_paths={**json.loads((HERE/'INHERITED_INPUTS.json').read_text())['files'], **json.loads((HERE/'SOURCE_MANIFEST.json').read_text())['files']}
    for path in (REPO/rel for rel in source_paths if rel.endswith('.tex')):
        labels.update(re.findall(r'\\label\{([^}]+)\}',path.read_text(errors='replace')))
    for row in rows:
        require(row['status'] in {'verified_protocol_adapter','conditional_interface_only','independent_not_consumed','verified_model_scoped_consumer'},'Unknown dependency status')
        ...
        require(row['full_historical_target_closed_by_v10'] is False,'Model-scoped proof falsely closes the entire historical target')
        if row['status']=='verified_model_scoped_consumer':
            require(row['component'] in {'A4','C1','B4','D1'},'Unregistered model consumer')
            require(bool(row['model_scope']['hypotheses']) and bool(row['model_scope']['transported_quantity']),'Missing model-scoped hypothesis/conclusion')
            require((REPO/row['model_scope']['proof_source']).is_file(),'Missing model proof')
    ...
    return {'status':'passed','components':len(rows),'verified_protocol_adapters':sum(r['status']=='verified_protocol_adapter' for r in rows),'verified_model_scoped_consumers':sum(r['status']=='verified_model_scoped_consumer' for r in rows),'other_conditional_interfaces':sum(r['status']=='conditional_interface_only' for r in rows),'historical_model_gates_imported':0,'a2_protocol_blob_checked':True,'moved_ref_negative_control':'rejected','live_heads_checked':False}
