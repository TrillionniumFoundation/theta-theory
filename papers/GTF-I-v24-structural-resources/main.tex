...
\newcommand{\Qstar}{Q_*}
\title[Statistical resource duality]{General Theta Foundations I:\\
Statistical Resource Duality and Two-Preparation Physical Certification}
\author{Qian Qi}
\date{September 24, 2026. Revision 24}
...
\begin{document}
\begin{abstract}
We study the resources needed to distinguish a marked experiment from a
family of private causal simulators. We separate the optimization of one
finite architecture from the optimization over all admissible architectures.
For reset experiments whose feedback rows are garblings of a common row,
we express the unrestricted-memory preparation value as a minimum of
posterior-predictive total variations. The resulting dual bounds every
adaptive finite-memory architecture, without selecting an architecture in
advance. For the positive-noise marked collision experiment, a four-point
rational dual certificate excludes all auditors with at most one training
preparation. A deterministic auditor with two training preparations attains
a strictly larger uniform score. Its complete bit-level register profile
has peak twelve, proving that two training preparations are necessary and
sufficient at every peak budget at least twelve on an explicit interval of
score thresholds. For general finite architectures we give subdivided
positive-basis certificates of explicitly bounded degree and error,
including reused stochastic rows. We also give a fully priced autonomous
binary-testing construction, with the stationary bound attributed to
Hellman and Cover. The earlier compatible-occupation, noisy-testing,
continuation and physical-comparison developments are retained in the
accompanying complete mathematical volumes.
\end{abstract}
\maketitle
\tableofcontents

\section{Introduction}
An exact description of finite stochastic controllers does not by itself
determine a resource frontier. There are two distinct questions. The first
asks for the best controller within a specified architecture. The second
asks whether any architecture satisfying a resource budget can perform a
specified statistical task. A lower bound for the first question need not
answer the second. We develop certificates that respect this distinction.

The principal application is a marked collision experiment. Its ideal
all-on law is supported on four atoms, but neither the simulator nor the
auditor is restricted to a four-atom transcript. The simulator produces two
independent report bits and an independent actual mark; the target has a
correlation between its reports and that mark. The auditor may use the
original feedback rows, arbitrary stochastic transitions and stopping
rules, subject to the reset and validation interface. Our upper certificate
for the auditor is valid simultaneously for all such architectures. The
matching construction uses the mark during validation and not during
training. This distinction permits a much shorter preparation schedule
than an audit based on unmarked diagonal events.
...
\end{equation}
The main resource conclusion is
\begin{equation}\label{eq:headline}
 N_c^{\rm phys}(W,Q)=2\quad\text{if}\quad
 W\ge12,\quad \TV(Q,\Qstar)\le\beta,\quad
 u_1+\beta\le c<m_2-\beta.
\end{equation}
Here $N$ counts candidate training preparations; two additional, fresh
validation preparations are charged separately. The objective is a strict
expected signed score greater than $c$. In particular, \eqref{eq:headline}
applies to $c=2/5$ and $\beta=\beta_0$. The lower bound permits unlimited
memory, whereas the construction has peak width twelve. This is an exact
preparation boundary on a high-width segment, not a claim that twelve is
the least possible peak width.

Three mechanisms organize the proof. First, a dominating experimental row
removes the outer choice of feedback schedules from the converse. Second,
minimax duality turns a prior on fixed simulator parameters into a
posterior-predictive upper certificate. Third, residual continuation
functions compile the winning decision into a compatible small-memory
machine. The general algebraic encoding is useful for checking that the
compiled machine belongs to the intended class, but no generic
Positivstellensatz is needed to prove \eqref{eq:headline}.

For finite architectures we also supply quantitative information not
contained in eventual sum-of-squares completeness. A partition of the
stochastic-row simplices into cells of diameter $O(m^{-1})$ gives
positive-basis certificates with error at most $HS/m$, where $H$ bounds
the number of row uses and $S$ bounds the number of destinations in a
row. The block degree is the maximum number of times the corresponding
row can be used. The argument works with autonomous row reuse. It gives
an effective, generally exponential, certification procedure; it is not a
uniform degree bound for the unmodified Putinar hierarchy.

\subsection{Relation to the literature}
Comparison of experiments and minimax theory provide the classical
background \cite{Blackwell,Sion}. Polynomial feasible-frequency
formulations for partially observed stochastic control are treated by
M\"uller and Mont\'ufar \cite{MM}; the rank-one compatibility constraints
below are not presented as a new general conditional-independence device.
Likewise, strict positivity certificates use established real algebraic
geometry \cite{Putinar}. Our quantitative subdivision theorem is a
positive-basis construction with a causal error estimate, rather than a
new general Positivstellensatz.

Finite-state testing has a substantial earlier theory. Hellman and Cover
\cite{HC,HCrandom}, Flower and Hellman \cite{FH}, and Cover, Freedman and
Hellman \cite{CFH} distinguish time-invariant rules, finite horizons and
randomization. We explicitly identify the stationary result in Section~
\ref{sec:autonomous} as a specialization of that theory. Managoli and
Prabhakaran \cite{MP}, posted in May 2026, study randomized time-invariant
finite-state binary testing against adaptive adversarial distribution
choices and establish matching state exponents, with exact bounds for a
class of problems. Our physical theorem has a different objective: a
finite training--validation signed score against a fixed private simulator,
with bit-level memory and preparations charged. We make no priority claim
for the general phenomenon that randomization can save states.

Norberg's filtered comparison of experiments \cite{Norberg} is pertinent
to the broader causal comparison program. The original proof-level
comparison remains incomplete: the accessible publisher record verifies
the bibliographic identity but does not justify an assertion that its
results exclude the present constructions. None of our proofs uses such
an exclusion.

\subsection{The mathematical volumes}
The present article gives the new proof chain. The accompanying
\emph{Complete Mathematical Manuscript} contains this article followed by
the unaltered 96-page revision 23. The \emph{Complete Development} contains
this article followed by the unaltered 649-page development. Thus the
earlier general continuation, occupation, noisy binary, physical,
Gaussian and operator interfaces remain available with their full
hypotheses and proofs \cite{Companion}. They are not silently replaced by
\eqref{eq:headline}. In particular, the independent A2 geometric chain and
the unresolved historical B4 and broad C2 obligations remain distinct.
The mathematical role of the present first foundation is an experimental
and resource-comparison root; a universal derivation of every program
...
\cite[physical-complexity, feedback-completion lemma]{Companion}, explain
...
$9/10\le d\le11/10$ and $0<\sigma\le1/24$, the inherited microscopic
...
positive sensor noise, is proved in the retained microscopic-composition
and physical-complexity modules \cite{Companion}; the current deduction
...
\bibitem{FH} W. C. Flower and M. E. Hellman, Hypothesis testing with
