...
paper_I:
  ...
  actual_nonzero_witness: v68_v69_moving_seam_family
  product_CM2: CLOSED
  finite_DQ_l1: CLOSED
...
paper_II:
  ...
  common_operator_space: CLOSED_BY_FINITE_ATLAS_STABILIZATION
...
paper_III:
  ...
  full_scale_nonautonomous_limit: CLOSED_WITH_COMPATIBLE_WIP_MODULUS
  qualitative_uniform_WIP_only: SUBSEQUENTIAL_DIAGONAL_RESULT
  HJB_limit: CLOSED_RELATIVE_TO_DPP_AND_COMPARISON
  theta_semigroup: CLOSED
  nonconvex_non_subadditive_example: CLOSED

paper_IV:
  title: Filtering and Isaacs limits
  bounded_filter_contraction: CLOSED
  weighted_filter_branch: CLOSED_RELATIVE_TO_LYAPUNOV_MOMENT_AND_COMPARISON_PACKET
...
1. an actual nonconjugate specular finite-horizon Sinai family satisfying the
   complete Paper-I packet through third order;
...
3. optimal quantitative enhanced-WIP rates rather than a compatible modulus;
4. pure-strategy Isaacs saddles beyond the mixed relaxed theorem;
5. external expert review, novelty review, bibliography completion, and final
   journal-facing compression.
...
Every paper imports only named theorems from the immediately upstream paper.
No paper may import the old blanket label `thm:main_response_package`.
Downstream manuscripts must name the exact packet or theorem UID used.
