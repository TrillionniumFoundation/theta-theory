...
and only then is compactness used to take a finite cover. A sufficiently small
nonconjugate radial interval contained in one strict witness neighbourhood is
an actual compact uniform high-frequency family.
...
## Finding S3 — uniform BDL and gauge response were overread as table response

A family-uniform bound for `(z-X_a)^{-1}` does not differentiate the unbounded
moving-domain operator in `a`. Likewise, exact-coboundary gauge conjugacy gives
response in the twist parameter `q`, not automatically in the table parameter
`a`.

### Repair

The table-parameter theorem now requires `BDL-PARAMETER-DOMAIN-v1`:

```text
one common dense graph core
graph-topology C^k dependence of X_a
complete graded generator letters
resolvent domain preservation
graph-topology difference-quotient convergence.
```

Only under this packet are the ordered-composition formulas asserted:
...
For the radial exact flow coboundary, gauge conjugacy proves arbitrary finite
`q`-derivatives uniformly over the compact family. It is explicitly forbidden
to promote these to `a`-derivatives without the graph-domain packet.

---

## Finding S4 — “optimal WIP rate” was over-typed twice

No exponent is optimal simultaneously in endpoint, uniform, Hölder,
fractional-Sobolev, and rough-path metrics. In addition, the quantitative
...
Stein--Dirichlet class `Sigma_(eta,p)`. The law distance is
...
The scaled midpoint directions have uniform Cameron--Martin norm and disjoint
supports, so the required derivative bounds are uniform. The test vanishes on
every affine mesh walk, while Brownian midpoint defects give
...
Thus the rates match in `d_SD^(eta,p)`. The full bounded-Lipschitz
Kantorovich--Rubinstein exact rate is explicitly not claimed.
...
## Finding S5 — general pure-strategy Isaacs is false
...
four-branch saddle formulas were independently solved:
...
## Finding S6 — the first weighted/path construction retained incompatible fast state
...
contains the horizontal shift generator:
...
substantive_scope_or_formula_repairs: 7
actual_positive_systems:
  - nonconjugate_radial_specular_invariant_and_coboundary_U_infinity
  - nonconjugate_radial_compact_uniform_high_frequency_BDL_family
...
packet_theorem_closures:
  - generic_bilateral_specular_source_packet
  - BDL_FAMILY_WITNESS_v1
  - BDL_PARAMETER_DOMAIN_v1
  - Stein_Dirichlet_optimal_rate_metric
  - pure_Isaacs_equivalence_and_selector
  - weighted_filter_and_segment_PPDE_packets
