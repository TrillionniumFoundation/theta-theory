#!/usr/bin/env python3
"""Source-bound v33 build, with an optional small, archive-free core build."""
...
O=H.parent/'GTF-I-v32-causal-width'
E=H/'evidence'
BASE='eaa2661530ed473f8b79407eba7a1182c8e0a450'
PREVIOUS='d38fca741135d147dfdbdbd99cfd9a1a716a88a5'
OLD_NAMES=['GTF-I-v32-causal-width','GTF-I-v31-compatible-memory','GTF-I-v30-streaming-geometry',
...
def main()->None:
    ...
    names=['ambient-extension','face-count-as-K','mean-constant','profile-constant',
    ...
    with tempfile.TemporaryDirectory(prefix='gtf33-latex-') as td:
        work=Path(td)
    ...
    must=['thm:main','def:machine','def:vector','prop:separate','lem:reachable','prop:slack',
          'thm:extension-dual','prop:extension-gap','def:dynamic','thm:lift-equivalence',
          'thm:mean-growth','thm:profiles','lem:resonance','thm:upper','cor:contraction',
          'thm:rational','def:anytime','thm:autonomous']
    ...
    receipt={'schema':'gtf33.build/1','core_only':args.core_only,
       ...
       'scope':'Executed finite identities, source-bound build and preservation; not a solution of optimal hidden width, independent proof certification, priority clearance or journal acceptance.'}
    ...
    (root/'GENERAL_THETA_FOUNDATIONS_I_V33_REVIEW_READY.md').write_text(
      '# General Theta Foundations I — Revision 33\n\n**Compatible Polyhedral Lifts and Quantitative Causal Width**\n\n'
      f"Native source commit: `{receipt['source_commit']}`. Controlling r17 review: `{BASE}`.\n\n"
      f'[English article ({pages} pages)](papers/{H.name}/paper.pdf) · [Native LaTeX](papers/{H.name}/main.tex) · [Response to r17](papers/{H.name}/RESPONSE_TO_REFEREE.md)\n\n'
      ...
      f'[Unchanged v32 supporting article](papers/{H.name}/supporting-results.pdf) · [Mathematical archive](papers/{H.name}/complete-manuscript.pdf) · [Development archive](papers/{H.name}/complete-development.pdf)\n\n'
      'Compatible stochastic extensions and a sharp 1/4 extension defect; mean-support profile budgets; exact cube-root order for vector-state realizations at bounded-type angles; sharper general hidden lower and upper bounds; an explicit rational-angle rate; linear anytime autonomous order.\n\n'
      'The general hidden problem remains between logarithmic and cube-root width. The vector-state equality is not substituted for that problem. The autonomous theorem has one fixed decoder at all stopping lengths, a stronger specification. Static extension complexity and classical positive-realization precedents are credited. Historical analytic closure and exhaustive priority certification are not claimed.\n')
