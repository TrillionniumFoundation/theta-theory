...
from construction_contracts import inspect_construction, bind_request
...
def adaptive_compile(horizon: int, reports: int, budget: int, A: F, factory: Callable,
    ...
    for b in range(max_bits+1):
        ...
        request, name = bind_request(horizon,count,reports,budget,b+2,evaluate,predict,
                                     h/2,h,max_candidates,
                                     state_dimensions=state_dimensions,query_counts=query_counts)
        program,audit = compile_tables(horizon,count,reports,budget,b+2,
                                       name.evaluate,name.predict,max_candidates)
        contract = inspect_construction(program,audit,count,reports,budget,
                         name.evaluate,name.predict,max_candidates,request=request).require()
        rs = contract.radii
        r = max(rs[1:])
        lower,upper = max(F(0),r/2-request.total_error),r+A*h+2*request.total_error
        ...
        stages.append({'b':b,'h':h,'r':r,'lower':lower,'upper':upper,
                       ...
                       'construction_accepted':contract.accepted,'request_digest':request.digest,
                       'input_digest':request.input_digest,'requested_bits':request.output_bits,
                       'requested_horizon':request.horizon,'bound_total_error':request.total_error})
...
def robust_adaptive_compile(horizon: int, reports: int, budget: int, A: F,
    ...
    for b in range(max_bits+1):
        ...
        trace.append({'b':b,'h':h,'tau':tau,'r':r,
                      'lower':max(F(0),r/2-request.total_error),'upper':r+A*h+2*request.total_error,
                      'radius_stop':radius_stop,'floor_stop':floor_stop,
                      'construction_accepted':True,
                      'construction_checks':certificate.checks,
                      'candidate_counts':certificate.candidate_counts,'request_digest':request.digest,
                      'input_digest':request.input_digest,'requested_bits':request.output_bits,
                      'requested_horizon':request.horizon,'bound_total_error':request.total_error})
