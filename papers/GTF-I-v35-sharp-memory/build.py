#!/usr/bin/env python3
"""Source-bound v35 build, with an optional small, archive-free core build."""
...
O=H.parent/'GTF-I-v34-compatible-dynamics'
E=H/'evidence'
BASE='6016075083626f7c94cfb07912c49fa256fcf9e0'
PREVIOUS='47bc47e5995e4b1b99a18aaa472a1309f1668f40'
OLD_NAMES=['GTF-I-v34-compatible-dynamics','GTF-I-v33-compatible-lifts','GTF-I-v32-causal-width','GTF-I-v31-compatible-memory','GTF-I-v30-streaming-geometry',
...
def main()->None:
    ...
    names=['free-packet-time','dependent-fresh-packet','wrong-terminal-factor','interval-double-count',
           'omit-starting-direction','linear-angular','local-equals-final','omit-output-direction']
    controls=[]
    for mode in [[],['-O']]:
        for name in names:
    ...
    with tempfile.TemporaryDirectory(prefix='gtf35-latex-') as td:
        work=Path(td)
    ...
    must=['thm:main','def:machine','prop:separate','def:distortion','lem:packet',
          'thm:group-budget','prop:small-ball','lem:separated','lem:terminal','thm:occupation',
          'prop:upper','cor:common-row','cor:characters','lem:sphere','thm:sphere',
          'thm:final-error','cor:diagram','lem:angular']
    ...
    receipt={'schema':'gtf35.build/1','core_only':args.core_only,
    ...
    (root/'GENERAL_THETA_FOUNDATIONS_I_V35_REVIEW_READY.md').write_text(
      '# General Theta Foundations I — Revision 35\n\n**Sharp Width of Hidden Rotation Experiments**\n\n'
      f'Native source commit: `{receipt["source_commit"]}`. Controlling r19: `{BASE}`.\n\n'
      f'[Focused English article ({pages} pages)](papers/{H.name}/paper.pdf) · [Native LaTeX](papers/{H.name}/main.tex) · [Response to r19](papers/{H.name}/RESPONSE_TO_REFEREE.md)\n\n'
      ...
      f'[Unchanged v34 supporting article](papers/{H.name}/supporting-results.pdf) · [Mathematical archive](papers/{H.name}/complete-manuscript.pdf) · [Development archive](papers/{H.name}/complete-development.pdf)\n\n'
      'Matched Theta(N^(1/3)) hidden width for badly approximable rotation angles, including fixed uniform row error below 1/20; independent packet contraction; nonuniform occupation; general compact-group orbit budget; matched SO(d) width in its separate compact-command model; actual terminal-error obstruction.\n\n'
      'The lower bound allows arbitrary hidden states and cut-dependent stochastic rows. Exact resonant upper synthesis is inherited and reproved. Branching-program, circle-walk, positive-realization and geometric ingredients are credited. Constants and finite integer optima are not sharp; all irrational angles, adaptive collision scheduling and independent historical analytic closure are not inferred. Large cumulative PDFs are optional archives.\n')
