\section{Collision thresholds and their boundary laws}
\label{sec:geometric-results}
Consider a long stationary segment trapped near a normal channel of a
dispersing billiard. Its two endpoints vary on a fixed small square,
but the mixed derivative of its action is exponentially small in the
number of reflections. A bound for the action alone does not control
the relative flux. The central result of this paper separates that
flux into two nonlinear boundary contributions, with an exponentially
small relative error and uniform bounds for every fixed derivative.

The mechanism is a boundary-localized perturbation of a Jacobi
operator. We formulate it for spatially varying uniformly convex scalar
twist chains, without periodicity or a closed-form Green kernel
(Theorem~\ref{thm:v5-chain-factorization}). Exponential localization
makes the interior Hessian perturbation trace class uniformly in the
length. The corner-cofactor identity and a trace-norm comparison of
finite and half-line determinants then separate the two boundaries.
For billiards, geometric localization and equilibrium suspension
measure turn this factorization into a fixed-offset limiting physical
law (Theorem~\ref{thm:v4-law}).

The observations distinguish three levels of geometry. Leading count
amplitudes retain a scalar determinant ratio. Conditional endpoint
metrics retain the two contact curvatures; their entire leading
hierarchy is determined already at one complete flight. Nonlinear
probabilities retain higher boundary information. A fixed-area analytic
family separates this last observation from both leading ones. Its
fourth-jet parameter is visible at one flight as well as in the limiting
law; the long-segment theorem establishes the relative factorization
and convergence, rather than a strictly increasing hierarchy of inverse
coordinates with collision order.

Two quantitative consequences clarify what is recoverable from the
physical data. In the equal-gap three-channel family, four unlabelled
count amplitudes have a sharp pairwise one-third-H\"older inverse at
coalescing curvatures. Its proof uses regular symmetric coordinates,
not a separated-node assumption. For channel-resolved records, the
mean first residual time normalizes the endpoint variance and removes
the inverse-offset factor from the timing sensitivity of a fixed-order
extrapolation procedure. Both statements are proved with their data,
locality, and preparation costs specified.
...
\subsection{Marked observations}
...
The one-flight coefficient has derivative $8/9$ in the same family
(Proposition~\ref{prop:v5-one-flight}). The separation is thus between
leading and nonlinear observations. The half-line theorem identifies
the limiting nonlinear coefficient and its full physical law; it is not
needed merely to detect this one parameter at a finite flight number.

\subsection{Quantitative inverse statements}
For the fixed-gap three-channel family, write
$C_j=\lim_{d\downarrow0}d^{-2}\Pr(N_{jg+d}=j+1)$.
Theorem~\ref{thm:v5-pairwise} proves, for all pairs in a sufficiently
small neighborhood of the circular configuration,
\[
 \operatorname{dist}_{\rm match}(\kappa,\widetilde\kappa)
 \le C\left(\max_{1\le j\le4}|C_j-\widetilde C_j|\right)^{1/3}.
\]
The exponent is optimal, including when the complete amplitude sequence
is observed with the exponentially weighted absolute-error norm of
Section~\ref{sec:v4-coalescence}. The area is recovered with a Lipschitz
bound. These are finite-data pairwise statements, whereas the preserved
one-half exponent describes distance to the single circular reference.
The exact infinite-sequence inverse remains valid on its stated domain.

For selected records at odd $j$, the observable
$\operatorname{Var}(Q_b)/(3\E r)$ has the same leading variance as
\eqref{eq:v3-observed-variance}. Theorem~\ref{thm:v5-residual-recovery}
uses its sample analogue at $m$ offsets and proves curvature error
\[
 C_m\left(h^m+\sqrt{\log(C_m/\eta)/K}
                     +j|\Delta g|+|\Delta g|\right)
\]
with probability at least $1-\eta$. Here $2K$ independent successful
records are retained at each offset and $\Delta g$ is the supplied-gap
error. The expected preparation cost is of order
$K h^{-2}e^{j\gamma}$. The original programmed-offset estimator and its
harmonic inverse-offset sensitivity are also given, so the additional
information used by residual normalization is explicit.

The proof proceeds from geometric localization and the finite
Dirichlet chart to the general chain factorization, its explicit
billiard specialization, and the nonlinear physical law. Conditional
metrics, acquisition, the exact and quantitative unlabelled inverses,
and marked record response follow. The circular appendices retain
complete explicit calculations. The separate fixed-window two-collision
