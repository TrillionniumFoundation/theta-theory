\section{Comparison with variational and inverse billiard results}
...
Theorem~\ref{thm:v5-chain-factorization} isolates the additional
analytic step: for nonperiodic scalar chains satisfying the printed
uniform convexity and twist bounds, a finite relative determinant
factorizes into two half-line determinants with fixed-order smooth
exponential accuracy. Neither an explicit multiplier nor an alternating
Green kernel is used in that theorem. The billiard application also
requires the geometric localization and the suspension integral; these
are verified rather than included in the abstract hypotheses.
...
De Simoi, Kaloshin and Leguil \cite{DKLAnalytic} prove determination
of the geometry of analytic open dispersing billiards from their marked
length spectrum under non-eclipse, symmetry and genericity hypotheses.
This is a substantially stronger geometric conclusion for that
different observation and class. The local fourth-jet fiber here
concerns equilibrium threshold probabilities, not marked periodic
lengths, and it neither recovers an arbitrary analytic scatterer nor
removes the assumptions of that rigidity theorem. Its distinguishing
parameter is detected already at one complete flight, as
Proposition~\ref{prop:v5-one-flight} makes explicit. The role of the
long-segment construction is instead to identify the nonlinear boundary
law and control its relative error with all fixed derivatives.

There are quantitative consequences for the leading observations.
...
Theorem~\ref{thm:v5-pairwise} resolves a different conditioning
question from the circular-reference result: two arbitrary tables in
a small neighborhood of the triple curvature collision satisfy a sharp
one-third-H\"older estimate using only four direct count amplitudes.
The proof regularizes the polynomial coefficients of the curvature
multiset through an explicit nonzero four-amplitude Jacobian. The
root-matching exponent is then realized sharply by a two-sided path
of actual convex obstacles. This does not assert regular conditioning
of labelled roots or a separated-node Prony inverse at a collision.
Theorem~\ref{thm:v5-residual-recovery} uses an additional observed
residual time to replace a programmed offset denominator. It removes
the inverse-offset timing factor without removing the rarity factor,
the supplied-gap requirement, or the cost of independent preparation.
