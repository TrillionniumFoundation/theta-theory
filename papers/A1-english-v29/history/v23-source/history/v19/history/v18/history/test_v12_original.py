...
def integrated_physical():
    ...
    for gap,shape in ((F(0),1),(F(1,10**12),1),(F(1,7),3)):
        for domain,M in (('finite_two_command',1),('finite_two_command',2),
            ...
            cert=inspect_construction(p,a,len(truth.grid),4,M,advice.state,advice.predictions).require()
...
def interval_order():
    ...
    for M in (1,2,4,8,16,32):
        ...
        cert=inspect_construction(p,a,len(grid),1,M,state,state).require()
        e=F(1,8*M)
        maximum=F(0)
    ...
    c=inspect_construction(bad,a,2,1,2,lambda h:(F(1,2),),lambda h:(F(1,2),))
...
def robust_floor_tests():
    ...
    for M,rho in ((1,F(1,1024)),(2,F(1,32)),(4,F(1,16)),(4,F(1,4))):
        ...
        cert=inspect_construction(p,a,len(grid),1,M,
                 lambda hist:(F(1,2),) if not hist else ((F(1,2)+grid[hist[0][0]])/2,),
                 lambda hist:(F(1,2),) if not hist else ((F(1,2)+grid[hist[0][0]])/2,)).require()
