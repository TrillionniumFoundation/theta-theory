...
def main():
    ...
    result={'schema':'a2-v12-suite-run-v1','historical_input_mode':origin,
            'historical_sha256':PINS,'remote_ci':False,'formal_proof_verification':False,'suites':{}}
    with tempfile.TemporaryDirectory(prefix='a2-v12-checks-') as tmp:
        work=Path(tmp)
        for p in ROOT.rglob('*'):
        ...
        # Reproduce the archived suites on their exact v11 narrative layout.
        # Current v12 retention is checked separately and is order-independent.
        for rel in ('main.tex','article/01_introduction.tex','v5/references.tex'):
            shutil.copy2(ROOT/'history/v11-reviewed'/rel,work/rel)
        for rel,content in data.items():
            dest=work/rel;dest.parent.mkdir(parents=True,exist_ok=True);dest.write_bytes(content)
        for name,script in [('inherited','verify_revision.py'),('v9','verify_v9.py'),('v10','verify_v10.py'),('v11','verify_v11.py')]:
            outputs=[]
    ...
    result['inherited_suite_source']='Current inherited proofs with the pinned v11 main, introduction and bibliography restored in a disposable copy.'
    outputs=[]
    for optimized in (False,True):
        label='optimized' if optimized else 'normal'
        target=out/f'v12.{label}.json'
        command=[sys.executable]+(['-O'] if optimized else [])+[str(ROOT/'tools/verify_v12.py'),'--output',str(target)]
        run=subprocess.run(command,text=True,stdout=subprocess.PIPE,stderr=subprocess.STDOUT,check=False)
        if run.returncode:raise RuntimeError('v12: '+run.stdout)
        outputs.append(target.read_bytes())
    if outputs[0]!=outputs[1]:raise RuntimeError('v12 normal and optimized results differ.')
    parsed=json.loads(outputs[0])
    result['suites']['v12']={'status':parsed['status'],'counts':parsed['counts'],'normal_optimized_byte_identical':True,'result_sha256':digest(outputs[0])}
    result['status']='pass'
