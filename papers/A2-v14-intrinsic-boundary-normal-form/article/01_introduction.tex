...
The smooth physical construction also gives an intrinsic interpretation
without such a two-dimensional chart hypothesis. If $\varphi_b$ is
the first physical hit along the stable half-line and
$r_b=\sqrt{c_b/c_{1-b}}e^{-\gamma}$, then
Theorem~\ref{thm:v14-intrinsic-density} proves
\[
 B_{1-b}(\varphi_b(u))\varphi_b'(u)=r_b B_b(u).
\]
Consequently $\zeta_b(u)=\int_0^uB_b(x)\,\dd x$ is the uniquely
normalized scalar linearizing coordinate for the stable return, and
$B_b=\lim_{n\to\infty}\lambda^{-n}(R_b^n)'$ in every fixed
mixed norm. The determinant formula is therefore a construction of an
intrinsic branch measure, not merely a matrix representation of an
uninterpreted amplitude. In this coordinate the action has ordinary
Lebesgue sublevel widths. The complete normalized width is
\[
 \mathcal W_b(E)=\int_0^E x^{-1/2}\mathcal V_b(x)\,\dd x,
\]
and is determined by the even law
(Corollary~\ref{cor:v14-width}). For arbitrary smooth contacts the
width combines the two inverse branches; separate asymmetric graph
reconstruction is not inferred from it.
...
experiment. The analytic product mechanism is an ingredient of that
explanation. The independent-contact inverse and physical finite-jet
...
the hyperbolic comparison and intrinsic branch measure, and nonlinear
