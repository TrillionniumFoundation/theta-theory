...
REPO=HERE.parents[1]
REVIEW='87955c68505e76f6d8a7c64fc336af478078bde3'
MUTANTS=('negative_coefficient','wrong_scale','free_hidden_seed','hide_visible_seed','forget_kick_order','drop_actual_mark','omit_series_tail','zero_graph_residual','unscaled_exponential_error')
...
def main()->None:
    ...
    if not args.skip_diagnostics:
        ...
        import certified_examples as cert
        den, array = cert.bernstein_scaled(cert.gap_polynomial(),96)
        (evidence/'POLYNOMIAL_CERTIFICATE.json').write_text(json.dumps({'summary':result['polynomial'],'denominator':str(den),'numerators':[[str(v) for v in row] for row in array]},indent=2)+'\n')
        (evidence/'PHYSICAL_CERTIFICATE.json').write_text(json.dumps(result['physical'],indent=2)+'\n')
        for flags in ([],['-O']):
        ...
        for edition,path in [('v15','GTF-I-v15-certified-physical-comparison/verify.py'),('v14','GTF-I-v14-deficiency-certification/verify.py'),('v13','GTF-I-v13-intrinsic-deficiency/verify.py'),('v12','GTF-I-v12-causal-completion/verify.py'),('v11','GTF-I-v11-resource-comparison/verify.py'),('v10','GTF-I-v10-marked-duality/verify.py'),('v9','GTF-I-v9-causal-minimax/verify.py'),('v8','GTF-I-v8-decision-spectrum/verify.py'),('v7','GTF-I-v7-structural/verify.py'),('v6','GTF-I-v6-markov/verify.py'),('v5','GTF-I-v5-intrinsic/verify.py'),('v4','GTF-I-v4/verify.py'),('v3','GTF-I-v3/verify.py'),('v2','GTF-I-v2/verify.py'),('v1','GTF-I-v2/legacy/tools/verify.py')]:
    ...
    with tempfile.TemporaryDirectory(prefix='gtf-v16-build-') as work:
        work=Path(work)
    ...
    require(set(predecessor)<=set(maps['complete-development']),'Missing v15 companion labels: '+str(set(predecessor)-set(maps['complete-development'])))
    for rel,digest in {**inherited,**new}.items():
    ...
    receipt={'edition':'General Theta Foundations I, sixteenth constructive-causal-certification revision','review_commit':REVIEW,'source_commit':source_commit,'checkout_kind':checkout_kind,
             ...
             'source_manifest_sha256':sha(HERE/'SOURCE_MANIFEST.json'),'preserved_v15_companion_labels':len(predecessor),'retained_labels_resolved':len(retained),'shared_canonical_labels_same_numbers':len(core),
