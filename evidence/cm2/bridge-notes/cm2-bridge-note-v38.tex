...
\large Stopped positive sources, typed energy clocks,
two-cutoff face convergence, and a strongly conditional
pathwise CM2 criterion}
\author{Working note}
\date{13 July 2026 (version 38)}
...
auxiliary label law.  A full-product conditional $L^{p_W}$ lemma, including
the cemetery-envelope moment, gives a nonvacuity route from unweighted tails,
with every exponent explicitly degraded by $1/p_W'$; mere $L^1$
integrability is never used to preserve a tail exponent.  Every exceptional
event is additionally assigned its native clock: polynomial-in-energy,
prescribed PPE depth with or without Jensen, genuine two-time depth, or face
word depth.  Only genuine two-time tails enter the central exponent directly;
...
stated under the complete event-weight tilt, rather than inferred from an
unweighted probability tail and mere integrability of the mark.  Labels with
excessive weighted growth are removed by that tilted conditional tail.  All
...
\end{equation}
The same complete label law is used for the native stopping, PPE, endpoint
marks, and all cemetery failures;
...
\[
 ...
 d\mathbb P_{\rm stop},
\]
the augmented grazing excess has exponent $q_g^W$ under the normalized
positive source law.  This is the ``normalize the positive source, estimate,
...
\label{eq:USC-holder-sufficient-condition}
 ...
 \le C\,\mathbb E_{\rm stop}[\mathscr W_{\lambda,*}].
\end{equation}
H\"older then gives only $q_g^W=q_g/p_W'$, and all ledgers below use this
degraded exponent.
\end{definition}
...
failure surface and $U_i$ is the remaining propagation to the final test
pairing.  If $\mathfrak U_i$ denotes the corresponding source-to-test norm,
there must be an $\mathcal F_{\tau_i}$-measurable predictable factor
$\mathcal C_i\ge1$ such that
\begin{equation}
...
\label{eq:cemetery-current-weighted-bound}
 ...
 =C\|\Phi\|_{\rm test}\,
   ...
   \Pi_{s,N}^{W}(B_i).
\end{equation}
A future factor may be replaced by one only when $U_i$ is a positive
total-variation contraction.  Otherwise its predictable upper envelope is
frozen in $\mathcal C_i$ before the cemetery transition.  The five rows of
the interface are
\begin{center}
\begin{tabular}{c|c|c|c}
failure & stopped source & remaining propagation & predictable cost\\ \hline
ctx & context-gate flux & buffer/section/phase & $\mathcal C_{\rm ctx}$\\
stop & antichain complement & descendants/section & $\mathcal C_{\rm stop}$\\
buf & first buffer failure & endpoint/phase & $\mathcal C_{\rm buf}$\\
gate & physical-gate flux & Jacobian/section/phase & $\mathcal C_{\rm gate}$\\
cont & first continuation failure & recovery/endpoint & $\mathcal C_{\rm cont}$
\end{tabular}
\end{center}
...
falsifiable primitive.  Measurability of a cemetery label alone does not
imply \eqref{eq:cemetery-RN-domination}.
\end{definition}

\begin{lemma}[Finite stopped source and summable propagation]
\label{lem:stopped-source-propagation}
Assume that $\Sigma_i/|\Sigma_i|(1)$ is a proper standard family after at
most $R_i$ recovery iterates and that the ensuing forward or reverse loss of
memory is $C\gamma^{n-R_i}$, $0<\gamma<1$.  Then
\begin{equation}
\label{eq:stopped-source-propagation}
 |\langle U_i^n\Sigma_i,\phi\rangle|
 \le C\,|\Sigma_i|(1)\gamma^{(n-R_i)_+}
       \|\phi\|_{\rm test}.
\end{equation}
After the recovery factor has been included in $\mathcal C_i$,
\eqref{eq:stopped-source-propagation} implies
\eqref{eq:cemetery-RN-domination}.  This is the source-first structure of
...
\begin{proof}
Normalize the positive source, recover it, apply the one-time coupling or
dual memory estimate, and multiply its original mass back.  If recovery is
not immediate, the factor $\gamma^{-R_i}$ is predictable at $\tau_i$ and is
therefore part of $\mathcal C_i$.  No signed cancellation is used.
\end{proof}
...
\end{align}
Every additional exceptional event is entered exactly once in a clock
registry with fields
\[
 (\text{event},\text{first-failure stage},\text{tail},
   \text{clock},\text{passes through Jensen?}).
\]
If it is a discarded event, it refines exactly one of the five cemetery
stages and inherits that row of $\mathrm{CEM\_DOM}$; if it is a retained
absolute remainder, it stays under $\mathscr W_{\rm surv}$.
Only the following four central clocks are legal:
\begin{align}
 \Pi^W(B_j(r))&\le Cr^{\Theta_j^W},
   &&j\in\mathcal J_r,\label{eq:clock-polynomial-r}\\
 \Pi^W(B_j(N_{\rm PPE}(r)))&\le Ce^{-c_j^WN_{\rm PPE}(r)},
   &&j\in\mathcal J_{\rm PPE}^{J},\label{eq:clock-PPE-Jensen}\\
 \Pi^W(B_j(N_{\rm PPE}(r)))&\le Ce^{-c_j^WN_{\rm PPE}(r)},
   &&j\in\mathcal J_{\rm PPE}^{\rm TV},\label{eq:clock-PPE-TV}\\
 \Pi^W(B_j(N_{\rm time}))&\le Ce^{-c_j^WN_{\rm time}},
   &&j\in\mathcal J_{\rm time}.
...
\label{eq:weighted-energy-rates}
 \Theta_{\rm poly}^{W}:=\min\!\left\{\Theta_{\rm PPE}^{W},
       ...
       \inf_{j\in\mathcal J_r}\Theta_j^W\right\},
\end{equation}
\begin{equation}
\label{eq:weighted-clock-rates}
\begin{aligned}
 c_{\rm PPE,J}^{W}&:=\min\!\left\{c_{\rm buf}^{W},c_{\rm gate}^{W},
 c_{\rm cont}^{W},\inf_{j\in\mathcal J_{\rm PPE}^{J}}c_j^W\right\},\\
 c_{\rm PPE,TV}^{W}&:=\inf_{j\in\mathcal J_{\rm PPE}^{\rm TV}}c_j^W,
 &c_{\rm event,time}^{W}&:=\inf_{j\in\mathcal J_{\rm time}}c_j^W.
\end{aligned}
\end{equation}
An infimum over an empty class is $+\infty$ and is omitted from subsequent
minima.  Native stopping belongs to $\Theta_{\rm poly}^{W}$ and is not
counted again as a time event.
...
\label{eq:full-product-Lp-energy-route}
 ...
 \qquad p_W>1,
\end{equation}
holds uniformly, H\"older yields only
\begin{equation}
\label{eq:holder-degraded-energy-rates}
 \Theta_j^{W}=\Theta_j/p_W',\qquad
 c_i^{W}=c_i/p_W'.
\end{equation}
...
\end{equation}
The scale-good labels satisfy
$a_X+a_Y\le Ce^{C_{\rm scl}N}$, while the total marked mass of scale-bad
labels, with the full weight \eqref{eq:global-stopped-source-weight}, is at most
...
\label{eq:survivor-mass-lower-bound}
 M_{\mathsf G}^{W}\ge
 1-\Pi^W(B_{\rm ctx}\cup B_{\rm stop}\cup B_{\rm buf}\cup
 B_{\rm gate}\cup B_{\rm cont}\cup
 \textstyle\bigcup_{j\in\mathcal J_r\cup\mathcal J_{\rm PPE}^{J}}B_j)
 \ge\frac12.
\end{equation}
...
\eqref{eq:clock-PPE-Jensen}.  The finite or uniformly summable clock
registry tends to zero at the prescribed depth; shrink the admissible
$r$-range so the union bound is at most $1/2$.
\end{proof}
...
\label{eq:physical-energy-exponent}
 \Theta_{\rm poly}^{W}=\min\left\{\Theta_{\rm PPE}^{W},
        \Theta_{\rm ctx}^{W},\Theta_{\rm st}^{W},
        \inf_{j\in\mathcal J_r}\Theta_j^W\right\}>0,
\end{equation}
one has
\begin{align}
...
\label{eq:complete-energy-tail}
 \mathcal T_E^{W}(r):=
 e^{-c_{\rm buf}^{W}N_{\rm PPE}(r)}
 +e^{-c_{\rm gate}^{W}N_{\rm PPE}(r)}
 +e^{-c_{\rm cont}^{W}N_{\rm PPE}(r)}
...
antichain mass, and \eqref{eq:named-energy-tails} together with the
Jensen-class clock registry gives exactly $\mathcal T_E^{W}(r)$.  The
...
Use the complete tilted rates
\eqref{eq:weighted-energy-rates}--\eqref{eq:weighted-clock-rates} and set
\[
...
\label{eq:phase-energy-all-tails}
 ...
 +C\Xi^{-\delta C_{\rm buf}c_{\rm buf}^{W}/2}
 +C\Xi^{-\delta C_{\rm buf}c_{\rm gate}^{W}/2}
 +C\Xi^{-\delta C_{\rm buf}c_{\rm cont}^{W}/2}
 +C\sum_{j\in\mathcal J_{\rm PPE}^{J}}
        \Xi^{-\delta C_{\rm buf}c_j^W/2}.
\end{equation}
...
\label{eq:phase-PPE-TV-tails}
 C\sum_{j\in\mathcal J_{\rm PPE}^{\rm TV}}
       \Xi^{-\delta C_{\rm buf}c_j^W};
\end{equation}
...
\item The normal-cycle value $\mathfrak A=-1$ motivates the clean template;
      ...
      $\mathrm{TENERGY}_{W}$, together with the mass-preserving complete
      stopped-source tree and its typed clocks, give the weighted energy
      theorem and Jensen.
...
$\mathrm{CEM\_DOM}$, $\mathrm{TENERGY}_{W}$, $\mathrm{FZ}_{\rm geom}$,
...
$\mathrm{CEM\_DOM}$, $\mathrm{TENERGY}_{W}$ and
...
$\mathscr W_{\lambda,*}$, and fix all six conditional moment exponents,
including $q_C$.  Compute $\chi_W$ in
\eqref{eq:full-product-reciprocal-cost}; if $\chi_W<1$, choose the actual
$p_W$ from \eqref{eq:available-full-product-pW} and degrade every raw
context, stopping, typed-clock, PPE, and grazing rate by $1/p_W'$.  Otherwise
$\mathrm{TENERGY}_{W}$ and $\mathrm{USC}_{\rm end}^{W}$ remain independent
primitive inputs.  Complete the clock registry before choosing any phase
parameter; no bare event rate survives this step.  Next fix the coefficient
...
\begin{align}
 ...
 \mathbb E[\mathcal C^{q_C}\mid\mathcal F_{\tau_i}]&\le C
 \notag
\end{align}
uniformly for actual, split, and cemetery parents.  This conditional
...
conditional moments with this same $p_W$.  All coarser marginals, including
...
each earlier marginal is preserved.  Raise the piecewise weight
\eqref{eq:global-stopped-source-weight} to $p_W$.  On a survivor use
\eqref{eq:global-event-weight}; on $B_i$ use
\eqref{eq:cemetery-RN-domination}.  Conditional H\"older at the
component, native, scale, and context stages uses reciprocal exponents
$p_W/q_F$, $p_W/q_D$, $p_W/q_S$, $p_W/q_R$, and
$p_Wa_H/q_H$, while the cemetery step uses $p_W/q_C$.  Their sum is
$p_W\chi_W<1$, leaving one strict conjugate
slot.  Iterated conditional expectation therefore proves
...
buffer, gate, continuation, and prescribed-depth PPE bounds in
$\mathrm{NST}_{\rm phys}/\mathrm{PPE}_{N}$, and assume
\eqref{eq:conditional-refinement-moments} with $\chi_W<1$.  Choose $p_W$ by
\eqref{eq:available-full-product-pW}.  Then
$\mathrm{TENERGY}_{W}$ holds with
\[
 \Theta_j^{W}=\Theta_j/p_W',\qquad
 c_i^{W}=c_i/p_W',
\]
and the same conclusion applies to every additional named energy event.
...
\]
Apply this once to each named cemetery event and to the retained PPE
sublevel.  No branchwise Jensen or signed cancellation occurs in this
transfer.
\end{proof}
...
positive-scale, return/context, and predictable-cemetery moment exponents,
and assume
\begin{equation}
...
by $\mathrm{TENERGY}_{W}$ on this identical tilt.  For cemetery pieces the
...
\item the finite-horizon functional-correlation bound of \cite{LS}, the
      ...
      \eqref{eq:available-full-product-pW}; all five polynomial moments and
      the exponential return/context moment hold conditionally
...
\item $\mathrm{NST}_{\rm phys}$, $\mathrm{SS}(d_Z)$, and
      ...
      energy ledger $\mathrm{TENERGY}_{W}$, its typed clock registry, and the
...
$\mathrm{PPE}_{N}$, $\mathrm{CEM\_DOM}$, and
$\mathrm{TENERGY}_{W}$, together with the complete positive stopped-source
tree, its cemetery labels, and the typed clock registry, give
...
$\mathrm{TENERGY}_{W}$; no raw lattice cutoff or unweighted tail is used to
...
and every failure cemetery label under the one full event tilt.  The clock
registry must type each exceptional event exactly once as $r$-, PPE-, or
genuine-time depth before any exponent is assembled.
...
\begin{proposition}[Twenty-eight obstruction tests]
\label{prop:v38-obstruction-tests}
...
\begin{proof}
Items 1--3, 8--11, and 13--28 are direct finite-measure, frequency, or norm
