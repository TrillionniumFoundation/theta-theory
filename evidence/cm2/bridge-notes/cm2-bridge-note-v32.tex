...
\large Fixed-gauge endpoint unfolding, physical escape, normalized deep recovery,
and a strongly conditional
pathwise CM2 criterion}
\author{Working note}
\date{13 July 2026 (version 32)}
...
reference; the resulting gauge commutator is retained as a regular block.
...
survival mass into a logarithmic-time physical cover.  A scale-dependent
high-pressure clean scheme is recorded only as a separately falsifiable
route to this primitive input.  No affine jet is absorbed into the polynomial
...
physical-polynomial-escape, branch-section, geometric-flux,
split-standard-family, connected-pullback, shallow spacetime refinement,
normalized deep-recovery, and pathwise inputs
...
Throughout, $\{\widehat T_s:|s|<s_0\}$ is a $C^3$ compact family of
finite-horizon planar dispersing billiard collision maps generated by a
$C^3$ normal deformation of strictly convex scatterer boundaries, with
...
interface.  There are $C^2$ diffeomorphisms
...
\begin{definition}[Regular measure-gauge interface $\mathrm{MT}_{\rm reg}$]
\label{def:measure-gauge-interface}
Let $U_sh=h\circ A_s^{-1}$ in physical coordinates and let
$\mathcal X=\left.\partial_sU_s\right|_{s=0}$ in the fixed gauge.
Condition $\mathrm{MT}_{\rm reg}$ requires that differentiation of the
transported operator is exact and that the change from any preliminary
physical transport to \eqref{eq:measure-trivializing-gauge} contributes
\begin{equation}
\label{eq:gauge-commutator}
 K_{\rm gauge}=[P_0,\mathcal X]+K_{\rm jac}+K_{\rm obs}.
\end{equation}
Here $K_{\rm jac}$ and $K_{\rm obs}$ are the Jacobian and
observable-transport terms dictated by the chosen convention.  After the
global projection $\Pi^\perp(\cdot)\Pi^\perp$, these terms are a finite sum
of rank-one smooth blocks and an $M\times M$ density satisfying the regular
$W^{R,1}$ ledger in \textup{(H5)}.  Nothing in
\eqref{eq:gauge-commutator} is absorbed into a face current.
\end{definition}

\begin{lemma}[Fixed-gauge centering]
\label{lem:fixed-gauge-centering}
Under $\mathrm{MT}_{\rm reg}$, the same projection $\Pi_0$ centers the
finite-$s$ operators, endpoint observables, branch-section modes, and both
standard-family couplings.  The derivative of $\Pi_0$ is zero, and all gauge
derivatives are precisely the regular blocks
\eqref{eq:gauge-commutator}.
\end{lemma}

\begin{proof}
Equation~\eqref{eq:measure-trivializing-gauge} gives
$\mu_0(P_sh)=\mu_0(h)$ for every $s$, hence
$P_s\Pi_0=\Pi_0P_s=\Pi_0$.  Differentiating the conjugacy gives the
commutator and the explicit Jacobian/observable terms in
\eqref{eq:gauge-commutator}.  Since the reference probability is fixed,
there is no derivative of a moving marginal and no unrecorded rank-one
correction.
\end{proof}

The fixed zero-mass organization is consistent with the
measure-valued-section response framework of \cite{AlvesBahsoun} and the
extended-space loss-of-memory framework of \cite{GalatoloLucarini}.  Neither
reference supplies the moving-billiard transport or its singularity atlas;
those remain the explicit $\mathrm{MT}_{\rm reg}$ and
$\mathrm{UCR}_{\rm sh}$ interfaces here.
...
\begin{definition}[Actual physical polynomial escape $\mathrm{PPE}_{\rm phys}$]
\label{def:physical-legal-extension}
Let $\mu^u_{\beta,Q}$ be the normalized collision--SRB conditional on a
native interval $Q$.  The family satisfies
$\mathrm{PPE}_{\rm phys}(d_Z)$ if there are $\gamma_{\rm ph}>0$ and
$C<\infty$ such that the following statements hold on every actual
conditional, every admissible mark, and every table on the shallow spacetime
refinement.
\begin{enumerate}[label=\textup{(PPE\arabic*)}]
\item For every context polynomial $P\in\mathbb R_{d_Z}[z]$ and every
      sufficiently small native scale $\sigma_Q$ there is a physical
      resolving depth $J\le C|\log\sigma_Q|$ for which
      \begin{equation}
      \label{eq:ppe-physical-sublevel}
       \mu^u_{\beta,Q}
       \{|X^{[J]}_{\beta,Q}-P|\le\sigma_Q^{1+\alpha_L}\}
       \le C\sigma_Q^{\gamma_{\rm ph}}.
      \end{equation}
      This is an estimate for the full absolutely continuous collision--SRB
      conditional, not for a Cantor conditional or for a law conditioned on
      survival in a clean alphabet.
\item Multiplication by a positive amplitude preserves
      \eqref{eq:ppe-physical-sublevel} with the corresponding scaled amplitude
      mark.  The same constants hold conditionally on all frozen
      opposite-endpoint and context data used in $\mathrm{SS}(d_Z)$.
\item The finite endpoint/holonomy series is defined on retained open
      physical cylinders and
      \begin{equation}
      \label{eq:finite-thickening-shadow-error}
       \|X^{[J]}_{\beta,Q}-X_{\beta,Q}\|_\infty\le C\kappa^J
      \end{equation}
      on every clean core used as a verification device.  On a good parent,
      \begin{equation}
      \label{eq:physical-lipschitz-mark}
       1+\operatorname{Lip}(X^{[J]}_{\beta,Q}-P)
       \le CH_\beta\sigma_Q^{-d_{\rm ph}}.
      \end{equation}
      The resolving buffer has total physical time $O(|\log\sigma_Q|)$,
      bounded overlap, and retains all continuations.  No affine jet is
      absorbed into $P$.
\item All constants, marks, and labels are uniform for
      $0\le\theta\le1$ and $J\le L(s)$ on
      $\mathrm{UCR}_{\rm sh}$ cells.
\end{enumerate}
This is a primitive physical escape interface.  Leclerc's cutting theorem
supplies a possible rough-template seed, but it does not by itself verify
\eqref{eq:ppe-physical-sublevel} for the actual collision conditional.
\end{definition}
...
\begin{remark}[Scale-dependent high-pressure nonvacuity route]
\label{rem:high-pressure-clean-route}
A possible verification of $\mathrm{PPE}_{\rm phys}$ would choose a clean
alphabet $\mathcal A_J$ depending on $J$ with pressure gap
$\chi_J\le C/J$, so that its depth-$J$ survival is bounded below by
$e^{-C}$.  One must additionally prove uniform rough-template, cutting,
distortion, open-cylinder, and component constants as the alphabet expands.
If $\inf_J\chi_J>0$ or any of these constants degenerates, the route fails
and $\mathrm{PPE}_{\rm phys}$ remains primitive.  Open-system normalization
in \cite{CanestrariHoles,DLreview} preserves exactly this survival factor and
does not evade Lemma~\ref{lem:negative-pressure-trial-obstruction}.
\end{remark}
...
\end{equation}
The physical resolving time from $\mathrm{PPE}_{\rm phys}$ plus $N$ is
...
Under $\mathrm{PPE}_{\rm phys}(d_Z)$, uniformly for the admissible context
polynomials $P\in\mathbb R_{d_Z}[z]$ satisfying
...
small in the two induced depths.  Here all modes are those of the original
...
For completeness, put $K=e^{\eta N}$ with $\eta$ in the window
...
$0<\epsilon<1/2$, put $N=\max\{m,n\}$ and $K=e^{\eta N}$, and choose
\begin{equation}
\label{eq:delayed-recovery-eta-window}
 6\eta<\tfrac12\min\{c_-,c_+\},\qquad
 2\eta<(1-\epsilon)\min\{c_-,c_+\},\qquad
 (q_{\rm rec}C_1+2)\eta<q_{\rm rec}\epsilon/D.
\end{equation}
...
$\mathrm{SS}$, $\mathrm{PPE}_{\rm phys}$, $\mathrm{BR}_{\rm sec}$,
$\mathrm{FZ}_{\rm geom}$,
...
\item The branch sections, $\mathrm{PPE}_{\rm phys}$ labels, full-tangent
...
\end{enumerate}
The constants in $\mathrm{BR}_{\rm sec}$,
$\mathrm{PPE}_{\rm phys}$, $\mathrm{FZ}_{\rm geom}$,
$\mathrm{SF}_{\rm split}$, $\mathrm{CP}_{\rm env}$, and
$\mathrm{REC}(q_{\rm rec})$ are required to be uniform on these cells.
This is a primitive moving-singularity interface; compactness of the table
chart alone does not imply it.
\end{definition}

\begin{definition}[Deep envelope tail $\mathrm{TAIL}_{\rm env}$]
\label{def:deep-envelope-tail}
Let $\mathcal R_s^{>L}$ be the complete double-centered remainder formed by
words deeper than $L$.  Condition $\mathrm{TAIL}_{\rm env}$ requires
\begin{equation}
\label{eq:deep-tail-delta}
 \delta_{s,L}:=\|\mathcal R_s^{>L}\|_{\rm TV}
 \le C|s|^{-1}e^{-(c_{\rm tail}-A_{\rm loss})L},
\end{equation}
together with the exact actual/split expansion and one positive envelope
whose connected-pullback and boundary-oscillation constants are uniform in
$\theta,s,L$.  Artificial faces cancel before this signed terminal
remainder is formed.  Thus no single master cell is claimed to continue all
infinite words.
\end{definition}

\begin{definition}[Normalized deep recovery
$\mathrm{NREC}_{\rm deep}(q_{\rm deep})$]
\label{def:normalized-deep-recovery}
Disintegrate every positive constituent of the actual/split envelope of
$\mathcal R_s^{>L}$ into pieces of masses $M_{\lambda,s,L}$, recovery times
$R^\pm_{\lambda,s,L}$, and coupling mark
$\mathcal W_{\lambda,s,L}\ge1$.  Put
\[
 D_{s,L}:=\sum_\lambda M_{\lambda,s,L}
 \le C_{\rm env}\delta_{s,L},
\]
where the fixed envelope constant includes the finitely many absolute split
coefficients.  With the zero-mass case omitted, define the probability law
\[
 \pi_{s,L}(\lambda)=M_{\lambda,s,L}/D_{s,L}.
\]
Condition $\mathrm{NREC}_{\rm deep}(q_{\rm deep})$ requires, separately in
the two time orientations,
\begin{equation}
\label{eq:normalized-deep-recovery}
 \sup_{s,L}\sum_\lambda\pi_{s,L}(\lambda)
 \mathcal W_{\lambda,s,L}
 e^{q_{\rm deep}R^\pm_{\lambda,s,L}}\le C,
 \qquad
...
\label{eq:weighted-deep-recovery}
 ...
 \le CD_{s,L}\le C'\delta_{s,L}.
\end{equation}
The estimate is conditional on the normalized deep mass and is required for
the actual law and every split constituent on the same deep envelope.  An
unnormalized $O(1)$ recovery moment is not a substitute for
...
Assume that, after division by its total envelope mass $D_{s,L}$, every actual/split deep
constituent is a probability standard family and that in each orientation
\begin{equation}
\label{eq:normalized-initial-Z-moment}
 \sup_{s,L}\sum_\lambda\pi_{s,L}(\lambda)
 \mathcal W_{\lambda,s,L}(1+Z_{\lambda,s,L})^{q_{\rm deep}C_1}<\infty.
\end{equation}
Suppose the growth lemma gives
$R^\pm_\lambda\le C_0+C_1\log(1+Z^\pm_\lambda)$, after a refinement whose
conditional tail has the same bound.  Then
$\mathrm{NREC}_{\rm deep}(q_{\rm deep})$ holds.
\end{lemma}

\begin{proof}
The recovery inequality gives
\[
 e^{q_{\rm deep}R^\pm_\lambda}
 \le e^{q_{\rm deep}C_0}
 (1+Z^\pm_\lambda)^{q_{\rm deep}C_1}.
\]
Integrate this inequality against the normalized law $\pi_{s,L}$ and use
\eqref{eq:normalized-initial-Z-moment}.  Multiplication by $D_{s,L}$ and
$D_{s,L}\le C_{\rm env}\delta_{s,L}$ give
\eqref{eq:weighted-deep-recovery}.  The affine
recursion $Z(F^{n*}\mathcal G)\le\gamma Z(\mathcal G)+Z_0$ and the leaky
standard-family construction of \cite{CanestrariHoles} are the model for
verifying \eqref{eq:normalized-initial-Z-moment}; the displayed argument is
the exact normalization step needed here.
\end{proof}
...
$\delta_{s,L}$ equal to the right side of
...
along the usable unstable orientation preserves total mass.  Normalize this
disintegration by its envelope mass $D_{s,L}$ before applying the affine $Z$ recursion,
...
$D_{s,L}\le C_{\rm env}\delta_{s,L}$ restored.  Eventual recovery of each piece alone would not
...
$\mathrm{SS}(d_Z)$, $\mathrm{PPE}_{\rm phys}(d_Z)$,
...
\label{eq:global-ledger-sum}
 \sum_\lambda M_\lambda
 e^{\epsilon(|c^-|+|c^+|)}
 (1+H_\lambda)^{a_H}\mathfrak d_\lambda^{A_{\rm dom}}
 \operatorname{Mark}(\lambda)^2
 \{1+Z(\mathcal G_\lambda^-)+Z(\mathcal G_\lambda^+)\}<\infty.
\end{equation}
...
\item the finite-horizon functional-correlation bound of \cite{LS}, the
      ...
      $\sum_\lambda M_\lambda
      \mathfrak d_\lambda^{A_{\rm dom}q_D}<\infty$; the separate terminal-law
      recovery input $\mathrm{REC}(q_{\rm rec})$, with
      $q_{\rm rec},\epsilon,\eta$ chosen to satisfy
...
\item $\mathrm{SS}(d_Z)$ and
      $\mathrm{PPE}_{\rm phys}(d_Z)$ on the persistent clean gate, together with the
      ...
      parameter-uniform constants required by $\mathrm{UCR}_{\rm sh}$;
\item the two-chart nondegeneracy $|\nabla G|\ge c_G$ and
      ...
      then supplies \eqref{eq:FZ-dynamical-tube}; the terminal tail satisfies
      $\mathrm{TAIL}_{\rm env}$ and
      $\mathrm{NREC}_{\rm deep}(q_{\rm deep})$ with
      $q_{\rm deep}>\max\{\alpha_+,\alpha_-\}$;
\item $\mathrm{MT}_{\rm reg}$ holds, and the complete regular defect,
      including \eqref{eq:gauge-commutator}, satisfies, for some
      $1\le R\le3$,
      \[
...
\begin{proof}
Lemma~\ref{lem:fixed-gauge-centering} first puts the limiting current and all
finite-$s$ difference quotients under the same projection $\Pi_0$.  The
commutator, Jacobian, and observable-transport terms are included in the
regular ledger of \textup{(H5)}; no marginal correction remains in a face
current.
...
Conditions $\mathrm{SS}$ and $\mathrm{PPE}_{\rm phys}$, together with the
...
particular this includes $\mathrm{MT}_{\rm reg}$,
$\mathrm{PPE}_{\rm phys}$, $\mathrm{BR}_{\rm sec}$,
$\mathrm{REC}$, $\mathrm{FZ}_{\rm geom}$, $\mathrm{SF}_{\rm split}$,
$\mathrm{CP}_{\rm env}$, $\mathrm{UCR}_{\rm sh}$,
$\mathrm{TAIL}_{\rm env}$, and $\mathrm{NREC}_{\rm deep}$.  If the analytic boundary family
...
condition; it does not supply $\mathrm{REC}$,
$\mathrm{PPE}_{\rm phys}$, $\mathrm{BR}_{\rm sec}$,
$\mathrm{CP}_{\rm env}$, $\mathrm{UCR}_{\rm sh}$,
$\mathrm{TAIL}_{\rm env}$, or $\mathrm{NREC}_{\rm deep}$.
...
depth-dependent scalar quotient tail on the same labels.  Apply the low/high central estimates
...
the time--depth ledger and the normalized recovery factor
\[
...
The two off-diagonal cone prefactors retain $\delta_{s,L}$ because
\eqref{eq:weighted-deep-recovery}, not an unnormalized recovery moment, is
used before summing the labels.
\end{enumerate}
Choosing $\epsilon,\eta$ as in
\eqref{eq:delayed-recovery-eta-window} makes the first five
...
estimated label by label.  The regular $M\times M$ defect is included in
the same finite-rank low estimate and has its independent $W^{R,1}$ tail.

There are twelve falsifiable interfaces: $\mathrm{MT}_{\rm reg}$ must put all
operators and currents in one fixed invariant gauge and expose every gauge
commutator as a regular block; $\mathrm{SS}$ must contain exactly one
Leclerc rough source; $\mathrm{PPE}_{\rm phys}$ must give actual-SRB
polynomial escape and is not inferred from a fixed negative-pressure clean
subsystem;
$\mathrm{BR}_{\rm sec}$ must produce same-branch homogeneous sections with
uniformly recoverable oscillatory densities, while $\mathrm{REC}$ must give
the two separate terminal-law recovery tails; $\mathrm{FZ}_{\rm geom}$ must
...
connected event cells with a uniform diameter/tail estimate;
$\mathrm{UCR}_{\rm sh}$ must continue the finite shallow labels on one
spacetime refinement; $\mathrm{TAIL}_{\rm env}$ must form the aggregate deep
signed envelope; $\mathrm{NREC}_{\rm deep}$ must preserve its small mass
under recovery weighting; and the chosen
...
\begin{proposition}[Four obstruction tests]
\label{prop:v32-obstruction-tests}
The active interfaces reject the following four countermodels.
\begin{enumerate}[label=\textup{(T\arabic*)}]
\item If $\nu\ne\mu_0$ and $J=\delta_{(x,y)}$, then in general
      $\mathsf D_{\rm test}^{\nu}(g\otimes f)(x,y)$ differs from
      $(g-\mu_0g)(x)(f-\mu_0f)(y)$.  The fixed gauge excludes this mismatch
      by imposing $\nu^-=\nu^+=\mu_0$.
\item A clean Bernoulli gate of survival probability $p<1$ has depth-$J$
      mass $p^J$; Lemma~\ref{lem:negative-pressure-trial-obstruction}
      prevents $O(J)$ trials from being called an exponential physical
      cover.
\item A single deep piece of mass $\delta$ and recovery time
      $R=q^{-1}\log(1/\delta)$ has unnormalized moment
      $\delta e^{qR}=1$, while its normalized moment is
      $e^{qR}=\delta^{-1}$.  Thus
      $\mathrm{NREC}_{\rm deep}$ rejects exactly the example for which a
      coupling prefactor would degrade to a fractional power of $\delta$.
\item If one symbolic cylinder has two disconnected event components and a
      sharp boundary separates them, $\mathrm{CP}_{\rm env}$ assigns distinct
      component indices before taking oscillations.  No small-diameter claim
      is allowed to jump between the two components.
\end{enumerate}
\end{proposition}

\begin{proof}
The first and third statements are direct finite-measure calculations.  The
second is the Bernoulli specialization of
Lemma~\ref{lem:negative-pressure-trial-obstruction}.  The fourth is the
connected-refinement clause of $\mathrm{CP}_{\rm env}$.
\end{proof}
