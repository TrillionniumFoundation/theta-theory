...
\large Aggregate stopped-source partitions, survivor scale,
two-cutoff face convergence, and a strongly conditional
pathwise CM2 criterion}
\author{Working note}
\date{13 July 2026 (version 39)}
...
and one-derivative response blocks.  Every typed block is double-projected
before it is estimated.  Three nested distribution spaces separate the two
...
with every exponent explicitly degraded by $1/p_W'$; mere $L^1$
integrability is never used to preserve a tail exponent.  The cemetery factor
is never conditioned on a sigma-algebra on which it is already measurable.
Every exceptional event is additionally assigned its native clock: polynomial-in-energy,
prescribed PPE depth with or without Jensen, genuine two-time depth, or face
word depth.  A canonical first-hit disjointization turns every central clock
class into one positive partition function.  The theorem assumes aggregate
summability, or derives it from a strict tail--complexity gap before applying
H\"older; an infimum of atomic rates is never substituted for this partition
bound.  Only genuine two-time tails enter the central exponent directly;
PPE-depth tails are first converted through the physical frequency and carry
the Jensen half-loss exactly when they participate in the energy argument.  The
...
native-stopping, physical-polynomial-escape, complete-weighted-energy,
branch-section, geometric-flux, endpoint-upper-scale, split-standard-family,
connected-pullback, shallow spacetime refinement, two-cutoff face convergence,
normalized shallow/deep-recovery, and pathwise inputs
listed below, including the stopped-source domination and typed-clock
...
$\widehat\mu_s$.  We impose the following measure-trivializing transport
interface.  There are $C^{r+1}$ diffeomorphisms
...
For the collision measure this is a componentwise smooth
mass-coordinate transport; existence with the regularity and chart
compatibility used below is part of the interface.  The chosen
analytic-boundary or
...
\item Before projection the quotient has one exact typed decomposition.  If
      $\widetilde K_{s,j}^{\rm DQ}$ denotes a raw regular, physical-face, or
      response block, the block actually estimated is
      \begin{equation}
      \label{eq:typed-block-double-projection}
       \widehat K_{s,j}^{\rm DQ}
       :=\Pi_0^\perp\widetilde K_{s,j}^{\rm DQ}\Pi_0^\perp,
       \qquad j\in\{\mathrm{reg},\mathrm{face},\mathrm{sw}\}.
      \end{equation}
      Marginal and rank-one terms exposed by this projection in the physical
      atlas are entered explicitly in the regular or face ledger.  Thus every
      typed block, not merely their sum, maps zero-mass inputs to zero-mass
      outputs.  With this convention the complete quotient is
      \[
        K_s^{\rm DQ}=\widehat K_{s,\rm reg}^{\rm DQ}
          +\widehat K_{s,\rm face}^{\rm DQ}
      ...
      in $\widehat K_{s,\rm face}^{\rm DQ}$; it is never called an $M\times M$
      density.
\item Uniformly for $|s|<s_0$, the centered evolution has the two genuine
      ...
      \end{align}
      with $\theta_\pm<1$.  These are Banach-space estimates, not an
      assumed CM2 estimate for $\widehat K_{s,\rm sw}^{\rm DQ}$.
\item The same double-projected typed decomposition holds for the limiting
      ...
      assembled, oriented, double-projected currents on the common
      $\mathrm{UCR}_{\rm sh}$ atlas.  Throughout,
      ...
      \end{align}
      Here $\mathcal S_{L,m,n}$ is the finite union of genuine singularity
      and common-atlas boundaries encountered by the depth-$L$ face words
...
before signed recombination.  In particular
...
\end{equation}
and adding the part deeper than $L(s)$ changes the left side by $o(1)$.
\end{theorem}
...
$\ell>L(s)$.  The order is therefore $L_0$ first, $s\to0$ second, and the
deep positive-envelope estimate last.  A fixed-depth topology is never used
...
face, or response types in
Definition~\ref{def:measure-gauge-interface}.  Moreover
\begin{equation}
...
response block converges to $\widehat K_{V,\rm sw}$ and the $E_s$ pairing
...
absolute CM2 double sum for all three types.  Explicitly, if
$B_{s,m,n}^{j}$ denotes the pairing of type
$j\in\{\mathrm{reg},\mathrm{face},\mathrm{sw}\}$ and the corresponding
ledger gives
\begin{equation}
...
and the $E_s$ pairing tend to zero.  The double projection
\eqref{eq:typed-block-double-projection} makes each output eligible for the
zero-mass estimates without cancellation between types.  Dominated
...
For the face core, first test
\eqref{eq:face-DQ-weak-convergence} with a bounded--Lipschitz approximation
outside $[\mathcal S_{L,m,n}]_\rho$; then use
...
cemetery tilt.
The complete actual tree law extending $\mathrm{NST}_{\rm phys}$ and every
complete actual/split law used by the functional-correlation estimate
satisfy, with the
\emph{same} weight \eqref{eq:global-stopped-source-weight},
$0<\mathbb E_{\rm stop}[\mathscr W_{\lambda,*}]\le C_W$ uniformly in the
allowed parameters and depths, and
\begin{equation}
\label{eq:upper-scale-excess-tail}
 \mathbb E_{\rm stop}\!\left[
  \mathscr W_{\lambda,*}\one_{\mathsf G}
  \one_{\{\widetilde G_N^\pm>\gamma_gN+u\}}\right]
 \le C\,\mathbb E_{\rm stop}[\mathscr W_{\lambda,*}]
...
$\mathrm{CLOCK\_SUM}$ below.  The same complete label law is used for the
...
$L^1$ integrability.  Equivalently, with
\[
 d\mathbb P_{\rm stop}^{W}
 :=\frac{\mathscr W_{\lambda,*}}
 {\mathbb E_{\rm stop}[\mathscr W_{\lambda,*}]}
 d\mathbb P_{\rm stop},
\]
the survivor subprobability has augmented grazing exponent $q_g^W$ under the
normalized complete positive source law.  This is the ``normalize the
...
\eqref{eq:upper-scale-excess-tail} is an unweighted exponent $q_g$ together
with some $p_W>1$ satisfying
\begin{equation}
\label{eq:USC-holder-sufficient-condition}
 \|\mathscr W_{\lambda,*}\|_{L^{p_W}(\mathbb P_{\rm stop})}
 \le C\,\mathbb E_{\rm stop}[\mathscr W_{\lambda,*}].
\end{equation}
H\"older, applied once to the survivor excess event, gives only
$q_g^W=q_g/p_W'$, and all ledgers below use this degraded exponent.
\end{definition}
...
\begin{definition}[Stopped sources and cemetery-current domination
$\mathrm{CEM\_DOM}$]
\label{def:cemetery-source-domination}
Let
$\mathcal I_{\rm cem}=\{\mathrm{ctx},\mathrm{stop},\mathrm{buf},
\mathrm{gate},\mathrm{cont}\}$.  For $i\in\mathcal I_{\rm cem}$ let
$\tau_i$ be the first failure stage and $\mathcal F_{\tau_i}$ the information
available immediately before that failure.  On $B_i$ the exact discarded
current is required to factor as
\begin{equation}
\label{eq:exact-stopped-source-factorization}
 J_i^{\rm disc}=U_i\Sigma_i,
 \qquad d|\Sigma_i|=A_i\,d\Pi_{s,N}|_{B_i},
\end{equation}
where $\Sigma_i$ is the positive total-variation source created on the
failure surface.  This factorization is supplemented by fixed, parameter-
independent source and test spaces
$\mathfrak S_i^{\varepsilon_i}$ and $\mathfrak T_i$, with
orientation $\varepsilon_i\in\{+,-\}$, and by a recovery time $R_i$ such
that, labelwise,
\begin{equation}
\label{eq:cemetery-fixed-source-norm}
 \|\Sigma_i(\lambda)\|_{\mathfrak S_i^{\varepsilon_i}}
 \le A_i(\lambda).
\end{equation}
The spaces, orientation, and constants are fixed before $m,n$, the Fourier
mode, and the eventual observable are chosen.  Let
$\mathscr A_i^{\rm opp}\ge1$ be the already frozen amplitude on the opposite
endpoint.  There must be an $\mathcal F_{\tau_i}$-measurable predictable
factor $\mathcal C_i\ge1$, independent of those final choices, such that
\begin{equation}
\label{eq:cemetery-RN-domination}
 A_i\mathscr A_i^{\rm opp}\gamma_i^{-R_i}
 \le C\,\mathscr W_{i}^{\rm past}\mathcal C_i
 =:C\,\mathscr W_i^{\rm cem}
 \quad\Pi_{s,N}\text{-a.e. on }B_i.
\end{equation}
Here $\mathscr W_i^{\rm past}$ is the product of precisely those factors in
\eqref{eq:global-event-weight} already measurable at $\tau_i$; absent
factors are not presumed harmless and are instead included in
$\mathcal C_i$.
Consequently, for every admissible test $\Phi$,
\begin{equation}
\label{eq:cemetery-current-weighted-bound}
 |\langle J_i^{\rm disc},\Phi\rangle|
 \le C\|\Phi\|_{\rm test}
 ...
 =C\|\Phi\|_{\rm test}\,
   ...
   \Pi_{s,N}^{W}(B_i).
\end{equation}
A future factor may be replaced by one only when the remaining $U_i$ is a
positive total-variation contraction; then one takes $R_i=0$ and
$\gamma_i^{-R_i}=1$.  Otherwise its recovery and propagation envelope is
frozen in $\mathcal C_i$ before the cemetery transition.  In particular,
$\mathcal C_i$ cannot be defined retrospectively using the final test norm.
The five rows of the interface are
\begin{center}
\small
\begin{tabular}{c|c|c|c|c}
failure & fixed source space & orientation & stopped source & cost\\ \hline
ctx & $\mathfrak S_{\rm flux}^{\varepsilon}$ & $\varepsilon$ & context-gate flux & $\mathcal C_{\rm ctx}$\\
stop & $\mathfrak S_{\rm sf}^{\varepsilon}$ & $\varepsilon$ & antichain complement & $\mathcal C_{\rm stop}$\\
buf & $\mathfrak S_{\rm sf}^{\varepsilon}$ & $\varepsilon$ & first buffer failure & $\mathcal C_{\rm buf}$\\
gate & $\mathfrak S_{\rm flux}^{\varepsilon}$ & $\varepsilon$ & physical-gate flux & $\mathcal C_{\rm gate}$\\
cont & $\mathfrak S_{\rm sf}^{\varepsilon}$ & $\varepsilon$ & first continuation failure & $\mathcal C_{\rm cont}$
\end{tabular}
\end{center}
Each row is either proved from the displayed source factorization and the
positive-source propagation lemma below, or is retained as a separately
falsifiable primitive.  Its complete registry also records $U_{i,s}$,
$R_i$, $\mathscr A_i^{\rm opp}$, the event clock, and the moment law for
$\mathcal C_i$.  Measurability of a cemetery label alone does not imply
\eqref{eq:cemetery-RN-domination}.
\end{definition}

\begin{lemma}[Fixed-norm stopped source and summable propagation]
\label{lem:stopped-source-propagation}
For each $i\in\mathcal I_{\rm cem}$ assume that the remaining forward or
reverse propagator is a fixed operator family
$U_{i,s}^{k,\varepsilon_i}:\mathfrak S_i^{\varepsilon_i}\to
\mathfrak T_i^*$ and that, uniformly in the moving-table parameter, common
atlas, time, and orientation,
\begin{equation}
...
\eqref{eq:cemetery-RN-domination} imply
\eqref{eq:cemetery-current-weighted-bound} for every $m,n,s$, scalar mode,
and admissible test, with no constant depending on those choices.  This is
the source-first structure of
\cite[Proposition~2.5 and Hypothesis~H4]{CanestrariPerturb} and
\cite[Lemma~5.10]{CanestrariHoles}; those results are templates and do not
verify this interface for a moving billiard.
\end{lemma}

\begin{proof}
Normalize the positive source in the fixed norm, recover it in the declared
orientation, apply the one-time coupling or dual memory estimate, and
multiply its original mass and the already frozen opposite amplitude back.
The worst pre-recovery cost is bounded by $\gamma_i^{-R_i}$ and is part of
$\mathcal C_i$.  Since the operator estimate is uniform before the final
test is selected, $\mathcal C_i$ is a genuine source envelope rather than a
renaming of the desired discarded-current bound.  No signed cancellation is
used.
\end{proof}
...
\begin{align}
 ...
 \Pi_{s,N}^{W}(B_{\rm PPE}(r,P))&\le Cr^{\Theta_{\rm PPE}^{W}}.
 \label{eq:tilted-PPE-tail}
\end{align}
Every additional exceptional event must also satisfy the aggregate clock
interface below.  Atomic tail bounds by themselves are not part of
$\mathrm{TENERGY}_{W}$.
\end{definition}

\begin{definition}[Aggregate event-clock summability
$\mathrm{CLOCK\_SUM}_{W}$]
\label{def:aggregate-clock-summability}
Every additional exceptional event is entered in a registry with fields
\[
 (\text{event},\text{first-failure stage},\text{raw tail},
   \text{clock},\text{passes through Jensen?}).
\]
Fix a deterministic total order inside each first-failure stage and replace
the registered events by their first-hit disjointization
\begin{equation}
\label{eq:clock-first-hit-disjointization}
 \widehat B_{\tau,j}:=B_{\tau,j}\setminus
   \bigcup_{k<j}B_{\tau,k}.
\end{equation}
Thus every source/current occurs once.  A discarded atom refines exactly one
of the five cemetery stages and inherits that row of
$\mathrm{CEM\_DOM}$; a retained absolute remainder stays under
$\mathscr W_{\rm surv}$.  Put
\begin{align}
 \mathfrak Z_r^W(r)&:=
  \sum_{j\in\mathcal J_r}\Pi^W(\widehat B_{r,j}(r)),
 \label{eq:clock-partition-polynomial}\\
 \mathfrak Z_J^W(N)&:=
  \sum_{i\in\{\rm buf,gate,cont\}}\Pi^W(B_i(N))
  +\sum_{j\in\mathcal J_{\rm PPE}^{J}}
       \Pi^W(\widehat B_{J,j}(N)),
 \label{eq:clock-partition-PPE-Jensen}\\
 \mathfrak Z_{\rm TV}^W(N)&:=
  \sum_{j\in\mathcal J_{\rm PPE}^{\rm TV}}
       \Pi^W(\widehat B_{{\rm TV},j}(N)),
 \label{eq:clock-partition-PPE-TV}\\
 \mathfrak Z_{\rm time}^W(N)&:=
  \sum_{j\in\mathcal J_{\rm time}}
       \Pi^W(\widehat B_{{\rm time},j}(N)).
 \label{eq:clock-partition-time}
\end{align}
The four legal central clocks require the \emph{aggregate} bounds
\begin{align}
 \mathfrak Z_r^W(r)&\le Cr^{\Theta_{\rm clk}^{W}},
   &&\Theta_{\rm clk}^{W}>0,
   \label{eq:clock-polynomial-r}\\
 \mathfrak Z_J^W(N_{\rm PPE}(r))&\le
   Ce^{-c_{\rm PPE,J}^{W}N_{\rm PPE}(r)},
   &&c_{\rm PPE,J}^{W}>0,
   \label{eq:clock-PPE-Jensen}\\
 \mathfrak Z_{\rm TV}^W(N_{\rm PPE}(r))&\le
   Ce^{-c_{\rm PPE,TV}^{W}N_{\rm PPE}(r)},
   &&c_{\rm PPE,TV}^{W}>0,
   \label{eq:clock-PPE-TV}\\
 \mathfrak Z_{\rm time}^W(N_{\rm time})&\le
   Ce^{-c_{\rm event,time}^{W}N_{\rm time}},
   &&c_{\rm event,time}^{W}>0.
   \label{eq:clock-genuine-time}
\end{align}
An empty class is assigned zero partition function and, in the displayed
minimum formulas below, its exponent/rate is set to $+\infty$ so that it is
automatically omitted.  A fixed finite registry with uniform prefactors is a special case;
uniform positivity of the atomic rates without these partition bounds is not.
The active registry contains, at minimum,
\begin{center}
\begin{tabular}{c|c|c|c|c}
event & first stage & raw tail & clock & Jensen\\ \hline
bad context & context gate & $r^{\Theta_{\rm ctx}^W}$ & $r$ & yes\\
native stop & antichain & $r^{\Theta_{\rm st}^W}$ & $r$ & yes\\
buffer/gate/continuation & corresponding stop & $e^{-c_i^WN_{\rm PPE}}$ & PPE & yes\\
adaptive-alphabet energy loss & physical gate & $e^{-c_j^WN_{\rm PPE}}$ & PPE & yes\\
PPE absolute remainder & retained descendant & $e^{-c_j^WN_{\rm PPE}}$ & PPE & no\\
contracted-coordinate main error & endpoint replacement & $e^{-c_j^WN_{\rm time}}$ & time & no
\end{tabular}
\end{center}
Any further event must add a row before it is used.
The superscript $J$ means that the event is inside the energy estimate and
therefore suffers Jensen's square-root loss; the TV class is summed
absolutely outside Jensen.  Word-depth tails belong only to
$\mathrm{FACE}_{\rm 2CUT}$ or the deep ledger.  The last PPE line above includes
the positive retained amplitude: it is a statement
under the same full tree tilt, not merely the pointwise multiplication
clause in $\mathrm{PPE}_{N}$.  Put
\begin{equation}
\label{eq:weighted-energy-rates}
 \Theta_{\rm poly}^{W}:=\min\!\left\{\Theta_{\rm PPE}^{W},
       \Theta_{\rm ctx}^{W},\Theta_{\rm st}^{W},
       \Theta_{\rm clk}^{W}\right\},
\end{equation}
The three exponential rates in \eqref{eq:clock-PPE-Jensen}--
\eqref{eq:clock-genuine-time} are aggregate rates of the disjoint partition
functions, never infima of atomic rates.  Native stopping belongs to
$\Theta_{\rm poly}^{W}$ and is not counted again as a time event.

Here is a useful sufficient route which exposes registry complexity.  Suppose
for one exponential clock $\tau$ that, before tilting,
\begin{equation}
\label{eq:clock-raw-complexity-gap}
 \Pi_{s,N}(\widehat B_{\tau,j}(N))
 \le C_{\tau,j}(N)e^{-c_\tau N},\qquad
 \sum_j C_{\tau,j}(N)\le Ce^{a_\tau N},\qquad c_\tau>a_\tau.
\end{equation}
Summing the disjoint raw partition first and applying H\"older once gives the
effective tilted rate
\begin{equation}
\label{eq:clock-aggregate-holder-rate}
 c_\tau^W=\frac{c_\tau-a_\tau}{p_W'}.
\end{equation}
Tilting each atom first and only then summing would in general give the weaker
$c_\tau/p_W'-a_\tau$; these two operations are not interchangeable.  For the
polynomial clock, the corresponding sufficient condition is
\begin{equation}
\label{eq:clock-polynomial-complexity-gap}
 \sum_j C_{r,j}(r)\le Cr^{-a_r},\qquad
 \Pi_{s,N}(\widehat B_{r,j}(r))\le C_{r,j}(r)r^{\Theta_r},
 \qquad
 \Theta_{\rm clk}^{W}=\frac{\Theta_r-a_r}{p_W'}>0.
\end{equation}
Thus a nonpositive aggregate gap is a genuine no-go, not a matter of choosing
another atomic rate.
The ledger may be verified directly as a primitive.  Alternatively, if the
corresponding unweighted complete-tree tails hold and the full-product
estimate
\begin{equation}
\label{eq:full-product-Lp-energy-route}
 \|\mathscr W_*\|_{L^{p_W}(\Pi_{s,N})}
 \le C\mathbb E_{\Pi_{s,N}}\mathscr W_*,
 \qquad p_W>1,
\end{equation}
holds uniformly, an already aggregated polynomial or exponential tail is
degraded only to
\begin{equation}
\label{eq:holder-degraded-energy-rates}
 \Theta_{\rm agg}^{W}=\Theta_{\rm agg}/p_W',\qquad
 c_{\rm agg}^{W}=c_{\rm agg}/p_W'.
\end{equation}
Every subsequent phase and window ledger uses these degraded rates.  If no
such $p_W>1$ exists, the unweighted tails do not imply
$\mathrm{TENERGY}_{W}+\mathrm{CLOCK\_SUM}_{W}$.
\end{definition}
...
sufficient condition \eqref{eq:USC-holder-sufficient-condition} is used,
H\"older is applied before this split and the exponent is $q_g/p_W'$.  This
...
\eqref{eq:full-product-Lp-energy-route} then degrades it, and every named
exponential rate, according to
\eqref{eq:holder-degraded-energy-rates}.  No unweighted rate is retained
after multiplication by $\mathscr W_*$.
\end{proof}
...
$\mathrm{TENERGY}_{W}$, either directly or through
\eqref{eq:full-product-Lp-energy-route}.  It is not a deduction from the
...
\label{eq:survivor-mass-lower-bound}
 ...
 1-\Pi^W(B_{\rm ctx})-\Pi^W(B_{\rm stop})
   -\mathfrak Z_r^W(r)-\mathfrak Z_J^W(N_{\rm PPE}(r))
 \ge\frac12.
\end{equation}
...
\eqref{eq:tilted-auxiliary-tails}, \eqref{eq:clock-polynomial-r}, and
\eqref{eq:clock-PPE-Jensen}.  The two aggregate partition functions tend to
zero at the prescribed depth; shrink the admissible $r$-range so their sum
with the two named polynomial tails is at most $1/2$.  No atomic union bound
is used.
\end{proof}

\begin{theorem}[Complete tilted physical single-source energy tree]
\label{thm:direct-slope-small-ball}
Let $w_\beta^{W}$ and $\mu_\beta^{W}$ be the normalized branch
disintegration of the conditional survivor law in
Lemma~\ref{lem:retained-tree-normalization}, and put
$\nu_\beta^{W}=(S_\beta)_*\mu_\beta^{W}$.  With
\begin{equation}
\label{eq:physical-energy-exponent}
 \Theta_{\rm poly}^{W}=\min\left\{\Theta_{\rm PPE}^{W},
        \Theta_{\rm ctx}^{W},\Theta_{\rm st}^{W},
        \Theta_{\rm clk}^{W}\right\}>0,
\end{equation}
one has
\begin{align}
 ...
 &\le Cr^{\Theta_{\rm poly}^{W}}+C\mathcal T_E^{W}(r),\label{eq:conditional-energy-bound}\\
 \sup_z\sum_\beta w_\beta^{W}\nu_\beta^{W}([z-r,z+r])
 &\le Cr^{\Theta_{\rm poly}^{W}/2}+C\mathcal T_E^{W}(r)^{1/2}.
 \label{eq:slope-value-small-ball}
\end{align}
where, with $N_{\rm PPE}(r):=N(r)$,
\begin{equation}
\label{eq:complete-energy-tail}
 \mathcal T_E^{W}(r):=
 \mathfrak Z_J^W(N_{\rm PPE}(r))
 \le Ce^{-c_{\rm PPE,J}^{W}N_{\rm PPE}(r)}.
\end{equation}
\end{theorem}

\begin{proof}
Fix $t'$ and all frozen physical data, and disintegrate first over the
complete positive law $\Pi_{s,N}^{W}$, before restricting to surviving
labels.  By
...
$\sigma=(r/c_\ell)^{1/(1+\alpha_L)}$ and $N=N(r)$.  Apply
\eqref{eq:ppe-prescribed-sublevel} at this same depth; no comparison with an
auxiliary $J$ or a clean-core shadow is made.  On good contexts combine
...
handles bad contexts, the tilted stopping event handles the exceptional
antichain mass, and the aggregate Jensen-class partition function
\eqref{eq:clock-partition-PPE-Jensen} gives exactly
$\mathcal T_E^{W}(r)$.  The
$r$-polynomial registry is absorbed by $\Theta_{\rm poly}^{W}$.  All
failures are disjoint measurable pieces of the
same complete probability tree, so their union bound and the following
branch disintegration are legitimate before signed recombination.  Integrate
in $t'$ and then over branches.  Finally use
\[
 \nu_\beta^{W}(I)^2\le
 (\nu_\beta^{W}\otimes\nu_\beta^{W})\{|u-v|\le2|I|\}
\]
...
The conditional normalization costs at most two by
Lemma~\ref{lem:retained-tree-normalization}.  For cemetery pieces,
\eqref{eq:cemetery-current-weighted-bound} first converts the tilted
probability tail into the actual discarded-current bound.
\end{proof}
...
\eqref{eq:slope-value-small-ball} at $s=0$.  At finite $s$ the constants are
...
\begin{proof}
Apply Theorem~\ref{thm:direct-slope-small-ball}.  Multiplying the normalized
bound back by $\mathbb E_{\Pi_{s,N}}\mathscr W_*$ gives the complete marked
phase mass; no signed cancellation or unweighted Jensen step is used.
\end{proof}
...
\end{equation}
with $a_X,a_Y$ as in \eqref{eq:dominant-coordinate-normalization}.  Under
the complete event tilt, the mass of $\{|\psi_\xi'|\le\Xi r\}$ is at most
$C(r^{\Theta_{\rm poly}^{W}/2}+\mathcal T_E^{W}(r)^{1/2})$.  Smooth value-space cutoffs on its complement have
...
genuine $N_{\rm time}=\max\{M,N\}$ event and is registered in
$\mathcal J_{\rm time}$.  Any label on which uniform contraction is
unavailable is separately registered according to the stage where failure
is first observed: in $\mathcal J_r$, $\mathcal J_{\rm PPE}^{J}$,
$\mathcal J_{\rm PPE}^{\rm TV}$, or $\mathcal J_{\rm time}$.  It is never
...
\eqref{eq:clock-polynomial-r}--\eqref{eq:clock-genuine-time}.  In the diagonal band these imply
...
Use the complete aggregate tilted rates
\eqref{eq:weighted-energy-rates} and
\eqref{eq:clock-PPE-Jensen}--\eqref{eq:clock-genuine-time}, and set
\[
...
\begin{aligned}
 \beta_{\rm PPE,J}^{W}&:=\tfrac12\delta C_{\rm buf}c_{\rm PPE,J}^{W},
 &\beta_{\rm PPE,TV}^{W}&:=\delta C_{\rm buf}c_{\rm PPE,TV}^{W},\\
 \beta_{\rm phase}^{W}&:=
 \min\{\delta\Theta_{\rm poly}^{W}/2,\alpha-A_{\rm frac}\delta,
             \beta_{\rm PPE,J}^{W},\beta_{\rm PPE,TV}^{W}\}>0.
\end{aligned}
\end{equation}
Put $N=\max\{m,n\}$ and choose
\begin{equation}
...
\end{equation}
has saving \(C\Xi^{-\beta_{\rm phase}^{W}}\) times a fixed power of
...
\label{eq:phase-energy-all-tails}
 C\Xi^{-\delta\Theta_{\rm poly}^{W}/2}
 +C\Xi^{-\delta C_{\rm buf}c_{\rm PPE,J}^{W}/2}.
\end{equation}
Indeed $N_{\rm PPE}(r)=C_{\rm buf}\log(1/r)+O(1)$; no time-depth symbol is
substituted for this resolving depth.  The PPE-depth events in
$\mathcal J_{\rm PPE}^{\rm TV}$ are outside Jensen and contribute, by
weighted absolute summation,
\begin{equation}
\label{eq:phase-PPE-TV-tails}
 C\Xi^{-\delta C_{\rm buf}c_{\rm PPE,TV}^{W}};
\end{equation}
this is the origin of $\beta_{\rm PPE,TV}^{W}$ and has no square-root loss.
On the complement use two smooth
cutoffs in
...
\label{eq:complete-central-exponent}
 ...
 c_{\rm low},c_{\rm event,time}^{W}\}>0,
\end{equation}
where $c_{\rm low}$ is supplied by the low recovery theorem and
$c_{\rm event,time}^{W}>0$ contains only genuine $N_{\rm time}$ rates,
including the contracted-coordinate remainder when applicable.  All
...
\label{eq:regular-low-projector}
 ...
 \mathsf D_{\rm cur}[(\rho_K^{(4)}*k_{\rm reg})dm_{M\times M}]
...
the face estimate.  The ordinary four-dimensional low projector of the
...
\item The normal-cycle value $\mathfrak A=-1$ motivates the clean template;
      ...
      $\mathrm{TENERGY}_{W}+\mathrm{CLOCK\_SUM}_{W}$, together with the
...
$\mathrm{SS}$, $\mathrm{PPE}_{N}$, $\mathrm{BR}_{\rm sec}$,
$\mathrm{CEM\_DOM}$, $\mathrm{TENERGY}_{W}$,
$\mathrm{CLOCK\_SUM}_{W}$, $\mathrm{FZ}_{\rm geom}$,
$\mathrm{SF}_{\rm split}$, $\mathrm{CP}_{\rm env}$,
$\mathrm{FACE}_{\rm 2CUT}$, and pathwise
...
$C|s|^{1-\kappa_L(A+B)}=o(1)$.  The interval for $\kappa_L$ is nonempty precisely
...
\end{enumerate}
The constants in $\mathrm{NST}_{\rm phys}$,
$\mathrm{BR}_{\rm sec}$, $\mathrm{PPE}_{N}$, $\mathrm{FZ}_{\rm geom}$,
$\mathrm{SF}_{\rm split}$, $\mathrm{CP}_{\rm env}$, and
$\mathrm{REC}(q_{\rm rec})$, together with
$\mathrm{CEM\_DOM}$, $\mathrm{TENERGY}_{W}$,
$\mathrm{CLOCK\_SUM}_{W}$ and
$\mathrm{USC}_{\rm end}^{W}(q_g^W)$ on the
complete actual/split tree laws, are required
to be uniform on these cells.
...
\item $\mathrm{SHSRC}^{+}$: before signed recombination, the exact
      actual/split expansion has positive pieces of masses
      $M^{\rm sh}_{\lambda,s,L}$ and
      \begin{equation}
...
words deeper than $L$.  Before signed recombination, write every actual/split
constituent as positive pieces of masses $M_{\lambda,s,L}$ and define
\begin{equation}
...
\end{equation}
The finite split coefficients are included in the sum.  Connected-pullback
...
Assume $\mathrm{UCR}_{\rm sh}$, $\mathrm{NST}_{\rm phys}$,
$\mathrm{SS}$, $\mathrm{PPE}_{N}$, $\mathrm{CEM\_DOM}$,
$\mathrm{TENERGY}_{W}$,
$\mathrm{CLOCK\_SUM}_{W}$,
$\mathrm{USC}_{\rm end}^{W}$,
$\mathrm{BR}_{\rm sec}$, $\mathrm{REC}$, $\mathrm{MT}_{\rm DQ}$, the
complete labelled ledger \eqref{eq:global-ledger-sum}, and
$\mathrm{SHCOL}_{\rm env}$, and require the strict window
\begin{equation}
...
\end{equation}
For $N=\max\{m,n\}\le T_{\rm sh}(s)$ the shallow difference quotient has
the same product-CM2 majorant as the limiting shallow current.  For
$N>T_{\rm sh}(s)$,
\begin{equation}
...
upper endpoint-scale cost.  Thus the common shallow refinement supports the complete three-frequency
proof without an infinite-depth call.  For $N>T_{\rm sh}(s)$ do not continue
...
by $1/p_W'$; context, stopping, PPE, and grazing rates are degraded on their
corresponding aggregate events.  Otherwise $\mathrm{TENERGY}_{W}$,
$\mathrm{CLOCK\_SUM}_{W}$, and $\mathrm{USC}_{\rm end}^{W}$ remain independent
primitive inputs.  Complete the aggregate clock partition before choosing
any phase parameter; no bare atomic event rate survives this step.  Next fix the coefficient
...
the proof: it improves the typed $\beta_{\rm PPE,J}^{W}$ and
$\beta_{\rm PPE,TV}^{W}$ rates but enlarges $C_{\rm mid}$.
...
\begin{lemma}[Complete-tree refinement and full-product $L^{p_W}$]
...
\end{equation}
and, with $\mathfrak F$ denoting a positive mark--$Z$ envelope, require the
...
unbounded moment.  These estimates are uniform for actual, split, and
cemetery parents.  The conditional
...
\label{eq:available-full-product-pW}
 1<p_W<\chi_W^{-1}
\end{equation}
is available uniformly on the complete actual/split energy tree, and
\begin{equation}
\label{eq:proved-full-product-Lp}
 \|\mathscr W_*\|_{L^{p_W}(\Pi_{s,N})}
 \le C\le C\mathbb E_{\Pi_{s,N}}\mathscr W_*.
\end{equation}
In particular \eqref{eq:full-product-Lp-energy-route} and
\eqref{eq:USC-holder-sufficient-condition} follow from the displayed
moment assumptions with this same $p_W$.  All coarser marginals, including
the positive cemetery masses, are unchanged by the refinement.
\end{lemma}
...
\eqref{eq:global-stopped-source-weight} to $p_W$.  On a survivor use
\eqref{eq:global-event-weight}; on $B_i$ use
\eqref{eq:cemetery-RN-domination}.  Generalized H\"older on the complete tree
...
\eqref{eq:proved-full-product-Lp}.  Since $\mathscr W_*\ge1$, its expectation
is at least one, giving the second inequality.  This proves an actual
...
\begin{corollary}[Full-product H\"older transfer of the energy tree]
\label{cor:full-product-holder-energy}
Assume the unweighted complete-tree versions of the context, stopping,
buffer, gate, continuation, prescribed-depth PPE bounds, and aggregate clock
partition bounds in
$\mathrm{NST}_{\rm phys}/\mathrm{PPE}_{N}$, and assume
\eqref{eq:conditional-refinement-moments} and
\eqref{eq:complete-tree-cemetery-moment} with $\chi_W<1$.  Choose $p_W$ by
\eqref{eq:available-full-product-pW}.  Then
$\mathrm{TENERGY}_{W}+\mathrm{CLOCK\_SUM}_{W}$ hold with every already
aggregated polynomial and exponential rate degraded by
\[
 \Theta_{\rm agg}^{W}=\Theta_{\rm agg}/p_W',\qquad
 c_{\rm agg}^{W}=c_{\rm agg}/p_W',
\]
after registry complexity has been subtracted as in
\eqref{eq:clock-aggregate-holder-rate}--
\eqref{eq:clock-polynomial-complexity-gap}.
The tilted upper-scale bound follows with $q_g^{W}=q_g/p_W'$ whenever its
raw complete-tree tail is available.
\end{corollary}
...
\]
Apply this once to each already disjoint aggregate event and to the retained
PPE sublevel.  No branchwise Jensen or signed cancellation occurs in this
transfer; applying H\"older atom by atom would instead incur the weaker rate
described after \eqref{eq:clock-aggregate-holder-rate}.
\end{proof}
...
the complete-tree cemetery moment
\eqref{eq:complete-tree-cemetery-moment},
...
$\mathrm{SF}_{\rm split}$, $\mathrm{CP}_{\rm env}$,
$\mathrm{UCR}_{\rm sh}$, and the pathwise jet condition of
Definition~\ref{def:quantitative-circular-jets}.  Let
...
\end{equation}
where $\mathscr W_{\lambda,*}$ is exactly the piecewise weight in
\eqref{eq:global-stopped-source-weight}; no smaller unweighted ledger is substituted
when scale-bad labels are removed.
...
resulting $p_W$ in \eqref{eq:available-full-product-pW} proves both
\eqref{eq:global-ledger-sum} and the full-product estimate used to degrade
the raw energy tails.  No unrecorded $p>2$ moment and no intersection of
...
clock class is charged through its disjoint aggregate partition function in
$\mathrm{CLOCK\_SUM}_{W}$.  For cemetery pieces the
...
\item $\mathrm{NST}_{\rm phys}$, $\mathrm{SS}(d_Z)$, and
      $\mathrm{PPE}_{N}(d_Z)$ on the persistent clean gate, together with
      stopped-source domination $\mathrm{CEM\_DOM}$, the complete tilted
      energy ledger $\mathrm{TENERGY}_{W}$, the aggregate disjoint registry
      $\mathrm{CLOCK\_SUM}_{W}$, and the
      tilted endpoint upper-scale ledger $\mathrm{USC}_{\rm end}^{W}(q_g^W)$,
      dominant chart bounds \eqref{eq:primitive-dominant-chart-bounds} and
      the branch-section interface $\mathrm{BR}_{\rm sec}$, all with the
...
\item $\mathrm{MT}_{\rm DQ}$ holds for the fixed-gauge operator quotient,
      ...
      estimate \eqref{eq:face-middle-tail}.
\end{enumerate}
...
and time-depth splits.  At every fixed $(m,n)$ the response, regular, and face
blocks converge respectively by
...
\label{eq:final-CM2-exponent}
 ...
 \eta_{\rm hi}\rho_\infty-a_{\rm ultra},c_{\rm scl},
 c_{\rm event,time}^{W},\eta_{\rm lo}R,
...
transported observable changes the target.  Every graph-supported
...
majorant is instead supplied by the event/section/positive-envelope ledgers.
Thus no
...
$\mathrm{PPE}_{N}$, $\mathrm{CEM\_DOM}$, and
$\mathrm{TENERGY}_{W}+\mathrm{CLOCK\_SUM}_{W}$, together with the complete
...
assertion is used in this step.  The three fixed-term topologies,
$\mathrm{FACE}_{\rm 2CUT}$, and all uniform ledgers now meet exactly in
\eqref{eq:typed-common-dominated-convergence}; this is the final
limit--double-sum exchange.
\end{proof}

\begin{corollary}[Product-a.e. analytic paths and the circular boundary]
\label{cor:analytic-pathwise-CM2}
Assume all hypotheses of Theorem~\ref{thm:global-cm2-assembly}.  In
particular this includes $\mathrm{MT}_{\rm DQ}$,
$\mathrm{NST}_{\rm phys}$, $\mathrm{PPE}_{N}$,
$\mathrm{CEM\_DOM}$, $\mathrm{TENERGY}_{W}$,
$\mathrm{CLOCK\_SUM}_{W}$,
$\mathrm{USC}_{\rm end}^{W}$,
$\mathrm{BR}_{\rm sec}$,
...
$\mathrm{FACE}_{\rm 2CUT}$,
$\mathrm{SHCOL}_{\rm env}$, $\mathrm{TAIL}_{\rm env}^{+}$, and
$\mathrm{NREC}_{\rm deep}$.  If the analytic boundary family
...
The product randomness in this statement realizes only the quantitative jet
condition; it does not supply $\mathrm{MT}_{\rm DQ}$,
$\mathrm{REC}$,
$\mathrm{NST}_{\rm phys}$, $\mathrm{PPE}_{N}$,
$\mathrm{CEM\_DOM}$, $\mathrm{TENERGY}_{W}$,
$\mathrm{CLOCK\_SUM}_{W}$,
...
$\mathrm{CP}_{\rm env}$, $\mathrm{UCR}_{\rm sh}$,
$\mathrm{FACE}_{\rm 2CUT}$,
$\mathrm{SHCOL}_{\rm env}$, $\mathrm{TAIL}_{\rm env}^{+}$, or
...
In a compact table chart satisfying \textup{(H1)--(H5)} and
$\mathrm{UCR}_{\rm sh}$, $\mathrm{CEM\_DOM}$,
$\mathrm{TENERGY}_{W}$,
$\mathrm{CLOCK\_SUM}_{W}$,
$\mathrm{FACE}_{\rm 2CUT}$, $\mathrm{SHCOL}_{\rm env}$,
$\mathrm{TAIL}_{\rm env}^{+}$, and
$\mathrm{NREC}_{\rm deep}$, all constants in the
...
CM2 majorant of Theorem~\ref{thm:global-cm2-assembly}, while the deep
...
$\mathrm{TENERGY}_{W}+\mathrm{CLOCK\_SUM}_{W}$; no raw lattice cutoff or unweighted tail is used to
...
\begin{array}{ccl}
 \textup{(H1)--(H3),(H5)}&\Longrightarrow&
   \text{prescribed-depth endpoint low/high for }N\le T_{\rm sh}(s),\\
 \textup{(H1),(H4)}&\Longrightarrow&
   \text{positive-envelope removal for shallow }N>T_{\rm sh}(s),\\
 ...
 \textup{(H5)}&\Longrightarrow&
   \text{typed DQ limits plus the two-cutoff face limit}.
\end{array}
\]
\endgroup
...
response term is charged to the derived estimate
...
There are nineteen falsifiable interfaces.  $\mathrm{MT}_{\rm DQ}$ must
retain the fixed-gauge commutator, double-project each typed block, and
separate response-space mixing from difference-quotient convergence.
...
$\mathrm{PPE}_{N}$ must give actual-SRB escape at the prescribed depth.
...
$\mathrm{CLOCK\_SUM}_{W}$ must disjointize the clock registry and control its
aggregate partition functions after complexity, rather than merely type each
exceptional event as $r$-, PPE-, or genuine-time depth.
...
and $\mathrm{QT}_{\rm path}$ supplies the strict parameter-depth margin.
They are primitive quantitative conditions, not consequences hidden in
compactness or a Banach norm.

\begin{proposition}[Thirty-three obstruction tests]
\label{prop:v39-obstruction-tests}
...
\item A centered total quotient may split as two typed blocks with opposite
      nonzero masses.  The response-side zero-mass estimate is illegal for
      either block separately.  Equation
      \eqref{eq:typed-block-double-projection} rejects this model and records
      its marginal corrections.
...
\item If either $\beta_{\rm PPE,J}^{W}$ or
      $\beta_{\rm PPE,TV}^{W}$ is smaller than
      $\min\{\delta\Theta_{\rm poly}^{W}/2,
      \alpha-A_{\rm frac}\delta\}$, the old phase exponent overstates the
      saving.  The typed minimum \eqref{eq:complete-phase-exponent} returns
      the correct smaller rate.
\item Let $\mathbb P(j)\asymp e^{-qj}$,
      ...
      $L^{p_W}$ sufficient condition, rejects this correlated-mark model.
\item Let an auxiliary failure have raw probability $e^{-qN}$ and let the
      ...
      \eqref{eq:holder-degraded-energy-rates} use only the degraded rate.
...
\item Let a cemetery label set every unknown future factor to one, while the
      ...
      has only rate $q-a$.  Equation \eqref{eq:cemetery-RN-domination}
...
\item Counting native stopping both in $\Theta_{\rm poly}^{W}$ and again as
      a time event falsely improves a minimum.  The one-row-per-event rule
      and the disjoint clock classes forbid this double charge.
\item At depth $N$, let there be $e^{aN}$ disjoint events, each of mass
      ...
      rate $c-a$ and is order one when $a=c$.  The aggregate partition bounds
      \eqref{eq:clock-PPE-Jensen}--\eqref{eq:clock-genuine-time} reject the
      latter case.
\item In the preceding model, summing the raw partition and applying
      ...
      only $c/p_W'-a$.  Equation \eqref{eq:clock-aggregate-holder-rate}
      fixes the order and prevents the stronger rate from being borrowed by
      the weaker route.
...
\item A branch entering cemetery before depth $N$ has no future strip indices,
      ...
      $\mathrm{CEM\_DOM}$ and $\mathrm{CLOCK\_SUM}_{W}$.
\item A hierarchy may have exponentially decaying atomic activations but a
      ...
      survivor normalization nor a response majorant follows.  Conditions
      \eqref{eq:clock-raw-complexity-gap} and
      \eqref{eq:clock-polynomial-complexity-gap} declare this an explicit
      no-go rather than a tunable parameter loss.
\end{enumerate}
\end{proposition}

\begin{proof}
Items 1--3, 8--11, and 13--33 are direct finite-measure, frequency, or norm
