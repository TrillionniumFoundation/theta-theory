...
label have distinct duties: one records the exact physical mass and the
other the source-norm, recovery, and opposite-amplitude envelope.  Their
...
and operator-weighted integrability make the scalar weak pairing a bounded
...
exhaustive measurable owner partition prevents unclocked marked mass.
...
clock.  Its energy lift is performed on the actual joint
occurrence--carrier law.  The reference collision--SRB carrier and the source
carrier are related either by a uniform same-carrier density ratio or by an
explicit joint likelihood and, when necessary, a bounded-holonomy carrier
match.  Thus probability PPE is never moved by an outer occurrence
likelihood to an unrelated internal source law.  All H\"older and base-mass
losses enter explicit reference, source-energy, and value exponents before
this one global Cauchy step.  Only genuine
...
dual-mass source-kernel, joint occurrence--carrier likelihood,
...
source occurrence.  It never includes the two internal carrier coordinates
used by main energy; that change of law belongs exclusively to
Definition~\ref{def:joint-source-likelihood}.  A zero-mass source kernel is omitted.  Let
...
exact total-variation/probability mass of the stopped source, while $q_i$ is
only its source-norm envelope.  Put
$K_i(\lambda):=\|\nu_{i,\lambda}\|_{\mathfrak S_i^{\varepsilon_i}}$; in
general $K_i>1$.  The exact discarded current is
defined by its scalar pairing
\begin{equation}
\label{eq:exact-stopped-source-factorization}
 J_i^{\rm disc}(\Phi):=
 \int_{B_i}\int_{X_i}
   \epsilon_i(\lambda,x)[U_{i,\lambda}^{*}\Phi](x)
   \,d\nu_{i,\lambda}(x)\,m_i(\lambda)\,d\Pi_{s,N}(\lambda),
 \qquad |\epsilon_i|\le1.
\end{equation}
...
$(\lambda,x)\mapsto[U_{i,\lambda}^{*}\Phi](x)$ is measurable for every
$\Phi\in\mathfrak T_i$; this is part of the interface.  Consequently
Tonelli and the triangle inequality give
\begin{equation}
...
amplitude.
Any row not admitting this kernel representation remains a separately
falsifiable typed primitive.
\end{definition}
...
used.  The results of \cite{CanestrariPerturb,CanestrariHoles} are templates
for this source-first organization, not proofs for moving billiards.
\end{lemma}

\begin{definition}[Dual-mass marked source kernel
$\mathrm{DUAL\_MASS\_KERNEL}$]
\label{def:source-incidence-space}
Fix once and for all the standard-Borel mark space
\[
 Y_{\rm src}:=\mathcal O_{\rm src}\times\mathbb N\times
 \{r,\mathrm{PPE},N_{\rm time},\mathrm{word}\}\times
 \{J,\mathrm{TV}\}\times\{+,-\}\times
 \{\mathrm{sf}^+,\mathrm{sf}^-,\mathrm{flux}^+,\mathrm{flux}^-\}.
\]
The integer is the occurrence/component index supplied by the common
refinement.  Let $X_{\rm src}$ be the corresponding countable disjoint union
of the standard-family and flux carriers.  There are jointly measurable
...
$0\le g_\lambda\le1$.  The kernel $m_\lambda$ records exact physical
TV/probability mass; $q_\lambda$ records the larger source-norm envelope.
...
$K_{\lambda,a}:=\|\nu_{\lambda,a}\|_{\mathfrak S_a^{\varepsilon(a)}}$.
The record
\begin{equation}
\label{eq:source-incidence-record}
 (o(a),j(a),\kappa(a),\pi(a),\varepsilon(a),\operatorname{type}(a),
   U_{\lambda,a},K_{\lambda,a},R_{\lambda,a},
...
\label{eq:source-incidence-space}
 ...
 d\Pi_{s,N}(\lambda)\,q_\lambda(da)\,d\nu_{\lambda,a}(x).
\end{equation}
...
\label{eq:source-incidence-envelope}
 ...
 \gamma_a^{-R_{\lambda,a}}\mathscr A_{\lambda,a}^{\rm opp}
\end{equation}
for every active occurrence.  For every admissible test define
the exact assembled current only through the weak scalar integral
\begin{equation}
\label{eq:marked-source-assembled-pairing}
 \langle J_{\rm src},\Phi\rangle:=
 \int \epsilon(\lambda,a,x)
       g_\lambda(a)[U_{\lambda,a}^{*}\Phi](x)
       \,dM_{\rm src}^{q}(\lambda,a,x),
 \qquad |\epsilon|\le1.
\end{equation}
Let $C_U(\lambda,a)$ be the measurable operator mark satisfying
$\|U_{\lambda,a}\|_{\mathfrak S_a\to\mathfrak T_a^*}\le C_U(\lambda,a)$.
The global finite-envelope closure is the explicit requirement
\begin{equation}
\label{eq:global-finite-marked-envelope}
 Q_{s,N}:=\int q_\lambda(Y_{\rm src})\,d\Pi_{s,N}(\lambda)<\infty,
 \qquad
 Q_{s,N}^{U}:=\int C_U(\lambda,a)q_\lambda(da)
                  d\Pi_{s,N}(\lambda)<\infty.
\end{equation}
The allowed growth of these quantities with resolving depth is charged in
the BASE, CLOCK, FACE, and deep ledgers.  Fibrewise finiteness alone is not
enough.  Measurability of the integrand and
\eqref{eq:global-finite-marked-envelope} make
\eqref{eq:marked-source-assembled-pairing} a weak/Gelfand integral defining
one bounded functional on the fixed test space; no Bochner measurability is
presumed.  Tonelli gives uniformly on its unit ball
\begin{equation}
\label{eq:source-incidence-TV-bound}
 |\langle J_{\rm src},\Phi\rangle|
 \le Q_{s,N}^{U}\|\Phi\|_{\rm test}.
\end{equation}

Finally there is a measurable canonical owner map and, modulo
$M_{\rm src}^{q}$-null sets, the exhaustive identity
\begin{equation}
\label{eq:exhaustive-source-owner-partition}
 Y_{\rm src}=Y_{\rm cem}\sqcup Y_{\rm main}\sqcup
 Y_{\rm exc,J}\sqcup Y_{\rm exc,TV}\sqcup
 Y_{\rm face/word}\sqcup Y_{\rm deep}.
\end{equation}
The clock $r$, PPE, $N_{\rm time}$, or word is a measurable field/filter on
an owner cell, not a second owner.  Main cells at different resolutions are
filters of $Y_{\rm main}$, and the displayed partition is the coverage
identity used in the final phase assembly.  This construction replaces every
...
\begin{proof}
Use the exact identity $g_\lambda q_\lambda=m_\lambda$, the operator bound on
$m_\lambda\nu_{\lambda,a}$, and
\eqref{eq:source-incidence-envelope} to dominate it by
$C_Uq_\lambda$.  Tonelli then takes the supremum over the test unit ball and
bounds the result by $Q_{s,N}^{U}$.
...
$\kappa\in\{r,\mathrm{PPE},N_{\rm time}\}$ let
$\mathcal A_{\kappa,\mathrm{TV}}(t)$ be the Borel marked-source cell of every
\emph{exceptional} occurrence charged directly in total variation.  Let
$\mathcal A_{\kappa,J}(t)$ be the corresponding exceptional Jensen cell.
The main retained PPE source is excluded from both collections and is treated
by Theorem~\ref{thm:main-source-energy-lift}.

For a Jensen mark let $S_{\lambda,a}:X_{\operatorname{type}(a)}\to\mathbb R$
be its measurable slope/value coordinate and put
$\mu_{\lambda,a}:=(S_{\lambda,a})_*\nu_{\lambda,a}$.  Define the normalized
...
\end{align}
Here $h_r(r)=r$; the PPE- and time-clock radii are the declared resonance
windows at depth $N$.  The sets, source kernels, and slope maps are now
fully specified measurable data, rather than an unspecified ``bad/source
condition.''
...
\end{equation}
Every displayed value gap must be strictly positive.  The owner table now
contains only cemetery and retained exceptional remainders.  Context,
...
remainders in their TV cells.  The main retained resonant source is not a
primitive row.  Word-depth sources remain exclusively in
$\mathrm{FACE}_{\rm 2CUT}$ and the deep ledger.  An empty class has rate
...
$\mathcal T_{\rm prob}^{W}(r)$.  This proves the probability assertions.  All
...
\begin{definition}[Main marked-source base, energy, and value]
\label{def:main-source-BEV}
Let $\mathcal A_{\rm main}^{r}(r)$ and
$\mathcal A_{\rm main}^{\rm PPE}(N)$ be the Borel mark cells of the retained
main resonant source in the kernel of
Definition~\ref{def:source-incidence-space}.  They are disjoint from every
exceptional clock cell.  With the same slope laws $\mu_{\lambda,a}$ and
$M_*=\mathbb E\mathscr W_*$ as above, define
\begin{align}
 \mathcal B_{{\rm main},\kappa}(t)&:=M_*^{-1}
  \int_{\mathcal A_{\rm main}^{\kappa}(t)}q_\lambda(da)d\Pi(\lambda),
  \notag\\
 \mathcal E_{{\rm main},\kappa}(t)&:=M_*^{-1}
  \int_{\mathcal A_{\rm main}^{\kappa}(t)}
  (\mu_{\lambda,a}\otimes\mu_{\lambda,a})
  \{|u-v|\le2h_\kappa(t)\}\,q_\lambda(da)d\Pi(\lambda),
  \notag\\
...
\end{align}
These quantities are finite by the declared base-mass bounds below and are
defined on the marked kernel, not by summing currents over tree labels.
The two cells are measurable clock filters of $Y_{\rm main}$ in
\eqref{eq:exhaustive-source-owner-partition}; together with the exceptional,
TV, face/word, cemetery, and deep owner cells they leave no unclocked main
mass.
\end{definition}

\begin{definition}[Joint occurrence--carrier likelihood
$\mathrm{JSL}_{p}$]
\label{def:joint-source-likelihood}
Fix one nonzero main marked cell $A$ from
Definition~\ref{def:main-source-BEV}.  Let
$K_A^{\rm prob}(d\eta,da)$ be the normalized frozen-parent occurrence law
under which Theorem~\ref{thm:direct-slope-small-ball} is actually applied,
and let $\kappa_{\eta,a}\in\mathcal P(X_{\eta,a})$ be its collision--SRB
conditional on the same physical homogeneous carrier.  This is a declared
measurable lift of the theorem's branch disintegration to the canonical
occurrence mark, and its $D_h$ energy is required to be exactly the reference
energy bounded by \eqref{eq:conditional-energy-bound}; it is not an arbitrary
new occurrence law.  Define the reference
and source product laws
\begin{align}
 \mathsf R_A(d\eta,da,dx,dx')
 &:=K_A^{\rm prob}(d\eta,da)
    \kappa_{\eta,a}(dx)\kappa_{\eta,a}(dx'),\notag\\
 \mathsf S_A(d\eta,da,dx,dx')
 &:=\frac{\one_A(\eta,a)}{M_*\mathcal B_A}
    d\Pi(\eta)q_\eta(da)
    \nu_{\eta,a}(dx)\nu_{\eta,a}(dx').
 \label{eq:joint-reference-source-laws}
\end{align}
Here $\mathcal B_A=M_*^{-1}\int_Aq_\eta(da)d\Pi(\eta)$, so both laws are
probabilities.  The interface $\mathrm{JSL}_p$ requires
\begin{equation}
\label{eq:joint-source-likelihood}
 \mathsf S_A\ll\mathsf R_A,
 \qquad L_A:=\frac{d\mathsf S_A}{d\mathsf R_A},
 \qquad
 \sup_A\int L_A^p\,d\mathsf R_A\le C_{\rm JSL},
 \qquad p>1,
\end{equation}
uniformly in $s,N$, orientation, main clock filter, scalar mode, and
polynomial.  Consequently, for
$D_h=\{|S_{\eta,a}(x)-S_{\eta,a}(x')|\le2h\}$,
\begin{equation}
\label{eq:joint-source-energy-transfer}
 \mathsf S_A(D_h)
 \le C_{\rm JSL}^{1/p}\mathsf R_A(D_h)^{1/p'},
 \qquad p'=\frac p{p-1}.
\end{equation}
This is the likelihood at the conditional kernel where the law really
changes; the outer occurrence interface $\mathrm{KRH}_p$ alone does not imply
it, in agreement with the kernelwise organization of \cite{NiKernel}.
\end{definition}

\begin{lemma}[Factor criterion and same-carrier bridge]
\label{lem:joint-source-factor-criterion}
Suppose on a main cell
\begin{equation}
\label{eq:joint-source-likelihood-factorization}
 L_A(\eta,a,x,x')=
 L_A^{\rm occ}(\eta,a)\ell_{\eta,a}(x)\ell_{\eta,a}(x'),
 \quad
 \ell_{\eta,a}=\frac{d\nu_{\eta,a}}{d\kappa_{\eta,a}}.
\end{equation}
If $L_A^{\rm occ}$ is uniformly $L^{p_o}$,
$\ell_{\eta,a}$ is uniformly $L^{p_c}(\kappa_{\eta,a})$, and
\begin{equation}
\label{eq:joint-source-reciprocal-margin}
 \chi_{\rm joint}:=\frac1{p_o}+\frac2{p_c}<1,
\end{equation}
then $\mathrm{JSL}_{p_{\rm joint}}$ holds for every
$1<p_{\rm joint}<\chi_{\rm joint}^{-1}$.
If $\nu_{\eta,a}$ and $\kappa_{\eta,a}$ are normalized densities in one
...
$p_c=\infty$.  Then only the occurrence likelihood is paid, and if the
occurrence laws also agree one may take $p_{\rm joint}=\infty$ with no
change-of-law exponent loss.

If the probability and source sections lie on different carriers, this
lemma is unavailable unless $\mathrm{CARRIER\_MATCH}$ supplies a jointly
measurable map $H_{\eta,a}:W_{\rm prob}\to W_{\rm src}$ with Jacobian bounded
above and below and
$S_{\rm src}\circ H_{\eta,a}=S_{\rm prob}+O(h)$ on the relevant window.
Without the same-carrier or this bounded-holonomy alternative the main energy
lift is a no-go.
\end{lemma}

\begin{proof}
Apply generalized H\"older to the three factors in
\eqref{eq:joint-source-likelihood-factorization}; the strict reciprocal
margin leaves the probability slot.  The same-carrier conclusion is the
ratio of the displayed upper and lower cone bounds.  A bounded carrier match
changes the window and likelihood by fixed constants only.
\end{proof}

\begin{theorem}[Main-source energy lift
$\mathrm{MAIN\_SOURCE\_ENERGY\_LIFT}$]
\label{thm:main-source-energy-lift}
Assume the parentwise probability energy estimate of
Theorem~\ref{thm:direct-slope-small-ball} and
$\mathrm{JSL}_{p_{\rm joint}}$ on every main clock filter.  Put
...
$r\asymp e^{-N/C_{\rm buf}}$.  Every component is already a parentwise
tilted probability rate; any likelihood used to obtain those probability
rates is paid there.  The source occurrence--carrier likelihood is paid
exactly once through $p_{\rm joint}'$ below.
Suppose
\begin{equation}
\label{eq:main-source-base-mass}
 ...
 \le Ce^{a_{{\rm main},{\rm PPE}}^{B}N}.
\end{equation}
Then \eqref{eq:joint-source-energy-transfer} first gives the normalized
source-energy bounds
\begin{align}
 \frac{\mathcal E_{{\rm main},r}(r)}
      {\mathcal B_{{\rm main},r}(r)}
 &\le Cr^{\Theta_{{\rm ref},r}/p_{\rm joint}'},&
 \frac{\mathcal E_{{\rm main},{\rm PPE}}(N)}
      {\mathcal B_{{\rm main},{\rm PPE}}(N)}
 &\le Ce^{-c_{{\rm ref},{\rm PPE}}N/p_{\rm joint}'}.
 \label{eq:normalized-joint-source-energy}
\end{align}
Multiplying back the base envelope mass gives
\begin{align}
 \mathcal E_{{\rm main},r}(r)&\le
 Cr^{\Theta_{{\rm ref},r}/p_{\rm joint}'-a_{{\rm main},r}^{B}},
 &
 \mathcal E_{{\rm main},{\rm PPE}}(N)&\le
 Ce^{-(c_{{\rm ref},{\rm PPE}}/p_{\rm joint}'
       -a_{{\rm main},{\rm PPE}}^{B})N}.
...
\begin{align}
 ...
 \frac{c_{{\rm ref},{\rm PPE}}/p_{\rm joint}'
             -2a_{{\rm main},{\rm PPE}}^{B}}2.
 \label{eq:source-value-small-ball}
\end{align}
Both value gaps must be positive.  Define the complete Jensen value rates
\begin{equation}
\label{eq:complete-Jensen-value-rates}
 ...
 \quad
 c_{{\rm time},J}^{\rm val}:=c_{{\rm time},J}^{\rm exc,val}.
\end{equation}
An absent class has rate $+\infty$.  The main marked value tail is therefore
derived from reference probability energy, the joint occurrence--carrier
likelihood, finite base mass, and one global incidence Cauchy step; it is not
assumed in
$\mathrm{CLOCK\_SRC}_{W}$.
\end{theorem}

\begin{proof}
Theorem~\ref{thm:direct-slope-small-ball} on the actual reference law
$\mathsf R_A$ gives the two rates in
\eqref{eq:explicit-reference-energy-rates}.  Apply H\"older only on the joint
law $\mathsf R_A$ to obtain \eqref{eq:normalized-joint-source-energy}; this
step simultaneously transfers occurrence weights and both internal carrier
coordinates.  Multiplication by $M_*\mathcal B_A$ and Tonelli give
\eqref{eq:source-energy-bound}.  Apply
\eqref{eq:global-incidence-cauchy} once to each complete main clock cell.
Both base-mass costs are paid before the square root, giving the factor
$2a^B$ in the value gaps.  No occurrencewise Jensen or primitive main source
tail is used.
\end{proof}
...
Theorem~\ref{thm:main-source-energy-lift}.  Multiplying the normalized
kernel masses back by $\mathbb E_{\Pi_{s,N}}\mathscr W_*$ gives the complete
marked phase mass; no signed cancellation, occurrencewise square root, or
...
marked source kernel, the value mass of
...
Assume $\mathrm{JSL}_{p_{\rm joint}}$, the main-source base bounds
\eqref{eq:main-source-base-mass}, and use
the complete additive source rates
\eqref{eq:clock-source-matrix-tails} and
...
\[
 ...
 \mathrm{CEM\_DOM},\\
 \mathrm{TENERGY}_{W},\ \mathrm{DUAL\_MASS\_KERNEL},\ \mathrm{JSL}_{p_{\rm joint}},\\
 \mathrm{CLOCK\_PROB}_{W}
 \end{gathered}
\]
\[
...
\[
 ...
 \mathrm{DUAL\_MASS\_KERNEL},\ \mathrm{JSL}_{p_{\rm joint}},\
 \mathrm{CLOCK\_PROB}_{W},\\
 \mathrm{CLOCK\_SRC}_{W},\ \mathrm{KRH}_{p_W},\
 \mathrm{USC}_{\rm end}^{W}(q_g^W)
 \end{gathered}
...
cells, including the main-source base bounds
\eqref{eq:main-source-base-mass}.
...
\[
 ...
 \mathrm{SHCOL}_{\rm env}.
\]
Assume also the main-source base bounds
...
\]
Assume the main-source base bounds \eqref{eq:main-source-base-mass} with the
same constants.
...
\item $\mathrm{NST}_{\rm phys}$, $\mathrm{SS}(d_Z)$, and
      ...
      \eqref{eq:exhaustive-source-owner-partition}, and the main-source base
      bounds \eqref{eq:main-source-base-mass} uniformly on the same parent
      cells.  The joint source likelihood is verified either by the
...
\label{eq:final-CM2-exponent}
 ...
 \eta_{\rm hi}\rho_\infty-a_{\rm ultra},c_{\rm scl},
 c_{\rm time,J}^{\rm val},c_{\rm time,TV}^{\rm src},\eta_{\rm lo}R,
...
\[
\begin{gathered}
 \mathrm{MT}_{\rm DQ},\ \mathrm{NST}_{\rm phys},\ \mathrm{PPE}_{N},\
 \mathrm{CEM\_DOM},\ \mathrm{TENERGY}_{W},\\
 \mathrm{DUAL\_MASS\_KERNEL},\ \mathrm{JSL}_{p_{\rm joint}},\\
 \mathrm{CLOCK\_PROB}_{W},\ \mathrm{CLOCK\_SRC}_{W},\
...
condition; it supplies none of the dynamical/source interfaces in the
preceding display.
...
There are twenty-four falsifiable primitive interfaces.  $\mathrm{MT}_{\rm DQ}$ must
...
occurrence even on overlapping supports, separate exact mass from norm
envelope, satisfy global operator-weighted integrability, and exhaust all
owners before scalar weak assembly.  $\mathrm{JSL}_{p_{\rm joint}}$ must
compare the source law with the actual probability reference law on the
joint occurrence--carrier product; an occurrence-only likelihood is
insufficient.  Theorem~\ref{thm:main-source-energy-lift} then derives the main
marked value mass from the explicit reference energy, joint likelihood,
declared base-mass bounds, and one global incidence Cauchy step; it is not a
primitive source-clock row.  $\mathrm{CLOCK\_PROB}_{W}$
...
\begin{proposition}[Forty-nine obstruction tests]
\label{prop:v42-obstruction-tests}
...
\item Parentwise PPE may hold uniformly while the main occurrence base mass
      ...
      true but the marked value tail is false.  The base bounds
      \eqref{eq:main-source-base-mass} and positivity of the value gaps in
...
\item There may be one source occurrence, so the occurrence-level
      ...
      $\mathrm{JSL}_{p_{\rm joint}}$ in
      \eqref{eq:joint-source-likelihood} rejects this internal-law mismatch.
\item Two normalized regular densities may each satisfy the same cone bounds
      while living on different carriers with incompatible slope maps.  A
      density ratio is then not defined.  Lemma
      \ref{lem:joint-source-factor-criterion} requires the same carrier or
      the explicit bounded-holonomy $\mathrm{CARRIER\_MATCH}$ alternative.
\item If the joint likelihood is only $L^p$ and the reference energy is
      ...
      Equations \eqref{eq:normalized-joint-source-energy} and
...
\item Every fibre can satisfy $q_\lambda(Y_{\rm src})<\infty$ while
      ...
      The two global requirements in
      \eqref{eq:global-finite-marked-envelope} reject this model.
\item If cemetery, exceptional, face, and deep cells are controlled but one
      unclocked main current is omitted, all local estimates may hold while
      the final phase assembly is incomplete.  The exhaustive identity
      \eqref{eq:exhaustive-source-owner-partition} forces that occurrence
      into exactly one owner cell and treats clocks only as filters.
\end{enumerate}
\end{proposition}

\begin{proof}
Items 1--3, 8--11, and 13--49 are direct finite-measure, frequency, or norm
