...
\large Gauge-covariant endpoint unfolding, prescribed-depth escape, two-threshold removal,
and a strongly conditional
pathwise CM2 criterion}
\author{Working note}
\date{13 July 2026 (version 33)}
...
reference.  At finite difference-quotient level the coordinate commutator is
cancelled against observable transport in the complete physical bilinear
pairing; any intrinsic remainder keeps its regular, face, or
strong-to-weak type.
...
Fourier analysis only while all resolving depths lie below $L(s)$; shallow
long times are removed directly as finite-$s$ positive-envelope collars.
...
\begin{definition}[Gauge-covariant finite-difference interface
$\mathrm{MT}_{\rm DQ}$]
\label{def:measure-gauge-interface}
Let $U_s$ denote the measure-trivializing pullback in
\eqref{eq:measure-trivializing-gauge}.  Fix once and for all whether the
physical observables are material or Eulerian, and let $f_s,g_s$ be their
exact pullbacks to $(M,\mu_0)$; write $\widehat P_s$ for the transfer on
$M_s$.  Condition $\mathrm{MT}_{\rm DQ}$ requires
the following finite-$s$ statements before any absolute value or face
decomposition is taken.
\begin{enumerate}[label=\textup{(MT\arabic*)}]
\item The complete physical bilinear pairing is exactly equal to its
      fixed-gauge pairing.  In its difference quotient, the coordinate
      commutator from
      $U_s^{-1}\widehat P_sU_s$ and the Lie derivatives of $f_s,g_s$
      cancel algebraically.  Smooth endpoint terms which remain for
      Eulerian observables are displayed separately.
\item After this cancellation the intrinsic difference quotient
      $K_s^{\rm int,DQ}$ has one common decomposition
      \[
        K_s^{\rm int,DQ}=K_{s,\rm reg}^{\rm DQ}
          +K_{s,\rm face}^{\rm DQ}
          +K_{s,\rm sw}^{\rm DQ}+E_s .
      \]
      ...
      and $K_{s,\rm sw}^{\rm DQ}$ is, if nonzero, explicitly typed as a
      one-derivative strong-to-weak operator with a uniform CM2 bound.  It is
      never called an $M\times M$ density.
\item The same decomposition holds for the limiting intrinsic defect.
      Moreover $E_s\to0$ either in the norm in
      \eqref{eq:uniform-gauge-DQ-regular} or directly in the absolute CM2
      double-sum norm.  All constants are uniform on the shallow spacetime
      cells used below.
\end{enumerate}
This interface is gauge covariant: changing the auxiliary transport changes
the commutator and observable pullbacks but not their already-cancelled
physical bilinear defect.
\end{definition}

\begin{lemma}[Fixed-gauge centering and finite-DQ cancellation]
...
standard-family couplings.  Pure coordinate commutators do not constitute a
separate CM2 block.  After the exact finite-difference cancellation, every
remaining term has precisely one of the regular, face, or strong-to-weak
types in Definition~\ref{def:measure-gauge-interface}.
\end{lemma}

\begin{proof}
Equation~\eqref{eq:measure-trivializing-gauge} gives
$\mu_0(P_sh)=\mu_0(h)$, hence $P_s\Pi_0=\Pi_0P_s=\Pi_0$ and
$\partial_s\Pi_0=0$.  The finite-$s$ identity in
$\mathrm{MT}_{\rm DQ}$ is applied to the complete physical pairing before
differentiation, so the derivative of the conjugacy and the derivatives of
the pulled-back observables cancel term by term.  The asserted classification
and uniform bounds are exactly \textup{(MT2)--(MT3)}.
\end{proof}

The divergence formula for smooth Anosov transfer derivatives in
\cite{FroylandPhalempin} maps a strong anisotropic space to a weaker one and
is not an $M\times M$ Sobolev density.  This is the relevant type warning
behind $\mathrm{MT}_{\rm DQ}$.  The fixed zero-mass organization is
...
moving-billiard cancellation or its face atlas; those remain explicit
interfaces here.
...
native interval $Q$.  For an energy scale $0<r<r_0$ set
\begin{equation}
...
actual conditional, every admissible positive mark, and every frozen context
used in $\mathrm{SS}(d_Z)$.
...
\item For every $P\in\mathbb R_{d_Z}[z]$ the endpoint series at the
      ...
      conditional, not on a surviving Cantor conditional.
...
\item The depth-$N(r)$ endpoint is defined on retained open physical
      ...
      overlap, and retains all continuations.  No affine jet is absorbed into
      $P$ and no comparison with an auxiliary depth $J$ is used.
\item At $s=0$ the constants are uniform for all $r$.  Along
      $a(\theta s)$ they are uniform whenever the resolving depth is at most
      $L(s)$ on $\mathrm{UCR}_{\rm sh}$ cells.  The conversion from a
      two-time depth $N_{\rm time}$ to all geometric resolving depths costs
      at most $C_{\rm res}N_{\rm time}$.
\end{enumerate}
...
\end{equation}
and call a context good when $H_\beta\le\sigma^{-d_0}$.  Use the prescribed
depth $N=N(r)$ of \eqref{eq:prescribed-energy-depth}.  Then on good contexts the error in
...
continuations are retained, so the refinement
...
\item The branch sections, $\mathrm{PPE}_{N}$ labels, full-tangent
...
$\mathrm{PPE}_{N}$, $\mathrm{FZ}_{\rm geom}$,
...
\begin{definition}[Shallow long-time collar
...
$\mathrm{SHCOL}_{\rm env}$ requires that its exact actual/split law
expansion has one positive envelope and, for the corresponding bounded
shallow collar functional $F^{\rm sh}_{s,L}$,
\begin{equation}
...
\end{equation}
The connected partitions are separation-time admissible for every time gap,
with the same $c_{\rm FC},q_{\rm cyl}$ as in
Theorem~\ref{thm:law-adapted-cylinder-FCB}.  In the two off-diagonal cones,
the same positive source has forward/reverse signed standard-family
disintegrations with uniform delayed-coupling rate $c_{\rm off}^{\rm sh}>0$.
This is an all-time
positive-source condition; it is not inferred from a depth-$L$ common
partition.  The long-standard-pair mechanisms of
\cite{CanestrariPerturb,CanestrariHoles} are its structural model.
\end{definition}
...
\end{equation}
Assume $\mathrm{UCR}_{\rm sh}$, $\mathrm{SS}$, $\mathrm{PPE}_{N}$,
...
\label{eq:shallow-time-window}
 C_{\rm res}\kappa_T^{\rm sh}<\kappa_L,
 \qquad
...
\label{eq:shallow-long-time-sum}
 ...
 |B^{\rm sh}_{m,n}(s)|=o(1).
\end{equation}
The window is nonempty precisely when
$C_{\rm res}(1+\kappa_LA_{\rm sh})<\kappa_Lc_{\rm sh}$.
\end{theorem}

\begin{proof}
If $N\le T_{\rm sh}(s)$, every endpoint, section, and prescribed PPE
resolution has depth at most $C_{\rm res}N<L(s)$ by the first inequality in
\eqref{eq:shallow-time-window}.  Thus the common shallow refinement supports
the uniform endpoint low/high proof.  For $N>T_{\rm sh}(s)$ do not continue
...
\]
The two off-diagonal cones use the signed delayed coupling included in
$\mathrm{SHCOL}_{\rm env}$; the definition of $c_{\rm sh}$ makes
...
$\mathrm{SS}(d_Z)$, $\mathrm{PPE}_{N}(d_Z)$,
...
\item $\mathrm{SS}(d_Z)$ and
      ...
      $C_{\rm res}N\le L(s)$; longer shallow times are not estimated by this
      interface;
...
\item $\mathrm{MT}_{\rm DQ}$ holds, including the exact finite-DQ
      cancellation and uniform classification
      \eqref{eq:uniform-gauge-DQ-regular}.  The intrinsic regular defect
      satisfies, for some
      $1\le R\le3$,
      \[
...
\begin{proof}
Lemma~\ref{lem:fixed-gauge-centering} first applies the exact physical
finite-DQ identity and only then puts the intrinsic limiting current and all
finite-$s$ difference quotients under $\Pi_0$.  Pure coordinate commutators
have already cancelled the observable pullbacks; any remaining
strong-to-weak or graph-supported term keeps that type under \textup{(H5)}.
...
Conditions $\mathrm{SS}$ and $\mathrm{PPE}_{N}$, together with the
...
$\mathrm{PPE}_{N}$, $\mathrm{BR}_{\rm sec}$,
...
\begin{array}{ccl}
 ...
 \textup{(H5)}&\Longrightarrow&
   \text{gauge-covariant finite-DQ regular/face/strong--weak blocks}.
\end{array}
\]
\endgroup
...
\item The prescribed-depth face high-frequency ledger is
$K^{-\beta_{\rm cen}}$; the regular high tail is $K^{-R}$.
...
\item The finite-DQ gauge identity is used before absolute values.  Its
intrinsic regular, face, and strong-to-weak remainders are charged only to
their corresponding ledgers in $\mathrm{MT}_{\rm DQ}$.
\end{enumerate}
All exponents are required strictly positive; in particular the
$\kappa_T^{\rm sh}$ interval must be nonempty.

There are thirteen falsifiable interfaces.  $\mathrm{MT}_{\rm DQ}$ must
cancel pure coordinate commutators at the level of the physical bilinear
difference quotient and type every residual term correctly.
$\mathrm{SS}$ must contain exactly one rough source, and
...
\begin{proposition}[Eight obstruction tests]
\label{prop:v33-obstruction-tests}
...
\begin{proof}
Items 1, 2, 5, 6, and 7 are direct finite-measure or norm calculations.
Item 3 is the missing-depth countermodel, item 4 is the word/time-depth
countermodel, and item 8 is exactly the connected-refinement clause.
\end{proof}
