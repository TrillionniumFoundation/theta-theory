...
\large Three-space operator differentiation, upper-scale grazing, and three-frequency removal,
and a strongly conditional
pathwise CM2 criterion}
\author{Working note}
\date{13 July 2026 (version 35)}
...
convergence then exchanges the limit and the CM2 double sum.
...
derivative growth is charged to the encountered homogeneity indices, and
labels with excessive growth are removed by their own conditional tail.  All
...
\item Before projection the quotient has one exact typed decomposition.  If
      ...
      \end{equation}
      the graph-supported part belongs to the existing physical face atlas,
      and
      \begin{equation}
...
\item The same double-projected typed decomposition holds for the limiting
      ...
      \end{equation}
      The regular and face blocks converge in their stated ledgers.  For
...
\begin{lemma}[Fixed-gauge centering and strong-to-weak CM2]
...
tends to zero.  Consequently the limit may be exchanged with the absolute
CM2 double sum.
\end{lemma}
...
other two types are exactly \textup{(MT1)}.
\end{proof}
...
\begin{definition}[Endpoint upper-scale grazing ledger
$\mathrm{USC}_{\rm end}(q_g)$]
...
\end{equation}
The actual stopped law of $\mathrm{NST}_{\rm phys}$, every split law used by
the functional-correlation estimate, and all marked versions share constants
$\gamma_g,q_g,C>0$ such that
\begin{equation}
\label{eq:upper-scale-excess-tail}
 \mathbb P_{\rm stop}\{G_N^\pm>\gamma_gN+u\}
 \le Ce^{-q_gu},\qquad u\ge0.
\end{equation}
The same label law is used for the native stopping, PPE, and endpoint marks;
no marginal tail is transferred to a different conditional law.
\end{definition}
...
\label{eq:endpoint-upper-scale-growth}
 \log^+a_X\le C_0m+2G_m^-,\qquad
 \log^+a_Y\le C_0n+2G_n^+.
\end{equation}
Fix $\zeta_g>0$ and put
\begin{equation}
\label{eq:scale-good-constants}
 C_{\rm scl}:=C_0+2(\gamma_g+\zeta_g),\qquad
 c_{\rm scl}:=q_g\zeta_g.
\end{equation}
For $N=\max\{m,n\}$ the scale-good labels satisfy
$a_X+a_Y\le Ce^{C_{\rm scl}N}$, while the total marked mass of scale-bad
labels is at most $Ce^{-c_{\rm scl}N}$.  The latter labels may therefore be
estimated by total variation and the absolute Fourier coefficient sum,
without invoking $\mathrm{PPE}_{N}$.
\end{lemma}
...
\]
Multiplication along the branch gives
\eqref{eq:endpoint-upper-scale-growth}.  Apply
\eqref{eq:upper-scale-excess-tail} with $u=\zeta_gN$ to the two orientations
and use the union bound.  This argument explains the geometric form of the
interface; uniformity for the stopped actual/split marked laws remains the
explicit content of $\mathrm{USC}_{\rm end}$.  In particular it is not
asserted for arbitrary billiard invariant measures.
\end{proof}
...
Fourier coefficient sum, and \eqref{eq:upper-scale-excess-tail} give
$Ce^{-c_{\rm scl}N}$.
...
$\mathrm{USC}_{\rm end}(q_g)$ on the stopped actual/split laws, are required
...
First choose $\delta$ so that $\alpha-A_{\rm frac}\delta>0$.  Next choose
...
The upper endpoint scale is not inserted into that product: its excess is
split off by \eqref{eq:upper-scale-excess-tail}, on the same stopped terminal
law, before the phase estimate is applied.
\end{proof}
...
\item $\mathrm{NST}_{\rm phys}$, $\mathrm{SS}(d_Z)$, and
      ...
      endpoint upper-scale ledger $\mathrm{USC}_{\rm end}(q_g)$,
...
and time-depth splits.  The shallow long-time part converges to zero by
...
$\mathrm{USC}_{\rm end}$; no raw lattice cutoff is used to bound endpoint
...
regular and face terms use their corresponding ledgers, while the
...
$\mathrm{USC}_{\rm end}$ must charge positive endpoint-scale growth to the
homogeneity indices on that same stopped actual/split law.
...
\begin{proposition}[Sixteen obstruction tests]
\label{prop:v35-obstruction-tests}
...
\item A raw mode with $\Lambda=1$ may have $a_X=e^{\chi N}$ and hence
      ...
      $\mathrm{USC}_{\rm end}$ either puts it in the scale-good cost
...
\begin{proof}
Items 1--3, 8--11, and 13--16 are direct finite-measure, frequency, or norm
