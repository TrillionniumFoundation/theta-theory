#!/usr/bin/env python3
"""C79g v11 append-only zero-credit candidate builder.

This append-only v11 binds the independently verified C78s 13-member final
...
is permanently non-authoritative.  The separately committed v11 seal plus the
...
COLD_COORDINATION_PARENT_FD_ENV = "CM2_C79G_V11_COORDINATION_PARENT_FD"
...
SELF = OUT / "cm2_round306c79g_true_global_no_producer_consumer_v11.py"
...
STATIC_AUDIT_V11 = OUT / "cm2_round306c79g_true_global_no_producer_consumer_static_audit_v11.json"
COLD_LAUNCHER = OUT / "cm2_round306c79g_true_global_no_producer_consumer_cold_launch_v11.py"
COLD_MANIFEST = OUT / "cm2_round306c79g_true_global_no_producer_consumer_cold_launch_manifest_v11.sha256"
COLD_OUTER = OUT / "cm2_round306c79g_true_global_no_producer_consumer_cold_launch_outer_receipt_v11.json"
...
FINAL_V11_CORE_PINS_INSTALLED = True
...
COLD_EXACT8 = (
    ...
    CONTRACT,
    SELF,
    OUT / "cm2_round306c79g_true_global_no_producer_consumer_independent_verifier_assembler_authority_consumer_v11.py",
...
STATIC_AUDIT_INPUT_KEY_COUNT = 32
...
def _linux_libc() -> ctypes.CDLL:
    need(sys.platform.startswith("linux"), "Linux-only v11 publication protocol")
...
class HeldSelf:
    ...
    def __init__(self, exec_inherited: int, source_inherited: int,
        ...
        need(SELF == OUT / "cm2_round306c79g_true_global_no_producer_consumer_v11.py",
...
def new_candidate_stage(outdir: Path, parent: HeldParent) -> tuple[Path, HeldMutableStage]:
    ...
    name = ".c79g-v11-candidate-stage-" + orientation + "-" + CHECKPOINT_OBJECT_PIN
...
def strict_bool_predecessor_and_successor_census(
        ...
        v11_sources: Mapping[str, bytes]) -> dict[str, Any]:
    """Prove frozen v5 defects and zero-unsafe complete v6/v7/v11 boxes."""
    ...
    v11 = {role: _strict_bool_need_census(v11_sources[role], role)
    ...
    v11_unproved = sum(
        v11[role]["recursive_exact_bool_unproved_count"] for role in v11_roles)
    v11_arity = sum(
        v11[role]["arity_star_or_keyword_failure_count"] for role in v11_roles)
    v11_returns = sum(
        v11[role]["annotated_bool_return_failure_count"] for role in v11_roles)
    need(v6_unproved == 0 and v6_arity == 0 and v6_returns == 0 and
         v7_unproved == 0 and v7_arity == 0 and v7_returns == 0 and
         v11_unproved == 0 and v11_arity == 0 and v11_returns == 0,
         "v6/v7/v11 recursive exact-bool full boxes have zero unsafe or unproved sites")
    return {
        ...
        "same_seven_absent_from_v11": True,
        ...
        "v11_recursive_exact_bool_unproved_count": v11_unproved,
        ...
        "v11_direct_need_call_count": sum(
            v11[role]["direct_need_call_count"] for role in v11_roles),
...
def v6_held_self_identity_defect_regression(raw: bytes) -> dict[str, Any]:
    ...
    V11 first proves the complete three-site census, then independently locates
...
def v10_regression_label_prefix_incident_regression(
        producer_raw: bytes, launcher_v9_raw: bytes) -> dict[str, Any]:
    """Reproduce the rejected v10 sole-false colon-prefix gate by AST shape."""
    try:
        producer_tree = ast.parse(
            producer_raw.decode("utf-8"), filename="producer_v10", mode="exec")
        launcher_tree = ast.parse(
            launcher_v9_raw.decode("utf-8"), filename="cold_launcher_v9", mode="exec")
    except (UnicodeDecodeError, SyntaxError) as exc:
    ...
    constants = [
        ...
        if isinstance(node, ast.Constant) and isinstance(node.value, str)]
    failure_label = V10_REGRESSION_LABEL_PREFIX_INCIDENT[
        "failure_label_without_colon_prefix"]
    colon_label = ":" + failure_label
    checks = [
        len(expanded_dicts) == 1,
        len(expanded_dicts[0].keys) == 16,
        len(validator_calls) == 1,
        bool(equality_gates),
        failure_label in constants,
        sha_bytes(launcher_v9_raw) == V9_PUBLISHED_EXACT10_PINS[7][1],
    ]

    membership_gates = [
        node for node in ast.walk(regression)
        if isinstance(node, ast.Compare) and len(node.ops) == 1 and
           isinstance(node.ops[0], ast.In) and len(node.comparators) == 1 and
           isinstance(node.left, ast.Subscript) and
           isinstance(node.left.value, ast.Name) and
           node.left.value.id == "V9_V6_DEFECT_SHAPE_DRIFT_INCIDENT" and
           isinstance(node.left.slice, ast.Constant) and
           node.left.slice.value == "failure_label" and
           isinstance(node.comparators[0], ast.Name) and
           node.comparators[0].id == "constants"]
    hold_calls = [
        node for node in ast.walk(build_function)
        if isinstance(node, ast.Call) and isinstance(node.func, ast.Name) and
    ...
    mkdir_calls = [
        ...
        if isinstance(node, ast.Call) and isinstance(node.func, ast.Attribute) and
           ...
           node.func.value.id == "os" and node.func.attr == "mkdir"]
    ...
    hold_statement_index = (
        enclosing_statement_index(build_function, hold_calls[0])
        if len(hold_calls) == 1 else -1)
    stage_statement_index = (
        enclosing_statement_index(build_function, stage_calls[0])
        if len(stage_calls) == 1 else -1)

    need(sha_bytes(producer_raw) == V10_PUBLISHED_EXACT10_PINS[3][1] and
         checks == [True, True, True, True, False, True] and
         len(membership_gates) == 1 and
         constants.count(failure_label) == 0 and
         constants.count(colon_label) == 1 and
         sum(value.endswith(failure_label) for value in constants) == 1 and
         len(equality_gates) == 2 and
         len(hold_calls) == len(incident_calls) == len(stage_calls) == 1 and
         hold_statement_index < stage_statement_index and not mkdir_calls and
         V10_REGRESSION_LABEL_PREFIX_INCIDENT["colon_prefixed_failure_label_literal"] ==
             colon_label and
...
def _validate_v7_regression_section(
    ...
    need(strict_bool_census["v7_recursive_exact_bool_unproved_count"] == 0 and
         strict_bool_census["v11_recursive_exact_bool_unproved_count"] == 0 and
         strict_bool_census["all_twelve_sources_AST_parsed_and_compiled_in_memory"]
             is True and
         strict_bool_census["compiled_code_executed"] is False,
         label + ":v7/v11 full-callsite strict-bool closure")
    need(section.get("all_ten_file_pins_match") is True and
         section.get("all_declared_object_pins_match") is True and
         section.get("all_ten_regular_0444_nlink1") is True and
         section.get("exact8_manifest_reconstructs_first_eight_in_order") is True and
...
def _validate_final_static_audit(
    ...
    need(isinstance(dual, dict) and set(dual) == {
         ...
         }, "final v11 static audit dual-checker exact8 closure")
    ...
    need(isinstance(attacks, dict) and set(attacks) == {
         ...
         "final v11 coherent attack exact7, 121/121, routes, and deferred execution")
    ...
    need(isinstance(closure, dict) and set(closure) == {
         ...
         }, "final v11 schema/constructor exact21 closure")
    need(type(closure.get("schema_definition_count")) is int and
         closure.get("schema_definition_count") == 43 and
         type(closure.get("schema_ref_count")) is int and
         closure.get("schema_ref_count") == 235 and
         ...
         closure.get("closed_object_count") == 49 and
         type(closure.get(
         ...
         "final v11 schema 43 defs/235 refs/0 unresolved/49 closed/0 mismatch")
    need(closure.get("strict_JSON_duplicate_keys_rejected") is True and
         ...
         "final v11 schema and live-protocol type-exact bool closure")
    actual_keywords = [
    ...
    need(closure.get("actual_schema_keyword_universe") == actual_keywords and
         ...
         "final v11 exact canonical schema keyword universes and digests")
    shapes = closure.get("output_shape_key_counts", {})
    expected_shapes = {
        "selfIdentity": 33,
        "independentConsumerProof": 56,
        "staticFreezeProof": 73,
        "coldLaunchProof": 107,
        "laterRejection": 52,
        "producerSourceRegistry": 60,
        "liveRequest": 15,
        "liveACK": 32,
        "liveACKCensus": 39,
    }
    need(isinstance(shapes, dict) and set(shapes) == set(expected_shapes) and
         ...
         "final v11 exact nine output-shape key counts")
...
def hold_static_freeze_trust(self_guard: HeldSelf) -> dict[str, Any]:
    """Hold current v11 exact10 and every distinct append-only predecessor.

    Current exact8 begins with the official v10 rejection itself.  The v10,
    v9, v8, v7, v6, and v5 published exact10 surfaces remain distinct; v3 exact10,
    the separate v3 rejection, and the seven other v4 draft files are held.
    The exact distinct-file identity census is
    10+10+10+10+10+10+10+10+7+1 = 88.
    """
    manifest_guard = HeldPinnedInput(
        COLD_MANIFEST, "C79g v11 cold-launch manifest", 0o444)
    outer_guard = HeldPinnedInput(
        COLD_OUTER, "C79g v11 cold-launch outer-last receipt", 0o444)
    manifest_entries = parse_manifest_ordered(
        manifest_guard.raw, "C79g v11 cold-launch exact8 manifest")
    ...
    need(len(manifest_by_path) == 8 and
         ...
         manifest_by_path[V10_OFFICIAL_REJECTION] ==
             V10_OFFICIAL_REJECTION_FILE_PIN,
         "cold-launch manifest current SELF, v10 rejection, and policy pins")

    by_path: dict[Path, HeldPinnedInput] = {}
    for path in COLD_EXACT8:
        if path != SELF:
            by_path[path] = HeldPinnedInput(
                path, "C79g v11 cold exact8:" + path.name, 0o444,
                manifest_by_path[path])
    v10_rejection_guard = by_path[V10_OFFICIAL_REJECTION]
    ...
    need(len(current_cold_ten_guards) == 10 and
         ...
         "current v11 cold exact10 unique held identities on one statx mount")
    exact8_stats = [
    ...
    need(all(chronology.values()),
         "physical v11 exact8/manifest/outer freeze chronology")
    ...
    need(v10_regression_label_prefix_incident ==
             V10_REGRESSION_LABEL_PREFIX_INCIDENT,
         "v10 unprefixed membership incident independently reproduced")
    ...
    all_file_guards = [
        *current_cold_ten_guards, *v10_guards, *v9_guards, *v8_guards,
        *v7_guards, *v6_guards, *v5_guards, *v3_guards, *v4_guards,
        v3_rejection_guard,
    ]
    need(len(all_file_guards) == 88 and
         len({guard.identity for guard in all_file_guards}) == 88 and
         len({guard.mount_id for guard in all_file_guards}) == 1 and
         v10_namespace_guard.mount_id == v9_namespace_guard.mount_id ==
         ...
         "append-only v11/v10/v9/v8/v7/v6/v5/v3/v4/rejection history is 88 "
         "distinct files on one mount")
    require_v5_positive_and_stage_surfaces_absent()

    v11_consumer_path = COLD_EXACT8[4]
    strict_bool_census = strict_bool_predecessor_and_successor_census(
        ...
        {
            "producer_v11": self_guard.raw,
            "consumer_v11": by_path[v11_consumer_path].raw,
            "launcher_v11": by_path[COLD_LAUNCHER].raw,
        })

    transition_guard = by_path[V10_TO_V11_TRANSITION]
    audit_guard = by_path[STATIC_AUDIT_V11]
    transition = strict_json(transition_guard.raw, "C79g v11 transition trust")
    audit = strict_json(audit_guard.raw, "C79g v11 static audit trust")
    outer = strict_json(outer_guard.raw, "C79g v11 cold-launch outer trust")
    ...
    verify_object(transition, "C79g v11 transition trust")
    verify_object(audit, "C79g v11 static audit trust")
    verify_object(outer, "C79g v11 cold-launch outer trust")
    ...
    transition_pin = audited.get("v10_to_v11_transition_receipt", {})
    ...
    _validate_v7_regression_section(
        published_v7, strict_bool_census, "v11 transition v7 predecessor")
    _validate_v7_regression_section(
        audit_v7, strict_bool_census, "v11 audit v7 predecessor")
    _validate_v8_regression_section(
        published_v8, v8_rollout_control_flow_incident,
        "v11 transition v8 predecessor")
    _validate_v8_regression_section(
        audit_v8, v8_rollout_control_flow_incident,
        "v11 audit v8 predecessor")
    _validate_v9_regression_section(
        published_v9, "v11 transition v9 predecessor")
    _validate_v9_regression_section(
        audit_v9, "v11 audit v9 predecessor")
    _validate_v10_regression_section(
        published_v10, v10_regression_label_prefix_incident,
        "v11 transition v10 predecessor")
    _validate_v10_regression_section(
        audit_v10, v10_regression_label_prefix_incident,
        "v11 audit v10 predecessor")
    ...
    need(transition.get("status") ==
         ...
         audit_v4.get(
             "v11_closed_object_required_property_mismatch_count") == 0 and
         audit_v4.get("same_defects_absent_from_v11") is True and
         ...
         "transition/audit pin current v11 and preserve exact v3/v4/v7 history")
    ...
    need(set(outer) == {
         ...
         "cold-launch v11 outer-last exact8 and zero-credit closure")
    ...
    return {
        "guards": [
            *by_path.values(), *v10_guards, *v9_guards, *v8_guards,
            *v7_guards, *v6_guards, *v5_guards, *v3_guards,
            *v4_guards, v3_rejection_guard,
            v10_namespace_guard, v9_namespace_guard, v8_namespace_guard,
            v7_namespace_guard,
            v6_namespace_guard,
            v5_namespace_guard,
...
def ensure_launch_configuration() -> HeldSelf:
    ...
    need(FINAL_V11_CORE_PINS_INSTALLED is True,
         "v11 final core pins are not installed; runtime permanently disabled")
    need(all(value not in V11_DRAFT_CORE_PIN_SENTINELS for value in (
             CONTRACT_FILE_PIN, CONTRACT_OBJECT_PIN, CLOSED_SCHEMA_FILE_PIN)),
         "v11 draft contract/schema sentinels must all be replaced before runtime")
    ...
    need(all(value and len(value) <= 10 and
         ...
         SELF == ROOT / "deliverables/cm2_round306c79g_true_global_no_producer_consumer_v11.py",
    ...
    need(all(isinstance(value, str) and len(value) == 64 and
         ...
         "C79g v11 exact non-placeholder contract/schema/rejection pins")
    need(set(C78S_FINAL["pins"]) == set(C78S_NAMES) and
         ...
         "C79g v11 exact filled launch configuration")
...
def validate_static_authorities(static_trust: dict[str, Any]) -> dict[str, Any]:
    ...
    need(contract.get("effective_checkpoint_object_sha256") == CHECKPOINT_OBJECT_PIN and
         ...
         contract.get("v11_bundle", {}).get(
             ...
             "independent_verifier_assembler_authority_consumer_v11.py" and
         contract.get("v11_bundle", {}).get("closed_schema_validator_policy") == {
         ...
         "v11 frozen contract checkpoint/composite-only boundary")
    ...
    need(closed.get("$ref") == "#/$defs/coldLaunchedCommittedAuthority" and
         ...
         "append-only v11 cold root, zero-credit inner, and persisted zero boundary")
    ...
    return close_object({
        ...
        **self_guard.execution_proof(),
        ...
        "post_source_static_freeze_trust": {
            "v10_to_v11_transition_receipt": {
                "path": str(V10_TO_V11_TRANSITION.relative_to(ROOT)),
                "file_sha256": static_trust["transition_file_sha256"],
                "object_sha256": static_trust["transition_object_sha256"],
            },
            "static_audit_v11": {
                "path": str(STATIC_AUDIT_V11.relative_to(ROOT)),
...
def build(outdir: Path, self_guard: HeldSelf,
    ...
    lock = (
        "C79g v11 isolated candidate only; standalone non-authoritative. Formal-global credit "
