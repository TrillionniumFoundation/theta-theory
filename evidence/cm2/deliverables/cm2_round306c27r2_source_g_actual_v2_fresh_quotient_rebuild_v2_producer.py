...
def parse_manifest(capture: Capture) -> dict[str, str]:
    ...
    for ordinal, line in enumerate(payload.decode("ascii", "strict").splitlines()):
        pieces = line.split("  ", 1)
        need(len(pieces) == 2 and is_sha256(pieces[0]) and pieces[1],
...
def quotient(
    ...
    for group in dsu.groups():
        ...
        producer_initial = Capture(SELF, "producer-source-initial")
        try:
            producer_source_sha256 = producer_initial.sha256
            producer_initial.attestation()
        finally:
            producer_initial.close()
        body = {
...
def build(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        ...
        # Avoid leaking the short-lived producer-source descriptor while still
        # ensuring the source is stable and single-link.
        producer_capture = Capture(SELF, "producer-source-final")
        try:
            need(producer_capture.sha256 == body["producer_source_sha256"],
                 "producer source pre/post SHA")
            body["producer_source_attestation"] = producer_capture.attestation()
        finally:
