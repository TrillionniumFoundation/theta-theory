#!/usr/bin/env python3
"""C79g v15 append-only zero-credit candidate builder.

This append-only v15 binds the independently verified C78s 13-member final
...
is permanently non-authoritative.  The separately committed v15 seal plus the
...
COLD_COORDINATION_PARENT_FD_ENV = "CM2_C79G_V15_COORDINATION_PARENT_FD"
...
SELF = OUT / "cm2_round306c79g_true_global_no_producer_consumer_v15.py"
...
STATIC_AUDIT_V15 = OUT / "cm2_round306c79g_true_global_no_producer_consumer_static_audit_v15.json"
COLD_LAUNCHER = OUT / "cm2_round306c79g_true_global_no_producer_consumer_cold_launch_v15.py"
COLD_MANIFEST = OUT / "cm2_round306c79g_true_global_no_producer_consumer_cold_launch_manifest_v15.sha256"
COLD_OUTER = OUT / "cm2_round306c79g_true_global_no_producer_consumer_cold_launch_outer_receipt_v15.json"
...
FINAL_V15_CORE_PINS_INSTALLED = True
...
COLD_EXACT8 = (
    ...
    CONTRACT,
    SELF,
    OUT / "cm2_round306c79g_true_global_no_producer_consumer_independent_verifier_assembler_authority_consumer_v15.py",
    V14_TO_V15_TRANSITION,
    STATIC_AUDIT_V15,
    COLD_LAUNCHER,
)

# Frozen predecessor authority for the v15 successor draft.  This is a
...
def _linux_libc() -> ctypes.CDLL:
    need(sys.platform.startswith("linux"), "Linux-only v15 publication protocol")
...
class HeldSelf:
    """Hold the v15 exec/source pair plus exact12 frozen v14 authority.
    ...
    def __init__(self, exec_inherited: int, source_inherited: int,
        ...
        need(len(expected_rows) == V15_CHILD_AUTHORITY_DESCRIPTOR_COUNT == 12 and
        ...
        try:
            ...
            need(SELF == OUT /
                 "cm2_round306c79g_true_global_no_producer_consumer_v15.py",
...
def new_candidate_stage(outdir: Path, parent: HeldParent) -> tuple[Path, HeldMutableStage]:
    ...
    name = ".c79g-v15-candidate-stage-" + orientation + "-" + CHECKPOINT_OBJECT_PIN
...
def strict_bool_predecessor_and_successor_census(
        ...
        v15_sources: Mapping[str, bytes]) -> dict[str, Any]:
    """Prove frozen v5 defects and zero-unsafe complete v6/v7/v15 boxes."""
    ...
    v15 = {role: _strict_bool_need_census(v15_sources[role], role)
          for role in v15_roles}
    ...
    v15_unproved = sum(
        v15[role]["recursive_exact_bool_unproved_count"] for role in v15_roles)
    v15_arity = sum(
        v15[role]["arity_star_or_keyword_failure_count"] for role in v15_roles)
    v15_returns = sum(
        v15[role]["annotated_bool_return_failure_count"] for role in v15_roles)
    need(v6_unproved == 0 and v6_arity == 0 and v6_returns == 0 and
         v7_unproved == 0 and v7_arity == 0 and v7_returns == 0 and
         v15_unproved == 0 and v15_arity == 0 and v15_returns == 0,
         "v6/v7/v15 recursive exact-bool full boxes have zero unsafe or unproved sites")
    return {
        ...
        "same_seven_absent_from_v15": True,
        ...
        "v15_recursive_exact_bool_unproved_count": v15_unproved,
        ...
        "v15_direct_need_call_count": sum(
            v15[role]["direct_need_call_count"] for role in v15_roles),
...
def v6_held_self_identity_defect_regression(raw: bytes) -> dict[str, Any]:
    ...
    V15 first proves the complete three-site census, then independently locates
...
def _expected_rejected_prepublication_v13_receipt(
    ...
    """Project the frozen receipt into the exact seven-key v15 JSON surface."""
    return {
...
def _validate_v7_regression_section(
    ...
    need(strict_bool_census["v7_recursive_exact_bool_unproved_count"] == 0 and
         strict_bool_census["v15_recursive_exact_bool_unproved_count"] == 0 and
         strict_bool_census["all_twelve_sources_AST_parsed_and_compiled_in_memory"]
             is True and
         strict_bool_census["compiled_code_executed"] is False,
         label + ":v7/v15 full-callsite strict-bool closure")
    need(section.get("all_ten_file_pins_match") is True and
         section.get("all_declared_object_pins_match") is True and
         section.get("all_ten_regular_0444_nlink1") is True and
         section.get("exact8_manifest_reconstructs_first_eight_in_order") is True and
...
def _validate_final_static_audit(
    ...
    _validate_v12_regression_section(
        ...
        "final v15 static audit v12 predecessor")
    ...
    need(isinstance(supersession_receipt, dict) and
         ...
         "final v15 static audit rejected-prepublication v13 receipt")
    need(audit.get("v12_v5_rejection_shape_incident") ==
         ...
         "final v15 static audit v12 incident and exact9 historical witness")
    ...
    need(isinstance(dual, dict) and set(dual) == {
         ...
         }, "final v15 static audit dual-checker exact8 closure")
    ...
    need(helper_consensus.get("algorithm") ==
         ...
         "final v15 three-source helper AST, exact40 witness, and callsites")
    ...
    need(isinstance(attacks, dict) and set(attacks) == {
         ...
         "final v15 coherent attack exact7, 137/137 including ordered v11 "
    ...
    need(isinstance(sealed, dict) and set(sealed) == {
         ...
         "final v15 sealed-exec and scoped no-producer exact16 closure")
    ...
    need(isinstance(closure, dict) and set(closure) == {
         ...
         }, "final v15 schema/constructor exact21 closure")
    need(type(closure.get("schema_definition_count")) is int and
         ...
         "final v15 schema 46 defs/242 refs/0 unresolved/52 closed/0 mismatch")
    need(closure.get("strict_JSON_duplicate_keys_rejected") is True and
         ...
         "final v15 schema and live-protocol type-exact bool closure")
    actual_keywords = [
    ...
    need(closure.get("actual_schema_keyword_universe") == actual_keywords and
         ...
         "final v15 exact canonical schema keyword universes and digests")
    ...
    need(isinstance(shapes, dict) and set(shapes) == set(expected_shapes) and
         ...
         "final v15 exact nine output-shape key counts")
...
def hold_static_freeze_trust(self_guard: HeldSelf) -> dict[str, Any]:
    """Hold current v15 exact10 and every distinct append-only predecessor.
    ...
    manifest_guard = HeldPinnedInput(
        COLD_MANIFEST, "C79g v15 cold-launch manifest", 0o444)
    outer_guard = HeldPinnedInput(
        COLD_OUTER, "C79g v15 cold-launch outer-last receipt", 0o444)
    manifest_entries = parse_manifest_ordered(
        manifest_guard.raw, "C79g v15 cold-launch exact8 manifest")
    ...
    for path in COLD_EXACT8:
        if path != SELF:
            by_path[path] = HeldPinnedInput(
                path, "C79g v15 cold exact8:" + path.name, 0o444,
    ...
    need(len(current_cold_ten_guards) == 10 and
         ...
         "current v15 cold exact10 unique held identities on one statx mount")
    exact8_stats = [
    ...
    need(all(chronology.values()),
         "physical v15 exact8/manifest/outer freeze chronology")
    ...
    need(len(prior_history_guards) == 98 and
         ...
         len({guard.identity for guard in prepublication_guards}) ==
             V15_PREPUBLICATION_UNIQUE_LIVE_IDENTITY_COUNT and
         len(all_file_guards) == V15_TERMINAL_UNIQUE_LIVE_IDENTITY_COUNT and
         len({guard.identity for guard in all_file_guards}) ==
             V15_TERMINAL_UNIQUE_LIVE_IDENTITY_COUNT and
         group_vector == list(V15_TERMINAL_GROUP_VECTOR) and
    ...
    v15_consumer_path = COLD_EXACT8[4]
    strict_bool_census = strict_bool_predecessor_and_successor_census(
        ...
        {
            "producer_v15": self_guard.raw,
            "consumer_v15": by_path[v15_consumer_path].raw,
            "launcher_v15": by_path[COLD_LAUNCHER].raw,
        })

    transition_guard = by_path[V14_TO_V15_TRANSITION]
    audit_guard = by_path[STATIC_AUDIT_V15]
    transition = strict_json(transition_guard.raw, "C79g v15 transition trust")
    audit = strict_json(audit_guard.raw, "C79g v15 static audit trust")
    outer = strict_json(outer_guard.raw, "C79g v15 cold-launch outer trust")
    ...
    verify_object(transition, "C79g v15 transition trust")
    verify_object(audit, "C79g v15 static audit trust")
    verify_object(outer, "C79g v15 cold-launch outer trust")
    need(set(transition) == {
    ...
    contract_cross = strict_json(
        by_path[CONTRACT].raw, "C79g v15 contract cross-check")
    audit_schema = audit.get("schema_and_constructor_closure", {})
    transition_pin = audited.get("v14_to_v15_transition_receipt", {})
    ...
    _validate_v7_regression_section(
        published_v7, strict_bool_census, "v15 transition v7 predecessor")
    _validate_v7_regression_section(
        audit_v7, strict_bool_census, "v15 audit v7 predecessor")
    _validate_v8_regression_section(
        published_v8, v8_rollout_control_flow_incident,
        "v15 transition v8 predecessor")
    _validate_v8_regression_section(
        audit_v8, v8_rollout_control_flow_incident,
        "v15 audit v8 predecessor")
    _validate_v9_regression_section(
        published_v9, "v15 transition v9 predecessor")
    _validate_v9_regression_section(
        audit_v9, "v15 audit v9 predecessor")
    _validate_v10_regression_section(
        published_v10, v10_regression_label_prefix_incident,
        "v15 transition v10 predecessor")
    _validate_v10_regression_section(
        audit_v10, v10_regression_label_prefix_incident,
        "v15 audit v10 predecessor")
    _validate_v11_regression_section(
        published_v11, v10_colon_prefix_witness,
        "v15 transition v11 predecessor")
    _validate_v11_regression_section(
        audit_v11, v10_colon_prefix_witness,
        "v15 audit v11 predecessor")
    _validate_v12_regression_section(
        published_v12, "v15 transition v12 predecessor")
    _validate_v12_regression_section(
        audit_v12, "v15 audit v12 predecessor")
    ...
    need(transition.get("status") ==
         ...
         audit_v4.get(
             "v15_closed_object_required_property_mismatch_count") == 0 and
         audit_v4.get("same_defects_absent_from_v15") is True and
         ...
         "transition/audit pin current v15 and preserve exact v3/v4/v7 history")
    ...
    need(set(outer) == {
         ...
         "cold-launch v15 outer-last exact8 and zero-credit closure")
    ...
    return {
        ...
        "v15_terminal_unique_live_identity_count":
            V15_TERMINAL_UNIQUE_LIVE_IDENTITY_COUNT,
        "v15_terminal_group_vector": list(V15_TERMINAL_GROUP_VECTOR),
...
def ensure_launch_configuration() -> HeldSelf:
    ...
    need(FINAL_V15_CORE_PINS_INSTALLED is True,
         "v15 final core pins are not installed; runtime permanently disabled")
    need(all(value not in V15_DRAFT_CORE_PIN_SENTINELS for value in (
             CONTRACT_FILE_PIN, CONTRACT_OBJECT_PIN, CLOSED_SCHEMA_FILE_PIN)),
         "v15 draft contract/schema sentinels must all be replaced before runtime")
    ...
    need(all(value and len(value) <= 10 and
         ...
         SELF == ROOT / "deliverables/cm2_round306c79g_true_global_no_producer_consumer_v15.py",
    ...
    need(all(isinstance(value, str) and len(value) == 64 and
         ...
         "C79g v15 exact non-placeholder contract/schema/rejection pins")
    need(set(C78S_FINAL["pins"]) == set(C78S_NAMES) and
         ...
         "C79g v15 exact filled launch configuration")
...
def validate_static_authorities(static_trust: dict[str, Any]) -> dict[str, Any]:
    ...
    need(set(contract) == {
         ...
         contract.get("v15_bundle", {}).get(
             "independent_verifier_assembler_authority_consumer", {}).get("path") ==
             "deliverables/cm2_round306c79g_true_global_no_producer_consumer_"
             "independent_verifier_assembler_authority_consumer_v15.py" and
         contract.get("v15_bundle", {}).get("closed_schema_validator_policy") == {
         ...
         "v15 frozen contract checkpoint/composite-only boundary")
    ...
    need(closed.get("$ref") == "#/$defs/coldLaunchedCommittedAuthority" and
         ...
         "append-only v15 cold root, zero-credit inner, and persisted zero boundary")
...
def input_registry(state: dict[str, Any], self_guard: HeldSelf,
    ...
    return close_object({
        ...
        **self_guard.execution_proof(),
        ...
        "v15_predecessor_unique_live_identity_count":
            static_trust["v15_predecessor_unique_live_identity_count"],
        "v15_prepublication_unique_live_identity_count":
            static_trust["v15_prepublication_unique_live_identity_count"],
        "v15_terminal_unique_live_identity_count":
            static_trust["v15_terminal_unique_live_identity_count"],
        "v15_terminal_group_vector":
            static_trust["v15_terminal_group_vector"],
        "post_source_static_freeze_trust": {
            "v14_to_v15_transition_receipt": {
                "path": str(V14_TO_V15_TRANSITION.relative_to(ROOT)),
                "file_sha256": static_trust["transition_file_sha256"],
                "object_sha256": static_trust["transition_object_sha256"],
            },
            "static_audit_v15": {
                "path": str(STATIC_AUDIT_V15.relative_to(ROOT)),
        ...
        "current_v15_producer_source_content_opened_or_read": False,
        "current_v15_producer_source_content_imported_compiled_or_executed": False,
...
def build(outdir: Path, self_guard: HeldSelf,
    ...
    lock = (
        "C79g v15 isolated candidate only; standalone non-authoritative. Formal-global credit "
...
def v14_inherited_authority_draft_state(
    ...
    need(V15_DRAFT_RUNTIME_DISABLED is True and
         FINAL_V15_CORE_PINS_INSTALLED is False and
...
def main(argv: list[str] | None = None) -> int:
    ...
    if V15_DRAFT_RUNTIME_DISABLED and not FINAL_V15_CORE_PINS_INSTALLED:
        raise Reject(
