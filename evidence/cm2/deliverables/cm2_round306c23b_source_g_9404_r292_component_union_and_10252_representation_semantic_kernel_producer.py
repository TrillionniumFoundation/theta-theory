...
ROOT=Path(__file__).resolve().parent;P="cm2_round306c23b_source_g_9404_r292_component_union_and_10252_representation_semantic_kernel";L=P+"_ledger.jsonl.gz";R=P+"_result.json"
...
def validate_seals():
 ...
 face=json.loads((ROOT/FRESULT).read_bytes());face_sha=check_result(face,"face result");v=json.loads((ROOT/FVERIFY).read_bytes());need(face["status"]=="PASS_FORMAL_R292_SIGNED_FACE_COMPONENT_EDGE_PACKAGE_SEALED"and face["promotion_contract"]["accepted_self_endpoint_no_edge_count"]==2092 and v["status"].startswith("PASS_INDEPENDENT_CACHELESS_ROUND299C")and manifest_has(FMAN,FACES,PINS[FACES])and manifest_has(FMAN,SELF,PINS[SELF])and manifest_has(FMAN,FVERIFY,PINS[FVERIFY]),"face seal");return{"C23a_result_object_sha256":c23a_sha,"Round299C_result_object_sha256":face_sha}
def load_cells():
 cells={};owners=defaultdict(list)
 for row in gz_rows(ROOT/C23AL):
  need(row["formal_credit"]=={"R292_cell_support_set_equality":1,"member_normalized_support":0,"typed_representation_semantic_disposition":0},"C23a credit");cell=row["R292_refinement_cell_id"];cells[cell]=row;owners[row["owner_member_id"]].append(cell)
...
def load_members():
 ...
 for row in gz_rows(ROOT/C16M):
  ...
  need(row["formal_credit"]["identity"]==row["formal_credit"]["family"]==1,"member identity");out[row["member_id"]]={"fresh_component_id":row["fresh_component_id"],"base_root_id":row["base_root_id"],"official_key_id":row["official_key_id"],"row_sha256":row["row_sha256"]}
 need(len(out)==9404,"member census");return out
def load_components():
 out=[]
 with gzip.open(ROOT/R292,"rt")as stream:
  for row in iter_array(stream,'"rows"'):
   ...
   need(row["one_connected_positive_open_support"]is True and row["strictly_disjoint_from_complete_conditional_base_atom_registry"]is True and row["member_refinement_cell_count"]==len(row["member_refinement_cell_ids"]),"component construction");out.append(row)
 need(len(out)==9404,"component census");return out
def load_self_rows():
 out={};accepted=0
 with gzip.open(ROOT/SELF,"rt")as stream:
  for row in iter_array(stream,'"rows"'):
   ...
   accepted+=1;need(row["decision"]=="ACCEPT_STRICT_POSITIVE_AREA_FACE_PATCH_AND_TWO_SIDED_CORRIDOR"and row["left_formal_occurrence_id"]==row["right_formal_occurrence_id"]and row["formal_component_edge_credit"]==0,"self authority");out[row["source_Round299_classification_row_id"]]={"self_no_edge_row_id":row["Round299C_signed_face_no_edge_row_id"],"self_no_edge_row_sha256":row["row_sha256"],"source_classification_row_sha256":row["source_Round299_classification_row_sha256"],"formal_occurrence_id":row["left_formal_occurrence_id"],"accepted_patch":row["accepted_patch"],"accepted_patch_area":row["accepted_patch_area"],"left_corridor_trace_sha256":row["left_corridor"]["dyadic_inward_shrink_trace_sha256"],"right_corridor_trace_sha256":row["right_corridor"]["dyadic_inward_shrink_trace_sha256"]}
 need(accepted==len(out)==2092,"self census");return out
def load_internal_faces(cells,owner_cells):
 self_rows=load_self_rows();faces=defaultdict(list);classes=Counter();selected=0
 with gzip.open(ROOT/FACES,"rt")as stream:
  for row in iter_array(stream,'"rows"'):
   ...
   owner=cells[left]["owner_member_id"];face=validate_exact_face(row,"internal face");need(left!=right and row["same_formal_occurrence_endpoint"]is True and row["left_formal_occurrence_id"]==row["right_formal_occurrence_id"]==owner,"internal face");sealed=self_rows[row["Round299_independent_signed_face_classification_row_id"]];need(sealed["source_classification_row_sha256"]==row["row_sha256"]and sealed["formal_occurrence_id"]==owner,"self join");authority={"classification_row_id":row["Round299_independent_signed_face_classification_row_id"],"classification_row_sha256":row["row_sha256"],"self_no_edge_row_id":sealed["self_no_edge_row_id"],"self_no_edge_row_sha256":sealed["self_no_edge_row_sha256"],"left_cell_id":left,"right_cell_id":right,"contact_class":row["contact_class"],"common_face_axis":row["common_face_axis"],"exact_transformed_coordinate_face":face,"accepted_patch":sealed["accepted_patch"],"accepted_patch_area":sealed["accepted_patch_area"],"left_corridor_trace_sha256":sealed["left_corridor_trace_sha256"],"right_corridor_trace_sha256":sealed["right_corridor_trace_sha256"],"two_sided_positive_volume_corridors":True};faces[owner].append(authority);classes[row["contact_class"]]+=1;selected+=1
