#!/usr/bin/env python3
"""C73: exact-recenter the 19 falsely labelled source-grazing C72 children.
...
the 19 C72 rows it independently evaluates the collision-one equations at
...
chart is N or S rather than the required W.  Consequently all 19 rows are
...
SCHEMA = "cm2.round306c73.recentered-collision1-h1-strict-exclusion-oracle.v1"
...
def load_c72(path: Path) -> tuple[list[dict[str, Any]], str]:
    ...
    for row in iter_secure_gzip(
        ...
        if row.get("structural_category") != "SOURCE_GRAZING_ENDPOINT_CHART":
            continue
        need(row.get("required_decider") == "EXACT_SOURCE_GRAZING_ENDPOINT_ORACLE"
             and row.get("formal_credit") == row.get("whole_parent_credit")
             == row.get("D02_gate_credit") == 0, "C72 source-grazing boundary")
        selected.append(row)
        selection.update((row["row_sha256"] + "\n").encode("ascii"))
    need(len(selected) == 19, "exact C72 source-grazing child count")
...
def validate_lineage(c37_path: Path, c32_path: Path) -> None:
    ...
    need(set(pairs) == set(PAIR_LINEAGE), "three C37 pair rows")
    ...
    need(set(cells) == wanted, "three C32 cells")
    for pair_index, expected in PAIR_LINEAGE.items():
...
def build(args: argparse.Namespace) -> dict[str, Any]:
    ...
    need(pair_census == {"31": 9, "200": 4, "711": 6}
         and chart_census == {"N": 15, "S": 4}, "C73 decision census")
    ...
    result_body: dict[str, Any] = {
        ...
        "status": "PASS_19_OF_19_LEGACY_SOURCE_GRAZING_LABELS_ARE_STRICT_POSITIVE_DELTA1__9_N_PLUS_4_S_PLUS_6_N_OUTGOING_MISMATCH__STRICT_EXCLUSION__ZERO_GLOBAL_CREDIT",
        ...
        "input_domain": {"C72_SOURCE_GRAZING_ENDPOINT_CHART": 19},
    ...
    report_raw = (
        ...
        "All 19 C72 `SOURCE_GRAZING_ENDPOINT_CHART` children are false positives "
        ...
        "chart is N for 15 children and S for 4, never the frozen required W.\n\n"
        "Disposition: 19/19 `STRICT_EXCLUSION_COLLISION1_OUTGOING_CHART_MISMATCH`. "
