#!/usr/bin/env python3
"""Persistent fail-closed finalizer v2 for two real actual-v2 seed runs."""
...
VERIFIER = ROOT / "deliverables/cm2_c27_primitive_twenty_family_gate_v5_actual_v2_dual_seed_independent_verifier_v2.py"
ATTACKS = ROOT / "deliverables/cm2_c27_primitive_twenty_family_gate_v5_actual_v2_dual_seed_coherent_attack_harness_v2.py"
...
def runner_fingerprint(info: os.stat_result) -> list[int]:
    """Exact output-validation identity emitted by the pinned actual-v2 runner."""
...
def output_validation_regression(out_dir: Path, run_dir: Path) -> dict[str, Any]:
    """Catch verifier/runner stat-schema drift before creating any seal path."""
    validation_path = run_dir / "output_validation.json"
    validation = strict_json(validation_path)
    need(validation["status"] == "PASS_7_OF_7_GZIP_CANONICAL_COUNT_CLOSURE"
         and validation["output_file_count"] == 7,
         "regression native output-validation PASS")
    expected_names = {*(name + ".jsonl.gz" for name in LEDGERS),
                      "assembler_receipt.json", "manifest.sha256"}
    entries = sorted(out_dir.iterdir(), key=lambda item: item.name)
    need({item.name for item in entries} == expected_names,
         "regression exact current 7/7 outputs")
    for item in entries:
        recorded = validation["output_files"][item.name]
        need(type(recorded["stat"]) is list and len(recorded["stat"]) == 9,
             "regression runner nine-field stat schema:" + item.name)
        need(runner_fingerprint(item.stat()) == recorded["stat"],
             "regression current runner stat identity:" + item.name)
        need(fsha(item) == recorded["sha256"],
             "regression current output SHA256:" + item.name)
    return {
        "status": "PASS_RUNNER_NINE_FIELD_STAT_SCHEMA_AND_CURRENT_7_OF_7_SHA256",
...
def preflight(args: argparse.Namespace) -> dict[str, Any]:
    ...
    need(seed2_out.is_dir() and seed2_run.is_dir(), "seed2 paths exist")
    ...
    regressions = {
        "seed1": output_validation_regression(seed1_out, seed1_run),
        "seed2": output_validation_regression(seed2_out, seed2_run),
    }
    return {
        "schema": "cm2.c27-actual-v2-dual-seed-finalizer-preflight.v2",
        ...
        "runner_stat_schema_regression": regressions,
        "formal_credit": 0, "manifest_authorized": False,
        "C27R2_C28_C29": "UNAUTHORIZED_PENDING_ACTUAL_V2_TERMINAL_SEAL",
        "CM2": "NO-GO_FOR_CLAIM",
    }
...
def execute(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        ...
        failure = {
            "schema": "cm2.c27-actual-v2-dual-seed-finalizer-failure.v2",
