...
CERT_SCHEMA = "cm2.round152.open-strip-bridge-atlas.v1"
SCHEMA = "cm2.round152.open-strip-bridge-atlas-verification.v1"
STATUS = (
...
VERIFIER_PINS = {
    **CERT_UPSTREAM_PINS,
    "cm2_round152_open_strip_bridge_atlas.py":
        "4e57731a6c2649991f04b24482fb196c7c051e4f97022e8dddc0d3747218fb7d",
}
SPEC_ROWS = [
    ("1", "2", "-403", "1"),
    ("2", "4", "-394", "4"),
...
def independent_specs():
    ...
    } for abs_lower, abs_upper, beta_lower, beta_upper in SPEC_ROWS]


def reconstruct_spec(index_spec):
...
def reconstruct():
    ...
    require(len(official_hashes) == len(compact_hashes) == 1, "path hashes")
    for left, right in zip(rows, rows[1:]):
        ...
        require(lower < upper, "bridge beta overlap")
    ...
    require(len(c24_rows) == len(d3_rows) == len(rows), "boundary count")
...
def main():
    ...
    labels = (
        "bridge_return_to_survive",
        "delete_bridge",
        "erase_bridge_adjacency",
        "disable_anchor_exclusion",
        "flip_terminal_derivative",
        "invent_interior_event",
        "close_D02",
        "authorize_D03",
        "promote_Gate5",
        "claim_CM2",
    ...
    for label in labels:
        ...
        if label == "bridge_return_to_survive":
            result["typed_open_strip_atlas"]["strict_return_bridge_rows"][0][
                "terminal_classification"
            ] = "SURVIVE_THROUGH_3_INNER"
        elif label == "delete_bridge":
            result["typed_open_strip_atlas"][
                "strict_return_bridge_rows"
            ].pop()
        elif label == "erase_bridge_adjacency":
            result["typed_open_strip_atlas"][
                "bridge_exact_x_face_adjacency_rows"
            ].pop()
        elif label == "disable_anchor_exclusion":
            result["interior_event_exhaustion"][
                "collision3_anchor_D3_strict_negative_on_every_bridge"
            ] = False
        elif label == "flip_terminal_derivative":
            result["interior_event_exhaustion"][
                "terminal_event_beta_derivative_strict_positive_on_every_bridge"
            ] = False
        elif label == "invent_interior_event":
            result["interior_event_exhaustion"][
                "interior_competing_event_surface_count"
            ] = 1
    ...
    result = {
        ...
        "producer_source_sha256":
            VERIFIER_PINS["cm2_round152_open_strip_bridge_atlas.py"],
        "producer_imported_or_executed": False,
        "shared_pinned_mathematical_engine_used": True,
        "full_R1648_bridge_reconstruction_count": 21,
        "collision_stage_bridge_reconstruction_count": 21 * 1648,
        "full_radius4_candidate_tests_replayed": 21 * 161 * 1648,
        "typed_atlas_box_count_recomputed": 63,
        "bridge_exact_x_face_adjacency_count_recomputed": 20,
        "boundary_attachment_count_recomputed": 42,
        "interior_event_exhaustion_recomputed": True,
