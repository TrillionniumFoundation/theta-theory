...
ROOT=Path(__file__).resolve().parent;P="cm2_round306c22b_source_g_295336_r2_member_union_and_302624_representation_semantic_kernel";R=P+"_result.json";V=ROOT/(P+"_independent_verifier.py");PY=ROOT.parent/".venv-cm2/bin/python"
...
def main():
 candidate=Path(sys.argv[1]).resolve();base=json.loads((candidate/R).read_bytes());attacks=[("member_credit",lambda d:d.__setitem__("R2_member_normalized_support_set_equality_credit",295335)),("member_debt",lambda d:d.__setitem__("R2_remaining_member_support_debt",1)),("primary_equality",lambda d:d.__setitem__("R2_primary_representation_set_equality_credit",295335)),("alias_disposition",lambda d:d.__setitem__("R2_alias_exact_subcover_inclusion_disposition_credit",7287)),("global_debt",lambda d:d.__setitem__("remaining_global_member_support_debt",24795)),("B1A",lambda d:d["strict_nonpromotion"].__setitem__("B1A",1)),("B2",lambda d:d["strict_nonpromotion"].__setitem__("B2",1)),("CM2",lambda d:d["strict_nonpromotion"].__setitem__("CM2","GO"))];rejected=[]
 for name,mutate in attacks:
  with tempfile.TemporaryDirectory(prefix="c22b-attack-")as raw:
   target=Path(raw)
