#!/usr/bin/env python3
"""Independent verifier for the Round301 legal component DSU application.

The producer is never imported, executed, parsed, or tokenized.  It is
treated only as inert hash-pinned bytes.  Expected candidate artifacts are
independently reconstructed from sealed upstream data before candidate output
is opened.
"""

from __future__ import annotations

import argparse
from collections import Counter, defaultdict
import copy
from dataclasses import dataclass
import gzip
import hashlib
from io import BytesIO
import json
import os
from pathlib import Path
import re
import tempfile
from typing import Any, Callable, Iterable, Iterator, TextIO
import zlib


HERE = Path(__file__).resolve().parent
D = HERE
PREFIX = "cm2_round301_source_g_legal_component_dsu_application"
PRODUCER_PATH = HERE / f"{PREFIX}.py"
EDGE_PATH = HERE / f"{PREFIX}_edge_application_ledger.json.gz"
MEMBER_PATH = HERE / f"{PREFIX}_member_component_ledger.json.gz"
KEY_PATH = HERE / f"{PREFIX}_component_key_incidence_ledger.json.gz"
INELIGIBLE_PATH = (
    HERE / f"{PREFIX}_ineligible_source_consumption_ledger.json.gz"
)
RESULT_PATH = HERE / f"{PREFIX}_result.json"
ATTACK_PATH = HERE / f"{PREFIX}_attack_suite.json"
VERIFICATION_PATH = HERE / f"{PREFIX}_verification.json"
PRODUCER_SHA256 = (
    "4a0513885f21c5544be84964bb0d968ca1b04435aff4bbed36755a1684f3bcd5"
)
SCHEMA = "cm2.round301.source-g-legal-component-dsu-application.v1"
LEDGER_SCHEMA_PREFIX = SCHEMA.removesuffix(".v1")
ATTACK_SCHEMA = SCHEMA + ".attack-suite.v1"
VERIFICATION_SCHEMA = SCHEMA + ".verification.v1"

FILES = {
    "R266": (
        "cm2_round266_source_g_expanded_curved_face_closure_certificate.json",
        "2d30be104dc522ebb1664129894acf851180b9e5f51dd8471c33137447b599bf",
    ),
    "R294": (
        "cm2_round294_source_g_occurrence_registry_atomic_promotion_"
        "registry_ledger.json.gz",
        "c6b26f13e90072db99fa98f99fc62c77135ff1cbdb23bbbd5bac3e9f64a834bb",
    ),
    "R299A": (
        "cm2_round299a_source_g_refined_occurrence_official_key_binding_"
        "closure_ledger.json.gz",
        "ffea8120af2179990d5c9e7ff385193e2c5a08bed161cf5b570aa28b1f8b1ee0",
    ),
    "R297": (
        "cm2_round297_source_g_ordinary_face_occurrence_edge_promotion_"
        "edge_ledger.json.gz",
        "18a20b4679a8a3e94ccaf1b511694220cad1bc92e1590ded4585ae3735547371",
    ),
    "R296": (
        "cm2_round296_source_g_true_seam_occurrence_edge_ledger_closure_"
        "edge_ledger.json.gz",
        "1b57b10fac9317e1609fb8858972011165bd95e5b1b687604edd6c8ad7139ef7",
    ),
    "R299C": (
        "cm2_round299c_source_g_r292_signed_support_face_edge_promotion_"
        "canonical_occurrence_edge_pairs.json.gz",
        "e63f164bf9cc559ec8d3a2895e66493933b43b90f1ad3b163dfb41e12bb04df1",
    ),
    "R300B": (
        "cm2_round300b_source_g_registry_boundary_and_complete_r275_face_"
        "inventory_closure_canonical_novel_occurrence_edge_pairs.json.gz",
        "c3d1e604ed0e1256789c239b65d22546035fd3bd333dd11bfb907e221c3cec9e",
    ),
    "R300C": (
        "cm2_round300c_source_g_virtual_stratum_new_occurrence_positive_"
        "volume_edge_promotion_edge_ledger.json.gz",
        "8c9ed8b09e994a00ca3ca4906c35b454523383b7d488f66e7a082dbbd4b1fcec",
    ),
    "R300E": (
        "cm2_round300e_source_g_r248_half_open_owner_lower_component_edge_"
        "promotion_all_edge_ledger.json.gz",
        "4aa7ae76d984d15b345d5d6805ec4f06a47e30a46fff1315f346e81f1f17122f",
    ),
    "R300E_WITNESS": (
        "cm2_round300e_source_g_r248_half_open_owner_lower_component_edge_"
        "promotion_witness_ledger.json.gz",
        "69f480da55e917b7b75bfe1efa823a9228d9b563f6f3aabd4e2a364c9e50eb9f",
    ),
    "R300F": (
        "cm2_round300f_source_g_r245_half_open_owner_sheet_attachment_"
        "ledger.json.gz",
        "1f958f9f3e3aff7327d897b39851f2d2d030d820e00859d302241702613cae1f",
    ),
    "R300A": (
        "cm2_round300a_source_g_r287_graph_zero_lower_frontier_"
        "exhaustion_ledger.json.gz",
        "ddc1a8bc53861afeb93d3569c6efa228f17d86f161db31a39fa9b72458ab8f2d",
    ),
    "R300B_INELIGIBLE": (
        "cm2_round300b_source_g_registry_boundary_and_complete_r275_face_"
        "inventory_closure_self_exclusion_and_unresolved.json.gz",
        "7f820047e7fe2e37aead3c1b1e4ddab1b4d480157bfac24c6c95b3e3bd0cb897",
    ),
    "R300D": (
        "cm2_round300d_source_g_lower_physical_witness_component_edge_"
        "promotion_ledger.json.gz",
        "287d1382b25fd3d5cd0a52c6da8cabbb012040b8a6e35c9f887d7a425b812fa7",
    ),
    "R300G": (
        "cm2_round300g_source_g_r300a_explicit_outgoing_seam_attachment_"
        "exclusion_ledger.json.gz",
        "84b4e855cd95867758cd87a88f0ef55b712218999166796712495d70602bb61b",
    ),
    "R300H_PAIR": (
        "cm2_round300h_source_g_enriched_lower_witness_and_single_"
        "assignment_failclosed_closure_enriched_pair_disposition_ledger.json.gz",
        "29261a9a6b366252492c703b01714236c3a80f759fa540412f291a3bf586c4c9",
    ),
    "R300H_SINGLE": (
        "cm2_round300h_source_g_enriched_lower_witness_and_single_"
        "assignment_failclosed_closure_single_target_disposition_ledger.json.gz",
        "fba21c8297caca43eb1f9ff5dea5af258b7893550cc3d3dd97abf3be88acc8e7",
    ),
    "R300H_EDGE": (
        "cm2_round300h_source_g_enriched_lower_witness_and_single_"
        "assignment_failclosed_closure_eligible_component_edge_ledger.json.gz",
        "d3dc3a8fc94ea2f7c8bbf08b87eb663c102606e964e612334199e3bb32c8cdc2",
    ),
}

R266_MANIFEST = (
    "cm2_round266_source_g_expanded_curved_face_closure_manifest.sha256"
)
R266_VERIFICATION = (
    "cm2_round266_source_g_expanded_curved_face_closure_verification.json"
)
R294_MANIFEST = (
    "cm2_round294_source_g_occurrence_registry_atomic_promotion_manifest.sha256"
)
R294_RESULT = (
    "cm2_round294_source_g_occurrence_registry_atomic_promotion_result.json"
)
R294_VERIFICATION = (
    "cm2_round294_source_g_occurrence_registry_atomic_promotion_verification.json"
)
R299A_MANIFEST = (
    "cm2_round299a_source_g_refined_occurrence_official_key_binding_closure_"
    "manifest.sha256"
)
R299A_RESULT = (
    "cm2_round299a_source_g_refined_occurrence_official_key_binding_closure_"
    "result.json"
)
R299A_VERIFICATION = (
    "cm2_round299a_source_g_refined_occurrence_official_key_binding_closure_"
    "verification.json"
)

FOUNDATION_MANIFEST_PINS = {
    R266_MANIFEST:
        "63fb5b25d52ca5de579256499629d04d15f6a313e0a73f7e80fcb463385cbe09",
    R294_MANIFEST:
        "90d5cda0271610bf95a72f94e9bae8e192425580019dd3c823b5a69d20e52131",
    R299A_MANIFEST:
        "b1dfe718dd2b7477d9cc4067f1bade59d1589b3822eaf4b94b9dd721e61b468e",
}
FOUNDATION_FILE_PINS = {
    FILES["R266"][0]: FILES["R266"][1],
    R266_VERIFICATION:
        "a6436c716cbbe74f0195e2d87f4084a28ca2a2e27b9fe5eeb98b6835df92b75b",
    FILES["R294"][0]: FILES["R294"][1],
    R294_RESULT:
        "dc93ef564ce1aec4aabbc7ff717ac76749b92899e210179a00dba63f2d32d626",
    R294_VERIFICATION:
        "13dcb461f269a8e346c220b85cad0e87a0392b5e2682b7dd70132fd8bd7a1245",
    FILES["R299A"][0]: FILES["R299A"][1],
    R299A_RESULT:
        "4835bab4ebe7afc0da8d00395f83f881dd2aaf31e0697994fed54dc7d86302f5",
    R299A_VERIFICATION:
        "3a3ab65fdd6b5bd00062e1cebbd141e1c38bded7d920705a83a57ba24db0f27d",
}

R300A_PREFIX = (
    "cm2_round300a_source_g_r287_graph_zero_lower_frontier_exhaustion"
)
R300A_MANIFEST = R300A_PREFIX + "_manifest.sha256"
R300A_LEDGER = R300A_PREFIX + "_ledger.json.gz"
R300A_RESULT = R300A_PREFIX + "_result.json"
R300A_VERIFIER = R300A_PREFIX + "_verifier.py"
R300A_VERIFICATION = R300A_PREFIX + "_verification.json"
R300D_PREFIX = (
    "cm2_round300d_source_g_lower_physical_witness_component_edge_promotion"
)
R300D_MANIFEST = R300D_PREFIX + "_manifest.sha256"
R300D_LEDGER = R300D_PREFIX + "_ledger.json.gz"
R300D_RESULT = R300D_PREFIX + "_result.json"
R300D_VERIFIER = R300D_PREFIX + "_verifier.py"
R300D_VERIFICATION = R300D_PREFIX + "_verification.json"
INELIGIBLE_PACKAGE_PINS = {
    R300A_MANIFEST:
        "9f9e86d93aebe2b47e525af795a3a9e8331ab3b6f2d71ecafe69429238a1aee8",
    R300A_LEDGER: FILES["R300A"][1],
    R300A_RESULT:
        "b8f7c27f8761f1eb611f8fd57a0e773572f045963560f5d63b44baa1680e7ee4",
    R300A_VERIFIER:
        "b425a9836a474122a99cd76a0f4e81175c65c2f426ad7e38773cce46220c64d5",
    R300A_VERIFICATION:
        "e976fc2912c1d902029b8c4c7d4864bd8bb1cb6895bc09c215f2a5ac2400c696",
    R300D_MANIFEST:
        "8dd3907a363ae0c4fe7d524061a02b4c70944ce870ded941d5d617911ea1124e",
    R300D_LEDGER: FILES["R300D"][1],
    R300D_RESULT:
        "59b7e788ed217ceb590a3e2125cc13aefbf447af236315ea607f4f631cb29c84",
    R300D_VERIFIER:
        "d7fbbeeb3f0959de54e55f20928f276a36df75100907b1af16bb2e46317ddc26",
    R300D_VERIFICATION:
        "a5bd12b10103b4785574bd4633e608c7fd5107369ba8f2343ebffe7e08eba1c7",
}

R300E_MANIFEST = (
    "cm2_round300e_source_g_r248_half_open_owner_lower_component_edge_"
    "promotion_manifest.sha256"
)
R300E_MANIFEST_PIN = (
    "4e6c4eec1af04a907285d41395aeef63e911b3b5486db91b33a95fa62e45177a"
)
R300E_WITNESS_LEDGER = (
    "cm2_round300e_source_g_r248_half_open_owner_lower_component_edge_"
    "promotion_witness_ledger.json.gz"
)
R300E_WITNESS_LEDGER_PIN = FILES["R300E_WITNESS"][1]
R300B_FACE_INVENTORY = (
    "cm2_round300b_source_g_registry_boundary_and_complete_r275_face_"
    "inventory_closure_face_inventory.json.gz"
)
R300B_FACE_INVENTORY_PIN = (
    "443f26bdbed929d873972be0d2232809bf022feed1a5709001ee8f2dda49ac2d"
)

R300G_PREFIX = (
    "cm2_round300g_source_g_r300a_explicit_outgoing_seam_attachment_exclusion"
)
R300G_MANIFEST = R300G_PREFIX + "_manifest.sha256"
R300G_LEDGER = R300G_PREFIX + "_ledger.json.gz"
R300G_RESULT = R300G_PREFIX + "_result.json"
R300G_VERIFIER = R300G_PREFIX + "_verifier.py"
R300G_VERIFICATION = R300G_PREFIX + "_verification.json"
R300H_PREFIX = (
    "cm2_round300h_source_g_enriched_lower_witness_and_single_assignment_"
    "failclosed_closure"
)
R300H_PRODUCER = R300H_PREFIX + ".py"
R300H_MANIFEST = R300H_PREFIX + "_manifest.sha256"
R300H_PAIR_LEDGER = R300H_PREFIX + "_enriched_pair_disposition_ledger.json.gz"
R300H_SINGLE_LEDGER = R300H_PREFIX + "_single_target_disposition_ledger.json.gz"
R300H_ELIGIBLE_LEDGER = R300H_PREFIX + "_eligible_component_edge_ledger.json.gz"
R300H_RESULT = R300H_PREFIX + "_result.json"
R300H_VERIFIER = R300H_PREFIX + "_verifier.py"
R300H_ATTACKS = R300H_PREFIX + "_attack_suite.json"
R300H_VERIFICATION = R300H_PREFIX + "_verification.json"
R300H_REPORT = R300H_PREFIX + "_report.md"
R300H_COLD_REPLAY = R300H_PREFIX + "_cold_replay.md"

GATE_PACKAGES = {
    "R300G": {
        "manifest": R300G_MANIFEST,
        "manifest_sha256":
            "ce700bdc855406396ae89aa936d07c506057d7dfab806d064a91c0e08b0d8a4c",
        "members": {
            R300G_LEDGER:
                "84b4e855cd95867758cd87a88f0ef55b712218999166796712495d70602bb61b",
            R300G_RESULT:
                "5693f64262623dccbdc0138edc7dc876b06b2822f552eee26ac580c27f861c1c",
            R300G_VERIFIER:
                "2b3a241ed3769273113d17f3907007da57626f7eb3e5f7b75eb107b833b1f9a9",
            R300G_VERIFICATION:
                "2416d2efe04935fddc615c5542b1f0f0c56411111152d5e27d1021c59d8782ff",
        },
        "result": R300G_RESULT,
        "verification": R300G_VERIFICATION,
    },
    "R300H": {
        "manifest": R300H_MANIFEST,
        "manifest_sha256":
            "7cb31452e1f22c22d3d5dd7552f49c86526e60dba6cdedeb13888f4ff41bf71e",
        "members": {
            R300H_PRODUCER:
                "3d93a3a13b494fbf136916827e63fb5861960cec9de8dcb4facceb16c5f0b952",
            R300H_PAIR_LEDGER: FILES["R300H_PAIR"][1],
            R300H_SINGLE_LEDGER: FILES["R300H_SINGLE"][1],
            R300H_ELIGIBLE_LEDGER: FILES["R300H_EDGE"][1],
            R300H_RESULT:
                "378546acee06a11dce5da9c41c5d15d739888e163e14bab5fb80ba8d92ffa56c",
            R300H_VERIFIER:
                "eb545e912766993cb20a04a6ef2cfa8559a5a7ad474481bb3f55c8a036893c33",
            R300H_ATTACKS:
                "aad9c2e077a4770aa16380d784fc1d6cb4e7d825d430000e58b9f1f97446cd15",
            R300H_VERIFICATION:
                "7c06a189a642ba3068ea8e61a8170a7847fc4d5ae4260375af212773b18409bb",
            R300H_REPORT:
                "77426faa4b9032be9604a4208782bd529c290acf6ac0633d3c47294236ca6dbb",
            R300H_COLD_REPLAY:
                "1d1749ecf5d268b3a33088d7fd24af835cb7cf7fde99a670f849eb171d2d14fa",
        },
        "result": R300H_RESULT,
        "verification": R300H_VERIFICATION,
    },
}

R266_MEMBER_COMMITMENT = {
    "row_count": 259_752,
    "row_ids_sha256":
        "f064d177c25985b38c899c651923b83ba47ee36b465902cca85a4c21e0c33ca6",
    "row_hashes_sha256":
        "1f28c84d14cf3fabb38d0f2d1fdf862ce8f6a4559552d68fd9c0a5bd0a3ed3ee",
    "rows_sha256":
        "28398acde446047ff4a210b2fa0831d2c44e3758f7e9239b5ec63cecc386b257",
}
R266_ROOT_COMMITMENT = {
    "row_count": 63_224,
    "row_ids_sha256":
        "46889c11f7b1870bfe39faab0625054233c1c48b2cba2bd87cc9232b0405520a",
    "row_hashes_sha256":
        "a32bce700a94c39886a0d837e371e48562ae8380b71959e7326b0c4afef5e3b2",
    "rows_sha256":
        "bfdf5cde6fc68d39676e1543a6138672e7b8a68424280c6f895a8408f3eb579d",
}
R294_COMMITMENT = {
    "row_count": 431_208,
    "row_ids_sha256":
        "bbaca3ecbb804a87fd509aac2b8bd9f505d0c008e7bddbeb1a2ecb2a966f5509",
    "row_hashes_sha256":
        "ea98de3dab7e6f308f5a07d04bc29ca0d9dcd265a5323d8ecdb728cc501eed56",
    "rows_sha256":
        "33936ecd04cbce9f9a308b0da10381bd854b45028db53db5d3cbb9aa8b264044",
    "occurrence_ids_sha256":
        "169869b5755eda22f1527e7393a5bef0a77a842eefbdf45ab34d315b0dad1936",
}
R299A_COMMITMENT = {
    "row_count": 9_404,
    "row_ids_sha256":
        "a3786265b52feafd316cc81c71ab05ee32976b5ff7f1de091853d34a7b33e6e3",
    "row_hashes_sha256":
        "65e6f22b9085685b20a50dedfcb62cd08dd01e1ea70a61ee850d74450402b333",
    "rows_sha256":
        "6955fe1c36555c7aa492e4c5cfd1ab945fe16b824ea49bd3d72cea3bd3136c0f",
}
R300A_CANONICAL_PAIR_COMMITMENT = {
    "row_count": 3_232,
    "row_ids_sha256":
        "876617c3beda3cc50bcff712e6273786ba87a513d5422e875960353c6eb64332",
    "row_hashes_sha256":
        "2f34355d24534220e6cf42e636925d0f5adf9d7430b61563f20f2d4d928faac4",
    "rows_sha256":
        "7991da0c6f268cfb87628f1bee88f560ed319bec1028bee43e427c65ac8f6f16",
}
R300A_SOURCE_EXPANSION_COMMITMENT = {
    "row_count": 3_488,
    "row_ids_sha256":
        "22942899c9601c88f59014f4b9cd449b2758e60eaa803e949370b06fb58f0885",
    "row_hashes_sha256":
        "aeed60e7457dda5c1042f47198a223e5ad6cb4d1fa1a3d6b66187707f267cbe1",
    "rows_sha256":
        "99318f4f1c092a223448a5ccd2a70cbd6d9a8b9a9f2d5b9ee9fdb861c738cbcb",
}
R300D_INCIDENCE_COMMITMENT = {
    "row_count": 111_524,
    "row_ids_sha256":
        "19ce53e77604142bf0410e49b1f1c8bf921beb715538901776c517d098626c6b",
    "row_hashes_sha256":
        "97f5ae7ffde9c583bf0cc75e6ad685c4d22cdc487261576b8e904a69fcad17d2",
    "rows_sha256":
        "3623738c4a7a41980b6386d089f319d5149358e7e91fb422053289068eedbc8b",
}
R300D_SINGLE_COMMITMENT = {
    "row_count": 1_600,
    "row_ids_sha256":
        "74cedc49a8aa56c4520e56546390705e96faafae6c2d2547d8851ca123d9880c",
    "row_hashes_sha256":
        "96ff1c02ef4ea047a9b61bb0bfb806ced48e896cdf82000ba53a161a67e3ab24",
    "rows_sha256":
        "953522b88f21adb705505ec44ce121910fcdaa81dc13424068ec572733a28007",
}
R300E_WITNESS_COMMITMENT = {
    "row_count": 12_992,
    "row_ids_sha256":
        "8d4f4eb7e1a1058283c9c163eefe2516c01193e3d84a8ab8b0174faee3f9e9c8",
    "row_hashes_sha256":
        "71b8457998a7e5813d57b10197c2a8506111c7ec041f893bb177d8a01de5ab92",
    "rows_sha256":
        "6d2f26a0782e66fe01d440586c21846581a55944252e364909bb08600e09abaa",
}
R300B_FACE_INVENTORY_COMMITMENT = {
    "row_count": 62_548,
    "row_ids_sha256":
        "ec642912b4779fa7a19212ee34b3c2ff408b1e60f2d4862fb6a1a1d2127651f1",
    "row_hashes_sha256":
        "3ad41927e18ba1e527a091a3bb2536118810caed2659ec4f7e03134e4ba947d7",
    "rows_sha256":
        "4c8c0e4662f72301cc1c97fe71d2cdd2735bfce4bed1b5bb445bde9893876ee7",
}
R300G_COMMITMENT = {
    "row_count": 128,
    "row_ids_sha256":
        "77449ccec8ed05e1b1df63467c466225686eb5b73d1897708d406b671f3b14f9",
    "row_hashes_sha256":
        "fb36993227018ad0479e35bf6fae0b3f8ee8fbcaffbd38cf84e74f6822d5c22e",
    "rows_sha256":
        "cbf7483e4ad97570965dec413fa029053137316d702bb2f831d25f2472c737e9",
}
R300H_PAIR_COMMITMENT = {
    "row_count": 144,
    "row_ids_sha256":
        "1d077d69858a380df2c18923f73f0794a5a06b784ea6867431804bedc624f3cc",
    "row_hashes_sha256":
        "547baddd341f9177e43550c860b8965eb9ede290e1f8ce66fd488e98273afeaf",
    "rows_sha256":
        "9dd35a49e9c23e0228522d7a3c97ddaaa139a465e9a65f9cbacba9afada9a3cb",
}
R300H_SINGLE_COMMITMENT = {
    "row_count": 1_600,
    "row_ids_sha256":
        "23d05b1483c95be92b397eba7f4413eae03dbec9d0fd3a2e62d6586e8a585e44",
    "row_hashes_sha256":
        "6cb93443ba7653d8adc8151cccaf40752c99de924f5808e43d59f1e97c8f6e31",
    "rows_sha256":
        "f1ac5a8c3384bb33f2963ab0de73239834ff4b717bbb69da3621cfef882dc40b",
}
R300H_EMPTY_EDGE_COMMITMENT = {
    "row_count": 0,
    "row_ids_sha256":
        "4f53cda18c2baa0c0354bb5f9a3ecbe5ed12ab4d8e11ba873c2f11161202b945",
    "row_hashes_sha256":
        "4f53cda18c2baa0c0354bb5f9a3ecbe5ed12ab4d8e11ba873c2f11161202b945",
    "rows_sha256":
        "4f53cda18c2baa0c0354bb5f9a3ecbe5ed12ab4d8e11ba873c2f11161202b945",
}

PRESERVED = "PRESERVED_ROUND266_EXISTING_LOCAL_OCCURRENCE"
NEW288 = "CANDIDATE_NEW_ROUND288_CANONICAL_ATOM"
NEW292 = "CANDIDATE_NEW_ROUND292_REFINED_R287_SUPPORT_COMPONENT"
PIN_RE = re.compile(r"^[0-9a-f]{64}$")

@dataclass(frozen=True)
class Channel:
    name: str
    manifest: str
    manifest_sha256: str
    ledger: str
    ledger_sha256: str
    result: str
    verifier: str
    verification: str
    row_count: int
    row_id_field: str
    endpoint_field: str
    row_ids_sha256: str
    row_hashes_sha256: str
    rows_sha256: str
    required_values: tuple[tuple[str, Any], ...]

    @property
    def commitment(self) -> dict[str, Any]:
        return {
            "row_count": self.row_count,
            "row_ids_sha256": self.row_ids_sha256,
            "row_hashes_sha256": self.row_hashes_sha256,
            "rows_sha256": self.rows_sha256,
        }


LEGAL_CHANNELS = (
    Channel(
        "R297_ORDINARY_FACE",
        "cm2_round297_source_g_ordinary_face_occurrence_edge_promotion_"
        "manifest.sha256",
        "1feecefa897c5320eadc006509ba6bde84cdbf692dfaddd024472b93c13c38c0",
        FILES["R297"][0],
        FILES["R297"][1],
        "cm2_round297_source_g_ordinary_face_occurrence_edge_promotion_"
        "result.json",
        "cm2_round297_source_g_ordinary_face_occurrence_edge_promotion_"
        "verifier.py",
        "cm2_round297_source_g_ordinary_face_occurrence_edge_promotion_"
        "verification.json",
        330_724,
        "Round297_ordinary_face_occurrence_edge_row_id",
        "exact_occurrence_endpoint_pair",
        "6ee71aba6c26953b7b368dc89d985468fb7410e6d5a47b0300c2962ceca045a6",
        "02586985662d453791f1461d71c15cca7e5e109b25025a8eec5f5c7c441c937d",
        "52a6ee955af86ac186824c23d27c0372d3c14792839f372620d311c9f144dbe0",
        (
            ("formal_ordinary_component_edge_witness_credit", 1),
            ("occurrence_endpoint_pair_is_nonself", True),
            ("formal_occurrence_identity_collapse_credit", 0),
            ("formal_DSU_rank_reduction_credit", 0),
        ),
    ),
    Channel(
        "R296_TRUE_SEAM",
        "cm2_round296_source_g_true_seam_occurrence_edge_ledger_closure_"
        "manifest.sha256",
        "f7786b9cdec45cb381ec46489eb44b0365b8ee43d81ae9a9611cb2dcdee7fb59",
        FILES["R296"][0],
        FILES["R296"][1],
        "cm2_round296_source_g_true_seam_occurrence_edge_ledger_closure_"
        "result.json",
        "cm2_round296_source_g_true_seam_occurrence_edge_ledger_closure_"
        "verifier.py",
        "cm2_round296_source_g_true_seam_occurrence_edge_ledger_closure_"
        "verification.json",
        48_444,
        "Round296_true_seam_occurrence_edge_row_id",
        "unordered_formal_occurrence_endpoint_pair",
        "d6ad370122651272a384f4fc8f48ca8b3e6a6b3f2cd9c9f03a073c1d207f853a",
        "608bf8140bcb77b950f00628804c649e82ebdb1055816df2ae32f68b3e321a06",
        "e989828d774a08694404fe678c4ae74dc199f8f5cdf4d398dc755f5ff4b0561f",
        (
            ("formal_true_seam_edge_credit", 1),
            ("occurrence_identity_collapsed", False),
            ("formal_DSU_rank_reduction_credit", 0),
        ),
    ),
    Channel(
        "R299C_SIGNED_FACE",
        "cm2_round299c_source_g_r292_signed_support_face_edge_promotion_"
        "manifest.sha256",
        "62e04cdd7f9c3d2fb865be0a999e1ce5ed1de435151a1b7bb5bba0176709b9b3",
        FILES["R299C"][0],
        FILES["R299C"][1],
        "cm2_round299c_source_g_r292_signed_support_face_edge_promotion_"
        "result.json",
        "cm2_round299c_source_g_r292_signed_support_face_edge_promotion_"
        "verifier.py",
        "cm2_round299c_source_g_r292_signed_support_face_edge_promotion_"
        "verification.json",
        25_452,
        "Round299C_canonical_signed_face_occurrence_edge_row_id",
        "unordered_formal_occurrence_endpoint_pair",
        "56c09af202b36a867f1cc9893c968ab231a952fdea85cafdde999be4ebcba70d",
        "138395a9a63f16623f2df4ff3656d82c744a522de23f20fc728c925baa833dc6",
        "14c6f44ab989a075a46de4c842b14cad9dff211373b52d0be41c8083620c3c87",
        (
            ("formal_component_edge_credit", 1),
            ("formal_occurrence_identity_collapse_credit", 0),
            ("formal_DSU_rank_reduction_credit", 0),
        ),
    ),
    Channel(
        "R300B_COMPLETE_FACE",
        "cm2_round300b_source_g_registry_boundary_and_complete_r275_face_"
        "inventory_closure_manifest.sha256",
        "6208c4bfd8b147f06ffb735d4b5621faff22585831b5f19d48e8125514ac4a6b",
        FILES["R300B"][0],
        FILES["R300B"][1],
        "cm2_round300b_source_g_registry_boundary_and_complete_r275_face_"
        "inventory_closure_result.json",
        "cm2_round300b_source_g_registry_boundary_and_complete_r275_face_"
        "inventory_closure_verifier.py",
        "cm2_round300b_source_g_registry_boundary_and_complete_r275_face_"
        "inventory_closure_verification.json",
        10_416,
        "Round300B_canonical_novel_occurrence_edge_row_id",
        "unordered_formal_occurrence_endpoint_pair",
        "5b3c7f446db963d7b763ffebf5d596c1fda92e3d31e573fd3fd95b1c855b5119",
        "cbd960748bfed1d31e31cbad053c3a0c11d8264fe043e35fe85350c54bf8f648",
        "cbc1cf8de07559c50db2cd84920379c9e0bd839c22bf9be3267784adc801c808",
        (
            ("formal_component_edge_credit", 1),
            ("formal_occurrence_identity_collapse_credit", 0),
            ("formal_DSU_rank_reduction_credit", 0),
            ("all_prior_channel_memberships_false", True),
        ),
    ),
    Channel(
        "R300C_POSITIVE_VOLUME",
        "cm2_round300c_source_g_virtual_stratum_new_occurrence_positive_"
        "volume_edge_promotion_manifest.sha256",
        "cd327de2702ac209b0b0a03d09b3744596a901c2d65794dd86231d31a0be3503",
        FILES["R300C"][0],
        FILES["R300C"][1],
        "cm2_round300c_source_g_virtual_stratum_new_occurrence_positive_"
        "volume_edge_promotion_result.json",
        "cm2_round300c_source_g_virtual_stratum_new_occurrence_positive_"
        "volume_edge_promotion_verifier.py",
        "cm2_round300c_source_g_virtual_stratum_new_occurrence_positive_"
        "volume_edge_promotion_verification.json",
        6_314,
        "Round300C_virtual_occurrence_positive_volume_edge_row_id",
        "canonical_component_edge_endpoint_pair",
        "f35b03655c3fb6986d4798b1b9a2858cba915d1279d393396315358ef0632c65",
        "2f4c62e37a58c457bdc22c3271bce1229acfda942f46ec38341a7fd693897efb",
        "41716c0e35152d8e735310a2efdb55cbe7cf5813c8f94dc0ee4f88525bcd0a5b",
        (
            ("formal_positive_volume_component_edge_witness_credit", 1),
            ("formal_occurrence_identity_collapse_credit", 0),
            ("formal_DSU_rank_reduction_credit", 0),
        ),
    ),
    Channel(
        "R300E_HALF_OPEN_OWNER",
        R300E_MANIFEST,
        R300E_MANIFEST_PIN,
        FILES["R300E"][0],
        FILES["R300E"][1],
        "cm2_round300e_source_g_r248_half_open_owner_lower_component_edge_"
        "promotion_result.json",
        "cm2_round300e_source_g_r248_half_open_owner_lower_component_edge_"
        "promotion_verifier.py",
        "cm2_round300e_source_g_r248_half_open_owner_lower_component_edge_"
        "promotion_verification.json",
        12_992,
        "Round300E_half_open_owner_component_edge_row_id",
        "canonical_component_edge_endpoint_pair",
        "d2a8a525bd593f1e9cedcd3306b84b87ba6ccc88a4878d18572c32dca43f2272",
        "de692835ee6333a65e554a4f1567186995b5d3c80adfd374227fc82e096fc4c7",
        "76bfbac5c9706416be4ed1e4df3104006dc26cb68cf5e8f23c5b482aa44ac20a",
        (
            ("formal_half_open_owner_component_edge_credit", 1),
            ("eligible_for_component_DSU_application", True),
            ("formal_occurrence_identity_collapse_credit", 0),
            ("formal_DSU_rank_reduction_credit", 0),
        ),
    ),
    Channel(
        "R300F_R245_HALF_OPEN_OWNER",
        "cm2_round300f_source_g_r245_half_open_owner_sheet_attachment_"
        "manifest.sha256",
        "811656ed9160b622014a94901a1e08c7d3e6313db08f86d65ae01afefb49bbeb",
        FILES["R300F"][0],
        FILES["R300F"][1],
        "cm2_round300f_source_g_r245_half_open_owner_sheet_attachment_"
        "result.json",
        "cm2_round300f_source_g_r245_half_open_owner_sheet_attachment_"
        "verifier.py",
        "cm2_round300f_source_g_r245_half_open_owner_sheet_attachment_"
        "verification.json",
        264,
        "Round300F_R245_half_open_owner_attachment_row_id",
        "canonical_component_edge_endpoint_pair",
        "e836f0e338cd43b19cebc718fe5ba60d8e5a313bcf8aac9eea72b8bfc8246c22",
        "58fc944871aaba4d49708359d59a6c996953068cef3d59e7a1d3719179944180",
        "6be4a94b83fb5b1b61a93bf84ca2b61fb1f9dfd9cbf4425b5dbb6a0dcdc9f6c2",
        (
            ("formal_half_open_owner_component_edge_witness_credit", 1),
            ("eligible_for_component_DSU_application", True),
            ("Round300A_prior_explicit_graph_zero_pair_present", False),
            ("Round300A_two_endpoint_union_credit", 0),
            ("shadow_component_edge_witness_credit", 0),
            ("formal_occurrence_identity_collapse_credit", 0),
            ("formal_DSU_rank_reduction_credit", 0),
        ),
    ),
)


class AuditError(RuntimeError):
    pass


def need(condition: bool, label: str) -> None:
    if not condition:
        raise AuditError(label)


ENCODER = json.JSONEncoder(
    sort_keys=True,
    separators=(",", ":"),
    ensure_ascii=False,
    allow_nan=False,
)


def canonical(value: Any) -> bytes:
    return "".join(ENCODER.iterencode(value)).encode()


def digest(value: Any) -> str:
    return hashlib.sha256(canonical(value)).hexdigest()


def file_sha256(path: Path) -> str:
    state = hashlib.sha256()
    with path.open("rb") as stream:
        for block in iter(lambda: stream.read(1 << 20), b""):
            state.update(block)
    return state.hexdigest()


def sha256_bytes(data: bytes) -> str:
    return hashlib.sha256(data).hexdigest()


def guard_regular_path(
    path: Path,
    parent: Path,
    expected_name: str,
) -> None:
    need(path.name == expected_name, "path alias:" + expected_name)
    need(
        path.parent.resolve() == parent.resolve(),
        "path escape:" + expected_name,
    )
    need(path.exists(), "missing path:" + expected_name)
    need(not path.is_symlink(), "symlink:" + expected_name)
    need(path.is_file(), "not regular file:" + expected_name)
    need(path.stat().st_nlink == 1, "hardlink:" + expected_name)


def guard_exact_regular_path(path: Path, expected_name: str) -> None:
    guard_regular_path(path, HERE, expected_name)


def guard_inputs_and_inert_producer() -> None:
    guard_exact_regular_path(PRODUCER_PATH, PRODUCER_PATH.name)
    need(
        file_sha256(PRODUCER_PATH) == PRODUCER_SHA256,
        "inert producer byte pin",
    )
    for filename, expected in FILES.values():
        path = D / filename
        guard_exact_regular_path(path, filename)
        need(file_sha256(path) == expected, "input pin:" + filename)


def unique_object(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
    value: dict[str, Any] = {}
    for key, item in pairs:
        need(key not in value, "duplicate JSON key:" + key)
        value[key] = item
    return value


def iter_array_stream(
    stream: TextIO,
    marker: str,
    initial: str = "",
) -> Iterator[Any]:
    token = json.dumps(marker, ensure_ascii=False) + ":["
    buffer = initial
    while token not in buffer:
        block = stream.read(1 << 20)
        need(bool(block), "missing marker:" + marker)
        buffer += block
        if len(buffer) > len(token) + (1 << 20):
            buffer = buffer[-(len(token) + (1 << 20)) :]
    buffer = buffer.split(token, 1)[1]
    decoder = json.JSONDecoder(
        object_pairs_hook=unique_object,
        parse_float=lambda value: (_ for _ in ()).throw(
            AuditError("float:" + value)
        ),
        parse_constant=lambda value: (_ for _ in ()).throw(
            AuditError("constant:" + value)
        ),
    )
    while True:
        buffer = buffer.lstrip()
        if not buffer:
            block = stream.read(1 << 20)
            need(bool(block), "truncated array:" + marker)
            buffer = block
            continue
        if buffer[0] == ",":
            buffer = buffer[1:]
            continue
        if buffer[0] == "]":
            return
        while True:
            try:
                value, end = decoder.raw_decode(buffer)
                break
            except json.JSONDecodeError:
                block = stream.read(1 << 20)
                need(bool(block), "truncated row:" + marker)
                buffer += block
        yield value
        buffer = buffer[end:]


def iter_gzip_rows(
    path: Path,
    marker: str = "rows",
) -> Iterator[dict[str, Any]]:
    with gzip.open(path, "rt", encoding="utf-8", newline="") as stream:
        yield from iter_array_stream(stream, marker)


def iter_named_rows(path: Path, table: str) -> Iterator[dict[str, Any]]:
    token = json.dumps(table, ensure_ascii=False) + ":"
    with path.open("rt", encoding="utf-8", newline="") as stream:
        buffer = ""
        while token not in buffer:
            block = stream.read(1 << 20)
            need(bool(block), "missing table:" + table)
            buffer += block
            if len(buffer) > len(token) + (1 << 20):
                buffer = buffer[-(len(token) + (1 << 20)) :]
        initial = buffer.split(token, 1)[1]
        yield from iter_array_stream(stream, "rows", initial)


def verify_row(row: dict[str, Any], label: str) -> None:
    claimed = row["row_sha256"]
    payload = dict(row)
    payload.pop("row_sha256")
    need(digest(payload) == claimed, "row closure:" + label)


def close_row(payload: dict[str, Any]) -> dict[str, Any]:
    need("row_sha256" not in payload, "row closure field absent")
    row = dict(payload)
    row["row_sha256"] = digest(payload)
    return row


def strict_json(raw: bytes, label: str) -> dict[str, Any]:
    need(
        raw
        and not raw.startswith(b"\xef\xbb\xbf")
        and b"\x00" not in raw,
        "strict JSON bytes:" + label,
    )

    def reject(token: str) -> Any:
        raise AuditError("nonintegral JSON:" + label + ":" + token)

    try:
        value = json.loads(
            raw.decode("utf-8"),
            object_pairs_hook=unique_object,
            parse_float=reject,
            parse_constant=reject,
        )
    except (UnicodeDecodeError, json.JSONDecodeError) as error:
        raise AuditError("invalid JSON:" + label) from error
    need(type(value) is dict, "top-level JSON object:" + label)
    return value


def read_json_path(path: Path) -> dict[str, Any]:
    guard_exact_regular_path(path, path.name)
    return strict_json(path.read_bytes(), path.name)


def check_result_self(document: dict[str, Any], label: str) -> None:
    payload = dict(document)
    claimed = payload.pop("result_sha256", None)
    need(
        type(claimed) is str and claimed == digest(payload),
        "result self closure:" + label,
    )


def parse_manifest(name: str) -> dict[str, str]:
    path = HERE / name
    guard_exact_regular_path(path, name)
    need(path.stat().st_size <= 300_000, "manifest size:" + name)
    output: dict[str, str] = {}
    for line in path.read_text(encoding="ascii").splitlines():
        if not line:
            continue
        match = re.fullmatch(r"([0-9a-f]{64})  ([^\n]+)", line)
        need(match is not None, "manifest syntax:" + name)
        pin, member = match.groups()
        need(
            Path(member).name == member and member not in output,
            "manifest member policy:" + name,
        )
        output[member] = pin
    need(bool(output), "nonempty manifest:" + name)
    return output


def validate_manifest_selection(
    name: str,
    manifest_sha256: str,
    selected: dict[str, str],
) -> None:
    guard_exact_regular_path(HERE / name, name)
    need(
        PIN_RE.fullmatch(manifest_sha256) is not None
        and file_sha256(HERE / name) == manifest_sha256,
        "manifest pin:" + name,
    )
    entries = parse_manifest(name)
    for member, expected in selected.items():
        guard_exact_regular_path(HERE / member, member)
        need(
            entries.get(member) == expected
            and file_sha256(HERE / member) == expected,
            "manifest selected member:" + name + ":" + member,
        )


class ListHash:
    def __init__(self) -> None:
        self.state = hashlib.sha256(b"[")
        self.count = 0

    def add(self, value: Any) -> None:
        if self.count:
            self.state.update(b",")
        self.state.update(canonical(value))
        self.count += 1

    def finish(self) -> str:
        state = self.state.copy()
        state.update(b"]")
        return state.hexdigest()


def committed_rows(
    rows: Iterable[dict[str, Any]],
    id_field: str,
    expected: dict[str, Any],
    occurrence_field: str | None = None,
) -> Iterator[dict[str, Any]]:
    row_hash = ListHash()
    id_hash = ListHash()
    closure_hash = ListHash()
    occurrence_hash = ListHash() if occurrence_field else None
    seen: set[str] = set()
    for row in rows:
        verify_row(row, id_field)
        row_id = row.get(id_field)
        need(
            type(row_id) is str and row_id not in seen,
            "unique row id:" + id_field,
        )
        seen.add(row_id)
        row_hash.add(row)
        id_hash.add(row_id)
        closure_hash.add(row["row_sha256"])
        if occurrence_hash is not None:
            occurrence_hash.add(row[occurrence_field])
        yield row
    actual = {
        "row_count": row_hash.count,
        "row_ids_sha256": id_hash.finish(),
        "row_hashes_sha256": closure_hash.finish(),
        "rows_sha256": row_hash.finish(),
    }
    if occurrence_hash is not None:
        actual["occurrence_ids_sha256"] = occurrence_hash.finish()
    need(actual == expected, "complete table commitment:" + id_field)


def committed_gzip_rows(
    name: str,
    id_field: str,
    expected: dict[str, Any],
    table: str = "rows",
    occurrence_field: str | None = None,
) -> Iterator[dict[str, Any]]:
    path = HERE / name
    guard_exact_regular_path(path, name)
    yield from committed_rows(
        iter_gzip_rows(path, table),
        id_field,
        expected,
        occurrence_field,
    )


def committed_plain_rows(
    name: str,
    table: str,
    id_field: str,
    expected: dict[str, Any],
) -> Iterator[dict[str, Any]]:
    path = HERE / name
    guard_exact_regular_path(path, name)
    yield from committed_rows(
        iter_named_rows(path, table),
        id_field,
        expected,
    )


def package_members(channel: Channel) -> dict[str, str]:
    entries = parse_manifest(channel.manifest)
    required = {
        channel.ledger: channel.ledger_sha256,
        channel.result: entries.get(channel.result, ""),
        channel.verifier: entries.get(channel.verifier, ""),
        channel.verification: entries.get(channel.verification, ""),
    }
    need(
        all(PIN_RE.fullmatch(value) is not None for value in required.values()),
        "channel required members:" + channel.name,
    )
    return required


def validate_channel_package(channel: Channel) -> None:
    validate_manifest_selection(
        channel.manifest,
        channel.manifest_sha256,
        package_members(channel),
    )
    result = read_json_path(HERE / channel.result)
    check_result_self(result, channel.name)
    verification = read_json_path(HERE / channel.verification)
    need(
        result["status"].startswith("PASS")
        and verification["status"].startswith("PASS"),
        "channel status:" + channel.name,
    )


def validate_gate_packages() -> None:
    for name, package in GATE_PACKAGES.items():
        validate_manifest_selection(
            package["manifest"],
            package["manifest_sha256"],
            package["members"],
        )
        result = read_json_path(HERE / package["result"])
        check_result_self(result, name)
        verification = read_json_path(HERE / package["verification"])
        need(
            result["status"].startswith("PASS")
            and verification["status"].startswith("PASS"),
            "gate status:" + name,
        )


def validate_all_boundaries() -> None:
    guard_inputs_and_inert_producer()
    for manifest, pin in FOUNDATION_MANIFEST_PINS.items():
        selected = {
            member: member_pin
            for member, member_pin in FOUNDATION_FILE_PINS.items()
            if member in parse_manifest(manifest)
        }
        validate_manifest_selection(manifest, pin, selected)
    for result_name in (R294_RESULT, R299A_RESULT):
        check_result_self(read_json_path(HERE / result_name), result_name)
    for manifest, members in (
        (
            R300A_MANIFEST,
            (R300A_LEDGER, R300A_RESULT, R300A_VERIFIER, R300A_VERIFICATION),
        ),
        (
            R300D_MANIFEST,
            (R300D_LEDGER, R300D_RESULT, R300D_VERIFIER, R300D_VERIFICATION),
        ),
    ):
        validate_manifest_selection(
            manifest,
            INELIGIBLE_PACKAGE_PINS[manifest],
            {member: INELIGIBLE_PACKAGE_PINS[member] for member in members},
        )
    validate_manifest_selection(
        R300E_MANIFEST,
        R300E_MANIFEST_PIN,
        {R300E_WITNESS_LEDGER: R300E_WITNESS_LEDGER_PIN},
    )
    validate_gate_packages()
    for channel in LEGAL_CHANNELS:
        validate_channel_package(channel)


@dataclass
class Member:
    member_id: str
    member_kind: str
    base_root: str
    official_key_id: str | None
    source_row_id: str
    source_row_sha256: str


@dataclass(frozen=True)
class Edge:
    channel: str
    source_row_id: str
    source_row_sha256: str
    left_endpoint: str
    right_endpoint: str
    left_base_root: str
    right_base_root: str


class DSU:
    def __init__(self, roots: set[str]):
        self.parent = {value: value for value in roots}
        self.size = {value: 1 for value in roots}

    def find(self, value: str) -> str:
        parent = self.parent
        need(value in parent, "unknown DSU root:" + value)
        cursor = value
        while parent[cursor] != cursor:
            cursor = parent[cursor]
        root = cursor
        cursor = value
        while parent[cursor] != cursor:
            following = parent[cursor]
            parent[cursor] = root
            cursor = following
        return root

    def union(self, left: str, right: str) -> bool:
        left = self.find(left)
        right = self.find(right)
        if left == right:
            return False
        if (self.size[left], left) < (self.size[right], right):
            left, right = right, left
        self.parent[right] = left
        self.size[left] += self.size[right]
        return True


def reconstruct_member_universe() -> tuple[
    dict[str, Member], set[str], dict[str, Any]
]:
    members: dict[str, Member] = {}
    retained_roots: set[str] = set()
    kind_census: Counter[str] = Counter()
    for row in committed_plain_rows(
        FILES["R266"][0],
        "formal_post_Round266_component_member_frontier_ledger",
        "post_Round266_component_member_frontier_row_id",
        R266_MEMBER_COMMITMENT,
    ):
        member_id = row["component_member_id"]
        root = row["post_Round266_quotient_component_id"]
        need(
            member_id not in members
            and row["member_identity_preserved"] is True
            and row["maximality_credit"] == 0
            and type(row["official_key_id"]) is str,
            "Round266 identity-preserved member",
        )
        members[member_id] = Member(
            member_id,
            row["component_member_kind"],
            root,
            row["official_key_id"],
            row["post_Round266_component_member_frontier_row_id"],
            row["row_sha256"],
        )
        retained_roots.add(root)
        kind_census[row["component_member_kind"]] += 1
    need(
        len(members) == 259_752
        and len(retained_roots) == 63_224
        and kind_census == {
            "EXPANDED_OCCURRENCE": 126_468,
            "VALID_VIRTUAL_STRATUM": 133_284,
        },
        "Round266 complete member/root census",
    )

    root_keys: dict[str, str] = {}
    for row in committed_plain_rows(
        FILES["R266"][0],
        "formal_post_Round266_component_frontier_ledger",
        "post_Round266_component_frontier_row_id",
        R266_ROOT_COMMITMENT,
    ):
        root = row["post_Round266_quotient_component_id"]
        need(
            root in retained_roots
            and root not in root_keys
            and type(row["official_key_id"]) is str,
            "Round266 root row",
        )
        root_keys[root] = row["official_key_id"]
    need(
        set(root_keys) == retained_roots
        and all(
            member.official_key_id == root_keys[member.base_root]
            for member in members.values()
        ),
        "Round266 member/root key consistency",
    )

    preserved_seen: set[str] = set()
    refined_sources: dict[str, tuple[str, str]] = {}
    new_roots: set[str] = set()
    registry_kinds: Counter[str] = Counter()
    old_keys = set(root_keys.values())
    for row in committed_gzip_rows(
        FILES["R294"][0],
        "Round294_occurrence_registry_row_id",
        R294_COMMITMENT,
        occurrence_field="registry_occurrence_id",
    ):
        occurrence = row["registry_occurrence_id"]
        kind = row["registry_entry_kind"]
        registry_kinds[kind] += 1
        if kind == PRESERVED:
            member = members.get(occurrence)
            need(
                member is not None
                and member.member_kind == "EXPANDED_OCCURRENCE"
                and occurrence not in preserved_seen
                and member.official_key_id == row["official_key_id"],
                "Round294 preserved occurrence",
            )
            preserved_seen.add(occurrence)
            continue
        need(occurrence not in members, "Round294 append-only identity")
        if kind == NEW288:
            need(type(row["official_key_id"]) is str, "Round288 key")
            key: str | None = row["official_key_id"]
            old_keys.add(row["official_key_id"])
        else:
            need(
                kind == NEW292
                and row["official_key_id"] is None
                and row["official_key_ordinal"] is None,
                "Round292 prebinding null",
            )
            key = None
            refined_sources[occurrence] = (
                row["Round294_occurrence_registry_row_id"],
                row["row_sha256"],
            )
        members[occurrence] = Member(
            occurrence,
            kind,
            occurrence,
            key,
            row["Round294_occurrence_registry_row_id"],
            row["row_sha256"],
        )
        new_roots.add(occurrence)
    need(
        len(preserved_seen) == 126_468
        and registry_kinds == {
            PRESERVED: 126_468,
            NEW288: 295_336,
            NEW292: 9_404,
        }
        and len(new_roots) == 304_740
        and len(old_keys) == 116,
        "Round294 registry census",
    )

    binding_seen: set[str] = set()
    final_keys = set(old_keys)
    for row in committed_gzip_rows(
        FILES["R299A"][0],
        "Round299A_refined_occurrence_official_key_binding_row_id",
        R299A_COMMITMENT,
    ):
        occurrence = row["registry_occurrence_id"]
        member = members.get(occurrence)
        source = refined_sources.get(occurrence)
        need(
            member is not None
            and source is not None
            and member.official_key_id is None
            and occurrence not in binding_seen
            and row["source_Round294_occurrence_registry_row_id"] == source[0]
            and row["source_Round294_occurrence_registry_row_sha256"]
            == source[1]
            and row["append_only_official_key_binding"] is True
            and row["occurrence_identity_preserved"] is True
            and row[
                "formal_refined_occurrence_official_key_binding_credit"
            ] == 1
            and row["raw_key_merge_credit"] == 0,
            "Round299A binding",
        )
        member.official_key_id = row["official_key_id"]
        binding_seen.add(occurrence)
        final_keys.add(row["official_key_id"])
    base_roots = retained_roots | new_roots
    need(
        set(refined_sources) == binding_seen
        and len(members) == 564_492
        and len(base_roots) == 367_964
        and len(final_keys) == 124
        and all(member.official_key_id is not None for member in members.values()),
        "Round301 member universe",
    )
    return members, base_roots, {
        "member_count": len(members),
        "base_root_count": len(base_roots),
        "retained_Round266_member_count": 259_752,
        "retained_Round266_root_count": len(retained_roots),
        "new_Round294_singleton_member_and_root_count": len(new_roots),
        "final_official_key_count": len(final_keys),
        "unkeyed_member_count": 0,
        "member_kind_histogram": dict(sorted(
            Counter(member.member_kind for member in members.values()).items()
        )),
    }


def collect_legal_edges(
    members: dict[str, Member],
    base_roots: set[str],
) -> tuple[list[Edge], dict[str, Any]]:
    output: list[Edge] = []
    channel_rows: dict[str, int] = {}
    channel_unique_pairs: dict[str, int] = {}

    def project(endpoint: str) -> str:
        if endpoint in base_roots:
            return endpoint
        member = members.get(endpoint)
        need(member is not None, "legal edge endpoint in member universe")
        return member.base_root

    source_ids: set[str] = set()
    for channel in LEGAL_CHANNELS:
        seen_pairs: set[tuple[str, str]] = set()
        before = len(output)
        for row in committed_gzip_rows(
            channel.ledger,
            channel.row_id_field,
            channel.commitment,
        ):
            for field, expected in channel.required_values:
                need(
                    row.get(field) == expected,
                    channel.name + ":required:" + field,
                )
            endpoints = row[channel.endpoint_field]
            source_id = row[channel.row_id_field]
            need(
                type(endpoints) is list
                and len(endpoints) == 2
                and endpoints == sorted(endpoints)
                and endpoints[0] != endpoints[1]
                and source_id not in source_ids,
                channel.name + ":canonical edge",
            )
            source_ids.add(source_id)
            seen_pairs.add(tuple(endpoints))
            output.append(Edge(
                channel.name,
                source_id,
                row["row_sha256"],
                endpoints[0],
                endpoints[1],
                project(endpoints[0]),
                project(endpoints[1]),
            ))
        channel_rows[channel.name] = len(output) - before
        channel_unique_pairs[channel.name] = len(seen_pairs)
        need(
            channel_rows[channel.name] == channel.row_count,
            channel.name + ":row census",
        )
    need(len(output) == 434_606, "legal edge census")
    return output, {
        "channel_edge_row_count": channel_rows,
        "channel_unique_endpoint_pair_count": channel_unique_pairs,
        "total_sealed_legal_edge_row_count": len(output),
    }


def canonical_partition(
    dsu: DSU,
    base_roots: set[str],
) -> tuple[dict[str, str], str, int]:
    groups: dict[str, list[str]] = defaultdict(list)
    for root in sorted(base_roots):
        groups[dsu.find(root)].append(root)
    component_root_sets = sorted(
        (sorted(values) for values in groups.values()),
        key=lambda values: values[0],
    )
    root_to_component: dict[str, str] = {}
    for values in component_root_sets:
        component = (
            "round301-legal-component:" + digest(values)
        )
        for root in values:
            root_to_component[root] = component
    return root_to_component, digest(component_root_sets), len(component_root_sets)


def apply_edges(
    edges: Iterable[Edge],
    base_roots: set[str],
) -> tuple[dict[str, str], dict[str, Any]]:
    dsu = DSU(base_roots)
    per_channel: dict[str, Counter[str]] = defaultdict(Counter)
    total = 0
    for edge in edges:
        total += 1
        counter = per_channel[edge.channel]
        counter["edge_rows"] += 1
        if edge.left_base_root == edge.right_base_root:
            counter["same_base_root_before_application"] += 1
            counter["cycle_or_redundant"] += 1
        elif dsu.union(edge.left_base_root, edge.right_base_root):
            counter["rank_reduction"] += 1
        else:
            counter["cycle_or_redundant"] += 1
    root_to_component, partition_pin, component_count = canonical_partition(
        dsu, base_roots
    )
    rank = sum(value["rank_reduction"] for value in per_channel.values())
    need(rank == len(base_roots) - component_count, "DSU rank-nullity")
    return root_to_component, {
        "edge_rows_processed": total,
        "rank_reduction": rank,
        "component_count": component_count,
        "partition_sha256": partition_pin,
        "per_channel_application": {
            name: dict(sorted(value.items()))
            for name, value in sorted(per_channel.items())
        },
    }


def iter_edge_application_rows(
    edges: list[Edge],
    base_roots: set[str],
    final_root_to_component: dict[str, str],
) -> Iterator[dict[str, Any]]:
    dsu = DSU(base_roots)
    for ordinal, edge in enumerate(edges):
        left_before = dsu.find(edge.left_base_root)
        right_before = dsu.find(edge.right_base_root)
        rank_reduction = left_before != right_before
        if rank_reduction:
            need(
                dsu.union(edge.left_base_root, edge.right_base_root),
                "edge union",
            )
        final_component = final_root_to_component[edge.left_base_root]
        need(
            final_component
            == final_root_to_component[edge.right_base_root],
            "edge final component",
        )
        yield close_row({
            "Round301_legal_component_edge_application_row_id":
                "round301-legal-component-edge-application:"
                + digest([edge.channel, edge.source_row_id]),
            "application_ordinal": ordinal,
            "canonical_projected_base_root_pair": sorted([
                edge.left_base_root,
                edge.right_base_root,
            ]),
            "cycle_or_redundant_after_prior_legal_edges": not rank_reduction,
            "eligible_for_component_DSU_application": True,
            "final_Round301_component_id": final_component,
            "formal_DSU_rank_reduction_credit": 1 if rank_reduction else 0,
            "formal_component_edge_application_credit": 1,
            "formal_component_union_credit": 1 if rank_reduction else 0,
            "formal_occurrence_identity_collapse_credit": 0,
            "pre_application_DSU_representative_pair": sorted([
                left_before,
                right_before,
            ]),
            "rank_reduction_applied": rank_reduction,
            "same_pre_application_DSU_component": not rank_reduction,
            "source_canonical_endpoint_pair": [
                edge.left_endpoint,
                edge.right_endpoint,
            ],
            "source_channel": edge.channel,
            "source_edge_row_id": edge.source_row_id,
            "source_edge_row_sha256": edge.source_row_sha256,
            "source_occurrence_identities_preserved": True,
        })


def iter_member_component_rows(
    members: dict[str, Member],
    final_root_to_component: dict[str, str],
) -> Iterator[dict[str, Any]]:
    for member_id in sorted(members):
        member = members[member_id]
        yield close_row({
            "Round301_member_to_component_row_id":
                "round301-member-to-component:" + digest(member_id),
            "base_component_root_id": member.base_root,
            "final_Round301_component_id":
                final_root_to_component[member.base_root],
            "formal_component_membership_credit": 1,
            "formal_occurrence_identity_collapse_credit": 0,
            "member_identity_preserved": True,
            "member_kind": member.member_kind,
            "official_key_id": member.official_key_id,
            "registry_or_frontier_member_id": member.member_id,
            "source_member_row_id": member.source_row_id,
            "source_member_row_sha256": member.source_row_sha256,
        })


def build_component_key_incidence(
    members: dict[str, Member],
    base_roots: set[str],
    final_root_to_component: dict[str, str],
) -> tuple[dict[tuple[str, str], Counter[str]], dict[str, int]]:
    root_keys: dict[str, str] = {}
    incidence: dict[tuple[str, str], Counter[str]] = defaultdict(Counter)
    for member in members.values():
        key = member.official_key_id
        need(type(key) is str, "member key")
        prior = root_keys.setdefault(member.base_root, key)
        need(prior == key, "base-root key purity")
        incidence[
            (final_root_to_component[member.base_root], key)
        ]["member_count"] += 1
    need(set(root_keys) == base_roots, "key for every root")
    for root in base_roots:
        incidence[
            (final_root_to_component[root], root_keys[root])
        ]["base_root_count"] += 1
    multiplicity: Counter[str] = Counter()
    for component, _key in incidence:
        multiplicity[component] += 1
    return incidence, dict(multiplicity)


def iter_component_key_incidence_rows(
    incidence: dict[tuple[str, str], Counter[str]],
    component_key_multiplicity: dict[str, int],
) -> Iterator[dict[str, Any]]:
    for component, key in sorted(incidence):
        counts = incidence[(component, key)]
        yield close_row({
            "Round301_component_key_incidence_row_id":
                "round301-component-key-incidence:"
                + digest([component, key]),
            "final_Round301_component_id": component,
            "formal_official_key_merge_credit": 0,
            "member_count": counts["member_count"],
            "official_key_id": key,
            "official_key_identity_preserved": True,
            "official_key_multiplicity_in_component":
                component_key_multiplicity[component],
            "pre_application_base_root_count": counts["base_root_count"],
        })


def load_R300G_coverage() -> tuple[
    dict[tuple[str, str], list[dict[str, Any]]], dict[str, Any]
]:
    references: dict[tuple[str, str], list[dict[str, Any]]] = {}
    for row in committed_gzip_rows(
        R300G_LEDGER,
        "Round300G_explicit_outgoing_seam_exclusion_row_id",
        R300G_COMMITMENT,
    ):
        pair = tuple(
            row["canonical_unordered_Round294_registry_occurrence_ids"]
        )
        need(
            len(pair) == 2
            and pair[0] < pair[1]
            and pair not in references
            and row["attachment_exclusion_reason"]
            == (
                "NO_ROUND220_ANCESTOR_INTERFACE__"
                "NO_STRICT_VOLUME_INTERSECTION__"
                "CLOSED_CONTACT_INSUFFICIENT"
            )
            and row["included_one_sided_owner_attachment_proved"] is False
            and row["eligible_for_component_DSU_application"] is False,
            "R300G exact exclusion",
        )
        references[pair] = [{
            "gate": "R300G",
            "provenance_kind": "AUDITED_FAIL_CLOSED_NO_EDGE",
            "gate_evidence_row_id":
                row["Round300G_explicit_outgoing_seam_exclusion_row_id"],
            "gate_evidence_row_sha256": row["row_sha256"],
            "source_R300A_canonical_pair_row_id":
                row["source_Round300A_canonical_occurrence_pair_row_id"],
            "source_R300A_canonical_pair_row_sha256":
                row["source_Round300A_canonical_occurrence_pair_row_sha256"],
            "source_R295A_physical_incidence_binding_row_id":
                row["source_Round295A_physical_incidence_binding_row_id"],
            "source_R295A_physical_incidence_binding_row_sha256":
                row["source_Round295A_physical_incidence_binding_row_sha256"],
            "attachment_exclusion_reason": row["attachment_exclusion_reason"],
        }]
    need(len(references) == 128, "R300G coverage")
    return references, {
        "exact_pair_count": 128,
        "eligible_edge_row_count": 0,
        "disposition": "AUDITED_FAIL_CLOSED_NO_EDGE",
    }


def load_R300H_coverage() -> tuple[
    dict[tuple[str, str], list[dict[str, Any]]],
    dict[str, list[dict[str, Any]]],
    dict[str, Any],
]:
    pair_references: dict[
        tuple[str, str], list[dict[str, Any]]
    ] = {}
    pair_tranches: Counter[str] = Counter()
    pair_source_rows: set[str] = set()
    for row in committed_gzip_rows(
        R300H_PAIR_LEDGER,
        "Round300H_enriched_pair_disposition_row_id",
        R300H_PAIR_COMMITMENT,
    ):
        pair = tuple(
            row["canonical_unordered_Round294_registry_occurrence_ids"]
        )
        tranche = row["disposition_tranche"]
        source_id = row["source_Round300D_pair_row_id"]
        need(
            len(pair) == 2
            and pair[0] < pair[1]
            and pair not in pair_references
            and source_id not in pair_source_rows
            and tranche in {
                "GRAPH_TRANS_ANALYTIC_BOUNDARY_ONLY",
                "OWNER_POLICY_INCIDENCE_ONLY",
            }
            and row["eligible_for_component_DSU_application"] is False
            and row["included_stratum_two_attachment_lemma_pinned"] is False,
            "R300H pair exclusion",
        )
        pair_source_rows.add(source_id)
        pair_tranches[tranche] += 1
        pair_references[pair] = [{
            "gate": "R300H",
            "provenance_kind": "AUDITED_FAIL_CLOSED_NO_EDGE",
            "gate_evidence_row_id":
                row["Round300H_enriched_pair_disposition_row_id"],
            "gate_evidence_row_sha256": row["row_sha256"],
            "source_Round300D_pair_row_id": source_id,
            "source_Round300D_pair_row_sha256":
                row["source_Round300D_pair_row_sha256"],
            "disposition_tranche": tranche,
            "formal_disposition": row["formal_disposition"],
        }]
    need(
        len(pair_references) == 144
        and pair_tranches == {
            "GRAPH_TRANS_ANALYTIC_BOUNDARY_ONLY": 56,
            "OWNER_POLICY_INCIDENCE_ONLY": 88,
        },
        "R300H pair coverage",
    )

    single_references: dict[str, list[dict[str, Any]]] = {}
    single_dispositions: Counter[str] = Counter()
    for row in committed_gzip_rows(
        R300H_SINGLE_LEDGER,
        "Round300H_single_target_disposition_row_id",
        R300H_SINGLE_COMMITMENT,
    ):
        source_id = row["source_Round300D_single_row_id"]
        disposition = row["formal_disposition"]
        preserved = row["preserved_Round266_provenance"]
        need(
            source_id not in single_references
            and row["canonical_two_target_incidence_edge_issued"] is False
            and row["eligible_for_component_DSU_application"] is False
            and disposition in {
                "RECLOSED_EXISTING_ROUND266_OCCURRENCE_ROOT__ZERO_NEW_EDGE",
                (
                    "EXCLUDED__UNIQUE_TARGET_ASSIGNMENT_HAS_NO_FORMAL_"
                    "OWNER_ROOT_ATTACHMENT_LINEAGE"
                ),
            },
            "R300H single exclusion",
        )
        single_dispositions[disposition] += 1
        single_references[source_id] = [{
            "gate": "R300H",
            "provenance_kind": "AUDITED_FAIL_CLOSED_NO_EDGE",
            "gate_evidence_row_id":
                row["Round300H_single_target_disposition_row_id"],
            "gate_evidence_row_sha256": row["row_sha256"],
            "source_Round300D_single_row_id": source_id,
            "source_Round300D_single_row_sha256":
                row["source_Round300D_single_row_sha256"],
            "target_Round294_registry_entry_kind":
                row["target_Round294_registry_entry_kind"],
            "target_Round294_registry_occurrence_id":
                row["target_Round294_registry_occurrence_id"],
            "witness_kind": row["witness_kind"],
            "formal_disposition": disposition,
            "preserved_Round266_provenance": preserved,
        }]
    need(
        len(single_references) == 1_600
        and single_dispositions == {
            "RECLOSED_EXISTING_ROUND266_OCCURRENCE_ROOT__ZERO_NEW_EDGE":
                1_408,
            (
                "EXCLUDED__UNIQUE_TARGET_ASSIGNMENT_HAS_NO_FORMAL_"
                "OWNER_ROOT_ATTACHMENT_LINEAGE"
            ): 192,
        },
        "R300H single coverage",
    )
    need(
        not list(committed_gzip_rows(
            R300H_ELIGIBLE_LEDGER,
            "Round300H_eligible_component_edge_row_id",
            R300H_EMPTY_EDGE_COMMITMENT,
        )),
        "R300H zero edge ledger",
    )
    census = {
        "exact_pair_count": len(pair_references),
        "pair_disposition_tranche_histogram":
            dict(sorted(pair_tranches.items())),
        "exact_single_target_count": len(single_references),
        "single_formal_disposition_histogram":
            dict(sorted(single_dispositions.items())),
        "eligible_edge_row_count": 0,
    }
    return pair_references, single_references, census


def exact_stronger_edge_provenance() -> dict[
    tuple[str, str], list[dict[str, Any]]
]:
    channels = {channel.name: channel for channel in LEGAL_CHANNELS}
    b_channel = channels["R300B_COMPLETE_FACE"]
    e_channel = channels["R300E_HALF_OPEN_OWNER"]
    f_channel = channels["R300F_R245_HALF_OPEN_OWNER"]
    e_edge_by_witness: dict[str, dict[str, str]] = {}
    for row in committed_gzip_rows(
        e_channel.ledger,
        e_channel.row_id_field,
        e_channel.commitment,
    ):
        witness = row["source_half_open_owner_witness_row_id"]
        need(witness not in e_edge_by_witness, "R300E witness edge")
        e_edge_by_witness[witness] = {
            "legal_edge_channel": e_channel.name,
            "legal_edge_source_row_id": row[e_channel.row_id_field],
            "legal_edge_source_row_sha256": row["row_sha256"],
        }

    output: dict[
        tuple[str, str], list[dict[str, Any]]
    ] = defaultdict(list)
    for row in committed_gzip_rows(
        b_channel.ledger,
        b_channel.row_id_field,
        b_channel.commitment,
    ):
        pair = tuple(row[b_channel.endpoint_field])
        output[pair].append({
            "gate": "R300B",
            "provenance_kind": "RECLOSED_BY_STRONGER_LEGAL_EDGE",
            "gate_evidence_row_id": row[b_channel.row_id_field],
            "gate_evidence_row_sha256": row["row_sha256"],
            "legal_edge_channel": b_channel.name,
            "legal_edge_source_row_id": row[b_channel.row_id_field],
            "legal_edge_source_row_sha256": row["row_sha256"],
        })

    seen_witnesses: set[str] = set()
    for row in committed_gzip_rows(
        R300E_WITNESS_LEDGER,
        "Round300E_half_open_owner_component_edge_witness_row_id",
        R300E_WITNESS_COMMITMENT,
    ):
        witness = row[
            "Round300E_half_open_owner_component_edge_witness_row_id"
        ]
        edge = e_edge_by_witness.get(witness)
        need(
            edge is not None and witness not in seen_witnesses,
            "R300E witness bijection",
        )
        seen_witnesses.add(witness)
        pair = tuple(sorted((
            row["owner_Round294_registry_occurrence_id"],
            row["excluded_present_side_Round294_registry_occurrence_id"],
        )))
        output[pair].append({
            "gate": "R300E",
            "provenance_kind": "RECLOSED_BY_STRONGER_LEGAL_EDGE",
            "gate_evidence_row_id": witness,
            "gate_evidence_row_sha256": row["row_sha256"],
            **edge,
        })
    need(seen_witnesses == set(e_edge_by_witness), "R300E full bijection")

    for row in committed_gzip_rows(
        f_channel.ledger,
        f_channel.row_id_field,
        f_channel.commitment,
    ):
        pair = tuple(sorted((
            row["Round294_owner_occurrence_id"],
            row["Round294_shadow_occurrence_id"],
        )))
        output[pair].append({
            "gate": "R300F",
            "provenance_kind": "RECLOSED_BY_STRONGER_LEGAL_EDGE",
            "gate_evidence_row_id": row[f_channel.row_id_field],
            "gate_evidence_row_sha256": row["row_sha256"],
            "legal_edge_channel": f_channel.name,
            "legal_edge_source_row_id": row[f_channel.row_id_field],
            "legal_edge_source_row_sha256": row["row_sha256"],
        })
    for values in output.values():
        values.sort(key=lambda value: (
            value["gate"], value["gate_evidence_row_id"]
        ))
    return output


def complete_gate_provenance() -> tuple[
    dict[tuple[str, str], list[dict[str, Any]]],
    dict[str, list[dict[str, Any]]],
    dict[str, Any],
]:
    pairs = exact_stronger_edge_provenance()
    singles: dict[str, list[dict[str, Any]]] = {}
    g_pairs, g_census = load_R300G_coverage()
    h_pairs, h_singles, h_census = load_R300H_coverage()
    for mapping in (g_pairs, h_pairs):
        for pair, references in mapping.items():
            pairs[pair].extend(references)
    singles.update(h_singles)
    for references in pairs.values():
        references.sort(key=lambda value: (
            value["gate"], value["gate_evidence_row_id"]
        ))
    return pairs, singles, {
        "R300G": g_census,
        "R300H": h_census,
    }


def exact_R300A_pair_set() -> set[tuple[str, str]]:
    output: set[tuple[str, str]] = set()
    for row in committed_gzip_rows(
        R300A_LEDGER,
        "Round300A_canonical_occurrence_pair_row_id",
        R300A_CANONICAL_PAIR_COMMITMENT,
        table="canonical_occurrence_pair_rows",
    ):
        pair = tuple(
            row["canonical_unordered_Round294_registry_occurrence_ids"]
        )
        need(pair not in output, "R300A pair uniqueness")
        output.add(pair)
    return output


def iter_pinned_R300B_faces() -> Iterator[dict[str, Any]]:
    path = HERE / R300B_FACE_INVENTORY
    guard_exact_regular_path(path, R300B_FACE_INVENTORY)
    need(
        file_sha256(path) == R300B_FACE_INVENTORY_PIN,
        "R300B face pin",
    )
    seen: set[str] = set()
    for row in iter_gzip_rows(path):
        row_id = row.get("Round300B_face_inventory_row_id")
        need(
            type(row_id) is str
            and row_id not in seen
            and PIN_RE.fullmatch(row.get("row_sha256", "")) is not None,
            "R300B face boundary",
        )
        seen.add(row_id)
        yield row
    need(len(seen) == 62_548, "R300B face count")


def exact_R300B_face_coverage(
    target_pairs: set[tuple[str, str]],
) -> tuple[
    dict[tuple[str, str], list[dict[str, Any]]],
    dict[tuple[str, str], list[dict[str, Any]]],
]:
    b_channel = next(
        channel
        for channel in LEGAL_CHANNELS
        if channel.name == "R300B_COMPLETE_FACE"
    )
    validate_manifest_selection(
        b_channel.manifest,
        b_channel.manifest_sha256,
        {R300B_FACE_INVENTORY: R300B_FACE_INVENTORY_PIN},
    )
    accepted: dict[
        tuple[str, str], list[dict[str, Any]]
    ] = defaultdict(list)
    rejected: dict[
        tuple[str, str], list[dict[str, Any]]
    ] = defaultdict(list)
    for row in iter_pinned_R300B_faces():
        decision = row["decision"]
        left = row["left_formal_occurrence_endpoints"]
        right = row["right_formal_occurrence_endpoints"]
        pairs: set[tuple[str, str]] = set()
        if decision.startswith("ACCEPT_"):
            for first in left:
                for second in right:
                    if first != second:
                        pairs.add(tuple(sorted((first, second))))
            destination = accepted
            kind = (
                "FACE_ACCEPTED_WITH_EXACT_POSITIVE_PATCH_AND_TWO_CORRIDORS"
            )
        else:
            need(decision.startswith("REJECT_"), "R300B face decision")
            explicit = row[
                "unordered_nonself_formal_occurrence_endpoint_pair"
            ]
            if explicit is not None:
                pairs.add(tuple(explicit))
            destination = rejected
            kind = "AUDITED_FAIL_CLOSED_NO_EDGE"
        for pair in pairs & target_pairs:
            destination[pair].append({
                "gate": "R300B_FACE_INVENTORY",
                "provenance_kind": kind,
                "gate_evidence_row_id":
                    row["Round300B_face_inventory_row_id"],
                "gate_evidence_row_sha256": row["row_sha256"],
                "face_decision": decision,
            })
    for mapping in (accepted, rejected):
        for references in mapping.values():
            references.sort(key=lambda value: value["gate_evidence_row_id"])
    return dict(accepted), dict(rejected)


def iter_ineligible_source_consumption_rows(
    members: dict[str, Member],
    base_roots: set[str],
    exact_provenance: dict[
        tuple[str, str], list[dict[str, Any]]
    ],
    single_provenance: dict[str, list[dict[str, Any]]],
    face_accepted: dict[tuple[str, str], list[dict[str, Any]]],
    face_rejected: dict[tuple[str, str], list[dict[str, Any]]],
) -> Iterator[dict[str, Any]]:
    def project(endpoint: str) -> str:
        if endpoint in base_roots:
            return endpoint
        member = members.get(endpoint)
        need(member is not None, "ineligible endpoint in member universe")
        return member.base_root

    def pair_projection(pair: list[str]) -> list[str]:
        return sorted((project(pair[0]), project(pair[1])))

    for row in committed_gzip_rows(
        R300D_LEDGER,
        "Round300D_lower_physical_witness_incidence_edge_row_id",
        R300D_INCIDENCE_COMMITMENT,
        table="canonical_incidence_edge_rows",
    ):
        pair = row["canonical_unordered_Round294_registry_occurrence_ids"]
        provenance = exact_provenance.get(tuple(pair), [])
        gates = sorted({reference["gate"] for reference in provenance})
        stronger = [
            {
                "legal_edge_channel": reference["legal_edge_channel"],
                "legal_edge_source_row_id":
                    reference["legal_edge_source_row_id"],
                "legal_edge_source_row_sha256":
                    reference["legal_edge_source_row_sha256"],
            }
            for reference in provenance
            if reference["provenance_kind"]
            == "RECLOSED_BY_STRONGER_LEGAL_EDGE"
        ]
        if stronger:
            disposition = (
                "INELIGIBLE_SOURCE_ROW_NOT_CONSUMED__"
                "PAIR_RECLOSED_BY_STRONGER_LEGAL_EDGE"
            )
        elif provenance:
            disposition = (
                "INELIGIBLE_SOURCE_ROW_NOT_CONSUMED__"
                "AUDITED_FAIL_CLOSED_NO_EDGE"
            )
        else:
            disposition = (
                "INELIGIBLE_INCIDENCE_ONLY__NO_STRONGER_GATE_PROOF"
            )
        yield close_row({
            "Round301_ineligible_source_consumption_row_id":
                "round301-ineligible-source-consumption:"
                + digest([
                    "R300D_CANONICAL_INCIDENCE_PAIR",
                    row[
                        "Round300D_lower_physical_witness_incidence_edge_row_id"
                    ],
                ]),
            "canonical_occurrence_endpoint_pair": pair,
            "canonical_projected_base_root_pair": pair_projection(pair),
            "disposition": disposition,
            "eligible_for_component_DSU_application": False,
            "formal_DSU_rank_reduction_credit": 0,
            "formal_component_edge_application_credit": 0,
            "formal_component_union_credit": 0,
            "formal_occurrence_identity_collapse_credit": 0,
            "legal_stronger_edge_references": stronger,
            "exact_gate_provenance_references": provenance,
            "source_relation":
                "R300D_CANONICAL_TWO_TARGET_LOWER_PHYSICAL_WITNESS_"
                "INCIDENCE_EDGE",
            "source_row_fed_to_DSU": False,
            "source_row_id":
                row[
                    "Round300D_lower_physical_witness_incidence_edge_row_id"
                ],
            "source_row_sha256": row["row_sha256"],
            "stronger_gate_names": gates,
        })

    for row in committed_gzip_rows(
        R300D_LEDGER,
        "Round300D_single_target_assignment_exclusion_row_id",
        R300D_SINGLE_COMMITMENT,
        table="single_target_assignment_exclusion_rows",
    ):
        target_record = row["target_Round294_registry_occurrence"]
        target = target_record["registry_occurrence_id"]
        source_id = row[
            "Round300D_single_target_assignment_exclusion_row_id"
        ]
        later = single_provenance.get(source_id, [])
        yield close_row({
            "Round301_ineligible_source_consumption_row_id":
                "round301-ineligible-source-consumption:"
                + digest(["R300D_SINGLE_TARGET_ASSIGNMENT", source_id]),
            "canonical_occurrence_endpoint_pair": None,
            "canonical_projected_base_root_pair": None,
            "disposition":
                "INELIGIBLE_SINGLE_TARGET_ASSIGNMENT__NOT_AN_EDGE",
            "eligible_for_component_DSU_application": False,
            "formal_DSU_rank_reduction_credit": 0,
            "formal_component_edge_application_credit": 0,
            "formal_component_union_credit": 0,
            "formal_occurrence_identity_collapse_credit": 0,
            "legal_stronger_edge_references": [],
            "exact_gate_provenance_references": later,
            "single_target_occurrence_id": target,
            "single_target_projected_base_root": project(target),
            "single_target_registry_entry_kind":
                target_record["registry_entry_kind"],
            "single_target_Round294_registry_row_id":
                target_record["Round294_occurrence_registry_row_id"],
            "single_target_Round294_registry_row_sha256":
                target_record["Round294_occurrence_registry_row_sha256"],
            "source_relation": "R300D_SINGLE_TARGET_ASSIGNMENT",
            "source_row_fed_to_DSU": False,
            "source_row_id": source_id,
            "source_row_sha256": row["row_sha256"],
            "stronger_gate_names": sorted({
                reference["gate"] for reference in later
            }),
            "witness_kind": row["witness_kind"],
        })

    for row in committed_gzip_rows(
        R300A_LEDGER,
        "Round300A_canonical_occurrence_pair_row_id",
        R300A_CANONICAL_PAIR_COMMITMENT,
        table="canonical_occurrence_pair_rows",
    ):
        pair = row["canonical_unordered_Round294_registry_occurrence_ids"]
        provenance = exact_provenance.get(tuple(pair), [])
        gates = sorted({reference["gate"] for reference in provenance})
        stronger = [
            {
                "legal_edge_channel": reference["legal_edge_channel"],
                "legal_edge_source_row_id":
                    reference["legal_edge_source_row_id"],
                "legal_edge_source_row_sha256":
                    reference["legal_edge_source_row_sha256"],
            }
            for reference in provenance
            if reference["provenance_kind"]
            == "RECLOSED_BY_STRONGER_LEGAL_EDGE"
        ]
        accepted = face_accepted.get(tuple(pair), [])
        rejected = face_rejected.get(tuple(pair), [])
        provenance_gates = {reference["gate"] for reference in provenance}
        if "R300B" in provenance_gates:
            disposition = "RECLOSED_BY_R300B_STRONGER_LEGAL_EDGE"
        elif rejected:
            disposition = (
                "R300B_REJECTED_FACE__AUDITED_FAIL_CLOSED_NO_EDGE"
            )
        elif "R300G" in provenance_gates:
            disposition = "R300G_AUDITED_FAIL_CLOSED_NO_EDGE"
        else:
            disposition = (
                "R300B_COMPLETE_INVENTORY__NO_ENDPOINT_FACE__"
                "ROUND302_REOPEN"
            )
        yield close_row({
            "Round301_ineligible_source_consumption_row_id":
                "round301-ineligible-source-consumption:"
                + digest([
                    "R300A_CANONICAL_CLOSURE_PAIR",
                    row["Round300A_canonical_occurrence_pair_row_id"],
                ]),
            "canonical_occurrence_endpoint_pair": pair,
            "canonical_projected_base_root_pair": pair_projection(pair),
            "disposition": disposition,
            "eligible_for_component_DSU_application": False,
            "formal_DSU_rank_reduction_credit": 0,
            "formal_component_edge_application_credit": 0,
            "formal_component_union_credit": 0,
            "formal_occurrence_identity_collapse_credit": 0,
            "legal_stronger_edge_references": stronger,
            "exact_gate_provenance_references": provenance,
            "R300B_complete_face_inventory_references":
                accepted + rejected,
            "R300B_complete_face_inventory_rows_sha256":
                R300B_FACE_INVENTORY_COMMITMENT["rows_sha256"],
            "R300B_endpoint_face_present": bool(accepted or rejected),
            "R300B_no_endpoint_face_negative_set_difference_sha256":
                None if accepted or rejected
                else digest([
                    pair,
                    R300B_FACE_INVENTORY_COMMITMENT["rows_sha256"],
                    "NO_ENDPOINT_FACE_IN_COMPLETE_INVENTORY",
                ]),
            "prior_R295A_explicit_lower_graph_witness_present":
                row["R295A_explicit_lower_graph_sheet_witness_present"],
            "source_relation":
                "R300A_REGULAR_GRAPH_ZERO_CLOSURE_CONTACT",
            "source_row_fed_to_DSU": False,
            "source_row_id":
                row["Round300A_canonical_occurrence_pair_row_id"],
            "source_row_sha256": row["row_sha256"],
            "stronger_gate_names": gates,
        })

    for row in committed_gzip_rows(
        R300A_LEDGER,
        "Round300A_R287_source_pair_expansion_row_id",
        R300A_SOURCE_EXPANSION_COMMITMENT,
        table="source_pair_expansion_rows",
    ):
        yield close_row({
            "Round301_ineligible_source_consumption_row_id":
                "round301-ineligible-source-consumption:"
                + digest([
                    "R300A_SOURCE_PAIR_EXPANSION",
                    row["Round300A_R287_source_pair_expansion_row_id"],
                ]),
            "canonical_occurrence_endpoint_pair": None,
            "canonical_projected_base_root_pair": None,
            "canonical_pair_reference_count":
                len(row["canonical_occurrence_pair_row_ids"]),
            "canonical_pair_reference_row_ids":
                row["canonical_occurrence_pair_row_ids"],
            "disposition":
                "INELIGIBLE_SOURCE_CROSS_PRODUCT_OR_CLOSURE_CONTACT__"
                "CANONICAL_DERIVATIONS_TRACKED_SEPARATELY",
            "eligible_for_component_DSU_application": False,
            "formal_DSU_rank_reduction_credit": 0,
            "formal_component_edge_application_credit": 0,
            "formal_component_union_credit": 0,
            "formal_occurrence_identity_collapse_credit": 0,
            "legal_stronger_edge_references": [],
            "exact_gate_provenance_references": [],
            "source_relation": "R300A_R287_SOURCE_PAIR_EXPANSION",
            "source_row_fed_to_DSU": False,
            "source_row_id":
                row["Round300A_R287_source_pair_expansion_row_id"],
            "source_row_sha256": row["row_sha256"],
            "stronger_gate_names": [],
        })


def reconstruct_ineligible_census(
    factory: Callable[[], Iterator[dict[str, Any]]],
    edges: list[Edge],
    gate_census: dict[str, Any],
) -> dict[str, Any]:
    d_disposition: Counter[str] = Counter()
    d_single_kinds: Counter[str] = Counter()
    d_single_coverage: Counter[str] = Counter()
    a_stronger: Counter[str] = Counter()
    a_prior_channels: Counter[str] = Counter()
    a_face: Counter[str] = Counter()
    a_primary: Counter[str] = Counter()
    a_prior = 0
    a_unresolved = 0
    expansion_count = 0
    expansion_references = 0
    prior_sets: dict[str, set[tuple[str, str]]] = {
        name: set()
        for name in (
            "R296_TRUE_SEAM",
            "R297_ORDINARY_FACE",
            "R299C_SIGNED_FACE",
            "R300C_POSITIVE_VOLUME",
        )
    }
    for edge in edges:
        if edge.channel in prior_sets:
            prior_sets[edge.channel].add(
                (edge.left_endpoint, edge.right_endpoint)
            )

    row_count = 0
    for row in factory():
        verify_row(
            row,
            "Round301_ineligible_source_consumption_row_id",
        )
        row_count += 1
        relation = row["source_relation"]
        if relation.startswith("R300D_CANONICAL_"):
            references = row["exact_gate_provenance_references"]
            gates = sorted({item["gate"] for item in references})
            if not gates:
                label = "NO_STRONGER_GATE_PROOF__INCIDENCE_ONLY"
            elif len(gates) == 1:
                kinds = sorted({
                    item["provenance_kind"] for item in references
                })
                need(len(kinds) == 1, "one gate disposition")
                if kinds[0] == "RECLOSED_BY_STRONGER_LEGAL_EDGE":
                    label = (
                        gates[0]
                        + "_STRONGER_PROOF__R300D_ROW_STILL_INELIGIBLE"
                    )
                else:
                    label = (
                        gates[0] + "_" + kinds[0]
                        + "__R300D_ROW_STILL_INELIGIBLE"
                    )
            else:
                label = "MULTIPLE_STRONGER_GATE_PROOFS"
            d_disposition[label] += 1
        elif relation == "R300D_SINGLE_TARGET_ASSIGNMENT":
            d_single_kinds[row["witness_kind"]] += 1
            gates = row["stronger_gate_names"]
            d_single_coverage[
                "+".join(gates) if gates else "NO_LATER_GATE_AUDIT"
            ] += 1
        elif relation == "R300A_REGULAR_GRAPH_ZERO_CLOSURE_CONTACT":
            if row["prior_R295A_explicit_lower_graph_witness_present"]:
                a_prior += 1
            else:
                a_unresolved += 1
            references = row["exact_gate_provenance_references"]
            stronger = sorted({
                item["gate"] + ":" + item["provenance_kind"]
                for item in references
            })
            a_stronger[
                "+".join(stronger) if stronger
                else "NO_STRONGER_GATE_PROOF"
            ] += 1
            pair = tuple(row["canonical_occurrence_endpoint_pair"])
            for channel, pairs in prior_sets.items():
                if pair in pairs:
                    a_prior_channels[channel] += 1
            face_references = row[
                "R300B_complete_face_inventory_references"
            ]
            if any(
                item["provenance_kind"].startswith("FACE_ACCEPTED_")
                for item in face_references
            ):
                a_face["R300B_ACCEPTED_FACE"] += 1
            elif face_references:
                a_face[
                    "R300B_REJECTED_FACE__AUDITED_FAIL_CLOSED_NO_EDGE"
                ] += 1
            else:
                a_face[
                    "R300B_COMPLETE_INVENTORY__NO_ENDPOINT_FACE"
                ] += 1
            a_primary[row["disposition"]] += 1
        else:
            need(
                relation == "R300A_R287_SOURCE_PAIR_EXPANSION",
                "known ineligible relation",
            )
            expansion_count += 1
            expansion_references += row["canonical_pair_reference_count"]
    need(
        row_count == 119_844
        and sum(d_disposition.values()) == 111_524
        and sum(d_single_kinds.values()) == 1_600
        and a_prior == 128
        and a_unresolved == 3_104
        and expansion_count == 3_488
        and not a_prior_channels,
        "ineligible source census",
    )
    need(
        d_disposition[
            "R300E_STRONGER_PROOF__R300D_ROW_STILL_INELIGIBLE"
        ] == 472
        and d_disposition[
            "R300F_STRONGER_PROOF__R300D_ROW_STILL_INELIGIBLE"
        ] == 264
        and d_disposition[
            "R300G_AUDITED_FAIL_CLOSED_NO_EDGE__"
            "R300D_ROW_STILL_INELIGIBLE"
        ] == 128
        and sum(
            count
            for label, count in d_disposition.items()
            if label.startswith("R300H_")
        ) == 144
        and d_disposition[
            "NO_STRONGER_GATE_PROOF__INCIDENCE_ONLY"
        ] == 110_516
        and d_single_coverage == {"R300H": 1_600},
        "final R300D partition",
    )
    return {
        "R300D_canonical_incidence_pair_count": 111_524,
        "R300D_pair_source_consumption_disposition":
            dict(sorted(d_disposition.items())),
        "R300D_single_target_assignment_exclusion_count": 1_600,
        "R300D_single_target_witness_kind_histogram":
            dict(sorted(d_single_kinds.items())),
        "R300D_single_target_later_gate_coverage":
            dict(sorted(d_single_coverage.items())),
        "R300A_canonical_closure_pair_count": 3_232,
        "R300A_prior_explicit_lower_witness_pair_count": a_prior,
        "R300A_unresolved_zero_trace_pair_count": a_unresolved,
        "R300A_pair_stronger_gate_projection":
            dict(sorted(a_stronger.items())),
        "R300A_other_prior_channel_exact_pair_intersections":
            dict(sorted(a_prior_channels.items())),
        "R300A_complete_R300B_face_disposition":
            dict(sorted(a_face.items())),
        "R300A_primary_source_consumption_disposition":
            dict(sorted(a_primary.items())),
        "R300A_source_pair_expansion_count": expansion_count,
        "R300A_source_expansion_to_canonical_reference_count":
            expansion_references,
        "ineligible_source_rows_fed_to_DSU": 0,
        "registered_gate_coverage_census": gate_census,
    }


def measure_generated_rows(
    factory: Callable[[], Iterator[dict[str, Any]]],
    id_field: str,
    sample: list[dict[str, Any]] | None = None,
) -> dict[str, Any]:
    rows = ListHash()
    ids = ListHash()
    hashes = ListHash()
    seen: set[str] = set()
    for row in factory():
        verify_row(row, id_field)
        row_id = row.get(id_field)
        need(
            type(row_id) is str and row_id not in seen,
            "generated row id:" + id_field,
        )
        seen.add(row_id)
        rows.add(row)
        ids.add(row_id)
        hashes.add(row["row_sha256"])
        if sample is not None and not sample:
            sample.append(copy.deepcopy(row))
    return {
        "row_count": rows.count,
        "row_ids_sha256": ids.finish(),
        "row_hashes_sha256": hashes.finish(),
        "rows_sha256": rows.finish(),
    }


def write_expected_ledger(
    directory: Path,
    name: str,
    schema: str,
    status: str,
    id_field: str,
    factory: Callable[[], Iterator[dict[str, Any]]],
    sample: list[dict[str, Any]],
) -> dict[str, Any]:
    commitment = measure_generated_rows(factory, id_field, sample)
    path = directory / name
    with path.open("xb") as raw:
        with gzip.GzipFile(
            filename="",
            mode="wb",
            fileobj=raw,
            compresslevel=9,
            mtime=0,
        ) as stream:
            stream.write(
                b'{"every_row_closed_by_own_SHA256":true,'
                + b'"row_count":'
                + canonical(commitment["row_count"])
                + b',"row_hashes_sha256":'
                + canonical(commitment["row_hashes_sha256"])
                + b',"row_ids_sha256":'
                + canonical(commitment["row_ids_sha256"])
                + b',"rows":['
            )
            second_rows = ListHash()
            second_ids = ListHash()
            second_hashes = ListHash()
            count = 0
            for row in factory():
                verify_row(row, id_field)
                if count:
                    stream.write(b",")
                stream.write(canonical(row))
                second_rows.add(row)
                second_ids.add(row[id_field])
                second_hashes.add(row["row_sha256"])
                count += 1
            stream.write(
                b'],"rows_sha256":'
                + canonical(commitment["rows_sha256"])
                + b',"schema":'
                + canonical(schema)
                + b',"status":'
                + canonical(status)
                + b"}"
            )
    need(
        {
            "row_count": count,
            "row_ids_sha256": second_ids.finish(),
            "row_hashes_sha256": second_hashes.finish(),
            "rows_sha256": second_rows.finish(),
        } == commitment,
        "expected two-pass equality:" + name,
    )
    return {
        "filename": name,
        "file_sha256": file_sha256(path),
        **commitment,
        "schema": schema,
    }


def compare_exact_files(expected: Path, candidate: Path) -> None:
    guard_exact_regular_path(candidate, candidate.name)
    need(expected.stat().st_size == candidate.stat().st_size, "byte size")
    with expected.open("rb") as left, candidate.open("rb") as right:
        while True:
            a = left.read(1 << 20)
            b = right.read(1 << 20)
            need(a == b, "candidate exact byte equality:" + candidate.name)
            if not a:
                break


def exact_upstream_pins() -> dict[str, str]:
    output: dict[str, str] = {}
    output.update(FOUNDATION_MANIFEST_PINS)
    output.update(FOUNDATION_FILE_PINS)
    output.update(INELIGIBLE_PACKAGE_PINS)
    output[R300E_MANIFEST] = R300E_MANIFEST_PIN
    output[R300E_WITNESS_LEDGER] = R300E_WITNESS_LEDGER_PIN
    output[R300B_FACE_INVENTORY] = R300B_FACE_INVENTORY_PIN
    for channel in LEGAL_CHANNELS:
        output[channel.manifest] = channel.manifest_sha256
        output.update(package_members(channel))
    for package in GATE_PACKAGES.values():
        output[package["manifest"]] = package["manifest_sha256"]
        output.update(package["members"])
    return dict(sorted(output.items()))


def strict_single_gzip_bytes(raw: bytes, label: str) -> bytes:
    need(raw and len(raw) <= 10_000_000, "bounded gzip:" + label)
    decoder = zlib.decompressobj(16 + zlib.MAX_WBITS)
    try:
        output = decoder.decompress(raw)
        output += decoder.flush()
    except zlib.error as error:
        raise AuditError("invalid gzip:" + label) from error
    need(
        decoder.eof
        and not decoder.unused_data
        and not decoder.unconsumed_tail,
        "single-member trailing-free gzip:" + label,
    )
    return output


def expect_rejected(action: Callable[[], Any], label: str) -> None:
    try:
        action()
    except (AuditError, OSError, UnicodeError, zlib.error):
        return
    raise AuditError("attack accepted:" + label)


def resign_mutated_row(
    expected: dict[str, Any],
    mutate: Callable[[dict[str, Any]], None],
) -> dict[str, Any]:
    payload = copy.deepcopy(expected)
    payload.pop("row_sha256")
    mutate(payload)
    mutant = close_row(payload)
    verify_row(mutant, "semantic attack")
    need(mutant != expected, "semantic mutation changes row")
    return mutant


def set_nested(
    value: dict[str, Any],
    path: tuple[str, ...],
    replacement: Any,
) -> None:
    cursor: dict[str, Any] = value
    for field in path[:-1]:
        cursor = cursor[field]
    cursor[path[-1]] = replacement


def build_attack_suite(
    samples: dict[str, dict[str, Any]],
    expected_result: dict[str, Any],
    scratch: Path,
) -> dict[str, Any]:
    attacks: list[dict[str, Any]] = []

    def record(
        attack_id: str,
        target: str,
        mutation: str,
        semantic: bool,
    ) -> None:
        attacks.append(close_row({
            "attack_id": attack_id,
            "target": target,
            "mutation": mutation,
            "independent_verification_status": "REJECTED",
            "semantic_attack_fully_reclosed": semantic,
        }))

    semantic_specs: list[
        tuple[str, str, Callable[[dict[str, Any]], None]]
    ] = [
        (
            "A01_EDGE_ELIGIBILITY_FALSE",
            "edge",
            lambda row: row.__setitem__(
                "eligible_for_component_DSU_application", False
            ),
        ),
        (
            "A02_EDGE_RANK_CREDIT_FLIP",
            "edge",
            lambda row: row.__setitem__(
                "formal_DSU_rank_reduction_credit",
                1 - row["formal_DSU_rank_reduction_credit"],
            ),
        ),
        (
            "A03_EDGE_FINAL_COMPONENT",
            "edge",
            lambda row: row.__setitem__(
                "final_Round301_component_id", "forged-component"
            ),
        ),
        (
            "A04_EDGE_SOURCE_ROW",
            "edge",
            lambda row: row.__setitem__("source_edge_row_id", "forged-row"),
        ),
        (
            "A05_EDGE_ORDINAL",
            "edge",
            lambda row: row.__setitem__(
                "application_ordinal", row["application_ordinal"] + 1
            ),
        ),
        (
            "A06_EDGE_IDENTITY_COLLAPSE",
            "edge",
            lambda row: row.__setitem__(
                "formal_occurrence_identity_collapse_credit", 1
            ),
        ),
        (
            "A07_MEMBER_COMPONENT",
            "member",
            lambda row: row.__setitem__(
                "final_Round301_component_id", "forged-component"
            ),
        ),
        (
            "A08_MEMBER_IDENTITY",
            "member",
            lambda row: row.__setitem__("member_identity_preserved", False),
        ),
        (
            "A09_MEMBER_KEY",
            "member",
            lambda row: row.__setitem__("official_key_id", "forged-key"),
        ),
        (
            "A10_MEMBER_BASE_ROOT",
            "member",
            lambda row: row.__setitem__(
                "base_component_root_id", "forged-root"
            ),
        ),
        (
            "A11_KEY_MERGE_CREDIT",
            "key",
            lambda row: row.__setitem__(
                "formal_official_key_merge_credit", 1
            ),
        ),
        (
            "A12_KEY_MEMBER_COUNT",
            "key",
            lambda row: row.__setitem__(
                "member_count", row["member_count"] + 1
            ),
        ),
        (
            "A13_KEY_IDENTITY",
            "key",
            lambda row: row.__setitem__(
                "official_key_identity_preserved", False
            ),
        ),
        (
            "A14_KEY_MULTIPLICITY",
            "key",
            lambda row: row.__setitem__(
                "official_key_multiplicity_in_component",
                row["official_key_multiplicity_in_component"] + 1,
            ),
        ),
        (
            "A15_INELIGIBLE_TO_EDGE",
            "ineligible",
            lambda row: row.__setitem__(
                "eligible_for_component_DSU_application", True
            ),
        ),
        (
            "A16_INELIGIBLE_FED_TO_DSU",
            "ineligible",
            lambda row: row.__setitem__("source_row_fed_to_DSU", True),
        ),
        (
            "A17_INELIGIBLE_RANK_CREDIT",
            "ineligible",
            lambda row: row.__setitem__(
                "formal_DSU_rank_reduction_credit", 1
            ),
        ),
        (
            "A18_INELIGIBLE_COMPONENT_CREDIT",
            "ineligible",
            lambda row: row.__setitem__(
                "formal_component_edge_application_credit", 1
            ),
        ),
        (
            "A19_INELIGIBLE_DISPOSITION",
            "ineligible",
            lambda row: row.__setitem__("disposition", "FORGED_ELIGIBLE"),
        ),
        (
            "A20_INELIGIBLE_PROVENANCE_FORGE",
            "ineligible",
            lambda row: row.__setitem__(
                "exact_gate_provenance_references", [{"forged": True}]
            ),
        ),
    ]
    for attack_id, target, mutate in semantic_specs:
        mutant = resign_mutated_row(samples[target], mutate)
        need(mutant != samples[target], "semantic exact-equality rejection")
        record(attack_id, target, "re-sign semantic payload mutation", True)

    result_specs = [
        (
            "A21_RESULT_COMPONENT_COUNT",
            ("forward_application", "component_count"),
            121_947,
        ),
        (
            "A22_RESULT_RANK",
            ("forward_application", "rank_reduction"),
            246_015,
        ),
        (
            "A23_RESULT_PARTITION",
            ("forward_application", "partition_sha256"),
            "0" * 64,
        ),
        (
            "A24_RESULT_REVERSE_PARTITION",
            ("reverse_application", "partition_sha256"),
            "f" * 64,
        ),
        (
            "A25_RESULT_EDGE_CREDIT",
            (
                "formal_credit_transition",
                "formal_legal_component_edge_application_credit",
            ),
            434_605,
        ),
        (
            "A26_RESULT_DSU_CREDIT",
            ("formal_credit_transition", "formal_DSU_rank_reduction_credit"),
            246_015,
        ),
        (
            "A27_RESULT_INELIGIBLE_FED",
            ("strict_boundary", "R295A_or_R300D_incidence_rows_fed_to_DSU"),
            1,
        ),
        (
            "A28_RESULT_H_EDGE_FORGE",
            ("strict_boundary", "R300H_eligible_edge_row_count"),
            1,
        ),
    ]
    for attack_id, path, replacement in result_specs:
        mutant = copy.deepcopy(expected_result)
        mutant.pop("result_sha256")
        set_nested(mutant, path, replacement)
        mutant["result_sha256"] = digest(mutant)
        check_result_self(mutant, attack_id)
        need(mutant != expected_result, "result mutation changes candidate")
        record(attack_id, "result", "re-sign result semantic mutation", True)

    invalid_json = (
        ("A29_DUPLICATE_JSON_KEY", b'{"a":1,"a":2}'),
        ("A30_NONFINITE_JSON", b'{"a":NaN}'),
        ("A31_FLOAT_JSON", b'{"a":1.5}'),
        ("A32_NUL_JSON", b'{"a":"x\\u0000"}\x00'),
        ("A33_BOM_JSON", b'\xef\xbb\xbf{"a":1}'),
    )
    for attack_id, raw in invalid_json:
        expect_rejected(lambda raw=raw: strict_json(raw, attack_id), attack_id)
        record(attack_id, "JSON", "strict JSON boundary", False)

    valid_gzip = BytesIO()
    with gzip.GzipFile(
        filename="",
        mode="wb",
        fileobj=valid_gzip,
        mtime=0,
    ) as stream:
        stream.write(b'{"rows":[]}')
    member = valid_gzip.getvalue()
    gzip_attacks = (
        ("A34_MALFORMED_GZIP", member[:-5]),
        ("A35_CONCATENATED_GZIP", member + member),
        ("A36_TRAILING_GZIP", member + b"x"),
    )
    for attack_id, raw in gzip_attacks:
        expect_rejected(
            lambda raw=raw: strict_single_gzip_bytes(raw, attack_id),
            attack_id,
        )
        record(attack_id, "GZIP", "single-member gzip boundary", False)

    path_root = scratch / "path-attacks"
    path_root.mkdir()
    good = path_root / "good.json"
    good.write_bytes(b"{}")
    symlink = path_root / "symlink.json"
    symlink.symlink_to(good.name)
    hardlink = path_root / "hardlink.json"
    os.link(good, hardlink)
    expect_rejected(
        lambda: guard_regular_path(symlink, path_root, symlink.name),
        "A37_SYMLINK",
    )
    record("A37_SYMLINK", "path", "symlink substitution", False)
    expect_rejected(
        lambda: guard_regular_path(hardlink, path_root, hardlink.name),
        "A38_HARDLINK",
    )
    record("A38_HARDLINK", "path", "hardlink substitution", False)
    expect_rejected(
        lambda: guard_regular_path(good, scratch, good.name),
        "A39_PATH_ESCAPE",
    )
    record("A39_PATH_ESCAPE", "path", "parent-directory escape", False)

    need(
        sha256_bytes(PRODUCER_PATH.read_bytes() + b"\nforged")
        != PRODUCER_SHA256,
        "producer pin mutation rejected",
    )
    record(
        "A40_PRODUCER_BYTE_MUTATION",
        "producer",
        "fixed inert byte pin mismatch",
        False,
    )
    verifier_source = Path(__file__).read_text(encoding="utf-8")
    forbidden_calls = (
        r"\bexec\s*\(",
        r"\beval\s*\(",
        r"(?<!re\.)\bcompile\s*\(",
        (
            r"(?m)^\s*(?:from|import)\s+"
            r"(?:ast|token" + r"ize|runpy|importlib)\b"
        ),
        r"\b(?:ast|token" + r"ize|runpy|importlib)\.",
    )
    need(
        all(re.search(pattern, verifier_source) is None
            for pattern in forbidden_calls),
        "no producer execution/parser API",
    )
    record(
        "A41_PRODUCER_CODE_PATH",
        "producer",
        "import/execute/parse/tokenize API forbidden",
        False,
    )

    payload = {
        "schema": ATTACK_SCHEMA,
        "status":
            "PASS_INDEPENDENT_REJECTION_41_OF_41__"
            "28_SEMANTIC_ATTACKS_RECLOSED",
        "attack_count": len(attacks),
        "rejected_count": len(attacks),
        "semantic_reclosed_attack_count": 28,
        "all_attacks_rejected": True,
        "producer_imported_executed_parsed_or_tokenized": False,
        "seed_affects_output": False,
        "attacks": attacks,
    }
    need(len(attacks) == 41, "attack count")
    document = dict(payload)
    document["attack_suite_sha256"] = digest(payload)
    return document


def write_or_compare_json(
    path: Path,
    document: dict[str, Any],
    no_write: bool,
) -> None:
    raw = canonical(document)
    if no_write:
        if path.exists() or path.is_symlink():
            guard_exact_regular_path(path, path.name)
            need(path.read_bytes() == raw, "no-write replay:" + path.name)
        return
    work = HERE / (path.name + ".incomplete")
    if work.exists() or work.is_symlink():
        guard_exact_regular_path(work, work.name)
        work.unlink()
    if path.exists() or path.is_symlink():
        guard_exact_regular_path(path, path.name)
    try:
        with work.open("xb") as stream:
            stream.write(raw)
        os.replace(work, path)
    finally:
        if work.exists() and not work.is_symlink():
            work.unlink()


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--seed", default="301101")
    parser.add_argument("--no-write", action="store_true")
    args = parser.parse_args()
    need(bool(args.seed), "nonempty replay seed")

    validate_all_boundaries()
    members, base_roots, member_census = reconstruct_member_universe()
    edges, edge_census = collect_legal_edges(members, base_roots)
    forward_map, forward = apply_edges(edges, base_roots)
    reverse_map, reverse = apply_edges(reversed(edges), base_roots)
    need(
        forward_map == reverse_map
        and forward["partition_sha256"] == reverse["partition_sha256"]
        and forward["rank_reduction"] == reverse["rank_reduction"]
        and forward["component_count"] == reverse["component_count"]
        and forward["edge_rows_processed"] == 434_606
        and forward["rank_reduction"] == 246_016
        and forward["component_count"] == 121_948
        and forward["partition_sha256"]
        == "c1ab6beed75d5e7379103975d6a9ac1b1095a3eb24b34c87288782e7c97cfe09",
        "final DSU census and forward/reverse equality",
    )

    exact_provenance, single_provenance, gate_census = (
        complete_gate_provenance()
    )
    r300a_pairs = exact_R300A_pair_set()
    face_accepted, face_rejected = exact_R300B_face_coverage(r300a_pairs)
    ineligible_factory = lambda: iter_ineligible_source_consumption_rows(
        members,
        base_roots,
        exact_provenance,
        single_provenance,
        face_accepted,
        face_rejected,
    )
    ineligible_census = reconstruct_ineligible_census(
        ineligible_factory,
        edges,
        gate_census,
    )
    incidence, key_multiplicity = build_component_key_incidence(
        members, base_roots, forward_map
    )
    multiplicity_histogram = Counter(key_multiplicity.values())
    component_key_census = {
        "component_key_incidence_row_count": len(incidence),
        "component_official_key_multiplicity_histogram": {
            str(key): value
            for key, value in sorted(multiplicity_histogram.items())
        },
        "cross_official_key_component_count": sum(
            1 for value in key_multiplicity.values() if value > 1
        ),
        "maximum_official_key_multiplicity_in_one_component":
            max(key_multiplicity.values()),
        "official_key_identity_merge_count": 0,
    }

    samples: dict[str, dict[str, Any]] = {}
    with tempfile.TemporaryDirectory(
        prefix="cm2-r301-independent-", dir=HERE
    ) as temporary:
        scratch = Path(temporary)
        sample: list[dict[str, Any]] = []
        edge_ledger = write_expected_ledger(
            scratch,
            EDGE_PATH.name,
            LEDGER_SCHEMA_PREFIX + ".edge-application-ledger.v1",
            "PASS_COMPLETE_CANONICAL_FORWARD_LEGAL_EDGE_APPLICATION",
            "Round301_legal_component_edge_application_row_id",
            lambda: iter_edge_application_rows(
                edges, base_roots, forward_map
            ),
            sample,
        )
        samples["edge"] = sample[0]

        sample = []
        member_ledger = write_expected_ledger(
            scratch,
            MEMBER_PATH.name,
            LEDGER_SCHEMA_PREFIX + ".member-component-ledger.v1",
            "PASS_COMPLETE_564492_MEMBER_TO_COMPONENT_MAPPING",
            "Round301_member_to_component_row_id",
            lambda: iter_member_component_rows(members, forward_map),
            sample,
        )
        samples["member"] = sample[0]

        sample = []
        key_ledger = write_expected_ledger(
            scratch,
            KEY_PATH.name,
            LEDGER_SCHEMA_PREFIX + ".component-key-incidence-ledger.v1",
            "PASS_COMPLETE_COMPONENT_KEY_INCIDENCE_WITHOUT_KEY_MERGE",
            "Round301_component_key_incidence_row_id",
            lambda: iter_component_key_incidence_rows(
                incidence, key_multiplicity
            ),
            sample,
        )
        samples["key"] = sample[0]

        sample = []
        ineligible_ledger = write_expected_ledger(
            scratch,
            INELIGIBLE_PATH.name,
            LEDGER_SCHEMA_PREFIX + ".ineligible-source-consumption-ledger.v1",
            (
                "PASS_COMPLETE_R300A_R300D_SOURCE_CONSUMPTION__"
                "ZERO_INELIGIBLE_ROWS_FED_TO_DSU"
            ),
            "Round301_ineligible_source_consumption_row_id",
            ineligible_factory,
            sample,
        )
        samples["ineligible"] = sample[0]
        need(
            edge_ledger["row_count"] == 434_606
            and member_ledger["row_count"] == 564_492
            and key_ledger["row_count"] == len(incidence)
            and ineligible_ledger["row_count"] == 119_844,
            "four expected ledger censuses",
        )

        output_ledgers = {
            "edge_application": edge_ledger,
            "member_to_component": member_ledger,
            "component_key_incidence": key_ledger,
            "ineligible_source_consumption": ineligible_ledger,
        }
        result_payload = {
            "schema": SCHEMA,
            "status":
                "PASS_ROUND301_COMPLETE_LEGAL_COMPONENT_DSU__"
                "246016_RANK_REDUCTIONS__121948_COMPONENTS__"
                "FORWARD_REVERSE_SAME_PARTITION",
            "producer_file_sha256": PRODUCER_SHA256,
            "input_file_and_manifest_pins": exact_upstream_pins(),
            "member_universe": member_census,
            "legal_edge_census": edge_census,
            "sealed_gate_census": gate_census,
            "forward_application": forward,
            "reverse_application": reverse,
            "forward_reverse_same_partition": True,
            "component_key_incidence_census": component_key_census,
            "ineligible_source_consumption": ineligible_census,
            "output_ledgers": output_ledgers,
            "formal_credit_transition": {
                "formal_legal_component_edge_application_credit": 434_606,
                "formal_DSU_rank_reduction_credit": 246_016,
                "formal_component_union_credit": 246_016,
                "formal_component_quotient_credit": 1,
                "formal_post_Round301_component_count": 121_948,
                "formal_occurrence_identity_collapse_credit": 0,
                "formal_official_key_merge_credit": 0,
                "formal_maximality_credit": 0,
                "formal_fibre_credit": 0,
                "formal_global_disposition_credit": 0,
            },
            "strict_boundary": {
                "R295A_or_R300D_incidence_rows_fed_to_DSU": 0,
                "R300A_closure_contact_rows_fed_to_DSU": 0,
                "R300G_eligible_edge_row_count": 0,
                "R300H_eligible_edge_row_count": 0,
                "obsolete_Round298_or_tmp_spike_imported_executed_or_parsed":
                    False,
                "rejected_multi_endpoint_face_raw_cross_product_used":
                    False,
                "historical_29984_rank_number_used": False,
            },
            "required_next": [
                (
                    "Use the sealed member-to-component and component-key "
                    "incidence ledgers for downstream maximality, fibre, and "
                    "global disposition gates."
                ),
                (
                    "Reopen the exact 2664 R300A pairs with no endpoint face "
                    "only under new positive-area/two-corridor or separately "
                    "sealed gluing evidence."
                ),
            ],
            "seed_affects_output": False,
        }
        expected_result = dict(result_payload)
        expected_result["result_sha256"] = digest(result_payload)
        expected_result_path = scratch / RESULT_PATH.name
        expected_result_path.write_bytes(canonical(expected_result))

        for expected, candidate in (
            (scratch / EDGE_PATH.name, EDGE_PATH),
            (scratch / MEMBER_PATH.name, MEMBER_PATH),
            (scratch / KEY_PATH.name, KEY_PATH),
            (scratch / INELIGIBLE_PATH.name, INELIGIBLE_PATH),
            (expected_result_path, RESULT_PATH),
        ):
            compare_exact_files(expected, candidate)

        attacks = build_attack_suite(
            samples, expected_result, scratch
        )
        candidate_pins = {
            path.name: file_sha256(path)
            for path in (
                EDGE_PATH,
                MEMBER_PATH,
                KEY_PATH,
                INELIGIBLE_PATH,
                RESULT_PATH,
            )
        }
        attack_raw = canonical(attacks)
        verification_payload = {
            "schema": VERIFICATION_SCHEMA,
            "status":
                "PASS_INDEPENDENT_CACHELESS_ROUND301__"
                "564492_MEMBERS__434606_LEGAL_EDGES__"
                "119844_INELIGIBLE_ROWS_CONSUMED_WITH_ZERO_DSU_INPUT__"
                "246016_RANK_REDUCTIONS__121948_COMPONENTS__"
                "ALL_41_ATTACKS_REJECTED",
            "producer_sha256": PRODUCER_SHA256,
            "verifier_sha256": file_sha256(Path(__file__).resolve()),
            "producer_source_imported_executed_parsed_or_tokenized": False,
            "producer_used_only_as_inert_fixed_bytes": True,
            "seed_affects_verdict_or_artifacts": False,
            "independent_reconstruction": {
                "member_count": 564_492,
                "legal_edge_count": 434_606,
                "ineligible_source_consumption_row_count": 119_844,
                "rank_reduction": 246_016,
                "component_count": 121_948,
                "partition_sha256":
                    "c1ab6beed75d5e7379103975d6a9ac1b1095a3eb24b34c87288782e7c97cfe09",
                "forward_reverse_same_partition": True,
                "four_candidate_ledgers_and_result_rebuilt_before_open":
                    True,
            },
            "candidate_artifacts": {
                "exact_object_and_byte_equality": True,
                "deterministic_single_member_gzip_byte_equality": True,
                "file_sha256": candidate_pins,
                "result_sha256": expected_result["result_sha256"],
            },
            "independent_attack_suite": {
                "attack_count": attacks["attack_count"],
                "rejected_count": attacks["rejected_count"],
                "semantic_reclosed_attack_count":
                    attacks["semantic_reclosed_attack_count"],
                "all_attacks_rejected": True,
                "attack_suite_file_sha256": sha256_bytes(attack_raw),
                "attack_suite_sha256": attacks["attack_suite_sha256"],
            },
            "strict_boundary": {
                "duplicate_NUL_nonfinite_float_JSON_rejected": True,
                "malformed_concatenated_trailing_GZIP_rejected": True,
                "symlink_hardlink_path_escape_rejected": True,
                "producer_independence_attack_rejected": True,
                "ineligible_rows_never_fed_to_DSU": True,
            },
        }
        verification = dict(verification_payload)
        verification["verification_sha256"] = digest(verification_payload)
        write_or_compare_json(ATTACK_PATH, attacks, args.no_write)
        write_or_compare_json(
            VERIFICATION_PATH, verification, args.no_write
        )

    print(canonical({
        "status": verification["status"],
        "seed": str(args.seed),
        "seed_affects_output": False,
        "no_write": args.no_write,
        "attack_suite_file_sha256": sha256_bytes(canonical(attacks)),
        "attack_suite_sha256": attacks["attack_suite_sha256"],
        "verification_file_sha256":
            sha256_bytes(canonical(verification)),
        "verification_sha256": verification["verification_sha256"],
        "candidate_file_pins":
            verification["candidate_artifacts"]["file_sha256"],
        "partition_sha256":
            verification["independent_reconstruction"]["partition_sha256"],
    }).decode("utf-8"))


if __name__ == "__main__":
    main()
