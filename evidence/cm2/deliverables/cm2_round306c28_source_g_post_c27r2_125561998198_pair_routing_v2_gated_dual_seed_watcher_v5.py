...
ATTACK_PYTHON = ROOT / (".cm2-runtime/audit/c30a-p0-closure-verifier-"
                        "20260807/fresh-python-flint-0.9.0/bin/python")
SOURCES = {
    ...
    "coherent_attacks": ROOT / (
        "deliverables/cm2_round306c28_source_g_post_c27r2_125561998198_"
        "pair_routing_v2_coherent_attack_harness_v4.py"),
...
def source_preflight(expect_self: str) -> dict[str, str]:
    ...
    observed["attack_python"] = file_sha(ATTACK_PYTHON)
...
def run_transaction(stage: str, spec_path: Path, pinset_path: Path,
                    run_dir: Path, control: Path, label: str | None = None) -> dict[str, Any]:
    stdout = clean_run([
    ...
    ], (label or stage) + "_runner", control)
    ...
    need(hashlib.sha256(stdout).hexdigest()
         == file_sha(control / ((label or stage) + "_runner.stdout.log")),
    ...
    try:
        ...
        spec = command_spec(
            ...
            SOURCE_PINS["coherent_attacks"], [
                ...
                str(SOURCES["independent_verifier"]), "--python",
                str(ATTACK_PYTHON),
