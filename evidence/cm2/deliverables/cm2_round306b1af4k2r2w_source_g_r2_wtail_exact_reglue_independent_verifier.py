...
This independently implemented verifier reconstructs the four multiplicity-two Round271
...
face to one child.  Consequently every verified theorem row is
...
# Package members are held separately so that they do not enter the producer's
# canonical input commitment.  The verifier never imports or executes the
# producer; it treats the source as inert pinned bytes.
PACKAGE_PINS: Final = (
    ("PRODUCER", "cm2_round306b1af4k2r2w_source_g_r2_wtail_exact_reglue_producer.py", 29_794, "1c945797220f1c806a8ddadcb63d9a2e03d2be36cc6209a289709763598a66c7"),
    ("RESULT", "cm2_round306b1af4k2r2w_source_g_r2_wtail_exact_reglue_result.json", 25_794, "87e10fcc3a79854a1361e3f4e836c48febb97a69fce47073bd38cf62c28db936"),
    ("LEDGER", "cm2_round306b1af4k2r2w_source_g_r2_wtail_exact_reglue_theorem_ledger.json", 19_662, "12642bc980beea0eaa23c13ceb531e9633e2680ad08847f44208606c26b38aae"),
    ("ATTACK", "cm2_round306b1af4k2r2w_source_g_r2_wtail_exact_reglue_attack_suite.json", 1_488, "5ab7433c255bc24a385444c31a779d882dc0d0807e5104ffff9e35e1e522d37e"),
)
...
def type_strict_equal(left: Any, right: Any) -> bool:
    ...
    if type(left) is dict:
        if set(left.keys()) != set(right.keys()):
            return False
        return all(type_strict_equal(left[key], right[key]) for key in left)
...
class HeldPackage:
    """Two-pass held-FD pins for inert package members."""
    ...
    def __enter__(self) -> "HeldPackage":
        ...
        need(stat.S_ISDIR(before.st_mode), "package directory")
        flags = os.O_RDONLY | getattr(os, "O_DIRECTORY", 0) | getattr(os, "O_NOFOLLOW", 0) | getattr(os, "O_CLOEXEC", 0)
        self.dirfd = os.open(os.fspath(HERE), flags)
        self.dir_before = os.fstat(self.dirfd)
        need(dir_identity(before) == dir_identity(self.dir_before), "package directory open race")
        try:
            for label, filename, size, expected in PACKAGE_PINS:
                named = os.stat(filename, dir_fd=self.dirfd, follow_symlinks=False)
                need(stat.S_ISREG(named.st_mode) and named.st_nlink == 1 and named.st_size == size, "package pin identity:" + label)
                fd = os.open(filename, os.O_RDONLY | getattr(os, "O_NOFOLLOW", 0) | getattr(os, "O_CLOEXEC", 0), dir_fd=self.dirfd)
                opened = os.fstat(fd)
                need(identity(named) == identity(opened), "package open race:" + label)
                need(hash_fd(fd) == expected and hash_fd(fd) == expected, "package two-pass SHA:" + label)
                need(identity(opened) == identity(os.stat(filename, dir_fd=self.dirfd, follow_symlinks=False)), "package rebound:" + label)
                self.files[label] = (fd, opened)
            return self
        except BaseException:
            self.close()
    ...
    def json(self, label: str) -> dict[str, Any]:
        ...
        while True:
            ...
            if not block:
                break
            raw.extend(block)
            need(len(raw) <= 1 << 20, "small package JSON cap")
        os.lseek(fd, 0, os.SEEK_SET)
        try:
            text = bytes(raw).decode("utf-8")
            value, end = DECODER.raw_decode(text)
        except (UnicodeDecodeError, json.JSONDecodeError) as exc:
            raise AuditBlocked("package JSON decode:" + label) from exc
        need(text[end:].strip() == "" and type(value) is dict, "package JSON root/trailing:" + label)
        strict_tree(value)
        need(identity(os.fstat(fd)) == identity(before), "package FD stable:" + label)
        return value

    def final_rehash(self) -> None:
        for label, filename, _size, expected in PACKAGE_PINS:
            fd, before = self.files[label]
            need(hash_fd(fd) == expected, "package final SHA:" + label)
            need(identity(os.fstat(fd)) == identity(before) == identity(os.stat(filename, dir_fd=self.dirfd, follow_symlinks=False)), "package final path:" + label)
        assert self.dir_before is not None
        need(dir_identity(self.dir_before) == dir_identity(os.fstat(self.dirfd)) == dir_identity(os.stat(HERE, follow_symlinks=False)), "package final directory")
...
def set_path(root: Any, path: list[Any], value: Any) -> None:
    need(type(path) is list and path, "attack path")
    cursor = root
    for key in path[:-1]:
        need(type(key) in (str, int) and type(key) is not bool, "attack path token")
        cursor = cursor[key]
    key = path[-1]
    need(type(key) in (str, int) and type(key) is not bool, "attack final token")
    cursor[key] = value


def validate_candidate(candidate: Any, expected: dict[str, Any]) -> None:
    strict_tree(candidate)
    need(type_strict_equal(candidate, expected), "candidate differs from independent replay")
    result = candidate["result"]
    need(type(result["census"]["r2_artificial_face_reglue_credit"]) is int and result["census"]["r2_artificial_face_reglue_credit"] == 0, "census zero-credit type")
    need(type(result["theorem_ledger"]["row_count"]) is int and result["theorem_ledger"]["row_count"] == 4, "row-count type")
    for row in result["theorem_ledger"]["rows"]:
        need(row["status"] == "BLOCKED_MISSING_AUTHORITY", "blocked status")
        for key in ("r2_artificial_face_reglue_credit", "normalized_support_credit", "representation_cover_credit", "B1A_credit", "B2_credit"):
            need(type(row[key]) is int and row[key] == 0, "row zero-credit type:" + key)
        for key, expected_bool in (
            ...
            need(type(row[key]) is bool and row[key] is expected_bool, "row bool type/value:" + key)
        payload = dict(row)
        claimed = payload.pop("row_sha256")
        need(type(claimed) is str and digest(payload) == claimed, "output row closure")
    need(digest(result["theorem_ledger"]["rows"]) == result["theorem_ledger"]["rows_sha256"], "output rows commitment")
    need(digest([row["row_sha256"] for row in result["theorem_ledger"]["rows"]]) == result["theorem_ledger"]["row_hashes_sha256"], "output row hashes commitment")
    need(digest(result) == candidate["result_sha256"], "output result commitment")


def verify() -> dict[str, Any]:
    with HeldPackage() as package:
        result_document = package.json("RESULT")
        ledger_document = package.json("LEDGER")
        attacks = package.json("ATTACK")
        expected = build()
        validate_candidate(result_document, expected)
        expected_ledger = {
            "schema": SCHEMA + ".theorem-ledger.v1",
            **expected["result"]["theorem_ledger"],
        }
        need(type_strict_equal(ledger_document, expected_ledger), "theorem ledger independent replay")
        need(type(attacks.get("cases")) is list and type(attacks.get("expected_rejected")) is int, "attack suite shape")
        rejected = 0
        for case in attacks["cases"]:
            need(type(case) is dict and type(case.get("id")) is str and type(case.get("path")) is list, "attack case shape")
            mutant = copy.deepcopy(expected)
            set_path(mutant, case["path"], case["value"])
            try:
                validate_candidate(mutant, expected)
            except AuditBlocked:
                rejected += 1
        need(rejected == attacks["expected_rejected"] == len(attacks["cases"]), "all attacks rejected")
        package.final_rehash()
    return {
        "schema": SCHEMA + ".verification.v1",
        "status": "PASS_INDEPENDENT_REPLAY__FOUR_BLOCKED_ROWS__ZERO_REGLUE_CREDIT",
        "producer_imported_or_executed": False,
        "upstream_pins_two_pass_and_final_rehash": len(PINS),
        "package_pins_two_pass_and_final_rehash": len(PACKAGE_PINS),
        "ordered_table_commitments_recomputed": len(TABLES),
        "blocked_rows_recomputed": 4,
        "attacks_rejected": rejected,
        "false_equals_zero_rejected": True,
        "true_equals_one_rejected": True,
        "result_sha256": expected["result_sha256"],
        "theorem_rows_sha256": expected["result"]["theorem_ledger"]["rows_sha256"],
        "r2_artificial_face_reglue_credit": 0,
        "normalized_support_credit": 0,
        "B1A_credit": 0,
        "B2_credit": 0,
...
def self_test() -> dict[str, Any]:
    ...
    need(not type_strict_equal(False, 0), "false must not alias zero")
    need(not type_strict_equal(True, 1), "true must not alias one")
    need(type_strict_equal({"x": [0, False]}, {"x": [0, False]}), "recursive strict equality baseline")
    need(not type_strict_equal({"x": [0, False]}, {"x": [False, 0]}), "recursive bool/int alias rejection")
    return {"status": "PASS", "mutations_rejected": len(mutations), "distinct_input_commitments": 2, "false_equals_zero_rejected": True, "true_equals_one_rejected": True, "recursive_type_strict_equality": True}
...
def main() -> int:
    ...
    if args.print_contract:
        output = {"schema": SCHEMA + ".independent-verifier.v1", "status": STATUS, "upstream_pins": [{"label": x[0], "filename": x[1], "size": x[2], "sha256": x[3], "role": x[4]} for x in PINS], "package_pins": [{"label": x[0], "filename": x[1], "size": x[2], "sha256": x[3]} for x in PACKAGE_PINS], "writes_files": False, "executes_or_imports_producer": False}
    elif args.self_test:
        output = self_test()
    else:
        output = verify()
