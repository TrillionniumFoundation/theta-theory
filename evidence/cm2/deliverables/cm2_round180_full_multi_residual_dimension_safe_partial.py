#!/usr/bin/env python3
"""Round180 bounded full residual pass over the source-W multi block.

Every one of the 44,040 Round176 depth-14 residual cells enters this pass.
Origins that already carry a strict LIVE or MIXED witness are refined by two
additional dyadic levels.  The 1,476 exclusion-upper-bound origins are refined
by four levels.  Integer credit is granted only when every Round176 residual
root of one original physical depth-8 parent is completely replaced by closed
EXCLUDED cells and all inherited Round176 evidence is also EXCLUDED.

The rectangular pass is intentionally bounded.  Remaining Delta, root-order,
H, and compact-q collars are classified and retained as dimension-aware
residual arrangements; child counts and rational volumes are never promoted
to integer record credit.
"""
...
SCHEMA = "cm2.round180.full-multi-residual-dimension-safe-partial.v1"
PINS = {
...
def producer_protected_paths() -> set[Path]:
    return {
...
def caller_authorized_output(path: Path) -> bool:
    return (
        ...
        or (
            path.name.startswith(".cm2_round180_")
...
def producer_path_safety_attacks() -> dict[str, Any]:
    """Exercise the producer writer without ever aiming it at a real pin."""
    actual_protected = producer_protected_paths()
    expected_protected = {
        Path(__file__).resolve(),
        *((HERE / name).resolve() for name in PINS),
        *((HERE / name).resolve()
    ...
    require(actual_protected == expected_protected, "protected path set")
    require(OUTPUT.resolve() not in actual_protected, "certificate writable")

    pid = os.getpid()
    scratch = Path(tempfile.mkdtemp(prefix=".round180-producer-path.", dir=HERE))
    source = scratch / "source"
    source.write_bytes(b"fixture\n")
    symlink_output = (
        HERE / f".cm2_round180_symlink_{pid}_certificate.json"
    )
    hard_base = HERE / f".cm2_round180_hard_base_{pid}_certificate.json"
    hard_link = HERE / f".cm2_round180_hard_link_{pid}_certificate.json"
    fifo_output = HERE / f".cm2_round180_fifo_{pid}_certificate.json"
    outside = (
        HERE.parent / f".cm2_round180_escape_{pid}_certificate.json"
    )
    protected_fixtures: list[Path] = []
    rejected: list[str] = []
    try:
        ...
        rejected.append(expect_rejection(
            ...
            lambda: safe_atomic_write(
                symlink_output, b"x", actual_protected
            ),
        ))
        ...
        rejected.append(expect_rejection(
            "hardlink output",
            lambda: safe_atomic_write(
                hard_link, b"x", actual_protected
            ),
        ))

        rejected.append(expect_rejection(
            "parent directory escape",
            lambda: safe_atomic_write(outside, b"x", actual_protected),
        ))
        rejected.append(expect_rejection(
            "nested subdirectory output",
            lambda: safe_atomic_write(
                scratch / ".cm2_round180_nested_certificate.json",
                b"x",
                actual_protected,
            ),
        ))
        rejected.append(expect_rejection(
            ...
            lambda: safe_atomic_write(scratch, b"x", actual_protected),
        ))
        ...
        rejected.append(expect_rejection(
            "FIFO output",
            lambda: safe_atomic_write(
                fifo_output, b"x", actual_protected
            ),
        ))
        ...
        for index, label in enumerate(protected_labels):
            fixture = (
                HERE
                / (
                    f".cm2_round180_protected_{pid}_{index}"
                    "_certificate.json"
                )
            )
            fixture.write_bytes(b"fixture\n")
            protected_fixtures.append(fixture)
            fixture_protected = actual_protected | {fixture.resolve()}
            rejected.append(expect_rejection(
                label,
    ...
    require(len(rejected) == 19, "producer path attack count")
    return {
        "attack_count": 19,
        "rejected_count": len(rejected),
        "all_rejected": len(rejected) == 19,
...
def main() -> None:
    ...
    safe_atomic_write(
        arguments.output,
        pretty_bytes(document),
        producer_protected_paths(),
    )
