#!/usr/bin/env python3
"""Independent no-producer verifier for the C76M v4 collision-two decider.
...
DECLARED_PRODUCER_FILE_SHA256 = "b96dea419cc8e1e5389fd31fbfbd45ac5f20a25c0d17f022147d74c193fc7acd"
VERIFY_SCHEMA = "cm2.round306c76m.large-component-collision2-structural-decider-independent-verification.v2"
VERIFY_NAME = "cm2_round306c76m_large_component_collision2_structural_decider_independent_verification_v2.json"
RECEIPT_NAME = "cm2_round306c76m_large_component_collision2_structural_decider_dual_completion_receipt_v2.json"
...
def verify_stage(stage: Path, raw_map: Mapping[str, bytes]
    ...
    for name in PUBLICATION_ORDER:
        ...
        raw = raw_map[name]
        need(bool(raw), "stage-nonempty-terminal-replay:" + name)
        if name.endswith(".jsonl.gz"):
            one_member(raw, "stage-terminal-replay:" + name)
        else:
            need(raw[-1:] == b"\n", "stage-text-terminal-newline:" + name)
    need(raw_map[LOCK] ==
...
def verify(stage_a: Path, stage_b: Path, verification_a: Path,
    ...
    verification_a_stat = os.lstat(verification_a)
    verification_b_stat = os.lstat(verification_b)
    receipt_stat = os.lstat(receipt_path)
    need(verification_a_stat.st_mtime_ns < verification_b_stat.st_mtime_ns <
         receipt_stat.st_mtime_ns, "verification-receipt-strict-mtime-order")
    replay = [
        {"role": "verification_A",
