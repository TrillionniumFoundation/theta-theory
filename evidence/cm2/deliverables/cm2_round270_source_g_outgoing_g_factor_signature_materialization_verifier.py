#!/usr/bin/env python3
"""Independent full-recomputation verifier for Round270."""
...
VERIFICATION_SCHEMA="cm2.round270.source-g-outgoing-g-factor-signature-materialization-verification.v1"
PRODUCER_SHA256="0ed0df9e4873e93b03ba11e8ad8ce6274d3b9de01875363c1a1818fdbc8867df"
CANDIDATE_SHA256="72a47e53ff601660cb63fe8062403e41a54450fa4a432638faf18a2c76b3efea"
CANDIDATE_RESULT_SHA256="5c9c0585bf2cb664df493224f1005d4ef078d5e2f2e3044283019053b5e39add"
CANDIDATE_SIZE=56_705_100
...
def main():
 ...
 pb=raw(PRODUCER,5_000_000); need(hashlib.sha256(pb).hexdigest()==PRODUCER_SHA256,"producer pin")
 expected=build(PRODUCER_SHA256,a.processes); eh=digest(expected); need(eh==CANDIDATE_RESULT_SHA256,"independent result pin")
 cb=raw(CANDIDATE); need(len(cb)==CANDIDATE_SIZE and hashlib.sha256(cb).hexdigest()==CANDIDATE_SHA256,"candidate byte pin")
 candidate=json.loads(cb); need(candidate.get("schema")==SCHEMA and candidate.get("result_sha256")==CANDIDATE_RESULT_SHA256 and digest(candidate.get("result"))==CANDIDATE_RESULT_SHA256,"candidate closure"); need(candidate["result"]==expected,"exact result equality")
 attacks=[]
 for label,path,value in (("signed-count",("census","signed_region_count"),37711),("residual-count",("census","remaining_OUTGOING_G_failclosed_leaf_count"),7),("component-credit",("strict_nonpromotion","new_component_union_credit"),1),("jxjy-credit",("strict_nonpromotion","Jx_Jy_same_point_glue_credit"),1),("cm2-status",("strict_nonpromotion","CM2"),"GO_FOR_CLAIM")):
 ...
 verification={"status":"PASS_INDEPENDENT_ROUND270","schema":VERIFICATION_SCHEMA,"producer_sha256":PRODUCER_SHA256,"candidate_sha256":CANDIDATE_SHA256,"candidate_size":CANDIDATE_SIZE,"candidate_result_sha256":CANDIDATE_RESULT_SHA256,"independent_result_sha256":eh,"full_24512_leaf_recomputation":True,"exact_candidate_result_equality":True,"attack_tests":attacks,"attack_tests_rejected":len(attacks),"strict_nonpromotion_reconfirmed":True}
