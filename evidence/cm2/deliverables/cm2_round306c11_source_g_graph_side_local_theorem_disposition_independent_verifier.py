#!/usr/bin/env python3
"""Independent verifier for the scoped C11 graph/side routing audit.

This verifier does not import, execute, or parse the C11 producer.  It
reconstructs the 10,118 live graph-to-side routing dispositions directly from
held, hash-pinned upstream artifacts and compares every output byte.  The
9,950 rows called candidate-ready are routing classifications only: R248/R245
historical receipts do not themselves supply an exact side carrier,
closure-incidence theorem, or one-sided collar/common-boundary trace.
Consequently every physical-incidence, trace, pullback, support, DSU, B1A, B2,
maximality, and CM2 credit is forced to zero.
"""
...
# Frozen corrected zero-credit producer byte record.  The verifier never
# opens the producer as Python source; this record is compared only with
# candidate metadata and the manifest member bytes.
PRODUCER_RECORD: Final = {
    "filename": PRODUCER_NAME,
    "size": 82_496,
    "sha256": "a59e61ea0021046cf3fe313343d5dbdbd00b1ed426b3facd31e03a4bb0822ab1",
}
...
def r242_pair_certificate(
    ...
    need(r242["lower_t_face_F_sign"] == semantic["lower_t_face_F_sign"] and r242["upper_t_face_F_sign"] == semantic["upper_t_face_F_sign"], "R242 face-sign binding")
    need(r242["local_positive_2D_transition_sheet_patch_credit"] == 1 and r242["physical_component_credit"] == 0, "R242 authority scope")
    ...
    need(differing_signature_fields <= allowed_signature_differences, "R242 disallowed owner/shadow signature difference")
    ...
    need(owner_signature == r242["owner_signature"] and shadow_signature == r242["shadow_signature"], "R242 owner/shadow authority binding")
    ...
    for edge in related_edges:
        kind = edge["edge_kind"]
        if kind not in expected_edge_kinds:
            continue
...
def independently_reconstruct() -> tuple[dict[str, Any], bytes, bytes, bytes, bytes]:
    with Snapshot() as snapshot:
        ...
        for row in jsonl_rows(snapshot, "C10_IDENTITY"):
            ...
            need(row["graph_class"] == support["graph_class"] and row["sheet_member_id"] == support["sheet_member_id"], "C10 identity subject")
            ...
            need(row["disposition"] == "PRESERVE_EXISTING_MEMBER_ID" and row["member_natural_key_preservation_credit"] == 1, "C10 identity disposition")
            need(row["set_equality_proved"] is False and row["physical_incidence_proved"] is False and row["identity_representation_pullback_proved"] is False, "C10 identity nonpromotion")
        ...
        for graph_id, relations in positive_sides_by_graph.items():
            ...
            need(r242_commitment[1] == graph_id and r242_commitment[2] == object_sha(r242) and r242_commitment[3] == r242["row_sha256"], "R242 C5 commitment")
        ...
        for relation in c9_positive_side_rows:
            ...
            core = {
                ...
                "C10_member_identity_ref": row_ref(identity_by_graph[graph_id]),
                "graph_id": graph_id,
            ...
            if ready:
                ...
                ready_rows.append(closed_row(theorem_core))
            else:
                blocked_core = {
...
def parse_candidate_ledger(
    ...
    for ordinal, line in enumerate(plain.splitlines(keepends=True)):
        ...
        core = dict(row)
        claimed = core.pop("row_sha256", None)
        need(type(claimed) is str and claimed == object_sha(core), "candidate row closure:" + str(ordinal))
        rows.append(row)
    need(len(rows) == descriptor["row_count"], "candidate ledger row count")
    need(sequence_sha(row["row_id"] for row in rows) == descriptor["ordered_row_ids_sha256"], "candidate row-id order")
    need(sequence_sha(row["row_sha256"] for row in rows) == descriptor["ordered_row_hashes_sha256"], "candidate row-hash order")
    need(sequence_sha(rows) == descriptor["ordered_rows_sha256"], "candidate ordered rows")
    return rows
...
def closed_candidate_result(raw: bytes) -> dict[str, Any]:
    ...
    core = dict(value)
    claimed = core.pop("result_sha256", None)
    need(type(claimed) is str and claimed == object_sha(core), "candidate result closure")
    ...
    need(value.get("source_pins") == [pin.__dict__ for pin in PINS], "candidate source pin closure")
    need(value.get("disposition_census") == {
        "positive_graph_side_relations": 10_118,
        "CANDIDATE_READY_ROUTING": 9_950,
        "BLOCKED": 168,
        "ADJACENT_DOMAIN_REQUIRED": 152,
    ...
    need(value.get("formal_credit") == ZERO_DOWNSTREAM, "candidate all formal credit zero")
    nonpromotion = value.get("strict_nonpromotion")
    need(type(nonpromotion) is dict, "candidate strict nonpromotion")
    need(nonpromotion.get("candidate_ready_routing_is_local_graph_side_theorem") is False, "candidate routing is not theorem")
    need(nonpromotion.get("local_graph_side_physical_incidence_proved") is False, "candidate no physical theorem")
    need(nonpromotion.get("one_sided_trace_proved") is False, "candidate no trace theorem")
    need(nonpromotion.get("cross_component_forecast_is_DSU_edge_or_union") is False, "candidate no DSU promotion")
    need(nonpromotion.get("B1A_permitted") is False and nonpromotion.get("B2_permitted") is False, "candidate no B1A/B2")
    need(nonpromotion.get("maximality_permitted") is False and nonpromotion.get("CM2") == "NO-GO_FOR_CLAIM", "candidate no maximality/CM2")
    forecast = value.get("component_forecast_only")
    need(type(forecast) is dict and forecast == {
        "CANDIDATE_READY_same_base_root_relations": 5_302,
        "CANDIDATE_READY_cross_component_relations": 4_648,
        "CANDIDATE_READY_distinct_cross_component_pairs": 4_500,
        "BLOCKED_same_base_root_relations": 32,
        ...
        "DSU_edge_credit": 0,
        "DSU_union_credit": 0,
    }, "candidate component forecast")
    return value


def expected_missing_authority(graph_class: str) -> list[str]:
    if graph_class in {"R235_TARGET_POSITIVE_PARTIAL_BASE", "R235_SOURCE_EXACT_FACE_FULL_BASE"}:
        return R235_MISSING_AUTHORITY
    if graph_class == "R235D_SOURCE_EXACT_FACE_FULL_BASE":
        return R235D_MISSING_AUTHORITY
    need(graph_class == "R242_UNIQUE_GRAPH_FULL_PATCH", "candidate graph class")
    return R242_MISSING_AUTHORITY


def validate_candidate_payloads(
    result_raw: bytes,
    disposition_wire: bytes,
    ready_wire: bytes,
    blocked_wire: bytes,
    reroute_wire: bytes,
) -> dict[str, Any]:
    result = closed_candidate_result(result_raw)
    dispositions = parse_candidate_ledger(disposition_wire, result["disposition_ledger"], DISPOSITION_ROW_SCHEMA)
    candidates = parse_candidate_ledger(ready_wire, result["ready_local_theorem_ledger"], READY_ROW_SCHEMA)
    blocked = parse_candidate_ledger(blocked_wire, result["blocked_ledger"], BLOCKED_ROW_SCHEMA)
    reroutes = parse_candidate_ledger(reroute_wire, result["missing_local_shared_reroute_findings"], REROUTE_ROW_SCHEMA)
    need((len(dispositions), len(candidates), len(blocked), len(reroutes)) == (10_118, 9_950, 168, 10), "candidate ledger census")
    need([row["C9_mechanical_relation_ordinal"] for row in dispositions] == sorted(row["C9_mechanical_relation_ordinal"] for row in dispositions), "candidate disposition order")
    need(len({row["C9_relation_ref"]["row_id"] for row in dispositions}) == 10_118, "candidate relation uniqueness")

    disposition_by_id = {row["row_id"]: row for row in dispositions}
    class_disposition = Counter((row["graph_class"], row["disposition"]) for row in dispositions)
    need(class_disposition == {
        ("R235_TARGET_POSITIVE_PARTIAL_BASE", "CANDIDATE_READY_ROUTING"): 8_864,
        ("R235_SOURCE_EXACT_FACE_FULL_BASE", "CANDIDATE_READY_ROUTING"): 552,
        ("R235_SOURCE_EXACT_FACE_FULL_BASE", "ADJACENT_DOMAIN_REQUIRED"): 152,
        ("R235D_SOURCE_EXACT_FACE_FULL_BASE", "CANDIDATE_READY_ROUTING"): 6,
        ("R235D_SOURCE_EXACT_FACE_FULL_BASE", "OUTSIDE_DOMAIN_TRACE_AUTHORITY_REQUIRED"): 16,
        ("R242_UNIQUE_GRAPH_FULL_PATCH", "CANDIDATE_READY_ROUTING"): 528,
    }, "candidate class disposition census")
    component_census = Counter(("CANDIDATE" if row["candidate_ready_routing"] else "BLOCKED", row["component_relation"]) for row in dispositions)
    need(component_census == {("CANDIDATE", "SAME_BASE_ROOT"): 5_302, ("CANDIDATE", "CROSS_COMPONENT"): 4_648, ("BLOCKED", "SAME_BASE_ROOT"): 32, ("BLOCKED", "CROSS_COMPONENT"): 136}, "candidate component census")
    cross_pairs = {tuple(row["cross_component_pair"]) for row in dispositions if row["candidate_ready_routing"] and row["component_relation"] == "CROSS_COMPONENT"}
    need(len(cross_pairs) == 4_500, "candidate distinct cross-component pair census")
    for row in dispositions:
        expected_ready = row["disposition"] == "CANDIDATE_READY_ROUTING"
        need(row["candidate_ready_routing"] is expected_ready, "candidate disposition routing flag")
        need(row["missing_theorem_authority"] == expected_missing_authority(row["graph_class"]), "candidate missing theorem authority")
        need(row["local_graph_side_physical_incidence_proved"] is False and row["one_sided_trace_proved"] is False, "candidate no local theorem")
        need(row["representation_pullback_proved"] is False and row["graph_sheet_set_equality_proved"] is False, "candidate no pullback/set equality")
        need(row["DSU_edge_or_union_authorized"] is False and row["formal_credit"] == ZERO_DOWNSTREAM, "candidate row zero credit")
        basis = row["theorem_basis"]
        need(type(basis) is dict, "candidate routing basis")
        for key in ("one_sided_local_trace_closed", "full_base_local_trace_closed", "shared_same_sign_local_trace_closed", "one_sided_trace_proved", "closure_incidence_proved", "exact_side_carrier_proved"):
            if key in basis:
                need(basis[key] is False, "candidate routing basis nonpromotion:" + key)
        if row["graph_class"] == "R242_UNIQUE_GRAPH_FULL_PATCH":
            need(len(basis["three_edge_refs"]) == 3 and basis["routing_evidence_closed"] is True, "candidate R242 three-edge routing")
            need(basis["owner_shadow_signature_allowed_difference_fields"] == ["outgoing_cell", "target_chart"], "candidate R242 signature differences")

    candidate_disposition_ids: set[str] = set()
    for row in candidates:
        disposition = disposition_by_id.get(row["disposition_ref"]["row_id"])
        need(disposition is not None and row["disposition_ref"] == row_ref(disposition), "candidate routing-disposition binding")
        need(disposition["candidate_ready_routing"] is True and disposition["disposition"] == "CANDIDATE_READY_ROUTING", "candidate ready disposition")
        need(row["graph_id"] == disposition["graph_id"] and row["side_member_id"] == disposition["side_member_id"], "candidate routing subject")
        need(row["routing_basis_sha256"] == object_sha(disposition["theorem_basis"]), "candidate routing basis binding")
        need(row["missing_theorem_authority"] == disposition["missing_theorem_authority"], "candidate missing authority binding")
        need("theorem_statement" not in row, "candidate no theorem statement")
        need(row["local_graph_side_physical_incidence_credit"] == row["one_sided_trace_credit"] == 0, "candidate local credits zero")
        need(row["graph_sheet_set_equality_credit"] == row["representation_pullback_credit"] == row["DSU_edge_credit"] == row["DSU_union_credit"] == 0, "candidate downstream row credits zero")
        need(row["downstream_nonpromotion"] == ZERO_DOWNSTREAM, "candidate ledger nonpromotion")
        candidate_disposition_ids.add(disposition["row_id"])
    need(len(candidate_disposition_ids) == 9_950, "candidate routing exhaustion")

    blocked_disposition_ids: set[str] = set()
    for row in blocked:
        disposition = disposition_by_id.get(row["disposition_ref"]["row_id"])
        need(disposition is not None and row["disposition_ref"] == row_ref(disposition), "blocked disposition binding")
        need(disposition["candidate_ready_routing"] is False and row["blocker"] == disposition["disposition"], "blocked disposition")
        need(row["missing_theorem_authority"] == disposition["missing_theorem_authority"], "blocked missing theorem authority")
        need(row["local_graph_side_physical_incidence_credit"] == row["one_sided_trace_credit"] == row["DSU_edge_credit"] == 0, "blocked credits zero")
        need(row["downstream_nonpromotion"] == ZERO_DOWNSTREAM, "blocked nonpromotion")
        blocked_disposition_ids.add(disposition["row_id"])
    need(len(blocked_disposition_ids) == 168 and not (blocked_disposition_ids & candidate_disposition_ids), "blocked exhaustion")
    need(set(disposition_by_id) == blocked_disposition_ids | candidate_disposition_ids, "disposition partition")

    need([row["bridge_ordinal"] for row in reroutes] == sorted(row["bridge_ordinal"] for row in reroutes), "reroute order")
    need(len({row["positive_source_graph_id"] for row in reroutes}) == 10, "reroute source graph uniqueness")
    for row in reroutes:
        need(row["reroute_performed"] is False, "reroute not performed")
        need(row["local_graph_side_physical_incidence_credit"] == row["one_sided_trace_credit"] == row["DSU_edge_credit"] == 0, "reroute credits zero")
        need(row["downstream_nonpromotion"] == ZERO_DOWNSTREAM, "reroute nonpromotion")
    return result
...
class CandidateBundle:
    ...
    def __enter__(self) -> "CandidateBundle":
        ...
        validate_candidate_payloads(
            self.payloads[RESULT_NAME],
            self.payloads[DISPOSITION_LEDGER_NAME],
            self.payloads[READY_LEDGER_NAME],
            self.payloads[BLOCKED_LEDGER_NAME],
            self.payloads[REROUTE_LEDGER_NAME],
        )
        return self
...
def receipt(records: Mapping[str, CandidateRecord], expected: Mapping[str, Any]) -> dict[str, Any]:
    return {
        "status": "PASS_INDEPENDENT_10118_GRAPH_SIDE_ROUTING_DISPOSITIONS__9950_CANDIDATE_READY__168_BLOCKED__10_REROUTES__ZERO_THEOREM_CREDIT",
        "candidate_files": [records[name].__dict__ for name in OUTPUT_NAMES],
        "result_sha256": expected["result_sha256"],
        "positive_graph_side_relations": 10_118,
        "candidate_ready_routing_rows": 9_950,
        "structurally_blocked_rows": 168,
        "missing_local_shared_reroute_findings": 10,
...
def verify(candidate_dir: Path, expected_bundle: tuple[dict[str, Any], bytes, bytes, bytes, bytes] | None = None) -> dict[str, Any]:
    ...
    with CandidateBundle(candidate_dir) as candidate:
        expected_payloads = {
            RESULT_NAME: canonical(expected[0]),
            DISPOSITION_LEDGER_NAME: expected[1],
            READY_LEDGER_NAME: expected[2],
            BLOCKED_LEDGER_NAME: expected[3],
            REROUTE_LEDGER_NAME: expected[4],
        }
        for filename in OUTPUT_NAMES:
            need(candidate.payloads[filename] == expected_payloads[filename], "candidate independently reconstructed:" + filename)
...
def coherent_ledger_mutation(directory: Path, filename: str, descriptor_field: str, mutator: Any) -> None:
            ...
            with gzip.GzipFile(filename="", mode="wb", compresslevel=9, fileobj=raw_target, mtime=0) as target:
                for line in source:
                    ...
                    if row_count == 0:
                        mutator(row)
                        core = dict(row)
                        core.pop("row_sha256", None)
                        row = {**core, "row_sha256": object_sha(core)}
...
def attack_suite(candidate_dir: Path) -> dict[str, Any]:
    ...
    work = Path(tempfile.mkdtemp(prefix="cm2-c11-zero-credit-routing-attacks-"))
    ...
    def execute(attack_id: str, mutation: Any, cleanup: Any | None = None) -> None:
        ...
        try:
            ...
            except (Rejected, OSError, EOFError, gzip.BadGzipFile, json.JSONDecodeError, UnicodeDecodeError, KeyError, TypeError, ValueError) as error:
    ...
    try:
        ...
        execute("A08_READY_CENSUS", lambda: rewrite_result(work / RESULT_NAME, lambda value: value["disposition_census"].__setitem__("CANDIDATE_READY_ROUTING", 9_951)))
        execute("A09_BLOCKED_CENSUS", lambda: rewrite_result(work / RESULT_NAME, lambda value: value["disposition_census"].__setitem__("BLOCKED", 167)))
        execute("A10_PHYSICAL_RESULT_CREDIT", lambda: rewrite_result(work / RESULT_NAME, lambda value: value["formal_credit"].__setitem__("local_graph_side_physical_incidence", 1)))
        execute("A11_TRACE_RESULT_CREDIT", lambda: rewrite_result(work / RESULT_NAME, lambda value: value["formal_credit"].__setitem__("one_sided_trace", 1)))
        execute("A12_DSU_RESULT_CREDIT", lambda: rewrite_result(work / RESULT_NAME, lambda value: value["formal_credit"].__setitem__("DSU_edge", 1)))
        execute("A13_B1A_PROMOTION", lambda: rewrite_result(work / RESULT_NAME, lambda value: value["strict_nonpromotion"].__setitem__("B1A_permitted", True)))
        execute("A14_CM2_PROMOTION", lambda: rewrite_result(work / RESULT_NAME, lambda value: value["strict_nonpromotion"].__setitem__("CM2", "PASS")))
        execute("A15_COMPONENT_PAIR_CENSUS", lambda: rewrite_result(work / RESULT_NAME, lambda value: value["component_forecast_only"].__setitem__("CANDIDATE_READY_distinct_cross_component_pairs", 4_501)))
        execute("A16_DISPOSITION_PHYSICAL_PROOF", lambda: coherent_ledger_mutation(work, DISPOSITION_LEDGER_NAME, "disposition_ledger", lambda row: row.__setitem__("local_graph_side_physical_incidence_proved", True)))
        execute("A17_DISPOSITION_TRACE_PROOF", lambda: coherent_ledger_mutation(work, DISPOSITION_LEDGER_NAME, "disposition_ledger", lambda row: row.__setitem__("one_sided_trace_proved", True)))
        execute("A18_DISPOSITION_MISSING_AUTHORITY", lambda: coherent_ledger_mutation(work, DISPOSITION_LEDGER_NAME, "disposition_ledger", lambda row: row.__setitem__("missing_theorem_authority", [])))
        execute("A19_DISPOSITION_DSU_AUTH", lambda: coherent_ledger_mutation(work, DISPOSITION_LEDGER_NAME, "disposition_ledger", lambda row: row.__setitem__("DSU_edge_or_union_authorized", True)))
        execute("A20_DISPOSITION_FORMAL_CREDIT", lambda: coherent_ledger_mutation(work, DISPOSITION_LEDGER_NAME, "disposition_ledger", lambda row: row["formal_credit"].__setitem__("local_graph_side_physical_incidence", 1)))
        execute("A21_ROUTING_LEDGER_PHYSICAL_CREDIT", lambda: coherent_ledger_mutation(work, READY_LEDGER_NAME, "ready_local_theorem_ledger", lambda row: row.__setitem__("local_graph_side_physical_incidence_credit", 1)))
        execute("A22_ROUTING_LEDGER_TRACE_CREDIT", lambda: coherent_ledger_mutation(work, READY_LEDGER_NAME, "ready_local_theorem_ledger", lambda row: row.__setitem__("one_sided_trace_credit", 1)))
        execute("A23_ROUTING_LEDGER_DSU_CREDIT", lambda: coherent_ledger_mutation(work, READY_LEDGER_NAME, "ready_local_theorem_ledger", lambda row: row.__setitem__("DSU_edge_credit", 1)))
        execute("A24_ROUTING_TO_THEOREM", lambda: coherent_ledger_mutation(work, READY_LEDGER_NAME, "ready_local_theorem_ledger", lambda row: row.__setitem__("theorem_statement", "PROMOTED")))
        execute("A25_BLOCKED_PHYSICAL_CREDIT", lambda: coherent_ledger_mutation(work, BLOCKED_LEDGER_NAME, "blocked_ledger", lambda row: row.__setitem__("local_graph_side_physical_incidence_credit", 1)))
        execute("A26_BLOCKED_AUTHORITY_ERASURE", lambda: coherent_ledger_mutation(work, BLOCKED_LEDGER_NAME, "blocked_ledger", lambda row: row.__setitem__("missing_theorem_authority", [])))
        execute("A27_REROUTE_PERFORMED", lambda: coherent_ledger_mutation(work, REROUTE_LEDGER_NAME, "missing_local_shared_reroute_findings", lambda row: row.__setitem__("reroute_performed", True)))
        execute("A28_REROUTE_TRACE_CREDIT", lambda: coherent_ledger_mutation(work, REROUTE_LEDGER_NAME, "missing_local_shared_reroute_findings", lambda row: row.__setitem__("one_sided_trace_credit", 1)))
    finally:
        shutil.rmtree(work, ignore_errors=True)
    need(len(attacks) == 28, "attack count")
    body = {
        "schema": SCHEMA + ".coherent-attack-suite.v1",
        "status": "PASS_28_OF_28_COHERENT_ATTACKS_REJECTED",
...
def read_closed(path: Path, closure: str) -> tuple[dict[str, Any], bytes]:
    ...
    core = dict(value)
    claimed = core.pop(closure, None)
    need(type(claimed) is str and claimed == object_sha(core), "closed document closure:" + path.name)
    return value, raw


def verification_document(result: dict[str, Any], attack_raw: bytes | None = None, verifier_digest: str | None = None) -> dict[str, Any]:
    if attack_raw is None:
        ...
        core = dict(suite)
        claimed = core.pop("attack_suite_sha256", None)
        need(type(claimed) is str and claimed == object_sha(core), "held attack closure")
    ...
    need((suite["attack_count"], suite["rejected_count"], suite["all_rejected"]) == (28, 28, True), "attack suite census")
    body = {
        ...
        **result,
        ...
        "formal_credit_marker": ZERO_DOWNSTREAM,
    }
...
def manifest_first() -> dict[str, Any]:
    with ManifestBundle() as bundle:
        ...
        validate_candidate_payloads(
            bundle.payloads[RESULT_NAME],
            bundle.payloads[DISPOSITION_LEDGER_NAME],
            bundle.payloads[READY_LEDGER_NAME],
            bundle.payloads[BLOCKED_LEDGER_NAME],
            bundle.payloads[REROUTE_LEDGER_NAME],
        )
        expected = independently_reconstruct()
        expected_payloads = {
            RESULT_NAME: canonical(expected[0]),
        ...
        for filename in OUTPUT_NAMES:
            need(bundle.payloads[filename] == expected_payloads[filename], "manifest independently reconstructed:" + filename)
        records = {filename: CandidateRecord(filename, bundle.identities[filename][4], bundle.hashes[filename]) for filename in OUTPUT_NAMES}
        ...
        bundle.final()
    return {
        "status": "PASS_MANIFEST_FIRST_HELD_FD_10118_ROUTING_DISPOSITIONS__9950_CANDIDATE_READY__ZERO_WRITES_ZERO_THEOREM_CREDIT",
...
def main() -> int:
    ...
    if arguments.manifest_first_no_write:
        ...
        if arguments.attack_publish:
            ...
            result = {"status": suite["status"], "attack_count": 28, "attack_suite_sha256": suite["attack_suite_sha256"]}
        else:
