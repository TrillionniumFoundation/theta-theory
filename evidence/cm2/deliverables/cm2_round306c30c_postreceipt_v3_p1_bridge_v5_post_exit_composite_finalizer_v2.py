#!/usr/bin/env python3
"""Append-only v5 post-exit composite finalizer v2 (zero credit, CM2 NO-GO).
...
Even that filesystem marker is not authority by itself.  The append-only v2
CLI exposes a held-source ``--observe`` production mode which, after this
service exits, again binds the receipt's exact worker/finalizer units,
InvocationID, ExecStart, fragments, and READY tree and requires both terminal
success tuples.  The observer is read-only.  All objects remain zero-credit
and CM2 NO-GO.
"""
...
BASE_PIN_ROLES = frozenset({
    ...
    "bridge_worker_launcher", "bridge_finalizer_launcher",
...
def parse_pinset(
    ...
    for role, path in (
        ...
        launcher = observations[role]
        need(launcher.get("kind") == "regular"
             and launcher.get("path") == os.fspath(path)
             and stat.S_IMODE(launcher["stat9"]["mode"]) == 0o444,
             "pinset exact frozen launcher root:" + role)
    for role, prefix in (
...
def validate_executed_source_receipt_binding(
    receipt: Mapping[str, Any], executed_source: Mapping[str, Any],
) -> None:
    need(type(executed_source) is dict and set(executed_source) == {
             "path", "sha256", "stat9",
         } and executed_source.get("path") == os.fspath(SELF)
         and receipt.get("finalizer_source_sha256")
             == executed_source.get("sha256")
         and receipt.get("finalizer_source_stat9")
             == executed_source.get("stat9"),
         "joint executed finalizer source/receipt binding")
...
def validate_post_exit_joint_authority(
    ...
    try:
        ...
        validate_executed_source_receipt_binding(receipt, executed_source)
        need(anchor_file.sha256 == receipt["anchor_file_sha256"]
...
def observe_post_exit_joint_authority(
    bridge_path: Path, reverify_source: Callable[[], None],
) -> dict[str, Any]:
    """Run the production read-only joint gate after the finalizer exits."""
    bridge = HeldDir(bridge_path, "post-exit observer bridge")
    runtime = RuntimeAuthority()
    try:
        reverify_source()
        result = validate_post_exit_joint_authority(bridge, runtime)
        reverify_source()
        runtime.verify()
        bridge.verify()
        return result
...
def self_test() -> dict[str, Any]:
    ...
    stale_invocation = dict(post_fields)
    stale_invocation["InvocationID"] = "e" * 32
    attacks["observer_stale_invocation"] = rejected(
        lambda: need(post_exit_tuple_eligible(
            COMPOSITE_PASS_BYTES, receipt_fixture, stale_invocation,
            fragment_fixture,
        ), "observer stale InvocationID"),
        "observer stale InvocationID",
    )
    substituted_argv = list(argv)
    substituted_argv[-1] = "18"
    wrong_execstart = dict(post_fields)
    wrong_execstart["ExecStart"] = exec_raw(
        substituted_argv, pid=process_pid, running=False,
    )
    attacks["observer_execstart_substitution"] = rejected(
        lambda: need(post_exit_tuple_eligible(
            COMPOSITE_PASS_BYTES, receipt_fixture, wrong_execstart,
            fragment_fixture,
        ), "observer ExecStart substitution"),
        "observer ExecStart substitution",
    )
    for label, replacement in (
        ("path", os.fspath(fragment_path) + ".stale"),
        ("sha256", "e" * 64),
        ("stat9", {**expected_post["FragmentStat9"], "ino": 2}),
    ):
        changed_fragment = dict(fragment_fixture)
        changed_fragment[label] = replacement
        attacks["observer_fragment_" + label + "_substitution"] = rejected(
            lambda changed_fragment=changed_fragment, label=label: need(
                post_exit_tuple_eligible(
                    COMPOSITE_PASS_BYTES, receipt_fixture, post_fields,
                    changed_fragment,
                ), "observer fragment " + label + " substitution",
            ),
            "observer fragment " + label + " substitution",
        )

    executed_source_fixture = {
        "path": os.fspath(SELF), "sha256": source_sha,
        "stat9": file_stat_fixture,
    }
    validate_executed_source_receipt_binding(
        receipt_fixture, executed_source_fixture,
    )
    source_substitution_payload = dict(receipt_fixture)
    del source_substitution_payload["object_sha256"]
    substituted_source_sha = "e" * 64
    source_substitution_payload["finalizer_source_sha256"] = (
        substituted_source_sha
    )
    substituted_source_contract = dict(expected_post)
    substituted_source_contract["ExecStart"] = [
        substituted_source_sha if item == source_sha else item for item in argv
    ]
    source_substitution_payload["required_finalizer_post_exit"] = (
        substituted_source_contract
    )
    source_substitution_receipt = closed(source_substitution_payload)
    validate_composite_receipt(source_substitution_receipt)
    attacks["observer_source_substitution"] = rejected(
        lambda: validate_executed_source_receipt_binding(
            source_substitution_receipt, executed_source_fixture,
        ),
        "observer source substitution",
    )
    need(set(attacks) == {
        ...
        "observer_stale_invocation", "observer_execstart_substitution",
        "observer_fragment_path_substitution",
        "observer_fragment_sha256_substitution",
        "observer_fragment_stat9_substitution",
        "observer_source_substitution",
    ...
    return closed({
        ...
        "status": "PASS_17_COMPOSITE_FINALIZER_NEGATIVES_ZERO_CREDIT",
        ...
        "observer_stale_invocation_rejected": True,
        "observer_ExecStart_substitution_rejected": True,
        "observer_fragment_path_sha_stat_substitutions_rejected": True,
        "observer_source_substitution_rejected": True,
        "filesystem_marker_authoritative": False,
...
def main_arguments(
    ...
    try:
        ...
        if arguments.observe:
            need(arguments.bridge_dir is not None
                 and arguments.pinset is None and arguments.anchor is None
                 and arguments.worker_unit is None
                 and arguments.finalizer_unit is None,
                 "observer accepts only exact bridge root")
            result = observe_post_exit_joint_authority(
                arguments.bridge_dir, release_source,
            )
        ...
        if arguments.observe:
            failure = closed({
                ...
                "status": "BLOCKED_POST_EXIT_JOINT_GATE_FAIL_CLOSED",
                "error_type": type(error).__name__, "error": str(error),
                "filesystem_marker_authoritative_alone": False,
                "joint_gate_observed": False,
                "formal_credit": 0, "source_W_formal_remainder": 80,
                "source_W_transition_authorized": False,
