#!/usr/bin/env python3
"""Dual-seed, attacks, cold replay and input-stability runner for C30q9."""
...
SOURCE_PINS = {
    PRODUCER: "e18c4c403516aec5d76b930baa1a6e8c191d46bd8dc1b6973fa58fa1484efc4d",
    VERIFIER: "2b6e49ffd8fcaa0f3fd307bfdc539a99b2ac62fd8de0103a44a204cdff331155",
    ATTACK: "797ddc5d47be127f5f025d6f177aab49939fb4b63d4b56c765120d4f3fb20fe9",
}
RUNTIME_PINS = {
...
Q8_AUDIT = ROOT / ".cm2-runtime/audit/c30q8-full-54-global-template-zero-credit-v1-20260808T112500Z"
MANIFESTS = {Q8_AUDIT / "manifest.sha256": Q8_AUDIT}
DIRECT_INPUTS = {
    Q8_AUDIT / "receipt.json",
    Q8_AUDIT / "producer_seed30880079_stdout.json",
...
def manifest_members(manifest: Path, base: Path) -> set[Path]:
    ...
    for row in manifest.read_text("ascii").splitlines():
        ...
        need(all(char in "0123456789abcdef" for char in expected)
             and not part.is_absolute() and ".." not in part.parts and relative not in seen,
             "manifest row")
        seen.add(relative)
        workspace = ROOT / part
...
def tracked_inputs() -> list[Path]:
    ...
    paths: set[Path] = {runner, PRODUCER, VERIFIER, ATTACK, *DIRECT_INPUTS, *RUNTIME_PINS}
...
def snapshot(paths: list[Path]) -> tuple[bytes, bytes]:
    ...
    for path in paths:
        ...
        stats.append({"path": relative, "fields": list(FIELDS), "identity": list(before)})
...
def run_stage(output: Path, name: str, command: list[str], seed: int,
              cold: bool = False, timeout: int = 300) -> dict[str, Any]:
    environment = {"PATH": "/usr/bin:/bin", "LANG": "C", "LC_ALL": "C",
                   "PYTHONDONTWRITEBYTECODE": "1", "PYTHONHASHSEED": str(seed)}
    start = time.monotonic()
    if cold:
        with tempfile.TemporaryDirectory(prefix="cm2-c30q9-cold-") as directory:
    ...
    return {"numeric_exit_code": numeric_exit, "signal": signal_value,
            "stdout": process.stdout, "stderr": process.stderr}


def verifier_no_import() -> dict[str, Any]:
    ...
    need(PRODUCER.stem not in imported and "importlib" not in imported
         and "subprocess" not in imported, "no producer import/execute verifier")
    return {"AST_parsed": True, "producer_module_imported": False,
            "producer_executed": False, "importlib_used": False, "subprocess_used": False}
...
def main(argv: list[str]) -> int:
    ...
    try:
        ...
        provenance = {
            "schema": "cm2.round306c30q9.audit-provenance.zero-credit.v1",
            "python_invocation": str(PYTHON), "python_resolved": str(PYTHON_RESOLVED),
            "python_sha256": RUNTIME_PINS[PYTHON_RESOLVED], "python_flint_version": "0.9.0",
            "real_hash_seeds": list(SEEDS), "cold_hash_seed": COLD_SEED,
            "source_sha256": {str(path.relative_to(ROOT)): expected
                              for path, expected in SOURCE_PINS.items()},
            "runtime_sha256": {str(path): expected for path, expected in RUNTIME_PINS.items()},
        ...
        producers = [run_stage(output, f"producer_seed{seed}",
                               [str(PYTHON), "-B", "-s", str(PRODUCER)], seed)
                     for seed in SEEDS]
        need(all(row["numeric_exit_code"] == 0 and row["signal"] is None
                 and row["stderr"] == b"" for row in producers), "dual producer controls")
        need(producers[0]["stdout"] == producers[1]["stdout"], "dual producer byte identity")
        candidate = strict_canonical_json(producers[0]["stdout"], "producer")
        need(candidate["result_sha256"] == EXPECTED_RESULT_SHA256, "expected result")

        verifiers = [run_stage(output, f"verifier_seed{seed}",
                               [str(PYTHON), "-B", "-s", str(VERIFIER),
                                str(output / f"producer_seed{seed}_stdout.json")], seed)
                     for seed in SEEDS]
        need(all(row["numeric_exit_code"] == 0 and row["signal"] is None
                 and row["stderr"] == b"" for row in verifiers), "dual verifier controls")
        need(verifiers[0]["stdout"] == verifiers[1]["stdout"], "dual verifier byte identity")
        verification = strict_canonical_json(verifiers[0]["stdout"], "verifier")

        attacks = run_stage(output, "coherent_attacks",
        ...
        need(attacks["numeric_exit_code"] == 0 and attacks["signal"] is None
             and attacks["stderr"] == b"", "attack controls")
        attack_result = strict_canonical_json(attacks["stdout"], "attacks")
        need(attack_result["attack_count"] == attack_result["rejected"] == 44,
             "44 coherent attacks")

        cold = run_stage(output, "cold_replay",
                         [str(PYTHON), "-B", "-s", str(VERIFIER),
                          str(output / f"producer_seed{SEEDS[1]}_stdout.json")],
                         COLD_SEED, cold=True)
        need(cold["numeric_exit_code"] == 0 and cold["signal"] is None
             and cold["stderr"] == b"" and cold["stdout"] == verifiers[0]["stdout"],
             "cold replay byte identity")
        ...
        write(output / "PASS_ZERO_CREDIT.lock",
              b"PASS_C30Q9_FULL_PHYSICAL_FIRST_HIT_DISPOSITION_ZERO_CREDIT_PRE_TERMINAL\n")
        write(output / "runner_status.json", canonical({
            "schema": "cm2.round306c30q9.runner-status.zero-credit.v1",
            "status": "PASS_ALL_STAGES_ZERO_CREDIT_PRE_TERMINAL",
        ...
        payload = {
            "schema": "cm2.round306c30q9.full-physical-first-hit-disposition-receipt.zero-credit.v1",
            "status": (
                "PASS_C30Q9_DUAL_SEED_FULL_PHYSICAL_FIRST_HIT_DISPOSITION__"
                "44_EXCLUDED__10_RESOLVED_MIXED__44_ATTACKS__ZERO_CREDIT_PRE_TERMINAL"
            ),
            "candidate_result_sha256": candidate["result_sha256"],
            "producer": {"real_hash_seeds": list(SEEDS),
            ...
            "independent_verifier": {"real_hash_seeds": list(SEEDS),
                                     "dual_seed_stdout_byte_identical": True,
                                     "both_numeric_exit_zero": True, "both_signals_null": True,
            ...
            "coherent_attacks": {"numeric_exit_zero": True, "signal_null": True,
                                 "stderr_empty": True, "total": 44, "rejected": 44,
                                 "mandatory_coverage": attack_result["mandatory_coverage"]},
            "cold_replay": {"isolated_working_directory": True, "minimal_environment": True,
                            "hash_seed": COLD_SEED, "numeric_exit_zero": True,
                            "signal_null": True, "stderr_empty": True,
                            "byte_identical_to_dual_verifier": True},
            "input_stability": {"tracked_input_count": len(paths), "stat_field_count": 9,
            ...
            "research_classification": {"total_origins": 54,
            ...
            "future_C30f_handoff_candidate": candidate["result"]["future_C30f_gated_handoff_candidate"],
        ...
        if output is not None and output.exists():
            failure = {"schema": "cm2.round306c30q9.failure.zero-credit.v1",
            ...
            if not (output / "FAILED.lock").exists():
                (output / "FAILED.lock").write_bytes(b"FAILED_CLOSED_C30Q9\n")
        sys.stderr.write("REJECT_C30Q9_RUNNER:" + str(error) + "\n")
        return 2
