#!/usr/bin/env python3
"""Produce the append-only v2 C30e reduced-live source-W candidate.
...
The sole ledger predecessor is a future versioned C30d terminal graph.  It is
consumed through an external, dynamically SHA-pinned adapter; no legacy
``sealed`` alias is accepted.  The adapter must validate the terminal receipt,
payload/root manifests, outer verifier and terminal replay, then emit one
canonical projection with an exact 78-to-58 transition.  Missing or incomplete
terminal authority rejects before a candidate directory is created.

Every result from this program is deliberately candidate-only and has zero
formal credit.  A later release chain must still supply dual-seed process
attestations, independent verification, attacks, cold/TOCTOU replay, evidence,
payload/root manifests, an outer verifier and a terminal byte replay before the
58-to-56 transition can be authorized.
"""
...
def bootstrap_validate_runtime() -> None:
    """Reject fake ``-I`` hash-seed runs before reusable mathematics loads."""
...
def _disabled_legacy_capture_c30d_authority() -> dict[str, Any]:
    """Append-only tombstone: the legacy sealed alias can never be used."""
    raise BootstrapFailure("LEGACY_C30D_SEALED_ALIAS_DISABLED_IN_V2")
...
def _validate_c30d_projection(
    ...
    need(
        ...
        "C30d adapter exact projection schema/status/object closure",
    )
    authority = value.get("authority")
    need(type(authority) is dict, "C30d projection authority object")
    ...
    need(
        ...
        and all(_valid_sha256(item) for item in members.values())
    ...
    need(
        ...
        "C30d exact terminal 78-to-58 handoff",
    )
    need(
        ...
        "C30d projection exact C30e zero-credit release boundary",
    )
    assert type(members) is dict
    return {name: members[name] for name in sorted(members)}


def capture_c30d_authority_v2(
    adapter_path: Path,
    adapter_sha256: str,
    chain_path: Path,
    pins_path: Path,
    ...
    """Run the SHA-pinned adapter, then independently recapture its graph."""
    need(
        _valid_sha256(adapter_sha256) and _valid_sha256(pins_sha256),
        "well-formed dynamic C30d adapter/pins SHA256",
    )
    ...
    need(
        adapter != pins and adapter.name != pins.name,
        "distinct C30d adapter and dynamic-pins identities",
    )
    capture_regular_authority_file(adapter, adapter_sha256, 8 << 20)
    capture_regular_authority_file(pins, pins_sha256, 4 << 20)
    try:
    ...
    projection = strict_object_bytes(
        completed.stdout[:-1], "C30d adapter projection"
    )
    capture_regular_authority_file(adapter, adapter_sha256, 8 << 20)
    capture_regular_authority_file(pins, pins_sha256, 4 << 20)
    member_sha256 = _validate_c30d_projection(projection, chain, pins_sha256)
    captured_member_facts: dict[str, dict[str, Any]] = {}
    for name in C30D_TERMINAL_REQUIRED_MEMBERS:
        raw = capture_regular_authority_file(
            chain / name, member_sha256[name], 64 << 20
        )
        captured_member_facts[name] = {
            "sha256": hashlib.sha256(raw).hexdigest(),
            "size": len(raw),
        }
    need(
        _identity(directory_before) == _identity(chain.lstat()),
        "C30d terminal graph directory stable across adapter capture",
    )
    pins_map = {
        "external-adapter:" + adapter.name: adapter_sha256,
        "dynamic-pins:" + pins.name: pins_sha256,
        **{
            "c30d-terminal:" + name: member_sha256[name]
            for name in C30D_TERMINAL_REQUIRED_MEMBERS
        },
    }
    return {
        ...
        "pins_sha256": digest([
            {"filename": name, "sha256": value}
            for name, value in sorted(pins_map.items())
        ]),
        "adapter_sha256": adapter_sha256,
        "dynamic_pins_sha256": pins_sha256,
        "chain_dir_basename": chain.name,
        "projection_sha256": projection["projection_sha256"],
        "terminal_member_facts": captured_member_facts,
...
def inherited_same_sign_delta_followup_h_binding(
    ...
    """Bind the reduced-live lane's LIVE coarse cell to its H followup."""
    need(
        ...
        "reduced-live same-sign Delta followup-H evidence",
    )
    return {
...
def build(
    ...
    c30d_authority = capture_c30d_authority_v2(
        adapter_path,
        adapter_sha256,
        chain_path,
        pins_path,
...
def main() -> int:
    ...
    except (BootstrapFailure, OSError, ValueError, TypeError, KeyError,
