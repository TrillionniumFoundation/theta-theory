...
\large Marked source kernels, global incidence Cauchy, parent-kernel tilts,
two-cutoff face convergence, and a strongly conditional
pathwise CM2 criterion}
\author{Working note}
\date{13 July 2026 (version 41)}
...
marked source space.  A finite positive kernel over each complete-tree label
records its true envelope mass, and a normalized weakly measurable source
section records its carrier.  The assembled current is defined only through
the resulting scalar weak pairing integral.  Thus continuous tree labels are
integrated rather than nakedly summed, while two distinct currents with
identical support are still counted twice by their occurrence marks.  Tonelli
controls the positive envelope before signed recombination.  Source-incidence
...
clock: a separate energy-lift theorem derives its value tail from parentwise
PPE/KRH, total incidence mass, and this one global Cauchy step.  Only genuine
...
$X_i$.  On $B_i$ require a measurable scalar source mass $q_i(\lambda)<
\infty$ and a weakly measurable normalized section
$\nu_{i,\lambda}\in\mathcal P(X_i)$ representing an element of the positive
unit ball of $\mathfrak S_i^{\varepsilon_i}$.  The exact discarded current is
...
\label{eq:exact-stopped-source-factorization}
 ...
 \int_{B_i}\int_{X_i}
   ...
   \,d\nu_{i,\lambda}(x)\,q_i(\lambda)\,d\Pi_{s,N}(\lambda),
 \qquad |\epsilon_i|\le1.
\end{equation}
Thus $q_i$ is a density on the tree-label space while
$\nu_{i,\lambda}$ is the source on its physical carrier; no Radon--Nikodym
identity equates measures living on these different spaces.  Labelwise,
\begin{equation}
\label{eq:cemetery-fixed-source-norm}
 \|q_i(\lambda)\nu_{i,\lambda}\|_{
       \mathfrak S_i^{\varepsilon_i}}\le q_i(\lambda).
\end{equation}
...
\label{eq:cemetery-kernel-domination}
 q_i(\lambda)\mathscr A_i^{\rm opp}(\lambda)
 \gamma_i^{-R_i(\lambda)}
 \le C\mathscr W_i^{\rm cem}(\lambda)
...
a positive total-variation contraction.  Otherwise its recovery envelope is
frozen in $q_i$ or $\mathscr W_i^{\rm cem}$ before the cemetery transition.
...
orientation, $q_i,\nu_{i,\lambda},R_i,U_i$, and opposite-side amplitude.
...
test.  Normalize the positive section, propagate it, and multiply the scalar
mass back.  No signed cancellation or cross-space Radon--Nikodym formula is
...
\begin{definition}[Marked finite source kernel
$\mathrm{MARKED\_SOURCE\_KERNEL}$]
...
of the standard-family and flux carriers.  There is a jointly measurable
weight $w(\lambda,a)\ge0$ on the fixed marked product, supported on a Borel
countable-fibre set $I\subset\Lambda\times Y_{\rm src}$, and
\begin{equation}
\label{eq:marked-weighted-counting-kernel}
 q_\lambda(da):=\sum_{a\in I(\lambda)}w(\lambda,a)\delta_a(da),
 \qquad \sum_{a\in I(\lambda)}w(\lambda,a)<\infty.
\end{equation}
Thus $q_\lambda$ is a finite positive kernel on the fixed standard-Borel
space: $\lambda\mapsto q_\lambda(A)$ is measurable for every Borel $A$.
...
The positive marked source measure is
\begin{equation}
\label{eq:source-incidence-space}
 dM_{\rm src}(\lambda,a,x):=
 d\Pi_{s,N}(\lambda)\,q_\lambda(da)\,d\nu_{\lambda,a}(x).
\end{equation}
The atomic weight is the true envelope mass and obeys
\begin{equation}
\label{eq:source-incidence-envelope}
 w(\lambda,a)\ge
 \|\Sigma_{\lambda,a}\|_{\mathfrak S_a^{\varepsilon(a)}}
 \gamma_a^{-R_{\lambda,a}}\mathscr A_{\lambda,a}^{\rm opp}
\end{equation}
for every active occurrence.  For every admissible test define
the assembled current only through the weak scalar integral
\begin{equation}
\label{eq:marked-source-assembled-pairing}
 \langle J_{\rm src},\Phi\rangle:=
 \int \epsilon(\lambda,a,x)
       [U_{\lambda,a}^{*}\Phi](x)\,dM_{\rm src}(\lambda,a,x),
 \qquad |\epsilon|\le1.
\end{equation}
The integrand is required measurable and absolutely integrable for every
$\Phi$ in the fixed test unit ball.  Hence this is a weak/Gelfand integral
in that test-space dual; no Bochner measurability is presumed.  Tonelli gives
\begin{equation}
\label{eq:source-incidence-TV-bound}
 |\langle J_{\rm src},\Phi\rangle|
 \le C\|\Phi\|_{\rm test}
       \int q_\lambda(Y_{\rm src})\,d\Pi_{s,N}(\lambda).
\end{equation}
This replaces every naked sum over continuous labels.  It is also the precise
meaning of the former source-incidence interface.  Probability first-hit
disjointization is used only for survivor unions and never deletes a marked
occurrence.
\end{definition}
...
Assume the source-parent version of $\mathrm{KRH}_{p_W}$ and the parentwise
probability energy estimate of Theorem~\ref{thm:direct-slope-small-ball}.
Suppose
\begin{equation}
...
\end{equation}
Let $\Theta_{{\rm main},r}^{\rm par}$ and
$c_{{\rm main},{\rm PPE}}^{\rm par}$ be the uniform parent probability
energy rates furnished by $\mathrm{TENERGY}_{W}+\mathrm{KRH}_{p_W}$.
Then Tonelli over the marked kernel gives
\begin{align}
 \mathcal E_{{\rm main},r}(r)&\le
 Cr^{\Theta_{{\rm main},r}^{\rm par}-a_{{\rm main},r}^{B}},
 &
 \mathcal E_{{\rm main},{\rm PPE}}(N)&\le
 Ce^{-(c_{{\rm main},{\rm PPE}}^{\rm par}
...
derived from probability energy, source-parent KRH, finite base mass, and one
global incidence Cauchy step; it is not assumed in
$\mathrm{CLOCK\_SRC}_{W}$.
\end{theorem}

\begin{proof}
Inside every frozen source parent, apply Theorem
\ref{thm:direct-slope-small-ball} after the normalized likelihood transfer
given by $\mathrm{KRH}_{p_W}$.  Integrate the resulting fibre energy against
$q_\lambda(da)d\Pi(\lambda)$.  Tonelli produces the base-mass factor in
...
Assume the main-source base bounds \eqref{eq:main-source-base-mass} and use
...
\[
 ...
 \mathrm{CEM\_DOM},\\
 \mathrm{TENERGY}_{W},\ \mathrm{MARKED\_SOURCE\_KERNEL},\\
 \mathrm{CLOCK\_PROB}_{W}
 \end{gathered}
\]
\[
...
\[
 ...
 \mathrm{MARKED\_SOURCE\_KERNEL},\ \mathrm{CLOCK\_PROB}_{W},\\
 \mathrm{CLOCK\_SRC}_{W},\ \mathrm{KRH}_{p_W},\
 \mathrm{USC}_{\rm end}^{W}(q_g^W)
 \end{gathered}
\]
...
the positive marked measure $M_{\rm src}$ of
...
\item $\mathrm{NST}_{\rm phys}$, $\mathrm{SS}(d_Z)$, and
      ...
      \]
      The marked kernel satisfies the main-source base bounds
...
There are twenty-two falsifiable primitive interfaces.  $\mathrm{MT}_{\rm DQ}$ must
...
$\mathrm{MARKED\_SOURCE\_KERNEL}$ must measurably retain every additive
occurrence even on overlapping supports and assemble it by scalar weak
integration.  Theorem~\ref{thm:main-source-energy-lift} then derives the main
marked value mass from parent energy, the declared base-mass bounds, and one
global incidence Cauchy step; it is not a primitive interface or a row of
the source clock.  $\mathrm{CLOCK\_PROB}_{W}$
...
\begin{proposition}[Forty-three obstruction tests]
\label{prop:v41-obstruction-tests}
...
\begin{proof}
Items 1--3, 8--11, and 13--43 are direct finite-measure, frequency, or norm
