#!/usr/bin/env python3
"""Independent fail-closed verifier for Round183.

This file neither imports nor executes the Round183 producer.  It rebuilds
the complete bounded refinement and approximately 53 MB expected result from
the frozen Round181 chain, then requires full canonical equality.  Semantic
mutations are re-signed, so rejection cannot rely on a frozen result digest.
"""
...
CERTIFICATE = HERE / "cm2_round183_residual_face_bracket_refinement_certificate.json"
OUTPUT = HERE / "cm2_round183_residual_face_bracket_refinement_verification.json"
PRODUCER = "cm2_round183_residual_face_bracket_refinement.py"
SCHEMA = "cm2.round183.residual-face-bracket-refinement.v1"
VERIFICATION_SCHEMA = "cm2.round183.residual-face-bracket-refinement-verification.v1"
PRODUCER_SHA256 = "ac0e786146df08e1c8695bd9a1cfdbe0bed290d2fd77960f61ffa9e0785133ed"
...
def strict_json(raw: bytes, label: str,
    ...
    try:
        ...
        raise Round183Error(f"JSON:{label}:{exc}") from exc
    validate_tree(value)
...
def graph_records(row: dict[str, Any]) -> list[dict[str, Any]]:
    record = row.get("face_bracket_or_graph_record")
    if not record:
        return []
    if "candidate_graphs" in record:
        return record["candidate_graphs"]
    if "incumbent_graph" in record:
        return [record["incumbent_graph"]]
    if "outgoing_graph" in record:
        return [record["outgoing_graph"]]
    return []
...
def semantic_attacks(certificate: dict[str, Any],
    ...
    def all_nominal_graphs_nonempty(result: dict[str, Any]) -> None:
        result["face_bracket_dimension_ledger"][
            "certified_nonempty_regular_2D_graph_carriers"] = 25158

    def false_graph_without_bracket(result: dict[str, Any]) -> None:
        row = next(
            item for item in result["dynamic_residual_ledger"]["rows"]
            if any(not graph["nonempty_2D_graph_certified"]
                   for graph in graph_records(item))
        )
        graph = next(graph for graph in graph_records(row)
                     if not graph["nonempty_2D_graph_certified"])
        graph["nonempty_2D_graph_certified"] = True
        resign_row(row)

    def delete_strict_bracket(result: dict[str, Any]) -> None:
        row = next(
            item for item in result["dynamic_residual_ledger"]["rows"]
            if any(graph["nonempty_2D_graph_certified"]
                   for graph in graph_records(item))
        )
        graph = next(graph for graph in graph_records(row)
                     if graph["nonempty_2D_graph_certified"])
        graph["strict_opposite_sign_boundary_bracket"] = None
        resign_row(row)

    def mutate_bracket_sign(result: dict[str, Any]) -> None:
        row = next(
            item for item in result["dynamic_residual_ledger"]["rows"]
            if any(graph["nonempty_2D_graph_certified"]
                   for graph in graph_records(item))
        ...
        graph = next(graph for graph in graph_records(row)
                     if graph["nonempty_2D_graph_certified"])
        bracket = graph["strict_opposite_sign_boundary_bracket"]
        bracket["negative_side_chart_or_owner"] = (
            bracket["positive_side_chart_or_owner"]
        )
        resign_row(row)

    def outgoing_without_bracket(result: dict[str, Any]) -> None:
        row = next(item for item in result["dynamic_residual_ledger"]["rows"]
                   if item["status"] == "OUTGOING_H2")
        graph = row["face_bracket_or_graph_record"]["outgoing_graph"]
        graph["nonempty_2D_graph_certified"] = True
        resign_row(row)

    def close_pairwise(result: dict[str, Any]) -> None:
        result["face_bracket_dimension_ledger"][
            "certified_unique_pairwise_1D_incidences"] = 118

    def close_zero_dimensional(result: dict[str, Any]) -> None:
        result["face_bracket_dimension_ledger"][
            "certified_0D_multiple_incidence_points"] = 1

    def promote_dimension_credit(result: dict[str, Any]) -> None:
        result["face_bracket_dimension_ledger"][
            "two_or_one_or_zero_dimensional_integer_credit"] = 1

    def restore_wall_graphs(result: dict[str, Any]) -> None:
        result["face_bracket_dimension_ledger"][
            "nominal_wall_2D_graph_carriers"] = 168

    def edge_bracket_count(result: dict[str, Any]) -> None:
        result["face_bracket_dimension_ledger"]["strict_edge_brackets"] = 4876

    def diagonal_bracket_count(result: dict[str, Any]) -> None:
        result["face_bracket_dimension_ledger"][
            "strict_diagonal_brackets"] = 3594

    def exact_row_global(result: dict[str, Any]) -> None:
        row = result["new_local_exact_key_ledger"]["rows"][0]
        row["local_exact_key_is_global_disposition"] = True
        row["whole_parent_or_stratum_credit"] = 1
        resign_row(row)
        block = result["new_local_exact_key_ledger"]
        block["rows_sha256"] = digest(block["rows"])

    def exact_ledger_global(result: dict[str, Any]) -> None:
        result["new_local_exact_key_ledger"]["global_Gate5_dispositions"] = 1

    def exact_count(result: dict[str, Any]) -> None:
        result["new_local_exact_key_ledger"]["row_count"] = 17377

    def exact_volume(result: dict[str, Any]) -> None:
        result["new_local_exact_key_ledger"]["exact_coordinate_volume"] = "0"

    def residual_volume(result: dict[str, Any]) -> None:
        result["dynamic_residual_ledger"]["exact_coordinate_volume"] = "0"
    ...
    def inherited_count(result: dict[str, Any]) -> None:
        result["inherited_collision1_and_source_seam_replay"][
            "row_count"] = 529

    def inherited_digest(result: dict[str, Any]) -> None:
        result["inherited_collision1_and_source_seam_replay"][
            "rows_sha256"] = "0" * 64
    ...
    def duplicate_one_dimensional(result: dict[str, Any]) -> None:
        result["inherited_collision1_and_source_seam_replay"][
            "one_dimensional_Delta_or_H_intersections_counted_once"] = 5
    ...
    def full_one_parent(result: dict[str, Any]) -> None:
        block = result["per_parent_partial_ledger"]
        row = block["rows"][0]
    ...
    def point_global(result: dict[str, Any]) -> None:
        result["strict_nonpromotion"][
            "observed_point_289591_global_disposition"] = 1

    def inner_global(result: dict[str, Any]) -> None:
        result["strict_nonpromotion"][
            "inner_or_refined_box_290575_global_disposition"] = 1

    def status_full(result: dict[str, Any]) -> None:
        result["status"] = "PASS_FULL_COLLISION2_CLOSURE"

    attacks.extend([
        ("all_nominal_graphs_forged_nonempty", all_nominal_graphs_nonempty),
        ("unbracketed_graph_forged_nonempty", false_graph_without_bracket),
        ("certified_graph_strict_bracket_deleted", delete_strict_bracket),
        ("strict_bracket_side_sign_mutated", mutate_bracket_sign),
        ("outgoing_H2_forged_nonempty_without_bracket",
         outgoing_without_bracket),
        ("118_pairwise_outers_forged_unique", close_pairwise),
        ("zero_dimensional_point_forged", close_zero_dimensional),
        ("lower_dimensional_integer_credit_forged", promote_dimension_credit),
        ("168_closed_wall_inputs_forged_back_to_graphs", restore_wall_graphs),
        ("edge_bracket_count_mutated", edge_bracket_count),
        ("diagonal_bracket_count_mutated", diagonal_bracket_count),
        ("local_exact_row_forged_global", exact_row_global),
        ("local_exact_ledger_forged_global", exact_ledger_global),
        ("local_exact_count_mutated", exact_count),
        ("local_exact_volume_mutated", exact_volume),
        ("dynamic_residual_volume_mutated", residual_volume),
        ("volume_conservation_mutated", conservation),
        ("inherited_replay_count_mutated", inherited_count),
        ("inherited_replay_digest_mutated", inherited_digest),
        ("E_owned_source_seam_changed", source_seam_owner),
        ("one_dimensional_incidence_duplicated", duplicate_one_dimensional),
        ("sixteen_strata_forged_full", full_sixteen),
        ("one_parent_forged_full", full_one_parent),
        ("D02_forged_pass", d02_pass),
        ("Gate5_forged_18_of_18", gate5_full),
        ("point_289591_promoted", point_global),
        ("inner_290575_promoted", inner_global),
    ...
    for label, mutate in attacks:
        ...
        except Round183Error:
            rejected.append(label)
        else:
            raise Round183Error(f"semantic attack accepted:{label}")
    return rejected
...
def json_attacks(certificate: dict[str, Any]) -> list[str]:
    ...
    for label, raw in cases.items():
        try:
            ...
            raise Round183Error(f"JSON attack accepted:{label}")
    return rejected
...
def path_attacks() -> list[str]:
    rejected = []
    with tempfile.TemporaryDirectory(dir=HERE, prefix=".round183-path-") as name:
        root = Path(name)
        ...
        try:
            for label, operation in cases:
                try:
                    operation()
                except (Round183Error, OSError):
                    rejected.append(label)
                else:
                    raise Round183Error(f"path attack accepted:{label}")
        finally:
...
def main() -> int:
    ...
    result = {
        ...
        "independent_reconstruction": {
            "Round181_input_parent_box_count": 3812,
            "dynamic_input_terminal_box_count": 3282,
            "dynamic_input_exact_coordinate_volume":
                "338601/838860800000",
            "new_local_exact_key_box_count": 17376,
            "new_local_exact_key_exact_coordinate_volume":
                "6201903/26843545600000",
            "dynamic_residual_terminal_box_count": 25040,
            "dynamic_residual_exact_coordinate_volume":
                "4633329/26843545600000",
            "inherited_collision1_or_source_carrier_box_count": 530,
            "combined_residual_box_count": 25570,
            "combined_residual_exact_coordinate_outer_volume":
                "697725681/26843545600000",
            "combined_nominal_2D_graph_carriers": 25158,
            "certified_nonempty_regular_2D_graph_carriers": 8468,
            "strict_edge_brackets": 4875,
            "strict_diagonal_brackets": 3593,
            "nominal_pairwise_1D_incidence_outers": 118,
            "certified_unique_pairwise_1D_incidences": 0,
            "collision2_fully_closed_live_strata": 0,
            "collision2_partial_live_strata": 16,
            "integer_census_delta": 0,
        ...
        "verifier_provenance": {
            ...
            "effective_Arb_precision_bits": atlas.ctx.prec,
            "precision_source":
                "pinned Round181/Round178/registry transitive import chain",
        },
    }
    envelope = {
        "schema": VERIFICATION_SCHEMA,
