#!/usr/bin/env python3
"""C79g v12 append-only zero-credit candidate builder.

This append-only v12 binds the independently verified C78s 13-member final
...
is permanently non-authoritative.  The separately committed v12 seal plus the
...
COLD_COORDINATION_PARENT_FD_ENV = "CM2_C79G_V12_COORDINATION_PARENT_FD"
...
SELF = OUT / "cm2_round306c79g_true_global_no_producer_consumer_v12.py"
...
STATIC_AUDIT_V12 = OUT / "cm2_round306c79g_true_global_no_producer_consumer_static_audit_v12.json"
COLD_LAUNCHER = OUT / "cm2_round306c79g_true_global_no_producer_consumer_cold_launch_v12.py"
COLD_MANIFEST = OUT / "cm2_round306c79g_true_global_no_producer_consumer_cold_launch_manifest_v12.sha256"
COLD_OUTER = OUT / "cm2_round306c79g_true_global_no_producer_consumer_cold_launch_outer_receipt_v12.json"
...
FINAL_V12_CORE_PINS_INSTALLED = True
...
COLD_EXACT8 = (
    ...
    CONTRACT,
    SELF,
    OUT / "cm2_round306c79g_true_global_no_producer_consumer_independent_verifier_assembler_authority_consumer_v12.py",
...
def _linux_libc() -> ctypes.CDLL:
    need(sys.platform.startswith("linux"), "Linux-only v12 publication protocol")
...
class HeldSelf:
    ...
    def __init__(self, exec_inherited: int, source_inherited: int,
        ...
        need(set(incident_inherited) == set(expected_incident_pins),
             "exact five inherited incident fd roles")
        ...
        # nine inherited descriptors; no workspace pathname has been read yet.
        ...
        need(stat.S_ISREG(self.exec_before.st_mode) and
             ...
             len({(value.st_dev, value.st_ino)
                  for value in self.incident_before.values()}) == 5,
             "sealed exec/source/coordination/root plus five incident fds first gate")
        ...
        need(self.exec_raw == self.source_raw and
             ...
             "sealed exec/source plus five pinned incident fds stable first gate")
        ...
        need(SELF == OUT / "cm2_round306c79g_true_global_no_producer_consumer_v12.py",
    ...
    def execution_proof(self) -> dict[str, Any]:
        return {
            ...
            "five_incident_authority_inputs_inherited_as_held_fds": True,
            "five_incident_held_fds_path_identity_mount_and_hash_revalidated": True,
        }
    ...
    def terminal_replay(self) -> None:
        ...
        need(exec_replay == source_replay == self.exec_raw == self.source_raw and
             ...
             "sealed exec/source plus five incident held-fd terminal replay")
...
def new_candidate_stage(outdir: Path, parent: HeldParent) -> tuple[Path, HeldMutableStage]:
    ...
    name = ".c79g-v12-candidate-stage-" + orientation + "-" + CHECKPOINT_OBJECT_PIN
...
def strict_bool_predecessor_and_successor_census(
        ...
        v12_sources: Mapping[str, bytes]) -> dict[str, Any]:
    """Prove frozen v5 defects and zero-unsafe complete v6/v7/v12 boxes."""
    ...
    v12 = {role: _strict_bool_need_census(v12_sources[role], role)
          for role in v12_roles}
    ...
    v12_unproved = sum(
        v12[role]["recursive_exact_bool_unproved_count"] for role in v12_roles)
    v12_arity = sum(
        v12[role]["arity_star_or_keyword_failure_count"] for role in v12_roles)
    v12_returns = sum(
        v12[role]["annotated_bool_return_failure_count"] for role in v12_roles)
    need(v6_unproved == 0 and v6_arity == 0 and v6_returns == 0 and
         v7_unproved == 0 and v7_arity == 0 and v7_returns == 0 and
         v12_unproved == 0 and v12_arity == 0 and v12_returns == 0,
         "v6/v7/v12 recursive exact-bool full boxes have zero unsafe or unproved sites")
    return {
        ...
        "same_seven_absent_from_v12": True,
        ...
        "v12_recursive_exact_bool_unproved_count": v12_unproved,
        ...
        "v12_direct_need_call_count": sum(
            v12[role]["direct_need_call_count"] for role in v12_roles),
...
def v6_held_self_identity_defect_regression(raw: bytes) -> dict[str, Any]:
    ...
    V12 first proves the complete three-site census, then independently locates
...
def _validate_v7_regression_section(
    ...
    need(strict_bool_census["v7_recursive_exact_bool_unproved_count"] == 0 and
         strict_bool_census["v12_recursive_exact_bool_unproved_count"] == 0 and
         strict_bool_census["all_twelve_sources_AST_parsed_and_compiled_in_memory"]
             is True and
         strict_bool_census["compiled_code_executed"] is False,
         label + ":v7/v12 full-callsite strict-bool closure")
    need(section.get("all_ten_file_pins_match") is True and
         section.get("all_declared_object_pins_match") is True and
         section.get("all_ten_regular_0444_nlink1") is True and
         section.get("exact8_manifest_reconstructs_first_eight_in_order") is True and
...
def _validate_final_static_audit(
    ...
    need(isinstance(dual, dict) and set(dual) == {
         ...
         }, "final v12 static audit dual-checker exact8 closure")
    ...
    need(helper_consensus.get("algorithm") ==
         ...
         "final v12 three-source helper AST, exact40 witness, and callsites")
    ...
    need(isinstance(attacks, dict) and set(attacks) == {
         ...
         "final v12 coherent attack exact7, 137/137 including ordered v11 "
    ...
    need(isinstance(sealed, dict) and set(sealed) == {
         ...
         "final v12 sealed-exec and scoped no-producer exact16 closure")
    ...
    need(isinstance(closure, dict) and set(closure) == {
         ...
         }, "final v12 schema/constructor exact21 closure")
    need(type(closure.get("schema_definition_count")) is int and
         ...
         "final v12 schema 46 defs/242 refs/0 unresolved/52 closed/0 mismatch")
    need(closure.get("strict_JSON_duplicate_keys_rejected") is True and
         ...
         "final v12 schema and live-protocol type-exact bool closure")
    actual_keywords = [
    ...
    need(closure.get("actual_schema_keyword_universe") == actual_keywords and
         ...
         "final v12 exact canonical schema keyword universes and digests")
    ...
    need(isinstance(shapes, dict) and set(shapes) == set(expected_shapes) and
         ...
         "final v12 exact nine output-shape key counts")
...
def hold_static_freeze_trust(self_guard: HeldSelf) -> dict[str, Any]:
    """Hold current v12 exact10 and every distinct append-only predecessor.

    Current exact8 begins with the official v11 rejection itself.  The v11,
    v10, v9, v8, v7, v6, and v5 published exact10 surfaces remain distinct;
    v3 exact10, the separate v3 rejection, and the seven other v4 draft files
    are held.
    The exact distinct-file identity census is
    10+10+10+10+10+10+10+10+10+7+1 = 98.
    """
    manifest_guard = HeldPinnedInput(
        COLD_MANIFEST, "C79g v12 cold-launch manifest", 0o444)
    outer_guard = HeldPinnedInput(
        COLD_OUTER, "C79g v12 cold-launch outer-last receipt", 0o444)
    manifest_entries = parse_manifest_ordered(
        manifest_guard.raw, "C79g v12 cold-launch exact8 manifest")
    ...
    need(len(manifest_by_path) == 8 and
         ...
         manifest_by_path[V11_OFFICIAL_REJECTION] ==
             V11_OFFICIAL_REJECTION_FILE_PIN,
         "cold-launch manifest current SELF, v11 rejection, and policy pins")

    by_path: dict[Path, HeldPinnedInput] = {}
    for path in COLD_EXACT8:
        if path != SELF:
            by_path[path] = HeldPinnedInput(
                path, "C79g v12 cold exact8:" + path.name, 0o444,
                manifest_by_path[path], self_guard.incident_fds.get(path))
    v11_rejection_guard = by_path[V11_OFFICIAL_REJECTION]
    ...
    need(len(current_cold_ten_guards) == 10 and
         ...
         "current v12 cold exact10 unique held identities on one statx mount")
    exact8_stats = [
    ...
    need(all(chronology.values()),
         "physical v12 exact8/manifest/outer freeze chronology")
    ...
    all_file_guards = [
        *current_cold_ten_guards, *v11_guards, *v10_guards, *v9_guards,
        *v8_guards,
        *v7_guards, *v6_guards, *v5_guards, *v3_guards, *v4_guards,
        v3_rejection_guard,
    ]
    need(len(all_file_guards) == 98 and
         len({guard.identity for guard in all_file_guards}) == 98 and
         len({guard.mount_id for guard in all_file_guards}) == 1 and
         v11_namespace_guard.mount_id == v10_namespace_guard.mount_id ==
         ...
         "append-only v12/v11/v10/v9/v8/v7/v6/v5/v3/v4/rejection history is 98 "
         "distinct files on one mount")
    require_v5_positive_and_stage_surfaces_absent()

    v12_consumer_path = COLD_EXACT8[4]
    strict_bool_census = strict_bool_predecessor_and_successor_census(
        ...
        {
            "producer_v12": self_guard.raw,
            "consumer_v12": by_path[v12_consumer_path].raw,
            "launcher_v12": by_path[COLD_LAUNCHER].raw,
        })

    transition_guard = by_path[V11_TO_V12_TRANSITION]
    audit_guard = by_path[STATIC_AUDIT_V12]
    transition = strict_json(transition_guard.raw, "C79g v12 transition trust")
    audit = strict_json(audit_guard.raw, "C79g v12 static audit trust")
    outer = strict_json(outer_guard.raw, "C79g v12 cold-launch outer trust")
    ...
    verify_object(transition, "C79g v12 transition trust")
    verify_object(audit, "C79g v12 static audit trust")
    verify_object(outer, "C79g v12 cold-launch outer trust")
    need(set(transition) == {
    ...
    contract_cross = strict_json(
        by_path[CONTRACT].raw, "C79g v12 contract cross-check")
    audit_schema = audit.get("schema_and_constructor_closure", {})
    transition_pin = audited.get("v11_to_v12_transition_receipt", {})
    ...
    need(transition.get("status") ==
         ...
         audit_v4.get(
             "v12_closed_object_required_property_mismatch_count") == 0 and
         audit_v4.get("same_defects_absent_from_v12") is True and
         ...
         "transition/audit pin current v12 and preserve exact v3/v4/v7 history")
    ...
    need(set(outer) == {
         ...
         "cold-launch v12 outer-last exact8 and zero-credit closure")
    ...
    return {
        "guards": [
            *by_path.values(), *v11_guards, *v10_guards, *v9_guards, *v8_guards,
            *v7_guards, *v6_guards, *v5_guards, *v3_guards,
            *v4_guards, v3_rejection_guard,
            v11_namespace_guard, v10_namespace_guard, v9_namespace_guard,
            v8_namespace_guard,
            v7_namespace_guard,
            v6_namespace_guard,
            v5_namespace_guard,
...
def ensure_launch_configuration() -> HeldSelf:
    ...
    need(FINAL_V12_CORE_PINS_INSTALLED is True,
         "v12 final core pins are not installed; runtime permanently disabled")
    need(all(value not in V12_DRAFT_CORE_PIN_SENTINELS for value in (
             CONTRACT_FILE_PIN, CONTRACT_OBJECT_PIN, CLOSED_SCHEMA_FILE_PIN)),
         "v12 draft contract/schema sentinels must all be replaced before runtime")
    ...
    need(all(isinstance(value, str) and len(value) == 64 and
         ...
         "C79g v12 exact non-placeholder contract/schema/rejection pins")
    need(set(C78S_FINAL["pins"]) == set(C78S_NAMES) and
         ...
         "C79g v12 exact filled launch configuration")
...
def validate_static_authorities(static_trust: dict[str, Any]) -> dict[str, Any]:
    ...
    need(set(contract) == {
         ...
         contract.get("v12_bundle", {}).get(
             "independent_verifier_assembler_authority_consumer", {}).get("path") ==
             "deliverables/cm2_round306c79g_true_global_no_producer_consumer_"
             "independent_verifier_assembler_authority_consumer_v12.py" and
         contract.get("v12_bundle", {}).get("closed_schema_validator_policy") == {
         ...
         "v12 frozen contract checkpoint/composite-only boundary")
    ...
    need(closed.get("$ref") == "#/$defs/coldLaunchedCommittedAuthority" and
         ...
         "append-only v12 cold root, zero-credit inner, and persisted zero boundary")
...
def input_registry(state: dict[str, Any], self_guard: HeldSelf,
    ...
    return close_object({
        ...
        **self_guard.execution_proof(),
        ...
        "current_v12_producer_source_content_opened_or_read": False,
        "current_v12_producer_source_content_imported_compiled_or_executed": False,
        "frozen_predecessor_incident_source_exact5_held_fd_bytes_read_only": True,
...
def build(outdir: Path, self_guard: HeldSelf,
    ...
    lock = (
        "C79g v12 isolated candidate only; standalone non-authoritative. Formal-global credit "
