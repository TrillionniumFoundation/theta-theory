...
REQUEST_SCHEMA = PREFIX + "release-integrity-case-wrapper-request.v2"
REQUEST_STATUS = "FROZEN_REAL_PER_CASE_WRAPPER_REQUEST_V2__ZERO_CREDIT"
RUN_SCHEMA = PREFIX + "release-integrity-case-process-run.v6"
RUN_STATUS = (
...
def parse_service_properties(payload: bytes) -> dict[str, str]:
    ...
    need(len(lines) == len(pairs) == len(SERVICE_FIELDS)
         ...
         "wrapper service exact ten-key first-equals parse")
    return dict(pairs)


def service_snapshot(request: dict[str, Any]) -> dict[str, Any]:
    unit = request["core_unit"]
    invocation = request["core_invocation_id"]
    ...
    need(fields["Id"] == unit and fields["Result"] == "success"
         ...
         and fields["ExecStart"] != "", "exact service clean tuple")
    exec_line = ("ExecStart=" + fields["ExecStart"] + "\n").encode("ascii")
    need(valid_sha(request["core_exec_start_property_sha256"])
         and hashlib.sha256(exec_line).hexdigest()
             == request["core_exec_start_property_sha256"],
         "wrapper current ExecStart raw property SHA")
    fragment_path = Path(request["core_fragment_path"]).absolute()
    need(str(fragment_path) == request["core_fragment_path"]
         ...
         and fields["FragmentPath"] == request["core_fragment_path"],
         "wrapper exact real transient FragmentPath")
    _, fragment_record = capture_file(fragment_path, 16 << 20)
    expected_stat = request["core_fragment_stat_fingerprint"]
    expected_stat_from_cli = json.loads(request["core_fragment_stat9_json"])
    need(type(expected_stat) is list and len(expected_stat) == 9
         and all(type(item) is int for item in expected_stat)
         and expected_stat_from_cli == expected_stat
         and canonical(expected_stat).decode("ascii")
             == request["core_fragment_stat9_json"]
         and valid_sha(request["core_fragment_file_sha256"])
         and fragment_record["sha256"]
             == request["core_fragment_file_sha256"]
         and fragment_record["stat_fingerprint"] == expected_stat,
         "wrapper FragmentPath singleton SHA/full9stat current")
    return {"properties": fields,
        "exec_start_property_sha256":
            request["core_exec_start_property_sha256"],
        "fragment_file": {"path": request["core_fragment_path"],
                          **fragment_record}}
...
def full_snapshot(request: dict[str, Any]) -> dict[str, Any]:
    ...
    service = service_snapshot(request)
...
def execute(args: argparse.Namespace, request: dict[str, Any],
            request_record: dict[str, Any]) -> bytes:
...
def self_test() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="c27r2-real-case-wrapper-v2-") as raw:
        root = Path(raw)
        tree = root / "tree"
        tree.mkdir()
        empty = tree / "empty"
        ...
        request_body = {"schema": "atomic-request-fixture.v1",
                        "value": "original"}
        request_value = dict(request_body)
        request_value["request_sha256"] = digest(request_body)
        request_path = root / "request.json"
        write_once(request_path, canonical(request_value) + b"\n")
        loaded_request, loaded_record = closed_document(
            request_path, "request_sha256")
        replacement_body = {"schema": "atomic-request-fixture.v1",
                            "value": "replacement"}
        replacement_value = dict(replacement_body)
        replacement_value["request_sha256"] = digest(replacement_body)
        replacement_path = root / "request-replacement.json"
        write_once(replacement_path, canonical(replacement_value) + b"\n")
        os.replace(replacement_path, request_path)
        need(loaded_request == request_value
             and loaded_record["sha256"]
                 == hashlib.sha256(canonical(request_value) + b"\n").hexdigest(),
             "single-open request object/record remains original")
        try:
            need(capture_file(request_path)[1] == loaded_record,
                 "atomic request path replacement")
        except Rejected:
            pass
        else:
            raise Rejected("atomic request replacement negative rejected")
    ...
    def closed_calls(name: str) -> int:
        return sum(isinstance(node, ast.Call)
                   and isinstance(node.func, ast.Name)
                   and node.func.id == "closed_document"
                   for node in ast.walk(functions[name]))
    need(closed_calls("main") == 1 and closed_calls("execute") == 0,
         "formal wrapper request is single-open in main and never reopened")
    return {"schema": RUN_SCHEMA + ".wrapper-self-test.v1",
        "status": "PASS_REAL_TREE_EMPTY_DIR_WRITE_ALL_REOPEN_AND_SYMLINK_"
                  "REJECTION_SINGLE_OPEN_ATOMIC_REQUEST_NEGATIVE_TINY_FIXTURES",
...
def main() -> int:
    ...
    try:
        if args.self_test:
            ...
            stdout = execute(args, request, record)
