#!/usr/bin/env python3
"""Externally pinned cold launcher for the C79g v13 exact10 static bundle.
...
LAUNCHER_RELATIVE = Path("deliverables") / (BASE + "_cold_launch_v13.py")
...
ROOT = Path("/__C79G_V13_UNCONFIGURED_ROOT__")
...
V13_REJECTION_NAMESPACE = ROOT / "__unconfigured_v13_rejections__"
V13_LATER_REJECTION = V13_REJECTION_NAMESPACE / "rejection.json"
...
def configure_workspace_paths(root: Path) -> None:
    ...
    global V12_OFFICIAL_REJECTION
    global V13_REJECTION_NAMESPACE
    global V13_LATER_REJECTION, SCHEMA, CONTRACT, PRODUCER, CONSUMER
    ...
    V13_REJECTION_NAMESPACE = RUNTIME / ("c79g-v13-rejections-" + CHECKPOINT)
    V13_LATER_REJECTION = V13_REJECTION_NAMESPACE / "rejection.json"
    SCHEMA = OUT / (BASE + "_schema_v13.json")
    CONTRACT = OUT / (BASE + "_contract_v13.json")
    PRODUCER = OUT / (BASE + "_v13.py")
    CONSUMER = OUT / (BASE + "_independent_verifier_assembler_authority_consumer_v13.py")
    TRANSITION = OUT / (BASE + "_v12_to_v13_static_launch_transition_receipt_v1.json")
    AUDIT = OUT / (BASE + "_static_audit_v13.json")
    MANIFEST = OUT / (BASE + "_cold_launch_manifest_v13.sha256")
    OUTER = OUT / (BASE + "_cold_launch_outer_receipt_v13.json")
    BASE7_PINS = {
        # Historical predecessor authority is never normalized or replaced.
        V12_OFFICIAL_REJECTION:
            (V12_REJECTION_FILE_PIN, V12_REJECTION_OBJECT_PIN),
        # The remaining six entries are the final acyclic v13 static pins.
        # The audit binds this launcher through PIN_NORMALIZED AST, not raw bytes.
        SCHEMA: (_DRAFT_FILE_PIN, None),
        CONTRACT: (_DRAFT_FILE_PIN, _DRAFT_OBJECT_PIN),
        PRODUCER: (_DRAFT_FILE_PIN, None),
        CONSUMER: (_DRAFT_FILE_PIN, None),
        TRANSITION: (_DRAFT_FILE_PIN, _DRAFT_OBJECT_PIN),
        AUDIT: (_DRAFT_FILE_PIN, _DRAFT_OBJECT_PIN),
    }
    EXACT8 = (V12_OFFICIAL_REJECTION, SCHEMA, CONTRACT, PRODUCER,
...
INNER_SCHEMA = "cm2.round306c79g.true-global-no-producer-consumer.v13.inner-composite"
COLD_ROOT_SCHEMA = (
    "cm2.round306c79g.true-global-no-producer-consumer.v13."
    "cold-launched-committed-authority"
)
COLD_ROOT_DOMAIN = b"CM2_C79G_V13_COLD_LAUNCHED_AUTHORITY_ROOT_V1\0"
LIVE_PROTOCOL = "CM2_C79G_V13_COLD_TWO_PHASE_LIVE_ACK_V1"
LIVE_REQUEST_SCHEMA = (
    "cm2.round306c79g.true-global-no-producer-consumer.v13."
    "cold-live-commit-request"
)
LIVE_ACK_SCHEMA = (
    "cm2.round306c79g.true-global-no-producer-consumer.v13.cold-live-ack"
)
LIVE_RELEASE_SCHEMA = (
    "cm2.round306c79g.true-global-no-producer-consumer.v13.cold-live-release"
)
PREWRAPPER_BODY_DOMAIN = "CM2_C79G_V13_COLD_PREWRAPPER_BODY_V1"
TRANSACTION_BINDING_DOMAIN = "CM2_C79G_V13_COLD_TRANSACTION_BINDING_V1"
LIVE_ACK_BINDING_DOMAIN = "CM2_C79G_V13_COLD_LIVE_ACK_BINDING_V1"
SOURCE_FD_ENV = "CM2_C79G_V13_COLD_SOURCE_FD"
EXEC_FD_ENV = "CM2_C79G_V13_COLD_EXEC_FD"
COORDINATION_PARENT_FD_ENV = "CM2_C79G_V13_COORDINATION_PARENT_FD"
WORKSPACE_ROOT_ENV = "CM2_C79G_V13_COLD_WORKSPACE_ROOT"
WORKSPACE_ROOT_FD_ENV = "CM2_C79G_V13_COLD_WORKSPACE_ROOT_FD"
LAUNCHER_SHA_ENV = "CM2_C79G_V13_COLD_LAUNCHER_FILE_SHA256"
HELD_V10_PRODUCER_FD_ENV = "CM2_C79G_V13_V10_PRODUCER_FD"
HELD_V9_LAUNCHER_FD_ENV = "CM2_C79G_V13_V9_LAUNCHER_FD"
HELD_V11_PRODUCER_FD_ENV = "CM2_C79G_V13_V11_PRODUCER_FD"
HELD_V11_LAUNCHER_FD_ENV = "CM2_C79G_V13_V11_LAUNCHER_FD"
HELD_V11_REJECTION_FD_ENV = "CM2_C79G_V13_V11_REJECTION_FD"
HELD_V12_PRODUCER_FD_ENV = "CM2_C79G_V13_V12_PRODUCER_FD"
HELD_V12_CONSUMER_FD_ENV = "CM2_C79G_V13_V12_CONSUMER_FD"
HELD_V12_LAUNCHER_FD_ENV = "CM2_C79G_V13_V12_LAUNCHER_FD"
HELD_V5_REJECTION_FD_ENV = "CM2_C79G_V13_V5_REJECTION_FD"
HELD_V12_REJECTION_FD_ENV = "CM2_C79G_V13_V12_REJECTION_FD"
...
def expected_v5_published_rejected_segment() -> dict[str, Any]:
    """Construct the one exact v5 regression shape shared by v13 proofs."""
    names = (
...
PIN_NORMALIZED_AST_ALGORITHM = (
    ...
    "CURRENT_V13_SIX_BASE7_FILE_F64_OBJECT_E64_OR_NONE__"
...
FINAL_STATIC_AUDIT_TOP_LEVEL_KEY_ORDER = (
    ...
    "audited_v13_bundle", "predecessor_v3_exact10_regression",
...
FINAL_STATIC_AUDIT_STATUS = (
    "PASS_DUAL_STATIC_PIN_NORMALIZED_AST_GO_V13__"
...
FINAL_STATIC_AUDIT_OUTPUT_SHAPES = {
    "selfIdentity": 33, "independentConsumerProof": 59,
    "staticFreezeProof": 75, "coldLaunchProof": 108,
    "laterRejection": 56, "producerSourceRegistry": 70,
...
def pin_normalized_launcher_ast_sha256(raw: bytes, label: str) -> str:
    ...
    six current-v13 BASE7 values are replaced by fixed draft sentinels.  The v12
    official-rejection BASE7 member and every historical pin remain untouched.
    """
    try:
        tree = ast.parse(raw.decode("utf-8"), filename=label, mode="exec")
    except (UnicodeDecodeError, SyntaxError) as exc:
    ...
    expected_key_names = [
        "V12_OFFICIAL_REJECTION", *PIN_NORMALIZED_CURRENT_BASE7_KEY_ORDER,
    ]
    need(key_names == expected_key_names and
         len(base7.values) == len(expected_key_names),
         label + ":exact ordered v12-plus-current-v13 BASE7 key census")
    v12_before = ast.dump(
    ...
    need(ast.dump(
             ...
             include_attributes=False) == v12_before,
         label + ":v12 official-rejection BASE7 value preserved")
...
def producer_source_registry_shape_from_ast(raw: bytes) -> int:
    ...
    try:
        tree = ast.parse(raw.decode("utf-8"), filename="producer_v13",
                         mode="exec")
    except (UnicodeDecodeError, SyntaxError) as exc:
        raise Reject("current v13 producer registry AST parse") from exc
    ...
    need(isinstance(registry_function,
         ...
         "current v13 input_registry and HeldSelf.execution_proof")
    registry_returns = [
    ...
    need(len(registry_returns) == 1 and len(proof_returns) == 1,
         "unique current v13 registry/proof return dicts")
    ...
    need(len(explicit_keys) == len(set(explicit_keys)) == 62 and
         ...
         "current v13 registry explicit62 plus exact execution-proof7")
...
def validate_final_static_audit(
    ...
    need(set(audit) == audit_keys and
         audit.get("schema") ==
             "cm2.round306c79g.true-global-no-producer-consumer.static-audit.v13" and
         ...
         "final static audit exact top-level closure and PASS_V13 status")
    ...
    need(predecessor_v11.colon_witness is not None and
         ...
         sha_bytes(canonical(
             audit.get("v11_dual_validator_divergence_incident"))) ==
             V11_DUAL_VALIDATOR_DIVERGENCE_INCIDENT_DIGEST,
         "final static audit exact v10 witness and v11 incident closure")
    helper_digests = [
    ...
    input_keys = set(FINAL_STATIC_AUDIT_INPUT_KEY_ORDER)
    ...
    need(isinstance(input_a, dict) and set(input_a) == input_keys and
         ...
         "final static audit exact equal thirty-seven-pin A/B input census")
    expected_inputs = {
        ...
        "launcher_template": pin_normalized_launcher_ast_sha256(
            launcher_source_raw, "held_v13_launcher_source"),
    ...
    need(isinstance(closure, dict) and set(closure) == closure_keys and
         ...
         computed_registry_shape ==
             expected_shapes["producerSourceRegistry"] == 70 and
         closure.get(
    ...
    no_producer_keys = {
        ...
        "consumer_current_v13_producer_content_open_read_hash_decode_compile_import_or_execute_allowed",
    ...
    need(isinstance(no_producer, dict) and
         ...
         no_producer.get(
             "consumer_current_v13_producer_content_open_read_hash_decode_compile_import_or_execute_allowed") is False and
         ...
         "final static audit exact current-v13 no-producer and inherited incident evidence policy")
    ...
    need(isinstance(no_run, dict) and set(no_run) == {
             "C79_entrypoint_executed", "C79_v13_runtime_artifact_count",
             "C79_v13_process_count", "pyc_or___pycache___created",
             "cold_manifest_or_outer_created_before_dual_GO",
         } and no_run.get("C79_entrypoint_executed") is False and
         type(no_run.get("C79_v13_runtime_artifact_count")) is int and
         no_run.get("C79_v13_runtime_artifact_count") == 0 and
         type(no_run.get("C79_v13_process_count")) is int and
         no_run.get("C79_v13_process_count") == 0 and
    ...
    def __init__(self, source: HeldFile) -> None:
        ...
        try:
            self.fd = os.memfd_create(
                "c79g-v13-child-exec", MFD_CLOEXEC | MFD_ALLOW_SEALING)
        except OSError as exc:
...
class HeldEmptyRejectionNamespace:
    ...
    def __init__(self, coordination: HeldCoordinationParent) -> None:
        coordination.verify()
        name = V13_REJECTION_NAMESPACE.name
        ...
        try:
            try:
                ...
                state = os.stat(name, dir_fd=coordination.fd,
                                follow_symlinks=False)
            self.path = V13_REJECTION_NAMESPACE
...
def construct_launcher_native_rejection(
    ...
    return close_object({
        ...
        "status": "PERMANENT_FAIL_CLOSED_REJECTION__ZERO_CREDIT",
        "effective_checkpoint_object_sha256": CHECKPOINT,
        "namespace_exact_path": str(V13_REJECTION_NAMESPACE.relative_to(ROOT)),
        "target_exact_path": str(V13_LATER_REJECTION.relative_to(ROOT)),
        "rejection_reason": "ORPHANED_OR_INCOMPLETE_C79G_V13_SURFACE",
...
def install_or_replay_launcher_native_rejection(
    ...
    try:
        try:
            ...
            namespace_fd = openat2_beneath(
                V13_REJECTION_NAMESPACE, os.O_RDONLY | os.O_DIRECTORY)
    except BaseException:
        if namespace_fd >= 0:
    ...
    try:
        ...
        path_before = root_lstat(V13_REJECTION_NAMESPACE)
        ...
        target = os.stat(V13_LATER_REJECTION.name, dir_fd=namespace_fd,
                         follow_symlinks=False) if V13_LATER_REJECTION.name in universe else None
        if target is not None:
            need(universe == {V13_LATER_REJECTION.name},
                 "idempotent rejection exact singleton")
            rejection_guard = HeldFile(
                V13_LATER_REJECTION, "existing launcher-native v13 rejection")
        else:
            ...
            try:
                ...
                os.fsync(namespace_fd)
            rejection_guard = HeldFile(
                V13_LATER_REJECTION, "new launcher-native v13 rejection")
        need(rejection_guard is not None,
             "launcher-native rejection file held")
        value = strict_json(rejection_guard.raw, "launcher-native v13 rejection")
        need(isinstance(value, dict), "launcher-native rejection mapping")
        verify_object(value, "launcher-native v13 rejection")
        os.fsync(rejection_guard.fd)
        os.fsync(namespace_fd)
        os.fsync(coordination.fd)
        after = os.fstat(namespace_fd)
        path_after = root_lstat(V13_REJECTION_NAMESPACE)
        need(value == expected and rejection_guard.raw == expected_raw and
             set(os.listdir(namespace_fd)) == {V13_LATER_REJECTION.name} and
...
class HeldBundle:
    ...
    def _initialize(self, expected_launcher_sha256: str,
                    ...
                    bootstrap: HeldBootstrapEntry) -> None:
        coordination.verify()
        need(SELF == OUT / (BASE + "_cold_launch_v13.py"),
        ...
        for path, (file_pin, object_pin) in BASE7_PINS.items():
            need(re.fullmatch(r"[0-9a-f]{64}", file_pin) is not None and
                 file_pin != "0" * 64 and
                 (path == V12_OFFICIAL_REJECTION or
        ...
        for path in (V12_OFFICIAL_REJECTION, CONTRACT, TRANSITION, AUDIT):
        ...
        need(set(self.base_objects[TRANSITION]) == {
                 ...
                 "published_then_officially_rejected_predecessor_v10",
                 "published_then_officially_rejected_predecessor_v11",
                 "successor_v13_static_bundle", "physical_mode_policy",
             ...
             self.base_objects[TRANSITION].get("schema") ==
                 "cm2.round306c79g.true-global-no-producer-consumer."
                 "v12-to-v13-static-launch-transition.v1" and
             ...
             "transition exact top-level v12-to-v13 closed shape")
        need(set(self.base_objects[CONTRACT]) == {
                 ...
                 "v13_bundle", "object_sha256",
             } and
             self.base_objects[CONTRACT].get("schema") ==
                 "cm2.round306c79g.true-global-no-producer-consumer.v13.contract" and
             ...
             self.base_objects[TRANSITION].get("status") ==
                 "STATIC_BYTES_CLOSED_V12_TO_V13__PHYSICAL_FREEZE_PENDING__RUNTIME_NOT_AUTHORIZED" and
        ...
        need(set(self.outer_object) == {
             ...
             self.outer_object.get("schema") ==
                 "cm2.round306c79g.true-global-no-producer-consumer."
                 "cold-launch-outer-receipt.v13" and
        ...
        # Current exact8 owns the sole v12-rejection fd.  Each frozen exact10
        # owns the preceding rejection fd; v3 keeps its separate singleton.
        self.predecessor_v12 = HeldV12PredecessorExact10(
            coordination, self.by_path[V12_OFFICIAL_REJECTION])
        ...
        need(len(all_history_identities) == 108 and
             [len(group) for group in identity_groups] ==
                 [10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 7, 1] and
             ...
             "current v13 exact10, v12/v11/v10/v9/v8/v7/v6/v5/v3 exact10, "
             "v3 rejection and v4 draft7 are 108 unique live files on one mount")
    ...
    def inherited_incident_evidence_fds(
            self) -> tuple[int, int, int, int, int, int, int, int, int, int]:
        need(self.predecessor_v11 is not None and
             self.predecessor_v12 is not None,
             "v11 and v12 inherited incident evidence layers held")
        result = (
            *self.predecessor_v11.inherited_evidence_fds(),
            *self.predecessor_v12.inherited_v12_evidence_fds(),
        )
        need(len(result) == len(set(result)) == 10,
             "exact ten inherited incident evidence fds pairwise distinct")
        return result
...
def child_environment(bundle: HeldBundle, child_exec: HeldSealedChildExec,
    ...
    (v10_producer_fd, v9_launcher_fd, v11_producer_fd,
     ...
     v12_rejection_fd) = bundle.inherited_incident_evidence_fds()
    return {
        "LC_ALL": "C",
        "TZ": "UTC",
...
def child_pass_fds(bundle: HeldBundle, child_exec: HeldSealedChildExec,
    ...
    need(len(result) == 14 and len(set(result)) == 14,
         "child exact fourteen inherited descriptors are pairwise distinct")
    return result
...
def cold_root(bundle: HeldBundle, coordination: HeldCoordinationParent,
    ...
    proof = {
        ...
        "current_exact8_first_member_is_v12_official_rejection":
            EXACT8[0] == V12_OFFICIAL_REJECTION,
        ...
        "all_current_and_historical_108_identities_globally_unique_on_one_statx_mount": True,
        "current_v13_and_all_predecessor_unique_file_identity_count": 108,
        "current_v13_and_all_predecessor_same_statx_mount": True,
        **bundle.chronology,
        ...
        "reject_command_exempts_only_fresh_empty_v13_rejection_namespace": True,
...
def _validate_live_ack(
    ...
    expected_keys = {
        ...
        "static_v13_exact10_with_v11_exact10_and_official_rejection_v10_exact10_and_official_rejection_v9_exact10_and_official_rejection_v8_exact10_and_official_rejection_v7_exact10_and_official_rejection_v6_exact10_and_official_rejection_v5_exact10_and_official_rejection_v4_supersession_v3_exact10_and_official_rejection_terminal_replay_completed_by_launcher_before_request",
    ...
    need(set(ack) == expected_keys and ack.get("schema") == LIVE_ACK_SCHEMA and
         ...
         ack.get("static_v13_exact10_with_v11_exact10_and_official_rejection_v10_exact10_and_official_rejection_v9_exact10_and_official_rejection_v8_exact10_and_official_rejection_v7_exact10_and_official_rejection_v6_exact10_and_official_rejection_v5_exact10_and_official_rejection_v4_supersession_v3_exact10_and_official_rejection_terminal_replay_completed_by_launcher_before_request") is True and
...
def main(argv: list[str] | None = None) -> int:
    ...
    if V13_DRAFT_RUNTIME_DISABLED:
        raise Reject(
            "C79g v13 successor is a sentinel draft; runtime is disabled")
    ...
    try:
        ...
        if args.command == "reject":
            ...
            return 0
        # Every positive-capable command holds the exact-empty v13 rejection
