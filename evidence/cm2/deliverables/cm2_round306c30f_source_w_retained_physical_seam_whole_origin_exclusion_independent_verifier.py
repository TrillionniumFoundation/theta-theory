#!/usr/bin/env python3
"""Independently verify the candidate-only C30f physical-seam exclusion.
...
For each selected origin this verifier locally reconstructs the exact
...
The C30f producer is pinned and read only as inert bytes: it is never imported
or executed.  C30b--C30e remain unsealed order-only predecessors, so no source,
result, ledger, proof, or credit from those lanes is consumed.  Verification
is read-only, emits zero formal credit, and creates no result, manifest, or
sealed artifact.
"""
...
EXPECTED_SOURCE_STRATA_PER_ORIGIN = 497
...
class VerificationFailure(RuntimeError):
    """Typed fail-closed rejection used by the coherent attack harness."""
...
def validate_inert_producer_pin() -> None:
    """Hash the producer as data and prove that this process did not load it."""
    vneed(
        PRODUCER_MODULE_NAME not in sys.modules,
        "PRODUCER_IMPORT_OR_EXECUTION_FORBIDDEN",
        "preloaded",
    )
    try:
        raw = regular_bytes(ROOT / PRODUCER_SOURCE, maximum=2 * 1024 * 1024)
    except BaseException as error:
        raise VerificationFailure(
            "INPUT_PIN_CONTRACT_MISMATCH", PRODUCER_SOURCE
        ) from error
    vneed(
...
def reconstruct_reference() -> dict[str, Any]:
    """Rebuild every expected row from pinned inputs, never from the producer."""
    vneed(
        sys.flags.isolated == 1
        and sys.flags.dont_write_bytecode == 1
        and sys.dont_write_bytecode is True,
        "VERIFIER_RUNTIME_CONTRACT_MISMATCH",
        "run with python -I -B",
    )
    validate_inert_producer_pin()
    try:
        ctx.prec = 192
        ...
        for ordinal, origin in enumerate(inputs["origin_keys"]):
            origin_row, new_cells, new_source = build_origin(
                ordinal,
                len(cell_rows),
                origin,
        ...
        raise VerificationFailure(
            "REFERENCE_RECONSTRUCTION_FAILURE", str(error)
        ) from error
    ...
    per_origin: dict[str, dict[str, int | str]] = {}
    for origin in inputs["origin_keys"]:
        cells = [row for row in cell_rows if row["origin_key"] == origin]
        strata = [row for row in source_rows if row["origin_key"] == origin]
        kinds = Counter(row["source_kind"] for row in cells)
        origin_matches = [
            row for row in origin_rows if row["origin_key"] == origin
        ]
        vneed(
            len(cells) == EXPECTED_FINAL_PER_ORIGIN
            and kinds == Counter({
                "PINNED_ROUND201_EXACT_BEHIND_EXCLUDED":
                    EXPECTED_ROUND201_EXCLUDED_PER_ORIGIN,
                "SEALED_C30A_REDUCED_CLIPPED_KERNEL_REPLAY":
                    EXPECTED_NEW_CLIPPED_PER_ORIGIN,
            })
            and len(strata) == EXPECTED_SOURCE_STRATA_PER_ORIGIN
            and len(origin_matches) == 1
            and origin_matches[0]["whole_origin_disposition_candidate"]
            == "EXCLUDED",
            "REFERENCE_RECONSTRUCTION_FAILURE",
            "independent per-origin census:" + origin,
        )
        per_origin[origin] = {
            "Round201_exact_behind_excluded":
                kinds["PINNED_ROUND201_EXACT_BEHIND_EXCLUDED"],
            "sealed_C30a_reduced_clipped_excluded":
                kinds["SEALED_C30A_REDUCED_CLIPPED_KERNEL_REPLAY"],
            "source_atomic_strata": len(strata),
    ...
    vneed(
        ...
        and len(source_rows)
        == EXPECTED_ORIGINS * EXPECTED_SOURCE_STRATA_PER_ORIGIN,
        "REFERENCE_RECONSTRUCTION_FAILURE",
        "global census",
    )
    return {
...
def candidate_regular_bytes(path: Path, maximum: int) -> bytes:
    try:
        status = path.lstat()
    except OSError as error:
        raise VerificationFailure(
            "CANDIDATE_FORMAT_VIOLATION", "missing:" + path.name
        ) from error
    vneed(
        stat.S_ISREG(status.st_mode)
        and not path.is_symlink()
    ...
    try:
        ...
        raise VerificationFailure(
            "CANDIDATE_FORMAT_VIOLATION", path.name
        ) from error
    return bytes(output)


def parse_candidate_object(raw: bytes, label: str) -> dict[str, Any]:
    try:
        value = strict_object(raw, label)
    except BaseException as error:
        raise VerificationFailure(
            "CANDIDATE_FORMAT_VIOLATION", label
        ) from error
    vneed(
        canonical(value) == raw,
        "CANDIDATE_FORMAT_VIOLATION",
...
def read_candidate_ledger(
    ...
    raw = candidate_regular_bytes(candidate / filename, 256 * 1024 * 1024)
    try:
        with gzip.GzipFile(fileobj=io.BytesIO(raw), mode="rb") as stream:
            payload = stream.read(512 * 1024 * 1024 + 1)
    except BaseException as error:
        raise VerificationFailure(
            "CANDIDATE_FORMAT_VIOLATION", "gzip:" + filename
        ) from error
    vneed(
        len(payload) <= 512 * 1024 * 1024
        and payload.endswith(b"\n"),
        "CANDIDATE_FORMAT_VIOLATION",
        "JSONL framing:" + filename,
    )
    lines = payload[:-1].split(b"\n")
    vneed(
        0 < len(lines) <= maximum_rows and all(lines),
        "CANDIDATE_FORMAT_VIOLATION",
        "row count:" + filename,
    )
    rows: list[dict[str, Any]] = []
    ...
    for index, line in enumerate(lines):
        row = parse_candidate_object(line, filename + ":" + str(index))
        ...
        vneed(
            ...
            "CANDIDATE_FORMAT_VIOLATION",
            "row closure:" + filename + ":" + str(index),
        )
        try:
            ...
            raise VerificationFailure(
                "CANDIDATE_FORMAT_VIOLATION",
                "row hash alphabet:" + filename,
            ) from error
        rows.append(row)
    vneed(
        deterministic_gzip(rows) == raw,
        "CANDIDATE_FORMAT_VIOLATION",
        "nondeterministic gzip:" + filename,
    )
    return rows, {
        "filename": filename,
        "row_count": len(rows),
...
def predecessor_contracts() -> dict[str, Any]:
    contracts = {
        lane: {
...
def expected_result(
    ...
    origin_rows = reference["origin_rows"]
    cell_rows = reference["cell_rows"]
    ...
    body = {
        ...
        "unsealed_upstream_dependencies": predecessor_contracts(),
        ...
        "ledgers": {
            ...
            "source_half_open_strata_candidate":
                descriptors[SOURCE_STRATA_LEDGER],
...
def formal_credit_objects(value: Any) -> Iterable[dict[str, Any]]:
    if type(value) is dict:
        for key, child in value.items():
            if key == "formal_credit":
                if type(child) is not dict:
                    reject("ZERO_FORMAL_CREDIT_REQUIRED", "non-object")
                yield child
            yield from formal_credit_objects(child)
    elif type(value) is list:
        for child in value:
            yield from formal_credit_objects(child)


def all_exact_zero(values: dict[str, Any]) -> bool:
    return bool(values) and all(type(value) is int and value == 0
                                for value in values.values())


def semantic_nonpromotion_checks(
    result: dict[str, Any],
    cell_rows: list[dict[str, Any]],
    source_rows: list[dict[str, Any]],
    origin_rows: list[dict[str, Any]],
    ...
    vneed(
        result.get("input_pins") == reference["inputs"]["input_pins"],
        "INPUT_PIN_CONTRACT_MISMATCH",
    )
    vneed(
        result.get("unsealed_upstream_dependencies")
        == predecessor_contracts(),
        "CONDITIONAL_ORDER_CONTRACT_MISMATCH",
    )
    formal = result.get("formal_credit")
    strict = result.get("strict_nonpromotion")
    vneed(
        type(formal) is dict and type(strict) is dict,
        "ZERO_FORMAL_CREDIT_REQUIRED",
        "result formal/strict objects",
    )
    vneed(
        formal.get("D02") == 0
        and strict.get("D02")
    ...
    volume_minted = any(
        type(row.get("exact_rational_enclosure_volume_conservation")) is dict
        and row["exact_rational_enclosure_volume_conservation"].get(
    ...
    child_minted = (
        ...
        or theorem.get(
            "child_stratum_count_or_volume_used_as_integer_credit"
        ) is not False
        or any(
            formal.get(key) != 0
            for key in (
                "target_cell_dispositions",
                "target_whole_cell_exclusions",
                "source_half_open_stratum_dispositions",
            )
        )
    )
    vneed(
        not child_minted and not volume_minted,
        "CHILD_OR_VOLUME_CREDIT_VIOLATION",
    )
    for value in (result, cell_rows, source_rows, origin_rows):
        for credit in formal_credit_objects(value):
            vneed(
                all_exact_zero(credit),
                "ZERO_FORMAL_CREDIT_REQUIRED",
            )
...
def verify_candidate_dir(
    ...
    try:
        status = absolute.lstat()
        names = {path.name for path in absolute.iterdir()}
    except OSError as error:
        raise VerificationFailure(
            "CANDIDATE_FORMAT_VIOLATION", "candidate directory"
        ) from error
    vneed(
    ...
    cell_rows, cell_base = read_candidate_ledger(
        absolute,
        CELL_LEDGER,
        EXPECTED_ORIGINS * EXPECTED_FINAL_PER_ORIGIN,
    )
    source_rows, source_base = read_candidate_ledger(
        absolute,
        SOURCE_STRATA_LEDGER,
        EXPECTED_ORIGINS * EXPECTED_SOURCE_STRATA_PER_ORIGIN,
    )
    origin_rows, origin_base = read_candidate_ledger(
        absolute,
        ORIGIN_LEDGER,
        EXPECTED_ORIGINS,
    )
    result_raw = candidate_regular_bytes(absolute / RESULT, 16 * 1024 * 1024)
    result = parse_candidate_object(result_raw, RESULT)
    result_hash = result.get("result_sha256")
    vneed(
        type(result_hash) is str
        and result_hash == digest({
    ...
    vneed(
        origin_rows == reference["origin_rows"],
        "ORIGIN_RECONSTRUCTION_MISMATCH",
    )
    descriptors = {
        CELL_LEDGER: ledger_descriptor(
            cell_base,
            "LEXICOGRAPHIC_ROUND180_FINAL_CELL_KEY",
        ),
        SOURCE_STRATA_LEDGER: ledger_descriptor(
            source_base,
            "LEXICOGRAPHIC_ORIGIN_KEY_THEN_SOURCE_STRATUM_ID",
        ),
        ORIGIN_LEDGER: ledger_descriptor(
            origin_base,
            "LEXICOGRAPHIC_SOURCE_W_ORIGIN_KEY",
        ),
    }
    vneed(
        result == expected_result(reference, descriptors),
        "RESULT_RECONSTRUCTION_MISMATCH",
    )
    for origin in reference["inputs"]["origin_keys"]:
        candidate_cells = [
            row for row in cell_rows if row["origin_key"] == origin
        ]
        candidate_source = [
            row for row in source_rows if row["origin_key"] == origin
        ]
        candidate_origins = [
            row for row in origin_rows if row["origin_key"] == origin
        ]
        kinds = Counter(row["source_kind"] for row in candidate_cells)
        vneed(
            kinds["PINNED_ROUND201_EXACT_BEHIND_EXCLUDED"]
            == EXPECTED_ROUND201_EXCLUDED_PER_ORIGIN
            and kinds["SEALED_C30A_REDUCED_CLIPPED_KERNEL_REPLAY"]
            == EXPECTED_NEW_CLIPPED_PER_ORIGIN
            and len(candidate_source) == EXPECTED_SOURCE_STRATA_PER_ORIGIN
            and len(candidate_origins) == 1
            and candidate_origins[0]["whole_origin_disposition_candidate"]
            == "EXCLUDED",
            "RESULT_RECONSTRUCTION_MISMATCH",
            "post-compare census:" + origin,
        )
    validate_inert_producer_pin()
    report_body = {
        "schema": (
            "cm2.round306c30f.retained-physical-seam."
            "independent-verification.report.v1"
        ),
        "status": (
            "PASS_INDEPENDENT_C30F__2_DYNAMIC_ORIGINS__"
            "72_R201_PLUS_72_C30A_AND_497_SOURCE_ATOMS_EACH__"
            "ZERO_FORMAL_CREDIT"
        ),
        ...
        "dynamic_origin_count": EXPECTED_ORIGINS,
        "dynamic_origin_keys_sha256": digest(
            reference["inputs"]["origin_keys"]
        ),
        "per_origin_independent_census": reference["per_origin"],
...
def verify(candidate: Path) -> dict[str, Any]:
    ...
    return verify_candidate_dir(candidate, reference)
...
def main() -> int:
    ...
    try:
        report = verify(Path(arguments.candidate_dir))
    except VerificationFailure as error:
        print(canonical({
            "status": "REJECT",
            "reason": error.reason,
            "detail": error.detail,
            "formal_credit": {"D02": 0, "CM2": 0},
