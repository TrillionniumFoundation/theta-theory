#!/usr/bin/env python3
"""C79g v6 append-only zero-credit candidate builder.

This append-only v6 binds the independently verified C78s 13-member final
...
is permanently non-authoritative.  The separately committed v6 seal plus the
...
COLD_COORDINATION_PARENT_FD_ENV = "CM2_C79G_V6_COORDINATION_PARENT_FD"
...
SELF = OUT / "cm2_round306c79g_true_global_no_producer_consumer_v6.py"
...
V5_TO_V6_TRANSITION = OUT / "cm2_round306c79g_true_global_no_producer_consumer_v5_to_v6_static_launch_transition_receipt_v1.json"
STATIC_AUDIT_V6 = OUT / "cm2_round306c79g_true_global_no_producer_consumer_static_audit_v6.json"
COLD_LAUNCHER = OUT / "cm2_round306c79g_true_global_no_producer_consumer_cold_launch_v6.py"
COLD_MANIFEST = OUT / "cm2_round306c79g_true_global_no_producer_consumer_cold_launch_manifest_v6.sha256"
COLD_OUTER = OUT / "cm2_round306c79g_true_global_no_producer_consumer_cold_launch_outer_receipt_v6.json"
...
FINAL_V6_CORE_PINS_INSTALLED = True
...
COLD_EXACT8 = (
    ...
    CONTRACT,
    SELF,
    OUT / "cm2_round306c79g_true_global_no_producer_consumer_independent_verifier_assembler_authority_consumer_v6.py",
    V5_TO_V6_TRANSITION,
    STATIC_AUDIT_V6,
    COLD_LAUNCHER,
)
...
def _linux_libc() -> ctypes.CDLL:
    need(sys.platform.startswith("linux"), "Linux-only v6 publication protocol")
...
class HeldSelf:
    ...
    def __init__(self, exec_inherited: int, source_inherited: int,
        ...
        need(SELF == OUT / "cm2_round306c79g_true_global_no_producer_consumer_v6.py",
...
def new_candidate_stage(outdir: Path, parent: HeldParent) -> tuple[Path, HeldMutableStage]:
    ...
    name = ".c79g-v6-candidate-stage-" + orientation + "-" + CHECKPOINT_OBJECT_PIN
...
def strict_bool_predecessor_and_successor_census(
        ...
        v6_sources: Mapping[str, bytes]) -> dict[str, Any]:
    """Prove the frozen 700/7/3 v5 defect box and zero-unsafe v6 box."""
    v5_roles = ("producer_v5", "consumer_v5", "launcher_v5")
    v6_roles = ("producer_v6", "consumer_v6", "launcher_v6")
    need(set(v5_sources) == set(v5_roles) and set(v6_sources) == set(v6_roles),
    ...
    need(v6_unproved == 0 and v6_arity == 0 and v6_returns == 0,
         "v6 recursive exact-bool full box has zero unsafe or unproved sites")
    return {
...
def _validate_final_static_audit(
    ...
    """Fail closed on every final v6 dual-static-audit acceptance field.
    ...
    need(set(audit) == {
             ...
             "effective_checkpoint_object_sha256", "audited_v6_bundle",
         ...
         "final v6 static audit exact top-level PASS_V6 closure")
    ...
    need(isinstance(dual, dict) and set(dual) == {
         ...
         }, "final v6 static audit dual-checker exact8 closure")
    ...
    need(isinstance(checker_a, dict) and set(checker_a) == {
         ...
         isinstance(checker_c, dict) and set(checker_c) == {
             ...
             "starred_positional_total", "double_star_keyword_total",
             "failed_static_check_count",
         }, "final v6 checker A/B/C exact object closures")
    need(checker_a.get("algorithm") ==
         ...
         "final v6 checker A/B/C algorithms and exact GO statuses")
    ...
    need(isinstance(input_a, dict) and isinstance(input_b, dict) and
         ...
         "final v6 checker A/B exact12 identical input-pin closure")
    expected_inputs = {
    ...
    need(input_a == expected_inputs and
         ...
         "final v6 checker inputs pin exact current core and append-only history")
    ...
    need(_nonzero_lower_sha256(normalized_a) is True and
         ...
         "final v6 three-way normalized and B/C common-callsite consensus")
    ...
    need(isinstance(kind_census, dict) and set(kind_census) == {
         ...
         "final v6 checker C exact common-callsite kind census")
    need(type(checker_a.get("wider_local_callsite_census_row_count")) is int and
         ...
         "final v6 checker A positive census and three-file in-memory compile")
    need(type(checker_a.get("arity_failure_count")) is int and
         ...
         "final v6 checker A zero arity/undefined/duplicate/failed counts")
    for key in (
    ...
    need(type(dual.get("independent_normalizer_count")) is int and
         ...
         "final v6 dual exact 3/2 checker counts and type-exact consensuses")
    ...
    need(isinstance(attacks, dict) and set(attacks) == {
         ...
         "final v6 coherent attack exact7, 121/121, routes, and deferred execution")
    ...
    need(isinstance(closure, dict) and set(closure) == {
         ...
         }, "final v6 schema/constructor exact21 closure")
    need(type(closure.get("schema_definition_count")) is int and
         closure.get("schema_definition_count") == 37 and
         type(closure.get("schema_ref_count")) is int and
         closure.get("schema_ref_count") == 179 and
         ...
         closure.get("closed_object_count") == 43 and
         type(closure.get(
         ...
         "final v6 schema 37 defs/179 refs/0 unresolved/43 closed/0 mismatch")
    need(closure.get("strict_JSON_duplicate_keys_rejected") is True and
         ...
         "final v6 schema and live-protocol type-exact bool closure")
    need(isinstance(closure.get("actual_schema_keyword_universe"), list) and
         isinstance(closure.get(
             "cold_launcher_supported_schema_keyword_universe"), list) and
         _nonzero_lower_sha256(
             closure.get("actual_schema_keyword_universe_sha256")) is True and
         _nonzero_lower_sha256(closure.get(
             "cold_launcher_supported_schema_keyword_universe_sha256")) is True,
         "final v6 schema keyword universes and digest shapes")
    shapes = closure.get("output_shape_key_counts", {})
    expected_shapes = {
        "selfIdentity": 33,
        "independentConsumerProof": 44,
        "staticFreezeProof": 48,
        "coldLaunchProof": 76,
        "laterRejection": 41,
        "producerSourceRegistry": 37,
        "liveRequest": 15,
        "liveACK": 32,
        "liveACKCensus": 24,
    }
    need(isinstance(shapes, dict) and set(shapes) == set(expected_shapes) and
         ...
         "final v6 exact nine output-shape key counts")
    ...
    need(isinstance(acceptance, dict) and set(acceptance) == {
         ...
         "final v6 audit acceptance exact10 type-exact zero-runtime closure")


def hold_static_freeze_trust(self_guard: HeldSelf) -> dict[str, Any]:
    """Hold current v6 exact10 plus the published-then-rejected v5 chain.

    Current BASE7 begins with the official v5 rejection itself.  The frozen v5
    exact10 is a distinct predecessor surface; v3 and v4 remain append-only
    transition/audit regressions and are never substituted for the current
    base member.
    """
    manifest_guard = HeldPinnedInput(
        COLD_MANIFEST, "C79g v6 cold-launch manifest", 0o444)
    outer_guard = HeldPinnedInput(
        COLD_OUTER, "C79g v6 cold-launch outer-last receipt", 0o444)
    manifest_entries = parse_manifest_ordered(
        manifest_guard.raw, "C79g v6 cold-launch exact8 manifest")
    ...
    need(len(manifest_by_path) == 8 and
         ...
         manifest_by_path[V5_OFFICIAL_REJECTION] ==
             V5_OFFICIAL_REJECTION_FILE_PIN,
         "cold-launch manifest current SELF, v5 rejection, and policy pins")

    by_path: dict[Path, HeldPinnedInput] = {}
    for path in COLD_EXACT8:
        if path != SELF:
            by_path[path] = HeldPinnedInput(
                path, "C79g v6 cold exact8:" + path.name, 0o444,
                manifest_by_path[path])
    v5_rejection_guard = by_path[V5_OFFICIAL_REJECTION]
    ...
    need(len(current_cold_ten_guards) == 10 and
         ...
         "current v6 cold exact10 unique held identities on one statx mount")
    exact8_stats = [
    ...
    need(all(chronology.values()),
         "physical v6 exact8/manifest/outer freeze chronology")
    ...
    all_file_guards = [*current_cold_ten_guards, *v5_guards]
    need(len(all_file_guards) == 20 and
         len({guard.identity for guard in all_file_guards}) == 20 and
         len({guard.mount_id for guard in all_file_guards}) == 1 and
         v5_namespace_guard.mount_id == all_file_guards[0].mount_id,
         "current v6 exact10 and predecessor v5 exact10 are 20 distinct files on one mount")
    require_v5_positive_and_stage_surfaces_absent()

    v6_consumer_path = COLD_EXACT8[4]
    strict_bool_census = strict_bool_predecessor_and_successor_census(
        ...
        {
            "producer_v6": self_guard.raw,
            "consumer_v6": by_path[v6_consumer_path].raw,
            "launcher_v6": by_path[COLD_LAUNCHER].raw,
        })

    transition_guard = by_path[V5_TO_V6_TRANSITION]
    audit_guard = by_path[STATIC_AUDIT_V6]
    transition = strict_json(transition_guard.raw, "C79g v6 transition trust")
    audit = strict_json(audit_guard.raw, "C79g v6 static audit trust")
    outer = strict_json(outer_guard.raw, "C79g v6 cold-launch outer trust")
    ...
    verify_object(transition, "C79g v6 transition trust")
    verify_object(audit, "C79g v6 static audit trust")
    verify_object(outer, "C79g v6 cold-launch outer trust")
    ...
    transition_pin = audited.get("v5_to_v6_transition_receipt", {})
    ...
    _validate_v5_regression_section(
        published_v5, strict_bool_census, "v6 transition v5 predecessor")
    _validate_v5_regression_section(
        audit_v5, strict_bool_census, "v6 audit v5 predecessor")
    ...
    need(transition.get("status") ==
         ...
         "transition/audit pin current v6 and preserve exact v3/v4 history")
    ...
    need(set(outer) == {
         ...
         "cold-launch v6 outer-last exact8 and zero-credit closure")
    ...
    return {
        "guards": [
            *by_path.values(), *v5_guards, v5_namespace_guard,
...
def ensure_launch_configuration() -> HeldSelf:
    ...
    need(FINAL_V6_CORE_PINS_INSTALLED is True,
         "v6 final core pins are not installed; runtime permanently disabled")
    need(all(value not in V6_DRAFT_CORE_PIN_SENTINELS for value in (
             CONTRACT_FILE_PIN, CONTRACT_OBJECT_PIN, CLOSED_SCHEMA_FILE_PIN)),
         "v6 draft contract/schema sentinels must all be replaced before runtime")
    ...
    need(all(value and len(value) <= 10 and
         ...
         SELF == ROOT / "deliverables/cm2_round306c79g_true_global_no_producer_consumer_v6.py",
    ...
    need(all(isinstance(value, str) and len(value) == 64 and
         ...
         "C79g v6 exact non-placeholder contract/schema/rejection pins")
    need(set(C78S_FINAL["pins"]) == set(C78S_NAMES) and
         ...
         "C79g v6 exact filled launch configuration")
...
def validate_static_authorities(static_trust: dict[str, Any]) -> dict[str, Any]:
    ...
    rejection_raw = held[V5_OFFICIAL_REJECTION].raw
    ...
    need(contract.get("effective_checkpoint_object_sha256") == CHECKPOINT_OBJECT_PIN and
         ...
         contract.get("v6_bundle", {}).get(
             ...
             "independent_verifier_assembler_authority_consumer_v6.py" and
         contract.get("v6_bundle", {}).get("closed_schema_validator_policy") == {
         ...
         "v6 frozen contract checkpoint/composite-only boundary")
    ...
    need(closed.get("$ref") == "#/$defs/coldLaunchedCommittedAuthority" and
         ...
         "append-only v6 cold root, zero-credit inner, and persisted zero boundary")
    rejection = strict_json(rejection_raw, "C79g official v5 later rejection")
    verify_object(rejection, "C79g official v5 later rejection",
                  V5_OFFICIAL_REJECTION_OBJECT_PIN)
    need(rejection.get("schema") ==
         ...
         rejection.get("target_exact_path") ==
             str(V5_OFFICIAL_REJECTION.relative_to(ROOT)),
         "exact official v5 permanent later rejection semantics")
    ...
    return close_object({
        ...
        **self_guard.execution_proof(),
        ...
        "post_source_static_freeze_trust": {
            "v5_to_v6_transition_receipt": {
                "path": str(V5_TO_V6_TRANSITION.relative_to(ROOT)),
                "file_sha256": static_trust["transition_file_sha256"],
                "object_sha256": static_trust["transition_object_sha256"],
            },
            "static_audit_v6": {
                "path": str(STATIC_AUDIT_V6.relative_to(ROOT)),
...
def build(outdir: Path, self_guard: HeldSelf,
    ...
    lock = (
        "C79g v6 isolated candidate only; standalone non-authoritative. Formal-global credit "
