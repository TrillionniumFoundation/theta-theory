#!/usr/bin/env python3
"""Fail-closed one-shot launcher for the frozen C30c bridge-v4 service.

The launcher creates the matching empty audit/control namespaces and then
execves systemd-run.  The separately frozen ExecStartPre gate remains the only
publisher: it writes pinset.json first and launch-anchor.json last.
"""
...
SELF = DELIVERABLES / (
    "cm2_round306c30c_postreceipt_v3_p1_bridge_v4_one_shot_launcher_v1.py")
WATCHER = DELIVERABLES / "cm2_round306c30c_postreceipt_v3_p1_bridge_watch_v4.py"
GATE = DELIVERABLES / "cm2_round306c30c_postreceipt_v3_p1_bridge_start_gate_v1.py"
WATCHER_V3 = DELIVERABLES / "cm2_round306c30c_postreceipt_v3_p1_bridge_watch_v3.py"
...
WATCHER_SHA256 = "10fc2a6a43e8ea97067c3fa4ed1a040a7ef3a0f1e1b860d978be0b2663a9093a"
GATE_SHA256 = "56fd5489ec37ea6b67edb92fc74a77a6465ca5211b40ed2e2f26f39116544609"
WATCHER_V3_SHA256 = "8186a54f36fcef531611bbf5c2d8170b8961043327bba57e8187da285ba08508"
...
UNIT = re.compile(
    r"cm2-c30c-postreceipt-v3-p1-bridge-v4-[A-Za-z0-9_.:-]+\.service")
FORBIDDEN_ARG_BYTES = frozenset(b"'\"\\$%")
SELFTEST_STATUS = (
    "PASS_TEMP_ONLY_EXACT_18_WATCHER_20_GATE_ARGV_AND_SYSTEMD_PROPERTY_"
...
def namespace_paths(
    ...
    name = "c30c-postreceipt-v3-p1-bridge-v4-" + token
    unit = "cm2-" + name + ".service"
    need(UNIT.fullmatch(unit) is not None, "exact fresh bridge-v4 unit syntax")
    bridge = audit / name
...
def load_frozen_watcher(held: HeldFile) -> types.ModuleType:
    held.verify()
    module = types.ModuleType("_cm2_bridge_watch_v4_launcher_held")
    ...
    need(module.WATCHER == WATCHER and module.START_GATE == GATE
         ...
         "held watcher exact launch constants")
    return module
...
def source_holds(
    expect_self_sha256: str, formal: bool,
) -> dict[str, HeldFile]:
    ...
    specifications = {
        ...
        "watcher_v4": (WATCHER, WATCHER_SHA256, SOURCE_SIZES[WATCHER], 0o444, 1000, 1000),
        "start_gate": (GATE, GATE_SHA256, SOURCE_SIZES[GATE], 0o444, 1000, 1000),
        "watcher_v3": (WATCHER_V3, WATCHER_V3_SHA256, SOURCE_SIZES[WATCHER_V3], 0o444, 1000, 1000),
...
def verify_holds(holds: dict[str, HeldFile]) -> None:
    need(set(holds) == {
        "launcher", "watcher_v4", "start_gate", "watcher_v3",
        "wrapper_v1", "python", "systemd_run", "systemctl",
    }, "exact launcher source/tool pin roles")
...
def self_test(
    launch_token: str, maximum_wait_seconds: int,
    expect_self_sha256: str,
) -> dict[str, Any]:
    ...
    try:
        module = load_frozen_watcher(holds["watcher_v4"])
        with tempfile.TemporaryDirectory(prefix="c30c-bridge-v4-launcher-") as raw:
            root = Path(raw)
            audit = root / "audit"
            ...
            need(bridge_held.path == bridge and control_held.path == control_dir
                 ...
                 "temp-only exact fresh namespace construction")
            need(command[8] == "--property=ExecStartPre=" + " ".join(gate)
                 and command[10:] == watcher,
                 "temp-only exact systemd property and ExecStart")
        verify_holds(holds)
        return {
            "schema": "cm2.round306c30c.postreceipt-bridge-v4-launcher-self-test.v1",
...
def execute(
    ...
    try:
        ...
        need(os.listdir(bridge_held.fd) == [] and os.listdir(control_held.fd) == [],
             "final empty bridge/control namespaces before systemd-run")
        unit_absent(unit, holds["systemctl"])
        verify_holds(holds)
