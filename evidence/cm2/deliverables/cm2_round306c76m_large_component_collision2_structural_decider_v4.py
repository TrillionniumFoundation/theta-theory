...
SCHEMA = "cm2.round306c76m.large-component-collision2-structural-decider.v4"
PREFIX = "cm2_round306c76m_large_component_collision2_structural_decider_v4"
...
def build(output: Path, batch_size: int) -> None:
    ...
    publication_records = [publication_record(output / REGISTRY, 0, None)]
    ...
    writers = [decision_writer, branch_writer, handoff_writer, incidence_writer]
    ...
    try:
        ...
        for writer in writers:
            writer.discard()
        rejection_body = {
            "schema": SCHEMA + ".build-rejection",
            "status": "REJECTED_PARTIAL_STAGE",
            "producer_file_sha256": file_sha(SELF),
            "already_published_members": [REGISTRY],
            "exception_type": type(error).__name__,
            "exception_message": str(error),
            "zero_credit": True}
        publish_json(output / (PREFIX + "_build_rejection.json"),
                     {**rejection_body,
                      "object_sha256": digest(rejection_body)})
        raise
    for writer in writers:
        writer.finish()
        publication_records.append(publication_record(
            writer.path, len(publication_records),
            publication_records[-1]["mtime_ns"]))
    ...
    result_body = {
        ...
        "additional_dyadic_depth": 0,
        "pre_result_publication_record_projection":
            [public_record_projection(record)
    ...
    publication_records.append(publication_record(
        output / RESULT, len(publication_records),
        publication_records[-1]["mtime_ns"]))
    report = (
    ...
    publication_records.append(publication_record(
        output / REPORT, len(publication_records),
        publication_records[-1]["mtime_ns"]))
    publish(output / LOCK, b"ZERO_CREDIT_ONLY\nNO_CANONICAL_POINTER\nNO_SEAL\n")
    publication_records.append(publication_record(
        output / LOCK, len(publication_records),
        publication_records[-1]["mtime_ns"]))
    manifest_members = [record["filename"] for record in publication_records]
    need(manifest_members == [REGISTRY, DECISIONS, BRANCHES, HANDOFFS,
                              INCIDENCE, RESULT, REPORT, LOCK],
         "actual pre-manifest publication order")
    manifest_raw = b"".join(
        f"{record['sha256']}  {record['filename']}\n".encode("ascii")
        for record in publication_records)
    publish(output / MANIFEST, manifest_raw)
    publication_records.append(publication_record(
        output / MANIFEST, len(publication_records),
        publication_records[-1]["mtime_ns"]))
    replay_names = [record["filename"] for record in publication_records]
    capture = [public_record_projection(record)
               for record in publication_records]
    outer_body = {"schema": SCHEMA + ".outer-publication-receipt",
                  ...
                  "actual_pre_outer_publication_record_projection": capture,
                  "actual_pre_outer_stat_mtime_order_strict": True,
                  "outer_receipt_published_last": True,
                  "pre_outer_terminal_byte_replay": capture,
    ...
    publication_records.append(publication_record(
        output / OUTER, len(publication_records),
        publication_records[-1]["mtime_ns"]))
    need([record["filename"] for record in publication_records] ==
         replay_names + [OUTER], "actual outer-last publication order")
    terminal_capture = []
    for record in publication_records:
        name = record["filename"]
        raw = secure(output / name, record["sha256"])
        need(bool(raw), "nonempty-terminal-replay:" + name)
        if name.endswith(".jsonl.gz"):
            one_member(raw, "terminal-replay:" + name)
        else:
            need(raw[-1:] == b"\n", "text-terminal-newline:" + name)
        current = os.lstat(output / name)
        need(list(identity(current)) == record["identity"] and
             current.st_mtime_ns == record["mtime_ns"],
             "terminal-replay-identity:" + name)
        terminal_capture.append(public_record_projection(record))
    replay_body = {
        "schema": SCHEMA + ".ephemeral-post-outer-terminal-replay",
        "ordered_member_count": len(terminal_capture),
        "ordered_members": terminal_capture,
        "outer_was_last": True,
        "all_bytes_and_identities_stable": True}
    replay_audit = {**replay_body, "object_sha256": digest(replay_body)}
    print(json.dumps({"stage": str(output), "result": result,
                      ...
                      "post_outer_terminal_replay": replay_audit},
                     sort_keys=True))
