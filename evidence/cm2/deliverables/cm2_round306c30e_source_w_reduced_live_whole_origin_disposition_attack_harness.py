#!/usr/bin/env python3
"""Coherent-corruption rejection suite for the candidate-only C30e lane.
...
remain a mandatory release phase because they require the final C30d seal and
...
VERIFIER_SHA256 = (
    "6f8bb04ab8cd2a16cd0243062ec4212d76b8819aeed98372f88cccd6c14f1304"
)
...
def attack_absolute_predecessor_order(target: Path) -> None:
    ...
    delta = result["proposed_source_W_transition_if_C30e_is_independently_sealed"]
...
MANDATORY_RELEASE_ONLY_ATTACKS = (
    "C30d_root_manifest_missing_extra_or_member_sha_substitution",
    "C30d_payload_or_verification_substitution_and_fake_58_state",
    "C30d_sealed_member_symlink_hardlink_extra_member_or_atomic_replace",
    "C30d_authority_TOCTOU_during_single_capture",
    ...
    "manifest_first_pre_post_change",
...
def run(candidate: Path) -> dict[str, Any]:
    ...
    return {
        "schema": (
            ...
            "coherent-attack-harness.candidate.v2"
        ),
        "status": "PASS_ALL_C30E_COHERENT_ATTACKS_REJECTED",
...
def main() -> int:
    ...
    try:
        output = run(arguments.candidate)
    except (
        HarnessFailure,
        verifier.Reject,
