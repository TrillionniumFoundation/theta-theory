...
\large Complete tilted energy trees, two-cutoff face convergence,
three-frequency removal, and a strongly conditional
pathwise CM2 criterion}
\author{Working note}
\date{13 July 2026 (version 37)}
...
positive energy tree, with a cemetery label for each failure.  The entire tree
is tilted by the same full event weight used for the upper-scale grazing
ledger.  Thus context, stopping, auxiliary, and physical-escape tails are all
estimated in one positive probability law.  A full-product conditional
$L^{p_W}$ lemma gives a nonvacuity route from unweighted tails, with every
exponent explicitly degraded by $1/p_W'$; mere $L^1$ integrability is never
used to preserve a tail exponent.  The
...
listed below, the complete first
...
\item The same double-projected typed decomposition holds for the limiting
      ...
      $\mathrm{UCR}_{\rm sh}$ atlas.  With $\|\cdot\|_{\rm BL^*}$ denoting
...
deep remainder are three different estimates.
\end{definition}
...
\item Multiplication by a retained positive amplitude preserves
      ...
      by itself imply the full-$\mathscr W$ estimate
...
\end{equation}
On the unique complete positive label law define the full event
weight
\begin{equation}
...
satisfy, with the
\emph{same} weight \eqref{eq:global-event-weight},
...
last marks measurable before failure; every later missing factor in
\eqref{eq:global-event-weight} is set equal to one.  Thus
$\mathscr W$ is positive and measurable on the entire tree, not only on the
terminal survivor law.  Define
\begin{equation}
\label{eq:complete-energy-tree-tilt}
 d\Pi_{s,N}^{W}:=
 \frac{\mathscr W}{\mathbb E_{\Pi_{s,N}}\mathscr W}\,d\Pi_{s,N},
 \qquad
 1\le\mathbb E_{\Pi_{s,N}}\mathscr W\le C_W .
\end{equation}
...
\end{align}
Every additional contracted-coordinate, native-antichain,
adaptive-alphabet, or continuation event used in the central absolute sum is
given a name $B_{{\rm event},j}(N)$ and a tilted rate
$c_{{\rm event},j}^{W}>0$ in the same form as
\eqref{eq:tilted-auxiliary-tails}; put
$c_{\rm event}^{W}:=\inf_jc_{{\rm event},j}^{W}>0$.  The last line includes
...
\label{eq:weighted-energy-rates}
 \Theta_E^{W}:=\min\{\Theta_{\rm PPE}^{W},
       \Theta_{\rm ctx}^{W},\Theta_{\rm st}^{W}\},
 \qquad
 c_{\rm aux}^{W}:=\min\{c_{\rm buf}^{W},
       c_{\rm gate}^{W},c_{\rm cont}^{W}\}.
\end{equation}
...
\label{eq:full-product-Lp-energy-route}
 \|\mathscr W\|_{L^{p_W}(\Pi_{s,N})}
 \le C\mathbb E_{\Pi_{s,N}}\mathscr W,
 \qquad p_W>1,
\end{equation}
...
labels, with the full weight \eqref{eq:global-event-weight}, is at most
...
after multiplication by $\mathscr W$.
\end{proof}
...
\[
 C\,\mathbb E_{\Pi_{s,N}}\mathscr W\,r^{\Theta_{\rm PPE}^{W}}.
\]
\end{proposition}
...
disintegration of $\Pi_{s,N}^{W}$ on retained physical labels, and put
$\nu_\beta^{W}=(S_\beta)_*\mu_\beta^{W}$.  With
\begin{equation}
\label{eq:physical-energy-exponent}
 \Theta_E^{W}=\min\left\{\Theta_{\rm PPE}^{W},
...
\begin{align}
 ...
 &\le Cr^{\Theta_E^{W}}+C\mathcal T_E^{W}(r),\label{eq:conditional-energy-bound}\\
 \sup_z\sum_\beta w_\beta^{W}\nu_\beta^{W}([z-r,z+r])
 &\le Cr^{\Theta_E^{W}/2}+C\mathcal T_E^{W}(r)^{1/2}.
...
antichain mass, and \eqref{eq:named-energy-tails} gives exactly
$\mathcal T_E^{W}(r)$.  All failures are disjoint measurable pieces of the
...
bound back by $\mathbb E_{\Pi_{s,N}}\mathscr W$ gives the complete marked
...
$C(r^{\Theta_E^{W}/2}+\mathcal T_E^{W}(r)^{1/2})$.  Smooth value-space cutoffs on its complement have
...
weight and integration over $\Pi_{s,N}^{W}$.  Any label on which uniform
contraction is unavailable is retained as a named
$B_{{\rm event},j}$ cemetery event in $\mathrm{TENERGY}_{W}$, with rate
$c_{{\rm event},j}^{W}$.  Thus no unweighted exceptional tail is used in the
contracted-coordinate remainder.
...
charged by \eqref{eq:tilted-auxiliary-tails}.  In the diagonal band these imply
...
Use the complete tilted rates \eqref{eq:weighted-energy-rates} and set
\[
 A_{\rm frac}=3+\alpha,\qquad
 0<\delta<\frac{\alpha}{2A_{\rm frac}},\qquad
 c_{\rm aux}^{W}:=\min\{c_{\rm buf}^{W},c_{\rm gate}^{W},
 c_{\rm cont}^{W}\},
\]
\begin{equation}
\label{eq:complete-phase-exponent}
 \beta_{\rm tail}^{W}:=\tfrac12\delta C_{\rm buf}c_{\rm aux}^{W},\qquad
 \beta_{\rm phase}^{W}:=
 \min\{\delta\Theta_E^{W}/2,\alpha-A_{\rm frac}\delta,
             \beta_{\rm tail}^{W}\}>0.
\end{equation}
...
substituted for this resolving depth.  On the complement use two smooth
cutoffs in
...
\begin{align*}
 ...
 &\le C\operatorname{Mark}(Q)^2(1+Z(\mathcal G_Q))
       \Xi^{-\beta_{\rm phase}^{W}},
\end{align*}
where \eqref{eq:phase-energy-all-tails} is included through
$\beta_{\rm tail}^{W}$ in \eqref{eq:complete-phase-exponent}.
...
\label{eq:complete-central-exponent}
 ...
 c_{\rm low},c_{\rm event}^{W}\}>0,
\end{equation}
where $c_{\rm low}$ is supplied by the low recovery theorem and
$c_{\rm event}^{W}>0$ denotes the contracted-coordinate/event-tail rate
after its positive marked mass has been placed under the same complete
event tilt in
Lemma~\ref{lem:dominant-coordinate-reduction}.  Lemma~\ref{lem:gate-buffer} supplies the
...
\item The normal-cycle value $\mathfrak A=-1$ motivates the clean template;
      ...
      $\mathrm{PPE}_{N}$ and $\mathrm{TENERGY}_{W}$, together
      with the mass-preserving complete tree, give the weighted energy
      theorem and Jensen.
...
$\mathrm{TENERGY}_{W}$, $\mathrm{FZ}_{\rm geom}$,
...
$\mathrm{TENERGY}_{W}$ and $\mathrm{USC}_{\rm end}^{W}(q_g^W)$ on the
...
$\mathrm{SS}$, $\mathrm{PPE}_{N}$, $\mathrm{TENERGY}_{W}$,
...
First fix the complete event weight $\mathscr W_\lambda$ and all five
conditional moment exponents.  Compute $\chi_W$ in
...
context, stopping, auxiliary, PPE, and grazing rate by $1/p_W'$.  Otherwise
$\mathrm{TENERGY}_{W}$ and $\mathrm{USC}_{\rm end}^{W}$ remain independent
primitive inputs.  Next fix the coefficient of $\log S_\lambda^\pm$ in
...
the proof: it improves $\beta_{\rm tail}^{W}$ but enlarges $C_{\rm mid}$.
...
\label{eq:proved-full-product-Lp}
 \|\mathscr W\|_{L^{p_W}(\Pi_{s,N})}
 \le C\le C\mathbb E_{\Pi_{s,N}}\mathscr W.
\end{equation}
...
each earlier marginal is preserved.  Raise
\eqref{eq:global-event-weight} to $p_W$.  Conditional H\"older at the
component, native, scale, and context stages uses reciprocal exponents
$p_W/q_F$, $p_W/q_D$, $p_W/q_S$, $p_W/q_R$, and
$p_Wa_H/q_H$.  Their sum is $p_W\chi_W<1$, leaving one strict conjugate
slot.  Iterated conditional expectation therefore proves
\eqref{eq:proved-full-product-Lp}.  Since $\mathscr W\ge1$, its expectation
...
\[
 \Pi_{s,N}^{W}(B)
 =\frac{\mathbb E_{\Pi_{s,N}}[\mathscr W\one_B]}
        {\mathbb E_{\Pi_{s,N}}\mathscr W}
...
$q_H,q_F,q_D,q_S,q_R>1$ be the available context, flux, inverse-scale,
positive-scale, and return/context exponential moment exponents,
and assume
\begin{equation}
\label{eq:holder-conjugate-margin}
 \chi_W=\frac{a_H}{q_H}+q_F^{-1}+q_D^{-1}+q_S^{-1}+q_R^{-1}<1.
\end{equation}
...
\end{equation}
where $\mathscr W_\lambda$ is exactly the single weight in
\eqref{eq:global-event-weight}; no smaller unweighted ledger is substituted
...
by $\mathrm{TENERGY}_{W}$ on this identical tilt.
\end{proof}
...
\item the finite-horizon functional-correlation bound of \cite{LS}, the
      ...
      exponents $q_H,q_F,q_D,q_S,q_R$ satisfying
      \eqref{eq:holder-conjugate-margin}, with $p_W$ chosen in
      \eqref{eq:available-full-product-pW}; all four polynomial moments and
...
\item $\mathrm{NST}_{\rm phys}$, $\mathrm{SS}(d_Z)$, and
      $\mathrm{PPE}_{N}(d_Z)$ on the persistent clean gate, together with the
      complete tilted energy ledger $\mathrm{TENERGY}_{W}$ and the
...
$\mathrm{PPE}_{N}$, and $\mathrm{TENERGY}_{W}$, together with the complete
positive tree and its cemetery labels, give
...
$\mathrm{TENERGY}_{W}$, $\mathrm{USC}_{\rm end}^{W}$,
$\mathrm{BR}_{\rm sec}$,
...
$\mathrm{UCR}_{\rm sh}$, $\mathrm{TENERGY}_{W}$,
...
\eqref{eq:complete-phase-exponent} charges every named energy tail.
...
There are seventeen falsifiable interfaces.  $\mathrm{MT}_{\rm DQ}$ must
...
$\mathrm{PPE}_{N}$ must give actual-SRB escape at the prescribed depth, while
$\mathrm{TENERGY}_{W}$ must put that escape and every failure cemetery label
under the one full event tilt.
...
\begin{proposition}[Twenty-three obstruction tests]
\label{prop:v37-obstruction-tests}
...
\item If $\beta_{\rm tail}^{W}<
      \min\{\delta\Theta_E^{W}/2,\alpha-A_{\rm frac}\delta\}$, the old phase
      exponent overstates the resonant saving.  The minimum
      \eqref{eq:complete-phase-exponent} returns the correct
      $\beta_{\rm phase}^{W}=\beta_{\rm tail}^{W}$.
...
\begin{proof}
Items 1--3, 8--11, and 13--23 are direct finite-measure, frequency, or norm
