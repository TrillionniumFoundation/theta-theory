#!/usr/bin/env python3
"""Externally pinned cold launcher for the C79g v11 exact10 static bundle.
...
LAUNCHER_RELATIVE = Path("deliverables") / (BASE + "_cold_launch_v11.py")
...
ROOT = Path("/__C79G_V11_UNCONFIGURED_ROOT__")
...
V11_REJECTION_NAMESPACE = ROOT / "__unconfigured_v11_rejections__"
V11_LATER_REJECTION = V11_REJECTION_NAMESPACE / "rejection.json"
...
def configure_workspace_paths(root: Path) -> None:
    ...
    global V10_OFFICIAL_REJECTION
    global V11_REJECTION_NAMESPACE
    global V11_LATER_REJECTION, SCHEMA, CONTRACT, PRODUCER, CONSUMER
    ...
    global V9_EXACT10_PINS, V10_EXACT10_PINS
    ...
    global V10_FORBIDDEN_RUNTIME_PATHS
    ROOT = root
    OUT = ROOT / "deliverables"
    RUNTIME = ROOT / ".cm2-runtime"
    SELF = ROOT / LAUNCHER_RELATIVE
    ...
    V11_REJECTION_NAMESPACE = RUNTIME / ("c79g-v11-rejections-" + CHECKPOINT)
    V11_LATER_REJECTION = V11_REJECTION_NAMESPACE / "rejection.json"
    SCHEMA = OUT / (BASE + "_schema_v11.json")
    CONTRACT = OUT / (BASE + "_contract_v11.json")
    PRODUCER = OUT / (BASE + "_v11.py")
    CONSUMER = OUT / (BASE + "_independent_verifier_assembler_authority_consumer_v11.py")
    TRANSITION = OUT / (BASE + "_v10_to_v11_static_launch_transition_receipt_v1.json")
    AUDIT = OUT / (BASE + "_static_audit_v11.json")
    MANIFEST = OUT / (BASE + "_cold_launch_manifest_v11.sha256")
    OUTER = OUT / (BASE + "_cold_launch_outer_receipt_v11.json")
    BASE7_PINS = {
    ...
    EXACT8 = (V10_OFFICIAL_REJECTION, SCHEMA, CONTRACT, PRODUCER,
...
INNER_SCHEMA = "cm2.round306c79g.true-global-no-producer-consumer.v11.inner-composite"
COLD_ROOT_SCHEMA = (
    "cm2.round306c79g.true-global-no-producer-consumer.v11."
    "cold-launched-committed-authority"
)
COLD_ROOT_DOMAIN = b"CM2_C79G_V11_COLD_LAUNCHED_AUTHORITY_ROOT_V1\0"
...
LIVE_ACK_SCHEMA = (
    "cm2.round306c79g.true-global-no-producer-consumer.v11.cold-live-ack"
)
LIVE_RELEASE_SCHEMA = (
    "cm2.round306c79g.true-global-no-producer-consumer.v11.cold-live-release"
)
...
SOURCE_FD_ENV = "CM2_C79G_V11_COLD_SOURCE_FD"
EXEC_FD_ENV = "CM2_C79G_V11_COLD_EXEC_FD"
COORDINATION_PARENT_FD_ENV = "CM2_C79G_V11_COORDINATION_PARENT_FD"
WORKSPACE_ROOT_ENV = "CM2_C79G_V11_COLD_WORKSPACE_ROOT"
WORKSPACE_ROOT_FD_ENV = "CM2_C79G_V11_COLD_WORKSPACE_ROOT_FD"
LAUNCHER_SHA_ENV = "CM2_C79G_V11_COLD_LAUNCHER_FILE_SHA256"
...
def expected_v5_published_rejected_segment() -> dict[str, Any]:
    """Construct the one exact v5 regression shape shared by v11 proofs."""
    names = (
...
PIN_NORMALIZED_AST_ALGORITHM = (
    ...
    "PRESERVE_V10_AND_ALL_HISTORICAL_PINS_V1"
)
...
FINAL_STATIC_AUDIT_TOP_LEVEL_KEY_ORDER = (
    ...
    "audited_v11_bundle", "predecessor_v3_exact10_regression",
...
def pin_normalized_launcher_ast_sha256(raw: bytes, label: str) -> str:
    ...
    six current-v11 BASE7 values are replaced by fixed draft sentinels.  The v10
    ...
    expected_key_names = [
        "V10_OFFICIAL_REJECTION", *PIN_NORMALIZED_CURRENT_BASE7_KEY_ORDER,
    ]
    need(key_names == expected_key_names and
         len(base7.values) == len(expected_key_names),
         label + ":exact ordered v10-plus-current-v11 BASE7 key census")
    v10_before = ast.dump(
    ...
    need(ast.dump(
             ...
             include_attributes=False) == v10_before,
         label + ":v10 official-rejection BASE7 value preserved")
...
def producer_source_registry_shape_from_ast(raw: bytes) -> int:
    """Derive explicit54 + execution-proof5 + object closure1 structurally."""
    try:
        tree = ast.parse(raw.decode("utf-8"), filename="producer_v11",
                         mode="exec")
    except (UnicodeDecodeError, SyntaxError) as exc:
        raise Reject("current v11 producer registry AST parse") from exc
    ...
    need(isinstance(registry_function,
         ...
         "current v11 input_registry and HeldSelf.execution_proof")
    registry_returns = [
    ...
    need(len(registry_returns) == 1 and len(proof_returns) == 1,
         "unique current v11 registry/proof return dicts")
    ...
    need(len(explicit_keys) == len(set(explicit_keys)) == 54 and
         ...
         len(proof_dict.keys) == 5 and all(
             isinstance(key, ast.Constant) and isinstance(key.value, str)
             for key in proof_dict.keys),
         "current v11 registry explicit54 plus exact execution-proof5")
...
def validate_final_static_audit(
    ...
    need(set(audit) == audit_keys and
         audit.get("schema") ==
             "cm2.round306c79g.true-global-no-producer-consumer.static-audit.v11" and
         ...
         "final static audit exact top-level closure and PASS_V11 status")
    ...
    need(isinstance(input_a, dict) and set(input_a) == input_keys and
         ...
         "final static audit exact equal thirty-two-pin A/B input census")
    expected_inputs = {
        ...
        "launcher_template": pin_normalized_launcher_ast_sha256(
            launcher_source_raw, "held_v11_launcher_source"),
    ...
    need(isinstance(kind_census, dict) and set(kind_census) == kind_keys and
         ...
         dual.get(
             "pin_normalization_preserves_v10_and_all_historical_pins") is True and
         dual.get(
    ...
    need(isinstance(closure, dict) and set(closure) == closure_keys and
         ...
         closure.get("schema_definition_count") == 43 ==
             len(schema.get("$defs", {})) and
         type(closure.get("schema_ref_count")) is int and
         closure.get("schema_ref_count") == 235 ==
         ...
         closure.get("closed_object_count") == 49 == len(closed_nodes) and
         ...
         computed_registry_shape ==
             expected_shapes["producerSourceRegistry"] == 60 and
         closure.get(
    ...
    need(isinstance(no_run, dict) and set(no_run) == {
             "C79_entrypoint_executed", "C79_v11_runtime_artifact_count",
             "C79_v11_process_count", "pyc_or___pycache___created",
             "cold_manifest_or_outer_created_before_dual_GO",
         } and no_run.get("C79_entrypoint_executed") is False and
         type(no_run.get("C79_v11_runtime_artifact_count")) is int and
         no_run.get("C79_v11_runtime_artifact_count") == 0 and
         type(no_run.get("C79_v11_process_count")) is int and
         no_run.get("C79_v11_process_count") == 0 and
    ...
    def __init__(self, source: HeldFile) -> None:
        ...
        try:
            self.fd = os.memfd_create(
                "c79g-v11-child-exec", MFD_CLOEXEC | MFD_ALLOW_SEALING)
        except OSError as exc:
...
class HeldEmptyRejectionNamespace:
    ...
    def __init__(self, coordination: HeldCoordinationParent) -> None:
        coordination.verify()
        name = V11_REJECTION_NAMESPACE.name
        ...
        try:
            try:
                ...
                state = os.stat(name, dir_fd=coordination.fd,
                                follow_symlinks=False)
            self.path = V11_REJECTION_NAMESPACE
...
def construct_launcher_native_rejection(
    ...
    return close_object({
        ...
        "status": "PERMANENT_FAIL_CLOSED_REJECTION__ZERO_CREDIT",
        "effective_checkpoint_object_sha256": CHECKPOINT,
        "namespace_exact_path": str(V11_REJECTION_NAMESPACE.relative_to(ROOT)),
        "target_exact_path": str(V11_LATER_REJECTION.relative_to(ROOT)),
        "rejection_reason": "ORPHANED_OR_INCOMPLETE_C79G_V11_SURFACE",
...
def install_or_replay_launcher_native_rejection(
    ...
    try:
        try:
            ...
            namespace_fd = openat2_beneath(
                V11_REJECTION_NAMESPACE, os.O_RDONLY | os.O_DIRECTORY)
    except BaseException:
        if namespace_fd >= 0:
    ...
    try:
        ...
        path_before = root_lstat(V11_REJECTION_NAMESPACE)
        ...
        target = os.stat(V11_LATER_REJECTION.name, dir_fd=namespace_fd,
                         follow_symlinks=False) if V11_LATER_REJECTION.name in universe else None
        if target is not None:
            need(universe == {V11_LATER_REJECTION.name},
                 "idempotent rejection exact singleton")
            rejection_guard = HeldFile(
                V11_LATER_REJECTION, "existing launcher-native v11 rejection")
        else:
            ...
            try:
                ...
                os.fsync(namespace_fd)
            rejection_guard = HeldFile(
                V11_LATER_REJECTION, "new launcher-native v11 rejection")
        need(rejection_guard is not None,
             "launcher-native rejection file held")
        value = strict_json(rejection_guard.raw, "launcher-native v11 rejection")
        need(isinstance(value, dict), "launcher-native rejection mapping")
        verify_object(value, "launcher-native v11 rejection")
        os.fsync(rejection_guard.fd)
        os.fsync(namespace_fd)
        os.fsync(coordination.fd)
        after = os.fstat(namespace_fd)
        path_after = root_lstat(V11_REJECTION_NAMESPACE)
        need(value == expected and rejection_guard.raw == expected_raw and
             set(os.listdir(namespace_fd)) == {V11_LATER_REJECTION.name} and
...
class HeldBundle:
    ...
    def _initialize(self, expected_launcher_sha256: str,
                    ...
                    bootstrap: HeldBootstrapEntry) -> None:
        coordination.verify()
        need(SELF == OUT / (BASE + "_cold_launch_v11.py"),
        ...
        for path, (file_pin, object_pin) in BASE7_PINS.items():
            need(re.fullmatch(r"[0-9a-f]{64}", file_pin) is not None and
                 file_pin != "0" * 64 and
                 (path == V10_OFFICIAL_REJECTION or
        ...
        for path in (V10_OFFICIAL_REJECTION, CONTRACT, TRANSITION, AUDIT):
        ...
        # Current exact8 owns the sole v10-rejection fd.  Each frozen exact10
        # owns the preceding rejection fd; v3 keeps its separate singleton.
        self.predecessor_v10 = HeldV10PredecessorExact10(
            coordination, self.by_path[V10_OFFICIAL_REJECTION])
        ...
        identity_groups = [
            current_identities, predecessor_v10_identities,
        ...
        need(len(all_history_identities) == 88 and
             [len(group) for group in identity_groups] ==
                 [10, 10, 10, 10, 10, 10, 10, 10, 7, 1] and
             ...
             "current v11 exact10, v10/v9/v8/v7/v6/v5/v3 exact10, v3 "
             "rejection and v4 draft7 are 88 unique live files on one mount")
    ...
    def terminal_replay(self) -> None:
        ...
        self.predecessor_v10.terminal_replay(
            self.by_path[V10_OFFICIAL_REJECTION])
...
def cold_root(bundle: HeldBundle, coordination: HeldCoordinationParent,
    ...
    need(bundle.predecessor_v5 is not None and
         ...
         bundle.predecessor_v10 is not None,
         "v10 direct predecessor and v9/v8/v7/v6/v5 history remain held")
    ...
    proof = {
        ...
        "current_exact8_first_member_is_v10_official_rejection":
            EXACT8[0] == V10_OFFICIAL_REJECTION,
        ...
        "all_current_and_historical_88_identities_globally_unique_on_one_statx_mount": True,
        "current_v11_and_all_predecessor_unique_file_identity_count": 88,
        "current_v11_and_all_predecessor_same_statx_mount": True,
        **bundle.chronology,
        ...
        "reject_command_exempts_only_fresh_empty_v11_rejection_namespace": True,
...
def _validate_live_ack(
    ...
    need(set(ack) == expected_keys and ack.get("schema") == LIVE_ACK_SCHEMA and
         ...
         ack.get("static_v11_exact10_with_v10_exact10_and_official_rejection_v9_exact10_and_official_rejection_v8_exact10_and_official_rejection_v7_exact10_and_official_rejection_v6_exact10_and_official_rejection_v5_exact10_and_official_rejection_v4_supersession_v3_exact10_and_official_rejection_terminal_replay_completed_by_launcher_before_request") is True and
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "reject":
            ...
            return 0
        # Every positive-capable command holds the exact-empty v11 rejection
