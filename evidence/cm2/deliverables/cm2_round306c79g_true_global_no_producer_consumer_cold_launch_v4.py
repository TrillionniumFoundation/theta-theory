#!/usr/bin/env python3
"""Externally pinned cold launcher for the C79g v4 exact10 static bundle.
...
SCHEMA = OUT / (BASE + "_schema_v4.json")
CONTRACT = OUT / (BASE + "_contract_v4.json")
PRODUCER = OUT / (BASE + "_v4.py")
CONSUMER = OUT / (BASE + "_independent_verifier_assembler_authority_consumer_v4.py")
TRANSITION = OUT / (BASE + "_v3_to_v4_static_launch_transition_receipt_v1.json")
AUDIT = OUT / (BASE + "_static_audit_v4.json")
MANIFEST = OUT / (BASE + "_cold_launch_manifest_v4.sha256")
OUTER = OUT / (BASE + "_cold_launch_outer_receipt_v4.json")
REJECTION_NAMESPACE = RUNTIME / ("c79g-v4-rejections-" + CHECKPOINT)
...
BASE7_PINS: dict[Path, tuple[str, str | None]] = {
    REJECTION: ("57ce7a4361555c4ff403f725f17ef9cef50446a2e76d7086635c30a0f17bc62f",
                "c946e0d8170a75e32aa7031b42cf0dd5b7b585ba463b7b1ce0010cb65bb3b421"),
    SCHEMA: ("b2cb58e88b66daed7d0449d11a4441095179b7e537a48a35ee9c630f76beca6a", None),
    CONTRACT: ("84608ada466fb9aa3d99adfd34662e9af02dfd009d8eb185374c065a6da0c23c",
               "62609ff0809ba9439be07a215ece772262f30ead7d87fda92a6e4e23bb5895b7"),
    PRODUCER: ("968e401852f45d5104f48526b50d4f59a86bb1fed8e7af23782c5f6d447d1cf1", None),
    CONSUMER: ("23753584725365cdc0174d518c26b7d8dbdeb12eea2345a5e3b281274548420c", None),
    TRANSITION: ("4c46f7cf8eaba93fd1b9562c288c2117b49d7c8cdeb62482b93c5e247d412256",
                 "d2d0797a6edb8463f9af7007e28356932472f2e0478fb73b35869d3f51184129"),
    AUDIT: ("5efa44ae5cf78000940a4640a98ac0fe486e1946aecc9d20e7023e27ed13049c",
...
INNER_SCHEMA = "cm2.round306c79g.true-global-no-producer-consumer.v4.inner-composite"
COLD_ROOT_SCHEMA = (
    "cm2.round306c79g.true-global-no-producer-consumer.v4."
    "cold-launched-committed-authority"
)
COLD_ROOT_DOMAIN = b"CM2_C79G_V4_COLD_LAUNCHED_AUTHORITY_ROOT_V1\0"
...
LIVE_ACK_SCHEMA = (
    "cm2.round306c79g.true-global-no-producer-consumer.v4.cold-live-ack"
)
LIVE_RELEASE_SCHEMA = (
    "cm2.round306c79g.true-global-no-producer-consumer.v4.cold-live-release"
)
...
SOURCE_FD_ENV = "CM2_C79G_V4_COLD_SOURCE_FD"
COORDINATION_PARENT_FD_ENV = "CM2_C79G_V4_COORDINATION_PARENT_FD"
WORKSPACE_ROOT_ENV = "CM2_C79G_V4_COLD_WORKSPACE_ROOT"
LAUNCHER_SHA_ENV = "CM2_C79G_V4_COLD_LAUNCHER_FILE_SHA256"
...
class HeldV3RejectionNamespace:
    ...
    def __init__(self, coordination: HeldCoordinationParent,
        ...
        coordination.verify()
        self.path = REJECTION.parent
        self.fd = openat2_beneath(self.path, os.O_RDONLY | os.O_DIRECTORY)
        self.before = os.fstat(self.fd)
        self.mount_id = mount_id(self.fd)
        before_path = self.path.lstat()
        first = set(os.listdir(self.fd))
        member = os.stat(REJECTION.name, dir_fd=self.fd, follow_symlinks=False)
        after = os.fstat(self.fd)
        after_path = self.path.lstat()
        second = set(os.listdir(self.fd))
        need(stat.S_ISDIR(self.before.st_mode) and
             ...
             first == second == {REJECTION.name} and
...
class HeldV3PredecessorExact10:
    ...
    def __init__(self, coordination: HeldCoordinationParent,
                 rejection: HeldFile) -> None:
        coordination.verify()
        self.files = [HeldFile(path, "frozen v3 exact10:" + path.name, file_pin)
                      for path, file_pin, _ in V3_EXACT10_PINS]
        self.by_path = {item.path: item for item in self.files}
        need(len(self.by_path) == 10 and len({item.identity for item in self.files}) == 10 and
        ...
        need(isinstance(rejected, dict) and
             ...
             rejected.get("target_exact_path") ==
                 str(REJECTION.relative_to(ROOT)) and
             rejected.get("namespace_exact_path") ==
                 str(REJECTION.parent.relative_to(ROOT)) and
...
class HeldBundle:
    def __init__(self, expected_launcher_sha256: str,
                 ...
                 launcher_self: HeldFile) -> None:
        coordination.verify()
        need(SELF == OUT / (BASE + "_cold_launch_v4.py"),
        ...
        need(launcher_self.path == SELF and
             launcher_self.file_sha256 == expected_launcher_sha256,
             "pre-lock external launcher anchor remains the held SELF fd")
        # SELF is the sole externally anchored file and is opened before the
        # coordination lock.  Every other exact8 member, including the
        # official v3 rejection, is opened only after that lock is held.
        self.files = [
            launcher_self if path == SELF else HeldFile(
        ...
        # The official later rejection is a shared held fd: it is part of the
        # v4 exact8 and simultaneously closes the predecessor surface.  The
        # predecessor helper opens and retains the live v3 exact10 and the
        # exact singleton rejection namespace under this same lock.
        self.predecessor = HeldV3PredecessorExact10(
            coordination, self.by_path[REJECTION])
        ...
        need(len(predecessor_identities) == 10 and
             ...
             len({item.mount_id for item in self.predecessor.files} |
                 {item.mount_id for item in [*self.files, self.manifest,
                                             self.outer]}) == 1,
             "v3 exact10 and v4 exact10 are disjoint live files on one mount")

    def terminal_replay(self) -> None:
        self.predecessor.terminal_replay(self.by_path[REJECTION])
    ...
    proof = {
        ...
        **bundle.chronology,
        ...
        "official_v3_later_rejection": {
            "path": str(REJECTION.relative_to(ROOT)),
            "namespace_path": str(REJECTION.parent.relative_to(ROOT)),
            "file_sha256": bundle.by_path[REJECTION].file_sha256,
            "object_sha256": BASE7_PINS[REJECTION][1],
        },
        "launcher_same_lock_live_v3_exact10_and_official_rejection_singleton_namespace_held": True,
        "reject_command_exempts_only_fresh_empty_v4_rejection_namespace": True,
...
def _validate_live_ack(
    ...
    need(set(ack) == expected_keys and ack.get("schema") == LIVE_ACK_SCHEMA and
         ...
         ack.get("static_v4_exact10_v3_exact10_and_official_rejection_namespace_terminal_replay_completed_by_launcher_before_request") is True and
...
def main(argv: list[str] | None = None) -> int:
    ...
    need(re.fullmatch(r"[0-9a-f]{64}", args.expected_launcher_sha256) is not None and
         args.expected_launcher_sha256 != "0" * 64,
         "caller-supplied sole external launcher SHA-256 anchor")
    # The external anchor is the only pre-lock evidence open.  The official
    # rejection and every other static/runtime surface are opened after the
    # launcher owns the coordination lock.
    launcher_self = HeldFile(
        SELF, "cold exact8:" + SELF.name, args.expected_launcher_sha256)
    wrapper_committed = False
    ...
    try:
        coordination = HeldCoordinationParent()
        bundle = HeldBundle(
            args.expected_launcher_sha256, coordination, launcher_self)
        ...
        if bundle is not None:
            ...
            # Bundle ownership of launcher_self begins only after successful
            # construction.  A constructor failure must not leak the anchor.
            launcher_self.close()
