#!/usr/bin/env python3
"""C79g v8 append-only zero-credit candidate builder.

This append-only v8 binds the independently verified C78s 13-member final
...
is permanently non-authoritative.  The separately committed v8 seal plus the
...
COLD_COORDINATION_PARENT_FD_ENV = "CM2_C79G_V8_COORDINATION_PARENT_FD"
...
SELF = OUT / "cm2_round306c79g_true_global_no_producer_consumer_v8.py"
...
STATIC_AUDIT_V8 = OUT / "cm2_round306c79g_true_global_no_producer_consumer_static_audit_v8.json"
COLD_LAUNCHER = OUT / "cm2_round306c79g_true_global_no_producer_consumer_cold_launch_v8.py"
COLD_MANIFEST = OUT / "cm2_round306c79g_true_global_no_producer_consumer_cold_launch_manifest_v8.sha256"
COLD_OUTER = OUT / "cm2_round306c79g_true_global_no_producer_consumer_cold_launch_outer_receipt_v8.json"
# Final v8 schema/contract pins are installed as one post-audit update.  The
# former draft sentinels remain listed only as permanently forbidden values.
FINAL_V8_CORE_PINS_INSTALLED = True
...
COLD_EXACT8 = (
    ...
    CONTRACT,
    SELF,
    OUT / "cm2_round306c79g_true_global_no_producer_consumer_independent_verifier_assembler_authority_consumer_v8.py",
    V7_TO_V8_TRANSITION,
    STATIC_AUDIT_V8,
    COLD_LAUNCHER,
)
...
def _linux_libc() -> ctypes.CDLL:
    need(sys.platform.startswith("linux"), "Linux-only v8 publication protocol")
...
class HeldSelf:
    ...
    def __init__(self, exec_inherited: int, source_inherited: int,
        ...
        need(SELF == OUT / "cm2_round306c79g_true_global_no_producer_consumer_v8.py",
...
def new_candidate_stage(outdir: Path, parent: HeldParent) -> tuple[Path, HeldMutableStage]:
    ...
    name = ".c79g-v8-candidate-stage-" + orientation + "-" + CHECKPOINT_OBJECT_PIN
...
def strict_bool_predecessor_and_successor_census(
        ...
        v8_sources: Mapping[str, bytes]) -> dict[str, Any]:
    """Prove frozen v5 defects and zero-unsafe complete v6/v7/v8 boxes."""
    ...
    v8 = {role: _strict_bool_need_census(v8_sources[role], role)
          for role in v8_roles}
    ...
    v8_unproved = sum(
        v8[role]["recursive_exact_bool_unproved_count"] for role in v8_roles)
    v8_arity = sum(
        v8[role]["arity_star_or_keyword_failure_count"] for role in v8_roles)
    v8_returns = sum(
        v8[role]["annotated_bool_return_failure_count"] for role in v8_roles)
    need(v6_unproved == 0 and v6_arity == 0 and v6_returns == 0 and
         v7_unproved == 0 and v7_arity == 0 and v7_returns == 0 and
         v8_unproved == 0 and v8_arity == 0 and v8_returns == 0,
         "v6/v7/v8 recursive exact-bool full boxes have zero unsafe or unproved sites")
    return {
        ...
        "same_seven_absent_from_v8": True,
        ...
        "v8_recursive_exact_bool_unproved_count": v8_unproved,
        ...
        "v8_direct_need_call_count": sum(
            v8[role]["direct_need_call_count"] for role in v8_roles),
...
def _validate_v7_regression_section(
    ...
    need(strict_bool_census["v7_recursive_exact_bool_unproved_count"] == 0 and
         strict_bool_census["v8_recursive_exact_bool_unproved_count"] == 0 and
         strict_bool_census["all_twelve_sources_AST_parsed_and_compiled_in_memory"]
             is True and
         strict_bool_census["compiled_code_executed"] is False,
         label + ":v7/v8 full-callsite strict-bool closure")
    need(section.get("all_ten_file_pins_match") is True and
         section.get("all_declared_object_pins_match") is True and
         section.get("all_ten_regular_0444_nlink1") is True and
         section.get("exact8_manifest_reconstructs_first_eight_in_order") is True and
...
def _validate_final_static_audit(
    ...
    """Fail closed on every final v8 dual-static-audit acceptance field.
    ...
    need(set(audit) == {
             ...
             "effective_checkpoint_object_sha256", "audited_v8_bundle",
         ...
         "final v8 static audit exact top-level PASS_V8 closure")
    ...
    need(isinstance(dual, dict) and set(dual) == {
         ...
         }, "final v8 static audit dual-checker exact8 closure")
    ...
    need(isinstance(checker_a, dict) and set(checker_a) == {
         ...
         isinstance(checker_c, dict) and set(checker_c) == {
             ...
             "starred_positional_total", "double_star_keyword_total",
             "failed_static_check_count",
         }, "final v8 checker A/B/C exact object closures")
    need(checker_a.get("algorithm") ==
         ...
         "final v8 checker A/B/C algorithms and exact GO statuses")
    ...
    need(isinstance(input_a, dict) and isinstance(input_b, dict) and
         ...
         "final v8 checker A/B exact17 identical input-pin closure")
    expected_inputs = {
    ...
    need(input_a == expected_inputs and
         ...
         "final v8 checker inputs pin exact current core and append-only history")
    ...
    need(_nonzero_lower_sha256(normalized_a) is True and
         ...
         "final v8 three-way normalized and B/C common-callsite consensus")
    ...
    need(isinstance(kind_census, dict) and set(kind_census) == {
         ...
         "final v8 checker C exact common-callsite kind census")
    need(type(checker_a.get("wider_local_callsite_census_row_count")) is int and
         ...
         "final v8 checker A positive census and three-file in-memory compile")
    need(type(checker_a.get("arity_failure_count")) is int and
         ...
         "final v8 checker A zero arity/undefined/duplicate/failed counts")
    for key in (
    ...
    need(type(dual.get("independent_normalizer_count")) is int and
         ...
         "final v8 dual exact 3/2 checker counts and type-exact consensuses")
    ...
    need(isinstance(attacks, dict) and set(attacks) == {
         ...
         "final v8 coherent attack exact7, 121/121, routes, and deferred execution")
    ...
    need(isinstance(closure, dict) and set(closure) == {
         ...
         }, "final v8 schema/constructor exact21 closure")
    need(type(closure.get("schema_definition_count")) is int and
         closure.get("schema_definition_count") == 40 and
         type(closure.get("schema_ref_count")) is int and
         closure.get("schema_ref_count") == 212 and
         ...
         closure.get("closed_object_count") == 46 and
         type(closure.get(
         ...
         "final v8 schema 40 defs/212 refs/0 unresolved/46 closed/0 mismatch")
    need(closure.get("strict_JSON_duplicate_keys_rejected") is True and
         ...
         "final v8 schema and live-protocol type-exact bool closure")
    actual_keywords = [
    ...
    need(closure.get("actual_schema_keyword_universe") == actual_keywords and
         ...
         "final v8 exact canonical schema keyword universes and digests")
    shapes = closure.get("output_shape_key_counts", {})
    expected_shapes = {
        "selfIdentity": 33,
        "independentConsumerProof": 53,
        "staticFreezeProof": 60,
        "coldLaunchProof": 96,
        "laterRejection": 46,
        "producerSourceRegistry": 45,
        "liveRequest": 15,
        "liveACK": 32,
        "liveACKCensus": 30,
    }
    need(isinstance(shapes, dict) and set(shapes) == set(expected_shapes) and
         ...
         "final v8 exact nine output-shape key counts")
    ...
    need(isinstance(acceptance, dict) and set(acceptance) == {
         ...
         "final v8 audit acceptance exact10 type-exact zero-runtime closure")
...
def hold_static_freeze_trust(self_guard: HeldSelf) -> dict[str, Any]:
    """Hold current v8 exact10 and every distinct append-only predecessor.

    Current BASE7 begins with the official v7 rejection itself.  The v7, v6,
    and v5 published exact10 surfaces remain distinct; v3 exact10, the
    separate v3 rejection, and the seven other v4 draft files are also held.
    The exact distinct-file identity census is 10+10+10+10+10+7+1 = 58.
    """
    manifest_guard = HeldPinnedInput(
        COLD_MANIFEST, "C79g v8 cold-launch manifest", 0o444)
    outer_guard = HeldPinnedInput(
        COLD_OUTER, "C79g v8 cold-launch outer-last receipt", 0o444)
    manifest_entries = parse_manifest_ordered(
        manifest_guard.raw, "C79g v8 cold-launch exact8 manifest")
    ...
    need(len(manifest_by_path) == 8 and
         ...
         manifest_by_path[V7_OFFICIAL_REJECTION] ==
             V7_OFFICIAL_REJECTION_FILE_PIN,
         "cold-launch manifest current SELF, v7 rejection, and policy pins")

    by_path: dict[Path, HeldPinnedInput] = {}
    for path in COLD_EXACT8:
        if path != SELF:
            by_path[path] = HeldPinnedInput(
                path, "C79g v8 cold exact8:" + path.name, 0o444,
                manifest_by_path[path])
    v7_rejection_guard = by_path[V7_OFFICIAL_REJECTION]
    ...
    need(len(current_cold_ten_guards) == 10 and
         ...
         "current v8 cold exact10 unique held identities on one statx mount")
    exact8_stats = [
    ...
    need(all(chronology.values()),
         "physical v8 exact8/manifest/outer freeze chronology")
    ...
    all_file_guards = [
        *current_cold_ten_guards, *v7_guards, *v6_guards, *v5_guards, *v3_guards,
        *v4_guards, v3_rejection_guard,
    ]
    need(len(all_file_guards) == 58 and
         len({guard.identity for guard in all_file_guards}) == 58 and
         len({guard.mount_id for guard in all_file_guards}) == 1 and
         v7_namespace_guard.mount_id == v6_namespace_guard.mount_id ==
             v5_namespace_guard.mount_id == all_file_guards[0].mount_id,
         "append-only v8/v7/v6/v5/v3/v4/rejection history is 58 distinct files on one mount")
    require_v5_positive_and_stage_surfaces_absent()

    v8_consumer_path = COLD_EXACT8[4]
    strict_bool_census = strict_bool_predecessor_and_successor_census(
        ...
        {
            "producer_v8": self_guard.raw,
            "consumer_v8": by_path[v8_consumer_path].raw,
            "launcher_v8": by_path[COLD_LAUNCHER].raw,
        })

    transition_guard = by_path[V7_TO_V8_TRANSITION]
    audit_guard = by_path[STATIC_AUDIT_V8]
    transition = strict_json(transition_guard.raw, "C79g v8 transition trust")
    audit = strict_json(audit_guard.raw, "C79g v8 static audit trust")
    outer = strict_json(outer_guard.raw, "C79g v8 cold-launch outer trust")
    ...
    verify_object(transition, "C79g v8 transition trust")
    verify_object(audit, "C79g v8 static audit trust")
    verify_object(outer, "C79g v8 cold-launch outer trust")
    ...
    transition_pin = audited.get("v7_to_v8_transition_receipt", {})
    ...
    _validate_v7_regression_section(
        published_v7, strict_bool_census, "v8 transition v7 predecessor")
    _validate_v7_regression_section(
        audit_v7, strict_bool_census, "v8 audit v7 predecessor")
    ...
    need(transition.get("status") ==
         ...
         audit_v4.get(
             "v8_closed_object_required_property_mismatch_count") == 0 and
         audit_v4.get("same_defects_absent_from_v8") is True and
         ...
         "transition/audit pin current v8 and preserve exact v3/v4/v7 history")
    ...
    need(set(outer) == {
         ...
         "cold-launch v8 outer-last exact8 and zero-credit closure")
    ...
    return {
        "guards": [
            *by_path.values(), *v7_guards, *v6_guards, *v5_guards, *v3_guards,
            *v4_guards, v3_rejection_guard,
            v7_namespace_guard, v6_namespace_guard, v5_namespace_guard,
        ...
        "append_only_history_unique_file_identity_count": 58,
...
def ensure_launch_configuration() -> HeldSelf:
    ...
    need(FINAL_V8_CORE_PINS_INSTALLED is True,
         "v8 final core pins are not installed; runtime permanently disabled")
    need(all(value not in V8_DRAFT_CORE_PIN_SENTINELS for value in (
             CONTRACT_FILE_PIN, CONTRACT_OBJECT_PIN, CLOSED_SCHEMA_FILE_PIN)),
         "v8 draft contract/schema sentinels must all be replaced before runtime")
    ...
    need(all(value and len(value) <= 10 and
         ...
         SELF == ROOT / "deliverables/cm2_round306c79g_true_global_no_producer_consumer_v8.py",
    ...
    need(all(isinstance(value, str) and len(value) == 64 and
         ...
         "C79g v8 exact non-placeholder contract/schema/rejection pins")
    need(set(C78S_FINAL["pins"]) == set(C78S_NAMES) and
         ...
         "C79g v8 exact filled launch configuration")
...
def validate_static_authorities(static_trust: dict[str, Any]) -> dict[str, Any]:
    ...
    need(contract.get("effective_checkpoint_object_sha256") == CHECKPOINT_OBJECT_PIN and
         ...
         contract.get("v8_bundle", {}).get(
             ...
             "independent_verifier_assembler_authority_consumer_v8.py" and
         contract.get("v8_bundle", {}).get("closed_schema_validator_policy") == {
         ...
         "v8 frozen contract checkpoint/composite-only boundary")
    ...
    need(closed.get("$ref") == "#/$defs/coldLaunchedCommittedAuthority" and
         ...
         "append-only v8 cold root, zero-credit inner, and persisted zero boundary")
    ...
    return close_object({
        ...
        **self_guard.execution_proof(),
        ...
        "post_source_static_freeze_trust": {
            "v7_to_v8_transition_receipt": {
                "path": str(V7_TO_V8_TRANSITION.relative_to(ROOT)),
                "file_sha256": static_trust["transition_file_sha256"],
                "object_sha256": static_trust["transition_object_sha256"],
            },
            "static_audit_v8": {
                "path": str(STATIC_AUDIT_V8.relative_to(ROOT)),
...
def build(outdir: Path, self_guard: HeldSelf,
    ...
    lock = (
        "C79g v8 isolated candidate only; standalone non-authoritative. Formal-global credit "
