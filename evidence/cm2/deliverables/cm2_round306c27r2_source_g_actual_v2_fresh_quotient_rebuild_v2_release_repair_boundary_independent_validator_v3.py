...
OUTPUT_SCHEMA = PREFIX + "release-boundary-validator.v3"
FORMAL_STATUS = (
...
def validate_run(resolver: Resolver, args: argparse.Namespace,
    ...
    normalized_inputs = {logical_name(path) for path in input_paths}
    required_inputs = {
        logical_name(args.seed1_edge_relative),
        logical_name(args.seed2_edge_relative),
        logical_name(args.frozen_c15_relative),
    ...
    need(spec.get("schema") == SPEC_SCHEMA and spec.get("stage") == stage
         ...
         and required_inputs <= normalized_inputs
...
def validate_cold(resolver: Resolver, args: argparse.Namespace,
    ...
    required_inputs = {
        ...
        logical_name(args.seed2_edge_relative),
        logical_name(args.frozen_c15_relative),
        logical_name(args.cold_helper_relative),
        SOURCE_PATHS["independent_verifier"],
        SOURCE_PATHS["transaction_runner"],
        *{str(Path(logical_name(args.candidate_relative)) / name)
          for name in CANDIDATE_FILES},
    }
...
def self_test() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="c27r2-boundary-v3-") as raw:
        root = Path(raw)
        normal = root / "normal"
