#!/usr/bin/env python3
"""No-import verifier for the append-only v2 C30e mathematical candidate.
...
The sole ledger predecessor is a future versioned C30d terminal graph.  A
dynamically SHA-pinned external adapter must validate that graph and emit the
exact consumer projection specified below.  This verifier never accepts a
legacy sealed alias and independently recaptures every projected terminal
member.  Verification success still grants zero formal credit: only a later
C30e release terminal can authorize the proposed 58-to-56 transition.
"""
...
PRODUCER_SHA256 = (
    "4e3f1759e170ed36a715eee815a8aca5d4e557fba919ac507bb39c92e69dcb08"
)
...
C30D_ADAPTER_PROJECTION_SCHEMA = (
    "cm2.round306c30e.c30d-terminal-predecessor-projection.v1"
)
C30D_ADAPTER_PROJECTION_STATUS = (
    "PASS_EXACT_VERSIONED_C30D_TERMINAL_GRAPH_AS_C30E_PREDECESSOR__"
    "ZERO_C30E_FORMAL_CREDIT"
)
C30D_TERMINAL_REQUIRED_MEMBERS = tuple(sorted((
    "chain_status.json",
...
def _disabled_legacy_verifier_capture_c30d_authority() -> dict[str, Any]:
    """Append-only tombstone: legacy sealed C30d can never be consumed."""
    raise Reject("LEGACY_C30D_SEALED_ALIAS_DISABLED_IN_V2")
    ...
    require(
        ...
        and verification.get("status") == "LEGACY_PATH_DISABLED"
...
def verifier_valid_sha256(value: Any) -> bool:
    return bool(
        type(value) is str
        and len(value) == 64
...
def verifier_identity(value: os.stat_result) -> tuple[int, int, int, int, int]:
    return (
        value.st_dev,
        value.st_ino,
        value.st_size,
...
def verifier_capture_file(
    ...
    require(
        ...
        "C30d adapter authority regular singleton:" + path.name,
    )
    descriptor = os.open(
    ...
    try:
        ...
        require(
            verifier_identity(opened) == verifier_identity(before),
            "C30d adapter authority open race:" + path.name,
        )
        chunks: list[bytes] = []
    ...
    raw = b"".join(chunks)
    require(
        verifier_identity(opened) == verifier_identity(closed)
        == verifier_identity(path.lstat())
        and len(raw) == opened.st_size
        and hashlib.sha256(raw).hexdigest() == expected_sha256,
        "C30d adapter authority stable capture/hash:" + path.name,
    )
    return raw


def verifier_strict_projection(raw: bytes) -> dict[str, Any]:
    def unique(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
        output: dict[str, Any] = {}
        for key, value in pairs:
            require(
                type(key) is str and key not in output,
                "C30d adapter projection unique JSON keys",
            )
            output[key] = value
        return output
    ...
    require(
        type(value) is dict and wire(value) == raw,
        "C30d adapter canonical projection object",
    )
    return value


def verifier_projection_members(
    ...
    require(
        ...
        and value.get("schema") == C30D_ADAPTER_PROJECTION_SCHEMA
        and value.get("status") == C30D_ADAPTER_PROJECTION_STATUS,
        "C30d adapter exact projection identity/closure",
    )
    authority = value.get("authority")
    require(type(authority) is dict, "C30d projection authority object")
    ...
    require(
        set(authority) == {
            ...
            "chain_dir_basename",
            "dynamic_pins_sha256",
            "required_members_sha256",
            "payload_manifest_member_count",
            "root_manifest_member_count",
            "root_binds_payload_manifest",
            "root_binds_outer_verification",
            "payload_all_members_verified",
            "all_receipt_object_hashes_closed",
            "terminal_seal_exact_pass",
            "terminal_replay_exact_pass",
            "chain_status_exact_pass",
        }
        and authority.get("adapter_contract")
        == "DIRECT_VERSIONED_C30D_TERMINAL_GRAPH__NO_LEGACY_SEALED_ALIAS"
        and authority.get("chain_dir_basename") == chain.name
        and authority.get("dynamic_pins_sha256") == pins_sha256
        and type(members) is dict
        and set(members) == set(C30D_TERMINAL_REQUIRED_MEMBERS)
        and all(verifier_valid_sha256(item) for item in members.values())
        ...
        "C30d adapter exact terminal graph closure",
    )
    require(
        value.get("formal_handoff") == {
            ...
            "after": C30D_AFTER,
            "transition_status": (
                "PASS_FORMAL_C30D_TERMINAL__78_TO_58_AUTHORIZED"
            ),
            "terminal_replay_authorizes_transition": True,
        },
        "C30d adapter exact formal handoff",
    )
    require(
        value.get("release_boundary") == {
            ...
            "formal_credit": C30E_ZERO_CREDIT,
        ...
        "C30d adapter exact zero-credit release boundary",
    )
    assert type(members) is dict
    return {name: members[name] for name in sorted(members)}


def verifier_capture_c30d_authority_v2(
    adapter_path: Path,
    adapter_sha256: str,
    chain_path: Path,
    pins_path: Path,
    ...
    require(
        verifier_valid_sha256(adapter_sha256)
        and verifier_valid_sha256(pins_sha256),
        "well-formed dynamic C30d adapter/pins hashes",
    )
    adapter = Path(os.path.abspath(os.fspath(adapter_path)))
    chain = Path(os.path.abspath(os.fspath(chain_path)))
    pins_path = Path(os.path.abspath(os.fspath(pins_path)))
    require(
        adapter != pins_path and adapter.name != pins_path.name,
        "distinct C30d adapter and dynamic-pins identities",
    )
    verifier_capture_file(adapter, adapter_sha256, 8 << 20)
    verifier_capture_file(pins_path, pins_sha256, 4 << 20)
    try:
        directory_before = chain.lstat()
    except OSError as error:
        raise Reject(
            "C30D_TERMINAL_GRAPH_INCOMPLETE:chain directory"
        ) from error
    require(
        stat.S_ISDIR(directory_before.st_mode) and not chain.is_symlink(),
        "C30D_TERMINAL_GRAPH_INCOMPLETE:chain directory",
    )
    for name in C30D_TERMINAL_REQUIRED_MEMBERS:
        path = chain / name
        require(
            path.is_file() and not path.is_symlink(),
            "C30D_TERMINAL_GRAPH_INCOMPLETE:" + name,
        )
    ...
    require(
        ...
        and 1 < len(completed.stdout) <= 4 << 20
        and completed.stdout.endswith(b"\n")
        and completed.stdout.count(b"\n") == 1,
        "C30d adapter exit zero, empty stderr and one canonical stdout line",
    )
    projection = verifier_strict_projection(completed.stdout[:-1])
    verifier_capture_file(adapter, adapter_sha256, 8 << 20)
    verifier_capture_file(pins_path, pins_sha256, 4 << 20)
    members = verifier_projection_members(projection, chain, pins_sha256)
    member_facts: dict[str, dict[str, Any]] = {}
    for name in C30D_TERMINAL_REQUIRED_MEMBERS:
        raw = verifier_capture_file(chain / name, members[name], 64 << 20)
        member_facts[name] = {
            "sha256": hashlib.sha256(raw).hexdigest(), "size": len(raw),
        }
    require(
        verifier_identity(directory_before) == verifier_identity(chain.lstat()),
        "C30d terminal directory stable across independent capture",
    )
    pins = {
        "external-adapter:" + adapter.name: adapter_sha256,
        "dynamic-pins:" + pins_path.name: pins_sha256,
        **{
            "c30d-terminal:" + name: members[name]
            for name in C30D_TERMINAL_REQUIRED_MEMBERS
        },
    }
    return {
        ...
        "terminal_member_facts": member_facts,
        "formal_before": C30D_BEFORE,
        "formal_after": C30D_AFTER,
        "formal_credit": C30D_CREDIT,
        "terminal_replay_sha256": members["terminal_replay.json"],
        "root_manifest_sha256": members["root_manifest.sha256"],
        "payload_manifest_sha256": members["payload_manifest.sha256"],
        "outer_verification_sha256": members["outer_verification.json"],
        "chain_status_sha256": members["chain_status.json"],
    }
...
def inherited_same_sign_binding(evidence: dict[str, Any]) -> dict[str, Any]:
    require(
        ...
        and evidence["coarse_disposition"] == "LIVE"
        ...
        "reduced-live inherited same-sign Delta followup H",
    )
    return {
...
def reconstruct_reference(
    ...
    authority = verifier_capture_c30d_authority_v2(
        adapter_path,
        adapter_sha256,
        chain_path,
        pins_path,
...
def expected_origin_row(
    ...
    body = {
        ...
        "strict_nonpromotion": (
            "PRODUCER_ONLY__C30D_TERMINAL_CONSUMED__NO_C30E_PUBLICATION"
        ),
    }
...
def expected_result(
    ...
    body = {
        "schema": (
            "cm2.round306c30e.source-w-reduced-live-"
            "whole-origin-disposition.candidate.v2"
        ),
        "status": (
            "PASS_CANDIDATE_ROUND306C30E_V2_REDUCED_LIVE_DISPOSITION__"
            "C30D_TERMINAL_BOUND__ZERO_CREDIT__UNAUTHORIZED_PENDING_RELEASE"
        ),
        "input_pins": pins,
        ...
        "controlled_dual_hash_seed_contract": {
            ...
            "hash_seed_sentinel": "CM2_C30E_HASH_SEED_SENTINEL_v2",
            "hash_fingerprints": {
                "30630071": 6003874087328305091,
                "30630929": -1144366648129349240,
            },
            "required_flags": {
                "isolated": 0,
        ...
        "upstream_authority": {
            ...
            "C30d": {
                "legacy_sealed_alias_consumed": False,
                "terminal_graph_consumed": True,
                "dependency_kind": (
                    "SOLE_VERSIONED_TERMINAL_58_STATE_LEDGER_AUTHORITY"
                ),
                "adapter_sha256": c30d_authority["adapter_sha256"],
                "dynamic_pins_sha256": c30d_authority[
                    "dynamic_pins_sha256"
                ],
                "chain_dir_basename": c30d_authority[
                    "chain_dir_basename"
                ],
                "projection_sha256": c30d_authority[
                    "projection_sha256"
                ],
                "authority_pins_sha256": c30d_authority["pins_sha256"],
                "root_manifest_sha256": c30d_authority[
                    "root_manifest_sha256"
                ],
                "payload_manifest_sha256": c30d_authority[
                    "payload_manifest_sha256"
                ],
                "outer_verification_sha256": c30d_authority[
                    "outer_verification_sha256"
                ],
                "terminal_replay_sha256": c30d_authority[
                    "terminal_replay_sha256"
                ],
                "chain_status_sha256": c30d_authority[
                    "chain_status_sha256"
                ],
                "formal_before": c30d_authority["formal_before"],
                "formal_after": c30d_authority["formal_after"],
                "formal_credit": c30d_authority["formal_credit"],
            },
        },
        "proposed_source_W_transition_if_C30e_terminal_replay_passes": {
        ...
        "strict_nonpromotion": {
            ...
            "C30d_terminal_predecessor_consumed": True,
        ...
        "required_next": (
            "RUN_NO_IMPORT_VERIFIER_COHERENT_AND_RELEASE_ATTACKS_DUAL_TRUE_"
            "HASH_SEED_COLD_TOCTOU_REPLAY_EVIDENCE_PAYLOAD_ROOT_OUTER_"
            "TERMINAL_BYTE_REPLAY_BEFORE_ANY_LEDGER_CREDIT"
        ),
    }
...
def verify_candidate_dir(candidate: Path, reference: Reference) -> dict[str, Any]:
    ...
    require(
        result["proposed_source_W_transition_if_C30e_terminal_replay_passes"]
        ...
        "terminal C30d baseline, exact 58-to-56 proposal and zero formal credit",
    )
    forbidden = {
    ...
    return {
        ...
        "status": (
            "PASS_INDEPENDENT_C30E_V2_CANDIDATE__2_DYNAMIC_ORIGINS__"
        ...
        "whole_origin_disposition_census": {"RESOLVED_MIXED": 2},
        "ordered_predecessor": "VERSIONED_C30D_TERMINAL_GRAPH_58_STATE",
...
def main() -> int:
    ...
    except (Reject, base.Reject, KeyError, OSError, ValueError, TypeError,
