...
\large Selection-safe occurrence--carrier lifts, propagated
constant-polarity source kernels,
global incidence Cauchy,
two-cutoff face convergence, and a strongly conditional
pathwise CM2 criterion}
\author{Working note}
\date{13 July 2026 (version 43)}
...
dual-mass constant-polarity source-kernel, resolution-exhaustive main-clock
...
\begin{lemma}[Measurable constant-polarity decomposition
$\mathrm{MPD}$]
\label{lem:measurable-positive-decomposition}
Every signed source admitted to a regular source row is first represented on
...
construction is made chartwise for a regular flux density.  If only the
abstract Jordan decomposition is available and either positive part falls
outside the declared regular cone, that source belongs to a typed TV or
cemetery row; it is not promoted to a recoverable standard family.
\end{lemma}

\begin{proof}
Measurability is an explicit part of $\mathrm{MPD}$; countable component
extensions preserve standard Borelness and are exactly the extensions used
by the weighted-counting kernel below.  An essentially uncountable
decomposition is not admitted to a regular row without a separately defined
general kernel interface.  For the displayed construction,
$c+f\ge c/2$ and
\[
 |\log(c+f(x))-\log(c+f(y))|
...
claimed cone control without cutting at the zero set of $f$.
\end{proof}
...
general $K_i>1$.  By Lemma~\ref{lem:measurable-positive-decomposition}, the
...
Lemma~\ref{lem:measurable-positive-decomposition}; $s_{\rm pol}(a)$ is never a
function of the carrier coordinate.
The record
\begin{equation}
\label{eq:source-incidence-record}
 (o(a),j(a),\kappa(a),\pi(a),s_{\rm pol}(a),
   \varepsilon_{\rm or}(a),\operatorname{type}(a),
...
\end{equation}
The compatible tests generated by one physical observable form a closed
subspace of this product.  Each $U_{\lambda,a}^*$ acts on the coordinate
...
\label{eq:source-incidence-envelope}
 ...
 C_U(\lambda,a)
\end{equation}
for every active occurrence, where $C_U(\lambda,a)\ge1$ is the measurable
operator mark satisfying
$\|U_{\lambda,a}\|_{\mathfrak S_a\to\mathfrak T_a^*}
\le C_U(\lambda,a)$.  Thus operator growth, recovery, and the frozen
...
\begin{gathered}
 ...
 (\nu,s_{\rm pol},U,K,R,\mathscr A^{\rm opp},C_U)
   (\lambda,a_i(\lambda))
 = (\nu_i,s_i,U_i,K_i,R_i,\mathscr A_i^{\rm opp},C_{U,i})(\lambda).
\end{gathered}
\end{equation}
...
propagated envelope $q_\lambda$.  Tonelli then takes the supremum over the
test unit ball and bounds the result by $Q_{s,\mathfrak d}^{\rm prop}$.
...
themselves are not part of $\mathrm{TENERGY}_{W}$.
\end{definition}
...
\eqref{eq:phase-endpoint-ledger}.  A cemetery or extra registered type for
...
windows at depth $N$.  Writing a cell as a function only of $t=r$ or $N$
means its section at fixed suppressed $\mathfrak d$; the sets, source
kernels, slope maps, and all bounds are jointly measurable and uniform in
the suppressed mode, polynomial, time, and depth coordinates.
...
Let $K^{\rm prob}(d\eta)$ be the unselected normalized branch law to which
Theorem~\ref{thm:direct-slope-small-ball} applies and let
$\kappa_\eta$ be its collision--SRB carrier section.  Its unselected pair law
is
\begin{equation}
\label{eq:unselected-reference-law}
 \mathsf R_0(d\eta,dx,dx'):=K^{\rm prob}(d\eta)
 \kappa_\eta(dx)\kappa_\eta(dx').
\end{equation}
Choose, before any cell $A$, a probability proposal kernel
$\zeta_\eta(x,x';da)$ with
$\zeta_\eta(x,x';Y_{\rm src})=1$ pointwise, and put
\begin{equation}
\label{eq:proposal-lift}
 \overline{\mathsf R}(d\eta,da,dx,dx')
 :=\mathsf R_0(d\eta,dx,dx')\zeta_\eta(x,x';da).
\end{equation}
For the $a$-independent reference resonance event
$D_h^0=\{|S_\eta^0(x)-S_\eta^0(x')|\le2h\}$, normalization gives the exact
...
\end{align}
Thus $\Sigma_A(1)=\mathcal B_A$ and
$\Sigma_A(D_h^{\rm src})=\mathcal E_A(h)$.  The interface requires
\begin{equation}
...
energies.  Apply H\"older once to the unnormalized $\Sigma_A$ against
...
Assume
\[
 \mathrm{MAIN\_CLOCK\_COVERAGE},\qquad
...
\]
Assume in addition the explicit physical-to-source bridge
$\mathrm{PROP\_Q\_MATCH}$.  Thus there is a jointly Borel positive
occurrence kernel $\mathcal K_{s,\mathfrak d}^{\rm occ}$ from physical
labels to the source incidence graph, fixed before the final mode and test,
such that, uniformly in the master datum,
\begin{equation}
\label{eq:propagated-q-physical-match}
 \overline M_{s,\mathfrak d}^{q}(B)
 \le C\sum_\lambda M_\lambda\mathscr W_{\lambda,*}
       \mathcal K_{s,\mathfrak d}^{\rm occ}(\lambda;B)
 \quad\text{for every Borel }B\subset I,
 \qquad
 \mathcal K_{s,\mathfrak d}^{\rm occ}(\lambda;I)\le1.
\end{equation}
All multiplicity needed to make the last normalization true is already
included in $\mathscr W_{\lambda,*}$.  This is a hypothesis, not a
consequence of $w^q\ge w^mK\gamma^{-R}\mathscr A^{\rm opp}C_U$.
...
when scale-bad labels are removed.  By $\mathrm{PROP\_Q\_MATCH}$, the
occurrence-kernel pushforward of this positive physical ledger dominates the
...
is the one used by the FACE, shallow, and deep clauses above.
...
flux sum.
The physical-to-source domination used here is precisely the assumed
\eqref{eq:propagated-q-physical-match}; none of the moment arguments in this
proof is claimed to derive it.
\end{proof}
...
\item $\mathrm{NST}_{\rm phys}$, $\mathrm{SS}(d_Z)$, and
      ...
      \eqref{eq:nonmain-clock-coverage}--
      \eqref{eq:main-clock-coverage}, the physical/source bridge
      \eqref{eq:propagated-q-physical-match}, and the main-source
      likelihood/base
...
\label{eq:final-CM2-exponent}
 ...
 \eta_{\rm hi}\rho_\infty-a_{\rm ultra},c_{\rm scl},\\
 c_{\rm time,J}^{\rm val},c_{\rm time,TV}^{\rm src},
 \eta_{\rm lo}R,c_{\rm tail}-A_{\rm loss},c_Z
 \end{gathered}
 \right\}
\end{equation}
...
$\mathrm{QT}_{\rm path}$, uniformly on the product-a.e. bases and directions
...
then the same conclusion holds.  No automatic circular transversality is
asserted.
\end{corollary}
...
\begin{array}{ccl}
 ...
 \textup{(H5)}&\Longrightarrow&
   \text{typed DQ limits plus the truncated two-cutoff face limit}.
\end{array}
\]
\endgroup
...
There are twenty-seven falsifiable primitive interfaces.  $\mathrm{MT}_{\rm DQ}$ must
...
source restriction with a uniformly dominated occurrence-kernel pushforward
of the physical ledger; finiteness of the latter alone does not bound an
independently enlarged $q$.  $\mathrm{MAIN\_CLOCK\_COVERAGE}$ supplies the
...
\begin{proposition}[Fifty-eight obstruction tests]
\label{prop:v43-obstruction-tests}
...
\item Positive decompositions may exist separately on every fibre while the
      ...
      extension in Lemma~\ref{lem:measurable-positive-decomposition} rejects
...
\begin{proof}
Items 1--3, 8--11, and 13--58 are direct finite-measure, frequency, or norm
