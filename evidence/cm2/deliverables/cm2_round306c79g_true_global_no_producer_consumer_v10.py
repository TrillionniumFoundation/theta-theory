#!/usr/bin/env python3
"""C79g v10 append-only zero-credit candidate builder.

This append-only v10 binds the independently verified C78s 13-member final
...
is permanently non-authoritative.  The separately committed v10 seal plus the
...
COLD_COORDINATION_PARENT_FD_ENV = "CM2_C79G_V10_COORDINATION_PARENT_FD"
...
SELF = OUT / "cm2_round306c79g_true_global_no_producer_consumer_v10.py"
...
STATIC_AUDIT_V10 = OUT / "cm2_round306c79g_true_global_no_producer_consumer_static_audit_v10.json"
COLD_LAUNCHER = OUT / "cm2_round306c79g_true_global_no_producer_consumer_cold_launch_v10.py"
COLD_MANIFEST = OUT / "cm2_round306c79g_true_global_no_producer_consumer_cold_launch_manifest_v10.sha256"
COLD_OUTER = OUT / "cm2_round306c79g_true_global_no_producer_consumer_cold_launch_outer_receipt_v10.json"
# Final current-core pins.  The historical draft sentinels remain explicit
# below so the absolute first runtime gate can prove none survived injection.
FINAL_V10_CORE_PINS_INSTALLED = True
...
COLD_EXACT8 = (
    ...
    CONTRACT,
    SELF,
    OUT / "cm2_round306c79g_true_global_no_producer_consumer_independent_verifier_assembler_authority_consumer_v10.py",
    V9_TO_V10_TRANSITION,
    STATIC_AUDIT_V10,
    COLD_LAUNCHER,
)
...
def _linux_libc() -> ctypes.CDLL:
    need(sys.platform.startswith("linux"), "Linux-only v10 publication protocol")
...
class HeldSelf:
    ...
    def __init__(self, exec_inherited: int, source_inherited: int,
        ...
        need(SELF == OUT / "cm2_round306c79g_true_global_no_producer_consumer_v10.py",
...
def new_candidate_stage(outdir: Path, parent: HeldParent) -> tuple[Path, HeldMutableStage]:
    ...
    name = ".c79g-v10-candidate-stage-" + orientation + "-" + CHECKPOINT_OBJECT_PIN
...
def strict_bool_predecessor_and_successor_census(
        ...
        v10_sources: Mapping[str, bytes]) -> dict[str, Any]:
    """Prove frozen v5 defects and zero-unsafe complete v6/v7/v10 boxes."""
    ...
    v10 = {role: _strict_bool_need_census(v10_sources[role], role)
          for role in v10_roles}
    ...
    v10_unproved = sum(
        v10[role]["recursive_exact_bool_unproved_count"] for role in v10_roles)
    v10_arity = sum(
        v10[role]["arity_star_or_keyword_failure_count"] for role in v10_roles)
    v10_returns = sum(
        v10[role]["annotated_bool_return_failure_count"] for role in v10_roles)
    need(v6_unproved == 0 and v6_arity == 0 and v6_returns == 0 and
         v7_unproved == 0 and v7_arity == 0 and v7_returns == 0 and
         v10_unproved == 0 and v10_arity == 0 and v10_returns == 0,
         "v6/v7/v10 recursive exact-bool full boxes have zero unsafe or unproved sites")
    return {
        ...
        "same_seven_absent_from_v10": True,
        ...
        "v10_recursive_exact_bool_unproved_count": v10_unproved,
        ...
        "v10_direct_need_call_count": sum(
            v10[role]["direct_need_call_count"] for role in v10_roles),
...
def v6_held_self_identity_defect_regression(raw: bytes) -> dict[str, Any]:
    ...
    V10 first proves the complete three-site census, then independently locates
...
def v9_prechild_shape_drift_regression(
    ...
    need(len(expanded_dicts) == 1 and len(expanded_dicts[0].keys) == 16 and
         len(validator_calls) == 1 and equality_gates and
         V9_V6_DEFECT_SHAPE_DRIFT_INCIDENT["failure_label"] in constants and
...
def _validate_v7_regression_section(
    ...
    need(strict_bool_census["v7_recursive_exact_bool_unproved_count"] == 0 and
         strict_bool_census["v10_recursive_exact_bool_unproved_count"] == 0 and
         strict_bool_census["all_twelve_sources_AST_parsed_and_compiled_in_memory"]
             is True and
         strict_bool_census["compiled_code_executed"] is False,
         label + ":v7/v10 full-callsite strict-bool closure")
    need(section.get("all_ten_file_pins_match") is True and
         section.get("all_declared_object_pins_match") is True and
         section.get("all_ten_regular_0444_nlink1") is True and
         section.get("exact8_manifest_reconstructs_first_eight_in_order") is True and
...
def _validate_final_static_audit(
    ...
    need(isinstance(attacks, dict) and set(attacks) == {
         ...
         "final v10 coherent attack exact7, 121/121, routes, and deferred execution")
    ...
    need(isinstance(closure, dict) and set(closure) == {
         ...
         }, "final v10 schema/constructor exact21 closure")
    need(type(closure.get("schema_definition_count")) is int and
         closure.get("schema_definition_count") == 42 and
         type(closure.get("schema_ref_count")) is int and
         closure.get("schema_ref_count") == 228 and
         ...
         closure.get("closed_object_count") == 48 and
         type(closure.get(
         ...
         "final v10 schema 42 defs/228 refs/0 unresolved/48 closed/0 mismatch")
    need(closure.get("strict_JSON_duplicate_keys_rejected") is True and
         ...
         "final v10 schema and live-protocol type-exact bool closure")
    actual_keywords = [
    ...
    need(closure.get("actual_schema_keyword_universe") == actual_keywords and
         ...
         "final v10 exact canonical schema keyword universes and digests")
    shapes = closure.get("output_shape_key_counts", {})
    expected_shapes = {
        "selfIdentity": 33,
        "independentConsumerProof": 55,
        "staticFreezeProof": 71,
        "coldLaunchProof": 101,
        "laterRejection": 50,
        "producerSourceRegistry": 55,
        "liveRequest": 15,
        "liveACK": 32,
        "liveACKCensus": 36,
    }
    need(isinstance(shapes, dict) and set(shapes) == set(expected_shapes) and
         ...
         "final v10 exact nine output-shape key counts")
...
def hold_static_freeze_trust(self_guard: HeldSelf) -> dict[str, Any]:
    """Hold current v10 exact10 and every distinct append-only predecessor.

    Current exact8 begins with the official v9 rejection itself.  The v9, v8,
    v7, v6, and v5 published exact10 surfaces remain distinct; v3 exact10,
    the separate v3 rejection, and the seven other v4 draft files are held.
    The exact distinct-file identity census is 10+10+10+10+10+10+10+7+1 = 78.
    """
    manifest_guard = HeldPinnedInput(
        COLD_MANIFEST, "C79g v10 cold-launch manifest", 0o444)
    outer_guard = HeldPinnedInput(
        COLD_OUTER, "C79g v10 cold-launch outer-last receipt", 0o444)
    manifest_entries = parse_manifest_ordered(
        manifest_guard.raw, "C79g v10 cold-launch exact8 manifest")
    ...
    need(len(manifest_by_path) == 8 and
         ...
         manifest_by_path[V9_OFFICIAL_REJECTION] ==
             V9_OFFICIAL_REJECTION_FILE_PIN,
         "cold-launch manifest current SELF, v9 rejection, and policy pins")

    by_path: dict[Path, HeldPinnedInput] = {}
    for path in COLD_EXACT8:
        if path != SELF:
            by_path[path] = HeldPinnedInput(
                path, "C79g v10 cold exact8:" + path.name, 0o444,
                manifest_by_path[path])
    v9_rejection_guard = by_path[V9_OFFICIAL_REJECTION]
    ...
    need(len(current_cold_ten_guards) == 10 and
         ...
         "current v10 cold exact10 unique held identities on one statx mount")
    exact8_stats = [
    ...
    need(all(chronology.values()),
         "physical v10 exact8/manifest/outer freeze chronology")
    ...
    all_file_guards = [
        *current_cold_ten_guards, *v9_guards, *v8_guards, *v7_guards, *v6_guards,
        *v5_guards, *v3_guards, *v4_guards, v3_rejection_guard,
    ]
    need(len(all_file_guards) == 78 and
         len({guard.identity for guard in all_file_guards}) == 78 and
         len({guard.mount_id for guard in all_file_guards}) == 1 and
         v9_namespace_guard.mount_id == v8_namespace_guard.mount_id ==
             v7_namespace_guard.mount_id ==
             v6_namespace_guard.mount_id == v5_namespace_guard.mount_id ==
             all_file_guards[0].mount_id,
         "append-only v10/v9/v8/v7/v6/v5/v3/v4/rejection history is 78 "
         "distinct files on one mount")
    require_v5_positive_and_stage_surfaces_absent()

    v10_consumer_path = COLD_EXACT8[4]
    strict_bool_census = strict_bool_predecessor_and_successor_census(
        ...
        {
            "producer_v10": self_guard.raw,
            "consumer_v10": by_path[v10_consumer_path].raw,
            "launcher_v10": by_path[COLD_LAUNCHER].raw,
        })

    transition_guard = by_path[V9_TO_V10_TRANSITION]
    audit_guard = by_path[STATIC_AUDIT_V10]
    transition = strict_json(transition_guard.raw, "C79g v10 transition trust")
    audit = strict_json(audit_guard.raw, "C79g v10 static audit trust")
    outer = strict_json(outer_guard.raw, "C79g v10 cold-launch outer trust")
    ...
    verify_object(transition, "C79g v10 transition trust")
    verify_object(audit, "C79g v10 static audit trust")
    verify_object(outer, "C79g v10 cold-launch outer trust")
    ...
    transition_pin = audited.get("v9_to_v10_transition_receipt", {})
    ...
    _validate_v7_regression_section(
        published_v7, strict_bool_census, "v10 transition v7 predecessor")
    _validate_v7_regression_section(
        audit_v7, strict_bool_census, "v10 audit v7 predecessor")
    _validate_v8_regression_section(
        published_v8, v8_rollout_control_flow_incident,
        "v10 transition v8 predecessor")
    _validate_v8_regression_section(
        audit_v8, v8_rollout_control_flow_incident,
        "v10 audit v8 predecessor")
    _validate_v9_regression_section(
        published_v9, "v10 transition v9 predecessor")
    _validate_v9_regression_section(
        audit_v9, "v10 audit v9 predecessor")
    ...
    need(transition.get("status") ==
         ...
         audit_v4.get(
             "v10_closed_object_required_property_mismatch_count") == 0 and
         audit_v4.get("same_defects_absent_from_v10") is True and
         ...
         "transition/audit pin current v10 and preserve exact v3/v4/v7 history")
    ...
    need(set(outer) == {
         ...
         "cold-launch v10 outer-last exact8 and zero-credit closure")
    ...
    return {
        "guards": [
            *by_path.values(), *v9_guards, *v8_guards, *v7_guards, *v6_guards,
            *v5_guards, *v3_guards,
            *v4_guards, v3_rejection_guard,
            v9_namespace_guard, v8_namespace_guard, v7_namespace_guard,
            v6_namespace_guard,
            v5_namespace_guard,
...
def ensure_launch_configuration() -> HeldSelf:
    ...
    need(FINAL_V10_CORE_PINS_INSTALLED is True,
         "v10 final core pins are not installed; runtime permanently disabled")
    need(all(value not in V10_DRAFT_CORE_PIN_SENTINELS for value in (
             CONTRACT_FILE_PIN, CONTRACT_OBJECT_PIN, CLOSED_SCHEMA_FILE_PIN)),
         "v10 draft contract/schema sentinels must all be replaced before runtime")
    ...
    need(all(value and len(value) <= 10 and
         ...
         SELF == ROOT / "deliverables/cm2_round306c79g_true_global_no_producer_consumer_v10.py",
    ...
    need(all(isinstance(value, str) and len(value) == 64 and
         ...
         "C79g v10 exact non-placeholder contract/schema/rejection pins")
    need(set(C78S_FINAL["pins"]) == set(C78S_NAMES) and
         ...
         "C79g v10 exact filled launch configuration")
...
def validate_static_authorities(static_trust: dict[str, Any]) -> dict[str, Any]:
    ...
    rejection_raw = held[V7_OFFICIAL_REJECTION].raw
    ...
    need(contract.get("effective_checkpoint_object_sha256") == CHECKPOINT_OBJECT_PIN and
         ...
         contract.get("v10_bundle", {}).get(
             ...
             "independent_verifier_assembler_authority_consumer_v10.py" and
         contract.get("v10_bundle", {}).get("closed_schema_validator_policy") == {
         ...
         "v10 frozen contract checkpoint/composite-only boundary")
    ...
    need(closed.get("$ref") == "#/$defs/coldLaunchedCommittedAuthority" and
         ...
         "append-only v10 cold root, zero-credit inner, and persisted zero boundary")
    ...
    return close_object({
        ...
        **self_guard.execution_proof(),
        ...
        "post_source_static_freeze_trust": {
            "v9_to_v10_transition_receipt": {
                "path": str(V9_TO_V10_TRANSITION.relative_to(ROOT)),
                "file_sha256": static_trust["transition_file_sha256"],
                "object_sha256": static_trust["transition_object_sha256"],
            },
            "static_audit_v10": {
                "path": str(STATIC_AUDIT_V10.relative_to(ROOT)),
...
def build(outdir: Path, self_guard: HeldSelf,
    ...
    lock = (
        "C79g v10 isolated candidate only; standalone non-authoritative. Formal-global credit "
