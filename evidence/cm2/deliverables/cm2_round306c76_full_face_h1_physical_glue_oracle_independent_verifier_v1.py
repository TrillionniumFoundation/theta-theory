#!/usr/bin/env python3
"""Independent no-producer verifier for the C76 full-face H1 sidecar."""
...
C75_LEDGER = ROOT / ".cm2-runtime/c75-build-a2.TG7bbZ/cm2_round306c75_first_tangency_exact_strata_oracle_v1.jsonl.gz"
C75_SHA = "e1e30a36f75c2ec71b0c4ec51da33e3c09fb5570426aa356f0c815406e251229"
FROZEN_OWNER = "W[1,0]"
PRECISION = 384
CORE_INDEX = {"W:E": 14, "W:N": 17, "W:S": 20, "W:W": 23}
PRODUCER_DECLARED_PIN = "3284b6ef6599dd4b7aab21ab9172647c0bf1dfbdf6779205bb37c827b534b709"
...
def worker() -> int:
    ...
    for atlas, candidate in payload["pairs"]:
        need(candidate["schema"] == SCHEMA + ".decision-row" and
             ...
             candidate["exact_reflected_box"] == atlas["exact_reflected_box"] and
             candidate["parent_volume_fraction"] ==
                atlas["parent_volume_fraction"] and
             candidate["selector"] ==
                "child_route_witness == REGULAR_FULL_FACE_GRAPH" and
        ...
        need(candidate["collision3_handoff"] is None and
             candidate["sealed_collision3_handoff_count"] == 0 and
             candidate["explicit_residual_strata"] == 0,
             "no hidden handoff or residual")
        ...
        W_exits = [row for row in candidate["stratum_exits"]
                   if row.get("representative_chart") == "W"]
        if W_exits:
            ...
            if clipped:
                ...
                need(len(graph_exit) == 1 and
                     graph_exit[0]["physical_chart_glue"] ==
                        expected_graph_glue(
                            ...
                            route, authority) and
                     graph_exit[0]["graph_is_not_a_terminal"] is True and
                     graph_exit[0]["representative_chart"] == "W" and
                     graph_exit[0]["reflected_chart"] == "E" and
                     graph_exit[0]["representative_reflected_owner_glue"] == {
                         "representative_half_open_physical_owner": "W",
                         "reflected_half_open_physical_owner": "E",
                         "representative_shadow_chart":
                             "N" if ny_sign > 0 else "S",
                         "reflected_shadow_chart":
                             "S" if ny_sign > 0 else "N",
                ...
                need(candidate["row_outcome"] ==
                     "STRICT_EXCLUSION_WHOLE_FULL_FACE_CHILD__PHYSICAL_GRAPH_GLUE__" +
...
def attacks(sample: dict[str, Any],
    ...
    result_mutations = [
        ("result_whole_10297", ["strict_whole_child_exclusion_count"], 10297),
        ("result_endpoint_20595", ["graph_endpoint_occurrence_count"], 20595),
        ("result_unique_endpoint_20595",
         ["graph_endpoint_occurrence_unique_count"], 20595),
...
def verify(stage_a: Path, stage_b: Path, c72: Path,
    ...
    authority = chart_authority()
    names = (ROWS, INCIDENCE, RESULT, REPORT, LOCK, MANIFEST, OUTER)
    identity = {}
    for name in names:
    ...
    need(result_a == result_b and
         ...
         result_a["scope"]["selector"] ==
            "child_route_witness == REGULAR_FULL_FACE_GRAPH" and
         result_a["scope"]["target_child_count"] == EXPECTED_SCOPE and
         result_a["scope"]["selected_C72_row_sequence_sha256"] ==
            EXPECTED_SEQUENCE and
         result_a["H1_GEOMETRY_CLOSED"] == EXPECTED_SCOPE and
         result_a["WHOLE_CHILD_STRICT_EXCLUSION_CLOSED"] == EXPECTED_SCOPE and
         result_a["ALLOWED_EXIT_CLOSED"] == EXPECTED_SCOPE and
         result_a["strict_whole_child_exclusion_count"] == EXPECTED_SCOPE and
         result_a["implicit_graph_strict_exclusion_count"] == EXPECTED_SCOPE and
         result_a["implicit_graph_terminal_count"] == 0 and
         result_a["sealed_collision3_handoff_count"] == 0 and
         result_a["collision3_handoff_ledger"] is None and
    ...
    origins = {}
    target_pairs = set(EXPECTED_PAIR_CENSUS)
    ...
    atlas_iter = (
        ...
        if row["child_route_witness"] ==
            "REGULAR_FULL_FACE_GRAPH")
    candidate_iter = iter_jsonl(
        stage_a / ROWS,
        result_a["ledgers"]["decision_rows"]["sha256"], EXPECTED_SCOPE)
    ...
    need(total == EXPECTED_SCOPE and sample is not None and
         len(target_hashes) == EXPECTED_SCOPE and
         dict(census) == result_a["H1_geometry_census"] and
         dict(census) == {
             "EXACT_CLIPPED_MONOTONE_GRAPH_AND_TWO_OFFGRAPH_REGIONS":
                 EXPECTED_SCOPE} and
         dict(routes) == EXPECTED_ROUTE_CENSUS and
         dict(routes) == result_a["W_route_census_on_positive_side_and_graph"] and
         dict(graph_routes) ==
            result_a["implicit_graph_physical_continuation_census"] and
         dict(graph_routes) == EXPECTED_ROUTE_CENSUS and
    ...
    tangency_hashes = {row["C72_obligation_row_sha256"] for row in iter_jsonl(
        C75_LEDGER, C75_SHA, 5884)}
    need(target_hashes.isdisjoint(outgoing_hashes) and
         target_hashes.isdisjoint(broad_hashes) and
         target_hashes.isdisjoint(tangency_hashes) and
         outgoing_hashes.isdisjoint(broad_hashes),
         "independent C76/C72o/C73/C75 exact dedup")
    need(len(candidate_occurrence_ids) == 2 * EXPECTED_SCOPE and
         independent_occurrence_ids == candidate_occurrence_ids and
         sum(len(rows) for rows in occurrences.values()) == 2 * EXPECTED_SCOPE and
         set(occurrences) == set(root_specs),
         "independent 2x10298 endpoint occurrence conservation")
    ...
    need(set(degree_census).issubset({1, 2}) and
         sum(degree * count for degree, count in degree_census.items()) ==
            2 * EXPECTED_SCOPE and
         sum(degree_census.values()) == len(root_specs) and
         result_a["graph_endpoint_occurrence_count"] == 2 * EXPECTED_SCOPE and
         result_a["graph_endpoint_occurrence_unique_count"] == 2 * EXPECTED_SCOPE and
    ...
    need(observed_bodies == expected and
         sum(row["incidence_count"] for row in expected) == 2 * EXPECTED_SCOPE and
    ...
    verification = {
        ...
        "status":
            "PASS_NO_C76_PRODUCER_READ_IMPORT_EXECUTION_OR_DECODE__"
            "DUAL_BYTE_IDENTITY__10298_FULL_FACE_PHYSICAL_GRAPH_EXCLUSIONS__"
            "20596_ENDPOINTS_CONSERVED__48_ATTACKS__ZERO_CREDIT",
        "verifier_file_sha256": file_sha(SELF),
        "producer_file_sha256_declared": PRODUCER_DECLARED_PIN,
        "candidate_result_object_sha256": result_a["object_sha256"],
        "no_producer_open_read_parse_import_exec_decode": True,
        ...
        "coverage": {
            ...
            "clipped_physical_graph_glue_count": EXPECTED_SCOPE,
            "implicit_graph_strict_exclusion_count": EXPECTED_SCOPE,
            "graph_terminal_count": 0,
            "sealed_collision3_handoff_count": 0,
            "graph_endpoint_occurrence_count": 2 * EXPECTED_SCOPE,
