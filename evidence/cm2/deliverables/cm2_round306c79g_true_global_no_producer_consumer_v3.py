#!/usr/bin/env python3
"""C79g v3 append-only zero-credit candidate builder.

This append-only v3 binds the independently verified C78s 13-member final
...
is permanently non-authoritative.  The separately committed v3 seal plus the
downstream consumer's secure replay may derive only a zero-credit inner
composite; only the externally SHA-256-pinned cold launcher may wrap that inner
value into the one virtual positive-credit root.
"""
...
COLD_COORDINATION_PARENT_FD_ENV = "CM2_C79G_V3_COORDINATION_PARENT_FD"
...
SELF = OUT / "cm2_round306c79g_true_global_no_producer_consumer_v3.py"
...
V2_TO_V3_TRANSITION = OUT / "cm2_round306c79g_true_global_no_producer_consumer_v2_to_v3_static_launch_transition_receipt_v1.json"
STATIC_AUDIT_V3 = OUT / "cm2_round306c79g_true_global_no_producer_consumer_static_audit_v3.json"
COLD_LAUNCHER = OUT / "cm2_round306c79g_true_global_no_producer_consumer_cold_launch_v3.py"
COLD_MANIFEST = OUT / "cm2_round306c79g_true_global_no_producer_consumer_cold_launch_manifest_v3.sha256"
COLD_OUTER = OUT / "cm2_round306c79g_true_global_no_producer_consumer_cold_launch_outer_receipt_v3.json"
...
def _linux_libc() -> ctypes.CDLL:
    need(sys.platform.startswith("linux"), "Linux-only v3 publication protocol")
...
class HeldSelf:
    ...
    def __init__(self) -> None:
        need(SELF == OUT / "cm2_round306c79g_true_global_no_producer_consumer_v3.py",
             "SELF exact frozen producer path")
        before_path = SELF.lstat()
        need(stat.S_ISREG(before_path.st_mode) and not SELF.is_symlink() and
             stat.S_IMODE(before_path.st_mode) == 0o444,
             "SELF regular immutable-mode path")
        inherited_text = os.environ.get(COLD_SOURCE_FD_ENV, "")
        need(inherited_text.isdigit(), "producer requires cold-launch inherited source fd")
        inherited = int(inherited_text)
        need(inherited >= 3 and EXECUTED_SOURCE == Path("/proc/self/fd") / inherited_text,
             "producer executes exact cold-launch held source fd")
        try:
            self.fd = os.dup(inherited)
        except OSError as exc:
            raise Reject("cold-launch inherited producer fd unavailable") from exc
        ...
        need(file_fingerprint(after_fd) == file_fingerprint(self.before) ==
             ...
             "SELF initial same-fd read stable before/after")
...
class HeldPinnedInput:
    """Hold one immutable pinned input fd until the producer terminal replay."""
    ...
    def __init__(self, path: Path, label: str, expected_sha256: str | None = None) -> None:
        ...
        need(stat.S_ISREG(before_path.st_mode) and not path.is_symlink() and
             ...
             label + ":initial regular 0444 single-link path")
    ...
    def terminal_replay(self) -> None:
        ...
        need(replay == self.raw and
             ...
             stat.S_IMODE(after_fd.st_mode) == 0o444 and
             statx_mount_id(self.fd) == self.mount_id,
...
class HeldDirectory:
    """Hold a directory identity, mount, final mode, and exact member universe."""
...
def new_candidate_stage(outdir: Path, parent: HeldParent) -> tuple[Path, HeldMutableStage]:
    ...
    name = ".c79g-v3-candidate-stage-" + orientation + "-" + CHECKPOINT_OBJECT_PIN
...
def hold_static_freeze_trust(self_guard: HeldSelf) -> dict[str, Any]:
    """Hold the outer-last cold-launch closure and all exact-eight members.

    The source contains only fixed paths for the post-source manifest/outer, so
    their later hashes do not form a cycle.  The sole externally pinned
    launcher reconstructs and holds the manifest and outer-last surface, which
    in turn pin the actual current SELF bytes.
    """
    manifest_guard = HeldPinnedInput(COLD_MANIFEST, "C79g v3 cold-launch manifest")
    outer_guard = HeldPinnedInput(COLD_OUTER, "C79g v3 cold-launch outer-last receipt")
    manifest_entries = parse_manifest_ordered(
        manifest_guard.raw, "C79g v3 cold-launch exact8 manifest")
    ...
    need(len(manifest_by_path) == 8 and
         ...
         manifest_by_path[V2_REJECTION] == V2_REJECTION_FILE_PIN,
         "cold-launch manifest current SELF and policy pins")
    by_path: dict[Path, HeldPinnedInput] = {}
    for path in COLD_EXACT8:
        if path != SELF:
            by_path[path] = HeldPinnedInput(
                path, "C79g v3 cold exact8:" + path.name, manifest_by_path[path])
    cold_ten_guards = [self_guard, *by_path.values(), manifest_guard, outer_guard]
    need(len(cold_ten_guards) == 10 and
         len({(guard.before.st_dev, guard.before.st_ino)
              for guard in cold_ten_guards}) == 10 and
         len({guard.mount_id for guard in cold_ten_guards}) == 1,
         "cold exact10 held identities globally unique on one statx mount")
    exact8_stats = [
    ...
    need(all(chronology.values()),
         "physical cold-launch exact8/manifest/outer freeze chronology")
    transition_guard = by_path[V2_TO_V3_TRANSITION]
    audit_guard = by_path[STATIC_AUDIT_V3]
    transition = strict_json(transition_guard.raw, "C79g v3 transition trust")
    audit = strict_json(audit_guard.raw, "C79g v3 static audit trust")
    outer = strict_json(outer_guard.raw, "C79g v3 cold-launch outer trust")
    need(isinstance(transition, dict) and isinstance(audit, dict) and
         isinstance(outer, dict), "post-source trust receipts are JSON objects")
    verify_object(transition, "C79g v3 transition trust")
    verify_object(audit, "C79g v3 static audit trust")
    verify_object(outer, "C79g v3 cold-launch outer trust")
    successor = transition.get("successor_v3_static_bundle", {})
    audited = audit.get("audited_v3_bundle", {})
    transition_pin = audited.get("v2_to_v3_transition_receipt", {})
    ...
    need(transition.get("status") ==
             "FROZEN_STATIC_NO_RUN_APPEND_ONLY_V2_TO_V3__ALL_PINS_CLOSED__RUNTIME_DEFERRED" and
         audit.get("status") ==
             "PASS_STATIC_NO_RUN_AUDIT_V3__ALL_BYTES_PINS_MODES_AND_COMPOSITE_BOUNDARIES_CLOSED" and
         ...
         audited.get("closed_schema", {}).get("file_sha256") == CLOSED_SCHEMA_FILE_PIN and
         audit.get("v2_rejection_regression", {}).get("file_sha256") ==
             V2_REJECTION_FILE_PIN and
         audit.get("v2_rejection_regression", {}).get("object_sha256") ==
             V2_REJECTION_OBJECT_PIN and
         ...
         "post-source transition/audit mechanically pin current producer and policy bundle")
    need(set(outer) == {
         ...
         outer_launcher.get("file_sha256") == manifest_by_path[COLD_LAUNCHER] and
         ...
         outer.get("sole_external_trust_anchor_is_launcher_sha256") is True and
         ...
         "cold-launch outer-last exact8 and zero-credit closure")
    need(os.environ.get(COLD_LAUNCHER_SHA_ENV) == manifest_by_path[COLD_LAUNCHER],
         "sole inherited external launcher anchor matches held closure")
    return {
        "guards": [*by_path.values(), manifest_guard, outer_guard],
        "by_path": by_path,
...
def ensure_launch_configuration() -> None:
    ...
    need(source_fd_text.isdigit() and int(source_fd_text) >= 3 and
         coordination_fd_text.isdigit() and int(coordination_fd_text) >= 3 and
         coordination_fd_text != source_fd_text and
         EXECUTED_SOURCE == Path("/proc/self/fd") / source_fd_text and
         _cold_root_text == str(ROOT) and ROOT.is_absolute() and
         SELF == ROOT / "deliverables/cm2_round306c79g_true_global_no_producer_consumer_v3.py",
         "producer accepts only cold-launch held-fd execution context")
    ...
    static_pins = (
        ...
        V2_REJECTION_FILE_PIN, V2_REJECTION_OBJECT_PIN,
    )
    need(all(isinstance(value, str) and len(value) == 64 and
         ...
         "C79g v3 exact non-placeholder contract/schema/rejection pins")
    need(set(C78S_FINAL["pins"]) == set(C78S_NAMES) and
         ...
         "C79g v3 exact filled launch configuration")
...
def validate_c42_c53_kraft(cell_rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    c42_candidate_guard = HeldDirectory(
        C42_CANDIDATE_DIR, c42_candidate_universe, 0o555,
        "C42 candidate exact9 held directory")
    before_directory = c42_candidate_guard.before
    c42_candidate_directory_state = (
        before_directory.st_dev, before_directory.st_ino, before_directory.st_mode,
        before_directory.st_mtime_ns, before_directory.st_ctime_ns,
        before_directory.st_nlink)
    ...
    for key in sorted(C42_C53_PATHS):
        ...
        held_by_path[path] = HeldPinnedInput(
            path, "C42/C53 full10:" + key, C42_C53_PINS[key])
    for name in sorted(c42_candidate_universe):
        path = C42_CANDIDATE_DIR / name
        if path not in held_by_path:
            held_by_path[path] = HeldPinnedInput(path, "C42 candidate exact9:" + name)
    ...
    c42_candidate_guard.terminal_replay()
    return {
        ...
        "C42_candidate_directory_state": c42_candidate_directory_state,
...
def validate_static_authorities(static_trust: dict[str, Any]) -> dict[str, Any]:
    ...
    rejection_raw = held[V2_REJECTION].raw
    ...
    need(contract.get("effective_checkpoint_object_sha256") == CHECKPOINT_OBJECT_PIN and
         contract.get("status") ==
             "FROZEN_APPEND_ONLY_V3__COLD_LAUNCHED_WRAPPER_ONLY__STATIC_NO_RUN_FREEZE" and
         contract.get("v3_bundle", {}).get(
             ...
             "independent_verifier_assembler_authority_consumer_v3.py" and
         ...
         "v3 frozen contract checkpoint/composite-only boundary")
    ...
    need(closed.get("$ref") == "#/$defs/coldLaunchedCommittedAuthority" and
         ...
         "append-only v3 cold root, zero-credit inner, and persisted zero boundary")
    rejection = strict_json(rejection_raw, "C79g v2 rejection supersession")
    verify_object(rejection, "C79g v2 rejection supersession", V2_REJECTION_OBJECT_PIN)
    rejected = rejection.get("rejection", {})
    frozen_v2 = rejection.get("frozen_v2_bundle", {})
    need(rejection.get("status") ==
             "FROZEN_APPEND_ONLY_V2_REJECTION__CREDIT_OUTER_PRECOMMIT__V3_COMPOSITE_AUTHORITY_SUCCESSOR_ONLY" and
         rejection.get("effective_checkpoint_object_sha256") == CHECKPOINT_OBJECT_PIN and
         rejected.get("v2_final_outer_exact_rejection_reason") ==
             "CREDIT_OUTER_PRECOMMIT" and
         rejected.get("v2_final_outer_formal_global_closure_credit") == 0 and
         rejected.get("v2_final_outer_D02_unlock") is False and
         rejected.get("v2_execution_allowed_after_frozen_publication") is False and
         frozen_v2.get("all_members_required_mode") == "0444" and
         frozen_v2.get("all_members_required_nlink") == 1 and
         frozen_v2.get("members_modified_by_this_receipt") is False and
         rejection.get("supersession", {}).get(
             "only_externally_pinned_cold_launcher_wrapper_may_be_authoritative") is True and
         rejection.get("supersession", {}).get(
             "direct_independent_source_inner_composite_credit") == 0,
         "exact frozen v2 CREDIT_OUTER_PRECOMMIT rejection and v3 supersession")
...
def validate_c78l() -> dict[str, Any]:
    ...
    directory_a_guard = HeldDirectory(C78L_A, base_names, 0o755, "C78l base A")
    directory_b_guard = HeldDirectory(C78L_B, base_names, 0o755, "C78l base B")
    ...
    completion_directory_guard = HeldDirectory(
        C78L_COMPLETION, completion_names, 0o755, "C78l completion")
    ...
    def hold(path: Path, label: str, expected_sha256: str) -> HeldPinnedInput:
        ...
        guard = HeldPinnedInput(path, label, expected_sha256)
...
def validate_c78s() -> dict[str, Any]:
    ...
    directory_a_guard = HeldDirectory(a, expected_stage_names, 0o555, "C78s stage A")
    directory_b_guard = HeldDirectory(b, expected_stage_names, 0o555, "C78s stage B")
    ...
    return {
        ...
        "C42_C53_Kraft_chain": {
            ...
            if key not in {
                ...
                "C42_candidate_directory_state", "C42_candidate_universe",
                "C42_candidate_guard",
            }
        },
        "direct_input_terminal_guard": {
            "held_inputs": kraft_chain["held_direct_inputs"],
            "C42_candidate_directory_state": kraft_chain["C42_candidate_directory_state"],
            "C42_candidate_universe": kraft_chain["C42_candidate_universe"],
            "C42_candidate_guard": kraft_chain["C42_candidate_guard"],
        },
        "C78s_terminal_guard": {
...
def input_registry(state: dict[str, Any], self_file_sha256: str,
    ...
    return close_object({
        ...
        "v2_rejection_supersession_receipt": {
            ...
            "reason": "CREDIT_OUTER_PRECOMMIT",
        },
        "post_source_static_freeze_trust": {
            "v2_to_v3_transition_receipt": {
                "path": str(V2_TO_V3_TRANSITION.relative_to(ROOT)),
                "file_sha256": static_trust["transition_file_sha256"],
                "object_sha256": static_trust["transition_object_sha256"],
            },
            "static_audit_v3": {
                "path": str(STATIC_AUDIT_V3.relative_to(ROOT)),
...
def build(outdir: Path, self_guard: HeldSelf,
    ...
    registry = input_registry(state, self_guard.file_sha256, static_trust)
    descriptors = {
    ...
    lock = (
        "C79g v3 isolated candidate only; standalone non-authoritative. Formal-global credit "
        "remains zero until the unique composite committed completion, live replay24, exact v2 "
        "rejection and authority seal predicate is independently consumed. D02 is not started.\n"
    ).encode("ascii")
    ...
    for guard in direct_guard["held_inputs"]:
        guard.terminal_replay()
    direct_guard["C42_candidate_guard"].terminal_replay()
    ...
    for guard in direct_guard["held_inputs"]:
        guard.close()
    direct_guard["C42_candidate_guard"].close()
