#!/usr/bin/env python3
"""Build the exact-role, manifest-first C27R2 release-repair package v4.

There is no caller-selected ``--source-file`` escape hatch.  Every immutable
authority/source role is frozen in the externally pinned v4 plan; every
...
SOURCE_ROLES = {
    "python", "transaction_runner", "cold_runner", "boundary_validator",
...
BOUNDARY_CONTRACT_FROZEN_GO = False  # boundary v4 historical NO-GO; v5 pending.
...
def execute(args: argparse.Namespace) -> dict[str, Any]:
    ...
    post_rows = records_ledger(post_rows_path,
                               BASE + "post-authority-member-row.v4")
...
def self_test() -> dict[str, Any]:
    ...
    need("source-file" not in {"source-file" for _ in ()}
         and len(SOURCE_ROLES) == 14 and len(RUNTIME_FILES) == 17,
         "exact role counts")
    with tempfile.TemporaryDirectory(dir=ROOT / ".cm2-runtime",
            prefix="c27r2-manifest-v4-selftest-") as raw:
        root = Path(raw)
    ...
    return {"status": "PASS_C27R2_RELEASE_REPAIR_MANIFEST_V4_SELF_TEST"}
