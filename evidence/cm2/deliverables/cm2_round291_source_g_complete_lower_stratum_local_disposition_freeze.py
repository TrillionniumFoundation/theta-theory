#!/usr/bin/env python3
"""Independently freeze all 55,428 nominal lower-stratum dispositions.

This program does not import, execute, or trust the Round280/Round281
producers or their outputs.  It rebuilds the complete local
existence/absence partition from the pinned Round174, Round179, Round182,
Round204, Round208, and Round267 artifacts.  The 396 fully-replaced origins
are rechecked with a local pure-Fraction interval evaluator; no upstream
producer or verifier is imported or executed.

The result is deliberately local and ZERO-CREDIT.  A physical lower stratum
is not an expanded occurrence, a component edge, a maximality proof, an
exact-key fibre disposition, or a same-point Jx/Jy identification.
"""
...
def main() -> None:
    ...
    parser.add_argument("--seed", default="291071")
    ...
    safe_write(args.ledger, ledger_bytes)
    safe_write(args.result, canonical(result))
    print(json.dumps({
