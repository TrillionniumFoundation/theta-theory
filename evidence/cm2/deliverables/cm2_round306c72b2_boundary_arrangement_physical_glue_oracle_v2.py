#!/usr/bin/env python3
"""C72b2: exact H1 boundary arrangements and physical chart glue.
    ...
    for atlas in payload["rows"]:
        ...
        if kind == "STRICT_NEGATIVE":
            ...
            if kind == "STRICT_POSITIVE":
                ...
                need(kind == "CLIPPED", "geometry kind")
                chart = "N" if ny_sign > 0 else "S"
...
def build(c72_a: Path, c72_b: Path, output: Path, batch_size: int) -> dict[str, Any]:
    ...
    need(dict(route_census) == {"COLLISION1_OFFICIAL_WORD_MISMATCH": 19881,
         ...
         "W-route census")
    need(dict(graph_route_census) == {
        "COLLISION1_OFFICIAL_WORD_MISMATCH": 19879,
        "COLLISION2_STRICT_OWNER_MISMATCH": 35334}, "graph route census")
    expected_endpoint_occurrences = 2 * 55213
    need(len(endpoint_occurrence_ids) == expected_endpoint_occurrences and
         ...
         "2x55213 endpoint occurrence conservation")
    ...
    result = {"schema": SCHEMA + ".result",
              "status": "PASS_55216_EXACT_WHOLE_CHILD_STRICT_EXCLUSIONS__55213_PHYSICAL_GRAPH_GLUES__ZERO_CREDIT",
              ...
              "strict_whole_child_exclusion_count": 55216,
              ...
              "endpoint_occurrence_conservation_identity":
                  "2*55213=110426=sum(root_degree*root_count)=sum(incidence_count)",
              ...
              "WHOLE_CHILD_STRICT_EXCLUSION_CLOSED": 55216,
              ...
              "proof_invariants": {
                  ...
                  "exactly_110426_unique_endpoint_occurrences_materialized": True,
                  "every_endpoint_occurrence_maps_to_exactly_one_root_id_and_face": True,
                  "root_degree_census_conserves_all_110426_occurrences": True,
                  "sum_incidence_count_equals_110426": True,
    ...
    report = ("# C72b2 boundary arrangement and physical-glue oracle\n\n"
              "- H1 geometry closed: 55,216 / 55,216.\n"
              "- Exact whole-child strict exclusions: 55,216 / 55,216.\n"
              "- Clipped graphs: 55,213; each continues as the same physical state under the W half-open owner and is strictly excluded downstream.\n"
