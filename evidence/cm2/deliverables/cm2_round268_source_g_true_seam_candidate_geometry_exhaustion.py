#!/usr/bin/env python3
"""Exhaust the omitted Round182 true source-chart seam candidate pool.
...
def main() -> int:
    ...
    parser.add_argument("--seed", type=int, default=268_071)
    parser.add_argument("--no-write", action="store_true")
    args = parser.parse_args()
    need(isinstance(args.seed, int) and not isinstance(args.seed, bool), "seed")
    producer_sha256 = hashlib.sha256(regular_bytes(Path(__file__).resolve(), 5_000_000)).hexdigest()
    ...
    print("pairs=13924 positive=152 exact_empty=13772 area=1/80 component_credit=0")
    return 0
