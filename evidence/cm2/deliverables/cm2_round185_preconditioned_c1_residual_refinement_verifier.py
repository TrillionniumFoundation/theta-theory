#!/usr/bin/env python3
"""Independent fail-closed verifier for Round185.

The Round185 producer is pinned as inert source bytes and is neither imported
nor executed.  This verifier independently rebuilds the complete expected
result from the frozen Round183 chain, including all C0/C1, signed-factor,
adaptive-face, rank, and dimension ledgers.  Acceptance requires full
canonical-result equality.  Re-signed mutations are rejected without using a
frozen expected Round185 result digest.
"""
...
CERTIFICATE = HERE / "cm2_round185_preconditioned_c1_residual_refinement_certificate.json"
OUTPUT = HERE / "cm2_round185_preconditioned_c1_residual_refinement_verification.json"
PRODUCER = "cm2_round185_preconditioned_c1_residual_refinement.py"
SCHEMA = "cm2.round185.preconditioned-c1-residual-refinement.v1"
VERIFICATION_SCHEMA = "cm2.round185.preconditioned-c1-residual-refinement-verification.v1"
PRODUCER_SHA256 = "7b48f3ee3417fcfdf5ef6c852e0ab591eb849b357e704e46ee3aaa259d20acc2"
...
def read_regular(path: Path, expected: str | None = None,
    ...
    require(st.st_size <= maximum, f"oversized:{path.name}")
...
def validate_tree(value: Any, path: str = "$") -> None:
    require(type(value) in {dict, list, str, int, bool, type(None)},
            f"type:{path}")
...
def strict_json(raw: bytes, label: str,
    ...
    try:
        ...
        raise Round185Error(f"JSON:{label}:{exc}") from exc
    validate_tree(value)
...
def validate_candidate(candidate: dict[str, Any],
    ...
    require(candidate["result_sha256"] == digest(candidate["result"]),
            "self digest")
    require(candidate["result"] == expected,
            "full independently rebuilt expected equality")
...
def semantic_attacks(certificate: dict[str, Any],
    ...
    def derivative_census(result: dict[str, Any]) -> None:
        result["H2_full_box_preconditioned_probe"][
            "full_box_strict_derivative_counts"]["dp"] = 11907

    def h2_c0_absence(result: dict[str, Any]) -> None:
        block = result["H2_full_box_preconditioned_probe"][
            "centered_mean_value_C0_sign_counts"]
        block["POSITIVE"] = 3453
        block["UNRESOLVED"] = 5527

    def oblique_newton(result: dict[str, Any]) -> None:
        result["H2_full_box_preconditioned_probe"][
            "boxes_with_any_oblique_interval_Newton_self_map"] = 1

    def factor_overwrap(result: dict[str, Any]) -> None:
        result["H2_signed_factor_union_probe"][
            "both_factor_centered_C0_overwrap_box_count"] = 1

    def factor_absence(result: dict[str, Any]) -> None:
        result["H2_signed_factor_union_probe"][
            "both_factors_centered_C0_absent_box_count"] = 6905

    def factor_existence(result: dict[str, Any]) -> None:
        result["H2_signed_factor_union_probe"][
            "boxes_with_any_independent_factor_existence"] = 4469

    def simultaneous_factor_zero(result: dict[str, Any]) -> None:
        result["H2_signed_factor_union_probe"][
            "simultaneous_h_plus_h_minus_zero_impossible"] = False

    def exact_row_global(result: dict[str, Any]) -> None:
        block = result["dynamic_local_exact_key_ledger"]
        row = block["rows"][0]
        row["global_credit"] = 1
        resign_row(row)
        block["rows_sha256"] = digest(block["rows"])

    def exact_count(result: dict[str, Any]) -> None:
        result["dynamic_local_exact_key_ledger"]["row_count"] = 19051

    def exact_volume(result: dict[str, Any]) -> None:
        result["dynamic_local_exact_key_ledger"][
            "exact_coordinate_volume"] = "0"

    def arrangement_credit(result: dict[str, Any]) -> None:
        result["dimensionally_arranged_H2_factor_seams"][
            "whole_parent_or_stratum_credit"] = 1

    def arrangement_owner(result: dict[str, Any]) -> None:
        block = result["dimensionally_arranged_H2_factor_seams"]
        row = block["rows"][0]
        row["detail"]["three_stratum_chart_assignment"][
            "active_factor_zero_2D_half_open_owner"] = "N"
        resign_row(row)
        block["rows_sha256"] = digest(block["rows"])

    def residual_volume(result: dict[str, Any]) -> None:
        result["dynamic_residual_ledger"][
            "exact_coordinate_outer_volume"] = "0"

    def forged_factor_existence(result: dict[str, Any]) -> None:
        block = result["dynamic_residual_ledger"]
        row = next(item for item in block["rows"]
                   if item["status"] == "H2_FACTOR_EXISTENCE_RESIDUAL")
        graph = next(item for item in row["surface_summaries"]
                     if not item["independently_certified_nonempty"])
        graph["independently_certified_nonempty"] = True
        resign_row(row)
        block["rows_sha256"] = digest(block["rows"])

    def split_face_owner(result: dict[str, Any]) -> None:
        block = result["all_adaptive_split_face_dimension_ledger"]
        row = block["rows"][0]
        row["half_open_owner_child_path"] = row[
            "excluded_duplicate_child_path"]
        resign_row(row)
        block["rows_sha256"] = digest(block["rows"])

    def face_not_reclassified(result: dict[str, Any]) -> None:
        result["all_adaptive_split_face_dimension_ledger"][
            "faces_independently_reclassified"] = 0

    def one_dimensional_credit(result: dict[str, Any]) -> None:
        result["all_adaptive_split_face_dimension_ledger"][
            "certified_nonempty_transverse_1D_intersections"] = 1

    def corner_count(result: dict[str, Any]) -> None:
        result["all_adaptive_split_face_dimension_ledger"][
            "zero_dimensional_corner_deduplication"][
                "unique_0D_corner_key_count"] = 20987

    def corner_credit(result: dict[str, Any]) -> None:
        result["all_adaptive_split_face_dimension_ledger"][
            "zero_dimensional_corner_deduplication"][
                "certified_0D_boundary_incidence_credit"] = 1

    def rank_implies_existence(result: dict[str, Any]) -> None:
        result["two_Delta_rank_ledger"][
            "intersection_existence_certified_count"] = 14232

    def rank_promotes_1d(result: dict[str, Any]) -> None:
        result["two_Delta_rank_ledger"][
            "certified_unique_1D_incidence_count"] = 14232

    def inherited_global(result: dict[str, Any]) -> None:
        result["inherited_local_exclusion_ledger"]["global_credit"] = 170

    def source_seam_owner(result: dict[str, Any]) -> None:
        result["inherited_residual_ledger"][
            "source_seam_2D_half_open_owner"] = "N"

    def inherited_count(result: dict[str, Any]) -> None:
        result["inherited_residual_ledger"]["row_count"] = 889

    def all_parents_full(result: dict[str, Any]) -> None:
        block = result["per_parent_partial_ledger"]
        block["fully_closed_live_strata"] = 16
        block["partial_live_strata"] = 0

    def one_parent_full(result: dict[str, Any]) -> None:
        block = result["per_parent_partial_ledger"]
        row = block["rows"][0]
    ...
    def point_credit(result: dict[str, Any]) -> None:
        result["strict_nonpromotion"][
            "observed_point_289591_global_credit"] = 1

    def inner_credit(result: dict[str, Any]) -> None:
        result["strict_nonpromotion"][
            "inner_or_refined_box_290575_global_credit"] = 1

    def status_full(result: dict[str, Any]) -> None:
        result["status"] = "PASS_FULL_D02_CLOSURE"

    def conservation(result: dict[str, Any]) -> None:
        result["dynamic_residual_ledger"]["exact_conservation"] = "forged"

    attacks.extend([
        ("H2_full_box_derivative_census_mutated", derivative_census),
        ("H2_centered_C0_absence_mutated", h2_c0_absence),
        ("oblique_Newton_forged_success", oblique_newton),
        ("both_factor_overwrap_forged", factor_overwrap),
        ("factor_absence_census_mutated", factor_absence),
        ("factor_existence_census_mutated", factor_existence),
        ("simultaneous_factor_zero_allowed", simultaneous_factor_zero),
        ("local_exact_row_forged_global", exact_row_global),
        ("dynamic_exact_count_mutated", exact_count),
        ("dynamic_exact_volume_mutated", exact_volume),
        ("H2_seam_arrangement_forged_whole_credit", arrangement_credit),
        ("H2_seam_half_open_owner_changed", arrangement_owner),
        ("dynamic_residual_volume_mutated", residual_volume),
        ("unbracketed_factor_forged_nonempty", forged_factor_existence),
        ("adaptive_split_face_owner_swapped", split_face_owner),
        ("split_faces_not_reclassified", face_not_reclassified),
        ("split_face_1D_outer_forged_certified", one_dimensional_credit),
        ("deduplicated_corner_count_mutated", corner_count),
        ("zero_dimensional_corner_credit_forged", corner_credit),
        ("rank2_forged_as_existence", rank_implies_existence),
        ("rank2_forged_as_unique_1D", rank_promotes_1d),
        ("inherited_exclusions_forged_global", inherited_global),
        ("source_seam_half_open_owner_changed", source_seam_owner),
        ("inherited_residual_count_mutated", inherited_count),
        ("sixteen_parents_forged_full", all_parents_full),
        ("one_parent_forged_full", one_parent_full),
        ("D02_forged_pass", d02_pass),
        ("Gate5_forged_18_of_18", gate5_full),
        ("point_289591_forged_global_credit", point_credit),
        ("inner_290575_forged_global_credit", inner_credit),
        ("partial_status_forged_full", status_full),
        ("dynamic_volume_conservation_forged", conservation),
    ])
    rejected = []
    for label, mutate in attacks:
        ...
        except Round185Error:
            rejected.append(label)
        else:
            raise Round185Error(f"semantic attack accepted:{label}")
    return rejected
...
def json_attacks(certificate: dict[str, Any]) -> list[str]:
    ...
    for label, raw in cases.items():
        try:
            ...
            raise Round185Error(f"JSON attack accepted:{label}")
    return rejected
...
def validate_output(path: Path) -> None:
    ...
    protected = {
        (HERE / PRODUCER).resolve(),
        Path(__file__).resolve(),
        CERTIFICATE.resolve(),
    }
    protected.update((HERE / name).resolve() for name in PINS)
...
def path_attacks() -> list[str]:
    rejected = []
    with tempfile.TemporaryDirectory(dir=HERE, prefix=".round185-path-") as name:
        root = Path(name)
        ...
        try:
            for label, operation in cases:
                try:
                    operation()
                except (Round185Error, OSError):
                    rejected.append(label)
                else:
                    raise Round185Error(f"path attack accepted:{label}")
        finally:
...
def main() -> int:
    ...
    require(certificate_path.parent.resolve() == HERE,
            "certificate directory")
    certificate = strict_json(read_regular(certificate_path),
    ...
    result = {
        ...
        "independent_reconstruction": {
            "Round183_dynamic_residual_input_count": 25040,
            "Round183_inherited_carrier_input_count": 530,
            "H2_direct_centered_C0_strict_count": 6380,
            "H2_factorized_C0_absent_count": 6904,
            "H2_nonempty_signed_seam_arrangement_count": 4468,
            "H2_factor_existence_residual_terminal_count": 592,
            "dynamic_local_exact_key_count": 19050,
            "dynamic_residual_outer_count": 15830,
            "inherited_local_exclusion_count": 170,
            "inherited_residual_outer_count": 890,
            "combined_remaining_residual_outer_count": 16720,
            "all_split_face_count": 14838,
            "faces_with_different_child_statuses": 5002,
            "nominal_split_face_1D_outers": 12636,
            "unique_deduplicated_0D_corner_keys": 20986,
            "strict_rank2_if_intersection_count": 14232,
            "certified_unique_1D_incidence_count": 0,
            "fully_closed_live_strata": 0,
            "partial_live_strata": 16,
            "global_credit": 0,
            "D02": "BLOCKED",
            "global_Gate5_fields": "10/18",
        },
        ...
        "verifier_provenance": {
            ...
            "effective_Arb_precision_bits": atlas.ctx.prec,
            "precision_source":
                "pinned Round183/Round181/Round178 transitive chain",
        },
    }
    envelope = {
        "schema": VERIFICATION_SCHEMA,
