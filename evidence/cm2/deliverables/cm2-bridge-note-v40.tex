...
\large Source-incidence partitions, parent-kernel tilts, survivor scale,
two-cutoff face convergence, and a strongly conditional
pathwise CM2 criterion}
\author{Working note}
\date{13 July 2026 (version 40)}
...
actual additive source occurrence instead lives in a branch--occurrence
incidence space and retains its stopped source, orientation, recovery time,
opposite-side amplitude, clock, and Jensen/TV passage.  Hence two distinct
currents with identical support are counted twice.  Source-incidence
...
have separate Jensen and TV columns, while word depth remains exclusively in
the face/deep ledger.  The theorem assumes aggregate summability of both the
probability union and the additive source mass, or derives it from strict
complexity gaps before applying H\"older.  Only genuine two-time TV tails enter
the central exponent without loss; genuine-time Jensen tails lose one half
exactly as PPE-depth Jensen tails do.  The
...
source-incidence, parent-kernel reverse-H\"older, branch-section,
...
source occurrence.  A zero-mass source kernel is omitted.  Let
...
\begin{definition}[Source-incidence space]
\label{def:source-incidence-space}
For a complete-tree label $\lambda$ let $\mathcal I(\lambda)$ be the finite
or countable set of every physical source/current occurrence actually
present in the assembled defect on that branch.  It includes the unique
first-failure cemetery source when one occurs and every retained
$r$-, PPE-, time-, or word-depth remainder current.  Before any estimate, a
canonical owner map $o:\mathcal I(\lambda)\to\mathcal O_{\rm src}$ assigns
each physical current exactly once.  Two different currents remain different
occurrences even when their supports and trigger events coincide.  Define
\begin{equation}
\label{eq:source-incidence-space}
 \Omega_{\rm src}:=\{(\lambda,a):a\in\mathcal I(\lambda)\},
 \qquad d\widehat\Pi_{\rm src}(\lambda,a)
 :=d\Pi_{s,N}(\lambda)\,d\#_{\mathcal I(\lambda)}(a).
\end{equation}
Every occurrence has fixed data
\begin{equation}
\label{eq:source-incidence-record}
 (o(a),\tau(a),\mathfrak S_a^{\varepsilon_a},\varepsilon_a,
   \Sigma_a,U_a,R_a,\mathscr A_a^{\rm opp},\kappa(a),\pi(a)),
\end{equation}
where $\kappa(a)\in\{r,\mathrm{PPE},N_{\rm time},\mathrm{word}\}$ is its
clock and $\pi(a)\in\{J,\mathrm{TV}\}$ is its passage through Jensen or
absolute total variation.  Put $J_a=U_a\Sigma_a$ and require a positive
incidence envelope, measurable at the declared first stage,
\begin{equation}
\label{eq:source-incidence-envelope}
 \mathscr W_{\rm src}(\lambda,a)\ge
 \|\Sigma_a(\lambda)\|_{\mathfrak S_a^{\varepsilon_a}}
 \gamma_a^{-R_a(\lambda)}\mathscr A_a^{\rm opp}(\lambda).
\end{equation}
The source spaces and propagators are those of
Definition~\ref{def:concrete-source-spaces} and
Lemma~\ref{lem:stopped-source-propagation}, or an explicitly named typed
primitive.  In particular their constants do not depend on $m,n,s$, scalar
mode, polynomial, or final observable.  Then
\begin{equation}
\label{eq:source-incidence-TV-bound}
 \left|\sum_{(\lambda,a)\in\Omega_{\rm src}}
       \langle J_a,\Phi\rangle\right|
 \le C\|\Phi\|_{\rm test}
 \int_{\Omega_{\rm src}}\mathscr W_{\rm src}\,
      d\widehat\Pi_{\rm src}.
\end{equation}
This is an additive positive-source bound.  It is not an event union and no
first-hit support disjointization may remove a retained occurrence.  The
cemetery occurrences are already mutually exclusive by first failure; that
fact is used only for their probability union, not to merge different
currents.
\end{definition}
...
them.  Atomic tail bounds by themselves are not part of
$\mathrm{TENERGY}_{W}$.
\end{definition}
...
additive currents coexist on an atom.  With word-depth kept in
$\mathrm{FACE}_{\rm 2CUT}$, define
\begin{align}
...
\begin{definition}[Source clock--passage matrix]
\label{def:source-clock-matrix}
For $(\kappa,\pi)\in
\{r,\mathrm{PPE},N_{\rm time}\}\times\{J,\mathrm{TV}\}$ let
$\mathcal A_{\kappa,\pi}(t)\subset\Omega_{\rm src}$ contain precisely the
source occurrences with clock $\kappa$, passage $\pi$, and bad/source
condition at scale $t$.  Define their normalized positive source masses by
\begin{equation}
\label{eq:clock-source-partition}
 \mathfrak Z_{\kappa,\pi}^{\rm src}(t):=
 \frac{1}{\mathbb E_{\Pi_{s,N}}\mathscr W_*}
 \int_{\mathcal A_{\kappa,\pi}(t)}
     \mathscr W_{\rm src}\,d\widehat\Pi_{\rm src}.
\end{equation}
The six central aggregate bounds are
\begin{align}
 \mathfrak Z_{r,J}^{\rm src}(r)&\le Cr^{\Theta_{r,J}^{\rm src}},&
 \mathfrak Z_{r,\rm TV}^{\rm src}(r)&\le Cr^{\Theta_{r,\rm TV}^{\rm src}},
 \notag\\
 \mathfrak Z_{\rm PPE,J}^{\rm src}(N)&\le
    Ce^{-c_{\rm PPE,J}^{\rm src}N},&
 \mathfrak Z_{\rm PPE,TV}^{\rm src}(N)&\le
    Ce^{-c_{\rm PPE,TV}^{\rm src}N},
 \notag\\
 \mathfrak Z_{\rm time,J}^{\rm src}(N)&\le
    Ce^{-c_{\rm time,J}^{\rm src}N},&
 \mathfrak Z_{\rm time,TV}^{\rm src}(N)&\le
    Ce^{-c_{\rm time,TV}^{\rm src}N}.
 \label{eq:clock-source-matrix-tails}
\end{align}
Every rate is strictly positive for a nonempty class; an empty class is
omitted from subsequent minima.  Word-depth occurrences are summed only in
$\mathrm{FACE}_{\rm 2CUT}$ or the deep positive-source ledger.  At minimum
the owner table contains
\begin{center}
\small
\begin{tabular}{c|c|c|c|c}
owner/source & first stage & raw tail & clock & passage\\ \hline
main PPE small-ball source & retained native cell & $r^{\Theta}$ & $r$ & $J$\\
bad context current & context gate & $r^{\Theta}$ & $r$ & $J$\\
native-stop current & antichain & $r^{\Theta}$ & $r$ & $J$\\
buffer/gate/continuation source & named failure & $e^{-cN_{\rm PPE}}$ & PPE & $J$\\
adaptive-alphabet energy source & physical gate & $e^{-cN_{\rm PPE}}$ & PPE & $J$\\
retained PPE absolute remainder & descendant & $e^{-cN_{\rm PPE}}$ & PPE & TV\\
contracted-coordinate main source & endpoint replacement & $e^{-cN_{\rm time}}$ & time & TV
\end{tabular}
\end{center}
Any new genuine-time Jensen source must enter the time--$J$ cell; it may not
be hidden in the time--TV rate.  Bad context and native stop are named
probability rows, not additional $\mathcal J_r^{\rm prob}$ events, but their
actual currents occur once in the source matrix.  Distinct currents on the
same support remain distinct incidence points.

The source-incidence complexity is the growth of the \emph{sum of source
prefactors}.  For example, if before tilting
\begin{equation}
\label{eq:source-raw-complexity-gap}
 \int_{\mathcal A_{\kappa,\pi}(N)}
     \mathscr W_{\rm src}^{\rm raw}\,d\widehat\Pi_{\rm src}
 \le Ce^{-(c_{\kappa,\pi}^{\rm raw}-a_{\kappa,\pi}^{\rm src})N},
 \qquad c_{\kappa,\pi}^{\rm raw}>a_{\kappa,\pi}^{\rm src},
\end{equation}
then aggregate-then-H\"older yields
\begin{equation}
\label{eq:source-aggregate-holder-rate}
 c_{\kappa,\pi}^{\rm src}
 =\frac{c_{\kappa,\pi}^{\rm raw}-a_{\kappa,\pi}^{\rm src}}{p'}.
\end{equation}
For a polynomial source sum with prefactor $r^{-a_{r,\pi}^{\rm src}}$,
\begin{equation}
\label{eq:source-polynomial-complexity-gap}
 \Theta_{r,\pi}^{\rm src}
 =\frac{\Theta_{r,\pi}^{\rm raw}-a_{r,\pi}^{\rm src}}{p'}>0.
\end{equation}
A nonpositive source gap is a genuine no-go even when the bad-event union is
exponentially small.  The H\"older step in
\eqref{eq:source-aggregate-holder-rate} requires an $L^p$ bound for the
normalized source-incidence weight on its own incidence law; a branch-level
complete-tree moment alone proves no such statement.
\end{definition}
...
\eqref{eq:clock-prob-tails}.  The named failures and three probability
partition functions tend to zero at their declared scales; shrink the
admissible parameter range so their sum is at most $1/2$.  No source
...
\label{eq:physical-energy-exponent}
 \Theta_{\rm par}^{W}:=\min\left\{\Theta_{\rm PPE}^{W},
        \Theta_{\rm ctx}^{W},\Theta_{\rm st}^{W}\right\}>0,
\end{equation}
one has
\begin{align}
...
\label{eq:complete-energy-tail}
 \mathcal T_{\rm prob}^{W}(r):=
 \sum_{i\in\{\rm buf,gate,cont\}}\Pi^W(B_i(N_{\rm PPE}(r)))
 +\mathfrak Z_r^{\rm prob}(r)
 +\mathfrak Z_{\rm PPE}^{\rm prob}(N_{\rm PPE}(r)).
\end{equation}
The positive mass of all additive sources which actually pass through this
energy/Jensen step satisfies the incidence statements
\begin{align}
 \mathcal E_{\rm src}^{W}(r)&\le C\left\{
 r^{\Theta_{r,J}^{\rm src}}+
 e^{-c_{\rm PPE,J}^{\rm src}N_{\rm PPE}(r)}\right\},
 \label{eq:source-energy-bound}\\
 \mathcal V_{\rm src}^{W}(r)&\le C\left\{
 r^{\Theta_{r,J}^{\rm src}/2}+
 e^{-c_{\rm PPE,J}^{\rm src}N_{\rm PPE}(r)/2}\right\}.
 \label{eq:source-value-small-ball}
\end{align}
These are additive source masses, not probabilities of a bad-support union.
\end{theorem}
...
antichain mass, and $\mathrm{CLOCK\_PROB}_{W}$ gives exactly
$\mathcal T_{\rm prob}^{W}(r)$.  This proves the probability assertions.  For
the actual current sum, attach the canonical owner occurrence to every
retained or cemetery source and use
\eqref{eq:source-incidence-TV-bound}.  The two Jensen cells of
\eqref{eq:clock-source-matrix-tails} give
\eqref{eq:source-energy-bound}; applying the displayed Cauchy--Jensen
argument inside each positive occurrence and then summing on
$\Omega_{\rm src}$ gives \eqref{eq:source-value-small-ball}.  All
...
probability tail into the corresponding source bound; retained additive
remainders are never removed by the probability first-hit partition.
\end{proof}
...
$C\{r^{\Theta_{r,J}^{\rm src}/2}
+e^{-c_{\rm PPE,J}^{\rm src}N_{\rm PPE}(r)/2}\}$.  This is the
source-incidence mass which actually enters Jensen, not the probability of
...
\eqref{eq:clock-source-matrix-tails}.  In the diagonal band these imply
...
Use the complete additive source rates
\eqref{eq:clock-source-matrix-tails}, and set
\[
...
\begin{aligned}
 \beta_{\rm PPE,J}^{\rm src}&:=
   \tfrac12\delta C_{\rm buf}c_{\rm PPE,J}^{\rm src},
 &\beta_{\rm PPE,TV}^{\rm src}&:=
   \delta C_{\rm buf}c_{\rm PPE,TV}^{\rm src},\\
 \beta_{\rm phase}^{W}&:=
 \min\left\{
 \begin{gathered}
  \delta\Theta_{r,J}^{\rm src}/2,
...
\label{eq:phase-energy-all-tails}
 C\Xi^{-\delta\Theta_{r,J}^{\rm src}/2}
 +C\Xi^{-\delta C_{\rm buf}c_{\rm PPE,J}^{\rm src}/2}.
\end{equation}
...
\label{eq:complete-central-exponent}
 ...
 \tfrac12c_{\rm time,J}^{\rm src},c_{\rm time,TV}^{\rm src},
 \eta_{\rm hi}\rho_\infty-a_{\rm ultra},c_{\rm scl},
 c_{\rm low}\}>0,
\end{equation}
where $c_{\rm low}$ is supplied by the low recovery theorem and
$c_{\rm time,J}^{\rm src},c_{\rm time,TV}^{\rm src}>0$ are the two genuine
$N_{\rm time}$ source rates.  The first loses one half because it passes
through Jensen; the second, which includes the contracted-coordinate main
source in the present organization, is summed by TV.  All
...
\[
 ...
 \mathrm{CEM\_DOM},\\
 \mathrm{TENERGY}_{W},\ \mathrm{SOURCE\_INCIDENCE},\
 \mathrm{CLOCK\_PROB}_{W}
 \end{gathered}
\]
\[
...
\[
 ...
 \mathrm{SOURCE\_INCIDENCE},\ \mathrm{CLOCK\_PROB}_{W},\\
 \mathrm{CLOCK\_SRC}_{W},\ \mathrm{KRH}_{p_W},\
 \mathrm{USC}_{\rm end}^{W}(q_g^W)
 \end{gathered}
\]
...
the proof: it improves the typed $\beta_{\rm PPE,J}^{\rm src}$ and
$\beta_{\rm PPE,TV}^{\rm src}$ rates but enlarges $C_{\rm mid}$.
...
\end{equation}
In particular \eqref{eq:full-product-Lp-energy-route} follows from the
...
$\chi_W<1$, then every $1<p_W<\chi_W^{-1}$ satisfies
\begin{equation}
...
$K_\eta$, using the same strict reciprocal margin.  Since $W_\eta\ge1$ on
probability parents, its normalizing denominator is at least one.  On a
source-incidence parent one first normalizes the positive incidence mass;
the displayed hypothesis includes the resulting likelihood ratio.  The
...
Assume global unweighted aggregate probability-clock bounds on the complete
tree and assume
...
source occurrences the same calculation is valid only after integrating the
positive incidence density on $\Omega_{\rm src}$.  No branchwise Jensen,
...
route, all of its component moments must be verified under every such
...
\eqref{eq:complete-tree-cemetery-moment}, and
\[
...
\]
Assume also the pathwise jet condition of
...
resulting $p_W$ in \eqref{eq:available-full-product-pW} proves
\eqref{eq:global-ledger-sum} and transfers only global probability
aggregates.  Parentwise energy and upper-scale tails use
...
\label{eq:final-CM2-exponent}
 ...
 \eta_{\rm hi}\rho_\infty-a_{\rm ultra},c_{\rm scl},
 c_{\rm time,J}^{\rm src}/2,c_{\rm time,TV}^{\rm src},\eta_{\rm lo}R,
...
There are twenty-two falsifiable interfaces.  $\mathrm{MT}_{\rm DQ}$ must
...
$\mathrm{SOURCE\_INCIDENCE}$ must retain every additive occurrence even on
overlapping supports.  $\mathrm{CLOCK\_PROB}_{W}$ controls only bad-support
...
$\mathrm{KRH}_{p_W}$ supplies the uniform frozen-parent likelihood bound;
...
\begin{proposition}[Thirty-eight obstruction tests]
\label{prop:v40-obstruction-tests}
...
\item If either $\beta_{\rm PPE,J}^{\rm src}$ or
      $\beta_{\rm PPE,TV}^{\rm src}$ is smaller than
      $\min\{\delta\Theta_{r,J}^{\rm src}/2,
      \delta\Theta_{r,\rm TV}^{\rm src},
...
\item A hierarchy may have exponentially decaying atomic activations but a
      ...
      \eqref{eq:source-raw-complexity-gap}, and
      \eqref{eq:source-polynomial-complexity-gap} declare this an explicit
...
\item Let one rare support have weighted probability $e^{-cN}$ but carry
      ...
      \eqref{eq:source-raw-complexity-gap} record the true
      $e^{-(c-a)N}$ source mass.
...
\item A genuine $N_{\rm time}$ source with mass $e^{-cN}$ which enters the
      energy/Jensen passage contributes only $e^{-cN/2}$.  The two time
      cells in \eqref{eq:clock-source-matrix-tails} and the final minimum
...
\begin{proof}
Items 1--3, 8--11, and 13--38 are direct finite-measure, frequency, or norm
