#!/usr/bin/env python3
"""Close C27R2 v4 evidence after 70 validator and eight lock checks.
...
SNAPSHOT_SCHEMA = BASE + "current-snapshot-evidence.v4"
SNAPSHOT_STATUS = (
    "PASS_C27R2_RELEASE_REPAIR_V4_CORE_FRESH_COLD_BOUNDARY_TRUST_EXACT_"
    "TERMINAL_INVENTORIES_AND_FULL9STAT_SNAPSHOT_FROZEN__ZERO_CREDIT_"
    "NOT_MANIFEST_AUTHORIZED"
)
POST_SCHEMA = BASE + "post-integrity-evidence-closure.v4"
POST_STATUS = (
    "PASS_C27R2_RELEASE_REPAIR_V4_SNAPSHOT_BASELINE_VALIDATOR70_LOCK8_REAL_"
...
LAUNCH_CONTROL_FILES = {
    "command_spec.json", "pinset.json", "stage_launch_receipt.json",
}
FIXTURE_SCHEMA = (
...
BOUNDARY_VALIDATOR_PATH = (
    ...
    "rebuild_v2_release_repair_boundary_independent_validator_v5.py"
)
BOUNDARY_VALIDATOR_SHA = "0" * 64  # Pending final boundary-v5 source pin.
INTEGRITY_RUNNER_PATH = (
    "deliverables/cm2_round306c27r2_source_g_actual_v2_fresh_quotient_"
    "rebuild_v2_release_integrity_fixture_runner_v5.py"
)
INTEGRITY_RUNNER_SHA = "0" * 64  # Pending final boundary-v5 source pin.
...
def read_snapshot(directory: Path) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    for ordinal, line in enumerate(raw.splitlines()):
        ...
        need(type(row) is dict and canonical(row) == line
             and row.get("schema") == BASE + "snapshot-member-row.v4"
...
def transaction(directory: Path, plan: dict[str, Any], role: str) -> dict[str, Any]:
    need(directory.is_dir() and not directory.is_symlink()
         and {x.name for x in directory.iterdir()} == RUN_FILES,
         "transaction inventory:" + role)
    need(read_current(directory / "PASS.lock") == RUN_PASS
         and read_current(directory / "exit_code.txt") == b"0\n"
         and read_current(directory / "signal.json") == b"null\n"
         and read_current(directory / "stderr.log") == b""
         and read_current(directory / "input_pre.json")
             == read_current(directory / "input_post.json"),
         "transaction scalar closure:" + role)
...
def case_transaction(directory: Path, case: str, ordinal: int,
    ...
    need(directory.is_dir() and not directory.is_symlink()
         and {x.name for x in directory.iterdir()} == RUN_FILES,
         "case exact eleven files")
    need(read_current(directory / "PASS.lock") == CASE_RUN_PASS
         and read_current(directory / "exit_code.txt") == b"0\n"
         and read_current(directory / "signal.json") == b"null\n"
         and read_current(directory / "stderr.log") == b"",
         "case scalar closure")
...
def execute(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for ordinal, item in enumerate(records):
        body = {"schema": BASE + "post-authority-member-row.v4",
...
def self_test() -> dict[str, Any]:
    need(FORMAL_EXECUTION_AUTHORIZED is False
         ...
         "development source remains launch-blocked on boundary v5 audit")
    try:
    ...
    with tempfile.TemporaryDirectory(dir=ROOT / ".cm2-runtime",
            prefix="c27r2-post-v4-selftest-") as raw:
        root = Path(raw)
        member = root / "member"
        ...
        need({x.name for x in case.iterdir()} == RUN_FILES,
             "real eleven-member inventory fixture")
    return {"status": "PASS_C27R2_RELEASE_REPAIR_POST_V4_SELF_TEST"}
