#!/usr/bin/env python3
"""Externally pinned cold launcher for the C79g v5 exact10 static bundle.
...
LAUNCHER_RELATIVE = Path("deliverables") / (BASE + "_cold_launch_v5.py")
...
ROOT = Path("/__C79G_V5_UNCONFIGURED_ROOT__")
...
V5_REJECTION_NAMESPACE = ROOT / "__unconfigured_v5_rejections__"
V5_LATER_REJECTION = V5_REJECTION_NAMESPACE / "rejection.json"
...
def configure_workspace_paths(root: Path) -> None:
    ...
    global V4_REJECTION_SUPERSESSION, V5_REJECTION_NAMESPACE
    global V5_LATER_REJECTION, SCHEMA, CONTRACT, PRODUCER, CONSUMER
    global TRANSITION, AUDIT, MANIFEST, OUTER, BASE7_PINS, EXACT8
    global V3_EXACT10_PINS, V4_FROZEN_DRAFT7_PINS
    global V4_FORBIDDEN_RUNTIME_PATHS
    ROOT = root
    OUT = ROOT / "deliverables"
    RUNTIME = ROOT / ".cm2-runtime"
    SELF = ROOT / LAUNCHER_RELATIVE
    ...
    V5_REJECTION_NAMESPACE = RUNTIME / ("c79g-v5-rejections-" + CHECKPOINT)
    V5_LATER_REJECTION = V5_REJECTION_NAMESPACE / "rejection.json"
    SCHEMA = OUT / (BASE + "_schema_v5.json")
    CONTRACT = OUT / (BASE + "_contract_v5.json")
    PRODUCER = OUT / (BASE + "_v5.py")
    CONSUMER = OUT / (BASE + "_independent_verifier_assembler_authority_consumer_v5.py")
    TRANSITION = OUT / (BASE + "_v4_to_v5_static_launch_transition_receipt_v1.json")
    AUDIT = OUT / (BASE + "_static_audit_v5.json")
    MANIFEST = OUT / (BASE + "_cold_launch_manifest_v5.sha256")
    OUTER = OUT / (BASE + "_cold_launch_outer_receipt_v5.json")
    BASE7_PINS = {
        V4_REJECTION_SUPERSESSION:
            (V4_SUPERSESSION_FILE_PIN, V4_SUPERSESSION_OBJECT_PIN),
        SCHEMA: ("1048740103257351abb1266ed91bb80028436cb1497918a0f0f177ec3268ff4f", None),
        CONTRACT: ("162cd554ea972b434c019924a9ab8b87621ab65aae7d4896b6dac72d6288b997",
                   "27d457a583f4d9892e0a866917ea4add25ff677669ee814861e0608036189377"),
        PRODUCER: ("bc6d48903f61257cd75b20b83c6cd369ca3748d19427cf18129c0fb9bb59e76e", None),
        CONSUMER: ("03aed000a94fc7b7d7e68d3c55611bcc55e7ce293f65ddfb3be7709e44fc854d", None),
        TRANSITION: ("6784668e91a4a4cc0812c6405cb1df8c10ad90e40daf321ac75c16cadd5cd715",
                     "0678e3b81d5a4f6088967613df0cd585910b9b26800dfbf1724b637fc7c43526"),
        AUDIT: ("b852a41aaa992b85abec5f7139dc4669d2f1fe38bd79ab8ab812453c5cfca4f0",
                "ba9bf728ce08b795b5dd92191f2ccac3b2cfbc6ef7f7b919379feb6b9557e546"),
    }
    EXACT8 = (V4_REJECTION_SUPERSESSION, SCHEMA, CONTRACT, PRODUCER,
...
INNER_SCHEMA = "cm2.round306c79g.true-global-no-producer-consumer.v5.inner-composite"
COLD_ROOT_SCHEMA = (
    "cm2.round306c79g.true-global-no-producer-consumer.v5."
    "cold-launched-committed-authority"
)
COLD_ROOT_DOMAIN = b"CM2_C79G_V5_COLD_LAUNCHED_AUTHORITY_ROOT_V1\0"
...
LIVE_ACK_SCHEMA = (
    "cm2.round306c79g.true-global-no-producer-consumer.v5.cold-live-ack"
)
LIVE_RELEASE_SCHEMA = (
    "cm2.round306c79g.true-global-no-producer-consumer.v5.cold-live-release"
)
...
SOURCE_FD_ENV = "CM2_C79G_V5_COLD_SOURCE_FD"
EXEC_FD_ENV = "CM2_C79G_V5_COLD_EXEC_FD"
COORDINATION_PARENT_FD_ENV = "CM2_C79G_V5_COORDINATION_PARENT_FD"
WORKSPACE_ROOT_ENV = "CM2_C79G_V5_COLD_WORKSPACE_ROOT"
WORKSPACE_ROOT_FD_ENV = "CM2_C79G_V5_COLD_WORKSPACE_ROOT_FD"
LAUNCHER_SHA_ENV = "CM2_C79G_V5_COLD_LAUNCHER_FILE_SHA256"
...
def openat2_from_root_fd(root_fd: int, relative: bytes,
    ...
    need(relative and not relative.startswith(b"/") and
    ...
    def __init__(self, source: HeldFile) -> None:
        ...
        try:
            self.fd = os.memfd_create(
                "c79g-v5-child-exec", MFD_CLOEXEC | MFD_ALLOW_SEALING)
        except OSError as exc:
...
class HeldEmptyRejectionNamespace:
    ...
    def __init__(self, coordination: HeldCoordinationParent) -> None:
        coordination.verify()
        name = V5_REJECTION_NAMESPACE.name
        ...
        try:
            try:
                ...
                state = os.stat(name, dir_fd=coordination.fd,
                                follow_symlinks=False)
            self.path = V5_REJECTION_NAMESPACE
...
def reconstruct_unopened_publication_hashes(
    ...
    need(FINAL_BASE7_PINS_INSTALLED and len(BASE7_PINS) == 7 and
         EXACT8 == (V4_REJECTION_SUPERSESSION, SCHEMA, CONTRACT, PRODUCER,
                    CONSUMER, TRANSITION, AUDIT, SELF),
...
def construct_launcher_native_rejection(
    ...
    return close_object({
        ...
        "effective_checkpoint_object_sha256": CHECKPOINT,
        "namespace_exact_path": str(V5_REJECTION_NAMESPACE.relative_to(ROOT)),
        "target_exact_path": str(V5_LATER_REJECTION.relative_to(ROOT)),
        "rejection_reason": "ORPHANED_OR_INCOMPLETE_C79G_V5_SURFACE",
...
def install_or_replay_launcher_native_rejection(
    ...
    try:
        try:
            ...
            namespace_fd = openat2_beneath(
                V5_REJECTION_NAMESPACE, os.O_RDONLY | os.O_DIRECTORY)
    except BaseException:
        if namespace_fd >= 0:
    ...
    try:
        ...
        path_before = root_lstat(V5_REJECTION_NAMESPACE)
        ...
        target = os.stat(V5_LATER_REJECTION.name, dir_fd=namespace_fd,
                         follow_symlinks=False) if V5_LATER_REJECTION.name in universe else None
        if target is not None:
            need(universe == {V5_LATER_REJECTION.name},
                 "idempotent rejection exact singleton")
            rejection_guard = HeldFile(
                V5_LATER_REJECTION, "existing launcher-native v5 rejection")
        else:
            ...
            try:
                ...
                os.fsync(namespace_fd)
            rejection_guard = HeldFile(
                V5_LATER_REJECTION, "new launcher-native v5 rejection")
        need(rejection_guard is not None,
             "launcher-native rejection file held")
        value = strict_json(rejection_guard.raw, "launcher-native v5 rejection")
        need(isinstance(value, dict), "launcher-native rejection mapping")
        verify_object(value, "launcher-native v5 rejection")
        os.fsync(rejection_guard.fd)
        os.fsync(namespace_fd)
        os.fsync(coordination.fd)
        after = os.fstat(namespace_fd)
        path_after = root_lstat(V5_REJECTION_NAMESPACE)
        need(value == expected and rejection_guard.raw == expected_raw and
             set(os.listdir(namespace_fd)) == {V5_LATER_REJECTION.name} and
...
class HeldBundle:
    def __init__(self, expected_launcher_sha256: str,
        ...
        self.predecessor: HeldV3PredecessorExact10 | None = None
    ...
    def _initialize(self, expected_launcher_sha256: str,
                    ...
                    bootstrap: HeldBootstrapEntry) -> None:
        coordination.verify()
        need(SELF == OUT / (BASE + "_cold_launch_v5.py"),
             "exact cold launcher path")
        need(FINAL_BASE7_PINS_INSTALLED,
        ...
        for path in (V4_REJECTION_SUPERSESSION, CONTRACT, TRANSITION, AUDIT):
        ...
        need(self.base_objects[CONTRACT].get("status") ==
             ...
             self.base_objects[AUDIT].get("status") ==
                 "PASS_DUAL_STATIC_BYTES_GO_V5__PHYSICAL_COLD_FREEZE_PENDING__RUNTIME_NOT_AUTHORIZED",
        ...
        # The v3 official rejection is outside current exact8.  One live fd is
        # shared by the independent v3-exact10 and rejected-v4-draft7 proofs.
        ...
        verify_object(rejected_value, "official v3 later rejection object",
                      V3_REJECTION_OBJECT_PIN)
        self.predecessor = HeldV3PredecessorExact10(
            coordination, self.v3_rejection)
        self.rejected_v4 = HeldV4RejectedDraft7(
            coordination, self.by_path[V4_REJECTION_SUPERSESSION],
            self.v3_rejection)
        predecessor_identities = {
            item.identity for item in self.predecessor.files
        }
        ...
        need(len(predecessor_identities) == 10 and
             len(rejected_v4_identities) == 7 and
             predecessor_identities.isdisjoint(current_identities) and
             rejected_v4_identities.isdisjoint(current_identities) and
             predecessor_identities.isdisjoint(rejected_v4_identities) and
             self.v3_rejection.identity not in predecessor_identities |
                 rejected_v4_identities | current_identities and
             len({item.mount_id for item in self.predecessor.files} |
             ...
             "current exact10, v3 exact10, v3 rejection and v4 draft7 are 28 unique live files on one mount")

    def terminal_replay(self) -> None:
        need(self.predecessor is not None and self.rejected_v4 is not None and
        ...
        self.predecessor.terminal_replay(self.v3_rejection)
...
def schema_keyword_universe(root: Mapping[str, Any]) -> frozenset[str]:
    ...
    def walk(node: Any, label: str) -> None:
        ...
        for name, child in definitions.items():
            need(isinstance(name, str) and name, label + ":$defs name")
            walk(child, label + ".$defs." + name)
        properties = node.get("properties", {})
        need(isinstance(properties, dict), label + ":properties object")
        for name, child in properties.items():
            need(isinstance(name, str) and name, label + ":property name")
...
def cold_root(bundle: HeldBundle, coordination: HeldCoordinationParent,
    ...
    proof = {
        ...
        "current_exact8_first_member_is_v4_rejection_supersession":
            EXACT8[0] == V4_REJECTION_SUPERSESSION,
        "v4_rejection_supersession_identity": {
            **bundle.by_path[V4_REJECTION_SUPERSESSION].identity_object(),
        ...
        **bundle.chronology,
        ...
        "reject_command_exempts_only_fresh_empty_v5_rejection_namespace": True,
...
def _validate_live_ack(
    ...
    need(set(ack) == expected_keys and ack.get("schema") == LIVE_ACK_SCHEMA and
         ...
         ack.get("static_v5_exact10_with_v4_supersession_v3_exact10_and_official_rejection_namespace_terminal_replay_completed_by_launcher_before_request") is True and
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "reject":
            ...
            return 0
        # Every positive-capable command holds the exact-empty v5 rejection
