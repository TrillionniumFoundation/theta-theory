...
CERTIFICATE_SCHEMA = "cm2.round158.upper-endpoint-continuation-to-320h.v1"
SCHEMA = "cm2.round158.upper-endpoint-continuation-to-320h.verification.v1"
STATUS = "CERTIFIED_TYPED_UPPER_ENDPOINT_CONTINUATION_TO_320H__D02_STILL_BLOCKED"
PINS = {
    ...
    "cm2_round158_upper_endpoint_continuation_to_320h.py": "8731fac2ae5f89bd15736b379969f83ff3bc8e2358353acc643ab07456c763c9",
    "cm2-round157-upper-endpoint-continuation-to-288h-2026-07-25.json": "3c25d0f3fc813c8e26760c54cee858a463dcafa65721129c3a58d5463af28351",
    "cm2-round157-upper-endpoint-continuation-to-288h-verification-2026-07-25.json": "724e35ea187096be6dee38639e1259a90c14924c7ebc9524bfdfbc6bf4150851",
}
SLABS = [
    ("upper-36", "288", "292", "377", "429", "800", "826"),
    ("upper-37", "292", "296", "388", "440", "811", "837"),
    ("upper-38", "296", "300", "399", "451", "822", "848"),
    ("upper-39", "300", "304", "410", "462", "833", "859"),
    ("upper-40", "304", "308", "421", "473", "844", "870"),
    ("upper-41", "308", "312", "432", "484", "855", "881"),
    ("upper-42", "312", "316", "443", "495", "866", "892"),
    ("upper-43", "316", "320", "454", "506", "877", "903"),
]
...
def validate(result, rows, prior):
    require(
        ...
        == {
            ...
            "inherited_round157_result_sha256",
    ...
    require(
        result["inherited_round157_result_sha256"] == digest(prior),
        "inherited result",
    )
    require(
        result["certified_abs_delta_x_corridor_in_h_units"] == ["0", "320"],
        "corridor",
    )
    ...
    require(
        ...
        == {
            ...
            "new_rows",
            "new_rows_sha256",
            "exact_face_adjacencies_from_288h_through_320h",
    ...
    require(
        same(
            continuation["exact_face_adjacencies_from_288h_through_320h"],
    ...
    require(
        ...
        == (
            "continue beyond 320h with 4h C24-controlled slabs until terminal "
...
def attacks(result, rows, prior):
    ...
    for label in labels:
        ...
        if label == "delete_row":
            ...
            candidate["inherited_round157_result_sha256"] = "0" * 64
            ...
            candidate["certified_abs_delta_x_corridor_in_h_units"][1] = "321"
...
def document_attacks(document, rows, prior):
    ...
    for label in labels:
        ...
        if label == "wrapper_wrong_schema":
            candidate["schema"] = "cm2.round158.invalid"
...
def main():
    ...
    require(
        ...
        == "cm2.round157.upper-endpoint-continuation-to-288h.v1"
        and prior_document["result_sha256"] == digest(prior_document["result"]),
        "prior certificate",
    )
    require(
        prior_verification["schema"]
        == "cm2.round157.upper-endpoint-continuation-to-288h.verification.v1"
    ...
    for raw in strict_attacks:
        try:
            ...
            strict_rejected += 1
    require(strict_rejected == 9, "strict attacks")
    result = {
        "status": "PASS",
        "certificate_result_sha256": certificate["result_sha256"],
        "producer_source_sha256": PINS[
            "cm2_round158_upper_endpoint_continuation_to_320h.py"
        ],
        ...
        "combined_typed_box_count_recomputed": 195,
        "upper_endpoint_reached_in_h_units": "320",
