#!/usr/bin/env python3
"""Materialize the Round270 outgoing tails and single-active-factor WALL leaves."""
...
def main():
 ap=argparse.ArgumentParser();ap.add_argument("--seed",type=int,default=271071);ap.add_argument("--processes",type=int,default=40);ap.add_argument("--no-write",action="store_true");a=ap.parse_args();need(1<=a.processes<=64,"processes");psha=hashlib.sha256(raw(Path(__file__).resolve(),5_000_000)).hexdigest();result=build(psha,a.processes);doc={"schema":SCHEMA,"result":result,"result_sha256":digest(result)};b=canonical(doc)+b"\n";
