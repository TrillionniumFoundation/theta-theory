#!/usr/bin/env python3
"""Independent fail-closed verifier for the Round204 formal package.

The candidate producer is never imported or executed.  This verifier pins
the complete seven-entry Round182 package and independently reconstructs the
full canonical Round204 result from that frozen chain.  It then requires the
candidate certificate to match the independently reconstructed wrapper
byte-for-byte, exercises re-signed semantic attacks across every ledger, and
tests strict JSON plus path/type/output-alias/temporary-file guards.
"""
...
EXPECTED_CERTIFICATE_SIZE = 7_157_575
...
ROUND204_REPORT = f"{ROUND204_PREFIX}_report.md"
ROUND204_COLD = f"{ROUND204_PREFIX}_cold_replay.md"
ROUND204_MANIFEST = f"{ROUND204_PREFIX}_manifest.sha256"
...
EXPECTED_1D_STRATUM_COUNT = 1_024
EXPECTED_0D_STRATUM_COUNT = 580
EXPECTED_EXACT_INTERSECTION_SEGMENT_COUNT = 16
EXPECTED_EXACT_INTERSECTION_ENDPOINT_COUNT = 20
...
def build_result(producer_sha256: str) -> dict[str, Any]:
    ...
    audit_reconstructed_result(result)
    return result
...
def audit_zero_global_credits(value: Any) -> int:
    """Require every whole/global credit field, at every depth, to stay zero."""
    count = 0
    if type(value) is dict:
        for key, child in value.items():
            if key in {
                ...
                require(
                    ...
                    f"strict zero credit:{key}",
                )
                count += 1
            count += audit_zero_global_credits(child)
    elif type(value) is list:
        for child in value:
            count += audit_zero_global_credits(child)
    return count
...
def audit_reconstructed_result(result: dict[str, Any]) -> dict[str, int]:
    ...
    require(
        scope["Round182_origin_count"] == EXPECTED_ORIGIN_COUNT
        and scope["Round182_leaf_count"] == EXPECTED_LEAF_COUNT
        and scope["strict_open_3D_region_count"]
        == EXPECTED_OPEN_REGION_COUNT
        and scope["tail_open_3D_region_count"]
        == EXPECTED_TAIL_REGION_COUNT
        and scope["distinct_exact_key_join_count"]
        == len(EXPECTED_KEY_ORDINALS)
        and scope["all_64_origins_fully_locally_signature_replaced"] is True
        and scope["all_signature_rows_formally_materialized"] is True
        and scope["local_replacement_only"] is True,
        "formal scope census",
    )

    anchor = result["empty_word_anchor_evidence_ledger"]
    parent_anchor_ids = audit_closed_rows(
        anchor["parent_rows"],
        id_key="parent_id",
        label="anchor parent",
    )
    origin_anchor_ids = audit_closed_rows(
        anchor["origin_rows"],
        id_key="origin_row_id",
        label="anchor origin",
    )
    require(
        len(parent_anchor_ids) == EXPECTED_PARENT_COUNT
        and len(origin_anchor_ids) == EXPECTED_ORIGIN_COUNT
        and anchor["parent_row_count"] == EXPECTED_PARENT_COUNT
        and anchor["origin_row_count"] == EXPECTED_ORIGIN_COUNT
        and anchor["parent_rows_sha256"] == digest(anchor["parent_rows"])
        and anchor["origin_rows_sha256"] == digest(anchor["origin_rows"]),
        "anchor ledgers",
    )

    obstruction = result["single_obstruction_evidence_ledger"]
    obstruction_ids = audit_closed_rows(
        obstruction["rows"],
        id_key="leaf_row_id",
        label="single obstruction",
    )
    require(
        len(obstruction_ids) == EXPECTED_LEAF_COUNT
        and obstruction["row_count"] == EXPECTED_LEAF_COUNT
        and obstruction["rows_sha256"] == digest(obstruction["rows"])
        and obstruction[
            "all_512_have_exactly_one_recorded_wall_reason"
        ] is True
        and obstruction["all_nonwall_signature_fields_strict"] is True,
        "single obstruction ledger",
    )

    tail = result["tail_geometry_evidence_ledger"]
    tail_leaf_ids = audit_closed_rows(
        tail["leaf_rows"],
        id_key="leaf_row_id",
        label="tail leaf geometry",
    )
    tail_pair_ids = audit_closed_rows(
        tail["pair_rows"],
        id_key="tail_pair_row_id",
        label="tail pair geometry",
    )
    require(
        len(tail_leaf_ids) == EXPECTED_TAIL_LEAF_COUNT
        and len(tail_pair_ids) == EXPECTED_TAIL_PAIR_COUNT
        and tail["leaf_row_count"] == EXPECTED_TAIL_LEAF_COUNT
        and tail["pair_row_count"] == EXPECTED_TAIL_PAIR_COUNT
        and tail["leaf_rows_sha256"] == digest(tail["leaf_rows"])
        and tail["pair_rows_sha256"] == digest(tail["pair_rows"]),
        "tail geometry ledgers",
    )
    ...
    region_ids = audit_closed_rows(
        ...
        label="strict open 3D region",
    )
    tail_regions = [row for row in regions if row["tail_region"]]
    require(
        len(region_ids) == EXPECTED_OPEN_REGION_COUNT
        and len(tail_regions) == EXPECTED_TAIL_REGION_COUNT
        and region_ledger["row_count"] == EXPECTED_OPEN_REGION_COUNT
        and region_ledger["tail_row_count"] == EXPECTED_TAIL_REGION_COUNT
        and region_ledger["rows_sha256"] == digest(regions)
        and region_ledger["tail_rows_sha256"] == digest(tail_regions)
        and region_ledger["unique_signature_count"]
        == EXPECTED_OPEN_REGION_COUNT
        and region_ledger["conflicting_signature_count"] == 0
        and region_ledger["missing_signature_count"] == 0
        and all(
            row["ambient_dimension"] == 3
            and row["strict_open_region"] is True
            and row["positive_coordinate_volume"] is True
            and row["key_binding_join_cardinality"] == 1
            and row["formal_local_signature_credit"] == 1
            for row in regions
        ),
        "strict open 3D region ledger",
    )

    sheet_ledger = result["formal_2D_sheet_lineage"]
    source_sheets = sheet_ledger["source_sheet_rows"]
    target_sheets = sheet_ledger["target_sheet_rows"]
    source_sheet_ids = audit_closed_rows(
        source_sheets,
        id_key="sheet_row_id",
        label="source 2D sheet",
    )
    target_sheet_ids = audit_closed_rows(
        target_sheets,
        id_key="sheet_row_id",
        label="target 2D sheet",
    )
    sheet_ids = source_sheet_ids | target_sheet_ids
    require(
        source_sheet_ids.isdisjoint(target_sheet_ids)
        and len(source_sheet_ids) == EXPECTED_SOURCE_SHEET_CELL_COUNT
        and len(target_sheet_ids) == EXPECTED_TARGET_SHEET_CELL_COUNT
        and sheet_ledger["source_sheet_row_count"]
        == EXPECTED_SOURCE_SHEET_CELL_COUNT
        and sheet_ledger["target_sheet_row_count"]
        == EXPECTED_TARGET_SHEET_CELL_COUNT
        and sheet_ledger["source_sheet_rows_sha256"]
        == digest(source_sheets)
        and sheet_ledger["target_sheet_rows_sha256"]
        == digest(target_sheets)
        and all(
            row["ambient_dimension"] == 2
            and row["local_dimension_lineage_materialized"] is True
            and row["lower_dimensional_credit"] == 0
            and row["half_open_owner_region_row_id"] in region_ids
            and set(row["incident_open_region_row_ids"]) <= region_ids
            for row in source_sheets
        )
        and all(
            row["ambient_dimension"] == 2
            ...
            and row["lower_dimensional_credit"] == 0
            and row["half_open_owner_region_row_id"] in region_ids
            and row["half_open_shadow_region_row_id"] in region_ids
            for row in target_sheets
        ),
        "complete 2D lineage",
    )
    ...
    edges = edge_ledger["rows"]
    edge_ids = audit_closed_rows(
        edges,
        id_key="edge_row_id",
        label="1D stratum",
    )
    require(
        len(edge_ids) == EXPECTED_1D_STRATUM_COUNT
        and edge_ledger["row_count"] == EXPECTED_1D_STRATUM_COUNT
        and edge_ledger["rows_sha256"] == digest(edges)
        and edge_ledger["exact_source_target_intersection_segment_count"]
        == EXPECTED_EXACT_INTERSECTION_SEGMENT_COUNT
        and edge_ledger["exact_source_target_intersection_rows_sha256"]
        == digest([
            row for row in edges
            if row["exact_source_target_intersection"]
        ])
        and all(
            row["ambient_dimension"] == 1
            and row["local_dimension_lineage_materialized"] is True
            and row["lower_dimensional_credit"] == 0
            and len(row["endpoint_0D_row_ids"]) == 2
            and row["half_open_owner_region_row_id"] in region_ids
            and row["half_open_owner_sheet_row_id"] in sheet_ids
            and set(row["incident_sheet_row_ids"]) <= sheet_ids
            for row in edges
        ),
        "complete 1D lineage",
    )

    point_ledger = result["formal_0D_endpoint_and_corner_lineage"]
    points = point_ledger["rows"]
    point_ids = audit_closed_rows(
        points,
        id_key="point_row_id",
        label="0D stratum",
    )
    require(
        len(point_ids) == EXPECTED_0D_STRATUM_COUNT
        and point_ledger["row_count"] == EXPECTED_0D_STRATUM_COUNT
        and point_ledger["rows_sha256"] == digest(points)
        and point_ledger["exact_source_target_endpoint_row_count"]
        == EXPECTED_EXACT_INTERSECTION_ENDPOINT_COUNT
        and point_ledger["exact_source_target_endpoint_rows_sha256"]
        == digest([
            row for row in points
            if row["exact_source_target_intersection_endpoint"]
        ])
        and all(
            row["ambient_dimension"] == 0
            and row["local_dimension_lineage_materialized"] is True
            and row["lower_dimensional_credit"] == 0
            and row["half_open_owner_region_row_id"] in region_ids
            and row["half_open_owner_1D_row_id"] in edge_ids
            and set(row["incident_1D_row_ids"]) <= edge_ids
            and set(row["incident_2D_sheet_row_ids"]) <= sheet_ids
            for row in points
        )
        and all(
            set(row["endpoint_0D_row_ids"]) <= point_ids
            for row in edges
        )
        and all(
            set(row["boundary_0D_row_ids"]) <= point_ids
            and set(row["boundary_1D_row_ids"]) <= edge_ids
            and set(row["source_target_intersection_1D_row_ids"])
            <= edge_ids
            for row in [*source_sheets, *target_sheets]
        ),
        "complete 0D lineage and cross-incidence",
    )

    glue_ledger = result["tail_pair_signature_glue_ledger"]
    glues = glue_ledger["rows"]
    glue_ids = audit_closed_rows(
        glues,
        id_key="pair_glue_row_id",
        label="tail signature glue",
    )
    require(
        len(glue_ids) == EXPECTED_TAIL_PAIR_COUNT
        and glue_ledger["row_count"] == EXPECTED_TAIL_PAIR_COUNT
        and glue_ledger["rows_sha256"] == digest(glues)
        and glue_ledger["empty_event_empty_pair_count"]
        == EXPECTED_TAIL_PAIR_COUNT
        and glue_ledger["unexpected_signature_change_count"] == 0
        and all(
            row["tail_pair_row_id"] in tail_pair_ids
            and row["source_sheet_2D_row_id"] in source_sheet_ids
            and row["target_sheet_2D_row_id"] in target_sheet_ids
            and row["source_target_intersection_1D_row_id"] in edge_ids
            and row["source_adjacent_crossing_region_row_id"]
            in region_ids
            and set(row["outer_empty_region_row_ids"]) <= region_ids
            and row["unexpected_signature_change_count"] == 0
            for row in glues
        ),
        "tail signature glue ledger",
    )

    origin_ledger = result["origin_local_completion_ledger"]
    origin_rows = origin_ledger["rows"]
    ...
    require(
        len(origin_ids) == EXPECTED_ORIGIN_COUNT
        and origin_ledger["row_count"] == EXPECTED_ORIGIN_COUNT
        and origin_ledger["rows_sha256"] == digest(origin_rows)
        and origin_ledger["fully_locally_signature_replaced_origin_count"]
        == EXPECTED_ORIGIN_COUNT
        and origin_ledger["formal_local_replacement_credit"]
        == EXPECTED_ORIGIN_COUNT
        and set(
            region_id
            for row in origin_rows
            for region_id in row["strict_open_3D_region_row_ids"]
        ) == region_ids
        and set(
            sheet_id
            for row in origin_rows
            for sheet_id in row["incident_2D_sheet_row_ids"]
        ) == sheet_ids
        and set(
            edge_id
            for row in origin_rows
            for edge_id in row["incident_1D_stratum_row_ids"]
        ) == edge_ids
        and set(
            point_id
            for row in origin_rows
            for point_id in row["incident_0D_stratum_row_ids"]
        ) == point_ids,
        "64 origin completion ledger",
    )

    join_ledger = result["exact_key_local_join_ledger"]
    joins = join_ledger["rows"]
    ...
    require(
        len(join_ids) == len(EXPECTED_KEY_ORDINALS)
        and join_ledger["row_count"] == len(EXPECTED_KEY_ORDINALS)
        and join_ledger["rows_sha256"] == digest(joins)
        and join_ledger["official_key_ordinals"] == EXPECTED_KEY_ORDINALS
        and len(joined_region_ids) == EXPECTED_OPEN_REGION_COUNT
        and set(joined_region_ids) == region_ids
        and len(set(joined_region_ids)) == EXPECTED_OPEN_REGION_COUNT
        and all(
            row["formal_local_join_materialized"] is True
            and row["join_cardinality_per_local_region"] == 1
            and row["global_exact_key_fibre_exhausted"] is False
            for row in joins
        ),
        "12 exact-key local joins",
    )
    ...
    require(
        conservation["Round182_relevant_leaf_count"] == EXPECTED_LEAF_COUNT
        and conservation["Round182_tail_leaf_count"]
        == EXPECTED_TAIL_LEAF_COUNT
        and conservation["Round204_locally_replaced_leaf_count"]
        == EXPECTED_LEAF_COUNT
        and conservation["strict_positive_open_3D_region_count"]
        == EXPECTED_OPEN_REGION_COUNT
        and conservation["integer_leaf_delta"] == 0
        and conservation["exact_coordinate_volume_delta"] == "0"
        and conservation[
            "all_2D_1D_0D_strata_have_zero_ambient_3D_volume"
        ] is True,
        "exact dimension and positive-volume conservation",
    )
    nonpromotion = result[
        "formal_local_replacement_and_strict_nonpromotion"
    ]
    require(
        nonpromotion["materialized_local_signature_region_count"]
        == EXPECTED_OPEN_REGION_COUNT
        and nonpromotion["materialized_local_origin_completion_count"]
        == EXPECTED_ORIGIN_COUNT
    ...
    require(
        ...
        and result["provenance"]["Round182_files_modified"] is False,
        "provenance",
    )
    zero_credit_fields = audit_zero_global_credits(result)
    require(zero_credit_fields > 0, "zero-credit field census nonempty")
    return {
        "origin_count": len(origin_ids),
        "leaf_count": len(obstruction_ids),
        "strict_open_3D_region_count": len(region_ids),
        "tail_open_3D_region_count": len(tail_regions),
        "source_2D_sheet_count": len(source_sheet_ids),
        "target_2D_sheet_count": len(target_sheet_ids),
        "one_D_stratum_count": len(edge_ids),
        "zero_D_stratum_count": len(point_ids),
        "tail_signature_glue_count": len(glue_ids),
        "exact_key_local_join_count": len(join_ids),
        "whole_and_global_zero_credit_field_occurrence_count":
            zero_credit_fields,
    }
...
def static_inert_boundary_audit() -> dict[str, Any]:
    ...
    require(
        ...
        "forbidden Round204/probe import",
    )
    dangerous_calls = {
        node.func.id
    ...
    require(PRODUCER.stem not in sys.modules, "producer module not loaded")
...
def resign_closed_rows(rows: list[dict[str, Any]]) -> None:
    for row in rows:
        payload = {
            key: value
            for key, value in row.items()
...
def resign_internal_ledgers(result: dict[str, Any]) -> None:
    anchor = result["empty_word_anchor_evidence_ledger"]
    resign_closed_rows(anchor["parent_rows"])
    resign_closed_rows(anchor["origin_rows"])
    anchor["parent_rows_sha256"] = digest(anchor["parent_rows"])
    anchor["origin_rows_sha256"] = digest(anchor["origin_rows"])

    obstruction = result["single_obstruction_evidence_ledger"]
    resign_closed_rows(obstruction["rows"])
    obstruction["rows_sha256"] = digest(obstruction["rows"])

    tail = result["tail_geometry_evidence_ledger"]
    resign_closed_rows(tail["leaf_rows"])
    resign_closed_rows(tail["pair_rows"])
    tail["leaf_rows_sha256"] = digest(tail["leaf_rows"])
    tail["pair_rows_sha256"] = digest(tail["pair_rows"])

    regions = result["formal_local_open_3D_region_ledger"]
    resign_closed_rows(regions["rows"])
    regions["rows_sha256"] = digest(regions["rows"])
    regions["tail_rows_sha256"] = digest([
        row for row in regions["rows"] if row["tail_region"]
    ])

    sheets = result["formal_2D_sheet_lineage"]
    resign_closed_rows(sheets["source_sheet_rows"])
    resign_closed_rows(sheets["target_sheet_rows"])
    sheets["source_sheet_rows_sha256"] = digest(
        sheets["source_sheet_rows"]
    )
    sheets["target_sheet_rows_sha256"] = digest(
        sheets["target_sheet_rows"]
    )

    edges = result["formal_1D_boundary_and_intersection_lineage"]
    resign_closed_rows(edges["rows"])
    edges["rows_sha256"] = digest(edges["rows"])
    edges["exact_source_target_intersection_rows_sha256"] = digest([
        row for row in edges["rows"]
        if row["exact_source_target_intersection"]
    ])

    points = result["formal_0D_endpoint_and_corner_lineage"]
    resign_closed_rows(points["rows"])
    points["rows_sha256"] = digest(points["rows"])
    points["exact_source_target_endpoint_rows_sha256"] = digest([
    ...
    for name in (
        ...
        ledger = result[name]
        resign_closed_rows(ledger["rows"])
...
def semantic_attack_suite(
    ...
    attacks: list[tuple[str, tuple[Any, ...], Any]] = [
        ("status promotion", ("result", "status"), "PROMOTED"),
        (
            "Round182 binding modification",
            ("result", "Round182_binding", "formal_files_modified"),
            True,
        ),
        (
            "formal scope region count",
            ("result", "formal_scope_contract",
             "strict_open_3D_region_count"),
            EXPECTED_OPEN_REGION_COUNT - 1,
        ),
        (
            "empty anchor semantics",
            ("result", "empty_word_anchor_evidence_ledger",
             "parent_rows", 0, "formal_anchor_binding"),
            False,
        ),
        (
            "single obstruction semantics",
            ("result", "single_obstruction_evidence_ledger",
             "rows", 0, "only_signature_failure"),
            "FABRICATED_NONWALL_FAILURE",
        ),
        (
            "tail leaf geometry",
            ("result", "tail_geometry_evidence_ledger",
             "leaf_rows", 0, "target_zero_classification"),
            "FABRICATED_ZERO_CLASS",
        ),
        (
            "tail pair geometry",
            ("result", "tail_geometry_evidence_ledger",
             "pair_rows", 0, "owner_count"),
            2,
        ),
        (
            "3D local signature credit",
            ("result", "formal_local_open_3D_region_ledger",
             "rows", 0, "formal_local_signature_credit"),
            0,
        ),
        (
            "source 2D half-open owner",
            ("result", "formal_2D_sheet_lineage",
             "source_sheet_rows", 0, "half_open_owner_policy"),
            "FABRICATED_OWNER",
        ),
        (
            "target 2D half-open owner",
            ("result", "formal_2D_sheet_lineage",
             "target_sheet_rows", 0, "half_open_owner_policy"),
            "FABRICATED_OWNER",
        ),
        (
            "1D endpoint lineage",
            ("result", "formal_1D_boundary_and_intersection_lineage",
             "rows", 0, "endpoint_0D_row_ids"),
            [],
        ),
        (
            "0D edge lineage",
            ("result", "formal_0D_endpoint_and_corner_lineage",
             "rows", 0, "incident_1D_row_ids"),
            [],
        ),
        (
            "tail signature glue",
            ("result", "tail_pair_signature_glue_ledger",
             "rows", 0, "unexpected_signature_change_count"),
            1,
        ),
        (
            "origin whole-tube credit",
            ("result", "origin_local_completion_ledger",
             "rows", 0, "whole_original_tube_credit"),
            1,
        ),
        (
            "exact-key global disposition",
            ("result", "exact_key_local_join_ledger",
             "rows", 0, "global_exact_key_disposition_credit"),
            1,
        ),
        (
            "positive-volume conservation",
            ("result", "exact_dimension_and_positive_volume_conservation",
        ...
        (
            "D02 promotion",
            ("result",
             "formal_local_replacement_and_strict_nonpromotion", "D02"),
            "OPEN",
        ),
        (
            "probe dependency",
            ("result", "probe_nondependency",
             "formal_result_rebuilt_from_pinned_Round182_chain"),
            False,
        ),
        (
            "producer provenance",
        ...
        (
            ...
            "declare the global exact-key fibres complete",
        ),
    ]
    rejected: list[str] = []
    for name, path, replacement in attacks:
        ...
        if original == replacement:
            if type(original) is bool:
                set_path(certificate, path, not original)
            elif type(original) is int:
                set_path(certificate, path, original + 1)
            elif type(original) is str:
                set_path(certificate, path, original + "__MUTATED")
            else:
                raise Round204Error(f"attack collision:{name}")
        try:
            resign_internal_ledgers(certificate["result"])
            ...
            try:
                ...
                raise Round204Error(f"semantic attack accepted:{name}")
        finally:
...
def expect_rejection(
    ...
    try:
        ...
        return name
    raise Round204Error(f"attack accepted:{name}")
...
def strict_json_attack_suite() -> dict[str, Any]:
    ...
    oversize = HERE / (
        f".cm2_round204_{os.getpid()}_oversize_JSON_input.json"
    )
    try:
...
def protected_output_paths() -> set[Path]:
    protected = {
        ...
        PRODUCER.resolve(),
        CERTIFICATE.resolve(),
        (HERE / ROUND204_REPORT).resolve(),
        (HERE / ROUND204_COLD).resolve(),
        (HERE / ROUND204_MANIFEST).resolve(),
...
def path_type_alias_and_temp_attack_suite() -> dict[str, Any]:
    ...
    scratch = Path(tempfile.mkdtemp(
        prefix=".cm2_round204_verifier_attack.",
        dir=HERE,
    ))
    ...
    input_symlink = HERE / f".cm2_round204_{pid}_input_symlink"
    input_hardlink = HERE / f".cm2_round204_{pid}_input_hardlink"
    input_fifo = HERE / f".cm2_round204_{pid}_input_fifo"
    output_symlink = (
        HERE / f".cm2_round204_{pid}_symlink_verification.json"
    )
    output_hard_base = (
        HERE / f".cm2_round204_{pid}_base_verification.json"
    )
    output_hardlink = (
        HERE / f".cm2_round204_{pid}_hard_verification.json"
    )
    output_fifo = (
        HERE / f".cm2_round204_{pid}_fifo_verification.json"
    )
    outside = (
        HERE.parent / f".cm2_round204_{pid}_escape_verification.json"
    )
    parent_alias = HERE / f".cm2_round204_parent_alias_{pid}"
    safe_target = (
        HERE / f".cm2_round204_{pid}_safe_verification.json"
    )
    predictable_temp = (
    ...
    try:
        ...
        rejected.extend([
            ...
            expect_rejection(
                ...
                lambda: same_directory_regular_bytes(
                    HERE.parent / "round204-outside-input", 64
                ),
            ),
        ])
        ...
        rejected.extend([
            ...
            expect_rejection(
                ...
                lambda: safe_write(
                    scratch
                    / f".cm2_round204_{pid}_nested_verification.json",
                    b"x",
                ),
            ),
            expect_rejection(
                ...
                lambda: safe_write(
                    parent_alias
                    / f".cm2_round204_{pid}_alias_verification.json",
                    b"x",
                ),
            ),
            expect_rejection(
                ...
                lambda: safe_write(
                    HERE / f"cm2_round204_{pid}_unlisted.json", b"x"
                ),
            ),
            expect_rejection(
                "certificate-shaped hidden output",
                lambda: safe_write(
                    HERE / f".cm2_round204_{pid}_certificate.json",
                    b"x",
                ),
            ),
            expect_rejection(
                ...
                lambda: safe_write(
                    HERE
                    / f".cm2_round204_{pid}_verification.json.tmp",
                    b"x",
                ),
            ),
            expect_rejection(
    ...
    require(len(rejected) == 21, "path/type/alias/temp attack census")
    return {
        "attack_count": len(rejected),
...
def load_candidate_certificate() -> tuple[dict[str, Any], bytes]:
    raw = same_directory_regular_bytes(CERTIFICATE, 20_000_000)
    require(
        len(raw) == EXPECTED_CERTIFICATE_SIZE
        and sha256_bytes(raw) == EXPECTED_CERTIFICATE_SHA256,
...
def main() -> int:
    ...
    expected_result = build_result(EXPECTED_PRODUCER_SHA256)
    ...
    require(
        ...
        and sha256_bytes(expected_raw) == EXPECTED_CERTIFICATE_SHA256,
    ...
    require(PRODUCER.stem not in sys.modules, "producer never imported")
    pin_round182()

    verification_result = {
        "status": "PASS_PARTIAL_FORMAL_ROUND204",
        "verdict": "PASS",
        ...
        "complete_expected_result_rebuilt_from_Round182": True,
        "full_expected_python_object_equality": True,
        "full_expected_canonical_byte_equality": True,
        "verified_census": census,
        "verified_exact_intersection_census": {
            "one_D_segment_count":
                EXPECTED_EXACT_INTERSECTION_SEGMENT_COUNT,
            "zero_D_endpoint_count":
                EXPECTED_EXACT_INTERSECTION_ENDPOINT_COUNT,
            "source_sheet_boundary_incidence_count": 32,
            "target_sheet_boundary_incidence_count": 32,
        },
        "strict_nonpromotion": {
            "whole_original_tube_credit": 0,
            "global_exact_key_disposition_credit": 0,
        ...
        "independence_contract": {
            **boundary,
            "imports_only_pinned_Round182_independent_verifier_chain":
                True,
        ...
        "output_safety": {
            ...
            "unpredictable_same_directory_O_EXCL_temporary": True,
            "file_fsync_before_replace": True,
            "parent_directory_fsync_after_replace": True,
        },
    }
