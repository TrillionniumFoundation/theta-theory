...
def terminal(args: argparse.Namespace) -> tuple[dict[str, Any], list[Capture],
    ...
    try:
        ...
        if kind == "C28":
            ...
            need(receipt.get("C27R2_terminal_root_manifest_sha256") == C27_ROOT
                 and values and all(value == C27_ROOT for value in values),
        ...
        if kind == "C27R2":
            ...
            # The minted C28 authority binds C27R2 at the terminal layer.  Its
            # producer result is a zero-credit mathematical payload and does
            # not independently carry the C27R2 terminal root.  Requiring that
            # payload field would reject the correctly sealed authority after
            # the receipt/replay/chain/root closure above has already proved
            # the exact C27R2 predecessor.
            need(receipt.get("C27R2_terminal_root_manifest_sha256") == C27_ROOT,
                 "C28 terminal binds original C27R2 terminal")
