#!/usr/bin/env python3
"""Direct-Python ExecStartPre publisher for the C30c bridge v4.

The implementation is loaded from the exact watcher-v4 inode and held for the
...
SELF = WORKSPACE / "deliverables/cm2_round306c30c_postreceipt_v3_p1_bridge_start_gate_v1.py"
WATCHER = WORKSPACE / "deliverables/cm2_round306c30c_postreceipt_v3_p1_bridge_watch_v4.py"
...
def read_held(fd: int, maximum: int = 8 << 20) -> bytes:
    ...
    if not (stat.S_ISREG(before.st_mode) and before.st_nlink == 1
        ...
        raise GateLoaderError("bounded singleton watcher-v4 source")
    ...
    while remaining:
        ...
        if not block:
            raise GateLoaderError("complete watcher-v4 source read")
        chunks.append(block); remaining -= len(block)
    if os.read(fd, 1) != b"" or identity(before) != identity(os.fstat(fd)):
        raise GateLoaderError("stable watcher-v4 source read")
...
def load_held_watcher(
    ...
    if expected_sha256 is not None and re.fullmatch(
        ...
        raise GateLoaderError("approved watcher-v4 source SHA")
    ...
    if not (stat.S_ISREG(before.st_mode) and before.st_nlink == 1
        ...
        raise GateLoaderError(
            "raw watcher-v4 frozen owned bounded regular singleton",
        )
    fd = os.open(
    ...
    try:
        ...
        if identity(before) != identity(opened):
            raise GateLoaderError("watcher-v4 loader open race")
        raw = read_held(fd)
        digest = hashlib.sha256(raw).hexdigest()
        if expected_sha256 is not None and digest != expected_sha256:
            raise GateLoaderError("watcher-v4 source SHA before compile")
        module = types.ModuleType("_cm2_bridge_watch_v4_held")
...
def verify_loaded(fd: int, state: tuple[int, ...], digest: str) -> None:
    if identity(WATCHER.lstat()) != state or identity(os.fstat(fd)) != state:
        raise GateLoaderError("held watcher-v4 path/inode drift")
    if hashlib.sha256(read_held(fd)).hexdigest() != digest:
        raise GateLoaderError("held watcher-v4 byte drift")
    ...
    try:
        ...
        if module is not None:
            failure = module.closed({
                "schema": "cm2.round306c30c.postreceipt-bridge-v4-start-gate-failure.v1",
