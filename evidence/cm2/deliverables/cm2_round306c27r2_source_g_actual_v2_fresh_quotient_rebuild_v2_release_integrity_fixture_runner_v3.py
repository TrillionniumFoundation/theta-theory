#!/usr/bin/env python3
"""Run the exact 64 real C27R2 release-boundary integrity fixtures.
...
It then executes 64 isolated validator subprocesses.  Each negative case has
...
VALIDATOR_DEFAULT = ROOT / "deliverables/cm2_round306c27r2_source_g_actual_v2_fresh_quotient_rebuild_v2_release_repair_boundary_independent_validator_v3.py"
PREFIX = "cm2.round306c27r2.source-g-actual-v2-fresh-quotient-rebuild.v2."
OVERRIDE_SCHEMA = PREFIX + "release-repair-boundary-override-map.v3"
RESULT_SCHEMA = PREFIX + "release-integrity-fixture.v3"
RESULT_STATUS = (
    "PASS_C27R2_RELEASE_BOUNDARY_CONTROL_AND_64_OF_64_REAL_SUBPROCESS_"
    "INTEGRITY_FIXTURES_REJECTED__ZERO_CREDIT"
)
PASS_BYTES = b"PASS_C27R2_RELEASE_BOUNDARY_64_REAL_FIXTURES__ZERO_CREDIT\n"
...
CASE_RUN_SCHEMA = PREFIX + "release-integrity-case-process-run.v3"
CASE_RUN_STATUS = (
...
def case_specs(args: argparse.Namespace) -> list[dict[str, Any]]:
    ...
    need([row[0] for row in specs] == CASE_NAMES, "exact ordered 64 cases")
...
def execute(args: argparse.Namespace) -> dict[str, Any]:
    ...
    need(work.parent == AUDIT and work.name.startswith(
         "c27r2-release-repair-integrity-v3-")
...
def self_test() -> dict[str, Any]:
    need(len(CASE_NAMES) == 64 and len(set(CASE_NAMES)) == 64,
         "exact unique 64-case inventory")
    ...
    need(len(specs) == 64 and [row["name"] for row in specs] == CASE_NAMES
         and all(set(row) == {"name", "logical", "mutation", "changes"}
                 for row in specs),
         "actual exact64 case-spec construction fixture")
    ...
    with tempfile.TemporaryDirectory(prefix="c27r2-fixture-runner-v3-") as raw:
        root = Path(raw)
    ...
    return {"schema": RESULT_SCHEMA + ".self-test",
            "status": "PASS_EXACT_64_CASE_INVENTORY_WIRED_EXACT11_CLEANUP_"
                      "BOUNDED_PROCESS_AND_PRIVATE_LINK_TINY_FIXTURES",
