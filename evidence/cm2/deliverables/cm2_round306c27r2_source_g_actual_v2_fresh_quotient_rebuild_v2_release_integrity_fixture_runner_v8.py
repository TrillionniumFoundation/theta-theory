#!/usr/bin/env python3
"""Run the exact 73 real C27R2 release-boundary integrity fixtures.
...
then executes eight independent publication-lock protocol preflights and 73
...
VALIDATOR_DEFAULT = ROOT / "deliverables/cm2_round306c27r2_source_g_actual_v2_fresh_quotient_rebuild_v2_release_repair_boundary_independent_validator_v8.py"
...
RESULT_SCHEMA = PREFIX + "release-integrity-fixture.v8"
RESULT_STATUS = (
    "PASS_C27R2_RELEASE_BOUNDARY_V8_CONTROL_73_OF_73_REAL_SINGLE_OPEN_"
...
PASS_BYTES = (
    b"PASS_C27R2_RELEASE_BOUNDARY_V8_73_VALIDATOR_PLUS_8_LOCK_CHECKS__"
    b"ZERO_CREDIT\n"
)
...
COLD_NEGATIVE_EXPECTED_REJECTIONS = {
    "cold-authority-missing-key":
        "REJECT:cold authority exact fourteen-key shape\n",
    "cold-authority-extra-key":
        "REJECT:cold authority exact fourteen-key shape\n",
    "cold-authority-service-projection-mismatch":
        "REJECT:cold authority exact7 frozen service equals current exact10 "
        "projection\n",
}
...
def targets(args: argparse.Namespace) -> dict[str, str]:
    ...
    return {
        ...
        "cold_pinset": child("cold_control_relative", "pinset.json"),
        "cold_control": args.cold_control_relative,
...
def case_specs(args: argparse.Namespace) -> list[dict[str, Any]]:
    ...
    specs = [
        ...
        ("cold-authority-missing-key", "cold_pinset",
            "reclose-cold-authority-missing", {}),
        ("cold-authority-extra-key", "cold_pinset",
            "reclose-cold-authority-extra", {}),
        ("cold-authority-service-projection-mismatch", "cold_control",
            "reclose-cold-authority-service-projection-mismatch", {}),
    ]
    need([row[0] for row in specs] == CASE_NAMES, "exact ordered 73 cases")
    need(digest({"validator_negative_case_count": len(CASE_NAMES),
                 "ordered_validator_negative_case_names": CASE_NAMES})
             == VALIDATOR_NEGATIVE_INVENTORY_SHA256,
         "frozen ordered 73 validator inventory digest")
...
def prepare_override(spec: dict[str, Any], case_root: Path) -> dict[str, Any]:
    ...
    if mutation == "symlink":
        ...
        elif mutation in {"reclose-cold-authority-missing",
                          "reclose-cold-authority-extra"}:
            def change_cold_pinset_authority(value: dict[str, Any]) -> None:
                authority = value.get("actual_v2_authority")
                need(type(authority) is dict and len(authority) == 14,
                     "cold pinset exact14 mutation source")
                if mutation == "reclose-cold-authority-missing":
                    removed = authority.pop("declared_input_role_map_sha256",
                                            None)
                    need(valid_sha(removed),
                         "cold pinset missing-key fixture removes frozen role digest")
                else:
                    need("__fixture_extra__" not in authority,
                         "cold pinset extra-key fixture is new")
                    authority["__fixture_extra__"] = False
            reclose_json(target, "pinset_sha256",
                         change_cold_pinset_authority)
        elif mutation == "reclose-cold-authority-service-projection-mismatch":
            need(target.is_dir() and not target.is_symlink(),
                 "cold control directory override")

            def change_cold_service_projection(value: dict[str, Any],
                                               authority_key: str) -> None:
                authority = value.get(authority_key)
                need(type(authority) is dict and len(authority) == 14,
                     "cold authority exact14 projection mutation source")
                control_plane = authority.get("control_plane")
                service = control_plane.get("service") \
                    if type(control_plane) is dict else None
                invocation = service.get("InvocationID") \
                    if type(service) is dict else None
                need(type(invocation) is str
                     and re.fullmatch(r"[0-9a-f]{32}", invocation) is not None,
                     "cold authority service InvocationID fixture source")
                service["InvocationID"] = (
                    ("1" if invocation[0] != "1" else "0") + invocation[1:])

            pinset_path = target / "pinset.json"
            receipt_path = target / "cold_replay_receipt.json"
            reclose_json(pinset_path, "pinset_sha256",
                lambda value: change_cold_service_projection(
                    value, "actual_v2_authority"))
            reclose_json(receipt_path, "cold_replay_receipt_sha256",
                lambda value: change_cold_service_projection(
                    value, "authority_closure"))
            pinset_value = strict_document(pinset_path, "pinset_sha256")
            receipt_value = strict_document(
                receipt_path, "cold_replay_receipt_sha256")
            need(pinset_value.get("actual_v2_authority")
                     == receipt_value.get("authority_closure")
                 and len(pinset_value["actual_v2_authority"]) == 14,
                 "cold projection mismatch preserves receipt/pinset exact14 equality")
...
def execute_case(args: argparse.Namespace, spec: dict[str, Any], index: int,
    ...
    try:
        ...
        expected_rejection = COLD_NEGATIVE_EXPECTED_REJECTIONS.get(
            spec["name"])
        if expected_rejection is not None:
            rejection_bytes = expected_rejection.encode("utf-8")
            need(output.get("child_stderr_prefix") == expected_rejection
                 and output.get("child_stderr_size") == len(rejection_bytes)
                 and output.get("child_stderr_sha256")
                     == hashlib.sha256(rejection_bytes).hexdigest(),
                 "exact intended cold validator rejection reason:"
                 + spec["name"])
        ...
        body = {
            ...
            **output_projection,
            ...
            "expected_rejection_stderr": expected_rejection,
            "intended_rejection_reason_matched":
                (True if expected_rejection is not None else None),
...
def execute(args: argparse.Namespace) -> dict[str, Any]:
    ...
    need(work.parent == AUDIT and work.name.startswith(
         "c27r2-release-repair-integrity-v8-")
    ...
    need(baseline_value.get("status")
             == "PASS_C27R2_RELEASE_REPAIR_BOUNDARY_V8_FORMAL_EMPTY_OVERRIDE_EXACT_SPEC_ATTESTATION_INPUT_SET_FIXED_USER_BUS_SERVICE_QUERY_TEN_PROPERTY_EXECSTART_FRAGMENT_COLD_EXACT14_CURRENT_PROCESS_INVENTORY_AND_AUTHORITY_CLOSURE__ZERO_CREDIT"
    ...
    body = {
        ...
        "validator_rejected_count": 73,
        "publication_lock_preflight_case_count": 8,
        "publication_lock_preflight_rejected_count": 8,
        "total_integrity_check_count": 81,
        "validator_wrapper_process_execution_count": 73,
    ...
    need(sorted(path.name for path in work.iterdir()) == expected_before_pass
         ...
         "exact current pre-PASS receipt/control/73 wrapper inventories")
...
def self_test() -> dict[str, Any]:
    ...
    need(set(COLD_NEGATIVE_EXPECTED_REJECTIONS) == set(CASE_NAMES[-3:])
         and all(value.startswith("REJECT:cold authority ")
                 and value.endswith("\n")
                 for value in COLD_NEGATIVE_EXPECTED_REJECTIONS.values()),
         "three appended production cold negatives bind exact rejection reasons")
    need(digest({"validator_negative_case_count": len(CASE_NAMES),
            "ordered_validator_negative_case_names": CASE_NAMES})
             == VALIDATOR_NEGATIVE_INVENTORY_SHA256
    ...
    need(len(specs) == 73 and [row["name"] for row in specs] == CASE_NAMES
         and all(set(row) == {"name", "logical", "mutation", "changes"}
                 for row in specs),
         "actual exact73 case-spec construction fixture")
    ...
    with tempfile.TemporaryDirectory(prefix="c27r2-fixture-runner-v8-") as raw:
        root = Path(raw)
        lock_body = {
    ...
    return {"schema": RESULT_SCHEMA + ".self-test",
            "status": "PASS_EXACT_73_VALIDATOR_REAL_WRAPPERS_AND_8_LOCK_"
