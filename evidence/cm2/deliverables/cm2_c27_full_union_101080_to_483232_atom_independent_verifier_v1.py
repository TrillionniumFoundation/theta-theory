...
def same(a:Cap,b:Cap)->bool:
    ...
    while off<a.pre[2]:
        ...
        off+=len(x)
    return True
def rid(x:str)->str:s,h=x.split(":",1);return s.lower()+"-support-atom:"+h
def main()->int:
    ...
    try:
        ...
        for u in caps["union_ledger"].rows():
            ...
            if u["source_authority"]=="CURRENT_SUPPORT_V4B_SEED1_PRIORITY_LEDGER":
                ...
                if t=="POSITIVE_VOLUME_CARRIERS":p=positive[r["positive_primitive_row_sha256"]];aids=tuple(sorted(rid(x)for x in p["representative_atom_ids"]));m="EXACT_UNIQUE_POSITIVE_VOLUME_ATOM_WITNESS";ev=p["row_sha256"]
