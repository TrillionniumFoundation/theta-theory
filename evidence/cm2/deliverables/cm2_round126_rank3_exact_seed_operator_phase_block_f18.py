#!/usr/bin/env python3
"""Round126 exact-seed direct-standard-N F18 phase registration.

Round125 leaves one field absent on the 120 immutable
``(word, homogeneous-subbranch, roof-level, field-name)`` exact-seed keys.
This producer installs F18 by rebuilding, for every base key, the unique
F1--F17 rows from the frozen Round121, Round122 and Round125 certificates.
Each rebuilt same-key map is tied to one common child, collision stage,
materialized input recut and Round124 standard-family operator carrier.

The phase data use the direct ``standard_N`` route.  At roof level ``j`` of
a stage of return length ``r``, the registration is

    prefix z^j, suffix z^(r-j), block z^r.

The three stage lengths are 2, 1 and 2, so one stage block per stage gives
the child path ``z^5``.  Roof levels are symbolic factorizations of a stage
block and are not five additional collisions.

This is a finite exact-seed structural registration only.  It does not prove
an operator Wiener theorem, a Kac closure theorem, a global Gate5 block or
CM2.
"""
...
SCHEMA = "cm2.round126.rank3-exact-seed-operator-phase-block-f18.v1"
OUTPUT = (
    HERE
...
def load_inputs() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    verify_pins()
    document121 = strict_document(
        ROUND121,
        "cm2.round121.rank3-exact-seed-three-leg-recut-f5f6.v1",
    )
    document122 = strict_document(
        ROUND122,
        "cm2.round122.rank3-exact-seed-physical-face-field-bridge.v1",
    )
    document125 = strict_document(
        ROUND125,
        "cm2.round125.rank3-exact-seed-graph-current-dynamic-test-f17.v2",
    )
    require(
        document121["result_sha256"] == ROUND121_RESULT_SHA256,
        "Round121 result pin",
    )
    require(
        document122["result_sha256"] == ROUND122_RESULT_SHA256,
        "Round122 result pin",
    )
    require(
        document125["result_sha256"] == ROUND125_RESULT_SHA256,
        "Round125 result pin",
    )
    r121 = document121["result"]
    r122 = document122["result"]
    r125 = document125["result"]
    require(
    ...
    require(
        ...
        "Round125 global safety state",
    )
    require(
        len(r121["common_refinement_rows"]) == 24
        and len(r121["gate5_F1_F6_slot_rows"]) == 720
        and len(r122["child_face_incidence_rows"]) == 24
        and len(r122["gate5_F7_F13_F16_slot_rows"]) == 960
        and len(r125["gate5_F14_slot_rows"]) == 120
        and len(r125["gate5_F15_slot_rows"]) == 120
        and len(r125["gate5_F17_slot_rows"]) == 120
        and len(r125["standard_family_leg_operator_rows"]) == 72,
        "input census",
    )
    return r121, r122, r125
...
def source_rows_by_coordinate(
    ...
    for source_round, rows in sources:
        require(type(rows) is list, f"Round{source_round} slot rows")
        for row in rows:
            validate_row(row, f"Round{source_round} slot")
            field = row.get("field_index")
            require(type(field) is int and 1 <= field <= 17, "field index")
            require(
                SOURCE_ROUND_BY_FIELD[field] == source_round,
                "field source round",
            )
            require(row.get("field_name") == FIELD_NAMES[field], "field name")
            base_key = coordinate(row)
            require(
                row.get("immutable_slot_key")
                == [*base_key, FIELD_NAMES[field]],
                "immutable source slot key",
            )
            require(type(row.get("slot_id")) is str, "source slot id")
            require(row["slot_id"] not in slot_ids, "unique source slot id")
            slot_ids.add(row["slot_id"])
            immutable = canonical(row["immutable_slot_key"])
            require(immutable not in immutable_keys, "unique immutable source key")
            ...
            field_count[field] += 1
    require(
        len(slot_ids) == len(immutable_keys) == 2040,
        "2040 unique preexisting slots",
    )
    require(
        field_count == Counter({field: 120 for field in range(1, 18)}),
        "17 times 120 field census",
    )
    require(
        len(by_coordinate) == 120
        and all(set(rows) == set(range(1, 18))
                for rows in by_coordinate.values()),
        "120 complete F1-F17 same-key maps",
    )
    return by_coordinate


def build_preexisting_map_rows(
    r121: dict[str, Any],
    r122: dict[str, Any],
    r125: dict[str, Any],
) -> list[dict[str, Any]]:
    ...
    for operator in r125["standard_family_leg_operator_rows"]:
        validate_row(operator, "Round125 operator carrier")
        child_id = operator.get("common_child_id")
        stage = operator.get("stage")
        require(
            type(child_id) is str and type(stage) is int
            and stage in STAGE_RETURN_LENGTH,
            "operator carrier key",
        )
        key = (child_id, stage)
        require(key not in operators, "unique operator carrier")
    ...
    child_level_census: Counter[str] = Counter()
    stage_level_census: Counter[int] = Counter()
    for base_key in ordered_coordinates:
        source = source_map[base_key]
        first = source[1]
        child_id = first["common_child_id"]
        stage = first["stage"]
        require(type(stage) is int and stage in STAGE_RETURN_LENGTH, "stage")
        child = child_rows[child_id]
        rank = child["common_rank"]
        input_recut = child[STAGE_RECUT_KEY[stage]]
        require(type(input_recut) is str, "input recut")
        ...
        require(
            ...
            "operator coordinate crosswalk",
        )
        require(
            operator["input_materialized_recut_instance_id"] == input_recut,
            "operator input recut crosswalk",
        )
        require(
        ...
        for field, source_row in source.items():
            ...
            if field in list(range(7, 14)) + [16]:
                require(
                    ...
                    "F7-F13/F16 child incidence",
                )
        require(
            source[14]["round122_same_key_F10_slot_id"]
            == source[10]["slot_id"]
        ...
        require(
            source[15]["round122_same_key_F7_slot_id"]
            == source[7]["slot_id"]
            ...
            "F15 same-key and carrier links",
        )
        require(
            source[17]["round122_same_key_F11_slot_id"]
            == source[11]["slot_id"]
        ...
        rows.append(row)
        child_level_census[child_id] += 1
        stage_level_census[stage] += 1
    require(len(rows) == 120, "120 preexisting map rows")
    require(
        len({row["same_key_map_row_id"] for row in rows}) == 120,
        "unique map row IDs",
    )
    require(
        child_level_census == Counter({child: 5 for child in child_rows}),
        "five levels per child",
    )
    require(
        stage_level_census == Counter(EXPECTED_STAGE_LEVEL_COUNTS),
        "48/24/48 map stages",
    )
    return rows
...
def build_f18_slots(
    ...
    for map_row in map_rows:
        ...
        require(0 <= roof < return_length and suffix_length > 0,
                "phase exponent range")
        immutable_key = [
...
def phase_registration_contract() -> dict[str, Any]:
    return {
        ...
        "stage_return_length_by_stage": {"0": 2, "1": 1, "2": 2},
        "roof_level_js_by_stage": {"0": [0, 1], "1": [0], "2": [0, 1]},
...
def build() -> dict[str, Any]:
    r121, r122, r125 = load_inputs()
    map_rows = build_preexisting_map_rows(r121, r122, r125)
    f18_rows = build_f18_slots(map_rows)
    ...
    require(
        ...
        "rebuilt 2040 slot IDs",
    )
    require(
        len(new_ids) == len(set(new_ids)) == 120
        and not set(preexisting_ids).intersection(new_ids),
        "new F18 ID separation",
    )
    prior_registry = r125[
    ...
    require(
        ...
        and all(value.startswith("CERTIFIED_ON_ALL_24_")
                for value in gate_status.values()),
        "18 certified local field statuses",
    )
    ...
    result = {
        ...
        "direct_standard_N_phase_registration":
            phase_registration_contract(),
...
def main() -> int:
    ...
    print("ROUND126_EXACT_SEED_FIELD_MATURITY=18/18")
    print("SEED_LOCAL_COMPLETE_LEVEL_BLOCKS=120")
    print("SEED_LOCAL_COMPLETE_CHILD_PACKETS=24")
