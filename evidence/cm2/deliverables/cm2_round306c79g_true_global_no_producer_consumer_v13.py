#!/usr/bin/env python3
"""C79g v13 append-only zero-credit candidate builder.

This append-only v13 binds the independently verified C78s 13-member final
...
is permanently non-authoritative.  The separately committed v13 seal plus the
...
COLD_COORDINATION_PARENT_FD_ENV = "CM2_C79G_V13_COORDINATION_PARENT_FD"
...
SELF = OUT / "cm2_round306c79g_true_global_no_producer_consumer_v13.py"
...
STATIC_AUDIT_V13 = OUT / "cm2_round306c79g_true_global_no_producer_consumer_static_audit_v13.json"
COLD_LAUNCHER = OUT / "cm2_round306c79g_true_global_no_producer_consumer_cold_launch_v13.py"
COLD_MANIFEST = OUT / "cm2_round306c79g_true_global_no_producer_consumer_cold_launch_manifest_v13.sha256"
COLD_OUTER = OUT / "cm2_round306c79g_true_global_no_producer_consumer_cold_launch_outer_receipt_v13.json"
...
COLD_EXACT8 = (
    ...
    CONTRACT,
    SELF,
    OUT / "cm2_round306c79g_true_global_no_producer_consumer_independent_verifier_assembler_authority_consumer_v13.py",
    V12_TO_V13_TRANSITION,
    STATIC_AUDIT_V13,
    COLD_LAUNCHER,
)

# Frozen predecessor authority for the v13 successor draft.  This is a
...
def _linux_libc() -> ctypes.CDLL:
    need(sys.platform.startswith("linux"), "Linux-only v13 publication protocol")
...
class HeldSelf:
    ...
    def __init__(self, exec_inherited: int, source_inherited: int,
        ...
        need(set(incident_inherited) == set(expected_incident_pins),
             "exact ten inherited incident fd roles")
        ...
        # Four bootstrap descriptors plus ten incident descriptors are held;
        ...
        need(stat.S_ISREG(self.exec_before.st_mode) and
             ...
             len({(value.st_dev, value.st_ino)
                  for value in self.incident_before.values()}) == 10,
             "sealed exec/source/coordination/root plus ten incident fds first gate")
        ...
        need(self.exec_raw == self.source_raw and
             ...
             "sealed exec/source plus ten pinned incident fds stable first gate")
        ...
        need(SELF == OUT / "cm2_round306c79g_true_global_no_producer_consumer_v13.py",
    ...
    def terminal_replay(self) -> None:
        ...
        need(exec_replay == source_replay == self.exec_raw == self.source_raw and
             ...
             "sealed exec/source plus ten incident held-fd terminal replay")
...
def new_candidate_stage(outdir: Path, parent: HeldParent) -> tuple[Path, HeldMutableStage]:
    ...
    name = ".c79g-v13-candidate-stage-" + orientation + "-" + CHECKPOINT_OBJECT_PIN
...
def strict_bool_predecessor_and_successor_census(
        ...
        v13_sources: Mapping[str, bytes]) -> dict[str, Any]:
    """Prove frozen v5 defects and zero-unsafe complete v6/v7/v13 boxes."""
    ...
    v13 = {role: _strict_bool_need_census(v13_sources[role], role)
          for role in v13_roles}
    ...
    v13_unproved = sum(
        v13[role]["recursive_exact_bool_unproved_count"] for role in v13_roles)
    v13_arity = sum(
        v13[role]["arity_star_or_keyword_failure_count"] for role in v13_roles)
    v13_returns = sum(
        v13[role]["annotated_bool_return_failure_count"] for role in v13_roles)
    need(v6_unproved == 0 and v6_arity == 0 and v6_returns == 0 and
         v7_unproved == 0 and v7_arity == 0 and v7_returns == 0 and
         v13_unproved == 0 and v13_arity == 0 and v13_returns == 0,
         "v6/v7/v13 recursive exact-bool full boxes have zero unsafe or unproved sites")
    return {
        ...
        "same_seven_absent_from_v13": True,
        ...
        "v13_recursive_exact_bool_unproved_count": v13_unproved,
        ...
        "v13_direct_need_call_count": sum(
            v13[role]["direct_need_call_count"] for role in v13_roles),
...
def v6_held_self_identity_defect_regression(raw: bytes) -> dict[str, Any]:
    ...
    V13 first proves the complete three-site census, then independently locates
...
def _validate_v7_regression_section(
    ...
    need(strict_bool_census["v7_recursive_exact_bool_unproved_count"] == 0 and
         strict_bool_census["v13_recursive_exact_bool_unproved_count"] == 0 and
         strict_bool_census["all_twelve_sources_AST_parsed_and_compiled_in_memory"]
             is True and
         strict_bool_census["compiled_code_executed"] is False,
         label + ":v7/v13 full-callsite strict-bool closure")
    need(section.get("all_ten_file_pins_match") is True and
         section.get("all_declared_object_pins_match") is True and
         section.get("all_ten_regular_0444_nlink1") is True and
         section.get("exact8_manifest_reconstructs_first_eight_in_order") is True and
...
def _validate_final_static_audit(
    ...
    need(isinstance(dual, dict) and set(dual) == {
         ...
         }, "final v13 static audit dual-checker exact8 closure")
    ...
    need(helper_consensus.get("algorithm") ==
         ...
         "final v13 three-source helper AST, exact40 witness, and callsites")
    ...
    need(isinstance(input_a, dict) and isinstance(input_b, dict) and
         ...
         "final v13 checker A/B exact37 ordered identical input-pin closure")
    expected_inputs = {
    ...
    need(isinstance(attacks, dict) and set(attacks) == {
         ...
         "final v13 coherent attack exact7, 137/137 including ordered v11 "
    ...
    need(isinstance(sealed, dict) and set(sealed) == {
         ...
         "final v13 sealed-exec and scoped no-producer exact16 closure")
    ...
    need(isinstance(closure, dict) and set(closure) == {
         ...
         }, "final v13 schema/constructor exact21 closure")
    need(type(closure.get("schema_definition_count")) is int and
         ...
         "final v13 schema 46 defs/242 refs/0 unresolved/52 closed/0 mismatch")
    need(closure.get("strict_JSON_duplicate_keys_rejected") is True and
         ...
         "final v13 schema and live-protocol type-exact bool closure")
    actual_keywords = [
    ...
    need(closure.get("actual_schema_keyword_universe") == actual_keywords and
         ...
         "final v13 exact canonical schema keyword universes and digests")
    ...
    need(isinstance(shapes, dict) and set(shapes) == set(expected_shapes) and
         ...
         "final v13 exact nine output-shape key counts")
...
def hold_static_freeze_trust(self_guard: HeldSelf) -> dict[str, Any]:
    """Hold current v13 exact10 and every distinct append-only predecessor.

    Current exact8 begins with the official v12 rejection itself.  The v12,
    v11, v10, v9, v8, v7, v6, and v5 published exact10 surfaces remain distinct;
    v3 exact10, the separate v3 rejection, and the seven other v4 draft files
    are held.
    The exact distinct-file identity census is
    10+10+10+10+10+10+10+10+10+10+7+1 = 108.
    """
    manifest_guard = HeldPinnedInput(
        COLD_MANIFEST, "C79g v13 cold-launch manifest", 0o444)
    outer_guard = HeldPinnedInput(
        COLD_OUTER, "C79g v13 cold-launch outer-last receipt", 0o444)
    manifest_entries = parse_manifest_ordered(
        manifest_guard.raw, "C79g v13 cold-launch exact8 manifest")
    ...
    need(len(manifest_by_path) == 8 and
         ...
         manifest_by_path[V12_OFFICIAL_REJECTION] ==
             V12_OFFICIAL_REJECTION_FILE_PIN,
         "cold-launch manifest current SELF, v12 rejection, and policy pins")

    by_path: dict[Path, HeldPinnedInput] = {}
    for path in COLD_EXACT8:
        if path != SELF:
            by_path[path] = HeldPinnedInput(
                path, "C79g v13 cold exact8:" + path.name, 0o444,
                manifest_by_path[path], self_guard.incident_fds.get(path))
    v12_rejection_guard = by_path[V12_OFFICIAL_REJECTION]
    ...
    need(len(current_cold_ten_guards) == 10 and
         ...
         "current v13 cold exact10 unique held identities on one statx mount")
    exact8_stats = [
    ...
    need(all(chronology.values()),
         "physical v13 exact8/manifest/outer freeze chronology")
    ...
    all_file_guards = [
        *current_cold_ten_guards, *v12_guards, *v11_guards, *v10_guards, *v9_guards,
        *v8_guards,
        *v7_guards, *v6_guards, *v5_guards, *v3_guards, *v4_guards,
        v3_rejection_guard,
    ]
    need(len(all_file_guards) == 108 and
         len({guard.identity for guard in all_file_guards}) == 108 and
         ...
         "append-only v13/v12/v11/v10/v9/v8/v7/v6/v5/v3/v4/rejection history is 108 "
         "distinct files on one mount")
    require_v5_positive_and_stage_surfaces_absent()

    v13_consumer_path = COLD_EXACT8[4]
    strict_bool_census = strict_bool_predecessor_and_successor_census(
        ...
        {
            "producer_v13": self_guard.raw,
            "consumer_v13": by_path[v13_consumer_path].raw,
            "launcher_v13": by_path[COLD_LAUNCHER].raw,
        })

    transition_guard = by_path[V12_TO_V13_TRANSITION]
    audit_guard = by_path[STATIC_AUDIT_V13]
    transition = strict_json(transition_guard.raw, "C79g v13 transition trust")
    audit = strict_json(audit_guard.raw, "C79g v13 static audit trust")
    outer = strict_json(outer_guard.raw, "C79g v13 cold-launch outer trust")
    ...
    verify_object(transition, "C79g v13 transition trust")
    verify_object(audit, "C79g v13 static audit trust")
    verify_object(outer, "C79g v13 cold-launch outer trust")
    need(set(transition) == {
    ...
    contract_cross = strict_json(
        by_path[CONTRACT].raw, "C79g v13 contract cross-check")
    audit_schema = audit.get("schema_and_constructor_closure", {})
    transition_pin = audited.get("v12_to_v13_transition_receipt", {})
    ...
    need(all(contract_cross.get(key) == transition.get(key) == audit.get(key)
             for key in (
                 ...
                 "published_then_officially_rejected_predecessor_v9",
                 "published_then_officially_rejected_predecessor_v10",
                 "published_then_officially_rejected_predecessor_v11")) and
         ...
         contract_cross.get("v11_dual_validator_divergence_incident") ==
             ...
             v11_dual_validator_divergence_incident,
         "contract transition audit exact v5-v11 and witness/incident consensus")
    ...
    need(transition.get("status") ==
         ...
         audit_v4.get(
             "v13_closed_object_required_property_mismatch_count") == 0 and
         audit_v4.get("same_defects_absent_from_v13") is True and
         ...
         "transition/audit pin current v13 and preserve exact v3/v4/v7 history")
    ...
    need(set(outer) == {
         ...
         "cold-launch v13 outer-last exact8 and zero-credit closure")
...
def ensure_launch_configuration() -> HeldSelf:
    ...
    need(FINAL_V13_CORE_PINS_INSTALLED is True,
         "v13 final core pins are not installed; runtime permanently disabled")
    need(all(value not in V13_DRAFT_CORE_PIN_SENTINELS for value in (
             CONTRACT_FILE_PIN, CONTRACT_OBJECT_PIN, CLOSED_SCHEMA_FILE_PIN)),
         "v13 draft contract/schema sentinels must all be replaced before runtime")
    ...
    need(all(value and len(value) <= 10 and
         ...
         len({int(value) for value in fd_texts}) == 14 and
         EXECUTED_SOURCE == Path("/proc/self/fd") / exec_fd_text and
         _cold_root_text == str(ROOT) and ROOT.is_absolute() and
         SELF == ROOT / "deliverables/cm2_round306c79g_true_global_no_producer_consumer_v13.py",
         "producer accepts exactly fourteen distinct decimal cold-launch fds and "
    ...
    need(all(isinstance(value, str) and len(value) == 64 and
         ...
         "C79g v13 exact non-placeholder contract/schema/rejection pins")
    need(set(C78S_FINAL["pins"]) == set(C78S_NAMES) and
         ...
         "C79g v13 exact filled launch configuration")
...
def validate_static_authorities(static_trust: dict[str, Any]) -> dict[str, Any]:
    ...
    need(set(contract) == {
         ...
         contract.get("v13_bundle", {}).get(
             "independent_verifier_assembler_authority_consumer", {}).get("path") ==
             "deliverables/cm2_round306c79g_true_global_no_producer_consumer_"
             "independent_verifier_assembler_authority_consumer_v13.py" and
         contract.get("v13_bundle", {}).get("closed_schema_validator_policy") == {
         ...
         "v13 frozen contract checkpoint/composite-only boundary")
    ...
    need(closed.get("$ref") == "#/$defs/coldLaunchedCommittedAuthority" and
         ...
         "append-only v13 cold root, zero-credit inner, and persisted zero boundary")
    ...
    return close_object({
        ...
        **self_guard.execution_proof(),
        ...
        "post_source_static_freeze_trust": {
            "v12_to_v13_transition_receipt": {
                "path": str(V12_TO_V13_TRANSITION.relative_to(ROOT)),
                "file_sha256": static_trust["transition_file_sha256"],
                "object_sha256": static_trust["transition_object_sha256"],
            },
            "static_audit_v13": {
                "path": str(STATIC_AUDIT_V13.relative_to(ROOT)),
        ...
        "current_v13_producer_source_content_opened_or_read": False,
        "current_v13_producer_source_content_imported_compiled_or_executed": False,
...
def build(outdir: Path, self_guard: HeldSelf,
    ...
    lock = (
        "C79g v13 isolated candidate only; standalone non-authoritative. Formal-global credit "
