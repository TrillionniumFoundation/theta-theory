...
STAGE_SEEDS = {
    "c27_adapter": "30662901",
    "c27_adapter_verifier": "30662902",
    "c28_adapter": "30662903",
    "c28_adapter_verifier": "30662904",
    "producer": "30662905",
    "independent_verifier": "30662906",
    "coherent_attacks": "30662907",
}
STAGE_SOURCE_PINS = {
    "c27_adapter": "builder",
    "c27_adapter_verifier": "adapter_verifier",
    "c28_adapter": "builder",
    "c28_adapter_verifier": "adapter_verifier",
    "producer": "producer",
...
def pinset(args: argparse.Namespace, targets: dict[str, Path]) -> dict[str, Any]:
    body = {"schema": PINSET_SCHEMA,
            ...
            "source_pins": {**PINS, "watcher": args.expect_watcher_sha256},
            ...
            "stage_python_hash_seeds": STAGE_SEEDS,
            "producer_seed1": STAGE_SEEDS["producer"],
            "verifier_seed2": STAGE_SEEDS["independent_verifier"],
            "dual_real_hash_seeds": [STAGE_SEEDS["producer"],
                                     STAGE_SEEDS["independent_verifier"]],
...
def audit_command_specs(control: Path, pinset_path: Path,
                        pinset_value: dict[str, Any]) -> dict[str, str]:
    sources = {"builder": BUILDER, "adapter_verifier": ADAPTER_VERIFIER,
               "producer": PRODUCER, "verifier": VERIFIER,
               "attacks": ATTACKS}
    need(pinset_value.get("source_pins")
             == {**PINS, "watcher": pinset_value["source_pins"].get("watcher")}
         and valid_sha(pinset_value["source_pins"].get("watcher"))
         and pinset_value.get("stage_python_hash_seeds") == STAGE_SEEDS
         and pinset_value.get("producer_seed1") == STAGE_SEEDS["producer"]
         and pinset_value.get("verifier_seed2")
             == STAGE_SEEDS["independent_verifier"]
         and pinset_value.get("dual_real_hash_seeds")
             == [STAGE_SEEDS["producer"], STAGE_SEEDS["independent_verifier"]],
         "pinset source/seed provenance closure")
    result: dict[str, str] = {}
    for stage in STAGES:
        value = document(control / (stage + "_command_spec.json"),
                         "command_spec_sha256")
        source_name = STAGE_SOURCE_PINS[stage]
        seed = STAGE_SEEDS[stage]
        need(value.get("stage") == stage
             and value.get("source_path") == str(sources[source_name])
             and value.get("source_sha256") == PINS[source_name]
             and value.get("python_hash_seed") == seed
             and value.get("environment") == {
                 "PATH": "/usr/bin:/bin", "LANG": "C", "LC_ALL": "C",
                 "PYTHONHASHSEED": seed}
             and value.get("pinset_file_sha256") == file_sha(pinset_path)
             and value.get("pinset_object_sha256")
                 == pinset_value["pinset_sha256"]
             and value.get("runner_source_sha256") == PINS["runner"]
             and value.get("python_sha256") == PINS["python"],
             stage + ":command spec source/seed/pinset closure")
        result[stage] = value["command_spec_sha256"]
    return result
...
def execute(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        ...
        attestations[stage] = run_stage(control, targets, pinset_path, pinset_value,
            ...
            adapter_args("C27R2", c27, C27_PINS, targets[stage]), STAGE_SEEDS[stage],
        ...
        attestations[stage] = run_stage(control, targets, pinset_path, pinset_value,
            ...
            adapter_verifier_args("C27R2", c27, C27_PINS, targets["c27_adapter"],
                                  targets["c27_adapter_verification"]), STAGE_SEEDS[stage],
        ...
        attestations[stage] = run_stage(control, targets, pinset_path, pinset_value,
            stage, BUILDER, PINS["builder"],
            adapter_args("C28", c28, c28pins, targets[stage]), STAGE_SEEDS[stage],
        ...
        attestations[stage] = run_stage(control, targets, pinset_path, pinset_value,
            ...
            adapter_verifier_args("C28", c28, c28pins, targets["c28_adapter"],
                                  targets["c28_adapter_verification"]), STAGE_SEEDS[stage],
            ADAPTER_VERIFY_STATUS,
        ...
        attestations[stage] = run_stage(control, targets, pinset_path, pinset_value,
            ...
            [str(PYTHON), "-I", "-B", str(PRODUCER), *authorities,
             "--out-dir", str(targets["candidate"])], STAGE_SEEDS[stage], PRODUCER_STATUS,
        ...
        attestations[stage] = run_stage(control, targets, pinset_path, pinset_value,
            ...
            [str(PYTHON), "-I", "-B", str(VERIFIER), *authorities,
             ...
             "--out-file", str(targets["verification"])], STAGE_SEEDS[stage], VERIFIER_STATUS,
        ...
        attestations[stage] = run_stage(control, targets, pinset_path, pinset_value,
            ...
            [str(PYTHON), "-I", "-B", str(ATTACKS), "--python", str(PYTHON),
             ...
             "--timeout-seconds", str(args.timeout_seconds)], STAGE_SEEDS[stage], ATTACK_STATUS,
        ...
        command_spec_objects = audit_command_specs(control, pinset_path,
                                                    pinset_value)
        body = {"schema": CORE_SCHEMA, "status": CORE_STATUS,
                ...
                "pinset_file_sha256": file_sha(pinset_path),
                "pinset_object_sha256": pinset_value["pinset_sha256"],
                "source_pins": pinset_value["source_pins"],
                ...
                "stage_command_spec_objects": command_spec_objects,
                "stage_python_hash_seeds": STAGE_SEEDS,
                "producer_seed1": STAGE_SEEDS["producer"],
                "verifier_seed2": STAGE_SEEDS["independent_verifier"],
                "dual_real_hash_seeds": [STAGE_SEEDS["producer"],
...
def self_test() -> dict[str, Any]:
    ...
    fixture_args = argparse.Namespace(
        c27r2_terminal_dir="fixture-c27r2-terminal",
        c28_terminal_dir="fixture-c28-terminal",
        expect_c28_root_sha256="0" * 64,
        expect_c28_receipt_file_sha256="0" * 64,
        expect_c28_receipt_object_sha256="0" * 64,
        expect_c28_replay_file_sha256="0" * 64,
        expect_c28_replay_object_sha256="0" * 64,
        expect_c28_pass_lock_sha256="0" * 64,
        expect_watcher_sha256="1" * 64)
    fixture = pinset(fixture_args, test)
    fixture_body = dict(fixture)
    fixture_claim = fixture_body.pop("pinset_sha256")
    need(len(test) == 16 and CANDIDATE_FILES == sorted(set(CANDIDATE_FILES))
         and all(valid_sha(value) for value in PINS.values())
         and set(STAGE_SEEDS) == set(STAGES)
         and set(STAGE_SOURCE_PINS) == set(STAGES)
         and fixture["source_pins"] == {**PINS, "watcher": "1" * 64}
         and fixture["stage_python_hash_seeds"] == STAGE_SEEDS
         and fixture["producer_seed1"] == "30662905"
         and fixture["verifier_seed2"] == "30662906"
         and fixture["dual_real_hash_seeds"] == ["30662905", "30662906"]
         and fixture_claim == digest(fixture_body), "watcher fixture")
    return {"status": "PASS_C29_V2_GATED_CORE_WATCHER_V7_PROVENANCE_SELF_TEST",
