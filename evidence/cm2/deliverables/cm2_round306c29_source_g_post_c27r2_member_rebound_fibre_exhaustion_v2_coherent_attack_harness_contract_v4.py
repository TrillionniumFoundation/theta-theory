...
future transaction and terminal-seal layer.  After every rejection receipt is
closed, the two private link attacks are normalized to ordinary single-link
files so later evidence capture can remain strictly O_NOFOLLOW/single-link.
"""
...
def attacks() -> list[tuple[str, Callable[[Path], None], str]]:
    return [
        ...
        ("result_C27R2_replay_substitution", result_mutator(lambda x: x["source_authority"].__setitem__("C27R2_terminal_replay_object_sha256", "1" * 64)), "normal"),
        ("result_C28_replay_substitution", result_mutator(lambda x: x["source_authority"].__setitem__("C28_v2_terminal_replay_object_sha256", "2" * 64)), "normal"),
        ("result_C27R2_adapter_root_substitution", result_mutator(lambda x: x["source_authority"].__setitem__("C27R2_C29_adapter_root_manifest_sha256", "3" * 64)), "normal"),
        ("result_C28_adapter_root_substitution", result_mutator(lambda x: x["source_authority"].__setitem__("C28_v2_C29_adapter_root_manifest_sha256", "4" * 64)), "normal"),
        ("result_C27R2_adapter_receipt_substitution", result_mutator(lambda x: x["source_authority"].__setitem__("C27R2_C29_adapter_receipt_object_sha256", "5" * 64)), "normal"),
        ("result_C28_adapter_receipt_substitution", result_mutator(lambda x: x["source_authority"].__setitem__("C28_v2_C29_adapter_receipt_object_sha256", "6" * 64)), "normal"),
...
def normalize_private_link_cases(
    work: Path, candidate: Path, receipts: list[dict[str, Any]],
) -> list[dict[str, Any]]:
    """Normalize only the two rejected private link cases, never authority."""
    need(len(receipts) == 34 and all(item.get("rejected") is True for item in receipts),
         "all rejection receipts closed before private cleanup")
    records: list[dict[str, Any]] = []
    symlink_cases = list(work.glob("attack-*-symlink_substitution"))
    hardlink_cases = list(work.glob("attack-*-hardlink_extra_inventory"))
    need(len(symlink_cases) == 1 and len(hardlink_cases) == 1,
         "unique private link attack cases")
    symlink_target = symlink_cases[0] / FIBRE_LEDGER
    symlink_info = os.lstat(symlink_target)
    need(stat.S_ISLNK(symlink_info.st_mode), "expected rejected private symlink")
    symlink_target.unlink()
    clone_file(candidate / FIBRE_LEDGER, symlink_target)
    restored = os.lstat(symlink_target)
    need(stat.S_ISREG(restored.st_mode) and restored.st_nlink == 1
         and file_sha(symlink_target) == file_sha(candidate / FIBRE_LEDGER),
         "normalized private symlink case")
    records.append({"attack": "symlink_substitution",
                    "pre_lstat_kind": "symlink",
                    "post_lstat_kind": "regular_single_link",
                    "normalized_sha256": file_sha(symlink_target)})
    extra = hardlink_cases[0] / "HARDLINKED_RESULT.json"
    result = hardlink_cases[0] / RESULT
    first = os.lstat(result)
    second = os.lstat(extra)
    need(stat.S_ISREG(first.st_mode) and stat.S_ISREG(second.st_mode)
         ...
         "expected rejected private hardlink pair")
    extra.unlink()
    normalized = os.lstat(result)
    need(stat.S_ISREG(normalized.st_mode) and normalized.st_nlink == 1,
         "normalized private hardlink case")
    records.append({"attack": "hardlink_extra_inventory",
                    "pre_lstat_kind": "regular_two_link",
                    "post_lstat_kind": "regular_single_link",
                    "normalized_sha256": file_sha(result)})
    file_count = 0
    for current, directories, files in os.walk(work):
        base = Path(current)
        need(all(not (base / name).is_symlink() for name in directories),
             "no private symlink directory after cleanup")
        for name in files:
            info = os.lstat(base / name)
            need(stat.S_ISREG(info.st_mode) and info.st_nlink == 1,
                 "all private attack files regular single-link after cleanup")
            file_count += 1
    need(file_count > 0, "nonempty normalized private attack tree")
    return records
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    link_cleanup = normalize_private_link_cases(work, candidate, receipts)
    after = {name: file_sha(candidate / name) for name in sorted(FILES)}
    need(before == after, "source candidate pre/post SHA identical")
    body = {
        "schema": HARNESS_SCHEMA,
        "status": "PASS_34_OF_34_C29_V2_COHERENT_CANDIDATE_AND_AUTHORITY_ATTACKS_REJECTED__ZERO_CREDIT",
        "attack_census": {"required": 34, "rejected": len(receipts), "accepted": 0},
        "attacks": receipts,
        "source_candidate_pre_sha256": before,
        "source_candidate_post_sha256": after,
        "source_candidate_unchanged": True,
        "post_rejection_private_link_cleanup": {
            "normalized_cases": link_cleanup,
            "all_private_files_regular_single_link": True,
            "authoritative_candidate_unchanged": True,
        },
...
def self_test() -> dict[str, Any]:
    need(len(attacks()) == 34 and len({name for name, _action, _mode in attacks()}) == 34,
    ...
    with tempfile.TemporaryDirectory(
            prefix="cm2-c29-v2-link-cleanup-", dir=ROOT / ".cm2-runtime/audit") as name:
        base = Path(name)
        candidate = base / "candidate"
        work = base / "work"
        candidate.mkdir()
        work.mkdir()
        (candidate / FIBRE_LEDGER).write_bytes(b"fibre-fixture\n")
        (candidate / RESULT).write_bytes(b"result-fixture\n")
        symlink_case = work / "attack-00-symlink_substitution"
        symlink_case.mkdir()
        (symlink_case / FIBRE_LEDGER).symlink_to(FIBRE_LEDGER)
        hardlink_case = work / "attack-01-hardlink_extra_inventory"
        hardlink_case.mkdir()
        (hardlink_case / RESULT).write_bytes(b"result-fixture\n")
        os.link(hardlink_case / RESULT,
                hardlink_case / "HARDLINKED_RESULT.json")
        cleanup = normalize_private_link_cases(
            work, candidate, [{"rejected": True} for _ in range(34)])
        need(len(cleanup) == 2
             and all(item["post_lstat_kind"] == "regular_single_link"
                     for item in cleanup), "self-test private link cleanup")
    return {
        "schema": HARNESS_SCHEMA + ".self-test",
        "status": "PASS_C29_V2_34_ATTACK_CATALOG_AND_RECLOSURE_FIXTURE",
        "attack_count": 34, "formal_credit": 0,
