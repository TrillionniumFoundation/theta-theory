...
def validate_terminal(
    ...
    need(type(receipt.get("status")) is str
         and receipt["status"].startswith("PASS_")
    ...
    need(projection.get("terminal_replay_passed") is True
         ...
         and projection.get("terminal_root_manifest_sha256") == expected_root
         and projection.get("terminal_receipt_file_sha256") == expected_receipt_file
         and projection.get("terminal_receipt_object_sha256") == expected_receipt_object
         and projection.get("terminal_status") == receipt["status"]
         and projection.get("PASS_lock_sha256") == file_sha(pass_path),
         kind + ":projection terminal binding")
    pass_bytes = pass_path.read_bytes()
    need(pass_bytes.startswith(b"PASS_") and pass_bytes.endswith(b"\n")
         and pass_bytes.count(b"\n") == 1, kind + ":exact one-line PASS")
    ...
    if kind == "C27R2":
        need(census.get("members") == EXPECTED_MEMBERS
    ...
    return {
        ...
        "receipt_file_sha256": expected_receipt_file,
        "receipt_object_sha256": expected_receipt_object,
