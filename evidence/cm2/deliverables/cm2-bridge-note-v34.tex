...
\large Fixed-gauge operator differentiation, native stopping, three-frequency removal,
and a strongly conditional
pathwise CM2 criterion}
\author{Working note}
\date{13 July 2026 (version 34)}
...
and one-derivative strong-to-weak blocks.  A noncircular two-sided
loss-of-memory lemma converts the last block to CM2.
...
escape estimate is called beyond the finite spacetime resolving depth.
...
$\cB_{\rm str}\hookrightarrow\cB_{\rm weak}$ be two fixed distribution
spaces with their zero-mass subspaces understood.  Condition
...
\item The complete operator quotient has one exact decomposition
      \[
        K_s^{\rm DQ}=K_{s,\rm reg}^{\rm DQ}
          +K_{s,\rm face}^{\rm DQ}
      ...
      \label{eq:uniform-strong-weak-DQ}
       \sup_{0<|s|<s_0}
       \|K_{s,\rm sw}^{\rm DQ}\|_{\cB_{\rm str}\to\cB_{\rm weak}}<\infty,
       \qquad
       \|E_s\|_{\cB_{\rm str}\to\cB_{\rm weak}}\longrightarrow0.
      \end{equation}
      A graph-supported boundary contribution of the commutator is included
      in $K_{s,\rm face}^{\rm DQ}$; it is never called an $M\times M$
      density.
\item Uniformly for $|s|<s_0$, the centered evolution has the two genuine
      ...
      \begin{align}
       ...
       \|Q_s^nu\|_{(C^r_0)^*}
       &\le C\theta_+^n\|u\|_{\cB_{\rm weak}},
       &&u\in\cB_{\rm weak},\ u(\one)=0,
       \label{eq:weak-side-memory}
      \end{align}
      with $\theta_\pm<1$.  These are Banach-space estimates, not an
      assumed CM2 estimate for $K_{s,\rm sw}^{\rm DQ}$.
\item The same typed decomposition holds for the limiting fixed-gauge
      operator $K_V$.  The regular and face blocks converge in their stated
      ledgers, while \eqref{eq:uniform-strong-weak-DQ} controls the
      strong-to-weak remainder.  All constants are uniform on the shallow
      spacetime cells used below.
\end{enumerate}
...
face, or strong-to-weak types in
Definition~\ref{def:measure-gauge-interface}.  Moreover
\begin{equation}
\label{eq:derived-strong-weak-CM2}
 |\langle Q_s^nK_{s,\rm sw}^{\rm DQ}Q_s^mg,f\rangle|
 \le C\theta_-^m\theta_+^n\|g\|_{C^r}\|f\|_{C^r},
\end{equation}
and the analogous bound for $E_s$ has a prefactor tending to zero.
\end{lemma}
...
\eqref{eq:weak-side-memory}; duality gives
\eqref{eq:derived-strong-weak-CM2}.  Replacing
$K_{s,\rm sw}^{\rm DQ}$ by $E_s$ gives the same estimate with the vanishing
operator-norm prefactor.  The other two types are exactly \textup{(MT1)}.
\end{proof}
...
preserves conditional mass.  The exceptional stopping mass is
$O(r^{\Theta_{\rm st}})$.
\end{lemma}
...
and the buffer/gate tails give $e^{-cN}$.  Integrate in
...
\end{equation}
has saving \(C\Xi^{-\beta_{\rm cen}}\) times a fixed power of
\(\operatorname{Mark}(Q)\).  Modes with $\Lambda>H_N$ are estimated without
...
\end{equation}
If $r_{\rm obs}>2+\beta_{\rm cen}$, the complete high lattice sum is
...
The fixed compact endpoint charts have $a_X+a_Y\le C$, so
$\Xi\le C\Lambda$.  Therefore a mode in
\eqref{eq:intermediate-phase-range} calls the prescribed escape estimate only
to depth
\begin{equation}
\label{eq:intermediate-max-resolving-depth}
 N_{\rm PPE}\le C_{\rm buf}\delta\log(C H_N)
 \le (C_{\rm buf}\delta\eta_{\rm hi}+o(1))N.
\end{equation}
...
$e^{-\eta_{\rm lo}\beta_{\rm cen}N}$ in the intermediate range.  The
ultra-high range contributes $e^{-\eta_{\rm hi}\rho_\infty N}$ by
...
Under $\mathrm{SHREC}(q_{\rm sh})$, after decreasing the coupling exponent
to some $c_{\rm off}^{\rm sh}>0$,
\begin{multline}
...
\end{multline}
The one-sided versions hold in the two off-diagonal cones.
\end{lemma}

\begin{proof}
Split each orientation into $R^\pm\le\varepsilon(m+n)$ and its complement.
The good set gives the recovered coupling rate.  Markov's inequality under
$\pi^{\rm sh}_{s,L}$ and
\eqref{eq:normalized-shallow-recovery} charges the two bad sets separately.
Choose $\varepsilon$ so that the recovery-tail exponent and the recovered
coupling exponent are both positive, then multiply the normalized estimate
...
\label{eq:intermediate-resolving-constant}
 ...
 :=C_{\rm ep}+C_{\rm buf}\delta\eta_{\rm hi}+C_{\rm hom},
\end{equation}
...
Thus the common shallow refinement supports the complete three-frequency
...
\item $\mathrm{NST}_{\rm phys}$, $\mathrm{SS}(d_Z)$, and
      ...
      $r_{\rm obs}>2+\beta_{\rm cen}$; longer shallow times are not estimated
      by this interface;
...
\item $\mathrm{MT}_{\rm DQ}$ holds for the fixed-gauge operator quotient,
      ...
      \eqref{eq:weak-side-memory}.  The regular defect
      satisfies, for some
      $1\le R\le3$,
      \[
...
\label{eq:final-CM2-exponent}
 ...
 \eta_{\rm lo}\beta_{\rm cen},
 \eta_{\rm hi}\rho_\infty,\eta_{\rm lo}R,
...
commutators remain in the strong-to-weak block, and their product decay is
derived from \eqref{eq:strong-side-memory}--\eqref{eq:weak-side-memory}; no
...
ultra-high observable, and regular-tail exponents in
...
\begin{array}{ccl}
 ...
 \textup{(H5)}&\Longrightarrow&
   \text{fixed-gauge finite-DQ regular/face/strong--weak CM2 blocks}.
\end{array}
\]
\endgroup
...
$K^{-\beta_{\rm cen}}$; the ultra-high original-observable tail is
$H^{-\rho_\infty}$ with $H=e^{\eta_{\rm hi}N}$, and the regular high tail is
$K^{-R}$.
...
strong-to-weak term is charged to the derived estimate
\eqref{eq:derived-strong-weak-CM2} rather than an assumed CM2 norm.
\end{enumerate}
All exponents are required strictly positive; in particular the
$\kappa_T^{\rm sh}$ interval must be nonempty.

There are fourteen falsifiable interfaces.  $\mathrm{MT}_{\rm DQ}$ must
retain the fixed-gauge commutator, type it correctly, and verify the two
loss-of-memory bounds.  $\mathrm{NST}_{\rm phys}$ must provide a
...
\begin{proposition}[Twelve obstruction tests]
\label{prop:v34-obstruction-tests}
...
\begin{proof}
Items 1--3 and 8--11 are direct finite-measure, frequency, or norm
