#!/usr/bin/env python3
"""Independent verifier for Round135 wide positive-Borel local robustness.

This verifier never imports or executes the Round135 producer.  It independently
reconstructs all 970 evidence rows from byte-pinned upstream inputs, including
the 2^-17 collar and 2^-16 boundary obstruction, the normalized integral
envelopes, four complete physical subcollar audits, all 72 F17 guards, and the
18-field local transport/nonpromotion boundary.

The finite rows remain transport templates for an uncountable local family;
they are never upgraded to global Gate5 blocks.
"""
...
PRODUCER = HERE / "cm2_round135_rank3_wide_positive_borel_local_robustness.py"
CERTIFICATE = (
    HERE
...
CERTIFICATE_SCHEMA = "cm2.round135.rank3-wide-positive-borel-local-robustness.v1"
VERIFICATION_SCHEMA = (
...
class VerificationError(RuntimeError):
    """Fail-closed verification, parsing, reconstruction or path error."""
...
PRIMARY_ID_KEYS = {
    "root_partition_endpoint_rows": "root_partition_endpoint_id",
    "derivative_convergence_rows": "derivative_convergence_id",
    "normalized_derivative_cell_rows": "normalized_derivative_cell_id",
    "input_recut_root_rows": "input_recut_root_id",
    "stage3_recut_root_rows": "stage3_recut_root_id",
    "wide_common_rank_rows": "wide_common_rank_id",
    "wide_merged_cut_rows": "wide_merged_cut_id",
    "wide_stage3_output_fragment_rows": "wide_stage3_fragment_id",
    "physical_subcollar_audit_rows": "physical_subcollar_audit_id",
    "wide_F17_guard_rows": "wide_F17_guard_id",
    "wide_field_transport_rows": "wide_field_transport_id",
}
...
def validate_row_closures(result: dict[str, Any]) -> None:
    ...
    all_ids: list[str] = []
    rebuilt_count = 0
    for name, expected_count in ROW_GROUP_COUNTS.items():
        rows = result[name]
        require(type(rows) is list and len(rows) == expected_count, f"rows:{name}")
        require(ledger[name] == expected_count, f"ledger rows:{name}")
        require(
            result[f"{name}_sha256"] == digest(rows),
            f"group digest:{name}",
        )
        ...
        for row in rows:
            ...
            identifier = body.get(PRIMARY_ID_KEYS[name])
            require(type(identifier) is str, f"row identifier:{name}")
            all_ids.append(identifier)
    require(rebuilt_count == ledger["finite_evidence_row_count"] == 970, "970 rows")
    require(len(all_ids) == len(set(all_ids)) == 970, "970 unique evidence IDs")
...
def evaluate_document(
    ...
    require(result == expected, "independent 970-row reconstruction equality")
    require(
...
def reclose_group(result: dict[str, Any], group: str, index: int) -> None:
...
def semantic_mutations(
    ...
    simple("precision weakened", ("result", "precision_bits"), 1024)
    simple(
        "Round121 upstream pin altered",
        (
            "result",
            "frozen_input_contract",
            "byte_pins",
    ...
    simple(
        "wide half-width narrowed",
        (
            "result",
            "wide_positive_Borel_family_root_contract",
            "lambda_negative_power_of_two_half_width",
        ),
        "1/262144",
    )
    simple(
        "largest dyadic claim altered",
        (
            "result",
            "wide_positive_Borel_family_root_contract",
            "largest_negative_power_of_two_half_width_strictly_inside_parent",
    ...
    simple(
        "next boundary obstruction erased",
        (
            "result",
            "wide_positive_Borel_family_root_contract",
            "next_half_width_hits_both_parent_faces",
    ...
    simple(
        "root monotonicity erased",
        (
            "result",
            "wide_positive_Borel_family_root_contract",
            "root_t_strictly_increasing_in_lambda",
        ),
        False,
    )
    simple(
        "endpoint subtraction falsely enabled",
        (
            "result",
            "normalized_integral_contract",
            "endpoint_difference_subtraction_used",
        ),
        True,
    )
    simple(
        "stage-one natural count altered",
        (
            "result",
            "normalized_integral_contract",
            "stable_natural_cell_counts",
        ),
        [1, 6, 18, 193],
    )
    simple(
        "common minimum gap erased",
        ("result", "wide_geometry_summary", "minimum_input_common_gap"),
        "0",
    )
    simple(
        "physical four-slice proof erased",
        (
            "result",
            "four_subcollar_physical_empty_contract",
            "four_slice_audit_all_strict",
        ),
        False,
    )
    simple(
        "physical face invented",
        (
            "result",
            "four_subcollar_physical_empty_contract",
    ...
    simple(
        ...
        (
            "result",
            "wide_F17_guard_contract",
            "F17_graph_current_guard_remains_strict",
        ),
        False,
    )
    simple(
        "wide local maturity weakened",
        (
            "result",
            "wide_local_18_field_transport_contract",
            "wide_family_every_exact_fibre_local_field_maturity",
        ),
        "17/18",
    )
    simple(
        "actual object IDs conflated",
        (
            "result",
            "wide_local_18_field_transport_contract",
            "wide_family_actual_object_IDs_are_new_not_Round129_Round130_IDs",
        ),
        False,
    )
    simple(
        "finite templates promoted to census",
        (
            "result",
            "wide_local_18_field_transport_contract",
            "finite_rows_are_family_templates_not_an_uncountable_actual_census",
        ),
        False,
    )
    simple(
        "physical F10 invented",
        ("result", "F10_and_cemetery_separation_contract", "physical_F10"),
        "1",
    )
    simple(
        "ambient cemetery invented",
        (
            "result",
            "F10_and_cemetery_separation_contract",
            "ambient_pre_regularization_cemetery",
        ),
        "ZERO",
    )
    simple(
        "owner cemetery invented",
        (
            "result",
            "F10_and_cemetery_separation_contract",
    ...
    simple(
        "positive payment invented",
        (
            "result",
            "F10_and_cemetery_separation_contract",
            "neither_statement_is_positive_cemetery_payment",
        ),
        False,
    )
    simple(
        "finite evidence count altered",
        ("result", "count_ledger", "finite_evidence_row_count"),
        969,
    )
    simple(
        "per-fibre slot count altered",
        ("result", "count_ledger", "per_exact_lambda_field_slot_count"),
        2159,
    )
    simple(
        "global maturity promoted",
        ("result", "global_safety_and_nonpromotion", "gate5_global_maturity"),
        "18/18",
    )
    simple(
        ...
        (
            "result",
            "global_safety_and_nonpromotion",
    ...
    simple(
        "Gate5 promoted",
        ("result", "global_safety_and_nonpromotion", "gate5_status"),
        "CERTIFIED",
    )
    simple(
        "CM2 promoted",
    ...
    simple("top-level block invented", ("result", "complete_18_field_block_count"), 1)
    simple("top-level Gate5 promoted", ("result", "gate5_status"), "CERTIFIED")
    simple("top-level CM2 promoted", ("result", "cm2_verdict"), "GO_FOR_CLAIM")
    mutations.append(
        (
    ...
    for group in ROW_GROUP_COUNTS:
        def mutate_group(
            document: dict[str, Any],
            selected: str = group,
        ) -> None:
            row = document["result"][selected][0]
            identifier_key = PRIMARY_ID_KEYS[selected]
            row[identifier_key] += "-tampered"
            reclose_group(document["result"], selected, 0)
            resign(document)

        mutations.append((f"materialized row altered:{group}", mutate_group))
    ...
    for index in range(18):
        def promote_field(
            ...
            document["result"]["wide_field_transport_rows"][selected][
                "global_field_certification_claimed"
            ] = True
            reclose_group(
                document["result"],
                "wide_field_transport_rows",
                selected,
            )
            resign(document)

        mutations.append((f"field {index + 1} globally promoted", promote_field))
...
def strict_json_attacks(
    ...
    def test(label: str, payload: bytes) -> None:
    ...
    test(
        ...
        raw.replace(
            b'"status": "CERTIFIED_WIDE_POSITIVE_BOREL_LOCAL_ROBUSTNESS_ONLY"',
            b'"status": "bad", "status": "CERTIFIED_WIDE_POSITIVE_BOREL_LOCAL_ROBUSTNESS_ONLY"',
            1,
        ),
    )
    test(
        "floating-point evidence count",
        raw.replace(b'"finite_evidence_row_count": 970', b'"finite_evidence_row_count": 970.0', 1),
    )
    test(
        "NaN constant",
        raw.replace(b'"finite_evidence_row_count": 970', b'"finite_evidence_row_count": NaN', 1),
    )
    test(
        "positive Infinity",
        raw.replace(b'"finite_evidence_row_count": 970', b'"finite_evidence_row_count": Infinity', 1),
    )
    test(
        "negative Infinity",
        raw.replace(b'"finite_evidence_row_count": 970', b'"finite_evidence_row_count": -Infinity', 1),
    )
    test("UTF-8 BOM", b"\xef\xbb\xbf" + raw)
    test("invalid UTF-8", raw[:10] + b"\xff" + raw[10:])
    test("top-level array", b"[]\n")
    test("top-level null", b"null\n")
    test("trailing second document", raw + b"{}\n")
    test(
        "oversized integer",
        raw.replace(
            b'"finite_evidence_row_count": 970',
            b'"finite_evidence_row_count": ' + b"9" * (MAX_INTEGER_DIGITS + 1),
            1,
        ),
    )
    test(
        "negative zero",
        raw.replace(b'"finite_evidence_row_count": 970', b'"finite_evidence_row_count": -0', 1),
    )
    test(
        "leading-zero integer",
        raw.replace(b'"finite_evidence_row_count": 970', b'"finite_evidence_row_count": 0970', 1),
    )
    test(
        "unpaired surrogate",
        raw.replace(
            b'"CERTIFIED_WIDE_POSITIVE_BOREL_LOCAL_ROBUSTNESS_ONLY"',
            b'"\\ud800"',
            1,
        ),
    )
    ...
    stale["result"]["cm2_verdict"] = "GO_FOR_CLAIM"
    test(
        "stale outer digest",
...
def build_verification(
    ...
    integral = expected["normalized_integral_contract"]
    root = expected["wide_positive_Borel_family_root_contract"]
    physical = expected["four_subcollar_physical_empty_contract"]
    f17 = expected["wide_F17_guard_contract"]
    return {
        "status": "PASS",
        ...
        "independence_contract": {
            "Round135_producer_imported": False,
            "Round135_producer_executed": False,
            "all_970_evidence_rows_independently_reconstructed": True,
            "all_row_and_group_hashes_independently_reclosed": True,
            "all_970_evidence_IDs_independently_unique": True,
            "full_certificate_result_independently_reconstructed": True,
        },
        "replay_audit": {
            "row_group_counts": dict(ROW_GROUP_COUNTS),
            "finite_evidence_row_count": 970,
            "lambda_half_width": root[
                "lambda_negative_power_of_two_half_width"
            ],
            "lambda_closed_collar": root["lambda_closed_collar"],
            "next_dyadic_half_width": root[
                "next_negative_power_of_two_half_width"
            ],
            "next_half_width_hits_both_parent_faces": root[
                "next_half_width_hits_both_parent_faces"
            ],
            "D_global_strict_enclosures": {
                stage: integral[f"D{stage}_global_strict_enclosure"]
                for stage in (1, 2, 3)
            },
            "V_at_1_strict_enclosures": {
                stage: integral[f"V{stage}_at_1_strict_enclosure"]
                for stage in (1, 2, 3)
            },
            "input_recut_root_count": 23,
            "stage3_recut_root_count": 192,
            "common_child_count": 24,
            "merged_cut_count": 215,
            "stage3_output_fragment_count": 216,
            "physical_subcollar_count": physical["lambda_subdivision_count"],
            "physical_candidate_checks": physical[
                "rank3_candidate_occurrence_total_over_four_subcollars"
            ],
            "physical_F10_count": 0,
            "wide_F17_guard_row_count": f17["wide_guard_row_count"],
            "wide_F17_guard_remains_strict": f17[
                "F17_graph_current_guard_remains_strict"
            ],
            "wide_field_transport_row_count": 18,
        ...
        "safety": {
            "finite_rows_are_local_family_templates_not_global_blocks": True,
            "physical_F10": "0",
            "ambient_pre_regularization_cemetery": "NOT_INSTALLED",
            "all_time_owner_cemetery": "NOT_INSTALLED",
            "global_complete_18_field_block_count": 0,
...
def main() -> int:
    ...
    try:
        ...
        result = build_verification(expected, semantic_labels, strict_labels)
        document = {
