#!/usr/bin/env python3
"""Rebuild left/right graph envelopes for Round-82 residual intersection boxes."""
...
def process_chunk(tasks: list[tuple[str, str, int, str, str, str, tuple[Q, Q, Q, Q], tuple[Q, Q, Q, Q], tuple[Q, Q, Q, Q]]]) -> list[tuple[str, str, str, list[str], list[str], list[str]]]:
    output = []
    for left_id, right_id, source_index, second, left_candidate, right_candidate, left_box, right_box, box in tasks:
        ...
        try:
            ...
            pass
        output.append((
            left_id, right_id, "REQUIRES_GRAPH_DIFFERENCE",
            list(map(str, left_box)), list(map(str, right_box)), list(map(str, box)),
        ))
    return output
...
def build(workers: int = 24) -> dict[str, Any]:
    ...
    for source_index, boxes in boxes_by_core.items():
        ...
        for box, component_id in boxes:
            ...
            for other_box, other_id in active:
                ...
                ordered_boxes = (other_box, box) if left_id == other_id else (box, other_box)
                unique_tasks.add((
                    left_id, right_id, source_index, left["second_selected_target_id"],
                    left["third_candidate_id"], right["third_candidate_id"],
                    ordered_boxes[0], ordered_boxes[1], overlap,
                ))
    ...
    residual_rows: dict[tuple[str, str], list[dict[str, list[str]]]] = defaultdict(list)
    status_histogram = Counter()
    for left_id, right_id, status, left_box, right_box, overlap_box in rows:
        pair_counts[(left_id, right_id)][status] += 1
        status_histogram[status] += 1
        if overlap_box:
            residual_rows[(left_id, right_id)].append({
                "left_graph_box": left_box,
                "right_graph_box": right_box,
                "overlap_box": overlap_box,
            })
    pair_rows = []
    for pair, counts in sorted(pair_counts.items()):
        residual = sorted(residual_rows[pair], key=lambda row: (row["overlap_box"], row["left_graph_box"], row["right_graph_box"]))
        pair_rows.append({
            ...
            "residual_graph_geometry_count": len(residual),
            "residual_graph_geometry_rows": residual,
            "residual_graph_geometry_rows_sha256": digest(residual),
        })
    result = {
        "different_candidate_raw_root_box_overlap_count": raw_overlap_count,
        "different_candidate_graph_envelope_triple_count": len(tasks),
        ...
        "strictly_excluded_graph_envelope_triple_count": len(tasks) - status_histogram["REQUIRES_GRAPH_DIFFERENCE"],
        "residual_graph_geometry_count": status_histogram["REQUIRES_GRAPH_DIFFERENCE"],
        "residual_unique_overlap_box_count": len({
            (row["left_physical_root_component_id"], row["right_physical_root_component_id"], tuple(geometry["overlap_box"]))
            for row in pair_rows for geometry in row["residual_graph_geometry_rows"]
        }),
        "residual_component_pair_count": sum(row["residual_graph_geometry_count"] > 0 for row in pair_rows),
        "pair_rows": pair_rows,
        "pair_rows_sha256": digest(pair_rows),
        "strict_scope": "left/right certified graph envelopes and their overlap for every Round-82 residual different-candidate box",
    }
    if result["residual_unique_overlap_box_count"] != 37876:
        raise RuntimeError("Round-82 residual overlap count mismatch")
    return {"schema": "cm2.round83.residual-graph-geometry.v1", "result": result, "result_sha256": digest(result)}
