...
VALIDATOR_DEFAULT = ROOT / "deliverables/cm2_round306c27r2_source_g_actual_v2_fresh_quotient_rebuild_v2_release_repair_boundary_independent_validator_v5.py"
CASE_WRAPPER_DEFAULT = ROOT / "deliverables/cm2_round306c27r2_source_g_actual_v2_fresh_quotient_rebuild_v2_release_integrity_case_transaction_wrapper_v1.py"
...
RESULT_SCHEMA = PREFIX + "release-integrity-fixture.v5"
RESULT_STATUS = (
...
CASE_RUN_SCHEMA = PREFIX + "release-integrity-case-process-run.v5"
CASE_RUN_STATUS = (
...
CASE_WRAPPER_REQUEST_SCHEMA = PREFIX + "release-integrity-case-wrapper-request.v1"
CASE_WRAPPER_REQUEST_STATUS = "FROZEN_REAL_PER_CASE_WRAPPER_REQUEST__ZERO_CREDIT"
...
def service_snapshot(args: argparse.Namespace) -> dict[str, str]:
    ...
    completed = subprocess.run([
        str(SYSTEMCTL), "--user", "show", args.core_unit,
    ...
    need(len(lines) == 7 and all(line.count("=") == 1 for line in lines)
         ...
         "service snapshot exactly seven unique key/value lines")
...
def execute(args: argparse.Namespace) -> dict[str, Any]:
    ...
    need(work.parent == AUDIT and work.name.startswith(
         "c27r2-release-repair-integrity-v5-")
    ...
    need(baseline_value.get("status")
             == "PASS_C27R2_RELEASE_REPAIR_BOUNDARY_V5_FORMAL_EMPTY_OVERRIDE_EXACT_SPEC_ATTESTATION_INPUT_SET_FIXED_USER_BUS_SERVICE_QUERY_CURRENT_PROCESS_INVENTORY_AND_AUTHORITY_CLOSURE__ZERO_CREDIT"
...
def self_test() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="c27r2-fixture-runner-v5-") as raw:
        root = Path(raw)
        lock_body = {
