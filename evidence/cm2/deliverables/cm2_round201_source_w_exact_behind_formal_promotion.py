#!/usr/bin/env python3
"""Formal source-W exact-behind promotion over two disjoint origin cohorts.

This producer starts from the frozen, independently verified Round180 and
Round184 formal chain.  It does not import any Round187--Round197 diagnostic
probe.  It independently reconstructs:

* the complete 1,444-row Round184 priority registry;
* the pure-clipped target-first cohort and its bounded depth-2 refinement;
* every cell in the 596 mixed plus 54 compact-q priority origins; and
* every exact-behind candidate deletion and remaining-record disposition.

Integer credit is issued only for complete strict physical-chart-interior
origins.  Physical source seams, compact-q/source-grazing origins, incomplete
mixed origins, and every live or unresolved reduced disposition remain
residual.  Per-cell evidence, whole-origin ledgers, split lineage, exact
volumes, and the 76,832-record conservation identity are materialized in the
certificate.
"""
...
def preimport_pin() -> None:
    require(
        hashlib.sha256(read_regular(HERE / ROUND180_VERIFIER)).hexdigest()
        == ROUND180_VERIFIER_SHA256,
        "Round180 verifier pre-import pin",
    )
    require(
        hashlib.sha256(read_regular(HERE / ROUND176_VERIFIER)).hexdigest()
        == ROUND176_VERIFIER_SHA256,
        "Round176 verifier pre-import pin",
    )
...
def wrap_round180_terminal(
    ...
    exact_volume = (
        root_volumes[terminal["root_depth14_cell_key"]]
        * Q(
    ...
    row = {
        ...
        "whole_closed_cell_excluded":
            terminal["coarse_disposition"] == "EXCLUDED",
...
def build_result() -> dict[str, Any]:
    require(
        ...
        "diagnostic probe import exclusion",
    )
    upstream = check_upstream()
    registry = reconstruct_registry(upstream)
    pure = rebuild_pure_target_cohort(upstream, registry)
    generalized = rebuild_generalized_cohort(registry)
    ...
    result = {
        ...
        "provenance": {
            ...
            "producer_sha256":
                hashlib.sha256(read_regular(Path(__file__))).hexdigest(),
