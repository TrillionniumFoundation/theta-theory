...
ROOT=Path(__file__).resolve().parent;P="cm2_round306c16b_source_g_relation_theorem_component_replay";RESULT=P+"_result.json";V=ROOT/(P+"_independent_verifier.py")
...
def main():
 q=Path(sys.argv[1]).resolve();base=json.loads((q/RESULT).read_bytes());attacks=[("positive",lambda d:d["relation_census"].__setitem__("positive",15223)),("sheet",lambda d:d["relation_census"].__setitem__("graph_to_sheet",5263)),("side",lambda d:d["relation_census"].__setitem__("graph_to_side",9959)),("negative",lambda d:d["relation_census"].__setitem__("negative_dispositions",167)),("pullback",lambda d:d["strict_nonpromotion"].__setitem__("representation_pullback",1)),("support",lambda d:d["strict_nonpromotion"].__setitem__("normalized_support",1)),("B1A",lambda d:d["strict_nonpromotion"].__setitem__("B1A",1)),("CM2",lambda d:d["strict_nonpromotion"].__setitem__("CM2","GO"))];rejected=[]
 for name,mutate in attacks:
  with tempfile.TemporaryDirectory(prefix="c16b-attack-")as raw:
   d=Path(raw)
   for f in q.iterdir():
