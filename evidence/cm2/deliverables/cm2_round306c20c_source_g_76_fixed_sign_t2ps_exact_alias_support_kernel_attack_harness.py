...
ROOT=Path(__file__).resolve().parent;P="cm2_round306c20c_source_g_76_fixed_sign_t2ps_exact_alias_support_kernel";R=P+"_result.json";V=ROOT/(P+"_independent_verifier.py")
...
def main():
 q=Path(sys.argv[1]).resolve();base=json.loads((q/R).read_bytes());att=[("credit",lambda d:d.__setitem__("alias_representation_equality_credit",75)),("cumulative",lambda d:d.__setitem__("cumulative_preserved_representation_equality_credit",163223)),("remaining",lambda d:d.__setitem__("remaining_preserved_alias_representation_debt",2519)),("rows",lambda d:d["ledger"].__setitem__("row_count",75)),("B1A",lambda d:d["strict_nonpromotion"].__setitem__("B1A",1)),("B2",lambda d:d["strict_nonpromotion"].__setitem__("B2",1)),("max",lambda d:d["strict_nonpromotion"].__setitem__("maximality",1)),("CM2",lambda d:d["strict_nonpromotion"].__setitem__("CM2","GO"))];rej=[]
 for name,mut in att:
  with tempfile.TemporaryDirectory(prefix="c20c-attack-")as raw:
   d=Path(raw)
   for f in q.iterdir():
