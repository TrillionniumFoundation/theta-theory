#!/usr/bin/env python3
"""Independently replay the fresh G2 relation admissibility and envelope audit."""
...
PRODUCER_RECORD: Final = {"filename": "cm2_round306c9_source_g_g2_relation_admissibility_and_envelope_audit_producer.py", "size": 45_059, "sha256": "ffc20e75c646bf5c887c05d9e9cd3cfa8795c62c60ba7e974d988f20d98d51b6"}
...
def parse_ledger_wire(wire: bytes, descriptor: dict[str, Any], expected_schema: str) -> list[dict[str, Any]]:
    need(len(wire) == descriptor["compressed_size"] and hashlib.sha256(wire).hexdigest() == descriptor["compressed_sha256"], "candidate ledger compressed binding")
    with gzip.GzipFile(fileobj=io.BytesIO(wire), mode="rb") as stream:
        plain = stream.read()
    need(len(plain) == descriptor["uncompressed_size"] and hashlib.sha256(plain).hexdigest() == descriptor["uncompressed_sha256"], "candidate ledger plain binding")
    rows = []
    for ordinal, line in enumerate(plain.splitlines(keepends=True)):
        need(line.endswith(b"\n"), "candidate ledger newline")
        row = json.loads(line)
        need(type(row) is dict and line == canonical(row) + b"\n" and row["schema"] == expected_schema, "candidate ledger canonical row:" + str(ordinal))
        body = dict(row)
        claimed = body.pop("row_sha256", None)
        need(type(claimed) is str and claimed == object_sha(body), "candidate ledger row closure:" + str(ordinal))
        rows.append(row)
    need(len(rows) == descriptor["row_count"], "candidate ledger row count")
    need(sequence_sha(row["row_id"] for row in rows) == descriptor["ordered_row_ids_sha256"], "candidate ledger row id commitment")
    need(sequence_sha(row["row_sha256"] for row in rows) == descriptor["ordered_row_hashes_sha256"], "candidate ledger row hash commitment")
    need(sequence_sha(rows) == descriptor["ordered_rows_sha256"], "candidate ledger ordered row commitment")
    return rows


def closed_result(raw: bytes) -> dict[str, Any]:
    value = json.loads(raw)
    ...
    need(value["schema"] == SCHEMA and value["status"] == STATUS, "candidate contract")
    need(value["producer_source"] == PRODUCER_RECORD, "candidate producer binding")
    census = value["audit_census"]
    need(census == {"positive_graph_domain_audit_rows": 5_264, "mechanical_relation_statement_rows": 15_392, "positive_graph_physical_relation_candidates": 15_382, "empty_graph_no_incidence_dispositions": 10, "remaining_physical_incidence_theorem_rows": 15_382, "remaining_pullback_theorem_rows": 15_382, "remaining_one_sided_trace_rows": 10_118}, "candidate audit census")
    correction = value["denominator_correction"]
    need(correction["C7_mechanical_statement_count"] == 15_392 and correction["C9_positive_graph_relation_candidate_count"] == 15_382 and correction["removed_empty_graph_statement_count"] == 10, "candidate denominator correction")
    need(correction["C8_15392_may_be_called_proved_physical_incidence_count"] is False, "candidate C8 boundary")
    scope = value["support_scope_correction"]
    need(scope["R235_target_positive_graph_count"] == 4_432 and scope["all_R235_target_positive_graphs_have_nonempty_partial_base_domain"] is True, "candidate partial graph scope")
    need(scope["R248_complete_closed_base_rectangles_are_outer_envelopes_for_these_graphs"] is True and scope["outer_envelope_may_be_promoted_to_full_support"] is False, "candidate envelope boundary")
    need(scope["rekey_requirement_decided_by_this_audit"] is False, "candidate rekey boundary")
    ...
    need(value["required_next"]["materialize_5264_exact_graph_support_ASTs"] is True and value["required_next"]["prove_15382_physical_incidence_relations"] is True, "candidate required next")
    return value


def validate_payloads(result_raw: bytes, graph_wire: bytes, relation_wire: bytes) -> dict[str, Any]:
    result = closed_result(result_raw)
    graph_rows = parse_ledger_wire(graph_wire, result["graph_domain_audit_ledger"], GRAPH_ROW_SCHEMA)
    relation_rows = parse_ledger_wire(relation_wire, result["relation_disposition_ledger"], RELATION_ROW_SCHEMA)
    need(len(graph_rows) == 5_264 and len(relation_rows) == 15_392, "candidate output census")
    need(Counter(row["graph_class"] for row in graph_rows) == result["graph_class_census"], "candidate graph class comparator")
    need(Counter(row["disposition"] for row in relation_rows) == result["relation_disposition_census"], "candidate relation disposition comparator")
    need(sum(row["positive_graph_physical_relation_candidate"] for row in relation_rows) == 15_382, "candidate relation candidate sum")
    need(sum(row["disposition"] == "EMPTY_GRAPH_NO_PHYSICAL_INCIDENCE" for row in relation_rows) == 10, "candidate no-incidence sum")
    need(all(row["formal_credit"] == ZERO_CREDIT for row in graph_rows + relation_rows), "candidate row zero credit")
    return result
...
class CandidateBundle:
    ...
    def __enter__(self) -> "CandidateBundle":
        ...
        expected = {GRAPH_LEDGER_NAME, RELATION_LEDGER_NAME, RESULT_NAME}
        need(stat.S_ISDIR(before.st_mode) and not self.directory.is_symlink() and set(os.listdir(self.directory)) == expected, "candidate exact directory")
        self.directory_descriptor = os.open(self.directory, os.O_RDONLY | os.O_DIRECTORY | os.O_NOFOLLOW | os.O_CLOEXEC)
        for filename in (GRAPH_LEDGER_NAME, RELATION_LEDGER_NAME, RESULT_NAME):
            ...
            need(digest == hash_fd(file_descriptor), "candidate two-pass digest:" + filename)
            ...
            self.payloads[filename] = read_fd(file_descriptor)
        validate_payloads(self.payloads[RESULT_NAME], self.payloads[GRAPH_LEDGER_NAME], self.payloads[RELATION_LEDGER_NAME])
        return self

    def final(self) -> None:
        for filename in (RESULT_NAME, RELATION_LEDGER_NAME, GRAPH_LEDGER_NAME):
...
def receipt(records: dict[str, CandidateRecord], expected: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "candidate_files": [records[name].__dict__ for name in (GRAPH_LEDGER_NAME, RELATION_LEDGER_NAME, RESULT_NAME)],
        "result_sha256": expected["result_sha256"],
        "positive_graph_domain_audit_rows": 5_264,
        "mechanical_relation_statement_rows": 15_392,
        "positive_graph_physical_relation_candidates": 15_382,
        "empty_graph_no_incidence_dispositions": 10,
        "R235_target_partial_base_graphs": 4_432,
...
def verify(candidate_dir: Path, expected_bundle: tuple[dict[str, Any], bytes, bytes] | None = None) -> dict[str, Any]:
    expected = build() if expected_bundle is None else expected_bundle
    with CandidateBundle(candidate_dir) as candidate:
        need(candidate.payloads[RESULT_NAME] == canonical(expected[0]), "candidate result byte identity")
        need(candidate.payloads[GRAPH_LEDGER_NAME] == expected[1], "candidate graph ledger byte identity")
        need(candidate.payloads[RELATION_LEDGER_NAME] == expected[2], "candidate relation ledger byte identity")
...
def rewrite_result(path: Path, mutator: Any) -> None:
    ...
    value = {**body, "result_sha256": object_sha(body)}
...
def attack_suite(candidate_dir: Path) -> dict[str, Any]:
    ...
    work = Path(tempfile.mkdtemp(prefix="cm2-c9-relation-audit-attacks-"))
    attacks = []
    ...
    def execute(attack_id: str, mutation: Any, cleanup: Any | None = None) -> None:
        ...
        try:
            ...
            except (Rejected, OSError, EOFError, gzip.BadGzipFile, json.JSONDecodeError, UnicodeDecodeError) as error:
    ...
    try:
        ...
        def hardlink_result() -> None:
            ...
            shutil.copyfile(candidate_dir / RESULT_NAME, target)
            os.link(target, work / RESULT_NAME)
        execute("A04_RESULT_HARDLINK", hardlink_result, lambda: (work.parent / (work.name + "-target")).unlink())
        execute("A05_SCHEMA", lambda: rewrite_result(work / RESULT_NAME, lambda value: value.__setitem__("schema", "foreign")))
        ...
        execute("A07_MECHANICAL_COUNT", lambda: rewrite_result(work / RESULT_NAME, lambda value: value["audit_census"].__setitem__("mechanical_relation_statement_rows", 15_393)))
        execute("A08_CANDIDATE_COUNT", lambda: rewrite_result(work / RESULT_NAME, lambda value: value["audit_census"].__setitem__("positive_graph_physical_relation_candidates", 15_383)))
        execute("A09_NO_INCIDENCE_COUNT", lambda: rewrite_result(work / RESULT_NAME, lambda value: value["audit_census"].__setitem__("empty_graph_no_incidence_dispositions", 9)))
        execute("A10_PARTIAL_GRAPH_COUNT", lambda: rewrite_result(work / RESULT_NAME, lambda value: value["support_scope_correction"].__setitem__("R235_target_positive_graph_count", 4_431)))
        execute("A11_PROMOTE_C8_DENOMINATOR", lambda: rewrite_result(work / RESULT_NAME, lambda value: value["denominator_correction"].__setitem__("C8_15392_may_be_called_proved_physical_incidence_count", True)))
        execute("A12_PROMOTE_OUTER_ENVELOPE", lambda: rewrite_result(work / RESULT_NAME, lambda value: value["support_scope_correction"].__setitem__("outer_envelope_may_be_promoted_to_full_support", True)))
        execute("A13_DECIDE_REKEY", lambda: rewrite_result(work / RESULT_NAME, lambda value: value["support_scope_correction"].__setitem__("rekey_requirement_decided_by_this_audit", True)))
        execute("A14_PHYSICAL_CREDIT", lambda: rewrite_result(work / RESULT_NAME, lambda value: value["formal_credit"].__setitem__("physical_incidence", 1)))
        execute("A15_B1A_CREDIT", lambda: rewrite_result(work / RESULT_NAME, lambda value: value.__setitem__("B1A_permitted", True)))
        execute("A16_GRAPH_LEDGER_MUTATION", lambda: flip_last_byte(work / GRAPH_LEDGER_NAME))
        execute("A17_RELATION_LEDGER_MUTATION", lambda: flip_last_byte(work / RELATION_LEDGER_NAME))
        execute("A18_GRAPH_LEDGER_COUNT", lambda: rewrite_result(work / RESULT_NAME, lambda value: value["graph_domain_audit_ledger"].__setitem__("row_count", 5_263)))
        execute("A19_RELATION_LEDGER_COUNT", lambda: rewrite_result(work / RESULT_NAME, lambda value: value["relation_disposition_ledger"].__setitem__("row_count", 15_391)))
        execute("A20_DISABLE_REMATERIALIZATION", lambda: rewrite_result(work / RESULT_NAME, lambda value: value["required_next"].__setitem__("materialize_5264_exact_graph_support_ASTs", False)))
    finally:
        shutil.rmtree(work, ignore_errors=True)
    need(len(attacks) == 20, "attack count")
    body = {"schema": SCHEMA + ".coherent-attack-suite.v1", "status": "PASS_20_OF_20_COHERENT_ATTACKS_REJECTED", "verifier_filename": Path(__file__).name, "verifier_file_sha256": self_sha(), "attack_count": 20, "rejected_count": 20, "all_rejected": True, "baseline_result_file_sha256": next(row["sha256"] for row in baseline["candidate_files"] if row["filename"] == RESULT_NAME), "attacks": attacks}
...
def verification_doc(result: dict[str, Any], attack_raw: bytes | None = None, verifier_digest: str | None = None) -> dict[str, Any]:
    if attack_raw is None:
        suite, raw = read_closed(ROOT / ATTACK_NAME, "attack_suite_sha256")
    else:
        raw = attack_raw
        suite = json.loads(raw)
        need(type(suite) is dict and raw == canonical(suite), "held attack canonical document")
        body = dict(suite)
        claimed = body.pop("attack_suite_sha256", None)
        need(type(claimed) is str and claimed == object_sha(body), "held attack closure")
    expected_verifier_digest = self_sha() if verifier_digest is None else verifier_digest
    need(suite["verifier_file_sha256"] == expected_verifier_digest and (suite["attack_count"], suite["rejected_count"], suite["all_rejected"]) == (20, 20, True), "attack binding")
    body = {"schema": SCHEMA + ".verification.v1", **result, "attack_suite": {"filename": ATTACK_NAME, "file_sha256": hashlib.sha256(raw).hexdigest(), "object_self_sha256": suite["attack_suite_sha256"], "attack_count": 20, "rejected_count": 20, "all_rejected": True}, "formal_credit_marker": ZERO_CREDIT}
    return {**body, "verification_sha256": object_sha(body)}


MANIFEST_MEMBERS: Final = (PRODUCER_RECORD["filename"], GRAPH_LEDGER_NAME, RELATION_LEDGER_NAME, RESULT_NAME, Path(__file__).name, ATTACK_NAME, VERIFICATION_NAME, REPORT_NAME, COLD_NAME)
...
class ManifestBundle:
    ...
    def final(self) -> None:
        ...
        need(identity(os.fstat(self.manifest_descriptor)) == self.manifest_identity and identity(os.stat(MANIFEST_NAME, dir_fd=self.directory_descriptor, follow_symlinks=False)) == self.manifest_identity, "manifest final identity")
...
def manifest_first() -> dict[str, Any]:
    with ManifestBundle() as bundle:
        ...
        validate_payloads(bundle.payloads[RESULT_NAME], bundle.payloads[GRAPH_LEDGER_NAME], bundle.payloads[RELATION_LEDGER_NAME])
        expected = build()
        need(bundle.payloads[RESULT_NAME] == canonical(expected[0]), "manifest result byte identity")
        need(bundle.payloads[GRAPH_LEDGER_NAME] == expected[1], "manifest graph ledger byte identity")
        need(bundle.payloads[RELATION_LEDGER_NAME] == expected[2], "manifest relation ledger byte identity")
        records = {filename: CandidateRecord(filename, bundle.identities[filename][4], bundle.hashes[filename]) for filename in (GRAPH_LEDGER_NAME, RELATION_LEDGER_NAME, RESULT_NAME)}
        result = receipt(records, expected[0])
        need(bundle.payloads[VERIFICATION_NAME] == canonical(verification_doc(result, bundle.payloads[ATTACK_NAME], bundle.hashes[Path(__file__).name])), "manifest verification byte identity")
        bundle.final()
    return {"status": "PASS_MANIFEST_FIRST_HELD_FD_5264_GRAPH_DOMAIN_AND_15392_RELATION_AUDIT__15382_CANDIDATES_10_NO_INCIDENCE__ZERO_WRITES_ZERO_SUPPORT_CREDIT", "manifest_filename": MANIFEST_NAME, "manifest_member_count": len(MANIFEST_MEMBERS), "result_sha256": expected[0]["result_sha256"], "positive_graph_domain_audit_rows": 5_264, "mechanical_relation_statement_rows": 15_392, "positive_graph_physical_relation_candidates": 15_382, "empty_graph_no_incidence_dispositions": 10, "deliverables_write_syscalls": 0, "formal_credit": ZERO_CREDIT}
    ...
    if arguments.manifest_first_no_write:
        ...
        if arguments.attack_publish:
            ...
            if arguments.verify_publish:
                publish_document(VERIFICATION_NAME, verification_doc(result))
