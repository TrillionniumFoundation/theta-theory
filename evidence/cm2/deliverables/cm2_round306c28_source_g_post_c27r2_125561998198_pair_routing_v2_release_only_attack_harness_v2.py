...
def recursive_files(directories: list[Path]) -> list[Path]:
    ...
    # ``need`` deliberately accepts only a real bool.  Returning ``unique``
    # from the right-hand side would make a valid nonempty projection a list
    # and therefore reject it despite exact disjointness.
    need(len(unique) == len(paths) and bool(unique),
         "disjoint nonempty projection")
    return unique
