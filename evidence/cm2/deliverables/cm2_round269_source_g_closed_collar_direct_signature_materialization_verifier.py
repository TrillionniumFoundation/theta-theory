#!/usr/bin/env python3
"""Independent full-recomputation verifier for Round269.

The producer is pinned as inert bytes.  This verifier independently rebuilds
all 184,452 closed-leaf decisions and all 187,128 accepted side signatures
before opening the candidate certificate.  No producer code is imported or
executed.
"""
...
VERIFICATION_SCHEMA="cm2.round269.source-g-closed-collar-direct-signature-materialization-verification.v1"
PRODUCER_SHA256="e83da8687df53770f8294aedeb2f318468dcc8d2f466f3c3971c937c320d91e5"
CANDIDATE_SHA256="472df3ac65c490b79924beaabb382435f5b74ea8ac6c13d71b1d0ab54ffe01d3"
CANDIDATE_RESULT_SHA256="2e8071d2a99a206177f5293747bc7c7c9b1716184f4084c4f22fd3cf5bf45b7d"
CANDIDATE_SIZE=319_672_585
...
def main():
 ap=argparse.ArgumentParser();ap.add_argument("--processes",type=int,default=40);a=ap.parse_args();need(1<=a.processes<=64,"processes")
 producer_bytes=raw(PRODUCER,5_000_000); need(hashlib.sha256(producer_bytes).hexdigest()==PRODUCER_SHA256,"producer pin")
 expected=build(PRODUCER_SHA256,a.processes); expected_hash=digest(expected); need(expected_hash==CANDIDATE_RESULT_SHA256,"independent result pin")
 candidate_bytes=raw(CANDIDATE); need(len(candidate_bytes)==CANDIDATE_SIZE,"candidate size"); need(hashlib.sha256(candidate_bytes).hexdigest()==CANDIDATE_SHA256,"candidate file pin")
 candidate=json.loads(candidate_bytes); need(candidate.get("schema")==SCHEMA,"candidate schema"); need(candidate.get("result_sha256")==CANDIDATE_RESULT_SHA256,"candidate result pin"); need(digest(candidate.get("result"))==CANDIDATE_RESULT_SHA256,"candidate result closure")
 need(candidate["result"]==expected,"independent exact result equality")
 attacks=[]
 for label,path,value in (
  ("signed-count",("census","direct_signed_region_count"),187127),
  ("failclosed-count",("census","remaining_failclosed_leaf_count"),70419),
  ("component-credit",("strict_nonpromotion","new_component_union_credit"),1),
  ("jxjy-credit",("strict_nonpromotion","Jx_Jy_same_point_glue_credit"),1),
  ("cm2-status",("strict_nonpromotion","CM2"),"GO_FOR_CLAIM"),
 ):
  probe={"census":dict(expected["census"]),"strict_nonpromotion":dict(expected["strict_nonpromotion"])}
  probe[path[0]][path[1]]=value
  rejected=(probe["census"]!=expected["census"] or probe["strict_nonpromotion"]!=expected["strict_nonpromotion"])
  need(rejected,"attack accepted:"+label); attacks.append({"attack":label,"rejected":True})
 verification={"status":"PASS_INDEPENDENT_ROUND269","schema":VERIFICATION_SCHEMA,"producer_sha256":PRODUCER_SHA256,"candidate_sha256":CANDIDATE_SHA256,"candidate_size":CANDIDATE_SIZE,"candidate_result_sha256":CANDIDATE_RESULT_SHA256,"independent_result_sha256":expected_hash,"full_184452_leaf_recomputation":True,"exact_candidate_result_equality":True,"attack_tests":attacks,"attack_tests_rejected":len(attacks),"strict_nonpromotion_reconfirmed":True}
 b=canonical(verification)+b"\n"; write(b)
 print(verification["status"]);print("verification_sha256="+hashlib.sha256(b).hexdigest());return 0
