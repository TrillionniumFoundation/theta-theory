...
OUTPUT = HERE / "cm2-round158-upper-endpoint-continuation-to-320h-2026-07-25.json"
SCHEMA = "cm2.round158.upper-endpoint-continuation-to-320h.v1"
...
def check_pins():
    ...
    require(
        prior["schema"] == "cm2.round157.upper-endpoint-continuation-to-288h.v1",
        "prior schema",
    )
...
def build_result():
    ...
    for row in rows:
        ...
        previous = row
    require(
        rows[0]["abs_delta_x_in_h_units"][0] == "288"
        and rows[-1]["abs_delta_x_in_h_units"][1] == "320",
        "endpoints",
    )
    count = len(rows)
    r1648_tests = 2 * count * 161 * 1648
    d3_tests = count * (2 * 161 + 161)
    return {
        "status": STATUS,
        "physical_coordinate_scale": "h=2^-4296",
        "inherited_round157_result_sha256": digest(prior),
        "certified_abs_delta_x_corridor_in_h_units": ["0", "320"],
        "new_upper_continuation": {
            "new_typed_slab_count": count,
            "new_typed_box_count": 3 * count,
            "new_rows": rows,
            "new_rows_sha256": digest(rows),
            "exact_face_adjacencies_from_288h_through_320h": adjacencies,
            "new_event_family_count": 0,
            "upper_endpoint_continuation_complete": False,
        },
        "combined_typed_atlas": {
            "slab_count": 57 + count,
            "typed_box_count": 3 * (57 + count),
            "connected": True,
