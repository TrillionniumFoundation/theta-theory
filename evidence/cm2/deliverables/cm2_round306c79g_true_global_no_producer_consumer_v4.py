#!/usr/bin/env python3
"""C79g v4 append-only zero-credit candidate builder.

This append-only v4 binds the independently verified C78s 13-member final
...
is permanently non-authoritative.  The separately committed v4 seal plus the
...
COLD_COORDINATION_PARENT_FD_ENV = "CM2_C79G_V4_COORDINATION_PARENT_FD"
...
SELF = OUT / "cm2_round306c79g_true_global_no_producer_consumer_v4.py"
...
V3_REJECTION = ROOT / (
    ...
    "rejection.json")
V3_TO_V4_TRANSITION = OUT / "cm2_round306c79g_true_global_no_producer_consumer_v3_to_v4_static_launch_transition_receipt_v1.json"
STATIC_AUDIT_V4 = OUT / "cm2_round306c79g_true_global_no_producer_consumer_static_audit_v4.json"
COLD_LAUNCHER = OUT / "cm2_round306c79g_true_global_no_producer_consumer_cold_launch_v4.py"
COLD_MANIFEST = OUT / "cm2_round306c79g_true_global_no_producer_consumer_cold_launch_manifest_v4.sha256"
COLD_OUTER = OUT / "cm2_round306c79g_true_global_no_producer_consumer_cold_launch_outer_receipt_v4.json"
...
COLD_EXACT8 = (
    ...
    CONTRACT,
    SELF,
    OUT / "cm2_round306c79g_true_global_no_producer_consumer_independent_verifier_assembler_authority_consumer_v4.py",
    V3_TO_V4_TRANSITION,
    STATIC_AUDIT_V4,
    COLD_LAUNCHER,
)
...
def _linux_libc() -> ctypes.CDLL:
    need(sys.platform.startswith("linux"), "Linux-only v4 publication protocol")
...
class HeldSelf:
    ...
    def __init__(self) -> None:
        need(SELF == OUT / "cm2_round306c79g_true_global_no_producer_consumer_v4.py",
        ...
        need(stat.S_ISREG(before_path.st_mode) and not SELF.is_symlink() and
             ...
             "SELF exact frozen-mode regular path")
...
def new_candidate_stage(outdir: Path, parent: HeldParent) -> tuple[Path, HeldMutableStage]:
    ...
    name = ".c79g-v4-candidate-stage-" + orientation + "-" + CHECKPOINT_OBJECT_PIN
...
def hold_static_freeze_trust(self_guard: HeldSelf) -> dict[str, Any]:
    ...
    manifest_guard = HeldPinnedInput(
        COLD_MANIFEST, "C79g v4 cold-launch manifest", 0o444)
    outer_guard = HeldPinnedInput(
        COLD_OUTER, "C79g v4 cold-launch outer-last receipt", 0o444)
    manifest_entries = parse_manifest_ordered(
        manifest_guard.raw, "C79g v4 cold-launch exact8 manifest")
    ...
    need(len(manifest_by_path) == 8 and
         ...
         manifest_by_path[V3_REJECTION] == V3_REJECTION_FILE_PIN,
    ...
    for path in COLD_EXACT8:
        if path != SELF:
            by_path[path] = HeldPinnedInput(
                path, "C79g v4 cold exact8:" + path.name, 0o444,
    ...
    transition_guard = by_path[V3_TO_V4_TRANSITION]
    audit_guard = by_path[STATIC_AUDIT_V4]
    transition = strict_json(transition_guard.raw, "C79g v4 transition trust")
    audit = strict_json(audit_guard.raw, "C79g v4 static audit trust")
    outer = strict_json(outer_guard.raw, "C79g v4 cold-launch outer trust")
    ...
    verify_object(transition, "C79g v4 transition trust")
    verify_object(audit, "C79g v4 static audit trust")
    verify_object(outer, "C79g v4 cold-launch outer trust")
    successor = transition.get("successor_v4_static_bundle", {})
    predecessor = transition.get("frozen_predecessor_v3", {})
    ...
    transition_pin = audited.get("v3_to_v4_transition_receipt", {})
    ...
    need(transition.get("status") ==
             "FROZEN_STATIC_NO_RUN_APPEND_ONLY_V3_TO_V4__ALL_PINS_CLOSED__RUNTIME_DEFERRED" and
         audit.get("status") ==
             "PASS_STATIC_NO_RUN_AUDIT_V4__ALL_BYTES_PINS_MODES_AND_COMPOSITE_BOUNDARIES_CLOSED" and
...
def ensure_launch_configuration() -> None:
    ...
    need(source_fd_text.isdigit() and int(source_fd_text) >= 3 and
         ...
         SELF == ROOT / "deliverables/cm2_round306c79g_true_global_no_producer_consumer_v4.py",
    ...
    need(all(isinstance(value, str) and len(value) == 64 and
         ...
         "C79g v4 exact non-placeholder contract/schema/rejection pins")
    need(set(C78S_FINAL["pins"]) == set(C78S_NAMES) and
         ...
         "C79g v4 exact filled launch configuration")
...
def validate_static_authorities(static_trust: dict[str, Any]) -> dict[str, Any]:
    ...
    need(contract.get("effective_checkpoint_object_sha256") == CHECKPOINT_OBJECT_PIN and
         contract.get("status") ==
             "FROZEN_APPEND_ONLY_V4__COLD_LAUNCHED_WRAPPER_ONLY__STATIC_NO_RUN_FREEZE" and
         contract.get("v4_bundle", {}).get(
             ...
             "independent_verifier_assembler_authority_consumer_v4.py" and
         contract.get("v4_bundle", {}).get("closed_schema_validator_policy") == {
         ...
         "v4 frozen contract checkpoint/composite-only boundary")
    ...
    need(closed.get("$ref") == "#/$defs/coldLaunchedCommittedAuthority" and
         ...
         "append-only v4 cold root, zero-credit inner, and persisted zero boundary")
...
def input_registry(state: dict[str, Any], self_file_sha256: str,
    ...
    return close_object({
        ...
        "post_source_static_freeze_trust": {
            "v3_to_v4_transition_receipt": {
                "path": str(V3_TO_V4_TRANSITION.relative_to(ROOT)),
                "file_sha256": static_trust["transition_file_sha256"],
                "object_sha256": static_trust["transition_object_sha256"],
            },
            "static_audit_v4": {
                "path": str(STATIC_AUDIT_V4.relative_to(ROOT)),
...
def build(outdir: Path, self_guard: HeldSelf,
    ...
    lock = (
        "C79g v4 isolated candidate only; standalone non-authoritative. Formal-global credit "
