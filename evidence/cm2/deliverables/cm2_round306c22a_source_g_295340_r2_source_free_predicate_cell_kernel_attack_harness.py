...
ROOT=Path(__file__).resolve().parent;P="cm2_round306c22a_source_g_295340_r2_source_free_predicate_cell_kernel";R=P+"_result.json";V=ROOT/(P+"_independent_verifier.py");PY=ROOT.parent/".venv-cm2/bin/python"
...
def main():
 q=Path(sys.argv[1]).resolve();base=json.loads((q/R).read_bytes());attacks=[("cell_credit",lambda d:d.__setitem__("predicate_cell_support_set_equality_credit",295339)),("R2_member_credit",lambda d:d.__setitem__("R2_member_normalized_support_credit",1)),("R2_debt",lambda d:d.__setitem__("remaining_R2_member_support_debt",295335)),("global_debt",lambda d:d.__setitem__("remaining_global_member_support_debt",320131)),("DSU",lambda d:d["strict_nonpromotion"].__setitem__("new_DSU_edges",1)),("B1A",lambda d:d["strict_nonpromotion"].__setitem__("B1A",1)),("B2",lambda d:d["strict_nonpromotion"].__setitem__("B2",1)),("CM2",lambda d:d["strict_nonpromotion"].__setitem__("CM2","GO"))];rejected=[]
 for name,mutate in attacks:
  with tempfile.TemporaryDirectory(prefix="c22a-attack-")as raw:
   target=Path(raw)
