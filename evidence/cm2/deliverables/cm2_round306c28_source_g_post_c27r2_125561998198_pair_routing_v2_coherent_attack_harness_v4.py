...
def copy_regular_member(source: Path, target: Path) -> None:
    regular_single(source, "authoritative cleanup source")
    source_fd = os.open(
    ...
    try:
        ...
        os.close(target_fd)
        os.close(source_fd)
    regular_single(target, "normalized private route clone")
    need(file_hash(target) == file_hash(source),
         "normalized private route bytes")


def normalize_private_link_clones(
    work: Path, candidate: Path, rejected: list[dict[str, Any]],
) -> list[dict[str, Any]]:
    """Normalize only attacks 17/18 after every rejection hash is recorded."""
    ...
    authoritative = candidate / ROUTE_FILE
    authoritative_sha = file_hash(authoritative)
    need(len(rejected) == 26 and len(by_ordinal) == 26,
         "all 26 rejection records closed before cleanup")
    for ordinal, name, kind in cases:
        record = by_ordinal.get(ordinal)
        need(type(record) is dict and record.get("name") == name
             and record.get("exit_code") == 2
             and record.get("verification_output_created") is False
             and is_hash(record.get("stderr_sha256")),
             "link rejection hash closed before cleanup:" + name)
        attack_root = work / f"attack-{ordinal:02d}-{name}"
        private = attack_root / "candidate"
        target = private / ROUTE_FILE
        if kind == "symlink":
            info = os.lstat(target)
            need(stat.S_ISLNK(info.st_mode),
                 "expected private symlink attack clone")
            pre_lstat_kind = "symlink"
            pre_lstat_nlink = info.st_nlink
            target.unlink()
        else:
            ...
            need(stat.S_ISREG(first.st_mode) and stat.S_ISREG(second.st_mode)
                 ...
                 "expected private two-link attack clone")
            pre_lstat_kind = "regular_hardlink"
            pre_lstat_nlink = first.st_nlink
            target.unlink()
            sibling.unlink()
        copy_regular_member(authoritative, target)
        post = os.lstat(target)
        records.append({
            "ordinal": ordinal, "name": name,
            "rejection_stderr_sha256": record["stderr_sha256"],
            "pre_lstat_kind": pre_lstat_kind,
            "pre_lstat_nlink": pre_lstat_nlink,
            "nonregular_private_clone_removed_after_rejection": True,
            "normalized_route_sha256": file_hash(target),
            "post_lstat_kind": "regular",
            "post_lstat_nlink": post.st_nlink,
            "authoritative_route_sha256": authoritative_sha,
            "normalized_bytes_equal_authoritative":
                file_hash(target) == authoritative_sha,
        })
    return records
    ...
    for current, directory_names, file_names in os.walk(root):
        ...
        for name in directory_names:
            ...
            need(stat.S_ISDIR(info.st_mode) and not path.is_symlink(),
                 "non-symlink private directory:" + str(path))
            directory_count += 1
        for name in file_names:
            ...
            need(stat.S_ISREG(info.st_mode) and info.st_nlink == 1
                 ...
                 "regular single-link private output:" + str(path))
            file_count += 1
...
def main_run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    authoritative_candidate_pre = {
        name: file_hash(candidate / name)
        for name in (BLOCK_FILE, ROUTE_FILE, RESULT_FILE)
    }
    ...
    link_cleanup = normalize_private_link_clones(work, candidate, rejected)
    normalized_tree = strict_regular_tree(work)
    authoritative_candidate_post = {
        name: file_hash(candidate / name)
        for name in (BLOCK_FILE, ROUTE_FILE, RESULT_FILE)
    }
    need(authoritative_candidate_pre == authoritative_candidate_post,
         "authoritative candidate unchanged by private attack cleanup")
    ...
    body = {
        ...
        "post_rejection_private_link_clone_cleanup": {
            "performed_after_all_26_rejection_error_hashes_recorded": True,
            ...
            "successful_scratch_tree": normalized_tree,
        },
    ...
    need(strict_regular_tree(work) == normalized_tree,
         "final post-receipt private work tree remains all regular single-link")
    return result
...
def self_test() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="cm2-c28-v2-attacks-") as raw:
        ...
        for ordinal, name, kind in (
            ...
            route = private / ROUTE_FILE
            if kind == "symlink":
                os.symlink(source / ROUTE_FILE, route)
            else:
                sibling = attack_root / "hardlink-source-route.jsonl.gz"
                sibling.write_bytes((source / ROUTE_FILE).read_bytes())
                os.link(sibling, route)
            rejected.append({
        ...
        rejected.extend({
            "ordinal": ordinal, "name": f"fixture-{ordinal}", "exit_code": 2,
            "stderr_sha256": hashlib.sha256(str(ordinal).encode("ascii")).hexdigest(),
            "verification_output_created": False,
        } for ordinal in range(26) if ordinal not in {17, 18})
        rejected.sort(key=lambda record: record["ordinal"])
        cleanup = normalize_private_link_clones(work, source, rejected)
        tree = strict_regular_tree(work)
        need(len(cleanup) == 2
             and tree["all_files_regular_single_link"] is True
             and all(item["normalized_bytes_equal_authoritative"]
                     for item in cleanup),
             "post-rejection symlink/hardlink cleanup fixture")
    return {
        "schema": HARNESS_SCHEMA + ".self-test",
        "status": "PASS_SMALL_FIXTURE_CLONE_AND_POST_REJECTION_LINK_CLEANUP_TESTS",
