#!/usr/bin/env python3
"""Fail-closed process transaction runner for C28 pair-routing-v2 stages.
...
def failure_receipt(
    ...
    if not (run_dir / "FAILED.lock").exists():
        write_once(run_dir / "FAILED.lock",
                   b"FAILED_CLOSED_C28_PAIR_ROUTING_V2_PROCESS_TRANSACTION\n")
    return result
...
def execute(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        ...
        need(stage in {
            "authority_adapter", "authority_adapter_verifier",
        ...
        write_once(run_dir / "PASS.lock",
                   ("PASS_C28_PAIR_ROUTING_V2_" + stage.upper()
...
def parser() -> argparse.ArgumentParser:
    ...
    value.add_argument("--stage", choices=(
        "authority_adapter", "authority_adapter_verifier", "producer",
        "independent_verifier", "coherent_attacks"))
    ...
    try:
        ...
        if args.self_test:
            need(all(getattr(args, field) is None for field in fields),
                 "self-test accepts no transaction arguments")
            body = {"schema": ATTESTATION_SCHEMA + ".self-test",
                    "formal_credit": 0, "CM2": "NO-GO_FOR_CLAIM"}
            need(digest(body) == hashlib.sha256(canonical(body)).hexdigest(),
                 "canonical digest fixture")
            result = {"stage": "self_test", "status":
                      "PASS_TRANSACTION_RUNNER_CANONICAL_AND_STAGE_FIXTURE"}
        else:
            need(all(getattr(args, field) is not None for field in fields),
                 "all transaction arguments required")
            result = execute(args)
