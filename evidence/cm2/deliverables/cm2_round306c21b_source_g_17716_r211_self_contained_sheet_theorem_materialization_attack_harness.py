...
ROOT=Path(__file__).resolve().parent;P="cm2_round306c21b_source_g_17716_r211_self_contained_sheet_theorem_materialization";R=P+"_result.json";V=ROOT/(P+"_independent_verifier.py");PY=ROOT.parent/".venv-cm2/bin/python"
...
def main():
 q=Path(sys.argv[1]).resolve();base=json.loads((q/R).read_bytes());attacks=[("rows",lambda d:d.__setitem__("sheet_theorem_materialization_credit",17715)),("A1",lambda d:d.__setitem__("A1_obligation_discharge_credit",1)),("debt",lambda d:d.__setitem__("remaining_preserved_A1_A2_obligation_debt",79083)),("stereo",lambda d:d["proof_chart_census"].__setitem__("STEREOGRAPHIC_TUS",255)),("DSU",lambda d:d["strict_nonpromotion"].__setitem__("new_DSU_edges",1)),("B1A",lambda d:d["strict_nonpromotion"].__setitem__("B1A",1)),("B2",lambda d:d["strict_nonpromotion"].__setitem__("B2",1)),("CM2",lambda d:d["strict_nonpromotion"].__setitem__("CM2","GO"))];rejected=[]
 for name,mutate in attacks:
  with tempfile.TemporaryDirectory(prefix="c21b-attack-")as raw:
   target=Path(raw)
