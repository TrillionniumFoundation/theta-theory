...
OUTPUT = HERE / "cm2-round159-upper-endpoint-continuation-to-352h-2026-07-25.json"
SCHEMA = "cm2.round159.upper-endpoint-continuation-to-352h.v1"
...
def check_pins():
    ...
    require(
        ...
        and prior["schema"] == "cm2.round158.upper-endpoint-continuation-to-320h.v1",
        "prior schema",
    )
...
def build_result():
    ...
    for row in rows:
        ...
        previous = row
    require(
        rows[0]["abs_delta_x_in_h_units"][0] == "320"
        and rows[-1]["abs_delta_x_in_h_units"][1] == "352",
        "endpoints",
    )
    count = len(rows)
    ...
    return {
        ...
        "inherited_round158_result_sha256": digest(prior),
        "inherited_round158_verification_result_sha256": prior_verification[
            "result_sha256"
        ],
        "certified_abs_delta_x_corridor_in_h_units": ["0", "352"],
        "new_upper_continuation": {
            "new_typed_slab_count": count,
            "new_typed_box_count": 3 * count,
            "new_rows": rows,
            "new_rows_sha256": digest(rows),
            "exact_face_adjacencies_from_320h_through_352h": adjacencies,
            "new_event_family_count": 0,
            "upper_endpoint_continuation_complete": False,
        },
        "combined_typed_atlas": {
            "slab_count": prior_atlas["slab_count"] + count,
            "typed_box_count": prior_atlas["typed_box_count"] + 3 * count,
            "connected": True,
        ...
        "audit_census": {
            ...
            "new_collision_stage_object_pairs": collision_pairs,
