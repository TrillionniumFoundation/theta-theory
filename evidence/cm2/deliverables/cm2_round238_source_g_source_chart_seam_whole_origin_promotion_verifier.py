...
def verify()->dict[str,Any]:
 ...
 r230=load("cm2_round230_source_g_resolved_retained_bulk_continuation_certificate.json");acc={r["Round220_split_interface_id"]for r in r230["formal_certified_local_bulk_bridge_star_ledger"]["rows"]};roots=[(i,r)for i,r in ints if i["split_interface_id"]not in acc and r["reason_labels"]==["source_chart_seam"]];need(len(roots)==264,"universe");candidate=load("cm2_round238_source_g_source_chart_seam_whole_origin_promotion_certificate.json");got={r["Round220_split_interface_id"]:r for r in candidate["whole_origin_promotion_rows"]};charts=Counter();keys=set()
 for i,r in roots:
  iid=i["split_interface_id"];o=orig[r["origin_row_id"]];normal=seams[r["origin_row_id"]];need(normal["face_classification"]=="FULL_BASE_UNIQUE_GRAPH"and normal["exact_dimension"]==2,"normal");box=r179.box_from(r["box"],len(r["refinement_path"]),r["row_id"]);computed,reasons=r179.r174.certify_signature(r["chart"],box,o["owner_target"],reg);need(computed is not None and reasons==[],"signature");sid=i["lower_child_row_id"]if i["lower_child_kind"]=="RESOLVED"else i["upper_child_row_id"];signature=ksig(r["chart"],o["owner_target"],computed);need(signature==sig(res[sid]),"sibling");cell=r["chart"].split(":")[1];owner="E_OR_W_HALF_OPEN_OWNER"if cell in{"E","W"}else"N_OR_S_EXCLUDES_DIAGONAL_TIE";expected={"whole_origin_promotion_row_id":"round238-source-seam:"+dg(iid),"Round220_split_interface_id":iid,"origin_row_id":r["origin_row_id"],"Round179_resolved_sibling_row_id":sid,"Round179_retained_child_row_id":r["row_id"],"source_chart":r["chart"],"seam_equation":"2*t^2-1=0","seam_strict_t_gradient_sign":normal["gradient_sign"],"seam_face_classification":"FULL_BASE_UNIQUE_GRAPH","physical_inside_predicate":"2*t^2-1<0","coordinate_guard_predicate":"2*t^2-1>0","half_open_seam_owner_status":owner,"seam_dimension":2,"seam_three_dimensional_coordinate_volume":0,"whole_box_signature_recomputation_ignoring_chart_class_precheck":signature,"whole_origin_local_return_signature_credit":1,"coordinate_guard_global_credit":0,"known_block_incidence_credit":0,"global_exact_key_fibre_credit":0};need(got.get(iid)==expected,"row");charts[r["chart"]]+=1;keys.add(signature["official_key_ordinal"])
 need(candidate["whole_origin_promotion_rows_sha256"]==dg(candidate["whole_origin_promotion_rows"]),"hash");need(candidate["census"]["source_chart_histogram"]==dict(charts)and candidate["census"]["distinct_exact_key_count"]==len(keys),"census");need(candidate["strict_nonpromotion"]["CM2"]=="NO-GO_FOR_CLAIM","no-go");return{"status":"PASS_INDEPENDENT_ROUND238","candidate_sha256":P["cm2_round238_source_g_source_chart_seam_whole_origin_promotion_certificate.json"],"candidate_result_sha256":dg(candidate),"producer_imported_or_executed":False,"independently_recomputed_root_count":264,"distinct_exact_key_count":len(keys),"source_chart_histogram":dict(charts),"strict_nonpromotion_reconfirmed":True}
def main()->int:
 p=argparse.ArgumentParser();p.add_argument("--no-write",action="store_true");x=p.parse_args();r=verify();d={"schema":"cm2.round238.source-g-source-chart-seam-whole-origin-promotion.verification.v1","result":r,"result_sha256":dg(r)};b=can(d)+b"\n"
