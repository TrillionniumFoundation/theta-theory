...
ROOT=Path(__file__).resolve().parent;P="cm2_round306c19c_source_g_33344_empty_graph_surviving_side_support_kernel";R=P+"_result.json";V=ROOT/(P+"_independent_verifier.py")
...
def main():
 q=Path(sys.argv[1]).resolve();base=json.loads((q/R).read_bytes());att=[("member",lambda d:d.__setitem__("member_support_credit",33343)),("rep",lambda d:d.__setitem__("primary_representation_equality_credit",33343)),("remaining",lambda d:d.__setitem__("remaining_nongraph_debt",4431)),("rows",lambda d:d["ledger"].__setitem__("row_count",33343)),("B1A",lambda d:d["strict_nonpromotion"].__setitem__("B1A",1)),("B2",lambda d:d["strict_nonpromotion"].__setitem__("B2",1)),("max",lambda d:d["strict_nonpromotion"].__setitem__("maximality",1)),("CM2",lambda d:d["strict_nonpromotion"].__setitem__("CM2","GO"))];rej=[]
 for name,mut in att:
  with tempfile.TemporaryDirectory(prefix="c19c-attack-")as raw:
   d=Path(raw)
   for f in q.iterdir():
