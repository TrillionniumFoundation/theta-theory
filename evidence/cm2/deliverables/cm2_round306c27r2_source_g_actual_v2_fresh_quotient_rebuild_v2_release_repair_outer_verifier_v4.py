#!/usr/bin/env python3
"""No-import streaming outer verifier for the C27R2 release repair v4.
...
EVIDENCE_SCHEMA = BASE + "post-integrity-evidence-closure.v4"
EVIDENCE_STATUS = (
...
def execute(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for ordinal, line in enumerate(evidence_rows_raw.splitlines()):
        ...
        need(type(row) is dict and canonical(row) == line
             and row.get("schema") == BASE + "post-authority-member-row.v4"
             and row.get("ordinal") == ordinal
...
def self_test() -> dict[str, Any]:
    ...
    return {"status": "PASS_C27R2_RELEASE_REPAIR_OUTER_V4_SELF_TEST"}
