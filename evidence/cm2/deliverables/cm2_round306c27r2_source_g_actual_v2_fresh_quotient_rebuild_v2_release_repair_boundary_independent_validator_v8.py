...
OVERRIDE_SCHEMA = PREFIX + "release-repair-boundary-override-map.v8"
OUTPUT_SCHEMA = PREFIX + "release-boundary-validator.v8"
FORMAL_STATUS = (
    "PASS_C27R2_RELEASE_REPAIR_BOUNDARY_V8_FORMAL_EMPTY_OVERRIDE_EXACT_"
    "SPEC_ATTESTATION_INPUT_SET_FIXED_USER_BUS_SERVICE_QUERY_TEN_PROPERTY_"
    "EXECSTART_FRAGMENT_COLD_EXACT14_CURRENT_PROCESS_INVENTORY_AND_"
    "AUTHORITY_CLOSURE__"
    "ZERO_CREDIT"
)
PRIVATE_STATUS = (
    "PASS_C27R2_RELEASE_REPAIR_BOUNDARY_V8_PRIVATE_SINGLE_OVERRIDE_EXACT_"
    "SPEC_ATTESTATION_INPUT_SET_FIXED_USER_BUS_SERVICE_QUERY_TEN_PROPERTY_"
    "EXECSTART_FRAGMENT_COLD_EXACT14_CURRENT_PROCESS_INVENTORY_AND_"
    "AUTHORITY_CLOSURE__"
    "ZERO_CREDIT"
...
COLD_AUTHORITY_KEYS = frozenset({
    "actual_terminal_root_sha256", "actual_terminal_receipt_file_sha256",
    "actual_terminal_receipt_object_sha256",
    "actual_terminal_replay_object_sha256",
    "actual_base_receipt_object_sha256", "gate_receipt_object_sha256",
    "gate_execution_root_object_sha256", "seed1_edge_sha256",
    "seed2_edge_sha256", "frozen_C15_sha256", "control_plane",
...
def parse_manifest(resolver: Resolver, raw: str | Path,
    ...
    payload, record = resolver.capture(raw, 64 << 20)
    # Historical authority manifests have a producer-defined row order rather
    # than lexicographic path order.  The exact byte order remains immutable:
    # every caller supplies a manifest-file SHA from an already closed
    # CLI/receipt/root authority object before any row is interpreted.
    need(valid_sha(expected_file_sha256)
         and record["sha256"] == expected_file_sha256,
         "manifest exact pinned bytes/order")
    need(payload.endswith(b"\n"), "manifest newline")
    result: dict[str, str] = {}
...
def validate_actual(resolver: Resolver, args: argparse.Namespace) \
    ...
    terminal_payload_claim = terminal_receipt.get("terminal_payload_manifest")
    need(type(terminal_payload_claim) is dict,
         "terminal frozen payload manifest authority object")
    terminal_payload = parse_manifest(
        resolver, str(Path(terminal) / "payload_manifest.sha256"),
        terminal_payload_claim.get("file_sha256"))
    terminal_root = parse_manifest(
        resolver, terminal_root_path, args.expect_actual_terminal_root_sha256,
        True)
    need(len(terminal_payload) == 12
         and terminal_payload_claim == {
             "entry_count": 12,
    ...
    need(type(claimed_base) is dict,
         "terminal frozen base manifest authority object")
    base_payload = parse_manifest(
        resolver, str(Path(base) / "payload_manifest.sha256"),
        claimed_base.get("payload_manifest_file_sha256"))
    base_root = parse_manifest(
        resolver, str(Path(base) / "root_manifest.sha256"),
        claimed_base.get("root_manifest_file_sha256"), True)
...
def parse_service_properties(payload: bytes) -> dict[str, str]:
    ...
    need(len(lines) == len(pairs) == len(SERVICE_FIELDS)
         ...
         "systemd query exact ten-key set using first-equals split")
    return dict(pairs)


def exact_stat9(raw: str) -> list[int]:
    value = json.loads(raw)
    need(type(value) is list and len(value) == 9
         and all(type(item) is int for item in value)
         and canonical(value).decode("ascii") == raw,
         "canonical exact fragment stat9 argument")
    return value


def validate_core_service(unit: str, invocation: str,
                          exec_start_property_sha256: str,
                          fragment_path_raw: str,
                          fragment_file_sha256: str,
                          fragment_stat9_json: str,
                          service_query_fixture: str) -> dict[str, Any]:
    ...
    command = [str(SYSTEMCTL), "--user", "show", unit]
    ...
    need(fields["Id"] == unit and fields["Result"] == "success"
         ...
         and fields["InvocationID"] == invocation
         and fields["ExecStart"] != "",
         "exact ten-key loaded active/exited clean core service")
    exec_line = ("ExecStart=" + fields["ExecStart"] + "\n").encode("ascii")
    need(valid_sha(exec_start_property_sha256)
         and hashlib.sha256(exec_line).hexdigest()
             == exec_start_property_sha256,
         "exact current ExecStart property raw-line SHA")
    fragment_path = Path(fragment_path_raw).absolute()
    need(fragment_path_raw == str(fragment_path)
         and fragment_path.parent == SERVICE_TRANSIENT
         and fragment_path.name == unit
         and fields["FragmentPath"] == fragment_path_raw
         and SERVICE_TRANSIENT.is_dir()
         and not SERVICE_TRANSIENT.is_symlink(),
         "exact real transient FragmentPath")
    _, fragment_record = capture_direct(fragment_path, SERVICE_TRANSIENT,
                                        16 << 20)
    expected_stat = exact_stat9(fragment_stat9_json)
    need(valid_sha(fragment_file_sha256)
         and fragment_record["sha256"] == fragment_file_sha256
         and fragment_record["stat_fingerprint"] == expected_stat,
         "FragmentPath singleton regular SHA/full9stat current pin")
    return {"properties": fields,
        "exec_start_property_sha256": exec_start_property_sha256,
        "fragment_file": {"path": fragment_path_raw, **fragment_record}}
...
def validate_core(resolver: Resolver, args: argparse.Namespace,
    ...
    service = validate_core_service(args.core_unit, args.core_invocation_id,
        args.core_exec_start_property_sha256, args.core_fragment_path,
        args.core_fragment_file_sha256, args.core_fragment_stat9_json,
...
def validate_cold_authority_keys(value: Any) -> dict[str, Any]:
    need(type(value) is dict and len(value) == 14
         and set(value) == COLD_AUTHORITY_KEYS,
         "cold authority exact fourteen-key shape")
    return value


def validate_cold_control_plane(value: Any,
                                current_service: dict[str, Any]) \
        -> dict[str, Any]:
    need(type(value) is dict and set(value) == {"route", "service"},
         "cold authority control_plane exact keys")
    route = value["route"]
    need(type(route) is dict and set(route) == {"environment", "euid", "uid",
            "systemctl", "runtime_directory", "bus_socket"}
         and route["environment"] == CONTROL_BUS_ENV
         and route["uid"] == route["euid"] == SERVICE_UID,
         "cold authority fixed five-key user-bus route")
    systemctl = route["systemctl"]
    need(type(systemctl) is dict and set(systemctl) == {"O_NOFOLLOW", "path",
            "sha256", "single_link", "size", "stat_fingerprint"}
         and systemctl["O_NOFOLLOW"] is True
         and systemctl["single_link"] is True
         and systemctl["path"] == str(SYSTEMCTL)
         and systemctl["sha256"] == SYSTEMCTL_SHA256
         and type(systemctl["size"]) is int and systemctl["size"] > 0,
         "cold authority historical systemctl exact route record")
    runtime = route["runtime_directory"]
    need(type(runtime) is dict and set(runtime) == {"O_NOFOLLOW", "directory",
            "owner_gid", "owner_uid", "path", "stat_fingerprint"}
         and runtime["O_NOFOLLOW"] is True and runtime["directory"] is True
         and runtime["owner_uid"] == runtime["owner_gid"] == SERVICE_UID
         and runtime["path"] == str(SERVICE_RUNTIME),
         "cold authority historical runtime exact route record")
    bus = route["bus_socket"]
    need(type(bus) is dict and set(bus) == {"no_symlink", "owner_gid",
            "owner_uid", "path", "single_link", "socket",
            "stat_fingerprint"}
         and bus["no_symlink"] is True and bus["single_link"] is True
         and bus["socket"] is True
         and bus["owner_uid"] == bus["owner_gid"] == SERVICE_UID
         and bus["path"] == str(SERVICE_BUS),
         "cold authority historical bus exact route record")
    systemctl_stat = systemctl["stat_fingerprint"]
    runtime_stat = runtime["stat_fingerprint"]
    bus_stat = bus["stat_fingerprint"]
    need(all(type(record) is list and len(record) == 9
             and all(type(item) is int for item in record)
             for record in (systemctl_stat, runtime_stat, bus_stat))
         and stat.S_ISREG(systemctl_stat[2]) and systemctl_stat[3] == 1
         and systemctl_stat[4] == systemctl["size"]
         and systemctl_stat[7] == systemctl_stat[8] == 0
         and stat.S_ISDIR(runtime_stat[2])
         and runtime_stat[7] == runtime_stat[8] == SERVICE_UID
         and stat.S_ISSOCK(bus_stat[2]) and bus_stat[3] == 1
         and bus_stat[7] == bus_stat[8] == SERVICE_UID,
         "cold authority historical route full9stat/type/owner/link closure")
    service_keys = {"LoadState", "ActiveState", "SubState", "Result",
                    "ExecMainCode", "ExecMainStatus", "InvocationID"}
    service = value["service"]
    properties = current_service.get("properties")
    need(type(service) is dict and set(service) == service_keys
         and type(properties) is dict and set(properties) == set(SERVICE_FIELDS)
         and service == {key: properties[key] for key in service_keys},
         "cold authority exact7 frozen service equals current exact10 projection")
    return value
...
def validate_cold(resolver: Resolver, args: argparse.Namespace,
    ...
    receipt_authority_closure = receipt.get("authority_closure")
    pinset_authority_closure = pinset.get("actual_v2_authority")
    validate_cold_authority_keys(receipt_authority_closure)
    validate_cold_authority_keys(pinset_authority_closure)
    need(receipt_authority_closure == pinset_authority_closure,
         "cold receipt/pinset authority closure entire object equal")
    cold_control_plane = validate_cold_control_plane(
        receipt_authority_closure.get("control_plane"), core["service"])
    expected_authority_closure = {
        ...
        "control_plane": cold_control_plane,
        "exact_declared_input_count": 165,
        "exact_declared_input_paths_sha256": COLD_INPUT_PATHS_SHA256,
        "declared_input_role_map_sha256": COLD_ROLE_MAP_SHA256,
    }
    need(len(expected_authority_closure) == 14
         and receipt_authority_closure == pinset_authority_closure
             == expected_authority_closure,
         "cold authority exact fourteen-key receipt/pinset closure")
    expected_cold_math = {
    ...
    return {"receipt_file": receipt_record["sha256"],
            ...
            "authority_closure": expected_authority_closure,
            "current_core_service": core["service"]}
...
def execute(args: argparse.Namespace) -> dict[str, Any]:
    ...
    service_post = validate_core_service(args.core_unit,
        args.core_invocation_id, args.core_exec_start_property_sha256,
        args.core_fragment_path, args.core_fragment_file_sha256,
        args.core_fragment_stat9_json, args.service_query_fixture)
    need(service_post == core["service"],
         "validator core service ten-property/fragment pre-post identical")
    ...
    body = {
        ...
        "service_query_policy": {
            ...
            "expected_fragment_file_sha256":
                args.core_fragment_file_sha256,
            "expected_fragment_stat_fingerprint":
                exact_stat9(args.core_fragment_stat9_json),
        },
...
def self_test() -> dict[str, Any]:
    ...
    current_properties = {
        "Id": "u.service", "LoadState": "loaded", "ActiveState": "active",
        "SubState": "exited", "Result": "success", "ExecMainCode": "1",
        "ExecMainStatus": "0",
        "InvocationID": "0123456789abcdef0123456789abcdef",
        "ExecStart": "{ path=/x ; argv[]=x --a=b ; status=0 }",
        "FragmentPath": "/run/user/1000/systemd/transient/u.service"}
    service_keys = {"LoadState", "ActiveState", "SubState", "Result",
                    "ExecMainCode", "ExecMainStatus", "InvocationID"}
    historical_service = {key: current_properties[key] for key in service_keys}
    control_plane = {"route": {
        "environment": CONTROL_BUS_ENV, "uid": 1000, "euid": 1000,
        "systemctl": {"O_NOFOLLOW": True, "path": str(SYSTEMCTL),
            "sha256": SYSTEMCTL_SHA256, "single_link": True, "size": 10,
            "stat_fingerprint": [1, 2, stat.S_IFREG | 0o755, 1,
                                  10, 3, 4, 0, 0]},
        "runtime_directory": {"O_NOFOLLOW": True, "directory": True,
            "owner_gid": 1000, "owner_uid": 1000,
            "path": str(SERVICE_RUNTIME),
            "stat_fingerprint": [1, 3, stat.S_IFDIR | 0o700, 2,
                                  0, 3, 4, 1000, 1000]},
        "bus_socket": {"no_symlink": True, "owner_gid": 1000,
            "owner_uid": 1000, "path": str(SERVICE_BUS),
            "single_link": True, "socket": True,
            "stat_fingerprint": [1, 4, stat.S_IFSOCK | 0o600, 1,
                                  0, 3, 4, 1000, 1000]}},
        "service": historical_service}
    validate_cold_control_plane(
        control_plane, {"properties": current_properties})
    authority_fixture = {key: "0" * 64 for key in COLD_AUTHORITY_KEYS
                         if key not in {"control_plane",
                            "exact_declared_input_count"}}
    authority_fixture["control_plane"] = control_plane
    authority_fixture["exact_declared_input_count"] = 165
    validate_cold_authority_keys(authority_fixture)
    broken_control_missing = dict(control_plane)
    broken_control_missing.pop("service")
    broken_control_extra = {**control_plane, "extra": False}
    broken_control_projection = {**control_plane,
        "service": {**historical_service, "InvocationID": "0" * 32}}
    for broken, kind in ((dict(list(authority_fixture.items())[:10]),
                           "ten-not-fourteen"),
                          (broken_control_missing, "control-missing"),
                          (broken_control_extra, "control-extra"),
                          (broken_control_projection, "service-projection")):
        try:
            if kind == "ten-not-fourteen":
                validate_cold_authority_keys(broken)
            else:
                validate_cold_control_plane(
                    broken, {"properties": current_properties})
        except Rejected:
            pass
        else:
            raise Rejected("cold authority negative fixture:" + kind)
    with tempfile.TemporaryDirectory(prefix="c27r2-boundary-v8-") as raw:
        root = Path(raw)
        normal = root / "normal"
...
def parser() -> argparse.ArgumentParser:
    ...
    value.add_argument("--core-exec-start-property-sha256")
    value.add_argument("--core-fragment-path")
    value.add_argument("--core-fragment-file-sha256")
    value.add_argument("--core-fragment-stat9-json")
