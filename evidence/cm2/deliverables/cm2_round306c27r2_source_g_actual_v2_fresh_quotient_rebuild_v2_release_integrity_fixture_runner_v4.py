#!/usr/bin/env python3
"""Run the exact 68 real C27R2 release-boundary integrity fixtures.
...
It then executes 68 isolated validator subprocesses.  Each negative case has
...
VALIDATOR_DEFAULT = ROOT / "deliverables/cm2_round306c27r2_source_g_actual_v2_fresh_quotient_rebuild_v2_release_repair_boundary_independent_validator_v4.py"
PREFIX = "cm2.round306c27r2.source-g-actual-v2-fresh-quotient-rebuild.v2."
OVERRIDE_SCHEMA = PREFIX + "release-repair-boundary-override-map.v4"
RESULT_SCHEMA = PREFIX + "release-integrity-fixture.v4"
RESULT_STATUS = (
    "PASS_C27R2_RELEASE_BOUNDARY_V4_CONTROL_AND_68_OF_68_REAL_SUBPROCESS_"
    "EXACT_INPUT_AND_FINAL_ABSENCE_INTEGRITY_FIXTURES_REJECTED__ZERO_CREDIT"
)
PASS_BYTES = b"PASS_C27R2_RELEASE_BOUNDARY_V4_68_REAL_FIXTURES__ZERO_CREDIT\n"
...
CONTROL_OUTPUT_FILES = {
    ...
    "empty-override-map.json",
}
CASE_RUN_SCHEMA = PREFIX + "release-integrity-case-process-run.v4"
CASE_RUN_STATUS = (
...
def service_snapshot(args: argparse.Namespace) -> dict[str, str]:
    ...
    completed = subprocess.run([
        "/usr/bin/systemctl", "--user", "show", args.core_unit,
    ...
    need(fields == {"Result": "success", "ExecMainCode": "1",
                    ...
                    "InvocationID": args.core_invocation_id},
         "service snapshot exact current success")
    return fields
    ...
    for root in authoritative_roots(args):
        ...
        for path in paths:
            ...
            try:
                ...
                logical = "external:" + str(path)
            need(logical not in result, "disjoint authority roots")
            result[logical] = file_record(path)
    result["service:" + args.core_unit] = service_snapshot(args)
    for logical, record in future_absence_snapshot(args).items():
        key = "absence:" + logical
        need(key not in result, "future absence sentinel disjoint")
        result[key] = record
    return result
...
def write_case_process_transaction(process_dir: Path, record: dict[str, Any],
    ...
    input_body = {
        ...
        "source_pins": record["source_pins"],
        "child_argv": record["argv"],
        "child_environment": record["environment"],
        "logical_override": record["logical_override"],
        "mutation": record["mutation"],
        "formal_credit": 0,
    }
    input_state = dict(input_body)
    input_state["input_closure_sha256"] = digest(input_body)
    ...
    start = {
        ...
        "exact_argv": record["argv"],
        "exact_environment": record["environment"],
        "python_sha256": record["python_sha256"],
        "validator_sha256": record["validator_sha256"],
        "runner_sha256": record["runner_sha256"],
        "source_pins": record["source_pins"],
    ...
    output_validation = dict(output_body)
    output_validation["output_validation_sha256"] = digest(output_body)
    ...
    body = {
        ...
        "runner_sha256": record["runner_sha256"],
        "source_pins": record["source_pins"],
        "case_transcript_sha256": record["transcript_sha256"],
        "input_closure_sha256": input_state["input_closure_sha256"],
        "output_validation_sha256":
            output_validation["output_validation_sha256"],
...
def case_specs(args: argparse.Namespace) -> list[dict[str, Any]]:
    ...
    need([row[0] for row in specs] == CASE_NAMES, "exact ordered 68 cases")
...
def execute_case(args: argparse.Namespace, spec: dict[str, Any], index: int,
    ...
    try:
        ...
        need(result["capture_bounded"] is True
             and result["stdout_size"] <= MAX_CAPTURE_BYTES
             and result["stderr_size"] <= MAX_CAPTURE_BYTES,
             "bounded validator stdout/stderr:" + spec["name"])
    ...
    write_case_process_transaction(process_dir, transcript, projection)
...
def execute(args: argparse.Namespace) -> dict[str, Any]:
    ...
    need(work.parent == AUDIT and work.name.startswith(
         "c27r2-release-repair-integrity-v4-")
    ...
    need(baseline_value.get("status")
             == "PASS_C27R2_RELEASE_REPAIR_BOUNDARY_V4_FORMAL_EMPTY_OVERRIDE_EXACT_SPEC_ATTESTATION_INPUT_SET_CURRENT_PROCESS_INVENTORY_AND_AUTHORITY_CLOSURE__ZERO_CREDIT"
    ...
    need({path.name for path in control.iterdir()} == CONTROL_OUTPUT_FILES,
         "exact four-file baseline control inventory")
    ...
    body = {
        ...
        "rejected_count": len(records), "accepted_count": 0,
        "rejected": len(records), "accepted": 0,
        "real_process_execution_count": len(records),
        "case_process_directories": case_process_directories,
        "case_process_file_inventory": sorted(CASE_RUN_FILES),
        "control_file_inventory": sorted(CONTROL_OUTPUT_FILES),
        "expected_final_top_level_inventory": expected_final_top_inventory,
        "ordered_case_names": CASE_NAMES,
        "case_inventory_sha256": digest({"case_count": len(CASE_NAMES),
                                         "ordered_case_names": CASE_NAMES}),
        "cases": records,
...
def self_test() -> dict[str, Any]:
    need(len(CASE_NAMES) == 68 and len(set(CASE_NAMES)) == 68,
         "exact unique 68-case inventory")
    need(len(CONTROL_OUTPUT_FILES) == 4 and len(CASE_RUN_FILES) == 11,
         "exact control/case process output inventories")
    need(valid_sha(digest({"case_count": len(CASE_NAMES),
                           "ordered_case_names": CASE_NAMES})),
         "ordered case inventory object digest fixture")
    ...
    need(len(specs) == 68 and [row["name"] for row in specs] == CASE_NAMES
         and all(set(row) == {"name", "logical", "mutation", "changes"}
                 for row in specs),
         "actual exact68 case-spec construction fixture")
    ...
    with tempfile.TemporaryDirectory(prefix="c27r2-fixture-runner-v4-") as raw:
        root = Path(raw)
        ...
        process_dir = root / "case-process-00-tiny"
        record = {
            "case": "tiny", "ordinal": 0,
            "override_map_file_sha256": "1" * 64,
            "override_map_object_sha256": "2" * 64,
            "validator_sha256": "3" * 64, "runner_sha256": "4" * 64,
            "python_sha256": "5" * 64,
            "source_pins": {"tiny": {"path": "/tmp/tiny", "sha256": "6" * 64}},
            "argv": ["/usr/bin/python3.12", "-I", "-B", "tiny.py"],
            "environment": {"PATH": "/usr/bin:/bin", "LANG": "C",
                            "LC_ALL": "C", "PYTHONHASHSEED": "30662790"},
            "logical_override": "tiny/logical", "mutation": "tiny",
            "started_at_utc": "2000-01-01T00:00:00.000000Z",
            "finished_at_utc": "2000-01-01T00:00:01.000000Z",
            "elapsed_seconds": 1.0, "command_argv_sha256": "7" * 64,
            "stdout_sha256": EMPTY_SHA, "stderr_sha256": "8" * 64,
            "stdout_size": 0, "stderr_size": 1,
            "transcript_sha256": "a" * 64,
        }
        write_case_process_transaction(process_dir, record, "9" * 64)
        need({path.name for path in process_dir.iterdir()} == CASE_RUN_FILES
             and strict_document(process_dir / "run_attestation.json",
                                 "run_attestation_sha256").get(
                                     "expected_child_rejection") is True
             and (process_dir / "input_pre.json").read_bytes()
                 == (process_dir / "input_post.json").read_bytes(),
             "wired exact-eleven process transaction fixture")
    ...
    return {"schema": RESULT_SCHEMA + ".self-test",
            "status": "PASS_EXACT_68_CASE_INVENTORY_WIRED_EXACT11_CLEANUP_"
                      "BOUNDED_PROCESS_PRIVATE_LINK_AND_FINAL_WINDOW_ABSENCE_"
                      "TINY_FIXTURES",
