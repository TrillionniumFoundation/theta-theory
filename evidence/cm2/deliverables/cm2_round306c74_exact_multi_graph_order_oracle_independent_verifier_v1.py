#!/usr/bin/env python3
"""Independent no-producer verifier for the C74 multi-graph order oracle.
...
PRECISION_BITS = 384
DECLARED_PRODUCER_SHA256 = "8aaa40fcd28be40ecb958ba7880b99f1dc425da76b702014f1ff14d6594b2ad2"
VERIFIER_SCHEMA = "cm2.round306c74.exact-multi-graph-order-oracle-independent-verification.v1"
VERIFICATION_NAME = PREFIX + "_independent_verification_v1.json"
...
def reconstruct_endpoint_incidence(
        physical_decisions: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    need(len(drafts) == 34_734 and
         len({row["endpoint_occurrence_id"] for row in drafts}) == len(drafts),
         "exact unique retained endpoint occurrences")
    ...
    class_census = Counter(); root_class_census = Counter()
    internal_pair_root_count = 0; cross_scope_root_count = 0
    source_boundary_root_count = 0; atlas_boundary_root_count = 0
    ...
    for root_id in sorted(by_root):
        ...
        if len(selected) == 2:
            need(len(incidents) == 2 and selected_keys == incident_keys,
                 "internal pair closes complete incidence")
        ...
        for draft in selected:
            ...
            if incidence_class == "PAIRED_INTERNAL_C74_ENDPOINT":
                ...
                need(not counterparts and not adjacent, "explicit singleton consumer")
                consumer = {
            ...
            output_rows.append(close_row({
                "schema": SCHEMA + ".endpoint-incidence-row", **draft,
                "complete_C72_face_incidence": incidents,
                "complete_C72_face_incidence_count": len(incidents),
                "incidence_class": incidence_class, "consumer": consumer,
                ...
                "D02_gate_credit": 0, "global_unresolved_decrement": 0,
            }))
            class_census[incidence_class] += 1
    need(len(output_rows) == 34_734 and sum(class_census.values()) == 34_734 and
         2 * internal_pair_root_count + cross_scope_root_count +
         source_boundary_root_count + atlas_boundary_root_count == 34_734,
         "endpoint occurrence conservation")
    summary = {
    ...
    return sorted(output_rows, key=lambda value:
                  (value["endpoint_root_id"], value["endpoint_occurrence_id"])), summary
...
def coherent_attacks(base: dict[str, Any]) -> dict[str, Any]:
    ...
    need(all(attacks.values()) and len(attacks) == 19, "19 coherent attacks")
    return {"status": "PASS_19_OF_19_COHERENT_SCOPE_ORDER_TIE_STRATA_AND_CREDIT_ATTACKS_FAIL_CLOSED",
            "attack_count": 19, "attacks": dict(sorted(attacks.items()))}
...
def build_oracle(output: Path) -> dict[str, Any]:
    ...
    result_body = {
        ...
        "status": "PASS_42926_OF_42926_EXACT_MULTI_GRAPH_ARRANGEMENTS__ROOT_EQUALITY_EMPTY__25559_PURE_EXCLUSION__17367_CEMETERY_TANGENCY_GRAPHS_WITH_EXCLUSION_SLABS__ZERO_CREDIT",
        ...
        "analytic_closure": {
            ...
            "half_open_or_source_boundary_endpoint_incidence_materialized": True,
    ...
    report_raw = (
        ...
        "retained as cemetery terminals with both incident exclusion slabs; the remaining "
        "25,559 arrangements are pure strict exclusions with a nonfuture graph. All face "
        "incidence and noncorner endpoint definitions are materialized. Credits remain zero.\n"
    ).encode("utf-8")
    publication = [LEDGER_NAME]
...
def candidate_json(path: Path) -> tuple[dict[str, Any], str]:
    digest = sha_file(path)
    raw = path.read_bytes()
    need(raw.endswith(b"\n") and not raw.endswith(b"\n\n"),
         "candidate JSON framing:" + path.name)
    value = json.loads(raw.decode("utf-8", "strict"))
    need(type(value) is dict and canonical(value) == raw[:-1],
         "candidate canonical JSON:" + path.name)
    verify_object(value, path.name)
    need(sha_file(path) == digest, "candidate JSON terminal replay:" + path.name)
    return value, digest
...
def byte_identical(left: Path, right: Path) -> bool:
    if left.stat().st_size != right.stat().st_size:
        return False
    left_fd = os.open(left, os.O_RDONLY | getattr(os, "O_CLOEXEC", 0) |
                      getattr(os, "O_NOFOLLOW", 0))
    right_fd = os.open(right, os.O_RDONLY | getattr(os, "O_CLOEXEC", 0) |
                       getattr(os, "O_NOFOLLOW", 0))
    try:
        while True:
            a = os.read(left_fd, 1 << 20)
            b = os.read(right_fd, 1 << 20)
            if a != b:
                return False
            if not a:
                return True
...
def single_gzip_member(path: Path) -> None:
    descriptor = os.open(path, os.O_RDONLY | getattr(os, "O_CLOEXEC", 0) |
                         getattr(os, "O_NOFOLLOW", 0))
...
def verify_candidate_publication(stage_a: Path, stage_b: Path) -> tuple[dict[str, Any], dict[str, str]]:
    names = (LEDGER_NAME, ENDPOINT_LEDGER_NAME, RESULT_NAME, REPORT_NAME,
             MANIFEST_NAME, OUTER_NAME)
    hashes: dict[str, str] = {}
    for name in names:
        left, right = stage_a / name, stage_b / name
        left_hash, right_hash = sha_file(left), sha_file(right)
        need(left_hash == right_hash and byte_identical(left, right),
             "dual isolated build byte identity:" + name)
        need(sha_file(left) == left_hash and sha_file(right) == right_hash,
             "dual build terminal recapture:" + name)
        hashes[name] = left_hash
    for stage in (stage_a, stage_b):
        single_gzip_member(stage / LEDGER_NAME)
        single_gzip_member(stage / ENDPOINT_LEDGER_NAME)
    result_a, result_hash_a = candidate_json(stage_a / RESULT_NAME)
    result_b, result_hash_b = candidate_json(stage_b / RESULT_NAME)
    need(result_a == result_b and result_hash_a == result_hash_b == hashes[RESULT_NAME],
         "dual result identity")
    result_validator(result_a)
    need(result_a["producer_file_sha256"] == DECLARED_PRODUCER_SHA256,
         "declared producer source pin")
    manifested = (LEDGER_NAME, ENDPOINT_LEDGER_NAME, RESULT_NAME, REPORT_NAME)
    expected_manifest = "".join(
        f"{hashes[name]}  {name}\n" for name in sorted(manifested)
    ).encode("ascii")
    for stage in (stage_a, stage_b):
        need((stage / MANIFEST_NAME).read_bytes() == expected_manifest and
             sha_file(stage / MANIFEST_NAME) == hashes[MANIFEST_NAME],
             "exact one-global manifest")
    outer_a, outer_hash_a = candidate_json(stage_a / OUTER_NAME)
    outer_b, outer_hash_b = candidate_json(stage_b / OUTER_NAME)
    publication_order = [LEDGER_NAME, ENDPOINT_LEDGER_NAME, RESULT_NAME,
                         REPORT_NAME, MANIFEST_NAME, OUTER_NAME]
    member_hashes = {name: hashes[name] for name in publication_order[:-1]}
    need(outer_a == outer_b and outer_hash_a == outer_hash_b == hashes[OUTER_NAME] and
         outer_a["result_object_sha256"] == result_a["object_sha256"] and
         outer_a["manifest_sha256"] == hashes[MANIFEST_NAME] and
         outer_a["member_file_sha256"] == member_hashes and
         outer_a["publication_order"] == publication_order and
         outer_a["outer_receipt_published_last"] is True and
         outer_a["formal_credit"] == outer_a["D02_gate_credit"] == 0,
         "outer receipt closure")
    for stage in (stage_a, stage_b):
        outer_mtime = (stage / OUTER_NAME).stat().st_mtime_ns
        need(all(outer_mtime >= (stage / name).stat().st_mtime_ns
                 for name in publication_order[:-1]), "outer receipt filesystem-last")
    return result_a, hashes


def reconstruct_decisions_and_incidence(stage: Path, result: dict[str, Any]) -> dict[str, Any]:
    ctx.prec = PRECISION_BITS
    ctx.threads = 1
    ctx.pretty = True
    ctx.unicode = False
    targets, _c65_result, _c58_result = validate_upstreams()
    wanted_c65 = {row["C65_aggregate_child_row_sha256"] for row in targets}
    c65 = {row["row_sha256"]: row for row in iter_closed_rows(
        C65, FILE_PINS["C65_LEAVES"], 358_919) if row["row_sha256"] in wanted_c65}
    need(len(c65) == len(wanted_c65), "independent C65 join")
    wanted_c58 = {row["source_C58_leaf_row_sha256"] for row in c65.values()}
    c58 = {row["row_sha256"]: row for row in iter_closed_rows(
        C58, FILE_PINS["C58_LEAVES"], 5_548) if row["row_sha256"] in wanted_c58}
    need(len(c58) == len(wanted_c58), "independent C58 join")
    context = c41.load_context(C40, None, formal=False)
    c41.install_complete_immutable_cache()
    c40_rows = c41.c38.read_ledger(C40,
                                   context["result"]["ledgers"]["routed_leaf_cells"])
    c40_index = {row["row_sha256"]: (index, row)
                 for index, row in enumerate(c40_rows)}
    task_cache: dict[str, dict[str, Any]] = {}
    pair_census = Counter(); order_census = Counter(); arrangement_census = Counter()
    category_census = Counter(); chart_census = Counter(); graph_target_census = Counter()
    sequence = hashlib.sha256(); physical: list[dict[str, Any]] = []
    future_depths: list[int] = []
    observed_iter = iter_closed_rows(stage / LEDGER_NAME,
                                     result["ledger"]["sha256"], 42_926)
    first_decision = None
    c39 = c41.c39
    for atlas, observed in zip(targets, observed_iter, strict=True):
        source = c65[atlas["C65_aggregate_child_row_sha256"]]
        c58_source = c58[source["source_C58_leaf_row_sha256"]]
        c40_hash = c58_source["C40_source_row_sha256"]
        task = task_cache.get(c40_hash)
        if task is None:
            ordinal, c40_source = c40_index[c40_hash]
            task = c41.task_for_row(ordinal, c40_source, context)
            task_cache[c40_hash] = task
        expected = decision_row(c39, atlas, source, task)
        need(observed == expected, "independent decision row equality:" + atlas["row_sha256"])
        decision_row_validator(expected)
        if first_decision is None:
            first_decision = expected
        sequence.update((expected["row_sha256"] + "\n").encode("ascii"))
        pair_census[expected["pair_index"]] += 1
        order_census[expected["root_order"]["relation"]] += 1
        category_census[expected["source_structural_category"]] += 1
        graph_target_census[expected["discriminant_graph"]["graph_target"]] += 1
        chart = expected["outgoing_evidence"].get("strict_outgoing_chart",
                expected["outgoing_evidence"].get("chart"))
        chart_census[chart] += 1
        if expected["sealed_strata"][1]["physical_future_tangency"]:
            arrangement_census["EXCLUSION_SLABS_PLUS_CEMETERY_TANGENCY_GRAPH"] += 1
            physical.append(expected)
            future_depths.append(expected["discriminant_graph"]
                                 ["physical_future_margin"]["strict_dyadic_depth"])
        else:
            arrangement_census["PURE_STRICT_EXCLUSION_WITH_NONFUTURE_GRAPH"] += 1
        arrangement_census["SEALED_CHILD_ARRANGEMENTS"] += 1
    expected_decision_descriptor = {
        "filename": LEDGER_NAME,
        "order": "C72_ATLAS_ORDER_FILTERED_BY_EXACT_CURRENT_CHILD_WITNESS",
        "row_count": 42_926,
        "row_hash_line_sequence_sha256": sequence.hexdigest(),
        "sha256": sha_file(stage / LEDGER_NAME),
        "size": (stage / LEDGER_NAME).stat().st_size,
    }
    need(result["ledger"] == expected_decision_descriptor and
         dict(pair_census) == EXPECTED_PAIR_CENSUS and
         dict(order_census) == EXPECTED_ORDER_CENSUS and
         result["pair_census"] == {str(k): v for k, v in sorted(pair_census.items())} and
         result["root_order_census"] == dict(sorted(order_census.items())) and
         result["source_structural_category_census"] == dict(sorted(category_census.items())) and
         result["graph_target_census"] == dict(sorted(graph_target_census.items())) and
         result["outgoing_chart_census"] == dict(sorted(chart_census.items())) and
         result["arrangement_census"] == dict(sorted(arrangement_census.items())) and
         len(physical) == len(future_depths) == 17_367 and
         result["physical_future_tangency_margin"]["maximum_dyadic_depth"] ==
             max(future_depths), "independent decision aggregate equality")

    expected_endpoints, endpoint_summary = reconstruct_endpoint_incidence(physical)
    endpoint_sequence = hashlib.sha256()
    observed_endpoints = iter_closed_rows(
        stage / ENDPOINT_LEDGER_NAME,
        result["endpoint_incidence_ledger"]["sha256"], 34_734)
    first_endpoint = None
    for expected, observed in zip(expected_endpoints, observed_endpoints, strict=True):
        need(observed == expected,
             "independent endpoint incidence equality:" + expected["endpoint_occurrence_id"])
        endpoint_row_validator(expected)
        if first_endpoint is None:
            first_endpoint = expected
        endpoint_sequence.update((expected["row_sha256"] + "\n").encode("ascii"))
    expected_endpoint_descriptor = {
        "filename": ENDPOINT_LEDGER_NAME,
        "order": "ENDPOINT_ROOT_ID_THEN_C74_ENDPOINT_OCCURRENCE_ID",
        "row_count": 34_734,
        "row_hash_line_sequence_sha256": endpoint_sequence.hexdigest(),
        "sha256": sha_file(stage / ENDPOINT_LEDGER_NAME),
        "size": (stage / ENDPOINT_LEDGER_NAME).stat().st_size,
    }
    need(result["endpoint_incidence_ledger"] == expected_endpoint_descriptor and
         result["endpoint_incidence_closure"] == endpoint_summary,
         "independent endpoint aggregate equality")
    return {
        "first_decision": first_decision,
        "first_physical_decision": physical[0],
        "first_endpoint": first_endpoint,
        "decision_row_count": 42_926,
        "physical_future_tangency_graph_count": 17_367,
        "endpoint_occurrence_count": 34_734,
        "endpoint_summary": endpoint_summary,
        "maximum_future_margin_dyadic_depth": max(future_depths),
    }


def decision_row_validator(value: dict[str, Any]) -> None:
    verify_row(value, "decision row")
    need(value["schema"] == SCHEMA + ".decision-row" and
         value["complete_child_decision"] ==
             "SEALED_TWO_OPEN_SLABS_PLUS_EXACT_GRAPH_STRATUM" and
         value["additional_dyadic_depth_used"] == 0 and
         value["root_order"]["root_equality_realizable"] is False and
         value["root_order"]["first_contact_precedence_closed"] is True and
         len(value["discriminant_graph"]["boundary_incidence"]) == 2 and
         value["discriminant_graph"]["corner_tie_count"] == 0 and
         len(value["sealed_strata"]) == 3 and
         value["formal_credit"] == value["whole_parent_credit"] ==
             value["D02_gate_credit"] == value["global_unresolved_decrement"] == 0,
         "decision semantic closure")
    physical = value["sealed_strata"][1]["physical_future_tangency"]
    need(value["discriminant_graph"]["physical_future_tangency"] is physical and
         ((physical and type(value["discriminant_graph"]["physical_future_margin"]) is dict) or
          (not physical and value["discriminant_graph"]["physical_future_margin"] is None)),
         "decision physical future consistency")


def endpoint_row_validator(value: dict[str, Any]) -> None:
    verify_row(value, "endpoint row")
    classes = {"PAIRED_INTERNAL_C74_ENDPOINT", "ADJACENT_C72_CROSS_SCOPE_GLUE",
               "SOURCE_CELL_BOUNDARY_CONSUMER", "ATLAS_COMPLEMENT_BOUNDARY_CONSUMER"}
    need(value["schema"] == SCHEMA + ".endpoint-incidence-row" and
         value["incidence_class"] in classes and
         value["consumer"]["kind"] == value["incidence_class"] and
         value["endpoint_root_id"] == endpoint_root_id(
             value["exact_face"], value["collision1_target"]) and
         value["face_key_sha256"] == exact_face_key(value["exact_face"]) and
         value["unique_root_proof"]["unique_root_on_exact_face"] is True and
         value["unique_root_proof"]["corner_incidence"] is False and
         set(value["unique_root_proof"]["rational_face_end_signs"]) ==
             {"POSITIVE", "NEGATIVE"} and
         value["complete_C72_face_incidence_count"] ==
             len(value["complete_C72_face_incidence"]) and
         value["canonical_half_open_owner"]["unique"] is True and
         value["coordinate_and_seam_authority"]
             ["local_cross_chart_or_source_seam_reinterpretation"] is False and
         value["same_physical_trace_duplicate_unassigned"] is False and
         value["endpoint_incidence_closed"] is True and
         value["formal_credit"] == value["whole_parent_credit"] ==
             value["D02_gate_credit"] == value["global_unresolved_decrement"] == 0,
         "endpoint semantic closure")


def independent_attacks(result: dict[str, Any], physical_decision: dict[str, Any],
                        endpoint: dict[str, Any]) -> dict[str, Any]:
    outcomes: dict[str, bool] = {}

    def attack_object(name: str, base: dict[str, Any], mutation: Any,
                      validator: Any) -> None:
        body = copy.deepcopy(base)
        body.pop("object_sha256")
        mutation(body)
        try:
            validator(close_object(body))
        except Exception:
            outcomes[name] = True
        else:
            outcomes[name] = False

    def attack_row(name: str, base: dict[str, Any], mutation: Any,
                   validator: Any) -> None:
        body = copy.deepcopy(base)
        body.pop("row_sha256")
        mutation(body)
        try:
            validator(close_row(body))
        except Exception:
            outcomes[name] = True
        else:
            outcomes[name] = False
    ...
    result_mutations = [
        ("result_status", lambda v: v.__setitem__("status", "PASS")),
        ("result_scope", lambda v: v["scope"].__setitem__("selected_child_count", 42_925)),
        ("result_pair", lambda v: v["pair_census"].__setitem__("787", 9_979)),
        ("result_order_W", lambda v: v["root_order_census"].__setitem__(
            "FROZEN_W_STRICT_BEFORE_OTHER_ON_REAL_STRATUM", 16_795)),
        ("result_order_behind", lambda v: v["root_order_census"].__setitem__(
            "GRAPH_COMPETITOR_STRICT_BEHIND_ON_REAL_STRATUM", 25_558)),
        ("result_order_before", lambda v: v["root_order_census"].__setitem__(
            "GRAPH_COMPETITOR_STRICT_BEFORE_FROZEN_W_ON_REAL_STRATUM", 570)),
        ("result_tie", lambda v: v.__setitem__("root_equality_realizable_count", 1)),
        ("result_pure", lambda v: v["arrangement_census"].__setitem__(
            "PURE_STRICT_EXCLUSION_WITH_NONFUTURE_GRAPH", 25_558)),
        ("result_cemetery", lambda v: v["arrangement_census"].__setitem__(
            "EXCLUSION_SLABS_PLUS_CEMETERY_TANGENCY_GRAPH", 17_366)),
        ("result_sealed", lambda v: v["arrangement_census"].__setitem__(
            "SEALED_CHILD_ARRANGEMENTS", 42_925)),
        ("result_future_count", lambda v: v["physical_future_tangency_margin"].__setitem__(
            "terminal_graph_count", 17_366)),
        ("result_future_bound", lambda v: v["physical_future_tangency_margin"].__setitem__(
            "uniform_strict_lower_bound", "2^(-999)")),
        ("result_endpoint_count", lambda v: v["endpoint_incidence_closure"].__setitem__(
            "endpoint_occurrence_count", 34_733)),
        ("result_pair_roots", lambda v: v["endpoint_incidence_closure"].__setitem__(
            "internal_pair_root_count", v["endpoint_incidence_closure"]["internal_pair_root_count"] + 1)),
        ("result_dangling", lambda v: v["endpoint_incidence_closure"].__setitem__(
            "dangling_duplicate_count", 1)),
        ("result_incidence", lambda v: v["endpoint_incidence_closure"].__setitem__(
            "incidence_closed", False)),
        ("result_endpoint_ledger", lambda v: v["endpoint_incidence_ledger"].__setitem__(
            "row_count", 34_733)),
        ("result_formal", lambda v: v["strict_boundary"].__setitem__("formal_credit", 1)),
        ("result_whole", lambda v: v["strict_boundary"].__setitem__("whole_parent_credit", 1)),
        ("result_D02", lambda v: v["strict_boundary"].__setitem__("D02_gate_credit", 1)),
        ("result_decrement", lambda v: v["strict_boundary"].__setitem__(
            "global_unresolved_decrement", 1)),
        ("result_authority", lambda v: v["strict_boundary"].__setitem__(
            "candidate_is_global_authority", True)),
        ("result_pointer", lambda v: v["strict_boundary"].__setitem__(
            "runtime_canonical_pointer_or_seal_writes", True)),
    ]
    for name, mutation in result_mutations:
        attack_object(name, result, mutation, result_validator)
    corrupted = copy.deepcopy(result); corrupted["object_sha256"] = "0" * 64
    try:
        result_validator(corrupted)
    except Exception:
        outcomes["result_object_closure"] = True
    else:
        outcomes["result_object_closure"] = False
    decision_mutations = [
        ("decision_schema", lambda v: v.__setitem__("schema", "x")),
        ("decision_depth", lambda v: v.__setitem__("additional_dyadic_depth_used", 1)),
        ("decision_credit", lambda v: v.__setitem__("formal_credit", 1)),
        ("decision_root_tie", lambda v: v["root_order"].__setitem__(
            "root_equality_realizable", True)),
        ("decision_endpoint_drop", lambda v: v["discriminant_graph"]
            ["boundary_incidence"].pop()),
        ("decision_margin_drop", lambda v: v["discriminant_graph"].__setitem__(
            "physical_future_margin", None)),
    ]
    for name, mutation in decision_mutations:
        attack_row(name, physical_decision, mutation, decision_row_validator)
    endpoint_mutations = [
        ("endpoint_schema", lambda v: v.__setitem__("schema", "x")),
        ("endpoint_root", lambda v: v.__setitem__("endpoint_root_id", "0" * 64)),
        ("endpoint_consumer", lambda v: v["consumer"].__setitem__("kind", "UNBOUND")),
        ("endpoint_owner", lambda v: v["canonical_half_open_owner"].__setitem__(
            "unique", False)),
        ("endpoint_duplicate", lambda v: v.__setitem__(
            "same_physical_trace_duplicate_unassigned", True)),
        ("endpoint_closed", lambda v: v.__setitem__("endpoint_incidence_closed", False)),
        ("endpoint_credit", lambda v: v.__setitem__("D02_gate_credit", 1)),
    ]
    for name, mutation in endpoint_mutations:
        attack_row(name, endpoint, mutation, endpoint_row_validator)
    need(len(outcomes) == 37 and all(outcomes.values()),
         "37 independent coherent attacks")
    return {"status": "PASS_37_OF_37_COHERENT_FILE_SCOPE_ORDER_FUTURE_INCIDENCE_AND_CREDIT_ATTACKS_FAIL_CLOSED",
            "attack_count": 37,
            "attacks": {name: "FAIL_CLOSED" for name in sorted(outcomes)}}


def verify(stage_a: Path, stage_b: Path, output: Path) -> dict[str, Any]:
    need(stage_a.is_dir() and stage_b.is_dir() and stage_a != stage_b,
         "two distinct candidate directories")
    need(output.is_dir() and not any(output.iterdir()),
         "empty existing verification output")
    result, hashes = verify_candidate_publication(stage_a, stage_b)
    reconstruction = reconstruct_decisions_and_incidence(stage_a, result)
    attacks = independent_attacks(result, reconstruction["first_physical_decision"],
                                  reconstruction["first_endpoint"])
    endpoint_summary = reconstruction.pop("endpoint_summary")
    reconstruction.pop("first_decision")
    reconstruction.pop("first_physical_decision")
    reconstruction.pop("first_endpoint")
    body = {
        "schema": VERIFIER_SCHEMA,
        "status": "PASS_NO_PRODUCER__DUAL_BUILD_BYTE_IDENTICAL__42926_OF_42926_DECISIONS__17367_STRICT_FUTURE_TANGENCY_GRAPHS__34734_ENDPOINT_OCCURRENCES_INCIDENCE_CLOSED__37_OF_37_ATTACKS__ZERO_CREDIT",
        "declared_producer_file_sha256": DECLARED_PRODUCER_SHA256,
        "producer_opened_read_decoded_compiled_imported_or_executed": False,
        "verifier_file_sha256": sha_file(SELF),
        "candidate_file_sha256": hashes,
        "candidate_result_object_sha256": result["object_sha256"],
        "candidate_result_file_sha256": hashes[RESULT_NAME],
        "numeric_context": {"precision_bits": PRECISION_BITS,
                            "python_flint_version": __import__("flint").__version__,
                            "flint_version": __import__("flint").__FLINT_VERSION__,
                            "threads": 1},
        ...
        "dual_isolated_builds": {
            "all_six_members_byte_identical": True,
            "decision_and_endpoint_gzip_single_member": True,
            "manifest_exact": True,
            "outer_receipt_last": True,
            "all_members_terminal_byte_recaptured": True,
        },
    ...
    verification = close_object(body)
    raw = canonical(verification) + b"\n"
    emit_new(output / VERIFICATION_NAME, raw)
    need((output / VERIFICATION_NAME).read_bytes() == raw and
         sha_file(output / VERIFICATION_NAME) == hashlib.sha256(raw).hexdigest(),
         "verification terminal-byte replay")
    return {"status": "PASS_C74_INDEPENDENT_VERIFICATION_PUBLISHED_AND_REPLAYED",
            "verification_file_sha256": hashlib.sha256(raw).hexdigest(),
...
def main() -> int:
    ...
    args = parser.parse_args()
    try:
        result = verify(Path(args.candidate_a).absolute(),
                        Path(args.candidate_b).absolute(),
