...
Version 4 pins the cold-v4 consumer, which opens and closes all seven process
run attestations and their exact ten-member transaction sidecars.  Its child
environment preserves only the canonical user-bus coordinates required for
the cold consumer's live core-service replay.
"""
...
def child_environment(seed: str) -> dict[str, str]:
    need(re.fullmatch(r"[0-9]{8}", seed) is not None, "child hash seed")
    runtime = Path(f"/run/user/{os.getuid()}")
    bus = runtime / "bus"
    runtime_info = os.stat(runtime, follow_symlinks=False)
    bus_info = os.stat(bus, follow_symlinks=False)
    need(stat.S_ISDIR(runtime_info.st_mode) and not runtime.is_symlink()
         and stat.S_ISSOCK(bus_info.st_mode) and not bus.is_symlink(),
         "canonical user bus")
    return {"PATH": "/usr/bin:/bin", "LANG": "C", "LC_ALL": "C",
            "PYTHONHASHSEED": seed, "XDG_RUNTIME_DIR": str(runtime),
...
def run(program: str, argv: list[str], stage: str, control: Path,
    ...
    completed = subprocess.run([str(PYTHON), "-I", "-B", str(PROGRAMS[program]),
                                *argv], cwd=ROOT,
        env=child_environment(str(30662990 + len(list(control.iterdir())))),
...
def self_tests() -> None:
    for name, program in PROGRAMS.items():
        completed = subprocess.run([str(PYTHON), "-I", "-B", str(program),
                                    "--self-test"], cwd=ROOT,
            env=child_environment(str(30662970 + len(name))),
            stdin=subprocess.DEVNULL, stdout=subprocess.PIPE,
...
def initial(args: argparse.Namespace, targets: dict[str, Path]) -> dict[str, Any]:
    ...
    completed = subprocess.run([str(PYTHON), "-I", "-B", str(PROGRAMS["cold"]),
        ...
        env=child_environment("30662989"), stdin=subprocess.DEVNULL,
...
def self_test() -> dict[str, Any]:
    ...
    need(len(test) == 11 and all(valid_sha(value) for value in PINS.values()),
         "watcher fixtures")
    return {"status": "PASS_C29_V2_RELEASE_CHAIN_WATCHER_V4_SELF_TEST",
