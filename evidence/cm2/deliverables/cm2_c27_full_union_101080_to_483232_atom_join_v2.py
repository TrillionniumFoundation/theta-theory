...
def rep_atom_key(x:str)->tuple[str,str]:
    source,h=x.split(":",1);need(source in {"C19A","C19B","C19C","C20A","C22A","C23A"} and len(h)==64,"positive representative source key");return source,h
def main()->int:
    ...
    try:
        ...
        for u in caps["union_ledger"].rows():
            ...
            if auth=="CURRENT_SUPPORT_V4B_SEED1_PRIORITY_LEDGER":
                ...
                if t=="POSITIVE_VOLUME_CARRIERS":
                    ...
                    keys=[rep_atom_key(x) for x in p["representative_atom_ids"]];need([k[0] for k in keys]==p["representative_sources"],"current positive representative source agreement");aids=[atom_by_source.get(k) for k in keys];need(len(aids)==2 and None not in aids and len(set(aids))==2 and set(atom_owner[x] for x in aids)==set(pair),"current exact atom witness via frozen source-key map")
                    for aid in aids:
        ...
        a.out_dir.mkdir(parents=True,exist_ok=False);out=a.out_dir/"full_union_101080_on_483232_atom_incidence_or_complement_v2.jsonl.gz";rowseq=hashlib.sha256();atomids=hashlib.sha256();disp=Counter();sets=Counter();presence=Counter();multi=0;expanded=0;seen_ep=set()
