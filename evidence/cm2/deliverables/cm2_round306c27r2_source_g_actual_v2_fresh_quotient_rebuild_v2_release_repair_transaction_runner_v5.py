#!/usr/bin/env python3
"""Append-only exact-contract process runner for C27R2 release repair v5.
...
SPEC_SCHEMA = BASE + "release-repair-process-command-spec.v5"
SPEC_STATUS = "FROZEN_C27R2_RELEASE_REPAIR_V5_EXACT_PROCESS_COMMAND_SPEC"
PINSET_SCHEMA = BASE + "release-repair-process-pinset.v5"
PINSET_STATUS = "FROZEN_C27R2_RELEASE_REPAIR_V5_EXACT_PROCESS_PINSET"
...
FAIL_SCHEMA = BASE + "release-repair-process-failure-receipt.v5"
PASS = b"PASS_C27R2_RELEASE_REPAIR_V5_PROCESS_TRANSACTION__ZERO_CREDIT\n"
...
FORMAL_EXECUTION_AUTHORIZED = False  # boundary v6/full-chain audit still pending.
SPEC_KEYS = {
    "schema", "status", "stage", "pinset_file_sha256",
    "pinset_object_sha256", "runner_source_sha256", "python_path",
    "python_sha256", "source_path", "source_sha256",
    "fd_bound_wrapper_path", "fd_bound_wrapper_sha256", "input_paths",
...
PINSET_KEYS = {
    ...
    "python_sha256", "source_path", "source_sha256",
    "fd_bound_wrapper_path", "fd_bound_wrapper_sha256", "input_members",
...
def execute(args: argparse.Namespace, self_test_only: bool = False) -> dict[str, Any]:
    ...
    held_output_parent_fds: list[int] = []
    fd_bound_result_read = -1
    fd_bound_result_write = -1
    try:
        ...
        need(source_cap.sha256 == spec.get("source_sha256")
             ...
             "source pin")
        wrapper = workspace_path(spec["fd_bound_wrapper_path"])
        wrapper_cap = Capture(wrapper, "fd-bound stage wrapper")
        pinned_only.append(wrapper_cap)
        need(wrapper_cap.sha256 == spec.get("fd_bound_wrapper_sha256")
             and pinset.get("fd_bound_wrapper_path")
                 == spec.get("fd_bound_wrapper_path")
             and pinset.get("fd_bound_wrapper_sha256") == wrapper_cap.sha256,
             "FD-bound wrapper current pin")
        ...
        need(type(timeout) is int and 1 <= timeout <= 86_400,
             "timeout bound")
        fd_bound_request: dict[str, Any] | None = None
        fd_bound_request_cap: Capture | None = None
        fd_bound_request_path: Path | None = None
        wrapper_command: list[str] | None = None
        output_fd_contracts: list[dict[str, Any]] = []
        if not self_test_only:
            # All formal stage reads are inherited FDs.  The wrapper copies the
            # exact descriptor bytes into a private same-layout workspace, so
            # the stage's own ROOT-relative plan reads cannot fall back to the
            # mutable original namespace.  Outputs are published only below
            # these held parent dirfds.
            for item in outputs:
                path = workspace_path(item["path"], absent=True)
                need(path.parent.is_dir() and not path.parent.is_symlink()
                     and path.name not in {"", ".", ".."},
                     "existing non-symlink output parent and single leaf")
                parent_fd = os.open(path.parent, os.O_RDONLY
                    | getattr(os, "O_DIRECTORY", 0)
                    | getattr(os, "O_NOFOLLOW", 0)
                    | getattr(os, "O_CLOEXEC", 0))
                held_output_parent_fds.append(parent_fd)
                output_fd_contracts.append({"path": item["path"],
                    "parent_fd": parent_fd,
                    "parent_stat_fingerprint": fingerprint(os.fstat(parent_fd)),
                    "leaf": path.name, "exact_files": item["exact_files"]})
            source_declared = [cap for cap in declared_caps
                               if cap.path == source]
            need(len(source_declared) == 1,
                 "stage source exactly once in inherited declared FD set")
            fd_bound_result_read, fd_bound_result_write = os.pipe()
            request_body = {
                "schema": (BASE + "fd-bound-stage-request.v1"),
                "status": ("FROZEN_C27R2_BACKHALF_V5_FD_BOUND_STAGE_REQUEST__"
                           "NO_AUTHORITY"),
                "stage": stage, "python_path": str(python.path),
                "python_sha256": python.sha256,
                "stage_source_path": spec["source_path"],
                "stage_source_fd": source_declared[0].fd,
                "stage_source_sha256": source_cap.sha256,
                "input_files": [{"path": cap.attest()["path"], "fd": cap.fd,
                    "sha256": cap.sha256,
                    "stat_fingerprint": fingerprint(cap.before)}
                    for cap in declared_caps],
                "output_roots": output_fd_contracts,
                "original_argv": command, "exact_environment": env,
                "timeout_seconds": timeout, "publication_lock_fd": lock_fd,
                "publication_lock_sha256": lock_cap.sha256,
                "publication_lock_stat_fingerprint": fingerprint(lock_info),
                "result_fd": fd_bound_result_write, "formal_credit": 0,
                "manifest_authorized": False, "authority_minted": False,
            }
            fd_bound_request = {**request_body,
                "request_sha256": digest(request_body)}
            fd_bound_request_path = spec_cap.path.parent / "fd_bound_request.json"
            need(not fd_bound_request_path.exists()
                 and fd_bound_request_path.parent.is_dir(),
                 "fresh FD-bound request beside frozen command spec")
            write_json(fd_bound_request_path, fd_bound_request)
            fd_bound_request_cap = Capture(fd_bound_request_path,
                                            "fd-bound request")
            captures.append(fd_bound_request_cap)
            wrapper_command = [str(python.path), "-I", "-B",
                f"/proc/self/fd/{wrapper_cap.fd}", "--request-fd",
                str(fd_bound_request_cap.fd), "--expect-request-file-sha256",
                fd_bound_request_cap.sha256,
                "--expect-request-object-sha256",
                fd_bound_request["request_sha256"]]
        write_json(run_dir / "runner_start.json", {
            "schema": RUN_SCHEMA + ".start", "stage": stage,
            "started_at_utc": started, "formal_credit": 0,
            "fd_bound_formal_stage": not self_test_only,
            "fd_bound_request_object_sha256": None if self_test_only else
                fd_bound_request["request_sha256"],
        ...
        child_command = command if self_test_only else wrapper_command
        inherited = (lock_fd,) if self_test_only else tuple(sorted({lock_fd,
            wrapper_cap.fd, fd_bound_request_cap.fd, fd_bound_result_write,
            *(cap.fd for cap in declared_caps), *held_output_parent_fds}))
        process = subprocess.Popen(child_command, cwd=ROOT, env=env,
            stdin=subprocess.DEVNULL, stdout=subprocess.PIPE,
            stderr=subprocess.PIPE, start_new_session=True,
            pass_fds=inherited)
        if not self_test_only:
            os.close(fd_bound_result_write)
            fd_bound_result_write = -1
        timed_out = False
        try:
        ...
        fd_bound_result: dict[str, Any] | None = None
        if not self_test_only:
            result_chunks: list[bytes] = []
            while block := os.read(fd_bound_result_read, 1 << 20):
                result_chunks.append(block)
            os.close(fd_bound_result_read); fd_bound_result_read = -1
            result_raw = b"".join(result_chunks)
            need(result_raw.endswith(b"\n"), "FD-bound result pipe newline")
            fd_bound_result = strict(result_raw[:-1])
            result_body = dict(fd_bound_result)
            result_claim = result_body.pop("result_sha256", None)
            need(type(fd_bound_result) is dict
                 and canonical(fd_bound_result) == result_raw[:-1]
                 and result_claim == digest(result_body)
                 and fd_bound_result.get("schema")
                     == BASE + "fd-bound-stage-result.v1"
                 and fd_bound_result.get("status") ==
                    "PASS_C27R2_BACKHALF_V5_FD_BOUND_PRIVATE_EXEC_AND_DIRFD_"
                    "PUBLICATION__ZERO_CREDIT"
                 and fd_bound_result.get("request_object_sha256")
                     == fd_bound_request["request_sha256"],
                 "FD-bound wrapper result closure")
        ...
        expected_attested_paths = [spec_cap.attest()["path"],
            ...
            *([] if self_test_only else [fd_bound_request_cap.attest()["path"]])]
        ...
        pre_attestation_members = {name: file_record(run_dir / name)["sha256"]
        ...
        body = {"schema": RUN_SCHEMA, "status": RUN_STATUS,
            ...
            "fd_bound_wrapper_path": spec["fd_bound_wrapper_path"],
            "fd_bound_wrapper_sha256": wrapper_cap.sha256,
            "fd_bound_request_file_sha256": None if self_test_only else
                fd_bound_request_cap.sha256,
            "fd_bound_request_object_sha256": None if self_test_only else
                fd_bound_request["request_sha256"],
            "fd_bound_input_map_sha256": None if self_test_only else digest(
                fd_bound_request["input_files"]),
            "fd_bound_output_dirfd_map_sha256": None if self_test_only else digest(
                fd_bound_request["output_roots"]),
            "fd_bound_execution_result": fd_bound_result,
            "fd_bound_parent_directory_swap_defense": not self_test_only,
            ...
            "exact_inventory": sorted(RUN_FILES),
            "member_file_sha256": pre_attestation_members,
        ...
        if fd_bound_result_read >= 0:
            os.close(fd_bound_result_read)
        if fd_bound_result_write >= 0:
            os.close(fd_bound_result_write)
        for descriptor in held_output_parent_fds:
            os.close(descriptor)
...
def self_test_transaction() -> dict[str, Any]:
    """Run the actual v5 runner around a tiny isolated child transaction."""
    ...
    with tempfile.TemporaryDirectory(dir=ROOT / ".cm2-runtime",
            prefix="c27r2-transaction-v5-selftest-") as raw:
        root = Path(raw)
        ...
        output_body = {"schema": BASE + "runner-selftest-output.v5",
        ...
        wrapper_path = (ROOT / "deliverables/cm2_round306c27r2_source_g_actual_v2_"
            "fresh_quotient_rebuild_v2_release_repair_fd_bound_stage_wrapper_v1.py")
        wrapper_sha = file_record(wrapper_path)["sha256"]
        wrapper_rel = str(wrapper_path.relative_to(ROOT))
        ...
        pinset_body = {"schema": PINSET_SCHEMA, "status": PINSET_STATUS,
            ...
            "fd_bound_wrapper_path": wrapper_rel,
            "fd_bound_wrapper_sha256": wrapper_sha,
        ...
        return {"stage": "self_test",
            "status": "PASS_C27R2_RELEASE_REPAIR_TRANSACTION_RUNNER_V5_REAL_SELF_TEST"}
