...
\large Typed difference-quotient convergence, tilted upper-scale grazing, and three-frequency removal,
and a strongly conditional
pathwise CM2 criterion}
\author{Working note}
\date{13 July 2026 (version 36)}
...
by a fixed finite singularity refinement.
...
energy scale; its stopped terminal law carries all context and mark moments.
A mass-preserving resolving buffer and Jensen then give the reduced-slope
small-ball estimate.  The
...
native-stopping, physical-polynomial-escape, branch-section, geometric-flux,
endpoint-upper-scale, split-standard-family, connected-pullback, shallow spacetime refinement,
...
\item The same double-projected typed decomposition holds for the limiting
      ...
      \label{eq:regular-DQ-weak-convergence}
       ...
       \longrightarrow0.
      \end{equation}
      For the face type, fix a finite depth $L$ and let
      ...
      \begin{align}
       ...
       \sup_{0<|s|<s_0}|\mathcal J_{s,L}^{\rm face}|
          ([\mathcal S_L]_\rho)
       +|\mathcal J_{V,L}^{\rm face}|([\mathcal S_L]_\rho)
          &\le C_L\rho^{\theta_L},\qquad 0<\rho<1.
       \label{eq:face-DQ-boundary-tightness}
      \end{align}
      Here $\mathcal S_L$ is the finite union of genuine singularity and
      common-atlas boundaries encountered by fixed-depth tests.  In
      ...
      bounded test that is branchwise continuous off $\mathcal S_L$.  The
      ...
      $\mathcal S_L$, with uniform sup norm and the boundary tightness in
      \eqref{eq:face-DQ-boundary-tightness}.  Hence
      \eqref{eq:regular-DQ-weak-convergence} and
      \eqref{eq:face-DQ-weak-convergence} give their fixed $(m,n)$ limits.
...
tends to zero.  The regular and face pairings converge for every fixed
$(m,n)$ in the topologies
\eqref{eq:regular-DQ-weak-convergence} and
\eqref{eq:face-DQ-weak-convergence}, respectively.  Consequently, once the
...
\item the homogeneous buffer and every retained continuation are
      ...
      $Ce^{-c_{\rm cont}N}$, under the same stopped actual/split marked law;
\item the context variable $H_\beta$, the physical Mark--$Z$ factors,
      ...
      conditional moments under this one stopped terminal law.  In
      ...
      \end{equation}
\end{enumerate}
The scale comparison and the stopped-law moments are part of one interface;
...
stopped actual conditional, every admissible positive mark, and every frozen
context used in $\mathrm{SS}(d_Z)$, all under the terminal law in
$\mathrm{NST}_{\rm phys}$.
...
\item For every $P\in\mathbb R_{d_Z}[z]$ the endpoint series at the
      ...
      conditional.
\item Multiplication by a positive amplitude preserves
      \eqref{eq:ppe-prescribed-sublevel} with the corresponding mark.
...
\end{equation}
On the unique stopped terminal label law define the complete positive event
weight
\begin{equation}
...
The actual stopped law of $\mathrm{NST}_{\rm phys}$ and every actual/split
law used by the functional-correlation estimate satisfy, with the
...
\end{equation}
The same label law is used for the native stopping, PPE, and endpoint marks;
...
\begin{lemma}[Stopped gate, buffer, and native scale]
...
\label{eq:bad-context-native-scale}
 ...
 \le C\mathbb E_{\rm stop}(H_\beta^p)\,r^{\Theta_{\rm ctx}}.
\end{equation}
...
There are named constants $c_{\rm buf},c_{\rm gate},c_{\rm cont}>0$ such
that the retained buffer, gate, and exceptional-continuation tails are at
most
\begin{equation}
\label{eq:named-energy-tails}
 C\left(e^{-c_{\rm buf}N(r)}+e^{-c_{\rm gate}N(r)}
 +e^{-c_{\rm cont}N(r)}\right).
\end{equation}
The exceptional native stopping mass is $O(r^{\Theta_{\rm st}})$.
\end{lemma}

\begin{proof}
The first assertion is \eqref{eq:joint-native-good-gate} together with
$\kappa^{N(r)}\le r/100$.  By
\eqref{eq:native-stopping-comparability}, Markov's inequality, and
\eqref{eq:stopped-H-moment},
\[
 \mathbb P_{\rm stop}\{H_\beta\sigma_Q^{1+\alpha_B}>r/10\}
 \le C\mathbb E_{\rm stop}(H_\beta^p)
 \left(\frac{\sigma(r)^{1+\alpha_B}}r\right)^p
 \le Cr^{\Theta_{\rm ctx}}.
\]
The remaining claims are \textup{(NST1)--(NST3)}.
\end{proof}

\begin{proposition}[Primitive physical polynomial escape]
\label{prop:physical-legal-child}
Under $\mathrm{PPE}_{N}(d_Z)$, uniformly for the admissible context
polynomials $P\in\mathbb R_{d_Z}[z]$, every native interval obeys
\begin{equation}
...
\begin{proof}
This is exactly \textup{(PPE1)--(PPE2)}.  The purpose of the proposition is
to type the primitive physical input used by the energy argument.  It is not
a deduction from the fixed clean subsystem.  Lemma
...
\label{eq:physical-energy-exponent}
 \Theta_E=\min\left\{\Theta_{\rm PPE},
        \Theta_{\rm ctx},\Theta_{\rm st}\right\}>0,
\end{equation}
one has
\begin{align}
 ...
 &\le Cr^{\Theta_E}+C\mathcal T_E(r),\label{eq:conditional-energy-bound}\\
 \sup_z\sum_\beta w_\beta\nu_\beta([z-r,z+r])
 &\le Cr^{\Theta_E/2}+C\mathcal T_E(r)^{1/2}.
...
\label{eq:complete-energy-tail}
 \mathcal T_E(r):=
 e^{-c_{\rm buf}N_{\rm PPE}(r)}
 +e^{-c_{\rm gate}N_{\rm PPE}(r)}
 +e^{-c_{\rm cont}N_{\rm PPE}(r)}.
\end{equation}
\end{theorem}

\begin{proof}
Fix $t'$ and all frozen physical data, and disintegrate first over the
$\mathrm{NST}_{\rm phys}(r)$ stopped terminal law.  By
...
handles bad contexts, \textup{(NST1)} handles the exceptional stopping set,
and \eqref{eq:named-energy-tails} gives exactly $\mathcal T_E(r)$.  Integrate in
...
$C(r^{\Theta/2}+\mathcal T_E(r)^{1/2})$.  Smooth value-space cutoffs on its complement have
...
\[
 ...
 c_{\rm aux}:=\min\{c_{\rm buf},c_{\rm gate},c_{\rm cont}\},
\]
\begin{equation}
\label{eq:complete-phase-exponent}
 \beta_{\rm tail}:=\tfrac12\delta C_{\rm buf}c_{\rm aux},\qquad
 \beta_{\rm phase}:=
 \min\{\delta\Theta/2,\alpha-A_{\rm frac}\delta,
             \beta_{\rm tail}\}>0.
\end{equation}
...
\end{equation}
has saving \(C\Xi^{-\beta_{\rm phase}}\) times a fixed power of
...
\end{equation}
If $r_{\rm obs}>2+\beta_{\rm phase}$, the complete high lattice sum is
...
\label{eq:phase-energy-all-tails}
 C\Xi^{-\delta\Theta/2}
 +C\Xi^{-\delta C_{\rm buf}c_{\rm buf}/2}
 +C\Xi^{-\delta C_{\rm buf}c_{\rm gate}/2}
 +C\Xi^{-\delta C_{\rm buf}c_{\rm cont}/2}.
\end{equation}
...
$\beta_{\rm tail}$ in \eqref{eq:complete-phase-exponent}.
...
$e^{-\eta_{\rm lo}\beta_{\rm phase}N}$ in the intermediate range.  The
...
\(r_{\rm obs}>2+\beta_{\rm phase}\).  Together with scale-bad labels the
complete central exponent is
\begin{equation}
\label{eq:complete-central-exponent}
 c_{\rm cen}:=\min\{\eta_{\rm lo}\beta_{\rm phase},
 \eta_{\rm hi}\rho_\infty-a_{\rm ultra},c_{\rm scl},
 c_{\rm low},c_{\rm event}\}>0,
\end{equation}
where $c_{\rm low}$ is supplied by the low recovery theorem and
$c_{\rm event}>0$ denotes the contracted-coordinate/event-tail rate in
...
\item At each cutoff coarea currents converge at fixed depth in the explicit
      bounded--Lipschitz current topology
      \eqref{eq:face-DQ-weak-convergence}, with the boundary tightness
      \eqref{eq:face-DQ-boundary-tightness}.  The
...
$\mathrm{USC}_{\rm end}^{W}(q_g^W)$ on the stopped actual/split laws, are required
...
$\mathrm{SS}$, $\mathrm{PPE}_{N}$,
$\mathrm{USC}_{\rm end}^{W}$,
$\mathrm{BR}_{\rm sec}$, $\mathrm{REC}$, $\mathrm{MT}_{\rm DQ}$, the
terminal labelled ledger \eqref{eq:global-ledger-sum}, and
...
First fix the complete event weight $\mathscr W_\lambda$, the available
positive-scale exponent $a_{\rm scl}q_S$, and either the primitive tilted
rate $q_g^W$ or the pair $(p_W,q_g)$ giving
$q_g^W=q_g/p_W'$.  Next fix the coefficient of $\log S_\lambda^\pm$ in
...
the proof: it improves $\beta_{\rm tail}$ but enlarges $C_{\rm mid}$.
...
\begin{align}
 ...
 \notag
\end{align}
If $q_H^{-1}+q_F^{-1}+q_D^{-1}+q_S^{-1}<1$ and $q_R>1$ is chosen above the remaining
conjugate exponent, then every product of the five displayed marks is
...
component stage with exponents $q_F,q_S$ and their remaining conjugate, then at the native and
...
Definition~\ref{def:quantitative-circular-jets}.  Let $q_H,q_F,q_D,q_S>1$ be the
available context, flux, inverse-scale, and positive-scale moment exponents,
and assume
\begin{equation}
\label{eq:holder-conjugate-margin}
 q_H^{-1}+q_F^{-1}+q_D^{-1}+q_S^{-1}<1.
\end{equation}
...
\eqref{eq:separate-recovery-moments} are retained on the same terminal law
...
weighted stopped terminal law, before the phase estimate is applied.
\end{proof}
...
\item the finite-horizon functional-correlation bound of \cite{LS}, the
      ...
      exponents $q_H,q_F,q_D,q_S$ satisfying
      \eqref{eq:holder-conjugate-margin}; all four polynomial moments hold conditionally
...
\eqref{eq:regular-DQ-weak-convergence}, and
\eqref{eq:face-DQ-weak-convergence}--
\eqref{eq:face-DQ-boundary-tightness}.  The shallow long-time part converges to zero by
...
\label{eq:final-CM2-exponent}
 ...
 \eta_{\rm lo}\beta_{\rm phase},
 \eta_{\rm hi}\rho_\infty-a_{\rm ultra},c_{\rm scl},
 c_{\rm event},\eta_{\rm lo}R,
...
\eqref{eq:regular-DQ-weak-convergence}.  The face fixed-term limit follows
from \eqref{eq:face-DQ-weak-convergence} and the boundary-neighbourhood
estimate \eqref{eq:face-DQ-boundary-tightness}; its uniform CM2 majorant is
instead supplied by the event/section/positive-envelope ledgers.  Thus no
...
Conditions $\mathrm{NST}_{\rm phys}$, $\mathrm{SS}$, and
$\mathrm{PPE}_{N}$, together with the stopped mass-preserving buffer, give
...
$L(s)$.  Theorem~\ref{thm:shallow-two-threshold} removes shallow times beyond
...
assertion is used in this step.  The three fixed-term topologies and all
uniform ledgers now meet exactly in
...
$\mathrm{UCR}_{\rm sh}$, $\mathrm{SHCOL}_{\rm env}$,
$\mathrm{TAIL}_{\rm env}^{+}$, and
...
\eqref{eq:face-DQ-boundary-tightness}; independently, it has the CM2
majorant of Theorem~\ref{thm:global-cm2-assembly}, while the deep
...
upper-scale excess tail is uniform on the same stopped labels by
$\mathrm{USC}_{\rm end}^{W}$; no raw lattice cutoff is used to bound endpoint
derivative growth.  The
...
$C_L\rho^{\theta_L}$, which is exactly
...
\begin{array}{ccl}
 ...
 \textup{(H5)}&\Longrightarrow&
   \text{fixed-gauge finite-DQ regular/face/three-space response blocks}.
\end{array}
\]
\endgroup
...
$K^{-\beta_{\rm phase}}$; the ultra-high original-observable tail is
...
\eqref{eq:regular-DQ-weak-convergence} and
\eqref{eq:face-DQ-weak-convergence} for fixed-term convergence, while the
...
There are fifteen falsifiable interfaces.  $\mathrm{MT}_{\rm DQ}$ must
...
tilted event weight on that same stopped actual/split law.
...
\begin{proposition}[Nineteen obstruction tests]
\label{prop:v36-obstruction-tests}
...
\item If $\beta_{\rm tail}<
      \min\{\delta\Theta/2,\alpha-A_{\rm frac}\delta\}$, the old phase
      exponent overstates the resonant saving.  The minimum
      \eqref{eq:complete-phase-exponent} returns the correct
      $\beta_{\rm phase}=\beta_{\rm tail}$.
...
\begin{proof}
Items 1--3, 8--11, and 13--19 are direct finite-measure, frequency, or norm
