#!/usr/bin/env python3
"""Hardened append-only r2 verifier for the zero-credit C27R2 gate v3."""
...
SOURCE_PINS = {label: SOURCE_HASHES[path]
               for label, path in SOURCE_LABELS.items()}
...
POSITIVE_KEYS = {
    "schema", "status", "decision", "intended_process_exit_code",
    "self_test_mode", "actual_v2_terminal_evidence",
    "derived_post_rebuild_contract", "actual_v2_terminal_gate_validated",
    "fresh_C27R2_rebuild_may_start", "formal_credit",
    "manifest_authorized", "C27R2", "C28", "C29", "CM2",
    "gate_receipt_sha256",
}
NEGATIVE_KEYS = {
    "schema", "status", "decision", "intended_process_exit_code",
    "self_test_mode", "missing_required_paths",
    "actual_v2_terminal_gate_validated", "fresh_C27R2_rebuild_may_start",
    "formal_credit", "manifest_authorized", "C27R2", "C28", "C29",
    "CM2", "gate_receipt_sha256",
}
EVIDENCE_KEYS = {
    "terminal_receipt", "terminal_receipt_object_sha256", "terminal_replay",
    "terminal_replay_object_sha256", "base_root_manifest",
    "base_payload_manifest", "terminal_root_manifest",
    "terminal_payload_manifest", "base_payload_entries_verified",
    "terminal_payload_entries_verified",
    "all_payload_hashes_and_pre_post_stats_verified", "service",
    "source_attestations", "exact_census",
}
ATTESTATION_KEYS = {
    "path", "sha256", "size", "stat_fingerprint", "O_NOFOLLOW",
    "single_open_file_description_hash_parse_fstat",
}
...
def exact_attestation(path: Path, expected: str) -> dict[str, Any]:
    descriptor = os.open(path, os.O_RDONLY | getattr(os, "O_CLOEXEC", 0)
                         | getattr(os, "O_NOFOLLOW", 0))
    try:
        before = os.fstat(descriptor)
        need(stat.S_ISREG(before.st_mode) and before.st_nlink == 1,
             "attestation-regular-single-link:" + str(path))
        initial = fingerprint(before)
        state = hashlib.sha256()
        while block := os.read(descriptor, 4 << 20):
            state.update(block)
        need(state.hexdigest() == expected
             and fingerprint(os.fstat(descriptor)) == initial,
             "attestation-stable-pinned:" + str(path))
        ...
        return {
            "path": relative, "sha256": expected, "size": initial[4],
            "stat_fingerprint": initial, "O_NOFOLLOW": True,
...
def verify_payloads() -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    need(replay["schema"] == REPLAY_SCHEMA and replay["status"] == REPLAY_STATUS
         ...
         "replay-state")
    need(base["exact_census"] == terminal["exact_census"]
         == replay["exact_census"]
         and base["seed_invariant_mathematical_projection_sha256"]
             == terminal["seed_invariant_mathematical_projection_sha256"]
             == replay["seed_invariant_mathematical_projection_sha256"],
         "exact-census-and-projection-closure")
    ...
    need(strict_file(TERMINAL / "root_manifest.sha256",
         ...
         "terminal-root")
    need(base["payload_manifest"] == {
            "entry_count": 56, "file_sha256": base_payload_sha,
            "filename": "payload_manifest.sha256",
         }
         and terminal["terminal_payload_manifest"] == {
            "entry_count": 12, "file_sha256": terminal_payload_sha,
         }
         and replay["payload_entry_count"] == 56
         and replay["payload_manifest_file_sha256"] == base_payload_sha
         and replay["root_manifest_file_sha256"]
             == EXPECTED_HASHES[BASE_SEAL / "root_manifest.sha256"]
         and replay["base_receipt_file_sha256"] == base_receipt_sha
         and replay["base_receipt_object_sha256"] == base["receipt_sha256"],
         "manifest-descriptor-bindings")
    ...
    file_attestations = {
        "terminal_receipt": exact_attestation(
            TERMINAL / "terminal_receipt.json", terminal_receipt_sha),
        "terminal_replay": exact_attestation(
            TERMINAL / "terminal_replay.json",
            EXPECTED_HASHES[TERMINAL / "terminal_replay.json"]),
        "base_root_manifest": exact_attestation(
            BASE_SEAL / "root_manifest.sha256",
            EXPECTED_HASHES[BASE_SEAL / "root_manifest.sha256"]),
        "base_payload_manifest": exact_attestation(
            BASE_SEAL / "payload_manifest.sha256", base_payload_sha),
        "terminal_root_manifest": exact_attestation(
            TERMINAL / "root_manifest.sha256",
            EXPECTED_HASHES[TERMINAL / "root_manifest.sha256"]),
        "terminal_payload_manifest": exact_attestation(
            TERMINAL / "payload_manifest.sha256", terminal_payload_sha),
    }
    source_attestations = {
        label: exact_attestation(path, SOURCE_PINS[label])
        for label, path in SOURCE_LABELS.items()
    }
    binding = {
        "exact_census": terminal["exact_census"],
        "terminal_receipt_object_sha256": terminal["terminal_receipt_sha256"],
        "terminal_replay_object_sha256": replay["result_sha256"],
...
def verify_service(receipt: dict[str, Any],
                   terminal_object_sha256: str) -> dict[str, Any]:
    ...
    need(expected["unit"] == SERVICE_UNIT and expected["show"] == values
         and expected["show_sha256"] == hashlib.sha256(show.stdout).hexdigest(),
         "service-show-exact-binding")
    if values.get("LoadState") == "loaded":
        need(set(expected) == {"mode", "unit", "show", "show_sha256"}
             and expected["mode"] == "LOADED_INACTIVE_DEAD_SUCCESS_EXIT0"
        ...
        need(set(expected) == {
                "mode", "unit", "show", "show_sha256", "journal_sha256",
                "terminal_summary_line_sha256",
             }
             and journal.returncode == 0 and journal.stderr == b""
             and hashlib.sha256(journal.stdout).hexdigest()
                 == expected["journal_sha256"]
             and expected["terminal_summary_line_sha256"]
                 == hashlib.sha256(summary).hexdigest()
             and lines.count(summary.decode("ascii")) == 1
...
def verify_receipt(receipt_path: Path, producer: Path,
    ...
    if receipt["decision"] == "REJECT":
        need(set(receipt) == NEGATIVE_KEYS
             and receipt["status"] == NEGATIVE_STATUS
        ...
        need(set(receipt) == POSITIVE_KEYS
             and receipt["decision"] == "PASS"
             and receipt["status"] == POSITIVE_STATUS
        ...
        payloads, binding = verify_payloads()
        evidence = receipt["actual_v2_terminal_evidence"]
        need(type(evidence) is dict and set(evidence) == EVIDENCE_KEYS
             and evidence["terminal_receipt_object_sha256"]
                 == binding["terminal_receipt_object_sha256"]
             and evidence["terminal_replay_object_sha256"]
                 == binding["terminal_replay_object_sha256"]
             and evidence["exact_census"] == binding["exact_census"]
             ...
             and all(type(evidence[label]) is dict
                     and set(evidence[label]) == ATTESTATION_KEYS
                     and evidence[label] == expected
                     for label, expected in
                     binding["file_attestations"].items())
             and type(evidence["source_attestations"]) is dict
             and evidence["source_attestations"]
