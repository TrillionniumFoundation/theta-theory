...
OUTPUT = HERE / "cm2-round157-upper-endpoint-continuation-to-288h-2026-07-25.json"
SCHEMA = "cm2.round157.upper-endpoint-continuation-to-288h.v1"
...
def check_pins():
    ...
    require(
        prior["schema"] == "cm2.round156.upper-endpoint-continuation-to-256h.v1",
        "prior schema",
    )
...
def build_result():
    ...
    for row in rows:
        ...
        previous = row
    require(
        rows[0]["abs_delta_x_in_h_units"][0] == "256"
        and rows[-1]["abs_delta_x_in_h_units"][1] == "288",
        "endpoints",
    )
    count = len(rows)
