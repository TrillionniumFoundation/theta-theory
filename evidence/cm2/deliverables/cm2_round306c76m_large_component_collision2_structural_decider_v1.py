...
dimensional owner, and the only live branches are complete two-side
collision-two-to-three handoffs.
...
SCHEMA = "cm2.round306c76m.large-component-collision2-structural-decider.v1"
PREFIX = "cm2_round306c76m_large_component_collision2_structural_decider_v1"
...
BASE_MEMBERS = [DECISIONS, HANDOFFS, INCIDENCE, REGISTRY, RESULT, REPORT,
                LOCK, MANIFEST, OUTER]
PRECISION = 384
...
def publish(path: Path, raw: bytes) -> None:
    ...
    try:
        ...
        while view:
            ...
            view = view[count:]
        os.fsync(fd)
        os.lseek(fd, -1 if raw else 0, os.SEEK_END)
        if raw:
            need(os.read(os.open(path, os.O_RDONLY), 0) == b"", "fd-replay-open")
    finally:
        os.close(fd)
...
def registry_object() -> dict[str, Any]:
    body = {
        ...
        "branch_actions": {
            ...
            "H2_SINGLE_ZERO": "PHYSICAL_CHART_SEAM_GLUE_THEN_EXPECTED_BRANCH_TEST",
            "H2_DOUBLE_ZERO": "CODIMENSION_TWO_CHART_CORNER_GLUE_THEN_EXPECTED_BRANCH_TEST",
...
def worker() -> int:
    ...
    for item in payload["tasks"]:
        ...
        if klass == "UNRESOLVED_C41_COLLISION2_EVALUATION_EXCEPTION_OUTER":
            ...
            need(failing, "localized evaluation-domain branch")
...
def side_record(side: str, item: Mapping[str, Any], template: Mapping[str, Any],
    ...
    body = {"side": side, "cell_id": cell["cell_id"],
            ...
            "guard": ("Delta(owner)>0 AND near(owner)>0 AND "
                      "near(owner)<near(every_other_future_candidate) AND "
                      "official_wall_word=declared_word AND outgoing_chart=E"),
            "complete": True}
    return {**body, "occurrence_binding_sha256": digest(body)}


def output_rows(item: Mapping[str, Any], replay: Mapping[str, Any],
                templates: Mapping[str, Any], registry_sha: str
    ...
    sides = [side_record("REPRESENTATIVE", item, templates["original_c2"],
                         templates["original_c3"]),
             side_record("REFLECTED", item, templates["reflected_c2"],
                         templates["reflected_c3"])]
    handoff_fields = {
        "exact_owner": "branch-specific:G[0,1](representative)|G[0,0](reflected)",
        "discriminant": "all 55 Delta guards explicitly trichotomized",
        "root_order": "all future near-root pair differences explicitly trichotomized",
        "official_word": "side occurrence exact official_word_key_id",
        "chart": "E strict branch; H2=0 glued once by E/W half-open owner",
        "wall": "strict ordered-wall interior; endpoint/crossing equality terminalized",
        "homogeneity": "C37 occurrence-bound collar template, re-evaluated by guard",
        "incidence": "all Delta/order/wall/H2/denominator equality carriers owned once",
        "core": "collision3 template row bound separately on both physical sides",
        "terminal_margin": "conjunction of strict guarded algebraic inequalities",
    }
    handoff = {"schema": SCHEMA + ".collision3-handoff-row", "task_id": task_id,
               **identity_body, "semialgebraic_registry_object_sha256": registry_sha,
               "physical_sides": sides,
               "representative_reflected_glue": {
                   "horizontal_reflection_bijective": True,
                   "owner_map": {"G[0,1]": "G[0,0]"},
                   "same_physical_contact_outgoing_state": True,
                   "two_occurrences_not_two_credits": True,
                   "C37_reflected_occurrence_row_sha256":
                       templates["reflected_c2"]["row_sha256"]},
               "guarded_branch_handoff": {**handoff_fields,
                   "handoff_object_sha256": digest(handoff_fields)},
               "nonexpected_owner_or_word_or_chart_branch":
                   "WHOLE_BRANCH_STRICT_EXCLUSION",
               "all_equality_and_domain_boundary_branches_retained": True,
               "schema_complete_sealed_collision3_handoff": True,
               "conditional_side_handoff_envelope_count": 2,
               **ZERO}
    ...
    incidence = {"schema": SCHEMA + ".event-incidence-row", "task_id": task_id,
                 **identity_body, "semialgebraic_registry_object_sha256": registry_sha,
    ...
    decision = {"schema": SCHEMA + ".decision-row", "task_id": task_id,
                **identity_body, "scope_ordinal": item["scope_ordinal"],
                ...
                "semialgebraic_registry_object_sha256": registry_sha,
                "handoff_row_sha256": handoff["row_sha256"],
                "incidence_row_sha256": incidence["row_sha256"],
                "disposition": "SEALED_COLLISION3_HANDOFF",
                "allowed_branch_exits": ["WHOLE_TASK_STRICT_EXCLUSION",
                    "KNOWN_COMPONENT_CONNECTION",
                    "CEMETERY_OR_SOURCE_GRAZING_TERMINAL",
                    "SEALED_COLLISION3_HANDOFF"],
                "all_branch_exits_allowed": True,
                "whole_task_branch_partition_complete": True,
                "explicit_residual_branch_count": 0,
                "additional_dyadic_depth": 0,
                "candidate_is_authority": False,
                "global_consumption_ready": False, **ZERO}
    return closed(decision), handoff, incidence
...
def build(output: Path, batch_size: int) -> None:
    ...
    handoff_writer = Ledger(output / HANDOFFS, "SAME_TASK_ORDER_ONE_TWO_SIDE_HANDOFF_PER_TASK")
    ...
    with decision_writer, handoff_writer, incidence_writer:
        for offset in range(0, len(tasks), batch_size):
            ...
            completed = subprocess.run([sys.executable, str(SELF), "--worker"],
                input=canonical({"tasks": batch}), capture_output=True, check=True)
            ...
            for item, replay in zip(batch, replays, strict=True):
                decision, handoff, incidence = output_rows(
                    item, replay, templates, registry["object_sha256"])
    ...
    need(decision_writer.count == handoff_writer.count ==
         incidence_writer.count == EXPECTED_SCOPE, "output ledger counts")
    attacks = coherent_attacks()
    result_body = {
        "schema": SCHEMA + ".result",
        "status": "PASS_16436_OF_16436_EXACT_STRUCTURAL_C2_BRANCH_ATLASES__32872_TWO_SIDE_C3_HANDOFF_ENVELOPES__ZERO_RESIDUAL_ZERO_CREDIT",
        ...
        "disposition_census": {"SEALED_COLLISION3_HANDOFF": EXPECTED_SCOPE,
                               "WHOLE_TASK_STRICT_EXCLUSION": 0,
                               "KNOWN_COMPONENT_CONNECTION": 0,
                               "CEMETERY_OR_SOURCE_GRAZING_TERMINAL": 0},
    ...
    report = (
        ...
        "Only strict expected owner/word/chart branches enter the two-side C3 "
        "handoff; every equality and exception branch is retained explicitly.\n\n"
    ...
    manifest_members = [DECISIONS, HANDOFFS, INCIDENCE, REGISTRY, RESULT, REPORT, LOCK]
    manifest_raw = b"".join(
