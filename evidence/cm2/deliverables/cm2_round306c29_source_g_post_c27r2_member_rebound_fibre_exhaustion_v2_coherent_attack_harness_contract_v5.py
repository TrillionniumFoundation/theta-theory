...
future transaction and terminal-seal layer.  After each rejection receipt is closed, its entire private case is removed
without following links.  A canonical cleanup transcript remains, so later
evidence capture is strictly O_NOFOLLOW/single-link and bounded in space even
...
def clone_candidate(source: Path, target: Path,
                    expected: dict[str, str] | None = None) -> None:
    target.mkdir(mode=0o700)
    for name in sorted(FILES):
        clone_file(source / name, target / name)
        need((expected is None or file_sha(target / name) == expected[name])
             and target.joinpath(name).stat().st_nlink == 1
             and not target.joinpath(name).is_symlink(),
             "private clone exact bytes/regular single-link:" + name)
...
def remove_private_case(case: Path) -> None:
    """Remove one bounded private attack tree without following any link."""
    need(case.is_dir() and not case.is_symlink(), "private case directory")
    for current, directories, files in os.walk(case, topdown=False,
                                                followlinks=False):
        base = Path(current)
        for name in files:
            member = base / name
        ...
        for name in directories:
            ...
            if member.is_symlink():
                ...
                member.rmdir()
    case.rmdir()
    need(not case.exists() and not case.is_symlink(), "private case removed")
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for ordinal, (name, mutate, mode) in enumerate(attacks()):
        ...
        remove_private_case(case)
        receipt["private_case_removed_after_rejection"] = True
        receipts.append(receipt)
    need(list(work.iterdir()) == [], "all private cases removed before transcript")
    cleanup_body = {
        "schema": HARNESS_SCHEMA + ".private-case-cleanup.v1",
        "attack_count": len(receipts),
        "all_rejections_closed_before_case_removal": True,
        "all_private_cases_removed_without_following_links": True,
        "authoritative_candidate_unchanged": True,
    }
    cleanup_receipt = dict(cleanup_body)
    cleanup_receipt["cleanup_sha256"] = object_sha(cleanup_receipt)
    cleanup_path = work / "private_attack_cleanup.json"
    with cleanup_path.open("xb") as output:
        output.write(canonical(cleanup_receipt) + b"\n")
        output.flush()
        os.fsync(output.fileno())
    need(cleanup_path.stat().st_nlink == 1 and not cleanup_path.is_symlink(),
         "regular single-link cleanup transcript")
    ...
    body = {
        ...
        "post_rejection_private_case_cleanup": {
            "cleanup_file_sha256": file_sha(cleanup_path),
            "cleanup_object_sha256": cleanup_receipt["cleanup_sha256"],
            "all_34_private_cases_removed": True,
            "remaining_work_files_regular_single_link": True,
            "authoritative_candidate_unchanged": True,
        },
...
def self_test() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="cm2-c29-v2-case-cleanup-") as name:
        base = Path(name)
        source = base / "source"
        clone = base / "clone"
        source.write_bytes(b"copy-fallback-fixture\n")
        clone_file(source, clone)
        need(clone.read_bytes() == source.read_bytes()
             and clone.stat().st_nlink == 1, "self-test reflink/copy fallback")
        case = base / "case"
        case.mkdir()
        (case / "regular").write_bytes(b"x")
        (case / "link").symlink_to("regular")
        os.link(case / "regular", case / "hardlink")
        remove_private_case(case)
        need(not case.exists(), "self-test bounded private case removal")
    return {
