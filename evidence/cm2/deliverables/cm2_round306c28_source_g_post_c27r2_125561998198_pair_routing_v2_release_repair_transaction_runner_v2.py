#!/usr/bin/env python3
"""Append-only process transaction runner for the C28 release-repair chain.
...
SPEC_SCHEMA = BASE + "release-repair-process-command-spec.v2"
PINSET_SCHEMA = BASE + "release-repair-process-pinset.v2"
RUN_SCHEMA = BASE + "release-repair-process-run-attestation.v2"
RUN_STATUS = (
    "PASS_C28_RELEASE_REPAIR_PROCESS_EXIT0_NULL_SIGNAL_EMPTY_STDERR_"
    "EXACT_ENV_PRE_POST_SHA_NINE_STAT_AND_OUTPUT_INVENTORY__ZERO_CREDIT"
)
FAIL_SCHEMA = BASE + "release-repair-process-failure-receipt.v2"
PASS = b"PASS_C28_RELEASE_REPAIR_PROCESS_TRANSACTION__ZERO_CREDIT\n"
    ...
    try:
        ...
        if run_dir.exists() and not (run_dir / "PASS.lock").exists():
            body = {"schema": FAIL_SCHEMA,
                "status": "FAILED_CLOSED_C28_RELEASE_REPAIR_PROCESS",
            ...
            if not (run_dir / "FAILED.lock").exists():
                write_once(run_dir / "FAILED.lock",
                           b"FAILED_C28_RELEASE_REPAIR_PROCESS__ZERO_CREDIT\n")
        raise
    finally:
        for cap in captures:
            cap.close()
    ...
    try:
        if args.self_test:
            ...
            result = {"stage": "self_test",
                "status": "PASS_C28_RELEASE_REPAIR_TRANSACTION_RUNNER_V2_SELF_TEST"}
        else:
