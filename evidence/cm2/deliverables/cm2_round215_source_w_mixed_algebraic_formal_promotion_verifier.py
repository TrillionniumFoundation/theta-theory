...
CERTIFICATE_SCHEMA = (
    "cm2.round215.source-w-mixed-algebraic-formal-promotion.v1"
)
CERTIFICATE_NAME = (
...
VERIFICATION_SCHEMA = (
    "cm2.round215.source-w-mixed-algebraic-formal-promotion-"
    "verification.v1"
)
VERIFICATION_NAME = (
    "cm2_round215_source_w_mixed_algebraic_formal_promotion_"
...
PRODUCER_NAME = (
    "cm2_round215_source_w_mixed_algebraic_formal_promotion.py"
)
PRODUCER_SHA256 = (
...
def compose_expected(bounded: dict[str, Any]) -> dict[str, Any]:
...
def strict_decode(raw: bytes, label: str) -> dict[str, Any]:
    require(
        not raw.startswith(b"\xef\xbb\xbf") and b"\x00" not in raw,
        f"strict JSON encoding:{label}",
    )

    def pairs(values: list[tuple[str, Any]]) -> dict[str, Any]:
        result: dict[str, Any] = {}
        for key, value in values:
            require(key not in result, f"strict JSON duplicate:{label}")
            result[key] = value
        return result
    ...
    value = json.loads(
        ...
        object_pairs_hook=pairs,
        parse_float=lambda _x: (_ for _ in ()).throw(
            RuntimeError(f"strict JSON float:{label}")
        ),
        parse_constant=lambda _x: (_ for _ in ()).throw(
            RuntimeError(f"strict JSON constant:{label}")
        ),
    )
    require(isinstance(value, dict), f"strict JSON object:{label}")
    return value
...
def verify_certificate_document(
    ...
    require(
        document.get("schema") == CERTIFICATE_SCHEMA,
        "certificate schema",
    )
    require(
        isinstance(document.get("result"), dict)
        and document.get("result_sha256")
        == digest(document["result"]),
        "certificate genuine result signature",
    )
    require(
        canonical(document["result"]) == canonical(expected),
        "full expected Python-object/canonical certificate equality",
    )


def semantic_attack_suite(expected: dict[str, Any]) -> dict[str, Any]:
    ...
    add(
        "candidate_swap",
        lambda row: row["formal_promotion"]["promotion_rows"].reverse(),
    )
    add(
        "candidate_omit",
        lambda row: row["formal_promotion"]["promotion_rows"].pop(),
    )
    add(
        "candidate_key_rewrite",
        lambda row: row["formal_promotion"]["promotion_rows"][0]
        .__setitem__("origin_key", "W:N:07.00.11111011"),
    )
    add(
        "compact_q_candidate_injection",
        lambda row: row["selection_and_full_census"][
            "promoted_origin_keys"
        ].__setitem__(0, "COMPACT_Q_PRESENT_INJECTION"),
    )
    add(
        "new_cell_evidence_rewrite",
        lambda row: row["formal_promotion"]["promotion_rows"][0][
            "Round215_new_terminal_rows"
        ][0].__setitem__("method", "FORGED_METHOD"),
    )
    add(
        "inherited_nonexcluded_injection",
        lambda row: row["formal_promotion"]["promotion_rows"][0]
        .__setitem__(
            "Round180_inherited_nonexcluded_terminal_count", 1
        ),
    )
    add(
        "prior_leaf_omission",
        lambda row: row["formal_promotion"]["promotion_rows"][0]
        .__setitem__("Round176_prior_terminal_count", 4),
    )
    add(
        "prior_coverage_rewrite",
        lambda row: row["formal_promotion"]["promotion_rows"][0]
        .__setitem__("Round176_prior_coverage_numerator_64", 6),
    )
    add(
        "cell_restriction_flag_flip",
        lambda row: row["formal_promotion"]["promotion_rows"][0][
            "Round215_new_terminal_rows"
        ][0].__setitem__(
            "strict_closed_box_predicates_restrict_to_cell_"
    ...
    add(
        "Delta_endpoint_sign_flip",
        lambda row: row["formal_promotion"]["promotion_rows"][0][
            "Round215_new_terminal_rows"
        ][0]["closed_box_boundary_restriction_proof"].__setitem__(
            "Delta_p_lower_face_sign", "OVERWRAP"
        ),
    )
    add(
        "unique_first_owner_forge",
        lambda row: row["formal_promotion"]["promotion_rows"][0][
            "Round215_new_terminal_rows"
        ][0]["closed_box_boundary_restriction_proof"].__setitem__(
            "post_enhancement_unique_first_owner", r176.FROZEN_OWNER
        ),
    )
    add(
        "owner_mismatch_flag_flip",
        lambda row: row["formal_promotion"]["promotion_rows"][0][
            "Round215_new_terminal_rows"
        ][0]["closed_box_boundary_restriction_proof"].__setitem__(
            "unique_first_owner_mismatch_on_whole_closed_box", False
        ),
    )
    add(
        "Round176_face_owner_count_rewrite",
        lambda row: row["formal_promotion"]["promotion_rows"][0][
            "independent_lower_dimensional_owner_audit"
        ].__setitem__(
            "Round176_internal_2D_face_owner_row_count", 60
        ),
    )
    add(
        "Round180_edge_owner_hash_rewrite",
        lambda row: row["formal_promotion"]["promotion_rows"][0][
            "independent_lower_dimensional_owner_audit"
        ].__setitem__(
            "Round180_internal_1D_edge_owner_rows_sha256", "0" * 64
        ),
    )
    add(
        "owner_reduction_flag_flip",
        lambda row: row["formal_promotion"]["promotion_rows"][0][
            "independent_lower_dimensional_owner_audit"
        ].__setitem__(
    ...
    add(
        "legacy_ledger_boolean_trust_injection",
        lambda row: row["formal_promotion"]["promotion_rows"][0][
            "independent_lower_dimensional_owner_audit"
        ].__setitem__(
            "trusted_legacy_ledger_conclusion_boolean", True
        ),
    )
    add(
        "outer_internal_class_confusion",
        lambda row: row["formal_promotion"]["promotion_rows"][0][
            "independent_lower_dimensional_owner_audit"
        ].__setitem__(
            "internal_and_outer_strata_classes_are_disjoint", False
        ),
    )
    add(
        "outer_face_owner_hash_rewrite",
        lambda row: row["formal_promotion"]["promotion_rows"][0][
            "independent_lower_dimensional_owner_audit"
        ].__setitem__("outer_2D_face_owner_rows_sha256", "f" * 64),
    )
    add(
        "eight_margin_sign_flip",
        lambda row: row["bounded_probe_result"][
            "active_algebraic_census"
        ]["owner_margin_attack_fixture"][
            "enhanced_owner_margin_signs"
        ].__setitem__("E.first", "STRICT_POSITIVE"),
    )
    add(
        "source_chart_rewrite",
        lambda row: row["formal_promotion"]["promotion_rows"][0]
        .__setitem__("source_chart_id", "W:E"),
    )
    add(
        "active_cell_count_rewrite",
        lambda row: row["bounded_probe_result"][
            "active_algebraic_census"
        ].__setitem__("active_residual_cell_count", 18_431),
    )
    add(
        "global_650_split_rewrite",
        lambda row: row["bounded_probe_result"][
            "generalized_650_scope_split_outer_conservation"
        ]["global_650"].__setitem__("child_count", 182_775),
    )
    add(
        "ledger_credit_inflation",
        lambda row: row["ledger_composition"].__setitem__(
            "new_whole_origin_credit", 3
        ),
    )
    add(
        "D02_global_gate_promotion",
    ...
    for label, mutation in attacks:
        ...
        mutation(hostile)
        require(
            canonical(hostile) != expected_canonical,
            f"semantic attack mutation effective:{label}",
        )
        resigned_envelope = {
            "schema": CERTIFICATE_SCHEMA,
            "result": hostile,
            "result_sha256": digest(hostile),
        }
        require(
            resigned_envelope["result_sha256"]
            == digest(resigned_envelope["result"]),
            f"semantic hostile genuinely resigned:{label}",
        )
        try:
            verify_certificate_document(resigned_envelope, expected)
        except RuntimeError as error:
            require(
                "full expected Python-object/canonical" in str(error),
                f"semantic hostile rejected for equality:{label}:{error}",
            )
            rejected.append(label)
        else:
            raise RuntimeError(
                f"resigned semantic hostile accepted:{label}"
            )
    require(len(rejected) == 24, "24 semantic hostile attacks")
    return {
        "attack_count": len(rejected),
        "rejected_count": len(rejected),
        "rejected_labels": rejected,
        "all_hostiles_genuinely_resigned": True,
        "all_rejected_by_official_certificate_verification_entrypoint":
            True,
        "all_rejected_by_full_expected_canonical_equality": True,
    }
...
def json_attack_suite() -> dict[str, Any]:
    ...
    for label, raw in invalid:
        try:
            strict_decode(raw, label)
        except Exception:
            rejected.append(label)
    require(
        len(rejected) == len(invalid)
        and strict_decode(b'{"a":1}', "valid") == {"a": 1},
        "13/13 strict JSON attacks",
    )
    return {
        "attack_count": len(invalid),
        "rejected_count": len(rejected),
        "rejected_labels": rejected,
...
def exact_path_allowed(path: Path, exact_name: str) -> bool:
    try:
...
def path_attack_suite() -> dict[str, Any]:
    invalid = [
        ...
        HERE.parent / CERTIFICATE_NAME,
        HERE / ".." / "deliverables" / CERTIFICATE_NAME,
        HERE / "subdir" / CERTIFICATE_NAME,
        HERE / (CERTIFICATE_NAME + ".tmp"),
        HERE / "certificate.json",
        Path("/tmp") / CERTIFICATE_NAME,
        Path("/") / CERTIFICATE_NAME,
        HERE,
        HERE / ".",
        HERE / "",
        HERE / PRODUCER_NAME,
        HERE / PROBE_NAME,
        HERE / VERIFICATION_NAME,
        HERE / ("." + CERTIFICATE_NAME),
        HERE / (CERTIFICATE_NAME.upper()),
        HERE / (CERTIFICATE_NAME + "/child"),
    ]
    rejected = [
        os.fspath(path)
        for path in invalid
        if not exact_path_allowed(path, CERTIFICATE_NAME)
    ]
    symlink_parent_rejected = False
    symlink_root = Path(tempfile.mkdtemp(
        prefix="cm2_round215_path_attack_", dir="/tmp"
    ))
    symlink_parent = symlink_root / "deliverables-link"
    try:
        os.symlink(HERE, symlink_parent)
        symlink_parent_rejected = not exact_path_allowed(
            symlink_parent / CERTIFICATE_NAME,
            CERTIFICATE_NAME,
        )
    finally:
        ...
        symlink_root.rmdir()
    if symlink_parent_rejected:
        rejected.append("PARENT_SYMLINK")
    require(
        len(rejected) == 18
        and exact_path_allowed(
            HERE / CERTIFICATE_NAME, CERTIFICATE_NAME
        ),
        "18/18 path attacks including parent symlink",
    )
    output_invalid = [
        Path(CERTIFICATE_NAME),
        HERE.parent / CERTIFICATE_NAME,
    ...
    output_rejected = sum(
        not exact_path_allowed(path, CERTIFICATE_NAME)
        for path in output_invalid
    )
    require(output_rejected == 6, "6/6 producer output path attacks")
    return {
        "input_path_attack_count": len(invalid) + 1,
        "input_path_rejected_count": len(rejected),
        "parent_symlink_attack_rejected": symlink_parent_rejected,
        "producer_output_path_attack_count": len(output_invalid),
        "producer_output_path_rejected_count": output_rejected,
        "canonical_certificate_path_control_accepted": True,
    }


def output_object_allowed(path: Path, exact_name: str) -> bool:
    if not exact_path_allowed(path, exact_name):
        return False
...
def actual_file_object_attack_suite() -> dict[str, Any]:
    created: list[Path] = []
    input_rejected: list[str] = []
    output_rejected: list[str] = []

    def new_regular(prefix: str, payload: bytes) -> Path:
        descriptor, name = tempfile.mkstemp(prefix=prefix, dir=HERE)
        path = Path(name)
        created.append(path)
    ...
    control = new_regular(".cm2_round215_input_control_", b"{}")
    hardlink_target = new_regular(
        ".cm2_round215_input_hardlink_target_", b"{}"
    )
    symlink = HERE / (".cm2_round215_input_symlink_" + control.name)
    hardlink = HERE / (".cm2_round215_input_hardlink_" + control.name)
    fifo = HERE / (".cm2_round215_input_fifo_" + control.name)
    directory = HERE / (".cm2_round215_input_directory_" + control.name)
    empty = new_regular(".cm2_round215_input_empty_", b"")
    oversize = new_regular(
        ".cm2_round215_input_oversize_", b"x" * 1025
    )
    missing = HERE / (".cm2_round215_input_missing_" + control.name)
    try:
        symlink.symlink_to(control)
        ...
        attacks = [
            ("input_symlink", symlink, 4096),
            ("input_hardlink", hardlink, 4096),
            ("input_FIFO", fifo, 4096),
            ("input_directory", directory, 4096),
            ("input_empty", empty, 4096),
            ("input_oversize", oversize, 1024),
            ("input_missing", missing, 4096),
        ]
        for label, path, maximum in attacks:
            try:
                read_regular(path, maximum)
            except Exception:
                input_rejected.append(label)
            else:
                raise RuntimeError(
                    f"actual hostile input object accepted:{label}"
                )
        require(
            read_regular(control, 4096) == b"{}",
            "actual regular singleton input control",
        )

        output_target = new_regular(
            ".cm2_round215_output_target_", b"x"
        )
        output_objects: list[tuple[str, Path]] = []
        output_symlink = HERE / (
            ".cm2_round215_output_symlink_" + control.name
        )
        output_symlink.symlink_to(output_target)
        created.append(output_symlink)
        output_objects.append(("output_existing_symlink", output_symlink))
        output_hardlink = HERE / (
            ".cm2_round215_output_hardlink_" + control.name
        )
        os.link(output_target, output_hardlink)
        created.append(output_hardlink)
        output_objects.append(("output_existing_hardlink", output_hardlink))
        output_fifo = HERE / (
            ".cm2_round215_output_fifo_" + control.name
        )
        os.mkfifo(output_fifo)
        created.append(output_fifo)
        output_objects.append(("output_existing_FIFO", output_fifo))
        output_directory = HERE / (
            ".cm2_round215_output_directory_" + control.name
        )
        output_directory.mkdir()
        created.append(output_directory)
        output_objects.append(
            ("output_existing_directory", output_directory)
        )
        for label, path in output_objects:
            if not output_object_allowed(path, path.name):
                output_rejected.append(label)
            else:
                raise RuntimeError(
                    f"actual hostile output object accepted:{label}"
                )
        unused_output = HERE / (
            ".cm2_round215_output_absent_" + control.name
        )
        require(
            output_object_allowed(unused_output, unused_output.name),
            "absent canonical output object control",
        )
    finally:
        for path in reversed(created):
            try:
                ...
                pass
    require(
        len(input_rejected) == 7
        and len(output_rejected) == 4,
        "7/7 actual input and 4/4 actual output object attacks",
    )
    return {
        "actual_input_object_attack_count": 7,
        "actual_input_object_rejected_count": len(input_rejected),
        "actual_input_object_rejected_labels": input_rejected,
        "actual_output_object_attack_count": 4,
        "actual_output_object_rejected_count": len(output_rejected),
        "actual_output_object_rejected_labels": output_rejected,
        "regular_singleton_input_control_accepted": True,
        "absent_output_control_accepted": True,
        "temporary_objects_cleaned": True,
    }


def output_allowed(path: Path) -> bool:
    return output_object_allowed(path, VERIFICATION_NAME)


def atomic_write(path: Path, payload: bytes) -> None:
    require(output_allowed(path), "authorized Round215 verification output")
    descriptor, temporary = tempfile.mkstemp(
        prefix=".cm2_round215_verification_", suffix=".tmp", dir=HERE
    )
    tmp = Path(temporary)
    try:
        with os.fdopen(descriptor, "wb") as handle:
...
def verify() -> dict[str, Any]:
    # The full 596 -> 200 -> 198 / 18,432-cell rebuild happens before
    # the Round215 certificate is opened.
    independent_bounded = rebuild()
    expected = compose_expected(independent_bounded)
    producer_raw = read_regular(HERE / PRODUCER_NAME, 512 * 1024)
    probe_raw = read_regular(HERE / PROBE_NAME, 512 * 1024)
    require(
        hashlib.sha256(producer_raw).hexdigest() == PRODUCER_SHA256
        and hashlib.sha256(probe_raw).hexdigest() == PROBE_SHA256,
        "producer/probe source pins without import or execution",
    )
    certificate_raw = read_regular(
        HERE / CERTIFICATE_NAME, 16 * 1024 * 1024
    )
    certificate = strict_decode(certificate_raw, CERTIFICATE_NAME)
    verify_certificate_document(certificate, expected)
    require(
        certificate["result_sha256"]
        == EXPECTED_CERTIFICATE_RESULT_SHA256,
        "expected official certificate result hash",
    )
    semantic = semantic_attack_suite(expected)
    json_attacks = json_attack_suite()
    path_attacks = path_attack_suite()
    file_object_attacks = actual_file_object_attack_suite()
    rows = expected["formal_promotion"]["promotion_rows"]
    return {
        "status": "PASS_PARTIAL_FORMAL_ROUND215",
        "verdict": "PASS",
        ...
        "certificate_read_after_full_independent_rebuild": True,
        "producer_imported_or_executed": False,
        "probe_imported_or_executed": False,
        "independent_reconstruction": {
            "outcome_blind_registry_origin_count": 596,
            "Round201_incomplete_origin_count": 200,
            "frozen_seam_origin_count": 2,
            "active_origin_count": 198,
            "active_residual_cell_count": 18_432,
            "compact_q_outer_origin_count": 54,
            "global_generalized_origin_count": 650,
            "global_generalized_child_count": 182_776,
            "candidate_origin_count": 2,
            "candidate_new_terminal_cell_count": 16,
            "candidate_prior_terminal_count": sum(
                row["Round176_prior_terminal_count"] for row in rows
            ),
            "candidate_prior_coverage_numerator_64": sum(
                row["Round176_prior_coverage_numerator_64"]
                for row in rows
            ),
            "candidate_inherited_terminal_count": sum(
                row["Round180_inherited_terminal_count"]
                for row in rows
            ),
            "candidate_inherited_nonexcluded_terminal_count": sum(
                row[
                    "Round180_inherited_nonexcluded_terminal_count"
                ]
                for row in rows
            ),
            "candidate_closed_3D_leaf_count": sum(
                row["independent_lower_dimensional_owner_audit"][
                    "closed_3D_leaf_count"
                ]
                for row in rows
            ),
            "all_owner_rows_independently_reduced_to_excluded_3D":
                all(
                    row[
                        "independent_lower_dimensional_owner_audit"
                    ][
        ...
        "verified_ledger": expected["ledger_composition"],
        "strict_nonpromotion": expected["strict_nonpromotion"],
        "hostile_semantic_attacks": semantic,
        "strict_JSON_attacks": json_attacks,
        "path_and_output_safety_attacks": path_attacks,
        "actual_file_object_safety_attacks": file_object_attacks,
        "source_pins": {
            PRODUCER_NAME: PRODUCER_SHA256,
            PROBE_NAME: PROBE_SHA256,
        },
        "implementation_disclosure": {
            "verifier_does_not_import_or_execute_producer_or_probe":
                True,
            "verifier_reimplements_the_probe_math_and_formal_composition":
                True,
            "implementation_diverse_claim": False,
            "full_rebuild_precedes_certificate_read": True,
        },
    }
...
def main() -> None:
    ...
    parser.add_argument(
        "--output", type=Path, default=HERE / VERIFICATION_NAME
    )
