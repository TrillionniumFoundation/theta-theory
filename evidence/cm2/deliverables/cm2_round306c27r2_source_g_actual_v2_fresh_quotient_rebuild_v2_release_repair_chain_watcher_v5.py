#!/usr/bin/env python3
"""Fresh-path, exact-service watcher for the C27R2 v5 repair chain.
...
ENABLE_SCHEMA = BASE + "execution-enable-receipt.v5"
ENABLE_STATUS = (
    "AUTHORIZED_C27R2_RELEASE_REPAIR_V5_EXACT_PLAN_AND_SOURCES_FOR_ONE_"
    "FRESH_EXECUTION"
)
PREFLIGHT_SCHEMA = BASE + "watcher-preflight.v5"
STAGES = (
...
LAUNCH_SCHEMA = BASE + "stage-launch-receipt.v5"
...
SYSTEMD_RUN_POLICY = {"service_type": "exec", "working_directory": str(ROOT),
    ...
    "transient_collect": False, "remain_after_exit": True}
SYSTEMCTL_SHA = "7ba82b5ba146759c710e1b80fadaa3fdbc0f9b85c8fb2c8c3196b7b1a0037ef8"
SYSTEMD_RUN_SHA = "49f0bf95eb8a781b93853bf9fc981b4929dd0009f55a3e6db95534c0a2d11716"
...
def validate_plan(args: argparse.Namespace) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    need(type(sources) is dict and {"transaction_runner",
         "fd_bound_stage_wrapper"} <= set(sources),
    ...
    finalizer_inputs = plan.get("finalizer_input_members")
    need(type(finalizer_inputs) is list and len(finalizer_inputs) > 0
         and [row.get("path") for row in finalizer_inputs]
             == sorted({row.get("path") for row in finalizer_inputs})
         and all(type(row) is dict and set(row) == {"path", "sha256"}
                 and type(row["path"]) is str and valid_sha(row["sha256"])
                 for row in finalizer_inputs),
         "sorted exact finalizer predecessor member specs")
    ...
    need(type(finalizer) is dict and set(finalizer) == {"unit", "launch_mode",
         ...
         and finalizer["expected_active_state"] == "active"
         and finalizer["expected_sub_state"] == "exited"
...
def generate_and_run_stage(plan: dict[str, Any], plan_record: dict[str, Any],
    ...
    wrapper_spec = plan["sources"]["fd_bound_stage_wrapper"]
    wrapper = inside(wrapper_spec["path"])
    ...
    need(record(source)["sha256"] == source_spec["sha256"]
         ...
         and record(wrapper)["sha256"] == wrapper_spec["sha256"]
    ...
    pinset_body = {"schema": PINSET_SCHEMA, "status": PINSET_STATUS,
        ...
        "fd_bound_wrapper_path": wrapper_spec["path"],
        "fd_bound_wrapper_sha256": wrapper_spec["sha256"],
    ...
    launch_body = {"schema": LAUNCH_SCHEMA,
        "status": "FROZEN_C27R2_V5_STAGE_LAUNCH__ZERO_CREDIT", "stage": stage,
    ...
    need(expected_path.exists() and run.get("schema") == RUN_SCHEMA
         ...
         and run.get("fd_bound_wrapper_path") == wrapper_spec["path"]
         and run.get("fd_bound_wrapper_sha256") == wrapper_spec["sha256"]
         and valid_sha(run.get("fd_bound_request_file_sha256"))
         and valid_sha(run.get("fd_bound_request_object_sha256"))
         and valid_sha(run.get("fd_bound_input_map_sha256"))
         and valid_sha(run.get("fd_bound_output_dirfd_map_sha256"))
...
def finalizer_systemd_run_argv(plan: dict[str, Any],
    ...
    need(external_record(Path("/usr/bin/systemd-run"))["sha256"]
             == SYSTEMD_RUN_SHA,
         "frozen systemd-run binary")
    ...
    for member in plan["finalizer_input_members"]:
        current = record(inside(member["path"]))
        need(current["sha256"] == member["sha256"],
             "finalizer dynamic predecessor current plan pin")
        dynamic_predecessors.append(current)
    pinset_body = {"schema": BASE + "finalizer-pre-release-pinset.v1",
        "status": "FROZEN_FINALIZER_DYNAMIC_PREDECESSOR_PINS_UNDER_EX_LOCK",
        "plan_file_sha256": plan_record["sha256"],
        "plan_object_sha256": plan["plan_sha256"],
        "argv_template_sha256": spec["argv_template_sha256"],
        "argv_bindings": bindings, "argv_bindings_sha256": digest(bindings),
        "dynamic_predecessor_members": dynamic_predecessors,
        "dynamic_predecessor_members_sha256": digest(dynamic_predecessors),
        "publication_lock_object_sha256": lock_value["publication_lock_sha256"],
        "formal_credit": 0, "manifest_authorized": False,
        "authority_minted": False}
    pinset = {**pinset_body,
        "pre_release_pinset_sha256": digest(pinset_body)}
    pinset_path = finalizer_control / "pre_release_pinset.json"
    write_json(pinset_path, pinset)
    pinset_record = record(pinset_path)
    reopened_pinset, _ = document(pinset_path, "pre_release_pinset_sha256")
    need(reopened_pinset == pinset, "dynamic predecessor pinset reopened")
    inner.extend(["--pre-release-pinset", str(pinset_path),
        "--expect-pre-release-pinset-file-sha256", pinset_record["sha256"],
        "--expect-pre-release-pinset-object-sha256",
        pinset["pre_release_pinset_sha256"]])
    finalizer_projection_sha = digest({"path": inner[0],
        "argv": " ".join(inner)})
    ...
    environment["CM2_FINALIZER_EXECSTART_PROJECTION_SHA256"] = \
        finalizer_projection_sha
    ...
    command = ["/usr/bin/systemd-run", "--user", "--unit=" + finalizer["unit"],
        ...
        "--", *inner]
    command_body = {"schema": BASE + "frozen-finalizer-command.v5",
        "status": "FROZEN_FINALIZER_SYSTEMD_RUN_COMMAND_UNDER_EX_LOCK",
        "plan_file_sha256": plan_record["sha256"],
        "plan_object_sha256": plan["plan_sha256"],
        "publication_lock_object_sha256": lock_value["publication_lock_sha256"],
        "pre_release_pinset_file_sha256": pinset_record["sha256"],
        "pre_release_pinset_object_sha256": pinset["pre_release_pinset_sha256"],
        "expanded_finalizer_argv": inner,
        "expanded_finalizer_argv_sha256": digest(inner),
        "systemd_run_argv": command, "systemd_run_argv_sha256": digest(command),
        "finalizer_exec_start_projection_sha256": finalizer_projection_sha,
        "exact_environment": environment,
        "formal_credit": 0, "manifest_authorized": False,
        "authority_minted": False}
    frozen_command = {**command_body,
        "frozen_finalizer_command_sha256": digest(command_body)}
    frozen_command_path = finalizer_control / "frozen_finalizer_command.json"
    write_json(frozen_command_path, frozen_command)
    frozen_command_record = record(frozen_command_path)
    reopened_command, _ = document(
        frozen_command_path, "frozen_finalizer_command_sha256")
    need(reopened_command == frozen_command,
         "frozen finalizer command reopened under exclusive lock")
    body = {"schema": BASE + "finalizer-launch-receipt.v5",
        ...
        "systemd_run_file_sha256": SYSTEMD_RUN_SHA,
        "frozen_finalizer_command_file_sha256": frozen_command_record["sha256"],
        "frozen_finalizer_command_object_sha256":
            frozen_command["frozen_finalizer_command_sha256"],
        "pre_release_pinset_file_sha256": pinset_record["sha256"],
        "pre_release_pinset_object_sha256": pinset["pre_release_pinset_sha256"],
        "publication_lock_object_sha256": lock_value["publication_lock_sha256"],
        "dynamic_predecessor_pins_frozen_under_exclusive_lock": True,
        "formal_credit": 0, "manifest_authorized": False,
        "authority_minted": False}
    receipt = {**body, "finalizer_launch_receipt_sha256": digest(body)}
    receipt_path = finalizer_control / "finalizer_launch_receipt.json"
    write_json(receipt_path, receipt)
    reopened, _ = document(receipt_path, "finalizer_launch_receipt_sha256")
    need(reopened == receipt
         and reopened["systemd_run_argv"] == command
         and reopened["argv_bindings"] == bindings
         and reopened["frozen_finalizer_command_file_sha256"]
             == frozen_command_record["sha256"]
         and reopened["frozen_finalizer_command_object_sha256"]
             == frozen_command["frozen_finalizer_command_sha256"]
         and reopened["publication_lock_object_sha256"]
             == lock_value["publication_lock_sha256"],
         "frozen finalizer command/launch receipt/dynamic pins reopened under lock")
    return command
...
def verify_terminal(terminal: Path, plan: dict[str, Any],
    ...
    observed_service = {**finalizer, "invocation_id": state["InvocationID"],
        "exec_start_projection_sha256": digest(exec_start_projection(
            state["ExecStart"]))}
...
def execute(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        ...
        # The frozen successful core predecessor is RemainAfterExit and is
        # therefore correctly loaded/active/exited, not inactive/dead.
        verify_service(service_show(CORE_SERVICE["unit"]), CORE_SERVICE, False)
        validate_fresh_paths_under_lock(plan)
        if not args.enable_execution:
            return {"schema": PREFLIGHT_SCHEMA,
                "status": "PASS_DISABLED_V5_PLAN_SOURCE_SERVICE_LOCK_AND_FRESH_PATH_PREFLIGHT",
                "plan_file_sha256": plan_record["sha256"],
                "plan_object_sha256": plan["plan_sha256"],
        ...
        # Freeze and reopen the exact command plus all dynamic predecessor pins
        # while EX is still held.  After unlock no command is recomputed.
        ...
        verify_service(state, expected, False, expected_inner)
        ...
        terminal_objects = verify_terminal(terminal, plan, state,
            lock_value["publication_lock_sha256"])
        return {"schema": BASE + "watcher-completion.v5",
        "status": "PASS_C27R2_RELEASE_REPAIR_V5_JOINT_TERMINAL_AND_FINALIZER_SERVICE_GATE",
...
def self_test() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(dir=AUDIT,
            prefix="c27r2-watcher-v5-selftest-") as raw:
    ...
    return {"status": "PASS_C27R2_RELEASE_REPAIR_WATCHER_V5_SELF_TEST",
            "CM2": "NO-GO_FOR_CLAIM"}
