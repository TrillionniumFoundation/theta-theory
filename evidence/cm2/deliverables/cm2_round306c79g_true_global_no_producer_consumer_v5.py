#!/usr/bin/env python3
"""C79g v5 append-only zero-credit candidate builder.

This append-only v5 binds the independently verified C78s 13-member final
...
is permanently non-authoritative.  The separately committed v5 seal plus the
...
COLD_COORDINATION_PARENT_FD_ENV = "CM2_C79G_V5_COORDINATION_PARENT_FD"
...
SELF = OUT / "cm2_round306c79g_true_global_no_producer_consumer_v5.py"
...
V4_TO_V5_TRANSITION = OUT / "cm2_round306c79g_true_global_no_producer_consumer_v4_to_v5_static_launch_transition_receipt_v1.json"
STATIC_AUDIT_V5 = OUT / "cm2_round306c79g_true_global_no_producer_consumer_static_audit_v5.json"
COLD_LAUNCHER = OUT / "cm2_round306c79g_true_global_no_producer_consumer_cold_launch_v5.py"
COLD_MANIFEST = OUT / "cm2_round306c79g_true_global_no_producer_consumer_cold_launch_manifest_v5.sha256"
COLD_OUTER = OUT / "cm2_round306c79g_true_global_no_producer_consumer_cold_launch_outer_receipt_v5.json"
...
COLD_EXACT8 = (
    ...
    CONTRACT,
    SELF,
    OUT / "cm2_round306c79g_true_global_no_producer_consumer_independent_verifier_assembler_authority_consumer_v5.py",
    V4_TO_V5_TRANSITION,
    STATIC_AUDIT_V5,
    COLD_LAUNCHER,
)
...
def _linux_libc() -> ctypes.CDLL:
    need(sys.platform.startswith("linux"), "Linux-only v5 publication protocol")
...
class HeldSelf:
    ...
    def __init__(self, exec_inherited: int, source_inherited: int,
        ...
        need(SELF == OUT / "cm2_round306c79g_true_global_no_producer_consumer_v5.py",
...
def new_candidate_stage(outdir: Path, parent: HeldParent) -> tuple[Path, HeldMutableStage]:
    ...
    name = ".c79g-v5-candidate-stage-" + orientation + "-" + CHECKPOINT_OBJECT_PIN
...
def hold_static_freeze_trust(self_guard: HeldSelf) -> dict[str, Any]:
    ...
    their later hashes do not form a cycle.  The launcher SHA is the sole
    external static-file anchor, within the explicitly declared cold-start
    bootstrap/interpreter/kernel TCB.  The launcher reconstructs and holds the
    manifest and outer-last surface, which in turn pin current SELF bytes.
    """
    manifest_guard = HeldPinnedInput(
        COLD_MANIFEST, "C79g v5 cold-launch manifest", 0o444)
    outer_guard = HeldPinnedInput(
        COLD_OUTER, "C79g v5 cold-launch outer-last receipt", 0o444)
    manifest_entries = parse_manifest_ordered(
        manifest_guard.raw, "C79g v5 cold-launch exact8 manifest")
    ...
    need(len(manifest_by_path) == 8 and
         ...
         manifest_by_path[V4_REJECTION_SUPERSESSION] ==
             V4_REJECTION_SUPERSESSION_FILE_PIN,
    ...
    for path in COLD_EXACT8:
        if path != SELF:
            by_path[path] = HeldPinnedInput(
                path, "C79g v5 cold exact8:" + path.name, 0o444,
                manifest_by_path[path])
    v4_supersession_guard = by_path[V4_REJECTION_SUPERSESSION]
    v3_rejection_guard = HeldPinnedInput(
        V3_REJECTION, "C79g official v3 later rejection", 0o444,
    ...
    transition_guard = by_path[V4_TO_V5_TRANSITION]
    audit_guard = by_path[STATIC_AUDIT_V5]
    transition = strict_json(transition_guard.raw, "C79g v5 transition trust")
    audit = strict_json(audit_guard.raw, "C79g v5 static audit trust")
    v4_supersession = strict_json(
        v4_supersession_guard.raw, "C79g frozen v4 rejection/supersession")
    v3_rejection = strict_json(
        v3_rejection_guard.raw, "C79g official v3 later rejection")
    outer = strict_json(outer_guard.raw, "C79g v5 cold-launch outer trust")
    need(isinstance(transition, dict) and isinstance(audit, dict) and
         isinstance(v4_supersession, dict) and isinstance(v3_rejection, dict) and
         isinstance(outer, dict),
         "post-source and rejected-predecessor trust receipts are JSON objects")
    verify_object(transition, "C79g v5 transition trust")
    verify_object(audit, "C79g v5 static audit trust")
    verify_object(v4_supersession, "C79g frozen v4 rejection/supersession",
                  V4_REJECTION_SUPERSESSION_OBJECT_PIN)
    verify_object(v3_rejection, "C79g official v3 later rejection",
                  V3_REJECTION_OBJECT_PIN)
    verify_object(outer, "C79g v5 cold-launch outer trust")
    need(v4_supersession_guard.raw == canonical(v4_supersession) + b"\n" and
         v4_supersession.get("status") ==
             "FROZEN_APPEND_ONLY_V4_STATIC_NO_RUN_REJECTION__THREE_ROOT_DEFECTS__V5_SUCCESSOR_ONLY" and
         v4_supersession.get("receipt_path") ==
             str(V4_REJECTION_SUPERSESSION.relative_to(ROOT)) and
         v4_supersession.get("rejection", {}).get("v4_execution_allowed") is False and
         v4_supersession.get("no_run_attestation", {}).get(
             "v4_candidate_verification_completion_authority_surfaces_created") == 0 and
         v4_supersession.get("formal_global_closure_credit") == 0 and
         v4_supersession.get("D02_unlock") is False and
         v4_supersession.get("D02_started") is False and
         len(v4_supersession.get("frozen_v4_provisional_exact8", {}).get(
             "ordered_members", [])) == 8 and
         len(v4_supersession.get("root_defects", [])) == 3,
         "frozen v4 supersession is canonical, zero-credit, no-run, and exact8 closed")
    need(v3_rejection_guard.raw == canonical(v3_rejection) + b"\n" and
         ...
         v3_rejection.get("D02_started") is False,
         "official v3 later rejection remains canonical and permanently zero-credit")
    need(v3_rejection_guard.identity not in {
             guard.identity for guard in cold_ten_guards
         } and v3_rejection_guard.mount_id == cold_ten_guards[0].mount_id,
         "official v3 rejection is an additional distinct held predecessor surface")
    ...
    transition_pin = audited.get("v4_to_v5_transition_receipt", {})
    ...
    need(transition.get("status") ==
         ...
         rejected_v4.get("supersession_receipt", {}).get("file_sha256") ==
             V4_REJECTION_SUPERSESSION_FILE_PIN and
         rejected_v4.get("supersession_receipt", {}).get("object_sha256") ==
             V4_REJECTION_SUPERSESSION_OBJECT_PIN and
         rejected_v4.get("ordered_provisional_exact8") ==
             v4_supersession.get("frozen_v4_provisional_exact8", {}).get(
                 "ordered_members") and
         ...
         audit_predecessor.get("ordered_exact10") == predecessor.get("ordered_exact10") and
         ...
         audit_rejection.get("file_sha256") ==
             V3_REJECTION_FILE_PIN and
         audit_rejection.get("object_sha256") ==
             V3_REJECTION_OBJECT_PIN and
         audit_schema.get(
    ...
    return {
        "guards": [*by_path.values(), v3_rejection_guard,
                   manifest_guard, outer_guard],
        "by_path": {**by_path, V3_REJECTION: v3_rejection_guard},
        ...
        "audit": audit,
        "outer": outer,
        "v4_rejection_supersession": v4_supersession,
...
def ensure_launch_configuration() -> HeldSelf:
    ...
    need(all(value and len(value) <= 10 and
         ...
         SELF == ROOT / "deliverables/cm2_round306c79g_true_global_no_producer_consumer_v5.py",
    ...
    need(all(isinstance(value, str) and len(value) == 64 and
         ...
         "C79g v5 exact non-placeholder contract/schema/rejection pins")
    need(set(C78S_FINAL["pins"]) == set(C78S_NAMES) and
         ...
         "C79g v5 exact filled launch configuration")
...
def validate_static_authorities(static_trust: dict[str, Any]) -> dict[str, Any]:
    ...
    rejection_raw = held[V3_REJECTION].raw
    ...
    need(contract.get("effective_checkpoint_object_sha256") == CHECKPOINT_OBJECT_PIN and
         ...
         contract.get("v5_bundle", {}).get(
             ...
             "independent_verifier_assembler_authority_consumer_v5.py" and
         contract.get("v5_bundle", {}).get("closed_schema_validator_policy") == {
         ...
         "v5 frozen contract checkpoint/composite-only boundary")
    ...
    need(closed.get("$ref") == "#/$defs/coldLaunchedCommittedAuthority" and
         ...
         "append-only v5 cold root, zero-credit inner, and persisted zero boundary")
    rejection = strict_json(rejection_raw, "C79g official v3 later rejection")
    verify_object(rejection, "C79g official v3 later rejection",
                  V3_REJECTION_OBJECT_PIN)
    need(rejection.get("schema") ==
         ...
         rejection.get("target_exact_path") == str(V3_REJECTION.relative_to(ROOT)),
         "exact official v3 permanent later rejection semantics")
    ...
    return close_object({
        ...
        **self_guard.execution_proof(),
        ...
        "post_source_static_freeze_trust": {
            "v4_to_v5_transition_receipt": {
                "path": str(V4_TO_V5_TRANSITION.relative_to(ROOT)),
                "file_sha256": static_trust["transition_file_sha256"],
                "object_sha256": static_trust["transition_object_sha256"],
            },
            "static_audit_v5": {
                "path": str(STATIC_AUDIT_V5.relative_to(ROOT)),
...
def build(outdir: Path, self_guard: HeldSelf,
    ...
    lock = (
        "C79g v5 isolated candidate only; standalone non-authoritative. Formal-global credit "
