#!/usr/bin/env python3
"""C79g v16 append-only zero-credit candidate builder.

This append-only v16 binds the independently verified C78s 13-member final
...
is permanently non-authoritative.  The separately committed v16 seal plus the
...
COLD_COORDINATION_PARENT_FD_ENV = "CM2_C79G_V16_COORDINATION_PARENT_FD"
...
SELF = OUT / "cm2_round306c79g_true_global_no_producer_consumer_v16.py"
...
STATIC_AUDIT_V16 = OUT / "cm2_round306c79g_true_global_no_producer_consumer_static_audit_v16.json"
COLD_LAUNCHER = OUT / "cm2_round306c79g_true_global_no_producer_consumer_cold_launch_v16.py"
COLD_MANIFEST = OUT / "cm2_round306c79g_true_global_no_producer_consumer_cold_launch_manifest_v16.sha256"
COLD_OUTER = OUT / "cm2_round306c79g_true_global_no_producer_consumer_cold_launch_outer_receipt_v16.json"
...
COLD_EXACT8 = (
    ...
    CONTRACT,
    SELF,
    OUT / "cm2_round306c79g_true_global_no_producer_consumer_independent_verifier_assembler_authority_consumer_v16.py",
    V14_TO_V16_TRANSITION,
    STATIC_AUDIT_V16,
    COLD_LAUNCHER,
)

# Frozen predecessor authority for the v16 successor draft.  This is a
...
def _linux_libc() -> ctypes.CDLL:
    need(sys.platform.startswith("linux"), "Linux-only v16 publication protocol")
...
class HeldSelf:
    """Hold the v16 exec/source pair plus exact12 frozen v14 authority.
    ...
    def __init__(self, exec_inherited: int, source_inherited: int,
        ...
        need(len(expected_rows) == V16_CHILD_AUTHORITY_DESCRIPTOR_COUNT == 12 and
        ...
        try:
            ...
            need(SELF == OUT /
                 "cm2_round306c79g_true_global_no_producer_consumer_v16.py",
    ...
    def execution_proof(self) -> dict[str, Any]:
        return {
            ...
            "twelve_v14_inherited_authority_inputs_inherited_as_held_fds": True,
            "twelve_v14_inherited_authority_held_fds_path_identity_mount_and_hash_revalidated": True,
        }
...
def new_candidate_stage(outdir: Path, parent: HeldParent) -> tuple[Path, HeldMutableStage]:
    ...
    name = ".c79g-v16-candidate-stage-" + orientation + "-" + CHECKPOINT_OBJECT_PIN
...
def strict_bool_predecessor_and_successor_census(
        ...
        v16_sources: Mapping[str, bytes]) -> dict[str, Any]:
    """Prove frozen v5 defects and zero-unsafe complete v6/v7/v16 boxes."""
    ...
    v16 = {role: _strict_bool_need_census(v16_sources[role], role)
          for role in v16_roles}
    ...
    v16_unproved = sum(
        v16[role]["recursive_exact_bool_unproved_count"] for role in v16_roles)
    v16_arity = sum(
        v16[role]["arity_star_or_keyword_failure_count"] for role in v16_roles)
    v16_returns = sum(
        v16[role]["annotated_bool_return_failure_count"] for role in v16_roles)
    need(v6_unproved == 0 and v6_arity == 0 and v6_returns == 0 and
         v7_unproved == 0 and v7_arity == 0 and v7_returns == 0 and
         v16_unproved == 0 and v16_arity == 0 and v16_returns == 0,
         "v6/v7/v16 recursive exact-bool full boxes have zero unsafe or unproved sites")
    return {
        ...
        "same_seven_absent_from_v16": True,
        ...
        "v16_recursive_exact_bool_unproved_count": v16_unproved,
        ...
        "v16_direct_need_call_count": sum(
            v16[role]["direct_need_call_count"] for role in v16_roles),
...
def v6_held_self_identity_defect_regression(raw: bytes) -> dict[str, Any]:
    ...
    V16 first proves the complete three-site census, then independently locates
...
def _expected_rejected_prepublication_v13_receipt(
    ...
    """Project the frozen receipt into the exact seven-key v16 JSON surface."""
    return {
        ...
        "v16_successor_contract": copy.deepcopy(
            supersession.get("v16_successor_contract")),
    }
...
def _validate_v7_regression_section(
    ...
    need(strict_bool_census["v7_recursive_exact_bool_unproved_count"] == 0 and
         strict_bool_census["v16_recursive_exact_bool_unproved_count"] == 0 and
         strict_bool_census["all_twelve_sources_AST_parsed_and_compiled_in_memory"]
             is True and
         strict_bool_census["compiled_code_executed"] is False,
         label + ":v7/v16 full-callsite strict-bool closure")
    need(section.get("all_ten_file_pins_match") is True and
         section.get("all_declared_object_pins_match") is True and
         section.get("all_ten_regular_0444_nlink1") is True and
         section.get("exact8_manifest_reconstructs_first_eight_in_order") is True and
...
def _validate_final_static_audit(
    ...
    _validate_v12_regression_section(
        ...
        "final v16 static audit v12 predecessor")
    ...
    need(isinstance(supersession_receipt, dict) and
         ...
         "final v16 static audit rejected-prepublication v13 receipt")
    need(audit.get("v12_v5_rejection_shape_incident") ==
         ...
         "final v16 static audit v12 incident and exact9 historical witness")
    ...
    need(isinstance(dual, dict) and set(dual) == {
         ...
         }, "final v16 static audit dual-checker exact8 closure")
    ...
    need(helper_consensus.get("algorithm") ==
         ...
         "final v16 three-source helper AST, exact40 witness, and callsites")
    ...
    need(isinstance(attacks, dict) and set(attacks) == {
         ...
         "final v16 coherent attack exact7, 137/137 including ordered v11 "
    ...
    need(isinstance(sealed, dict) and set(sealed) == {
         ...
         "final v16 sealed-exec and scoped no-producer exact16 closure")
    ...
    need(isinstance(closure, dict) and set(closure) == {
         ...
         }, "final v16 schema/constructor exact21 closure")
    need(type(closure.get("schema_definition_count")) is int and
         ...
         "final v16 schema 46 defs/242 refs/0 unresolved/52 closed/0 mismatch")
    need(closure.get("strict_JSON_duplicate_keys_rejected") is True and
         ...
         "final v16 schema and live-protocol type-exact bool closure")
    actual_keywords = [
    ...
    need(closure.get("actual_schema_keyword_universe") == actual_keywords and
         ...
         "final v16 exact canonical schema keyword universes and digests")
    ...
    need(isinstance(shapes, dict) and set(shapes) == set(expected_shapes) and
         ...
         "final v16 exact nine output-shape key counts")
...
def hold_static_freeze_trust(self_guard: HeldSelf) -> dict[str, Any]:
    """Hold current v16 exact10 and every distinct append-only predecessor.
    ...
    manifest_guard = HeldPinnedInput(
        COLD_MANIFEST, "C79g v16 cold-launch manifest", 0o444)
    outer_guard = HeldPinnedInput(
        COLD_OUTER, "C79g v16 cold-launch outer-last receipt", 0o444)
    manifest_entries = parse_manifest_ordered(
        manifest_guard.raw, "C79g v16 cold-launch exact8 manifest")
    ...
    for path in COLD_EXACT8:
        if path != SELF:
            by_path[path] = HeldPinnedInput(
                path, "C79g v16 cold exact8:" + path.name, 0o444,
    ...
    need(len(current_cold_ten_guards) == 10 and
         ...
         "current v16 cold exact10 unique held identities on one statx mount")
    exact8_stats = [
    ...
    need(all(chronology.values()),
         "physical v16 exact8/manifest/outer freeze chronology")
    ...
    need(len(prior_history_guards) == 98 and
         ...
         len({guard.identity for guard in prepublication_guards}) ==
             V16_PREPUBLICATION_UNIQUE_LIVE_IDENTITY_COUNT and
         len(all_file_guards) == V16_TERMINAL_UNIQUE_LIVE_IDENTITY_COUNT and
         len({guard.identity for guard in all_file_guards}) ==
             V16_TERMINAL_UNIQUE_LIVE_IDENTITY_COUNT and
         group_vector == list(V16_TERMINAL_GROUP_VECTOR) and
    ...
    v16_consumer_path = COLD_EXACT8[4]
    strict_bool_census = strict_bool_predecessor_and_successor_census(
        ...
        {
            "producer_v16": self_guard.raw,
            "consumer_v16": by_path[v16_consumer_path].raw,
            "launcher_v16": by_path[COLD_LAUNCHER].raw,
        })

    transition_guard = by_path[V14_TO_V16_TRANSITION]
    audit_guard = by_path[STATIC_AUDIT_V16]
    transition = strict_json(transition_guard.raw, "C79g v16 transition trust")
    audit = strict_json(audit_guard.raw, "C79g v16 static audit trust")
    outer = strict_json(outer_guard.raw, "C79g v16 cold-launch outer trust")
    ...
    verify_object(transition, "C79g v16 transition trust")
    verify_object(audit, "C79g v16 static audit trust")
    verify_object(outer, "C79g v16 cold-launch outer trust")
    need(set(transition) == {
    ...
    contract_cross = strict_json(
        by_path[CONTRACT].raw, "C79g v16 contract cross-check")
    audit_schema = audit.get("schema_and_constructor_closure", {})
    transition_pin = audited.get("v14_to_v16_transition_receipt", {})
    ...
    _validate_v7_regression_section(
        published_v7, strict_bool_census, "v16 transition v7 predecessor")
    _validate_v7_regression_section(
        audit_v7, strict_bool_census, "v16 audit v7 predecessor")
    _validate_v8_regression_section(
        published_v8, v8_rollout_control_flow_incident,
        "v16 transition v8 predecessor")
    _validate_v8_regression_section(
        audit_v8, v8_rollout_control_flow_incident,
        "v16 audit v8 predecessor")
    _validate_v9_regression_section(
        published_v9, "v16 transition v9 predecessor")
    _validate_v9_regression_section(
        audit_v9, "v16 audit v9 predecessor")
    _validate_v10_regression_section(
        published_v10, v10_regression_label_prefix_incident,
        "v16 transition v10 predecessor")
    _validate_v10_regression_section(
        audit_v10, v10_regression_label_prefix_incident,
        "v16 audit v10 predecessor")
    _validate_v11_regression_section(
        published_v11, v10_colon_prefix_witness,
        "v16 transition v11 predecessor")
    _validate_v11_regression_section(
        audit_v11, v10_colon_prefix_witness,
        "v16 audit v11 predecessor")
    _validate_v12_regression_section(
        published_v12, "v16 transition v12 predecessor")
    _validate_v12_regression_section(
        audit_v12, "v16 audit v12 predecessor")
    ...
    need(transition.get("status") ==
         ...
         audit_v4.get(
             "v16_closed_object_required_property_mismatch_count") == 0 and
         audit_v4.get("same_defects_absent_from_v16") is True and
         ...
         "transition/audit pin current v16 and preserve exact v3/v4/v7 history")
    ...
    need(set(outer) == {
         ...
         "cold-launch v16 outer-last exact8 and zero-credit closure")
    ...
    return {
        ...
        "v16_terminal_unique_live_identity_count":
            V16_TERMINAL_UNIQUE_LIVE_IDENTITY_COUNT,
        "v16_terminal_group_vector": list(V16_TERMINAL_GROUP_VECTOR),
...
def ensure_launch_configuration() -> HeldSelf:
    ...
    need(FINAL_V16_CORE_PINS_INSTALLED is True,
         "v16 final core pins are not installed; runtime permanently disabled")
    need(all(value not in V16_DRAFT_CORE_PIN_SENTINELS for value in (
             CONTRACT_FILE_PIN, CONTRACT_OBJECT_PIN, CLOSED_SCHEMA_FILE_PIN)),
         "v16 draft contract/schema sentinels must all be replaced before runtime")
    ...
    need(all(value and len(value) <= 10 and
         ...
         SELF == ROOT / "deliverables/cm2_round306c79g_true_global_no_producer_consumer_v16.py",
    ...
    need(all(isinstance(value, str) and len(value) == 64 and
         ...
         "C79g v16 exact non-placeholder contract/schema/rejection pins")
    need(set(C78S_FINAL["pins"]) == set(C78S_NAMES) and
         ...
         "C79g v16 exact filled launch configuration")
...
def validate_static_authorities(static_trust: dict[str, Any]) -> dict[str, Any]:
    ...
    need(set(contract) == {
         ...
         contract.get("v16_bundle", {}).get(
             "independent_verifier_assembler_authority_consumer", {}).get("path") ==
             "deliverables/cm2_round306c79g_true_global_no_producer_consumer_"
             "independent_verifier_assembler_authority_consumer_v16.py" and
         contract.get("v16_bundle", {}).get("closed_schema_validator_policy") == {
         ...
         "v16 frozen contract checkpoint/composite-only boundary")
    ...
    need(closed.get("$ref") == "#/$defs/coldLaunchedCommittedAuthority" and
         ...
         "append-only v16 cold root, zero-credit inner, and persisted zero boundary")
...
def input_registry(state: dict[str, Any], self_guard: HeldSelf,
    ...
    return close_object({
        ...
        **self_guard.execution_proof(),
        ...
        "v16_predecessor_unique_live_identity_count":
            static_trust["v16_predecessor_unique_live_identity_count"],
        "v16_prepublication_unique_live_identity_count":
            static_trust["v16_prepublication_unique_live_identity_count"],
        "v16_terminal_unique_live_identity_count":
            static_trust["v16_terminal_unique_live_identity_count"],
        "v16_terminal_group_vector":
            static_trust["v16_terminal_group_vector"],
        "post_source_static_freeze_trust": {
            "v14_to_v16_transition_receipt": {
                "path": str(V14_TO_V16_TRANSITION.relative_to(ROOT)),
                "file_sha256": static_trust["transition_file_sha256"],
                "object_sha256": static_trust["transition_object_sha256"],
            },
            "static_audit_v16": {
                "path": str(STATIC_AUDIT_V16.relative_to(ROOT)),
        ...
        "current_v16_producer_source_content_opened_or_read": False,
        "current_v16_producer_source_content_imported_compiled_or_executed": False,
...
def build(outdir: Path, self_guard: HeldSelf,
    ...
    lock = (
        "C79g v16 isolated candidate only; standalone non-authoritative. Formal-global credit "
...
def v14_inherited_authority_draft_state(
    ...
    need(V16_DRAFT_RUNTIME_DISABLED is True and
         FINAL_V16_CORE_PINS_INSTALLED is False and
...
def main(argv: list[str] | None = None) -> int:
    ...
    if V16_DRAFT_RUNTIME_DISABLED and not FINAL_V16_CORE_PINS_INSTALLED:
        raise Reject(
