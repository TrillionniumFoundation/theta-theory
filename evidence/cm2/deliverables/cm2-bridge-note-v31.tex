...
\large Endpoint unfolding, delayed section recovery, positive-envelope oscillation,
and a conditional
pathwise CM2 criterion}
\author{Working note}
\date{13 July 2026 (version 31)}
...
an exact stopping-antichain cover; the physical weights are compared with
Gibbs weights only after conditioning on clean survival, with the relative
pressure factor retained.  Every good symbolic child is then refined
...
connected-pullback, parameter-uniform common-refinement, and pathwise inputs
...
On the transported collision fiber we use the Perron convention relative to
the invariant SRB probability $\mu_s$:
\begin{equation}
\label{eq:perron-composition-convention}
 P_sh=h\circ T_s^{-1},\qquad P_s^*f=f\circ T_s,qquad P_s\one=\one.
\end{equation}
Thus $P_s$ is the transfer operator on $L^1(\mu_s)$ and $P_s^*$ is its
$L^\infty(\mu_s)$ adjoint.  Let
\[
...
Since $P_s\Pi_s=\Pi_sP_s=\Pi_s$, for every $m\ge0$ and $n\ge1$,
\begin{equation}
\label{eq:centered-endpoint-iterates}
 Q_s^mg=g\circ T_s^{-m}\quad\text{if }\mu_s(g)=0,
 \qquad (Q_s^*)^nf=f\circ T_s^n-\mu_s(f).
\end{equation}
...
\item Leclerc's equilibrium state is chosen for the induced geometric
      potential $-\log J^uF$ on the clean subsystem.  Write
      $P_{\rm cl}:=P_{\rm clean}(-\log J^uF)-
      P_{\rm phys}(-\log J^uF)\le0$ for its relative pressure/escape
      exponent, while the clean equilibrium state itself is normalized by
      subtracting $P_{\rm clean}$ and the usual coboundary.  If
      $S_J(Q)$ is the part of $Q$ surviving in the clean alphabet for $J$
      induced steps, then for every clean word $w$ of length $J$,
      \begin{equation}
      \label{eq:pressure-normalized-physical-weight}
       \mu^u_{\beta,Q}(V_w\mid S_J(Q))\asymp\mu_{\rm eq}([w]),
       \qquad
       \mu^u_{\beta,Q}(S_J(Q))\asymp e^{JP_{\rm cl}}
      \end{equation}
      up to the recorded bounded distortion and trial mark.  Equivalently,
      the unnormalized physical word mass contains the survival factor
      $e^{JP_{\rm cl}}$; it is never compared directly with a normalized
      equilibrium weight.  Uniformly for every
      $P\in\mathbb R_{d_Z}[z]$, the finite cutting
      ...
      margin $|X_{\beta,Q}-P|\ge4\sigma_Q^{1+\alpha_L}$.  Consequently the
      bad physical fraction is at most $C\sigma_Q^\gamma$ only conditionally
      on the corresponding surviving trial.
\item On $V_{a_1\cdots a_J}$ the finite endpoint/holonomy series
      ...
      descendants are retained, and their conditional physical weights obey
      the comparison in \textup{(FT2)}.  The added depth is at most
...
fixed amount of actual SRB mass.  However, the pressure-corrected comparison
in \textup{(FT2)} and the cover/recurrence clauses in
\textup{(FT3)--(FT4)} are primitive
actual-SRB physical-thickening inputs: in particular, Leclerc's cutting theorem
does not supply the second-scale open cover, the conditional
Gibbs/physical comparison, the clean survival factor, or
...
continuous.  The relative-pressure bookkeeping is the same normalization
used for conditionally surviving open systems; see
\cite{CanestrariHoles,DLreview}.  Those references do not verify
\textup{(FT2)--(FT4)} for the present moving billiard.
\end{definition}
...
physical collision cylinders.  On each surviving trial,
\eqref{eq:pressure-normalized-physical-weight} transfers the Gibbs bad
fraction to a conditional physical bad fraction $C\sigma_Q^\gamma$.  The
factor $e^{JP_{\rm cl}}$ is common to all words in that trial and cancels only
in this conditional ratio; it is not discarded from unnormalized masses.
The shadow error
...
No change of polynomial is made.  The trial recurrence and physical cover
\eqref{eq:finite-antichain-physical-cover} sum the conditional bad fractions
against the actual trial masses.  Those masses sum to at most one, while the
first-ancestor overshoot assignment gives total uncovered mass $Ce^{-cJ}$
without a hidden covering step.  Hence
\[
 \mu^u_{\beta,Q}
 \{|X^{[J]}_{\beta,Q}-P|\le\sigma_Q^{1+\alpha_L}\}
...
\end{equation}
Let $g_s^\circ=g-\mu_s(g)$ and $f_s^\circ=f-\mu_s(f)$.  After the complete
...
\label{eq:exact-endpoint-unfolding}
 ...
 =\sum_\lambda\int a_{\lambda,s}(u)
   g_s^\circ(X^-_{\lambda,m,s}(u))
   f_s^\circ(Y^+_{\lambda,n,s}(u))\,du.
\end{equation}
The two marginal and constant product blocks are therefore assembled before
the face sum is estimated.  After the same-branch reduction, expand the
localized restrictions of the \emph{original centered} functions,
$g_s^\circ\circ\iota^-_\lambda$ and
$f_s^\circ\circ\iota^+_\lambda$, in the scalar quotient coordinates.  The
scaled composition clause in $\mathrm{BR}_{\rm sec}$ gives their $C^r$
Fourier decay with the recorded $\mathfrak d_\lambda$ and Mark costs,
uniformly in $m,n,s$; all time dependence lies in the
phase
\begin{equation}
\label{eq:unfolded-endpoint-phase}
 \psi_{\xi,\lambda,m,n,s}(u)
 =\xi_1\cdot X^-_{\lambda,m,s}(u)
  +\xi_2\cdot Y^+_{\lambda,n,s}(u).
\end{equation}
...
the fixed physical partition of unity and, after the branch-section
reduction, Fourier-expand the localized restrictions of the original
observables.  No derivative is applied to $Q_s^mg$ or
...
\begin{align}
 ...
 g(\iota^-_\lambda(X_\lambda(\bar s)))|\,d|\omega_\lambda|
 &\le Ce^{-c_-M}\|g\|_{C^1}|\omega_\lambda|,\notag\\
 \int|f(Y^+_{\lambda,N,s}(\bar s))-
 f(\iota^+_\lambda(Y_\lambda(\bar s)))|\,d|\omega_\lambda|
...
\begin{definition}[Spacetime uniform common refinement $\mathrm{UCR}$]
...
$\mathrm{UCR}$ requires a measurable master refinement with the following
properties.
\begin{enumerate}[label=\textup{(UCR\arabic*)}]
\item On each spacetime cell the word, orientation, connected-component
      index, and actual/split-law labels are independent of $\theta$; the
      branch and endpoint maps extend to the cell and obey the same cone,
      distortion, scaled-density, and parameter-derivative bounds uniformly
      for $0\le\theta\le1$.
\item The branch sections, physical thickening trials, full-tangent coarea
      charts, positive envelopes, and connected cylinder partitions are all
      restrictions of this one refinement.  Adjacent artificial boundaries
      cancel before absolute values, and every genuine moving boundary keeps
      its component index and Mark--$Z$ charge.  All words deeper than $L$
      are also retained as one signed terminal remainder: its individual
      moving words need not be continued, but its actual/split positive
      envelope and $\mathrm{CP}_{\rm env}$ constants must be uniform in
      $\theta$.  This is the aggregate to which
      Lemma~\ref{lem:finite-s-collar-admissibility} is applied.
\item The union of cells on which such a continuation fails has, for every
      actual or split constituent law, total marked mass at most
      ...
      \eqref{eq:strict-jet-tail-margin}.  The refinements are compatible when
      $L$ increases.
\end{enumerate}
The constants in $\mathrm{BR}_{\rm sec}$, $\mathrm{FT}$,
$\mathrm{FZ}_{\rm geom}$, $\mathrm{SF}_{\rm split}$,
$\mathrm{CP}_{\rm env}$, and $\mathrm{REC}(q_{\rm rec})$ are required to be
uniform on these cells.  This is a primitive moving-singularity interface;
compactness of the table chart alone does not imply it.
\end{definition}
...
while $\mathrm{UCR}$ supplies the common labels, components, branches, and
...
Assume $\mathrm{FZ}_{\rm geom}(\theta_Z)$,
$\mathrm{SF}_{\rm split}$, $\mathrm{CP}_{\rm env}$, and $\mathrm{UCR}$.
...
\begin{proof}
On the spacetime cells supplied by $\mathrm{UCR}$, the difference of the two moving domains is
...
$\mathrm{SF}_{\rm split}$, $\mathrm{CP}_{\rm env}$, $\mathrm{UCR}$, and the pathwise jet condition of
...
\item $\mathrm{SS}(d_Z)$ and
      ...
      parameter-uniform constants required by $\mathrm{UCR}$;
...
separate-recovery, geometric-flux, split-standard-family, connected-pullback,
and spacetime-common-refinement hypotheses of
...
condition; it does not supply $\mathrm{REC}$, $\mathrm{FT}$,
$\mathrm{BR}_{\rm sec}$, $\mathrm{CP}_{\rm env}$, or $\mathrm{UCR}$.
...
$\mathrm{UCR}$, all constants in the
...
\begin{proof}
The spacetime cells in $\mathrm{UCR}$, rather than compactness alone, make
...
There are nine falsifiable interfaces: $\mathrm{SS}$ must contain
exactly one Leclerc rough source; $\mathrm{FT}$ must thicken every finite
Leclerc child into an actual-SRB collision cylinder, with its
pressure-normalized conditional weight comparison and stopping-antichain
cover treated as primitive physical inputs;
...
$\mathrm{UCR}$ must continue all labels and estimates on one spacetime
refinement; and the chosen
