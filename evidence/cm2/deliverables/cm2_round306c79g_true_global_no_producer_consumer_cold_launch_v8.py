#!/usr/bin/env python3
"""Externally pinned cold launcher for the C79g v8 exact10 static bundle.
...
LAUNCHER_RELATIVE = Path("deliverables") / (BASE + "_cold_launch_v8.py")
...
ROOT = Path("/__C79G_V8_UNCONFIGURED_ROOT__")
...
V8_REJECTION_NAMESPACE = ROOT / "__unconfigured_v8_rejections__"
V8_LATER_REJECTION = V8_REJECTION_NAMESPACE / "rejection.json"
...
def configure_workspace_paths(root: Path) -> None:
    ...
    global V8_REJECTION_NAMESPACE
    global V8_LATER_REJECTION, SCHEMA, CONTRACT, PRODUCER, CONSUMER
    global TRANSITION, AUDIT, MANIFEST, OUTER, BASE7_PINS, EXACT8
    global V3_EXACT10_PINS, V4_FROZEN_DRAFT7_PINS, V5_EXACT10_PINS
    global V6_EXACT10_PINS, V7_EXACT10_PINS
    ...
    V8_REJECTION_NAMESPACE = RUNTIME / ("c79g-v8-rejections-" + CHECKPOINT)
    V8_LATER_REJECTION = V8_REJECTION_NAMESPACE / "rejection.json"
    SCHEMA = OUT / (BASE + "_schema_v8.json")
    CONTRACT = OUT / (BASE + "_contract_v8.json")
    PRODUCER = OUT / (BASE + "_v8.py")
    CONSUMER = OUT / (BASE + "_independent_verifier_assembler_authority_consumer_v8.py")
    TRANSITION = OUT / (BASE + "_v7_to_v8_static_launch_transition_receipt_v1.json")
    AUDIT = OUT / (BASE + "_static_audit_v8.json")
    MANIFEST = OUT / (BASE + "_cold_launch_manifest_v8.sha256")
    OUTER = OUT / (BASE + "_cold_launch_outer_receipt_v8.json")
    BASE7_PINS = {
    ...
    EXACT8 = (V7_OFFICIAL_REJECTION, SCHEMA, CONTRACT, PRODUCER,
...
INNER_SCHEMA = "cm2.round306c79g.true-global-no-producer-consumer.v8.inner-composite"
COLD_ROOT_SCHEMA = (
    "cm2.round306c79g.true-global-no-producer-consumer.v8."
    "cold-launched-committed-authority"
)
COLD_ROOT_DOMAIN = b"CM2_C79G_V8_COLD_LAUNCHED_AUTHORITY_ROOT_V1\0"
LIVE_PROTOCOL = "CM2_C79G_V8_COLD_TWO_PHASE_LIVE_ACK_V1"
LIVE_REQUEST_SCHEMA = (
    "cm2.round306c79g.true-global-no-producer-consumer.v8."
    "cold-live-commit-request"
)
LIVE_ACK_SCHEMA = (
    "cm2.round306c79g.true-global-no-producer-consumer.v8.cold-live-ack"
)
LIVE_RELEASE_SCHEMA = (
    "cm2.round306c79g.true-global-no-producer-consumer.v8.cold-live-release"
)
PREWRAPPER_BODY_DOMAIN = "CM2_C79G_V8_COLD_PREWRAPPER_BODY_V1"
TRANSACTION_BINDING_DOMAIN = "CM2_C79G_V8_COLD_TRANSACTION_BINDING_V1"
LIVE_ACK_BINDING_DOMAIN = "CM2_C79G_V8_COLD_LIVE_ACK_BINDING_V1"
SOURCE_FD_ENV = "CM2_C79G_V8_COLD_SOURCE_FD"
EXEC_FD_ENV = "CM2_C79G_V8_COLD_EXEC_FD"
COORDINATION_PARENT_FD_ENV = "CM2_C79G_V8_COORDINATION_PARENT_FD"
WORKSPACE_ROOT_ENV = "CM2_C79G_V8_COLD_WORKSPACE_ROOT"
WORKSPACE_ROOT_FD_ENV = "CM2_C79G_V8_COLD_WORKSPACE_ROOT_FD"
LAUNCHER_SHA_ENV = "CM2_C79G_V8_COLD_LAUNCHER_FILE_SHA256"
...
def expected_v5_published_rejected_segment() -> dict[str, Any]:
    """Construct the one exact v5 regression shape shared by v8 proofs."""
    names = (
...
def v6_held_self_identity_defect_regression(raw: bytes) -> dict[str, Any]:
    """Prove the frozen v6 build failure without importing or executing it."""
    try:
    ...
    need("identity" not in direct_methods and len(failing) == 1 and
         failing[0].lineno == 2041,
         "frozen v6 HeldSelf.identity missing and one deterministic callsite")
...
def validate_final_static_audit(
        ...
        predecessor_v7: "HeldV7PredecessorExact10") -> None:
    ...
    audit_keys = {
        ...
        "audited_v8_bundle", "predecessor_v3_exact10_regression",
    ...
    need(set(audit) == audit_keys and
         audit.get("schema") ==
             "cm2.round306c79g.true-global-no-producer-consumer.static-audit.v8" and
         audit.get("audit_path") == str(AUDIT.relative_to(ROOT)) and
         audit.get("effective_checkpoint_object_sha256") == CHECKPOINT and
         audit.get("status") ==
         "PASS_DUAL_STATIC_BYTES_GO_V8__PHYSICAL_COLD_FREEZE_PENDING__"
         "RUNTIME_NOT_AUTHORIZED",
         "final static audit exact top-level closure and PASS_V8 status")
    ...
    need(isinstance(input_a, dict) and set(input_a) == input_keys and
         ...
         "final static audit exact equal seventeen-pin A/B input census")
    expected_inputs = {
    ...
    expected_shapes = {
        "selfIdentity": 33, "independentConsumerProof": 53,
        "staticFreezeProof": 60, "coldLaunchProof": 96,
        "laterRejection": 46,
        "producerSourceRegistry": 45, "liveRequest": 15,
        "liveACK": 32, "liveACKCensus": 30,
    }
    need(isinstance(closure, dict) and set(closure) == closure_keys and
         ...
         closure.get("schema_definition_count") == 40 ==
             len(schema.get("$defs", {})) and
         type(closure.get("schema_ref_count")) is int and
         closure.get("schema_ref_count") == 212 ==
         ...
         closure.get("closed_object_count") == 46 == len(closed_nodes) and
    ...
    need(isinstance(no_run, dict) and set(no_run) == {
             "C79_entrypoint_executed", "C79_v8_runtime_artifact_count",
             "C79_v8_process_count", "pyc_or___pycache___created",
             "cold_manifest_or_outer_created_before_dual_GO",
         } and no_run.get("C79_entrypoint_executed") is False and
         type(no_run.get("C79_v8_runtime_artifact_count")) is int and
         no_run.get("C79_v8_runtime_artifact_count") == 0 and
         type(no_run.get("C79_v8_process_count")) is int and
         no_run.get("C79_v8_process_count") == 0 and
    ...
    def __init__(self, source: HeldFile) -> None:
        ...
        try:
            self.fd = os.memfd_create(
                "c79g-v8-child-exec", MFD_CLOEXEC | MFD_ALLOW_SEALING)
        except OSError as exc:
...
class HeldEmptyRejectionNamespace:
    ...
    def __init__(self, coordination: HeldCoordinationParent) -> None:
        coordination.verify()
        name = V8_REJECTION_NAMESPACE.name
        ...
        try:
            try:
                ...
                state = os.stat(name, dir_fd=coordination.fd,
                                follow_symlinks=False)
            self.path = V8_REJECTION_NAMESPACE
...
def construct_launcher_native_rejection(
    ...
    return close_object({
        ...
        "status": "PERMANENT_FAIL_CLOSED_REJECTION__ZERO_CREDIT",
        "effective_checkpoint_object_sha256": CHECKPOINT,
        "namespace_exact_path": str(V8_REJECTION_NAMESPACE.relative_to(ROOT)),
        "target_exact_path": str(V8_LATER_REJECTION.relative_to(ROOT)),
        "rejection_reason": "ORPHANED_OR_INCOMPLETE_C79G_V8_SURFACE",
...
def install_or_replay_launcher_native_rejection(
    ...
    try:
        try:
            ...
            namespace_fd = openat2_beneath(
                V8_REJECTION_NAMESPACE, os.O_RDONLY | os.O_DIRECTORY)
    except BaseException:
        if namespace_fd >= 0:
    ...
    try:
        ...
        path_before = root_lstat(V8_REJECTION_NAMESPACE)
        ...
        target = os.stat(V8_LATER_REJECTION.name, dir_fd=namespace_fd,
                         follow_symlinks=False) if V8_LATER_REJECTION.name in universe else None
        if target is not None:
            need(universe == {V8_LATER_REJECTION.name},
                 "idempotent rejection exact singleton")
            rejection_guard = HeldFile(
                V8_LATER_REJECTION, "existing launcher-native v8 rejection")
        else:
            ...
            try:
                ...
                os.fsync(namespace_fd)
            rejection_guard = HeldFile(
                V8_LATER_REJECTION, "new launcher-native v8 rejection")
        need(rejection_guard is not None,
             "launcher-native rejection file held")
        value = strict_json(rejection_guard.raw, "launcher-native v8 rejection")
        need(isinstance(value, dict), "launcher-native rejection mapping")
        verify_object(value, "launcher-native v8 rejection")
        os.fsync(rejection_guard.fd)
        os.fsync(namespace_fd)
        os.fsync(coordination.fd)
        after = os.fstat(namespace_fd)
        path_after = root_lstat(V8_REJECTION_NAMESPACE)
        need(value == expected and rejection_guard.raw == expected_raw and
             set(os.listdir(namespace_fd)) == {V8_LATER_REJECTION.name} and
...
class HeldBundle:
    ...
    def _initialize(self, expected_launcher_sha256: str,
                    ...
                    bootstrap: HeldBootstrapEntry) -> None:
        coordination.verify()
        need(SELF == OUT / (BASE + "_cold_launch_v8.py"),
        ...
        for path in (V7_OFFICIAL_REJECTION, CONTRACT, TRANSITION, AUDIT):
        ...
        need(self.base_objects[CONTRACT].get("status") ==
             ...
             self.base_objects[TRANSITION].get("status") ==
                 "STATIC_BYTES_CLOSED_V7_TO_V8__PHYSICAL_FREEZE_PENDING__RUNTIME_NOT_AUTHORIZED" and
             self.base_objects[AUDIT].get("status") ==
                 "PASS_DUAL_STATIC_BYTES_GO_V8__PHYSICAL_COLD_FREEZE_PENDING__RUNTIME_NOT_AUTHORIZED",
        ...
        # Current exact8 owns the sole v7-rejection fd.  Frozen v7 owns the
        # shared v6-rejection fd; frozen v6 owns v5 rejection; frozen v5 owns
        # the v4 supersession.  v3 keeps its separate singleton rejection.
        self.predecessor_v7 = HeldV7PredecessorExact10(
            coordination, self.by_path[V7_OFFICIAL_REJECTION])
        ...
        all_history_identities = (
            current_identities | predecessor_v7_identities |
        ...
        need(len(all_history_identities) == 58 and
             ...
             self.by_path[V7_OFFICIAL_REJECTION].identity not in
             ...
             "current v8 exact10, v7/v6/v5/v3 exact10, v3 rejection and v4 "
             "draft7 are 58 unique live files on one mount")
    ...
    def terminal_replay(self) -> None:
        ...
        self.predecessor_v7.terminal_replay(
            self.by_path[V7_OFFICIAL_REJECTION])
...
def cold_root(bundle: HeldBundle, coordination: HeldCoordinationParent,
    ...
    need(bundle.predecessor_v5 is not None and
         ...
         bundle.predecessor_v7 is not None,
         "v7 direct predecessor and v6/v5 history remain held for cold root")
    ...
    proof = {
        ...
        "current_exact8_first_member_is_v7_official_rejection":
            EXACT8[0] == V7_OFFICIAL_REJECTION,
        "v7_official_rejection_identity": {
            **bundle.by_path[V7_OFFICIAL_REJECTION].identity_object(),
            "object_sha256": V7_REJECTION_OBJECT_PIN,
        },
        "v7_official_rejection_file_sha256": V7_REJECTION_FILE_PIN,
        "v7_official_rejection_object_sha256": V7_REJECTION_OBJECT_PIN,
        ...
        "all_current_and_historical_58_identities_globally_unique_on_one_statx_mount": True,
        "current_v8_and_all_predecessor_unique_file_identity_count": 58,
        "current_v8_and_all_predecessor_same_statx_mount": True,
        **bundle.chronology,
        ...
        "reject_command_exempts_only_fresh_empty_v8_rejection_namespace": True,
...
def _validate_live_ack(
    ...
    expected_keys = {
        ...
        "static_v8_exact10_with_v7_exact10_and_official_rejection_v6_exact10_and_official_rejection_v5_exact10_and_official_rejection_v4_supersession_v3_exact10_and_official_rejection_terminal_replay_completed_by_launcher_before_request",
    ...
    expected_census = {
        ...
        "v7_official_rejection_in_static_policy_held_file_count": 1,
    ...
    need(set(ack) == expected_keys and ack.get("schema") == LIVE_ACK_SCHEMA and
         ...
         ack.get("static_v8_exact10_with_v7_exact10_and_official_rejection_v6_exact10_and_official_rejection_v5_exact10_and_official_rejection_v4_supersession_v3_exact10_and_official_rejection_terminal_replay_completed_by_launcher_before_request") is True and
...
def run_authorize_child(
    ...
    try:
        ...
        request = close_object({
            ...
            "wrapper_body_excludes_transaction_binding_request_ACK_and_RELEASE_to_avoid_hash_cycle": True,
            "transaction_binding_is_replay_identical_not_fresh_or_random": True,
            "static_v8_exact10_with_v7_exact10_and_official_rejection_v6_exact10_and_official_rejection_v5_exact10_and_official_rejection_v4_supersession_v3_exact10_and_official_rejection_terminal_replay_completed_by_launcher_before_request": True,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "reject":
            ...
            return 0
        # Every positive-capable command holds the exact-empty v8 rejection
