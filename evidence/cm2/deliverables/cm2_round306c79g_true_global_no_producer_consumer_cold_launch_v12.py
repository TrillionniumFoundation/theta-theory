#!/usr/bin/env python3
"""Externally pinned cold launcher for the C79g v12 exact10 static bundle.
...
LAUNCHER_RELATIVE = Path("deliverables") / (BASE + "_cold_launch_v12.py")
...
ROOT = Path("/__C79G_V12_UNCONFIGURED_ROOT__")
...
V12_REJECTION_NAMESPACE = ROOT / "__unconfigured_v12_rejections__"
V12_LATER_REJECTION = V12_REJECTION_NAMESPACE / "rejection.json"
...
def configure_workspace_paths(root: Path) -> None:
    ...
    global V12_REJECTION_NAMESPACE
    global V12_LATER_REJECTION, SCHEMA, CONTRACT, PRODUCER, CONSUMER
    ...
    V12_REJECTION_NAMESPACE = RUNTIME / ("c79g-v12-rejections-" + CHECKPOINT)
    V12_LATER_REJECTION = V12_REJECTION_NAMESPACE / "rejection.json"
    SCHEMA = OUT / (BASE + "_schema_v12.json")
    CONTRACT = OUT / (BASE + "_contract_v12.json")
    PRODUCER = OUT / (BASE + "_v12.py")
    CONSUMER = OUT / (BASE + "_independent_verifier_assembler_authority_consumer_v12.py")
    TRANSITION = OUT / (BASE + "_v11_to_v12_static_launch_transition_receipt_v1.json")
    AUDIT = OUT / (BASE + "_static_audit_v12.json")
    MANIFEST = OUT / (BASE + "_cold_launch_manifest_v12.sha256")
    OUTER = OUT / (BASE + "_cold_launch_outer_receipt_v12.json")
    BASE7_PINS = {
        ...
        # The remaining six entries are the final acyclic v12 static pins.
        # The audit binds this launcher through PIN_NORMALIZED AST, not raw bytes.
        SCHEMA: ("5ed911a55e8f90e750fdcaf6ab4fb6c9683bebaf66a4d6250329dff98acb2e28", None),
        CONTRACT: ("72246725b15891f92cbc6e3fa1c07ab4a76cdaa19f1b366a736f47f51989b343", "f5c1710817dc8e3aa7fe2bbf4b88e6e39baaa1b17bbeb0f8bf37c8c4575ae5d7"),
        PRODUCER: ("4cafc594f8a60063b7e7caaa82ddcc2b7983d0fbf0929034cd3cb5f360f3ed1d", None),
        CONSUMER: ("b74dee738257d485e9ca3357d7d138b5175b95e58f1c8ea87d1eca7bfc96b2ed", None),
        TRANSITION: ("45dabc1ef60eb2f8ba34aa7daa69f9d2b3bedccd4168d9c472bd4bbb696b5400", "0673ecafaa9991cd78e905e144e7c8c1b91717a3d753befa13e82552d44a4072"),
        AUDIT: ("ac29b1e31e0d1b51e8610b7699d1aaf55c80fa6f13f00b20b209c2889e121d37", "c1473ae0f5a08d8772227f61a55bd32c479a7b5c7d40da17b37e796abf4d9783"),
    }
    EXACT8 = (V11_OFFICIAL_REJECTION, SCHEMA, CONTRACT, PRODUCER,
...
INNER_SCHEMA = "cm2.round306c79g.true-global-no-producer-consumer.v12.inner-composite"
COLD_ROOT_SCHEMA = (
    "cm2.round306c79g.true-global-no-producer-consumer.v12."
    "cold-launched-committed-authority"
)
COLD_ROOT_DOMAIN = b"CM2_C79G_V12_COLD_LAUNCHED_AUTHORITY_ROOT_V1\0"
LIVE_PROTOCOL = "CM2_C79G_V12_COLD_TWO_PHASE_LIVE_ACK_V1"
LIVE_REQUEST_SCHEMA = (
    "cm2.round306c79g.true-global-no-producer-consumer.v12."
    "cold-live-commit-request"
)
LIVE_ACK_SCHEMA = (
    "cm2.round306c79g.true-global-no-producer-consumer.v12.cold-live-ack"
)
LIVE_RELEASE_SCHEMA = (
    "cm2.round306c79g.true-global-no-producer-consumer.v12.cold-live-release"
)
PREWRAPPER_BODY_DOMAIN = "CM2_C79G_V12_COLD_PREWRAPPER_BODY_V1"
TRANSACTION_BINDING_DOMAIN = "CM2_C79G_V12_COLD_TRANSACTION_BINDING_V1"
LIVE_ACK_BINDING_DOMAIN = "CM2_C79G_V12_COLD_LIVE_ACK_BINDING_V1"
SOURCE_FD_ENV = "CM2_C79G_V12_COLD_SOURCE_FD"
EXEC_FD_ENV = "CM2_C79G_V12_COLD_EXEC_FD"
COORDINATION_PARENT_FD_ENV = "CM2_C79G_V12_COORDINATION_PARENT_FD"
WORKSPACE_ROOT_ENV = "CM2_C79G_V12_COLD_WORKSPACE_ROOT"
WORKSPACE_ROOT_FD_ENV = "CM2_C79G_V12_COLD_WORKSPACE_ROOT_FD"
LAUNCHER_SHA_ENV = "CM2_C79G_V12_COLD_LAUNCHER_FILE_SHA256"
HELD_V10_PRODUCER_FD_ENV = "CM2_C79G_V12_V10_PRODUCER_FD"
HELD_V9_LAUNCHER_FD_ENV = "CM2_C79G_V12_V9_LAUNCHER_FD"
HELD_V11_PRODUCER_FD_ENV = "CM2_C79G_V12_V11_PRODUCER_FD"
HELD_V11_LAUNCHER_FD_ENV = "CM2_C79G_V12_V11_LAUNCHER_FD"
HELD_V11_REJECTION_FD_ENV = "CM2_C79G_V12_V11_REJECTION_FD"
...
def expected_v5_published_rejected_segment() -> dict[str, Any]:
    """Construct the one exact v5 regression shape shared by v12 proofs."""
    names = (
...
FINAL_STATIC_AUDIT_TOP_LEVEL_KEY_ORDER = (
    ...
    "audited_v12_bundle", "predecessor_v3_exact10_regression",
...
FINAL_STATIC_AUDIT_OUTPUT_SHAPES = {
    ...
    "laterRejection": 54, "producerSourceRegistry": 70,
...
def pin_normalized_launcher_ast_sha256(raw: bytes, label: str) -> str:
    ...
    six current-v12 BASE7 values are replaced by fixed draft sentinels.  The v11
    ...
    need(key_names == expected_key_names and
         ...
         label + ":exact ordered v11-plus-current-v12 BASE7 key census")
    v11_before = ast.dump(
    ...
    need(ast.dump(
         ...
         label + ":v11 official-rejection BASE7 value preserved")
...
def producer_source_registry_shape_from_ast(raw: bytes) -> int:
    ...
    try:
        tree = ast.parse(raw.decode("utf-8"), filename="producer_v12",
                         mode="exec")
    except (UnicodeDecodeError, SyntaxError) as exc:
        raise Reject("current v12 producer registry AST parse") from exc
    ...
    need(isinstance(registry_function,
         ...
         "current v12 input_registry and HeldSelf.execution_proof")
    registry_returns = [
    ...
    need(len(registry_returns) == 1 and len(proof_returns) == 1,
         "unique current v12 registry/proof return dicts")
    ...
    need(len(explicit_keys) == len(set(explicit_keys)) == 62 and
         ...
         "current v12 registry explicit62 plus exact execution-proof7")
...
def validate_final_static_audit(
    ...
    need(set(audit) == audit_keys and
         audit.get("schema") ==
             "cm2.round306c79g.true-global-no-producer-consumer.static-audit.v12" and
         ...
         "final static audit exact top-level closure and PASS_V12 status")
    ...
    expected_inputs = {
        ...
        "launcher_template": pin_normalized_launcher_ast_sha256(
            launcher_source_raw, "held_v12_launcher_source"),
    ...
    need(isinstance(no_producer, dict) and
         ...
         no_producer.get(
             "consumer_current_v12_producer_content_open_read_hash_decode_compile_import_or_execute_allowed") is False and
         ...
         "final static audit exact current-v12 no-producer and inherited incident evidence policy")
    ...
    need(isinstance(no_run, dict) and set(no_run) == {
             "C79_entrypoint_executed", "C79_v12_runtime_artifact_count",
             "C79_v12_process_count", "pyc_or___pycache___created",
             "cold_manifest_or_outer_created_before_dual_GO",
         } and no_run.get("C79_entrypoint_executed") is False and
         type(no_run.get("C79_v12_runtime_artifact_count")) is int and
         no_run.get("C79_v12_runtime_artifact_count") == 0 and
         type(no_run.get("C79_v12_process_count")) is int and
         no_run.get("C79_v12_process_count") == 0 and
    ...
    def __init__(self, source: HeldFile) -> None:
        ...
        try:
            self.fd = os.memfd_create(
                "c79g-v12-child-exec", MFD_CLOEXEC | MFD_ALLOW_SEALING)
        except OSError as exc:
...
class HeldEmptyRejectionNamespace:
    ...
    def __init__(self, coordination: HeldCoordinationParent) -> None:
        coordination.verify()
        name = V12_REJECTION_NAMESPACE.name
        ...
        try:
            try:
                ...
                state = os.stat(name, dir_fd=coordination.fd,
                                follow_symlinks=False)
            self.path = V12_REJECTION_NAMESPACE
...
def construct_launcher_native_rejection(
    ...
    return close_object({
        ...
        "status": "PERMANENT_FAIL_CLOSED_REJECTION__ZERO_CREDIT",
        "effective_checkpoint_object_sha256": CHECKPOINT,
        "namespace_exact_path": str(V12_REJECTION_NAMESPACE.relative_to(ROOT)),
        "target_exact_path": str(V12_LATER_REJECTION.relative_to(ROOT)),
        "rejection_reason": "ORPHANED_OR_INCOMPLETE_C79G_V12_SURFACE",
...
def install_or_replay_launcher_native_rejection(
    ...
    try:
        try:
            ...
            namespace_fd = openat2_beneath(
                V12_REJECTION_NAMESPACE, os.O_RDONLY | os.O_DIRECTORY)
    except BaseException:
        if namespace_fd >= 0:
    ...
    try:
        ...
        path_before = root_lstat(V12_REJECTION_NAMESPACE)
        ...
        target = os.stat(V12_LATER_REJECTION.name, dir_fd=namespace_fd,
                         follow_symlinks=False) if V12_LATER_REJECTION.name in universe else None
        if target is not None:
            need(universe == {V12_LATER_REJECTION.name},
                 "idempotent rejection exact singleton")
            rejection_guard = HeldFile(
                V12_LATER_REJECTION, "existing launcher-native v12 rejection")
        else:
            ...
            try:
                ...
                os.fsync(namespace_fd)
            rejection_guard = HeldFile(
                V12_LATER_REJECTION, "new launcher-native v12 rejection")
        need(rejection_guard is not None,
             "launcher-native rejection file held")
        value = strict_json(rejection_guard.raw, "launcher-native v12 rejection")
        need(isinstance(value, dict), "launcher-native rejection mapping")
        verify_object(value, "launcher-native v12 rejection")
        os.fsync(rejection_guard.fd)
        os.fsync(namespace_fd)
        os.fsync(coordination.fd)
        after = os.fstat(namespace_fd)
        path_after = root_lstat(V12_REJECTION_NAMESPACE)
        need(value == expected and rejection_guard.raw == expected_raw and
             set(os.listdir(namespace_fd)) == {V12_LATER_REJECTION.name} and
...
class HeldBundle:
    ...
    def _initialize(self, expected_launcher_sha256: str,
                    ...
                    bootstrap: HeldBootstrapEntry) -> None:
        coordination.verify()
        need(SELF == OUT / (BASE + "_cold_launch_v12.py"),
        ...
        for path, (file_pin, object_pin) in BASE7_PINS.items():
            need(re.fullmatch(r"[0-9a-f]{64}", file_pin) is not None and
                 file_pin != "0" * 64 and
                 (path == V11_OFFICIAL_REJECTION or
        ...
        for path in (V11_OFFICIAL_REJECTION, CONTRACT, TRANSITION, AUDIT):
        ...
        need(set(self.base_objects[TRANSITION]) == {
                 ...
                 "published_then_officially_rejected_predecessor_v10",
                 "published_then_officially_rejected_predecessor_v11",
                 "successor_v12_static_bundle", "physical_mode_policy",
             ...
             self.base_objects[TRANSITION].get("schema") ==
                 "cm2.round306c79g.true-global-no-producer-consumer."
                 "v11-to-v12-static-launch-transition.v1" and
             ...
             "transition exact top-level v11-to-v12 closed shape")
        need(set(self.base_objects[CONTRACT]) == {
                 ...
                 "v12_bundle", "object_sha256",
             } and
             self.base_objects[CONTRACT].get("schema") ==
                 "cm2.round306c79g.true-global-no-producer-consumer.v12.contract" and
             ...
             self.base_objects[TRANSITION].get("status") ==
                 "STATIC_BYTES_CLOSED_V11_TO_V12__PHYSICAL_FREEZE_PENDING__RUNTIME_NOT_AUTHORIZED" and
        ...
        # Current exact8 owns the sole v11-rejection fd.  Each frozen exact10
        # owns the preceding rejection fd; v3 keeps its separate singleton.
        self.predecessor_v11 = HeldV11PredecessorExact10(
            coordination, self.by_path[V11_OFFICIAL_REJECTION])
        self.predecessor_v10 = HeldV10PredecessorExact10(
            coordination,
        ...
        identity_groups = [
            current_identities, predecessor_v11_identities,
            predecessor_v10_identities,
        ...
        need(len(all_history_identities) == 98 and
             [len(group) for group in identity_groups] ==
                 [10, 10, 10, 10, 10, 10, 10, 10, 10, 7, 1] and
             ...
             "current v12 exact10, v11/v10/v9/v8/v7/v6/v5/v3 exact10, v3 "
             "rejection and v4 draft7 are 98 unique live files on one mount")
    ...
    def terminal_replay(self) -> None:
        ...
        self.predecessor_v11.terminal_replay(
            self.by_path[V11_OFFICIAL_REJECTION])
...
def child_environment(bundle: HeldBundle, child_exec: HeldSealedChildExec,
    ...
    need(bundle.predecessor_v11 is not None,
         "v11 inherited held-fd evidence available before child spawn")
    (v10_producer_fd, v9_launcher_fd, v11_producer_fd,
     v11_launcher_fd, v11_rejection_fd) = (
        bundle.predecessor_v11.inherited_evidence_fds())
    return {
        "LC_ALL": "C",
        "TZ": "UTC",
...
def child_pass_fds(bundle: HeldBundle, child_exec: HeldSealedChildExec,
    ...
    need(bundle.predecessor_v11 is not None,
         "v11 inherited held-fd evidence available for pass_fds")
    result = (
        child_exec.fd, source.fd, coordination.fd, bundle.bootstrap.root_fd,
        *bundle.predecessor_v11.inherited_evidence_fds(),
    )
    need(len(result) == 9 and len(set(result)) == 9,
         "child exact nine inherited descriptors are pairwise distinct")
    return result
...
def cold_root(bundle: HeldBundle, coordination: HeldCoordinationParent,
    ...
    proof = {
        ...
        "current_exact8_first_member_is_v11_official_rejection":
            EXACT8[0] == V11_OFFICIAL_REJECTION,
        ...
        "all_current_and_historical_98_identities_globally_unique_on_one_statx_mount": True,
        "current_v12_and_all_predecessor_unique_file_identity_count": 98,
        "current_v12_and_all_predecessor_same_statx_mount": True,
        **bundle.chronology,
        ...
        "reject_command_exempts_only_fresh_empty_v12_rejection_namespace": True,
...
def _validate_live_ack(
    ...
    expected_keys = {
        ...
        "static_v12_exact10_with_v11_exact10_and_official_rejection_v10_exact10_and_official_rejection_v9_exact10_and_official_rejection_v8_exact10_and_official_rejection_v7_exact10_and_official_rejection_v6_exact10_and_official_rejection_v5_exact10_and_official_rejection_v4_supersession_v3_exact10_and_official_rejection_terminal_replay_completed_by_launcher_before_request",
    ...
    need(set(ack) == expected_keys and ack.get("schema") == LIVE_ACK_SCHEMA and
         ...
         ack.get("static_v12_exact10_with_v11_exact10_and_official_rejection_v10_exact10_and_official_rejection_v9_exact10_and_official_rejection_v8_exact10_and_official_rejection_v7_exact10_and_official_rejection_v6_exact10_and_official_rejection_v5_exact10_and_official_rejection_v4_supersession_v3_exact10_and_official_rejection_terminal_replay_completed_by_launcher_before_request") is True and
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "reject":
            ...
            return 0
        # Every positive-capable command holds the exact-empty v12 rejection
