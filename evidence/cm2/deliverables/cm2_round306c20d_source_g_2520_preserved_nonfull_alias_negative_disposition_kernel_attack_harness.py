...
ROOT=Path(__file__).resolve().parent;P="cm2_round306c20d_source_g_2520_preserved_nonfull_alias_negative_disposition_kernel";R=P+"_result.json";V=ROOT/(P+"_independent_verifier.py")
...
def main():
 q=Path(sys.argv[1]).resolve();base=json.loads((q/R).read_bytes());att=[("credit",lambda d:d.__setitem__("typed_nonfull_representation_disposition_credit",2519)),("closure",lambda d:d.__setitem__("preserved_representation_semantic_closure_count",165743)),("debt",lambda d:d.__setitem__("remaining_preserved_alias_semantic_debt",1)),("rows",lambda d:d["ledger"].__setitem__("row_count",2519)),("B1A",lambda d:d["strict_nonpromotion"].__setitem__("B1A",1)),("B2",lambda d:d["strict_nonpromotion"].__setitem__("B2",1)),("max",lambda d:d["strict_nonpromotion"].__setitem__("maximality",1)),("CM2",lambda d:d["strict_nonpromotion"].__setitem__("CM2","GO"))];rej=[]
 for name,mut in att:
  with tempfile.TemporaryDirectory(prefix="c20d-attack-")as raw:
   d=Path(raw)
   for f in q.iterdir():
