...
[
0=igcap_i N_i,qquad Q_i=M/N_i,qquad operatorname{Ass}Q_i={mathfrak p_i},
]
together with associated-element injections
[
A/mathfrak p_ihookrightarrow M
]
and the diagonal injection
[
Mhookrightarrowigoplus_iQ_i.
]
...
[
operatorname{Ass}M_{ar s}
subset
igcup_ioperatorname{Ass}Q_{i,ar s}
subset
{mathfrak p_{i,ar s}},
]
...
[
T=(0:_Mmathfrak p^n)=H^0_{mathfrak p}(M),qquad Q=M/T.
]
...
[
Tsupsetmathfrak pTsupsetcdotssupsetmathfrak p^nT=0
]
...
[
mu_{i,j}:L_iotimes_A L_jlongrightarrow L_{i+j},
qquad i+jle N,
]
...
I regard the corank-four (d^4in J) mechanism as closed.

## 1.5 The global exponent-five theorem is a real advance

Revision 131 uses the full-matrix identity to show that, for every (Rin G_4^circ),
[
d^4in J
]
...
I checked the quantifiers in the inherited corank-three/corank-four statements. The manuscript is not silently promoting a generic calculation to an everywhere statement on (G_4^circ). The corank-three exclusion is formulated pointwise, and the corank-four argument uses the nonzero Jacobian covariant for every (Rin G_4^circ).
...
pi^{-1}(T)
=
Y_Rcapmathbf P((V/operatorname{im}T)^*)
]
...
This directly addresses the v130 request for a concrete (2ightsquigarrow3ightsquigarrow4) geometric specialization. I no longer list “no cross-corank family” as an open objection.
...
- and preservation of the exact relationship between packet generic points and (operatorname{Ass}(M_{ar s})).
...
[
Z_{q,
u}subsetoperatorname{Spec}A_{S_alpha}
]
...
For
[
gamma=operatorname{Sym}^rho,qquad
ho:V	woheadrightarrow W,
]
...
[
I(operatorname{Sym}^rphi)=I(phi)^a
]
