...
originals and five statement bodies and retained periodic module. It does not count semantic theorems.
"""
...
def main():
    ...
    archives=[]
    for n in changed:
        p='history/v64-review-baseline/'+n
    ...
    require(old['article/10a_periodic_itinerary_relative_v64.tex']==new['article/10a_periodic_itinerary_relative_v64.tex'], 'Periodic forward module changed')
    res={'complete_periodic_forward_module_identical':True,'complete_statistical_module_identical':True,'position_pilot_identical':True,'baseline_source':orr['source_commit'],'current_source':nrr['source_commit'],'baseline_artifact_sha256':oh,'current_artifact_sha256':nh,'baseline_files_verified':len(old),'current_files_verified':len(new),'unchanged_inherited_files':len(old)-len(changed),'changed_inherited_files':sorted(changed),'archived_originals_verified':archives,'new_paths':sorted(set(new)-set(old)),'baseline_active_union':len(ou),'current_active_union':len(nu),'new_active_paths':sorted(nu-ou),'five_finite_experiment_statement_bodies_identical':True,'scope':'Byte/mode and syntactic statement-body comparison; not a semantic theorem census.'}
