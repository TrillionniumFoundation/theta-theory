...
that is topologically equivalent to the product topology.  Logarithmic
zero-force washouts make the persistent state a uniformly summable
log-likelihood perturbation.  A supremum-norm compact-class strong law then
gives the uniform likelihood limit
\[
 \frac1n\log\frac{L_n(\beta)}{L_n(\beta_0)}
 \longrightarrow-\frac{a^2}{2\sigma^2}D(\beta,\beta_0)
\]
uniformly over the complete coefficient product.  Consequently the exact
posterior satisfies a full good large-deviation principle, with matching
open-set lower and closed-set upper bounds.  Uniform pairwise moduli
\(\kappa_J\) and \(\Omega_J\) convert response information into deterministic
stability and exponential contraction for every fixed finite coefficient
block.  Strong consistency is an immediate strict corollary.

The same response geometry also supports a second experiment with no
stagewise washout.  Before each fresh sign is drawn, the controller may choose
an arbitrary bounded adaptive baseline.  The difference between two models
then decomposes into a predictable intercept plus a signed current response.
Averaging only over the fresh sign removes the cross term and leaves the
intercept square nonnegative.  Compactness of the impulse responses in
\(L^1(0,\infty)\) supplies policy-uniform finite nets.  This gives the same
guaranteed response-information posterior upper exponent, multiplied by the
chosen exploration fraction, in total physical time \(\Theta(n)\), together
with washout-free finite-sample honest response and coefficient cylinders.
...
modulus explicit, while Theorem~\ref{thm:effective-response-jet-audit}
materializes the moment--Gram algorithm and its condition bound.
Theorems~\ref{thm:growing-depth-recovery} and
\ref{thm:explicit-shrinking-block-rate} give increasing-block recovery with
an explicit shrinking radius, and
Corollary~\ref{cor:weighted-operator-recovery} upgrades the conclusion to a
weighted operator norm.  Corollary~\ref{thm:jacobi-consistency} gives strong
consistency.
\item Theorem~\ref{thm:linear-time-adaptive-jacobi} proves the no-washout
linear-time adaptive posterior exponent, and
Theorem~\ref{thm:linear-time-honest-cylinders} gives its finite-sample honest
coefficient cylinders.
\end{enumerate}

\subsection{Effective stability, growing depth, and physical time}

The response-product homeomorphism is supplemented by a constructive modulus.
An exact response--moment generating identity, finite Gram linear systems,
explicit Vandermonde inversion, moment recursion, and Hankel determinant lower
bounds give
\[
 D(\beta,\gamma)\ge
 \exp\{-A_0(J+1)^4\}\,d_J(\beta,\gamma)^{A_1(J+1)}.
\]
This closes the inverse problem with visible depth dependence.  It yields an
explicit sequence
\[
 \delta_n=\exp\left\{-\frac{\eta\log n}{80(J_n+1)}\right\}
\]
for every \((J_n+1)^5=o(\log n)\), posterior recovery of that growing block,
contraction in exponentially weighted operator norm, and honest simultaneous
confidence cylinders.  A fresh exploration floor permits arbitrary
reward-seeking or stabilizing adaptive diagnostic choices off the exploration
subsequence.

For the exact two-sided LDP, logarithmic washout is summable; total physical
time is \(\kappa_wn\log n+O(n)\), and the posterior LDP has elapsed-time speed
\(t/(\kappa_wW(t/\kappa_w))\).  The no-washout theorem supplies a distinct,
stronger clock statement: predictable-intercept sign orthogonality gives a
uniform closed-set exponent under arbitrary bounded adaptive baselines with
\(\tau_gn\le T_n^{\rm phys}\le\tau_+n\).  The former theorem identifies the
complete rate function; the latter proves the guaranteed information floor in
strictly linear physical time.
...
total-variation limit, a full-sequence Jacobi posterior LDP, or a linear-time
adaptive information guarantee.
...
uniform noisy likelihood limit, the exact integrated transient correction,
the resulting full posterior large-deviation principle on the complete
compact coefficient product, the effective finite-depth inverse algorithm,
and the no-washout linear-time adaptive extension.
