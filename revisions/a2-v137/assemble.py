#!/usr/bin/env python3
"""Assemble only v137 from immutable reviewed source and readable overlays."""
...
REVIEWED='731729b9e391b507d27994f6db8d71d61cfca577'
R1=None
SOURCE=Path('papers/A2-v17-boundary-information-coarsening/article/v136');DEST=SOURCE.with_name('v137')
REVIEW='reviews/a2-v136-independent-harsh-top4-2026-09-23/REFEREE_REPORT.md'
REVIEW1=None
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='a2-v137-') as td:
        td=Path(td)
        ...
        for src in sorted((HERE/'overlay').rglob('*')):
            if src.is_file():
                ...
                if d.is_file() and d.read_bytes()!=src.read_bytes():
                    archived=target/'history/v136-modified-source'/rel
        ...
        for n,h in oldtex.items():
            if not(target/n).exists() or sha(target/n)!=h:
                d=target/'history/v136-modified-source'/n;d.parent.mkdir(parents=True,exist_ok=True);shutil.copyfile(base/n,d)
        reviewhash={}
        if not args.base_dir:
            for ref,path,name in [(BASE,REVIEW,'CONTROLLING')]:
                data=subprocess.check_output(['git','show',ref+':'+path],cwd=ROOT)
                d=target/'review_inputs'/('V136_'+name+'_REFEREE_REPORT.md');d.parent.mkdir(parents=True,exist_ok=True);d.write_bytes(data)
        ...
        print(json.dumps({'revision':137,'assembly_commit':commit,'retained_labels':len(set(oldlabels)),'current_labels':len(newlabels),'unchanged_tex':len(unchanged)},indent=2))
