...
PAPERS=['A1-exact-benchmarks','A2-sinai-homological-pressure','A3-full-empirical-path-ldp','A4-history-memory-universal-pressure','B1-microcanonical-preparation','B2-collision-clusters-dynamic-ldp','B3-hamilton-boltzmann-cotangents','B4-nonlinear-kinetic-semigroups','C1-information-risk-sensitive-saddles','C2-cotangent-rigidity-tangent-representations','D1-deterministic-theta-contractions']
LOGDIR=ROOT/'ROUND12_BUILD_LOGS';LOGDIR.mkdir(exist_ok=True)
...
def build(folder):
 ...
 proc=subprocess.run(['latexmk','-pdf','-interaction=nonstopmode','-halt-on-error','main.tex'],cwd=p,stdout=subprocess.PIPE,stderr=subprocess.STDOUT,text=True,timeout=240)
 ...
 return folder,{'returncode':proc.returncode,'status':'PASS' if ok else 'FAIL','pdf_bytes':pdf.stat().st_size if pdf.is_file() else 0,'pdf_pages':pages,'pdf_sha256':sha(pdf) if pdf.is_file() and pdf.stat().st_size else None,'undefined_reference_lines':undefined,'transcript':str(transcript.relative_to(ROOT)),'tail':proc.stdout[-4000:] if not ok else ''}
results={}
with ThreadPoolExecutor(max_workers=4) as ex:
 futs={ex.submit(build,f):f for f in PAPERS}
 for fut in as_completed(futs):
  folder,entry=fut.result();results[folder]=entry;print('ROUND12_BUILD',folder,entry['status'])
failed=[f for f in PAPERS if results[f]['status']!='PASS']
summary={'schema':'theta-theory-round12-build-summary-v1','papers':{f:results[f] for f in PAPERS},'failed':failed,'paper_count':11,'passed':11-len(failed),'total_pdf_pages':sum(results[f]['pdf_pages'] for f in PAPERS),'status':'PASS' if not failed else 'FAIL'}
for e in summary['papers'].values():e.pop('tail',None)
(ROOT/'ROUND12_BUILD_SUMMARY.json').write_text(json.dumps(summary,indent=2,sort_keys=True)+'\n')
if failed:
 for f in failed:print(f'ROUND12_BUILD_FAIL {f}\n{results[f]["tail"]}',file=sys.stderr)
 raise SystemExit(1)
print(f"ROUND12_BUILD_PASS 11/11 pages={summary['total_pdf_pages']}")
