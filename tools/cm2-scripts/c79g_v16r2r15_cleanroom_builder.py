#!/usr/bin/env python3
"""Append-only r15 static clean-room builder.

This builder consumes only the rejected r14 bytes and creates a new r15
...
PJ = {
    ...
    "transition": OUT / f"{BASE}_v16r2r13_to_{PREV}_static_launch_transition_receipt_v1.json",
...
J = {
    ...
    "transition": OUT / f"{BASE}_v16r2r14_to_{TAG}_static_launch_transition_receipt_v1.json",
...
def rejection() -> tuple[dict[str, Any], bytes]:
    ...
    value = close({
        ...
        "rejection_reason": (
            "R14_RUNTIME_ROOT_PROTOCOL_DIVERGENCE_AND_SCHEMA_ROOT_"
            "STAGING_EXTENSION_LEAK"),
        "independent_reviewer_failed_checks": [
            "generic_root_contract_transition_audit_protocol",
            "three_source_transition_validator_consensus",
            "schema_root_runtime_allowed_keyset",
            "v14_trust_receipt_keyset",
        ],
...
def normalize_transition_checks(text: str, role: str) -> str:
    if role == "producer":
        # The indentation differs between the producer's held-trust block and
        # the consumer block; patch the literal component rather than relying
        # on a column count.  This value occurs only in the live final check.
        text = replace_once(
            text, '"v12-to-v16r2-static-launch-transition.v1"',
            '"v16-to-v16r2-static-launch-transition.v1"',
        ...
        # The first occurrence is a historical v13 witness.  Only the final
        # base-object validator is live; patch its last occurrence.
        if text.count(old_kind) != 1:
            raise RuntimeError("launcher final transition kind occurrence")
        text = text.replace(
            old_kind,
            '"APPEND_ONLY_PUBLISHED_V14_RUNTIME_FAIL_CLOSED_REJECTION_TO_"\n'
            '                 "ZERO_CREDIT_V16R2_CLEAN_ROOM_SUCCESSOR"', 1)
    return text
...
def source_bytes(raw: bytes, role: str, anchor_value: dict[str, Any],
    ...
    old_anchor = SA.name
    old_edge = f"{BASE}_v16r2r13_to_{PREV}_static_launch_transition_receipt_v1.json"
    anchor_marker, edge_marker = "__R15_ANCHOR__", "__R14_TO_R15_EDGE__"
    text = text.replace(old_anchor, anchor_marker)
    text = text.replace(old_edge, edge_marker)
    text = text.replace("V16R2R14", "V16R2R15")
    ...
    if "v16r2r14_to_v16r2r14" in text or "v16r2r15_to_v16r2r15" in text:
...
def retag(value: Any) -> Any:
    ...
    if not isinstance(value, str):
        return value
    old_edge = f"{BASE}_v16r2r13_to_{PREV}_static_launch_transition_receipt_v1.json"
    return (value.replace(old_edge, "__R14_TO_R15_EDGE__")
            .replace("V16R2R14", "V16R2R15")
            .replace(PREV, TAG)
            .replace("__R14_TO_R15_EDGE__", J["transition"].relative_to(ROOT).as_posix()))
...
def patch_bundle(bundle: dict[str, Any], source_hashes: dict[str, str],
    ...
    bundle.update({
        ...
        "D02_unlock": False,
        "runtime_authorized": False,
        "pin_state": "R15_SOURCE_AND_JSON_PINS_UNINSTALLED__STATIC_ZERO_CREDIT",
    ...
    trust.update({
        "binding_direction": "R15_SOURCE_READS_POST_SOURCE_RECEIPTS__NO_HASH_CYCLE",
...
def build_quartet(source_hashes: dict[str, str]) -> dict[str, bytes]:
    ...
    # its root.  Historical extension bytes remain immutable in r14 and are
    # represented by the r15 rejection evidence, never by this live schema.
    ...
    for key in list(schema):
        if key.startswith("x-cm2-"):
            schema.pop(key, None)
    schema["$comment"] = (
        "CM2_R15_LIVE_SUCCESSOR__GENERIC_V16R2_ROOT_PROTOCOL__"
    ...
    values["contract"].update({
        ...
        "purpose": (
            "Append-only r15 semantic bundle with generic v16r2 runtime root "
...
def main() -> int:
    try:
        for path in (*PS.values(), *PJ.values(), SA):
            if not path.is_file():
                raise RuntimeError(f"missing r14 input: {path}")
        ...
        generated_sources = {
            role: source_bytes(stable(path), role, anc, anc_raw)
            for role, path in PS.items()
        }
        source_hashes = {role: sha(raw) for role, raw in generated_sources.items()}
        source_actions = {
            role: install(S[role], raw, 0o664)
            for role, raw in generated_sources.items()
        }
        generated_json = build_quartet(source_hashes)
        json_actions = {
            name: install(J[name], raw, 0o664)
            for name, raw in generated_json.items()
        }
        ...
        if manifest.exists() or outer.exists():
            raise RuntimeError("manifest/outer appeared during static build")
        if any(TAG in str(path) for path in ROOT.rglob("*.pyc")):
            raise RuntimeError("r15 pyc appeared")
        result = {
