#!/usr/bin/env python3
"""Clean-room r32 static successor with parameterized retagging.

The predecessor r31 namespace is sealed as a zero-credit namespace/tag-census
rejection.  This builder creates the r31 -> r32 rejection supersession and
active anchor with O_EXCL, then derives a fresh quartet from immutable r23
...
PREV = os.environ.get("CM2_PREDECESSOR_SUFFIX", "v16r2r31")
TAG = os.environ.get("CM2_SUCCESSOR_SUFFIX", "v16r2r32")
...
def load_generic() -> ModuleType:
    ...
    module = ModuleType("_r32_generic_builder")
...
def source_census(text: str, role: str) -> None:
    ...
    expected_ns = [f"{TAG}_semantic_source"] * (2 if role == "launcher" else 1)
    expected_nst = [f"{TAG}-semantic-regeneration"] * (2 if role == "launcher" else 1)
...
def definition_smoke(raw: bytes, role: str, anchor_file: str,
    ...
    ns: dict[str, Any] = {"__name__": f"_r32_static_{role}",
                          "__file__": str(b.SRC_OUT[role]),
                          "__package__": None}
    text = raw.decode("utf-8")
    exec(compile(text, str(b.SRC_OUT[role]), "exec"), ns, ns)
    root = ROOT if role != "launcher" else Path("/tmp/cm2-r32-path-crosscheck")
    if role == "launcher":
...
def path_crosscheck(generated: dict[str, bytes]) -> dict[str, Any]:
    ...
    anchor = json.loads((OUT / f"{BASE}_{TAG}_active_predecessor_supersession_receipt_v1.json").read_text())
    af = anchor["predecessor_supersession_file_sha256"]
    ao = anchor["predecessor_supersession_object_sha256"]
    ...
    for role in ("consumer", "launcher"):
        ns: dict[str, Any] = {"__name__": f"_r32_v14_{role}",
                              "__file__": str(b.SRC_OUT[role]), "__package__": None}
        text = generated[role].decode("utf-8")
        exec(compile(text, str(b.SRC_OUT[role]), "exec"), ns, ns)
        if role == "launcher":
            ns["configure_workspace_paths"](Path("/tmp/cm2-r32-path-crosscheck"))
...
def predecessor_rejection() -> dict[str, str]:
    ...
    value = b.close({
        ...
        "detail": {"candidate_install": False, "runtime_protocol_executed": False,
                    "required_successor_fix": "parameterized dynamic r32 retag"},
...
def main() -> int:
    ...
    if any(TAG in path for path in before_pyc):
        print(json.dumps({"status": "FAIL_CLOSED_R32_PYC_PREEXISTS",
    ...
    try:
        ...
        if any(not path.is_file() for path in inputs):
            raise RuntimeError("immutable r23/r31 input missing")
        if any(path.exists() for path in targets):
            raise RuntimeError("r32 target exists")
