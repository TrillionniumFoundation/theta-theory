#!/usr/bin/env python3
"""r43 independent checker/terminal replay evidence.

This wrapper runs the two independent static checkers plus the semantic/path
audits twice (hash seeds 1 and 99991), and verifies the r43 exact8 bytes are
...
CHECKERS = {
    ...
    "residue": (ROOT / "scripts/c79g_v16r2r42_stale_residue_scanner.py",
                "e5fdf024066984cbe74c8b988446b4c43204cb6a1cea7a9f67c511ee0bf9545b"),
}
...
def run_checker(name: str, seed: str) -> tuple[dict[str, Any], bytes]:
    ...
    env.update({"PYTHONDONTWRITEBYTECODE": "1", "PYTHONNOUSERSITE": "1",
                ...
                "CM2_PREDECESSOR_SUFFIX": PREV, "CM2_STALE_SCAN_TAG": TAG,
                "CM2_STALE_SCAN_PREV": PREV})
    proc = subprocess.run(["/usr/bin/python3", "-I", "-B", str(path)],
                          cwd=str(ROOT), env=env, text=True,
                          capture_output=True, check=False)
    if proc.returncode != 0:
        raise RuntimeError(f"{name} rc={proc.returncode}:{proc.stderr[-600:]}")
    value = json.loads(proc.stdout)
    if not isinstance(value, dict):
        raise RuntimeError(f"{name} report object required")
    if name == "residue":
        if (value.get("status") !=
                "PASS_V16R2R43_NO_STALE_ACTIVE_RESIDUE_READ_ONLY" or
                value.get("failed_check_count") != 0 or
                value.get("read_only") is not True or
                value.get("target_tag") != TAG or
                value.get("predecessor_tag") != PREV):
            raise RuntimeError(f"residue status:{value.get('status')}")
    else:
        if value.get("failed_check_count") != 0 or value.get("read_only") is not True:
            raise RuntimeError(f"{name} failed/read-write report")
        expected_prefix = {"A": "PASS_DUAL_STATIC_CANDIDATE_34_OF_34",
                           "B": "PASS_INDEPENDENT_CHECKER_B"}[name]
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if not (args.seal or args.install):
            print(json.dumps({"status": "PREFLIGHT_PASS_R43_TERMINAL_REPLAY__ZERO_CREDIT",
        ...
        print(json.dumps({"status": "FAIL_CLOSED_R43_TERMINAL_REPLAY_INTERRUPTED",
                          "installation_performed": False,
                          "formal_global_closure_credit": 0,
                          "D02_unlock": False, "runtime_authorized": False},
                         sort_keys=True))
        ...
        print(json.dumps({"status": "FAIL_CLOSED_R43_TERMINAL_REPLAY",
