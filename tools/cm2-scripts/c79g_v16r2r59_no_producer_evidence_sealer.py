#!/usr/bin/env python3
"""r59 no-producer dual-seed evidence sealer.

The reconstruction and the r59 role-aware reviewer run in isolated ``-I -B
...
REVIEWER = ROOT / "scripts/c79g_v16r2r59_role_aware_reviewer.py"
...
def assert_inputs() -> dict[str, str]:
    ...
    if (helper.get("status") !=
            "PASS_R59_DUAL_INDEPENDENT_VERSION_NEUTRAL_HELPER_REVIEW__ZERO_CREDIT" or
    ...
    tagged = [p for p in ROOT.rglob("*.pyc") if TAG in str(p)]
    if tagged:
...
def assert_reviewer(value: dict[str, Any]) -> None:
    if (not str(value.get("status", "")).startswith(
        ...
        raise RuntimeError("r59 reviewer gate")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if not (args.seal or args.install):
            print(json.dumps({"status": "PREFLIGHT_PASS_R59_NO_PRODUCER__ZERO_CREDIT",
        ...
        print(json.dumps({"status": "FAIL_CLOSED_R59_NO_PRODUCER_EVIDENCE_INTERRUPTED",
                          "installation_performed": False,
                          "formal_global_closure_credit": 0,
                          "D02_unlock": False, "runtime_authorized": False},
                         sort_keys=True))
        ...
        print(json.dumps({"status": "FAIL_CLOSED_R59_NO_PRODUCER_EVIDENCE",
