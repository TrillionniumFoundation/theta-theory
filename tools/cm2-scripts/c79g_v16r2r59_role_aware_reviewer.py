#!/usr/bin/env python3
"""Read-only r59 role-aware static reviewer.

This is deliberately independent of the historical v15 checker census.  It
checks the active r59 bytes, then compares the launcher registry helper after
...
ANCHOR_FILE = "26d919c2760eb8ce2c0cfe2588f91dc8061c2ad7bbab1575ae2736ca657a43fa"
ANCHOR_OBJECT = "15131c6ac0f7d09574a944fc99eb588ec79aa6d31afe70b2373936269329b716"
...
def main() -> int:
    ...
    add(rows, "instance_top_level_shapes", len(contract) == 30 and len(transition) == 31 and len(audit) == 30)
    credit_values = []
    ...
    add(rows, "active_namespace_consensus", len(succ) >= 3 and set(succ) == {f"{TAG}_semantic_source"} and all("v16r2r59" in x for x in pred))
    ...
    runtime_hits = [str(p) for p in ROOT.rglob("*.pyc") if TAG in p.name]
    add(rows, "no_r59_pyc_manifest_outer_runtime", not MANIFEST.exists() and not OUTER.exists() and not runtime_hits)
