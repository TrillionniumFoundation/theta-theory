#!/usr/bin/env python3
"""r61 no-producer dual-seed evidence sealer.

The reconstruction and the r61 role-aware reviewer run in isolated ``-I -B
...
REVIEWER = ROOT / "scripts/c79g_v16r2r61_role_aware_reviewer.py"
...
def assert_inputs() -> dict[str, str]:
    ...
    if (helper.get("status") !=
            "PASS_R61_DUAL_INDEPENDENT_VERSION_NEUTRAL_HELPER_REVIEW__ZERO_CREDIT" or
    ...
    if tagged:
        raise RuntimeError("r61 unexpected pyc present")
    return hashes
...
def assert_reviewer(value: dict[str, Any]) -> None:
    if (not str(value.get("status", "")).startswith(
        ...
        raise RuntimeError("r61 reviewer gate")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if not (args.seal or args.install):
            print(json.dumps({"status": "PREFLIGHT_PASS_R61_NO_PRODUCER__ZERO_CREDIT",
        ...
        print(json.dumps({"status": "FAIL_CLOSED_R61_NO_PRODUCER_EVIDENCE_INTERRUPTED",
                          "installation_performed": False,
                          "formal_global_closure_credit": 0,
                          "D02_unlock": False, "runtime_authorized": False},
                         sort_keys=True))
        ...
        print(json.dumps({"status": "FAIL_CLOSED_R61_NO_PRODUCER_EVIDENCE",
