#!/usr/bin/env python3
"""r56 clean-room successor builder.
...
REJECTION = OUT / f"{BASE}_v16r2r55_static_bundle_rejection_receipt_v1.json"
...
def load_constructor() -> dict[str, Any]:
    ...
    ns: dict[str, Any] = {"__name__": "_r56_constructor",
...
def replace_hex_top(text: str, name: str, value: str) -> str:
    ...
    if count != 1:
        raise RuntimeError(f"r56 top-level pin {name}:{count}")
    return out
...
def source_patch(raw: bytes, role: str, paths: dict[str, str],
    ...
    text = text.replace(old_edge, "__R56_EDGE__")
    text = text.replace("v16r2r54", TAG).replace("V16R2R54", "V16R2R56")
    text = text.replace("__R56_EDGE__", f"{BASE}_{PREV}_to_{TAG}_")
    if role != "launcher":
        ...
        if base7 is None:
            raise RuntimeError("r56 launcher BASE7 missing")
        tree = ast.parse(text, "r56_launcher", mode="exec")
        ...
        if pin_counts != {"file": 2, "object": 2}:
            raise RuntimeError(f"r56 launcher active-pin assignment census:{pin_counts}")
        ...
        assigns = ([n for n in funcs[0].body if isinstance(n, ast.Assign) and
                   ...
                   if len(funcs) == 1 else [])
        if len(assigns) != 1:
            raise RuntimeError("r56 launcher BASE7 assignment")
        pins = dict(base7)
    ...
    if edge not in text:
        raise RuntimeError(f"{role}: r56 edge absent")
    return text.encode()


def configure(ns: dict[str, Any]) -> Any:
    """Configure the current generic constructor for r54 -> r56."""
    ns.update({
...
def make_chain(ns: dict[str, Any]) -> dict[str, Any]:
    rejection_value = close({
        "schema": "cm2.c79g.v16r2r55.tooling-pyc-rejection.v1",
        "status": "PERMANENT_FAIL_CLOSED_R55_TOOLING_PYC_BEFORE_FIRST_EXECUTION__ZERO_CREDIT",
        "failed_namespace": "v16r2r55", "predecessor_namespace": PREV,
        "rejection_reason": "BUILDER_PYC_PRESENT_BEFORE_FIRST_EXECUTION",
        "failure_vector": {
            "builder_path": "scripts/c79g_v16r2r55_candidate_builder.py",
            "builder_file_sha256": "3b5122c27435d59b635cf0bb769566a239b43664507b11e8e488a1ff0d2e2065",
            "builder_pyc_path": "scripts/__pycache__/c79g_v16r2r55_candidate_builder.cpython-312.pyc",
            "builder_pyc_sha256": "4e0f9c623824e2fcff7617147fca1c8e367497330529ece13ebd0ed22a423599",
            "builder_pyc_size": 33541,
            "candidate_install": False,
            "r55_namespace_anchor_created": False,
            "target_namespace_not_installed": True,
            "candidate_bytes_remain_immutable": True,
        },
        "candidate_install": False, "runtime_protocol_executed": False,
...
def validate_generated(generated: dict[str, bytes], meta: dict[str, Any],
    ...
    launcher_tree = ast.parse(generated["launcher"].decode(), "r56_launcher", mode="exec")
        ...
        for target in targets:
            ...
            if target.id == "ACTIVE_PREDECESSOR_SUPERSESSION_FILE_PIN":
                if not isinstance(node.value, ast.Constant):
                    raise RuntimeError("r56 active file pin is not literal")
                file_values.append(node.value.value)
            elif target.id == "ACTIVE_PREDECESSOR_SUPERSESSION_OBJECT_PIN":
                if not isinstance(node.value, ast.Constant):
                    raise RuntimeError("r56 active object pin is not literal")
                object_values.append(node.value.value)
    if len(file_values) != 2 or set(file_values) != {chain["anchor_file_sha256"]}:
        raise RuntimeError(f"r56 launcher file-pin split:{file_values}")
    if len(object_values) != 2 or set(object_values) != {chain["anchor_object_sha256"]}:
        raise RuntimeError(f"r56 launcher object-pin split:{object_values}")
    ...
    if any("r50" in str(schema.get(k, "")).lower() or "r51" in str(schema.get(k, "")).lower() or "r52" in str(schema.get(k, "")).lower() or "r53" in str(schema.get(k, "")).lower() for k in ("title", "description")):
        raise RuntimeError("r56 schema current semantic label residue")
...
def build_in_memory() -> tuple[dict[str, Any], dict[str, bytes], dict[str, Any], dict[str, Any]]:
    ...
    if occupied:
        raise RuntimeError("r56 target already exists:" + ",".join(occupied))
    ns = load_constructor()
    ...
    if pyc_inventory() != before:
        raise RuntimeError("r56 pyc inventory changed during build")
...
def install() -> dict[str, Any]:
    ...
    if pyc_inventory() != before_install:
        raise RuntimeError("r56 pyc inventory changed during installation")
    return {"schema": f"cm2.c79g.{TAG}.candidate-builder.v1",
            "status": "R56_STATIC_CANDIDATE_INSTALLED__ZERO_CREDIT__RUNTIME_NOT_AUTHORIZED",
...
def main() -> int:
    try:
        ...
        print(json.dumps({"schema": f"cm2.c79g.{TAG}.candidate-builder.failure.v1",
                          "status": "FAIL_CLOSED_R56_STATIC_INSTALL",
