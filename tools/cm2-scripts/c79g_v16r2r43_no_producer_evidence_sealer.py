#!/usr/bin/env python3
"""r43 no-producer dual-seed evidence (read-only by default).
...
historical pre-r43 historical receipt is ever written or rebound by this program.
"""
...
def assert_namespace_and_chain() -> dict[str, str]:
    ...
    if any("v16r2r34" in str(p) or "v16r2r39" in str(p) for p in EXACT8):
        raise RuntimeError("historical pre-r43 historical path rebound")
    for path in EXACT8:
        raw = stable(path)
    ...
    if (anchor.get("successor_namespace") != f"{TAG}_semantic_source" or
        ...
        raise RuntimeError("active r43 anchor semantics")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if not (args.seal or args.install):
            print(json.dumps({"status": "PREFLIGHT_PASS_R43_NO_PRODUCER__ZERO_CREDIT",
        ...
        print(json.dumps({"status": "FAIL_CLOSED_R43_NO_PRODUCER_EVIDENCE_INTERRUPTED",
                          "installation_performed": False,
                          "formal_global_closure_credit": 0,
                          "D02_unlock": False, "runtime_authorized": False},
                         sort_keys=True))
        ...
        print(json.dumps({"status": "FAIL_CLOSED_R43_NO_PRODUCER_EVIDENCE",
