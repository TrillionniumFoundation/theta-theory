#!/usr/bin/env python3
"""Append-only r43 clean-room builder (static self-check by default).

The r42 candidate is rejected as a template-residue surface: its schema
description still says ``Append-only r9 schema``.  This entry point therefore
does not repair or re-use any r42 candidate bytes.  It loads the reviewed r40
constructor in memory, retags the immutable r39 source/JSON inputs to a fresh
r42 -> r43 edge, and explicitly normalizes the active schema metadata.  The
r42 active anchor and r42 supersession are the only predecessor-chain inputs.
...
path that can create the r42 rejection, r42->r43 chain, or r43 candidate
...
# predecessor chain for this attempt is r42, not the mutable r42 candidate
...
R42_ANCHOR = OUT / f"{BASE}_{PREV}_active_predecessor_supersession_receipt_v1.json"
R42_SUP = OUT / f"{BASE}_v16r2r41_to_v16r2r42_static_bundle_rejection_supersession_receipt_v1.json"
R42_TRANSITION = OUT / f"{BASE}_v16r2r41_to_v16r2r42_static_launch_transition_receipt_v1.json"
R42_SCHEMA = OUT / f"{BASE}_schema_v16r2r42.json"
R42_COLD_MANIFEST = OUT / f"{BASE}_cold_launch_manifest_v16r2r42.sha256"
R42_COLD_OUTER = OUT / f"{BASE}_cold_launch_outer_receipt_v16r2r42.json"
R42_SIDE_RECEIPTS = (
    OUT / f"{BASE}_v16r2r42_no_producer_dual_seed_evidence_receipt_v1.json",
    OUT / f"{BASE}_v16r2r42_mutation_attack_evidence_receipt_v1.json",
    OUT / f"{BASE}_v16r2r42_dual_checker_terminal_replay_receipt_v1.json",
)

R42_ANCHOR_SHA256 = "6950834216c142b34ee2f729305fe94a89adaaf28d28b28247e5c0db31ef79cb"
R42_ANCHOR_OBJECT_SHA256 = "cdb5bb0ae329d52e9e0c09bf477fda80a2caa641fee9f790aaaf5e5a6bbf83cc"
R42_SUP_SHA256 = "9c5f1ce1b7635c17afc230ce6c6544a0b3bbe8ce64635aa68c0feb597ee58324"
R42_SUP_OBJECT_SHA256 = "3405c46fa40c36856e31008b7cd9235ce828f980ef7cc80213a2c9375f1ed729"
R42_TRANSITION_SHA256 = "b7351660a29af1f0cc498d10aad726e3c86c0d4dbd250aa54cbda10ad55ee854"
R42_SCHEMA_SHA256 = "27e663e22ed14c88009f40c295156bc2a061fb7afff072633b9b939c409f2029"
R42_SCHEMA_OBJECT_SHA256 = "af1f5af6e829a8a0d35868cf54b5be44a0f24ea1b8e42ada9a7924e6fbfe36dd"
R42_SCHEMA_DESCRIPTION = (
    "Append-only v16r2r42 schema.  The active exact8 starts with the configured "
...
SCHEMA_DESCRIPTION = (
    "Append-only v16r2r43 schema.  The active exact8 starts with the "
    "configured recent predecessor supersession receipt; all persisted "
    "credit and runtime authority remain disabled."
)

# r42's closed-schema metadata retained an obsolete active-proof field name
...
def load_r40_constructor() -> dict[str, Any]:
    """Compile the reviewed r40 constructor in memory, retagged to r43."""
    ...
    ns: dict[str, Any] = {"__name__": "_c79g_r43_constructor",
...
def _schema_residue_fix(value: Any, ns: dict[str, Any]) -> Any:
    """Apply r43 metadata only to the active closed-schema root."""
    ...
    if "r9 schema" in str(value.get("description", "")).lower():
        raise RuntimeError("schema description residue survived r43 retag")
    return value


def _assert_r42_schema_residue() -> dict[str, Any]:
    """Pin the rejected r42 schema and retain the independent guard failure."""
    raw = stable(R42_SCHEMA)
    if sha(raw) != R42_SCHEMA_SHA256:
        raise RuntimeError("r42 schema file hash drift")
    value = json.loads(raw.decode("utf-8"))
    body = dict(value)
    body.pop("object_sha256", None)
    canonical = json.dumps(body, ensure_ascii=False, sort_keys=True,
                           separators=(",", ":"), allow_nan=False).encode()
    if sha(canonical) != R42_SCHEMA_OBJECT_SHA256:
        raise RuntimeError("r42 schema object hash drift")
    if value.get("description") != R42_SCHEMA_DESCRIPTION or \
       "r9 schema" in str(value.get("description", "")).lower():
        raise RuntimeError("r42 schema metadata drift")
    if value.get("$id") != f"cm2.round306c79g.true-global-no-producer-consumer.{PREV}.schema":
        raise RuntimeError("r42 schema id drift")
    return {"path": str(R42_SCHEMA.relative_to(ROOT)),
            "file_sha256": R42_SCHEMA_SHA256,
            "object_sha256": R42_SCHEMA_OBJECT_SHA256,
            "description": value["description"],
            "guard_failure_object_sha256": R42_FAILURE_OBJECT_SHA256,
            "guard_failure_report_sha256": R42_FAILURE_REPORT_SHA256,
            "rejection_reason": "R42_COLD_FREEZE_GUARD_PYC_FAIL_CLOSED"}


def _configure_r43(ns: dict[str, Any], b: Any) -> None:
    """Bind the generic constructor to immutable r39 inputs and r42 anchor."""
...
def _seal_r42_rejection(b: Any) -> dict[str, str]:
    ...
    if path.exists():
        ...
        if value.get("failed_namespace") != PREV or \
           ...
           "R42_COLD_FREEZE_GUARD_PYC_FAIL_CLOSED":
            raise RuntimeError("r42 rejection replay mismatch")
        return {"action": "replayed", "file_sha256": b.sha(raw),
                "object_sha256": value["object_sha256"]}
    anchor_raw = stable(R42_ANCHOR)
    ...
    value = b.close({
        ...
        "detail": {
            ...
            "anchor_path": str(R42_ANCHOR.relative_to(ROOT)),
            "anchor_file_sha256": sha(anchor_raw),
            "anchor_object_sha256": anchor.get("object_sha256"),
            "schema_path": str(R42_SCHEMA.relative_to(ROOT)),
            "schema_file_sha256": R42_SCHEMA_SHA256,
            "schema_object_sha256": R42_SCHEMA_OBJECT_SHA256,
            "schema_description": R42_SCHEMA_DESCRIPTION,
            "guard_failure_object_sha256": R42_FAILURE_OBJECT_SHA256,
            "guard_failure_report_sha256": R42_FAILURE_REPORT_SHA256,
            "required_successor_fix": "r43 clean-room no-pyc static build from immutable r39 bytes",
        },
        "append_only": True,
...
def static_self_check() -> dict[str, Any]:
    """Read-only r43 chain, AST, and in-memory schema-retag verification."""
    if R42_COLD_MANIFEST.exists() or R42_COLD_OUTER.exists() or \
       any(path.exists() for path in R42_SIDE_RECEIPTS):
        raise RuntimeError("r42 positive/evidence surface unexpectedly exists")
    if any("v16r2r42" in str(path) for path in
           (ROOT / ".cm2-runtime").rglob("*") if path.exists()):
        raise RuntimeError("r42 runtime surface unexpectedly exists")
    schema_witness = _assert_r42_schema_residue()
    anchor = _assert_frozen(R42_ANCHOR, R42_ANCHOR_SHA256,
                            R42_ANCHOR_OBJECT_SHA256)
    if anchor.get("successor_namespace") != f"{PREV}_semantic_source" or \
       anchor.get("predecessor_namespace") != "v16r2r41":
        raise RuntimeError("r42 active anchor semantics")
    sup = _assert_frozen(R42_SUP, R42_SUP_SHA256, R42_SUP_OBJECT_SHA256)
    if sup.get("predecessor_namespace") != "v16r2r41" or \
       sup.get("successor_namespace") != PREV:
        raise RuntimeError("r42 supersession semantics")
    if not R42_TRANSITION.is_file() or sha(stable(R42_TRANSITION)) != R42_TRANSITION_SHA256:
        raise RuntimeError("r40->r42 transition drift")
    ...
    for def_name in ACTIVE_PROOF_DEFS:
        ...
        if STALE_ACTIVE_PROOF_KEY in node.get("properties", {}) or \
           ...
           FRESH_ACTIVE_PROOF_KEY not in node.get("required", []):
            raise RuntimeError(f"r43 active proof key closure:{def_name}")
    bad = ns["schema_walk"](fixed)
    if bad:
        raise RuntimeError("r43 schema recursive node walk:" + ",".join(bad[:8]))
    ...
    if not str(contract_fixed.get("purpose", "")).startswith(
        ...
        raise RuntimeError("r43 contract purpose residue")
    ...
    if audit_fixed.get("schema_and_constructor_closure", {}).get(
        ...
        raise RuntimeError("r43 audit template-review residue")
    ...
    for role, rel in (
        ...
        if edge not in retagged or anchor_name not in retagged or old_edge in retagged:
            raise RuntimeError(f"{role}:r43 active edge/anchor retag")
    ...
    if any(p.exists() for p in target_files):
        raise RuntimeError("r43 candidate target already exists")
    if any(TAG in str(p) for p in ROOT.rglob("*.pyc")):
        raise RuntimeError("r43 tagged pyc preexists")
    ns["assert_r39_partial"]()
    return {
        "status": "PASS_R43_CLEAN_ROOM_AST_SCHEMA_ACTIVE_PATH_STATIC_ONLY",
        "template_inputs": "immutable_r39_source_json_bytes",
        "predecessor_anchor": str(R42_ANCHOR.relative_to(ROOT)),
        "predecessor_anchor_file_sha256": R42_ANCHOR_SHA256,
        "predecessor_anchor_object_sha256": R42_ANCHOR_OBJECT_SHA256,
...
def _install_r43() -> int:
    ns = load_r40_constructor()
    original_configure = ns["configure"]

    def configure_r43(module: Any, builder: Any) -> None:
        original_configure(module, builder)
        _configure_r43(ns, builder)

    ns["configure"] = configure_r43
    ns["seal_r39_rejection"] = _seal_r42_rejection
    return int(ns["main"]())
...
def main() -> int:
    try:
        ...
        print(json.dumps({
            "status": "FAIL_CLOSED_R43_CLEAN_ROOM_STATIC_ONLY",
