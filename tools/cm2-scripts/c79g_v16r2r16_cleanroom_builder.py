#!/usr/bin/env python3
"""Append-only r16 static clean-room builder.

This builder consumes only the rejected r15 bytes and creates a new r16
...
PJ = {
    ...
    "transition": OUT / f"{BASE}_v16r2r14_to_{PREV}_static_launch_transition_receipt_v1.json",
...
J = {
    ...
    "transition": OUT / f"{BASE}_v16r2r15_to_{TAG}_static_launch_transition_receipt_v1.json",
...
def rejection() -> tuple[dict[str, Any], bytes]:
    ...
    value = close({
        ...
        "rejection_reason": (
            "R15_RUNTIME_ROOT_PROTOCOL_DIVERGENCE_AND_SCHEMA_ROOT_"
...
def source_bytes(raw: bytes, role: str, anchor_value: dict[str, Any],
    ...
    old_anchor = SA.name
    old_edge = f"{BASE}_v16r2r14_to_{PREV}_static_launch_transition_receipt_v1.json"
    anchor_marker, edge_marker = "__R16_ANCHOR__", "__R15_TO_R16_EDGE__"
    text = text.replace(old_anchor, anchor_marker)
    text = text.replace(old_edge, edge_marker)
    text = text.replace("V16R2R15", "V16R2R16")
    ...
    if "v16r2r15_to_v16r2r15" in text or "v16r2r16_to_v16r2r16" in text:
...
def retag(value: Any) -> Any:
    ...
    if not isinstance(value, str):
        return value
    old_edge = f"{BASE}_v16r2r14_to_{PREV}_static_launch_transition_receipt_v1.json"
    return (value.replace(old_edge, "__R15_TO_R16_EDGE__")
            .replace("V16R2R15", "V16R2R16")
            .replace(PREV, TAG)
            .replace("__R15_TO_R16_EDGE__", J["transition"].relative_to(ROOT).as_posix()))
...
def patch_bundle(bundle: dict[str, Any], source_hashes: dict[str, str],
    ...
    bundle.update({
        ...
        "D02_unlock": False,
        "runtime_authorized": False,
        "pin_state": "R16_SOURCE_AND_JSON_PINS_UNINSTALLED__STATIC_ZERO_CREDIT",
    ...
    trust.update({
        "binding_direction": "R16_SOURCE_READS_POST_SOURCE_RECEIPTS__NO_HASH_CYCLE",
...
def build_quartet(source_hashes: dict[str, str]) -> dict[str, bytes]:
    ...
    # its root.  Historical extension bytes remain immutable in r15 and are
    # represented by the r16 rejection evidence, never by this live schema.
    ...
    for key in list(schema):
        if key.startswith("x-cm2-"):
            schema.pop(key, None)
    schema["$comment"] = (
        "CM2_R16_LIVE_SUCCESSOR__GENERIC_V16R2_ROOT_PROTOCOL__"
    ...
    values["contract"].update({
        ...
        "purpose": (
            "Append-only r16 semantic bundle with generic v16r2 runtime root "
...
def main() -> int:
    try:
        for path in (*PS.values(), *PJ.values(), SA):
            if not path.is_file():
                raise RuntimeError(f"missing r15 input: {path}")
        ...
        if any(TAG in str(path) for path in ROOT.rglob("*.pyc")):
            raise RuntimeError("r16 pyc appeared")
        result = {
