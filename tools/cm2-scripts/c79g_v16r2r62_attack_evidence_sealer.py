#!/usr/bin/env python3
"""Controlled r62 retag of the frozen attack-evidence sealer.

The r60 sealer is parsed as an AST recipe and executed only in an inert
namespace.  Version labels and paths are rebound in memory to r62; the
historical source is never edited.  The default delegated action remains the
historical read-only preflight, while ``--seal`` (not used by this preparation
step) can write only the r62 attack receipt.
"""
...
SOURCE = ROOT / "scripts/c79g_v16r2r60_attack_evidence_sealer.py"
SOURCE_SHA = "b0e4c8b736a08afb4953feb18848d6ede3ae69f3e2cd2d68d0add569af9581ee"
...
class _RetagStrings(ast.NodeTransformer):
    """Retag only string literals in the historical recipe."""
...
def load_recipe() -> dict[str, Any]:
    ...
    ns: dict[str, Any] = {"__name__": "_r60_attack_recipe_for_r62",
...
def main(argv: list[str] | None = None) -> int:
    try:
        ...
        print({"status": "FAIL_CLOSED_R62_ATTACK_EVIDENCE_WRAPPER",
