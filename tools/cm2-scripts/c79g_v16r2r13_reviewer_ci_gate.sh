#!/usr/bin/env bash
# Fail-closed CI wrapper for the versioned r13 reviewer.
# jq -e is intentionally used for the acceptance predicate: a FAIL report
# must produce a nonzero shell status rather than being masked by plain jq.
set -u -o pipefail

root_dir="$(CDPATH= cd -- "$(dirname -- "$0")/.." && pwd -P)"
reviewer="$root_dir/scripts/c79g_v16r2r13_independent_reviewer.py"
suffix="${CM2_SUCCESSOR_SUFFIX:-v16r2r13}"
predecessor="${CM2_PREDECESSOR_SUFFIX:-v16r2r12}"

command -v jq >/dev/null 2>&1 || {
  echo 'FAIL_CLOSED_R13_REVIEW_CI: jq required' >&2
  exit 2
}
[[ -f "$reviewer" ]] || {
  echo 'FAIL_CLOSED_R13_REVIEW_CI: reviewer missing' >&2
  exit 2
}

report_tmp="$(mktemp "${TMPDIR:-/tmp}/cm2-r13-review.XXXXXX.json")"
...
jq -e --arg suffix "$suffix" --arg predecessor "$predecessor" '
  type == "object"
  and (.schema | type == "string" and startswith("cm2.c79g.successor.v16r2r13-independent-read-only-review.v1"))
