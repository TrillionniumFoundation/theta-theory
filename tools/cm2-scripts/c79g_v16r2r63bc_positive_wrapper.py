#!/usr/bin/env python3
"""Run the r63bc positive authorize through a blocking held stdout pipe."""
...
BOOTSTRAP = ROOT / "scripts/c79g_v16r2r63bc_external_held_fd_bootstrap.py"
LAUNCHER_SHA256 = "27fc5caf1f13ea44dea1217c2e31c6cbac2c97e4dcc31dfc25e1e2a6dadcd2bb"
...
def main() -> int:
    ...
    env = {
        ...
        "PYTHONDONTWRITEBYTECODE": "1", "PYTHONNOUSITE": "1", "PYTHONNOUSERSITE": "1",
        "CM2_R63BC_AUTHORIZE_APPROVED": "1",
    }
    proc = subprocess.Popen(
        ["/usr/bin/python3", "-I", "-B", "-S", str(BOOTSTRAP),
         "--workspace-root", str(ROOT), "--expected-launcher-sha256", LAUNCHER_SHA256,
         "authorize"], cwd=str(ROOT), env=env, stdin=subprocess.DEVNULL,
        stdout=write_fd, stderr=subprocess.PIPE, close_fds=True)
    os.close(write_fd)
    raw, err, timed_out = drain(proc, read_fd)
    rc = proc.wait()
    report: dict[str, object] = {
        "schema": "cm2.c79g.v16r2r63bc-positive-wrapper-run-report.v1",
