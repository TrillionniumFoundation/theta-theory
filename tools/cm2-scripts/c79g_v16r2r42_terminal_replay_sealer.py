#!/usr/bin/env python3
"""r42 independent checker/terminal replay evidence.

This wrapper runs the two independent static checkers plus the semantic/path
audits twice (hash seeds 1 and 99991), and verifies the r42 exact8 bytes are
...
def run_checker(name: str, seed: str) -> tuple[dict[str, Any], bytes]:
    ...
    if name == "residue":
        if (value.get("status") !=
                "PASS_V16R2R42_NO_STALE_ACTIVE_RESIDUE_READ_ONLY" or
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if not (args.seal or args.install):
            print(json.dumps({"status": "PREFLIGHT_PASS_R42_TERMINAL_REPLAY__ZERO_CREDIT",
        ...
        print(json.dumps({"status": "FAIL_CLOSED_R42_TERMINAL_REPLAY_INTERRUPTED",
                          "installation_performed": False,
                          "formal_global_closure_credit": 0,
                          "D02_unlock": False, "runtime_authorized": False},
                         sort_keys=True))
        ...
        print(json.dumps({"status": "FAIL_CLOSED_R42_TERMINAL_REPLAY",
