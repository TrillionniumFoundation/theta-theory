#!/usr/bin/env python3
"""Clean-room r33 static successor with parameterized retagging.

The predecessor r32 namespace is sealed as a zero-credit launcher-scope
namespace/tag-census rejection.  This builder creates the r32 -> r33 rejection supersession and
...
def load_generic() -> ModuleType:
    ...
    module = ModuleType("_r33_generic_builder")
...
def definition_smoke(raw: bytes, role: str, anchor_file: str,
    ...
    ns: dict[str, Any] = {"__name__": f"_r33_static_{role}",
                          "__file__": str(b.SRC_OUT[role]),
                          "__package__": None}
    text = raw.decode("utf-8")
    exec(compile(text, str(b.SRC_OUT[role]), "exec"), ns, ns)
    root = ROOT if role != "launcher" else Path("/tmp/cm2-r33-path-crosscheck")
    if role == "launcher":
...
def path_crosscheck(generated: dict[str, bytes]) -> dict[str, Any]:
    ...
    for role in ("consumer", "launcher"):
        ns: dict[str, Any] = {"__name__": f"_r33_v14_{role}",
                              "__file__": str(b.SRC_OUT[role]), "__package__": None}
        text = generated[role].decode("utf-8")
        exec(compile(text, str(b.SRC_OUT[role]), "exec"), ns, ns)
        if role == "launcher":
            ns["configure_workspace_paths"](Path("/tmp/cm2-r33-path-crosscheck"))
