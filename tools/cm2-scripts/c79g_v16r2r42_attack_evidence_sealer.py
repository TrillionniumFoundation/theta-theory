#!/usr/bin/env python3
"""r42 mutation-attack evidence (read-only unless ``--seal``/``--install``).

The attack harness only reconstructs frozen upstream data in memory.  This
wrapper binds its receipt to the r42 exact8 namespace, runs two hash-seed
replays in ``-I -B`` subprocesses, and refuses every pre-r42 historical target.  Its only
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if not (args.seal or args.install):
            print(json.dumps({"status": "PREFLIGHT_PASS_R42_ATTACKS__ZERO_CREDIT",
        ...
        print(json.dumps({"status": "FAIL_CLOSED_R42_ATTACK_EVIDENCE_INTERRUPTED",
                          "installation_performed": False,
                          "formal_global_closure_credit": 0,
                          "D02_unlock": False, "runtime_authorized": False},
                         sort_keys=True))
        ...
        print(json.dumps({"status": "FAIL_CLOSED_R42_ATTACK_EVIDENCE",
