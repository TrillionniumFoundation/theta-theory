#!/usr/bin/env python3
"""V2 guard to freeze and permanently supersede the unpublished C79g v13 draft.
...
def one_direct_return(function: ast.FunctionDef) -> ast.Return:
    """Require a Return as a direct function-body statement.

    This intentionally strict helper remains for call sites whose terminal
    return is structurally direct.  Returns nested under ``try``/``with`` or
    another control-flow statement must use ``one_function_scope_return``;
    keeping the two contracts separate prevents a nested decoy from being
    accepted at a direct-return call site.
    """
...
class FunctionScopeReturnCollector(ast.NodeVisitor):
    """Collect Returns owned by one function, without nested-scope decoys.

    The inspected function itself is visited by identity.  Every other
    function/class/lambda node is a lexical child and is deliberately not
    traversed.  Visiting the root (instead of manually iterating ``body``)
    also makes the ownership rule explicit for ``try``/``except``/``finally``
    and for async function roots.
    """

    def __init__(self, root: ast.FunctionDef | ast.AsyncFunctionDef) -> None:
        self.root = root
        self.matches: list[ast.Return] = []
...
def function_scope_returns(
    ...
    collector = FunctionScopeReturnCollector(function)
    collector.visit(function)
    return collector.matches


def one_function_scope_return(
        function: ast.FunctionDef | ast.AsyncFunctionDef) -> ast.Return:
    matches = function_scope_returns(function)
    assert len(matches) == 1 and matches[0].value is not None
    return matches[0]


def assert_one_function_scope_return_rejected(source: str) -> None:
    tree = ast.parse(source, filename="<guard-v2-negative-return-self-test>")
    function = unique_top_function(tree, "sample")
    try:
        one_function_scope_return(function)
    except AssertionError:
        return
    raise AssertionError("negative function-scope Return self-test was accepted")


def run_function_scope_return_in_memory_self_tests() -> None:
...
def sample(flag):
    ...
    try:
        callback(flag)
        return 'semantic-top-level-try-return'
    finally:
        callback(False)
""",
        filename="<guard-v2-positive-return-self-test>")
    positive_function = unique_top_function(positive_tree, "sample")
    matches = function_scope_returns(positive_function)
    assert len(matches) == 1
    assert one_function_scope_return(positive_function) is matches[0]
    assert expression_ast_key(matches[0].value) == parsed_expression_ast_key(
        "'semantic-top-level-try-return'")
    try:
        one_direct_return(positive_function)
    except AssertionError:
        pass
    else:
        raise AssertionError(
            "direct-return helper incorrectly accepted a try-nested Return")
...
async def sample_async():
    ...
    try:
        await operation()
        return 'semantic-async-try-return'
    finally:
        await cleanup()
""",
        filename="<guard-v2-positive-async-return-self-test>")
    async_function = next(
        node for node in async_tree.body
        if isinstance(node, ast.AsyncFunctionDef) and node.name == "sample_async")
    async_matches = function_scope_returns(async_function)
    assert len(async_matches) == 1
    assert one_function_scope_return(async_function) is async_matches[0]
    assert expression_ast_key(async_matches[0].value) == parsed_expression_ast_key(
        "'semantic-async-try-return'")

    assert_one_function_scope_return_rejected(
        """
def sample():
    callback = lambda value: value
    def nested():
...
def launcher_inherited_incident_extraction(
    ...
    run_return = one_function_scope_return(run_child)
...
def main() -> int:
    ...
    run_function_scope_return_in_memory_self_tests()
