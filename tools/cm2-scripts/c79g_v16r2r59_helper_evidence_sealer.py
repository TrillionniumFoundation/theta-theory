#!/usr/bin/env python3
"""Seal r59's version-neutral launcher-helper evidence.
...
ANCHOR_FILE_SHA = "26d919c2760eb8ce2c0cfe2588f91dc8061c2ad7bbab1575ae2736ca657a43fa"
...
REVIEW_A = ROOT / "scripts/c79g_v16r2r59_role_aware_reviewer.py"
REVIEW_B = ROOT / "scripts/c79g_v16r2r59_helper_reviewer_b.py"
...
def preflight() -> tuple[dict[str, Any], bytes]:
    ...
    if sha(stable(ANCHOR)) != ANCHOR_FILE_SHA or json.loads(stable(ANCHOR).decode())["object_sha256"] != ANCHOR_OBJECT:
        raise RuntimeError("r59 anchor pin mismatch")
    ...
    if any(TAG in p.name for p in ROOT.rglob("*.pyc")):
        raise RuntimeError("r59 pyc appeared")
    body: dict[str, Any] = {
        "schema": f"cm2.c79g.{TAG}.launcher-registry-helper-version-neutral-review.v1",
        "status": "PASS_R59_DUAL_INDEPENDENT_VERSION_NEUTRAL_HELPER_REVIEW__ZERO_CREDIT",
...
def main() -> int:
    ...
    try:
        ...
        print(json.dumps({"schema": f"cm2.c79g.{TAG}.helper-evidence-sealer.failure.v1",
                          "status": "FAIL_CLOSED_R59_HELPER_EVIDENCE", "error": f"{type(exc).__name__}:{exc}",
                          "formal_global_closure_credit": 0, "D02_unlock": False,
