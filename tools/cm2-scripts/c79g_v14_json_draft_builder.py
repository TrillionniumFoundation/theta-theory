#!/usr/bin/env python3
"""Mechanically rebuild the C79g v14 static JSON drafts.

This helper never imports or executes any protocol Python source.  It derives
the four explicitly versioned v14 schema/contract/transition/audit drafts from
...
V14_DRAFT_BUILDER_ENABLED = True
V14_STAGE2_SCHEMA_CONTRACT_TRANSFORM_COMPLETE = True
...
V14_PREDECESSOR_UNIQUE_FILE_IDENTITY_COUNT = 105
V14_PREPUBLICATION_UNIQUE_FILE_IDENTITY_COUNT = 113
V14_TERMINAL_UNIQUE_FILE_IDENTITY_COUNT = 115
...
CONTRACT_AUTHORITY_ROOT_STRUCTURE = (
    "ZERO_CREDIT_INNER=(V13_PREPUBLICATION_SUPERSESSION_RECEIPT + "
    ...
    "FRESH_EMPTY_V14_REJECTION_NAMESPACE); "
...
CONTRACT_REQUIRED_CONJUNCTS = [
    ...
    "NO_LATER_V14_REJECTION_AT_EACH_READ",
...
def load_frozen_v13_supersession_receipt() -> dict[str, Any]:
    """Load and close the one-way v13 rejection authority for v14."""
    ...
    require(
        ...
        successor.get("v14_must_preserve_v12_exact10_and_official_rejection")
            is True and
        successor.get("v14_predecessor_unique_live_identity_count") ==
            V14_PREDECESSOR_UNIQUE_FILE_IDENTITY_COUNT and
        successor.get("v14_prepublication_unique_live_identity_count") ==
            V14_PREPUBLICATION_UNIQUE_FILE_IDENTITY_COUNT and
        successor.get("v14_terminal_unique_live_identity_count") ==
            V14_TERMINAL_UNIQUE_FILE_IDENTITY_COUNT and
        successor.get("v14_terminal_group_vector") ==
            V14_TERMINAL_GROUP_VECTOR and
        inherited.get(
...
def v13_supersession_summary(
    ...
    """Return the exact seven-key summary embedded in all v14 JSON proofs."""
    summary = {
...
def write_draft_pair_exclusive(
    ...
    try:
        ...
        for path, _ in raws:
            ...
            try:
                ...
                raise FileExistsError(
                    "refuse to replace existing v14 seed draft: " + str(path))
        ...
        for path, raw in raws:
            descriptor = os.open(
                path.name, open_flags, 0o600, dir_fd=parent_descriptor)
...
def pin_normalized_launcher_ast_sha256(raw: bytes, label: str) -> str:
    ...
    require(key_names == expected and len(base7.values) == len(expected),
            label + ": exact ordered receipt-plus-v14 BASE7 census")
...
def producer_source_registry_shape_from_ast(raw: bytes) -> int:
    tree = parse_source_ast(raw, "producer_v14")
...
def migrate_current_v12_surface_to_v14(node: Any) -> Any:
    """Purely rename the frozen v12 *current* surface before history append.

    The frozen v12 schema and contract do not yet contain a historical-v12
    branch.  Therefore every v12 token in those two seed objects denotes the
    then-current surface.  This transform is deliberately applied before the
    immutable v12 proof and rejected-v13 summary are appended.
    """
    def migrate_text(value: str) -> str:
        for old, new in (
                ("v11_to_v12", "v13_to_v14"),
                ("V11_TO_V12", "V13_TO_V14"),
                ("v11-to-v12", "v13-to-v14"),
                ("V11-TO-V12", "V13-TO-V14")):
            value = value.replace(old, new)
        return value.replace("v12", "v14").replace("V12", "V14")
    ...
    if isinstance(node, dict):
        ...
        for key, value in node.items():
            ...
            require(migrated_key not in result,
                    "v12-to-v14 migration key collision: " +
                    repr(migrated_key))
            result[migrated_key] = migrate_current_v12_surface_to_v14(value)
        return result
    if isinstance(node, list):
        return [migrate_current_v12_surface_to_v14(value) for value in node]
...
def replace_current_bundle_first_member(node: Any) -> None:
    old = V11_REJECTION_REL
    receipt = str(V13_SUPERSESSION_RECEIPT.relative_to(ROOT))
    if isinstance(node, dict):
        for value in node.values():
            replace_current_bundle_first_member(value)
    elif isinstance(node, list):
        if (len(node) >= 2 and node[0] == old and isinstance(node[1], str) and
                f"{BASE}_schema_v14.json" in node[1]):
            node[0] = receipt
        if (len(node) >= 2 and node[0] == "v11_official_rejection" and
                node[1] == "closed_schema_v14"):
            node[0] = "v13_supersession_receipt"
        for value in node:
            replace_current_bundle_first_member(value)


def build_schema(
        frozen_v12_schema: dict[str, Any],
        v12_proof: dict[str, Any],
        v13_summary: dict[str, Any]) -> dict[str, Any]:
    """Pure v12-schema to v14-schema transform; never reads its v14 target."""
    require(isinstance(frozen_v12_schema, dict) and
            frozen_v12_schema.get("$id") ==
                "cm2.round306c79g.true-global-no-producer-consumer."
                "v12.composite-authority-schema",
            "build_schema receives the pinned frozen v12 schema seed")
    schema = migrate_current_v12_surface_to_v14(frozen_v12_schema)
    require(V14_STAGE2_SCHEMA_CONTRACT_TRANSFORM_COMPLETE is True,
            "FAIL_CLOSED_STAGE2_BLOCKER__V12_PROOF_INCIDENT_HISTORICAL_"
            "KEYSET_EXACT56_AND_84_112_SCHEMA_TRANSFORM_NOT_COMPLETE")
    require(schema.get("$id") ==
            "cm2.round306c79g.true-global-no-producer-consumer."
            "v14.composite-authority-schema",
            "schema current-version id migrated to v14")
    schema["title"] = (
        "C79g v14 static successor after frozen published/rejected v12 and "
        "rejected prepublication v13")
    schema["description"] = (
        "Closed v14 schema for the append-only zero-credit successor after "
        "the frozen v12 exact10 plus official rejection and the separately "
        "frozen v13 prepublication pyc-contamination supersession receipt; "
        "all v3-v12 rejection history remains byte-pinned and runtime stays "
    ...
    rename_closed_property(
        ...
        "current_v14_producer_source_content_opened_read_decoded_parsed_compiled_imported_or_executed")
    for obsolete in (
    ...
    add_required_const(
        ...
        True,
        after="current_v14_producer_source_content_opened_read_decoded_parsed_compiled_imported_or_executed")
    add_required_const(
        independent,
        "v10_colon_prefix_witness_independently_rederived_from_inherited_exact10_held_fds",
        True,
        after="inherited_incident_authority_exact17_held_and_terminally_replayed")
    ...
    for old, new in (
            ("current_v12_and_all_predecessor_unique_file_identity_count",
             "current_v14_and_all_predecessor_unique_file_identity_count"),
            ("current_v12_and_all_predecessor_same_statx_mount",
             "current_v14_and_all_predecessor_same_statx_mount"),
            ("v11_to_v12_transition_identity", "v13_to_v14_transition_identity"),
            ...
            ("v12_rejection_namespace", "v14_rejection_namespace"),
            ("v12_later_rejection", "v14_later_rejection"),
            ("reject_command_exempts_only_fresh_empty_v12_rejection_namespace",
             "reject_command_exempts_only_fresh_empty_v14_rejection_namespace")):
    ...
    defs["staticFreezeProof"]["properties"][
        "append_only_history_unique_file_identity_count"]["const"] = \
            V14_TERMINAL_UNIQUE_FILE_IDENTITY_COUNT
    defs["coldLaunchProof"]["properties"][
        "current_v14_and_all_predecessor_unique_file_identity_count"]["const"] = \
            V14_PREPUBLICATION_UNIQUE_FILE_IDENTITY_COUNT
...
def build_contract(
        frozen_v12_contract: dict[str, Any], schema_file_sha256: str,
        v12_proof: dict[str, Any],
        v13_summary: dict[str, Any]) -> dict[str, Any]:
    """Pure v12-contract to v14-contract transform; never reads its target."""
    require(isinstance(frozen_v12_contract, dict),
            "build_contract receives a frozen v12 contract object")
    verify_object(
        frozen_v12_contract, "frozen v12 contract seed",
        V12_CONTRACT_OBJECT_SHA256)
    contract = migrate_current_v12_surface_to_v14(frozen_v12_contract)
    require(V14_STAGE2_SCHEMA_CONTRACT_TRANSFORM_COMPLETE is True,
            "FAIL_CLOSED_STAGE2_BLOCKER__V12_PROOF_INCIDENT_HISTORICAL_"
            "KEYSET_AUTHORITY_ROOT_AND_84_112_CONTRACT_TRANSFORM_NOT_COMPLETE")
    require(contract.get("schema") ==
            "cm2.round306c79g.true-global-no-producer-consumer.v14.contract",
            "contract current-version schema migrated to v14")
    require("v14_bundle" in contract and "v12_bundle" not in contract,
            "contract current bundle key migrated to v14")
    contract["purpose"] = (
        "Append-only v14 zero-credit clean-room successor after the frozen "
        "published-then-officially-rejected v12 exact10 and the separately "
        "frozen v13 prepublication pyc-contamination supersession receipt. "
        "The v13 draft is never treated as a published predecessor; its "
        "receipt binds inherited exact16 incident evidence and the "
        "105-to-113-to-115 live-identity census.")
    ...
    contract["v11_dual_validator_divergence_incident"] = copy.deepcopy(
        V11_INCIDENT)
    bundle = contract["v14_bundle"]
    base7_paths = [
        str(V13_SUPERSESSION_RECEIPT.relative_to(ROOT)),
    ...
    bundle["exact10_ordered_paths"] = [
        *bundle["exact8_ordered_paths"],
        str(V14_MANIFEST.relative_to(ROOT)),
        str(V14_OUTER.relative_to(ROOT)),
    ]
    ...
    receipts["v13_to_v14_transition_path"] = str(
    ...
    receipts.pop(
        ...
        None)
    receipts["runtime_consumer_must_hold_strict_parse_object_close_and_terminally_replay_inherited_exact17"] = True
    receipts.pop(
    ...
    outer["exact8_manifest_path"] = str(V14_MANIFEST.relative_to(ROOT))
    outer["outer_last_path"] = str(V14_OUTER.relative_to(ROOT))
    outer["manifest_order"] = (
        "V13_SUPERSESSION_RECEIPT_THEN_SCHEMA_THEN_CONTRACT_THEN_PRODUCER_THEN_"
        "INDEPENDENT_SOURCE_THEN_V13_TO_V14_TRANSITION_THEN_STATIC_AUDIT_THEN_LAUNCHER")
    outer.pop("current_v12_exact10_plus_all_append_only_predecessors_unique_file_identity_count", None)
    outer["current_v14_exact10_plus_all_append_only_predecessors_unique_file_identity_count"] = \
        V14_TERMINAL_UNIQUE_FILE_IDENTITY_COUNT
    outer["all_115_file_identities_share_one_statx_mount"] = True
    ...
    outer["v14_predecessor_unique_live_identity_count"] = \
        V14_PREDECESSOR_UNIQUE_FILE_IDENTITY_COUNT
    outer["v14_prepublication_unique_live_identity_count"] = \
        V14_PREPUBLICATION_UNIQUE_FILE_IDENTITY_COUNT
    outer["v14_terminal_unique_live_identity_count"] = \
        V14_TERMINAL_UNIQUE_FILE_IDENTITY_COUNT
    outer["v14_terminal_group_vector"] = copy.deepcopy(
        V14_TERMINAL_GROUP_VECTOR)
    outer[
    ...
    old_terminal = next((key for key in outer if key.startswith(
        "launcher_terminally_replays_v14_exact10_v10_exact10_")), None)
    if old_terminal is not None:
        value = outer.pop(old_terminal)
        outer[old_terminal.replace("v14_exact10_v10_exact10_",
                                   "v14_exact10_v11_exact10_v10_exact10_")
    ...
    bundle["acyclic_binding_order"] = (
        "V13_SUPERSESSION_RECEIPT_THEN_SCHEMA_THEN_CONTRACT_THEN_PRODUCER_THEN_"
        "INDEPENDENT_SOURCE_THEN_V13_TO_V14_TRANSITION_THEN_STATIC_AUDIT_THEN_"
    ...
    runtime.pop(
        ...
        None)
    runtime[
        "current_v14_producer_source_open_read_decode_parse_compile_import_or_execute_allowed"] = False
    runtime.pop(
    ...
    runtime[
        "v10_colon_prefix_witness_independently_rederived_by_consumer_from_inherited_exact17_held_fds"] = True
    ...
    census["v14_predecessor_unique_live_identity_count"] = \
        V14_PREDECESSOR_UNIQUE_FILE_IDENTITY_COUNT
    census["v14_prepublication_unique_live_identity_count"] = \
        V14_PREPUBLICATION_UNIQUE_FILE_IDENTITY_COUNT
    census["v14_terminal_unique_live_identity_count"] = \
        V14_TERMINAL_UNIQUE_FILE_IDENTITY_COUNT
    census["v14_terminal_group_vector"] = copy.deepcopy(
        V14_TERMINAL_GROUP_VECTOR)
    runtime.pop(
    ...
    producer_bundle = contract["v14_bundle"]["build_only_producer"]
    ...
    authority["preseal_root_domain"] = "CM2_C79G_V14_PRESEAL_ROOT_V2"
...
def build_transition(
        v12_transition: dict[str, Any], schema_file_sha256: str,
    ...
    body: dict[str, Any] = {
        "schema": (
            "cm2.round306c79g.true-global-no-producer-consumer."
            "v13-to-v14-static-launch-transition.v1"),
        "status": (
        ...
        "append_only_predecessor_v3_regression": copy.deepcopy(
            v12_transition["append_only_predecessor_v3_regression"]),
        "rejected_unpublished_predecessor_v4": copy.deepcopy(
            v12_transition["rejected_unpublished_predecessor_v4"]),
        ...
        "successor_v14_static_bundle": {
            ...
            "static_audit_v14_path": str(AUDIT.relative_to(ROOT)),
            "cold_launcher_v14_path": str(LAUNCHER.relative_to(ROOT)),
        },
        "physical_mode_policy": {
            "v14_working_files_mode_before_cold_freeze": "0664",
            "v14_exact8_required_final_mode": "0444",
            "v14_exact8_required_final_nlink": 1,
            "v14_exact8_physical_freeze_completed": False,
            "v14_manifest_physical_freeze_completed": False,
            "v14_outer_physical_freeze_completed": False,
            "historical_modes_are_exact_observed_snapshot_guards_not_immutability_claims":
                True,
        },
        "cold_launch_boundary": {
            "base7_order": [
                "v13_supersession_receipt", "closed_schema_v14",
                "contract_v14", "producer_v14", "consumer_v14",
                "transition_v13_to_v14", "static_audit_v14",
            ],
            "base7_first_member_is_v13_supersession_receipt": True,
            ...
            "expected_terminal_unique_file_identity_count":
                V14_TERMINAL_UNIQUE_FILE_IDENTITY_COUNT,
            "terminal_group_vector": copy.deepcopy(V14_TERMINAL_GROUP_VECTOR),
            "all_115_file_identities_must_share_one_statx_mount": True,
...
def v14_static_no_run() -> dict[str, Any]:
    runtime_artifacts = []
    ...
    if stat.S_ISDIR(runtime_named.st_mode):
        runtime_artifacts = [
            ...
            if "c79g-v14" in path.name]
    require(not runtime_artifacts,
            "v14 runtime artifacts already exist; refuse static GO audit")
    ...
    require(process_count == 0,
            "v14 protocol process exists; refuse static GO audit")
    ...
    require(pyc_count == 0,
            "v14 protocol pyc exists; refuse static GO audit")
    require(not path_lexists(V14_MANIFEST) and not path_lexists(V14_OUTER),
            "v14 manifest/outer already exists; refuse draft audit rebuild")
    return {
...
def build_static_audit(
        v12_audit: dict[str, Any], schema: dict[str, Any],
    ...
    require(tuple(key for key in old_inputs
            ...
            "frozen v12 audit inherited historical input order")
    historical_inputs = {
    ...
    require(all(is_sha256(value) for value in historical_inputs.values()),
            "frozen v12 audit inherited historical input pins")
    ...
    v4 = copy.deepcopy(
        v12_audit["predecessor_v4_rejection_supersession_regression"])
    v4["same_defects_absent_from_v14"] = v4.pop(
        "same_defects_absent_from_v12")
    v4["v14_closed_object_required_property_mismatch_count"] = v4.pop(
        "v12_closed_object_required_property_mismatch_count")
    v4["v14_closed_object_required_property_mismatch_count"] = \
        schema_closure["closed_object_mismatch_count"]

    body: dict[str, Any] = {
        "schema": (
            "cm2.round306c79g.true-global-no-producer-consumer.static-audit.v14"),
        "status": (
        ...
        "predecessor_v3_exact10_regression": copy.deepcopy(
            v12_audit["predecessor_v3_exact10_regression"]),
        "v3_official_later_rejection_regression": copy.deepcopy(
            v12_audit["v3_official_later_rejection_regression"]),
        ...
        "sealed_exec_and_no_producer_static_proof": {
            ...
            "consumer_inherited_incident_authority_exact17_bytes_read_only_noncredit_allowed":
                True,
...
def verify_constants() -> None:
    ...
    assert sha256_bytes(canonical(list(STATIC_AUDIT_INPUT_KEY_ORDER))) == \
        STATIC_AUDIT_INPUT_KEY_ORDER_SHA256
    assert len(STATIC_AUDIT_TOP_LEVEL_KEY_ORDER) == 28
    assert len(set(STATIC_AUDIT_TOP_LEVEL_KEY_ORDER)) == 28
...
def build_schema_contract_from_frozen_v12_seed(
    ...
    require(isinstance(schema_seed, dict) and isinstance(contract_seed, dict),
            "frozen v12 schema/contract seed bundle")
    v12_proof = seed_bundle.get("v12_published_rejected_proof")
    v13_summary = seed_bundle.get("v13_supersession_summary")
    require(isinstance(v12_proof, dict) and isinstance(v13_summary, dict),
            "frozen v12 proof and v13 supersession summary seed")
    schema = build_schema(schema_seed, v12_proof, v13_summary)
    ...
    verify_object(contract, "rebuilt v14 contract draft")
...
def command_seed_schema_contract(
    ...
    """O_EXCL-install the first v14 S/C drafts from pinned frozen v12 bytes."""
    verify_constants()
    seed_bundle = load_frozen_v12_static_seed_bundle()
    schema, contract, schema_raw, contract_raw = \
        build_schema_contract_from_frozen_v12_seed(seed_bundle)
    ...
    terminal_seed_bundle = load_frozen_v12_static_seed_bundle()
    terminal_schema, terminal_contract, terminal_schema_raw, \
        terminal_contract_raw = build_schema_contract_from_frozen_v12_seed(
    ...
    writer_window.revalidate()
    return {
        "status": "SEEDED_V14_SCHEMA_CONTRACT_DRAFTS__RUNTIME_NOT_AUTHORIZED",
        "official_writer_lock": official_lock_result(),
        "seed_schema_file_sha256": V12_SCHEMA_FILE_SHA256,
        "seed_contract_file_sha256": V12_CONTRACT_FILE_SHA256,
        "seed_contract_object_sha256": V12_CONTRACT_OBJECT_SHA256,
...
def command_schema_contract(
    ...
    return {
        "status": (
            "REPLAYED_EXISTING_V14_SCHEMA_CONTRACT_EXACT_BYTES__"
...
def command_transition_audit(
        ...
        writer_window: OfficialStaticWriterWindow) -> dict[str, Any]:
    """Rebuild only the two writable post-core v14 JSON drafts.
    ...
    seed_bundle = load_frozen_v12_static_seed_bundle()
    require(V14_STAGE2_SCHEMA_CONTRACT_TRANSFORM_COMPLETE is True,
            "FAIL_CLOSED_STAGE2_BLOCKER__V14_TRANSITION_AUDIT_REBUILD_NOT_READY")

    v12_transition = seed_bundle["transition"]
    v12_audit = seed_bundle["audit"]
    ...
    for role, raw in source_raw.items():
        parse_source_ast(raw, "current_v14_" + role)

    schema = json.loads(
        input_raw[SCHEMA], object_pairs_hook=_no_duplicate_pairs)
    contract = json.loads(
        input_raw[CONTRACT], object_pairs_hook=_no_duplicate_pairs)
    require(isinstance(schema, dict), "current v14 schema JSON object")
    require(isinstance(contract, dict), "current v14 contract JSON object")
    verify_object(contract, "current v14 contract")
    ...
    pin_normalized = pin_normalized_launcher_ast_sha256(
        source_raw["launcher"], "current_v14_launcher")
    ...
    transition = build_transition(
        v12_transition, schema_file_sha256, contract_file_sha256,
    ...
    audit = build_static_audit(
        v12_audit, schema, closure, transition, transition_raw,
    ...
    require(isinstance(terminal_schema, dict) and
            ...
            "terminal v14 schema/contract JSON objects")
    verify_object(terminal_contract, "terminal v14 contract")
    ...
    terminal_pin_normalized = pin_normalized_launcher_ast_sha256(
        terminal_source_raw["launcher"], "terminal_current_v14_launcher")
    ...
    return {
        "status": (
            "INSTALLED_EXCLUSIVE_V14_TRANSITION_AUDIT_DRAFT_PAIR__"
            "RUNTIME_NOT_AUTHORIZED"
            if pair_action == "installed_exclusive" else
            "REPLAYED_EXISTING_V14_TRANSITION_AUDIT_EXACT_BYTES__"
            "NO_WRITE__RUNTIME_NOT_AUTHORIZED"),
        "official_writer_lock": official_lock_result(),
...
def main() -> None:
    if not V14_DRAFT_BUILDER_ENABLED:
        raise SystemExit(
            "C79g v14 builder draft is disabled until v12 held-FD proof, "
            "incident closure, exact56 rejection schema, and 84/112 census "
    ...
    transition_audit.add_argument(
        ...
        help=(
            "canonically object-closed current-v14 external checker report; "
