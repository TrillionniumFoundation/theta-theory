#!/usr/bin/env bash
# Fail-closed jq -e wrapper for the read-only r15 34-check reviewer.
set -u -o pipefail

root_dir="$(CDPATH= cd -- "$(dirname -- "$0")/.." && pwd -P)"
reviewer="$root_dir/scripts/c79g_v16r2r15_independent_reviewer.py"
suffix="${CM2_SUCCESSOR_SUFFIX:-v16r2r15}"
predecessor="${CM2_PREDECESSOR_SUFFIX:-v16r2r14}"

command -v jq >/dev/null 2>&1 || {
  echo 'FAIL_CLOSED_R15_REVIEW_CI: jq required' >&2
  exit 2
}
[[ -f "$reviewer" ]] || {
  echo 'FAIL_CLOSED_R15_REVIEW_CI: reviewer missing' >&2
  exit 2
}

report_tmp="$(mktemp "${TMPDIR:-/tmp}/cm2-r15-review.XXXXXX.json")"
...
jq -e --arg suffix "$suffix" --arg predecessor "$predecessor" '
  type == "object"
  and (.schema | type == "string" and startswith("cm2.c79g.successor.v16r2r15-independent-read-only-review.v1"))
