#!/usr/bin/env python3
"""Mechanically rebuild the C79g v13 static JSON drafts.

This helper never imports or executes any protocol Python source.  It only
edits the seven explicitly versioned v13 draft JSON surfaces and refuses to
...
V13_STAGE2_SCHEMA_CONTRACT_TRANSFORM_COMPLETE = False
...
V13_PREPUBLICATION_UNIQUE_FILE_IDENTITY_COUNT = 106
V13_TERMINAL_UNIQUE_FILE_IDENTITY_COUNT = 108
V11_REJECTION_REL = (
...
CONTRACT_AUTHORITY_ROOT_STRUCTURE = (
    ...
    "FRESH_EMPTY_V13_REJECTION_NAMESPACE); "
...
CONTRACT_REQUIRED_CONJUNCTS = [
    ...
    "NO_LATER_V13_REJECTION_AT_EACH_READ",
...
def read_frozen_regular_file(path: Path, file_sha256: str) -> bytes:
    ...
    try:
        ...
        require(stat.S_ISREG(before.st_mode), path.name + ": frozen regular file")
        require(stat.S_IMODE(before.st_mode) == 0o444 and before.st_nlink == 1,
                path.name + ": frozen 0444 nlink1 identity")
        ...
        require(_stat_identity(before) == _stat_identity(after) ==
                ...
                path.name + ": stable held/path identity")
        require(len(raw) == before.st_size,
                path.name + ": held byte count equals stat size")
        require(sha256_bytes(raw) == file_sha256,
...
def write_seed_drafts_exclusive(
        ordered_drafts: tuple[tuple[Path, dict[str, Any]], ...],
        ) -> list[dict[str, Any]]:
    """Install a complete seed set with O_EXCL, fsync, and same-FD replay.

    This is intentionally separate from ``write_draft``: it can only create a
    previously absent v13 schema/contract pair and can never overwrite one.
    ...
    """
    require(tuple(path for path, _ in ordered_drafts) == (SCHEMA, CONTRACT),
            "exclusive seed writer exact schema-then-contract targets")
    raws = [(path, draft_bytes(value)) for path, value in ordered_drafts]
    parent_flags = os.O_RDONLY | os.O_DIRECTORY | os.O_CLOEXEC
    parent_descriptor = os.open(OUT, parent_flags)
    ...
    try:
        ...
        for path, _ in raws:
            ...
            try:
                ...
                raise FileExistsError(
                    "refuse to replace existing v13 seed draft: " + str(path))
        ...
        for path, raw in raws:
            ...
            require(stat.S_ISREG(initial.st_mode) and initial.st_nlink == 1 and
                    initial.st_dev == parent_before.st_dev,
                    path.name + ": new unique regular seed inode")
            ...
            require((frozen_draft.st_dev, frozen_draft.st_ino) ==
                    ...
                    path.name + ": exclusive seed inode/mode/size replay")
            require(_read_all_fd(descriptor) == raw,
                    path.name + ": exclusive seed same-FD terminal replay")
...
def pin_normalized_launcher_ast_sha256(raw: bytes, label: str) -> str:
    ...
    require(key_names == expected and len(base7.values) == len(expected),
            label + ": exact ordered v11-plus-v13 BASE7 census")
    v11_before = ast.dump(
...
def producer_source_registry_shape_from_ast(raw: bytes) -> int:
    tree = parse_source_ast(raw, "producer_v13")
...
def replace_current_bundle_first_member(node: Any) -> None:
    ...
    if isinstance(node, dict):
        ...
        if (len(node) >= 2 and node[0] == old and isinstance(node[1], str) and
                f"{BASE}_schema_v13.json" in node[1]):
            node[0] = V11_REJECTION_REL
        if (len(node) >= 2 and node[0] == "v10_official_rejection" and
                node[1] == "closed_schema_v13"):
            node[0] = "v11_official_rejection"
        for value in node:
            replace_current_bundle_first_member(value)


def build_schema(frozen_v12_schema: dict[str, Any]) -> dict[str, Any]:
    """Pure v12-schema to v13-schema transform; never reads its v13 target."""
    ...
    schema = copy.deepcopy(frozen_v12_schema)
    require(V13_STAGE2_SCHEMA_CONTRACT_TRANSFORM_COMPLETE is True,
            "FAIL_CLOSED_STAGE2_BLOCKER__V12_PROOF_INCIDENT_HISTORICAL_"
            "KEYSET_EXACT56_AND_106_108_SCHEMA_TRANSFORM_NOT_COMPLETE")
    defs = schema["$defs"]
    ...
    rename_closed_property(
        ...
        "current_v13_producer_source_content_opened_read_decoded_parsed_compiled_imported_or_executed")
    add_required_const(
        independent,
        "frozen_predecessor_incident_source_exact5_held_fd_bytes_read_only_noncredit",
        True,
        after="current_v13_producer_source_content_opened_read_decoded_parsed_compiled_imported_or_executed")
    add_required_const(
        independent,
    ...
    defs["coldLaunchProof"]["properties"][
        "current_v13_and_all_predecessor_unique_file_identity_count"]["const"] = 98
    return schema


def build_contract(frozen_v12_contract: dict[str, Any],
                   schema_file_sha256: str) -> dict[str, Any]:
    """Pure v12-contract to v13-contract transform; never reads its target."""
    ...
    verify_object(
        ...
        V12_CONTRACT_OBJECT_SHA256)
    contract = copy.deepcopy(frozen_v12_contract)
    require(V13_STAGE2_SCHEMA_CONTRACT_TRANSFORM_COMPLETE is True,
            "FAIL_CLOSED_STAGE2_BLOCKER__V12_PROOF_INCIDENT_HISTORICAL_"
            "KEYSET_AUTHORITY_ROOT_AND_106_108_CONTRACT_TRANSFORM_NOT_COMPLETE")
    contract["purpose"] = (
        "Append-only v13 zero-credit static successor after the frozen v11 "
    ...
    outer["manifest_order"] = (
        ...
        "INDEPENDENT_SOURCE_THEN_V12_TO_V13_TRANSITION_THEN_STATIC_AUDIT_THEN_LAUNCHER")
    outer["current_v13_exact10_plus_all_append_only_predecessors_unique_file_identity_count"] = 98
    ...
    old_terminal = next((key for key in outer if key.startswith(
        "launcher_terminally_replays_v13_exact10_v10_exact10_")), None)
    if old_terminal is not None:
        value = outer.pop(old_terminal)
        outer[old_terminal.replace("v13_exact10_v10_exact10_",
                                   "v13_exact10_v11_exact10_v10_exact10_")
    ...
    bundle["acyclic_binding_order"] = (
        "V11_OFFICIAL_REJECTION_THEN_SCHEMA_THEN_CONTRACT_THEN_PRODUCER_THEN_"
        "INDEPENDENT_SOURCE_THEN_V12_TO_V13_TRANSITION_THEN_STATIC_AUDIT_THEN_"
    ...
    runtime.pop(
        ...
        None)
    runtime[
        "current_v13_producer_source_open_read_decode_parse_compile_import_or_execute_allowed"] = False
    runtime[
    ...
    producer_bundle = contract["v13_bundle"]["build_only_producer"]
    ...
    authority["preseal_root_domain"] = "CM2_C79G_V13_PRESEAL_ROOT_V2"
...
def build_transition(
    ...
    body: dict[str, Any] = {
        "schema": (
            "cm2.round306c79g.true-global-no-producer-consumer."
            "v12-to-v13-static-launch-transition.v1"),
        "status": (
            "STATIC_BYTES_CLOSED_V12_TO_V13__PHYSICAL_FREEZE_PENDING__"
        ...
        "successor_v13_static_bundle": {
            ...
            "static_audit_v13_path": str(AUDIT.relative_to(ROOT)),
            "cold_launcher_v13_path": str(LAUNCHER.relative_to(ROOT)),
        },
        "physical_mode_policy": {
            "v13_working_files_mode_before_cold_freeze": "0664",
            "v13_exact8_required_final_mode": "0444",
            "v13_exact8_required_final_nlink": 1,
            "v13_exact8_physical_freeze_completed": False,
            "v13_manifest_physical_freeze_completed": False,
            "v13_outer_physical_freeze_completed": False,
            "historical_modes_are_exact_observed_snapshot_guards_not_immutability_claims":
                True,
        },
        "cold_launch_boundary": {
            "base7_order": [
                "v11_official_rejection", "closed_schema_v13",
                "contract_v13", "producer_v13", "consumer_v13",
                "transition_v12_to_v13", "static_audit_v13",
            ],
...
def v13_static_no_run() -> dict[str, Any]:
    runtime = ROOT / ".cm2-runtime"
    runtime_artifacts = []
    if runtime.is_dir():
        runtime_artifacts = [
            path for path in runtime.rglob("*")
            if "c79g-v13" in path.name]
    require(not runtime_artifacts,
            "v13 runtime artifacts already exist; refuse static GO audit")
    ...
    require(process_count == 0,
            "v13 protocol process exists; refuse static GO audit")
    ...
    require(pyc_count == 0,
            "v13 protocol pyc exists; refuse static GO audit")
    require(not V13_MANIFEST.exists() and not V13_OUTER.exists(),
            "v13 manifest/outer already exists; refuse draft audit rebuild")
    return {
        "C79_entrypoint_executed": False,
        "C79_v13_runtime_artifact_count": 0,
        "C79_v13_process_count": 0,
...
def build_static_audit(
    ...
    v4["same_defects_absent_from_v13"] = v4.pop(
        "same_defects_absent_from_v11")
    v4["v13_closed_object_required_property_mismatch_count"] = v4.pop(
        "v11_closed_object_required_property_mismatch_count")
    v4["v13_closed_object_required_property_mismatch_count"] = \
        schema_closure["closed_object_mismatch_count"]

    body: dict[str, Any] = {
        "schema": (
            "cm2.round306c79g.true-global-no-producer-consumer.static-audit.v13"),
        "status": (
        ...
        "static_credit_census": {
            "all_persisted_v13_objects_D02_started": False,
            "all_persisted_v13_objects_D02_unlock": False,
            "all_persisted_v13_objects_formal_global_closure_credit": 0,
...
def verify_constants() -> None:
    ...
    assert len(STATIC_AUDIT_INPUT_KEY_ORDER) == 44
...
def build_schema_contract_from_frozen_v12_seed(
    ...
    schema = build_schema(schema_seed)
    schema_raw = draft_bytes(schema)
    contract = build_contract(contract_seed, sha256_bytes(schema_raw))
    contract_raw = draft_bytes(contract)
    verify_object(contract, "rebuilt v13 contract draft")
    return schema, contract, schema_raw, contract_raw


def command_seed_schema_contract() -> None:
    """O_EXCL-install the first v13 S/C drafts from pinned frozen v12 bytes."""
    verify_constants()
    ...
    details = write_seed_drafts_exclusive((
        (SCHEMA, schema),
        (CONTRACT, contract),
    ))
    require(SCHEMA.read_bytes() == schema_raw and
            CONTRACT.read_bytes() == contract_raw,
            "exclusive v13 schema/contract pathname terminal replay")
    print(json.dumps({
        "status": "SEEDED_V13_SCHEMA_CONTRACT_DRAFTS__RUNTIME_NOT_AUTHORIZED",
...
def command_schema_contract() -> None:
    ...
    require(SCHEMA.read_bytes() == schema_raw and
            CONTRACT.read_bytes() == contract_raw,
            "rebuilt v13 schema/contract pathname terminal replay")
...
def command_transition_audit(checker_report_path: Path) -> None:
    """Rebuild only the two writable post-core v13 JSON drafts.
    ...
    seed_bundle = load_frozen_v12_static_seed_bundle()
    require(V13_STAGE2_SCHEMA_CONTRACT_TRANSFORM_COMPLETE is True,
            "FAIL_CLOSED_STAGE2_BLOCKER__V13_TRANSITION_AUDIT_REBUILD_NOT_READY")
    for path in (TRANSITION, AUDIT):
        require(path.is_file(), "missing writable v13 JSON draft: " + str(path))
        if not (os.stat(path).st_mode & 0o200):
    ...
    for role, raw in source_raw.items():
        parse_source_ast(raw, "current_v13_" + role)

    schema = json.loads(
        input_raw[SCHEMA], object_pairs_hook=_no_duplicate_pairs)
    contract = json.loads(
        input_raw[CONTRACT], object_pairs_hook=_no_duplicate_pairs)
    require(isinstance(schema, dict), "current v13 schema JSON object")
    require(isinstance(contract, dict), "current v13 contract JSON object")
    verify_object(contract, "current v13 contract")
    ...
    pin_normalized = pin_normalized_launcher_ast_sha256(
        source_raw["launcher"], "current_v13_launcher")
...
def main() -> None:
    if not V13_DRAFT_BUILDER_ENABLED:
        raise SystemExit(
            "C79g v13 builder draft is disabled until v12 held-FD proof, "
            "incident closure, exact56 rejection schema, and 106/108 census "
    ...
    transition_audit.add_argument(
        ...
        help=(
            "canonically object-closed current-v13 external checker report; "
