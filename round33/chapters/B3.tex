...
If scalar increments satisfy $E|Z(e_i)|^p\le C a^p$ for all tests of unit $H_m$ norm, Minkowski in $L^{p/2}$ gives
\[
 (E\|Z\|_{H_{-(m+s)}}^p)^{2/p}
 \le\sum_i\lambda_i^{-2s}(E|Z(e_i)|^p)^{2/p}
 \le C a^2\sum_i\lambda_i^{-2s}.
\]
This supplies the actual summation argument and keeps the same final path space for all terms.
