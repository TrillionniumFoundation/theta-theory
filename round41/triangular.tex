...
implies \(\sup_i\abs{Y_i}\le B_0+\abs{\xi_i}\) in the applications below;
...
\begin{proof}
Extend the mean fields to one fixed rectangle containing \(\Theta\), with the
same derivative bound up to a constant.  For each \(\abs\alpha\le s\), the
martingale orthogonality following from
\(\E(\xi_i\mid\cG_i)=0\) gives
\[
 \E_E\int
 \abs{\partial_\theta^\alpha S_{n,\beta}(\theta)}^2d\theta
 =\sigma^2\sum_{i=1}^n\E_E\int
...
\(O_{\frE}(\sqrt n)\abs{\theta-\theta_0}\), uniformly in \(\theta\).  Because \(\abs{d_i(\theta)}\le B\abs{\theta-\theta_0}\), the true
...
terms by \(O_{\frE}(1)\abs{\theta-\theta_0}\).  Combining these bounds with
...
integral of the displayed global envelope outside \(\abs h>M\) tends to zero
uniformly as \(M\to\infty\).  Uniform tightness of \(Z_{n,E}\) and the
vanishing contrast-failure probability prove \eqref{eq:root-n-contraction}.
\end{proof}

\subsection{A relative random-information Laplace lemma}

The next statement records all estimates needed to normalize a moving random
Gaussian.  It is formulated separately so that no convergence of \(I_n\) or
\(\Delta_n\) is hidden in the proof.

\begin{lemma}[Uniform relative Laplace principle]
\label{lem:random-laplace}
Let \(K_{n,E}(h)\) be nonnegative random functions on
\(H_{n,E}=\sqrt n(\Theta-\theta_0)\).  Suppose:
\begin{enumerate}
\item with probability tending to one uniformly, the eigenvalues of a random
matrix \(J_{n,E}\) lie in \([\underline j,\overline j]\);
\item \(U_{n,E}=O_{\frE}(1)\);
\item for every fixed \(M\),
\[
 \sup_{\abs h\le M}\left|
 \log K_{n,E}(h)-h^TU_{n,E}+\tfrac12h^TJ_{n,E}h
 \right|=o_{\frE}(1);
\]
\item on the same high-probability events,
\[
 K_{n,E}(h)\le
 C_{n,E}\exp(-a\abs h^2+B_{n,E}\abs h),
 \quad C_{n,E},B_{n,E}=O_{\frE}(1);
\]
\item \(r_{n,E}(h)\) is nonnegative, uniformly bounded, and
\(\sup_{\abs h\le M}\abs{r_{n,E}(h)-1}=o(1)\) deterministically for each
fixed \(M\).
\end{enumerate}
Put
\[
 q_{n,E}(h)=\exp(h^TU_{n,E}-\tfrac12h^TJ_{n,E}h),
 \qquad
 Q_{n,E}=\int_{\R^p}q_{n,E}(h)dh.
\]
Then
\begin{equation}
 \frac{\int_{H_{n,E}}
 \abs{K_{n,E}(h)r_{n,E}(h)-q_{n,E}(h)}dh}
 {Q_{n,E}}=o_{\frE}(1).
 \label{eq:relative-L1}
\end{equation}
Consequently the normalized densities converge in total variation.  Moreover
\begin{equation}
 Q_{n,E}=(2\pi)^{p/2}(\det J_{n,E})^{-1/2}
 \exp\{\tfrac12U_{n,E}^TJ_{n,E}^{-1}U_{n,E}\}.
 \label{eq:random-gaussian-normalizer}
\end{equation}
\end{lemma}

\begin{proof}
Fix \(K\) and restrict to the event
\[
 \abs{U_{n,E}}+B_{n,E}+C_{n,E}\le K,
 \qquad
 \underline j\Id\le J_{n,E}\le\overline j\Id.
\]
Its complement is uniformly small after \(K\to\infty\).  Completing the
square shows that \(Q_{n,E}\) is bounded above and below by positive
constants depending only on \(K,\underline j,\overline j,p\), and proves
\eqref{eq:random-gaussian-normalizer}.  On \(\abs h\le M\), exponentiation of
the uniform log error, together with the deterministic prior-ratio error,
gives an \(o_{\frE}(1)\) relative \(L^1\) difference.  Outside that ball,
both \(q_{n,E}\) and the common envelope
\(C\exp(-a\abs h^2+K\abs h)\) have integrals tending to zero uniformly as
\(M\to\infty\).  The scaled domains exhaust \(\R^p\) because
\(\theta_0\) remains in the fixed interior set.  Divide by the lower bound
for \(Q_{n,E}\), then let \(n\to\infty\), \(M\to\infty\), and
\(K\to\infty\).  This proves \eqref{eq:relative-L1}.  If two nonnegative
integrable functions have relative \(L^1\) error tending to zero and one
normalizer is bounded below, their normalized densities have total variation
...
\begin{equation}
 \TV\left(
 \mathcal L\{\sqrt n(\theta-\theta_0)\mid\cF_n\},
 N(I_n^{-1}\Delta_n,I_n^{-1})
 \right)=o_{\frE}(1).
 \label{eq:abstract-bvm}
\end{equation}
On the event where \(I_n\) is singular, define the comparison law
arbitrarily; the probability of that event tends to zero uniformly.  The
marginal evidence \(\mathcal Z_n^{\nu_n}=\int\pi(\theta)
L_n^{\nu_n}(\theta)d\theta\) satisfies
\begin{equation}
 \frac{\mathcal Z_n^{\nu_n}}{L_n^{\nu_n}(\theta_0)}
...
\end{equation}
Neither assertion assumes that \(I_n\) converges.
\end{theorem}

\begin{proof}
Apply Lemma~\ref{lem:random-laplace} with
\[
 K_{n,E}(h)=
 \frac{L_n^{\nu_n}(\theta_0+h/\sqrt n)}
      {L_n^{\nu_n}(\theta_0)},
 \quad J_{n,E}=I_n,\quad U_{n,E}=\Delta_n,\quad
 r_{n,E}(h)=\frac{\pi(\theta_0+h/\sqrt n)}{\pi(\theta_0)}.
\]
The spectral and score hypotheses follow from the discussion after
Lemma~\ref{lem:sobolev-score}; the local and tail hypotheses are
Proposition~\ref{prop:local-global}.  Uniform continuity and the positive
lower bound of \(\pi\) give the prior-ratio hypothesis.  Equation
\eqref{eq:relative-L1} gives \eqref{eq:abstract-bvm} after normalization.
The same relative integral estimate, the Jacobian
\(d\theta=n^{-p/2}dh\), and
\eqref{eq:random-gaussian-normalizer} give
\eqref{eq:abstract-evidence}.
\end{proof}
